...
Процедура ПередЗаписью(Отказ, Замещение, ТолькоЗапись, ЗаписьФактическогоПериодаДействия, ЗаписьПерерасчетов)
	...
	ПреобразоватьСторноСтрокиТекущегоПериода();
	ЗаписатьНаборИсправляемыйВТекущемПериоде();
...
Процедура ПриЗаписи(Отказ, Замещение, ТолькоЗапись, ЗаписьФактическогоПериодаДействия, ЗаписьПерерасчетов)
	...
	Если ТипЗнч(Регистратор) <> Тип("ДокументСсылка.НачислениеЗарплаты")
		...
		Если Не РезультатЗапроса.Пустой() И Не ДополнительныеСвойства.Свойство("ОтключитьРегистрациюПерерасчетов") Тогда
...
Процедура ПреобразоватьСторноСтрокиТекущегоПериода()
	
	Если ДополнительныеСвойства.Свойство("ЕстьПерерасчетыПриИсправленииВТекущемПериоде") Тогда
		РегистрыРасчета.Начисления.ПреобразоватьСторноВСторноТекущегоПериода(
			ЭтотОбъект, ДополнительныеСвойства.ИсправленныйДокумент);
	КонецЕсли;
	
КонецПроцедуры

Процедура ЗаписатьНаборИсправляемыйВТекущемПериоде()
	
	Ключ = "НаборИсправляемыйВТекущемПериоде";
	НаборИсправленный = Неопределено;
	
	Если ДополнительныеСвойства.Свойство(Ключ, НаборИсправленный) Тогда
		...
		ДополнительныеСвойства.Удалить(Ключ);
	КонецЕсли;
