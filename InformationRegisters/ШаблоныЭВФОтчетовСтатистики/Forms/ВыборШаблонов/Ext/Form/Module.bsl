...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ОтборОКУД = Новый Структура("ОКУД");
	ДляДобавления = Новый Массив;
	Для Каждого Стр Из ТаблицаСвойствШаблонов Цикл 
		ЗаполнитьЗначенияСвойств(ОтборОКУД, Стр);
		Если ТаблицаСвойствШаблонов.НайтиСтроки(ОтборОКУД).Количество() = 0 Тогда 
			ДляДобавления.Добавить(Стр);
		КонецЕсли;
	КонецЦикла;
	
	Для Каждого Стр Из ДляДобавления Цикл
		ЗаполнитьЗначенияСвойств(ТаблицаСвойствШаблонов.Добавить(), Стр);
	КонецЦикла;
	...
	|	ВЫРАЗИТЬ(ТаблицаСвойствШаблонов.ИДОбъекта КАК Строка(100)) КАК ИДОбъекта,
	|	ВЫРАЗИТЬ(ТаблицаСвойствШаблонов.ОКУД КАК СТРОКА(20)) КАК ОКУД,
	...
	|	ВЫРАЗИТЬ(ТаблицаСвойствШаблонов.Версия КАК СТРОКА(20)) КАК Версия
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ РАЗРЕШЕННЫЕ
	|	ТаблицаСвойствШаблонов.ИДОбъекта КАК ИДОбъекта,
	|	ТаблицаСвойствШаблонов.ОКУД КАК ОКУД,
	|	ТаблицаСвойствШаблонов.КодШаблона КАК КодШаблона,
	|	ТаблицаСвойствШаблонов.Шифр КАК Шифр,
	|	ТаблицаСвойствШаблонов.Версия КАК Версия,
	|	ШаблоныЭВФОтчетовСтатистики.Версия КАК ВерсияСохраненного
	|ПОМЕСТИТЬ ОбновляемыеШаблоны
	|ИЗ
	...
	|			И ШаблоныЭВФОтчетовСтатистики.ОКУД = ТаблицаСвойствШаблонов.ОКУД
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	...
	|	ТаблицаСвойствШаблонов.Шифр КАК Шифр,
	|	ТаблицаСвойствШаблонов.Версия КАК Версия,
	|	""-"" КАК ВерсияСохраненного
	|ИЗ
	|	РегистрСведений.ШаблоныЭВФОтчетовСтатистики КАК ШаблоныЭВФОтчетовСтатистики
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ТаблицаСвойствШаблонов КАК ТаблицаСвойствШаблонов
	|		ПО ШаблоныЭВФОтчетовСтатистики.ОКУД = ТаблицаСвойствШаблонов.ОКУД
	|ГДЕ
	|	НЕ ТаблицаСвойствШаблонов.КодШаблона В
	|				(ВЫБРАТЬ
	|					ОбновляемыеШаблоны.КодШаблона
	|				ИЗ
	|					ОбновляемыеШаблоны КАК ОбновляемыеШаблоны)
	|
	|ОБЪЕДИНИТЬ
	|
	|ВЫБРАТЬ
	|	ОбновляемыеШаблоны.ИДОбъекта,
	|	ОбновляемыеШаблоны.ОКУД,
	|	ОбновляемыеШаблоны.КодШаблона,
	|	ОбновляемыеШаблоны.Шифр,
	|	ОбновляемыеШаблоны.Версия,
	|	ОбновляемыеШаблоны.ВерсияСохраненного
	|ИЗ
	|	ОбновляемыеШаблоны КАК ОбновляемыеШаблоны";
	...
	Для Каждого СтрокаТаблицыОбновляемыхШаблонов Из ТаблицаОбновляемыхШаблонов Цикл
		...
		СтрокаТаблицыОбновляемыхШаблонов.ИДОбъекта = СокрЛП(СтрокаТаблицыОбновляемыхШаблонов.ИДОбъекта);
		
		КлючШаблона = СокрЛП(СтрокаТаблицыОбновляемыхШаблонов.КодШаблона) + "_" + СокрЛП(СтрокаТаблицыОбновляемыхШаблонов.ОКУД);
