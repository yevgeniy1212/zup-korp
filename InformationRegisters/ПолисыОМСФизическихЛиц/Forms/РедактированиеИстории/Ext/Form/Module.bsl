...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если НЕ Пользователи.РолиДоступны("ДобавлениеИзменениеПолисовОМС") Тогда
	...
	Если Элемент.ТекущиеДанные <> Неопределено Тогда
		...
		Если НоваяСтрока Тогда
			...
			Если НаборЗаписей.Количество() > 1 Тогда
				ЗаписьНабора = НаборЗаписей.Получить(НаборЗаписей.Количество() - 2);
				ПоследнийПериод = ЗаписьНабора.Период;
			Иначе
...
Процедура КомандаОК(Команда)
	...
	Если ПроверитьЗаполнение() Тогда
		РедактированиеПериодическихСведенийКлиент.ОповеститьОЗавершении(ЭтаФорма, "ПолисыОМСФизическихЛиц", ОбъектВладелец);
	КонецЕсли; 
