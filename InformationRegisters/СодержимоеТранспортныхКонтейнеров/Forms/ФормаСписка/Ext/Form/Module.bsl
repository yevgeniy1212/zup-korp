...
Функция ДанныеСтроки(ТранспортноеСообщение, ИмяФайла)
	...
	КонтекстЭДОСервер = ДокументооборотСКО.ПолучитьОбработкуЭДО();
	Выгрузка = КонтекстЭДОСервер.ПолучитьВложенияТранспортногоСообщения(ТранспортноеСообщение, , , ИмяФайла);
...
Процедура КомандаЗагрузить(Команда)
	...
	ДополнительныеПараметры.Вставить("ТранспортноеСообщение", ТекущиеДанные.ТранспортноеСообщение);
...
Процедура КомандаЗагрузить_ПослеВыбораФайла(Результат, ВходящийКонтекст) Экспорт
	...
	Для каждого ОписаниеФайла Из ОписанияФайлов Цикл
		ЗагрузитьФайлыНаСервере(ОписаниеФайла, ВходящийКонтекст);
	КонецЦикла; 
	
КонецПроцедуры

&НаСервереБезКонтекста
Процедура ЗагрузитьФайлыНаСервере(ОписаниеФайла, ВходящийКонтекст)
	
	ДанныеСтроки = ВходящийКонтекст.ДанныеСтроки;
	
	// Удаляем старое
	КонтекстЭДОСервер = ДокументооборотСКО.ПолучитьОбработкуЭДО();
	КонтекстЭДОСервер.УдалитьСодержимоеТранспортногоКонтейнера(
		ВходящийКонтекст.ТранспортноеСообщение, 
	...
	КонтекстЭДОСервер.ДобавитьСодержимоеТранспортногоКонтейнера(
		...
		КонтекстЭДОСервер.ТипСодержимогоПоИмениФайла(ОписаниеФайла.Имя));
