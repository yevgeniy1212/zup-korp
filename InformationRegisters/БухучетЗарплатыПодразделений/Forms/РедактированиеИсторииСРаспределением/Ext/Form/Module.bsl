...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Для каждого Период Из Периоды Цикл
		...
		Если СтрокиНабора.Количество()>1 Тогда
			...
			НоваяСтрокаИстории.КомандаРедактированияРаспределения = ПредставлениеРаспределения(НоваяСтрокаИстории.Распределение, Период);
		Иначе	
	...
	ОтражениеЗарплатыВБухучете.УстановитьУсловноеОформлениеНастройкиБухучетаЕНВД(ЭтаФорма, "ТаблицаИстории");
...
&НаСервере
Функция ПредставлениеРаспределения(ТаблицаРаспределения, Период)
	...
	Если ОтражениеЗарплатыВБухучете.ИспользуетсяЕНВД(Период) Тогда
...
Функция ОшибкиРаспределенияВТаблицеИстории()
	...
	Для каждого СтрокаТЗ Из ТаблицаИстории Цикл
		...
		Если СтрокаТЗ.Распределяется Тогда
			...
			ТаблицаРаспределения = СтрокаТЗ.Распределение;
			ПроверятьЕНВД = ОтражениеЗарплатыВБухучете.ИспользуетсяЕНВД(СтрокаТЗ.Период);
...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	Если ИмяСобытия = "ИзмененоРаспределениеБухучетЗарплаты" И Источник = ЭтаФорма Тогда
		...
		Если НайденныеСтроки.Количество()>0 Тогда
			...
			НайденныеСтроки[0].КомандаРедактированияРаспределения = ПредставлениеРаспределения(НайденныеСтроки[0].Распределение, Параметр.Период);
		КонецЕсли;	
