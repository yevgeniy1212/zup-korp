...
Процедура ЗаполнитьТаблицуМакетовПечатныхФорм()
    ...
    // ЗУПК
	ИсточникиКомандПечатиЗУПК = Новый Массив;
	Для Каждого СтрокаИсточника Из ИсточникиКомандПечати Цикл
		Если Метаданные.ФункциональныеОпции.ОтключенныйФункционалЗарплатаКадры.Состав.Найти(СтрокаИсточника) = Неопределено Тогда
			ИсточникиКомандПечатиЗУПК.Добавить(СтрокаИсточника);
		КонецЕсли;
	КонецЦикла;
	ИсточникиКомандПечати = ИсточникиКомандПечатиЗУПК;
	// ЗУПК
