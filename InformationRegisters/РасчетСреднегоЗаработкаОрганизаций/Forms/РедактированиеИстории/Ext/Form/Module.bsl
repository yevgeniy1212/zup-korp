...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если НЕ ПравоДоступа("Изменение", Метаданные.РегистрыСведений.РасчетСреднегоЗаработкаОрганизаций) Тогда
...
Процедура КомандаОК(Команда)
	РедактированиеПериодическихСведенийКлиент.ОповеститьОЗавершении(ЭтаФорма, "РасчетСреднегоЗаработкаОрганизаций", ОбъектВладелец);
КонецПроцедуры
...
Процедура НаборЗаписейПередОкончаниемРедактирования(Элемент, НоваяСтрока, ОтменаРедактирования, Отказ)
	...
	Если НЕ ОтменаРедактирования Тогда
		Если Элемент.ТекущиеДанные <> Неопределено Тогда
			...
			Если Элемент.ТекущиеДанные.ВариантУчетаКадровыхПерестановок = ПредопределенноеЗначение("Перечисление.ВариантыУчетаКадровыхПерестановок.ПустаяСсылка") Тогда
				
				Если Элемент.ТекущиеДанные.РассчитыватьКоэффициентИндексацииВПределахКадровыхПерестановок Тогда
					СообщениеОбОшибке = НСтр("ru = 'Необходимо указать ""Вариант учета кадровых перестановок"" или снять признак ""Учитывать кадровые перестановки""'");
					ОбщегоНазначенияКлиентСервер.СообщитьПользователю(СообщениеОбОшибке,,"НаборЗаписей.ВариантУчетаКадровыхПерестановок", , Отказ);
				КонецЕсли;
