...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если НЕ ПравоДоступа("Изменение", Метаданные.РегистрыСведений.УчетнаяПолитикаЗарплатаКадры) Тогда
...
Процедура ОбновитьЗначениеНастроекПоПериоду(Форма)
	...
	Если ТекущиеДанные.Период >= '2021-03-01' Тогда
        ТекущиеДанные.ДеньУвольненияЯвляетсяРабочимДнем = Ложь;
    Иначе
    	ТекущиеДанные.ДеньУвольненияЯвляетсяРабочимДнем = Истина;
    КонецЕсли;
...
Процедура КомандаОК(Команда)
	РедактированиеПериодическихСведенийКлиент.ОповеститьОЗавершении(ЭтаФорма, "УчетнаяПолитикаЗарплатаКадры", ОбъектВладелец);
КонецПроцедуры
