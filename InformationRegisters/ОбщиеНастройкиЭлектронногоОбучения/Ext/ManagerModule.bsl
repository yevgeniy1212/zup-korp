...
Функция ОбщийКаталогНастроен(Знач АдресОбщегоКаталога = Неопределено, Знач ПутьКОбщемуКаталогу = Неопределено) Экспорт
	
	Если АдресОбщегоКаталога = Неопределено
		ИЛИ ПутьКОбщемуКаталогу = Неопределено Тогда
	...
	Возврат ?(ЗначениеЗаполнено(АдресОбщегоКаталога) И ЗначениеЗаполнено(ПутьКОбщемуКаталогу), Истина, Ложь)
