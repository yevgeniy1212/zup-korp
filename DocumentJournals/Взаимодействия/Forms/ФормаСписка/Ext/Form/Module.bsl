...
Процедура ВосстановитьРаскрытыеУзлыДерева()
	...
	Для каждого Идентификатор Из ИдентификаторыРазвернутыхУзлов Цикл
		Элементы[Настройки.ИмяДерева].Развернуть(Идентификатор.Значение);
	КонецЦикла;
	
	УдаляемыеУзлы = Новый Массив;
	Для каждого УзелДерева Из РазвернутыеУзлы Цикл
		Если ИдентификаторыРазвернутыхУзлов[УзелДерева.Значение] = Неопределено Тогда
			УдаляемыеУзлы.Добавить(УзелДерева);
		КонецЕсли;
	КонецЦикла;
	
	Для каждого УзелДерева Из УдаляемыеУзлы Цикл
		РазвернутыеУзлы.Удалить(УзелДерева);
	КонецЦикла;

КонецПроцедуры

&НаКлиенте
Процедура ОпределитьИдентификаторыРазвернутыхУзлов(Знач СписокРазвернутыхУзлов, Знач СтрокиДерева, ИдентификаторыУзлов)
