...
Функция ПропущенныйНалоговыйПериод(Вариант, Организация, Знач ДатаРегистрации = Неопределено, Знач ДатаПостановкиНаУчет = Неопределено) Экспорт
	...
	Если Вариант = РегистрацияВДекабре Тогда
		// Закон от 29.09.2019 N 325-ФЗ отменил особые правила определения первого налогового периода для УСН.
		// В законе указано, что изменения действуют с 1 января 2018 года.
		// Однако ретроспективное применение закона ухудшает положение налогоплательщиков, зарегистрированных
		// в декабре 2018 года: по старым правилам они не сдавали декларацию за 2018 год, по новым - обязаны сдать,
		// причем автоматически с нарушением срока. Согласно п.2 ст. 5 НК РФ законы, ухудшающие положение налогоплательщиков,
		// не имеют обратной силы. Поэтому новые правила для УСН применяются только с 2019 года.
		//
		Дата325ФЗ = '20190929';
		Если НачалоПервогоНалоговогоПериода >= Дата325ФЗ
			И РегламентированнаяОтчетность.ПрименяетсяУСН(Организация, НачалоПервогоНалоговогоПериода) Тогда
			Возврат Неопределено;
		КонецЕсли;
