...
Процедура СформироватьТаблицуКалендара(ТребуемыйГод)
	...
	Если ПроверятьИзменения() Тогда
		СкорректироватьДанныеНакопительныхТаблиц(НакопительнаяТаблицаПредставленияОтчетов, НакопительнаяТаблицаСроковУплат);
	КонецЕсли;
...
Функция ПолучитьСледующуюРабочуюДату(СрокСдачи)
	...
	Возврат ГрафикиРаботыСотрудниковЗУПК.ПолучитьСледующуюРабочуюДату(СрокСдачи);
				...
				ИначеЕсли Квартал < 4 Тогда
					...
					ИначеЕсли (Найти(СрокУплатыЕжеквартально, "+1Месяц") > 0)
						...
						Если Квартал = 1 Тогда                                                  							
							...
							НовСтр.ЗаПериодПо = КонецКвартала(НовСтр.ЗаПериодС);
						Иначе							
							НовСтр.ЗаПериодС  = НачалоКвартала(Дата(ТребуемыйГод, (Квартал -1) * 3, 1));
							НовСтр.ЗаПериодПо = КонецКвартала(НовСтр.ЗаПериодС);
						КонецЕсли;
                    // ЗУПК
...
&НаКлиенте
Процедура ПолеТабличногоДокументаНеделяПриАктивизации(Элемент)
...
#Область ПереносСроков
&НаСервере
Функция ПроверятьИзменения()
	Возврат Истина;
КонецФункции

&НаСервере
Процедура СкорректироватьДанныеНакопительныхТаблиц(НакопительнаяТаблицаПредставленияОтчетов, НакопительнаяТаблицаСроковУплат)
	КолонкиСтр = Новый Структура("Налог,ЗаПериодС,ЗаПериодПо,ДатаПредставления,Мес,НоваяДата");
	ТаблицаПереносаПредставлений = ПолучитьТаблицуИзменений("СрокиПредставления", КолонкиСтр,);
	КолонкиСтр = Новый Структура("Налог,ЗаПериодС,ЗаПериодПо,ДатаУплаты,Мес,НоваяДатаУплаты");
	ТаблицаПереносаУплат = ПолучитьТаблицуИзменений("СрокиУплаты", КолонкиСтр,);
	Для Каждого СтрокаКорректировки Из ТаблицаПереносаПредставлений Цикл
		ДатаПредставления = ПолучитьСледующуюРабочуюДату(СтрокаКорректировки.ДатаПредставления);
		НоваяДата = ПолучитьСледующуюРабочуюДату(СтрокаКорректировки.НоваяДата);
		СтруктураПоиска = Новый Структура("Объект,ДатаПредставленияОтчета,ЗаПериодС,ЗаПериодПо", СтрокаКорректировки.Налог,
			ДатаПредставления, СтрокаКорректировки.ЗаПериодС, СтрокаКорректировки.ЗаПериодПо);
		НайденныеСтроки = НакопительнаяТаблицаПредставленияОтчетов.НайтиСтроки(СтруктураПоиска);
		Для Каждого НайденнаяСтрока Из НайденныеСтроки Цикл
			НайденнаяСтрока.ПоследняяДатаПредставленияОтчета = НоваяДата;
			НайденнаяСтрока.ДатаПредставленияОтчета = НоваяДата;
			НайденнаяСтрока.КтоСдаетОтчет = НайденнаяСтрока.КтоСдаетОтчет + Символы.ПС + "*(Срок представления перенесен с "
				+ Формат(ДатаПредставления, "ДФ=dd.MM.yyyy") + " " + СокрЛП(СтрокаКорректировки.Мес) + ")";
		КонецЦикла;
	КонецЦикла;
	Для Каждого СтрокаКорректировки Из ТаблицаПереносаУплат Цикл
		ДатаУплаты = ПолучитьСледующуюРабочуюДату(СтрокаКорректировки.ДатаУплаты);
		НоваяДатаУплаты = ПолучитьСледующуюРабочуюДату(СтрокаКорректировки.НоваяДатаУплаты);
		СтруктураПоиска = Новый Структура("НалогПолноеНазв,СрокУплаты,ЗаПериодС,ЗаПериодПо", СтрокаКорректировки.Налог,
			ДатаУплаты, СтрокаКорректировки.ЗаПериодС, СтрокаКорректировки.ЗаПериодПо);
		НайденныеСтроки = НакопительнаяТаблицаСроковУплат.НайтиСтроки(СтруктураПоиска);
		Для Каждого НайденнаяСтрока Из НайденныеСтроки Цикл
			НоваяСтрока = НакопительнаяТаблицаСроковУплат.Добавить();
			ЗаполнитьЗначенияСвойств(НоваяСтрока, НайденнаяСтрока);
			НоваяСтрока.Кто = НоваяСтрока.Кто + Символы.ПС + "*(Для субъектов МСП пострадавших отраслей срок уплаты перенесен с "
				+ Формат(ДатаУплаты, "ДФ=dd.MM.yyyy") + " " + СокрЛП(СтрокаКорректировки.Мес) + ")";
			НоваяСтрока.СрокУплаты = НоваяДатаУплаты;
		КонецЦикла;
	КонецЦикла;
КонецПроцедуры

&НаСервере
Функция ПолучитьТаблицуИзменений(НазваниеОбласти, КолонкиСтр, РазмерСтрок = 100)
	МакетИзменений = Обработки.КалендарьБухгалтера.ПолучитьМакет("Изменения");
	ОбластьМакетаИзменений = МакетИзменений.Области.Найти(НазваниеОбласти);
	ТаблРезультат = Новый ТаблицаЗначений;
	Если РазмерСтрок = 100 Тогда
		Строка100 = ПолучитьОписаниеТиповСтроки(100);
	Иначе
		Строка100 = ПолучитьОписаниеТиповСтроки(РазмерСтрок);
	КонецЕсли;
	ТипДата = ПолучитьОписаниеТиповДаты(ЧастиДаты.Дата);
	Для Каждого Зап Из КолонкиСтр Цикл
		Если ПривестиКТипуДата(Зап.Ключ) Тогда
			ТаблРезультат.Колонки.Добавить(Зап.Ключ, ТипДата);
		Иначе
			ТаблРезультат.Колонки.Добавить(Зап.Ключ, Строка100);
		КонецЕсли;
	КонецЦикла;
	Для Ном = ОбластьМакетаИзменений.Верх По ОбластьМакетаИзменений.Низ Цикл
		НовСтр = ТаблРезультат.Добавить();
		НомерКолонкиВМакете = 0;
		Для Каждого Зап Из ТаблРезультат.Колонки Цикл
			НомерКолонкиВМакете = НомерКолонкиВМакете + 1;
			Сведение = МакетИзменений.Область(Ном, НомерКолонкиВМакете).Текст;
			Если (НомерКолонкиВМакете = 1) И (ПустаяСтрока(Сведение)) Тогда
				ТаблРезультат.Удалить(НовСтр);
				Прервать;
			КонецЕсли;
			Если ПривестиКТипуДата(Зап.Имя)Тогда
				НовСтр[Зап.Имя] = ПеревестиВДату(Сведение);
			Иначе
				НовСтр[Зап.Имя] = Сведение;
			КонецЕсли;
		КонецЦикла;
...
&НаСервере
Функция ПривестиКТипуДата(ИмяЭлемента)
	Если Найти(ИмяЭлемента, "Период") > 0 Тогда
		Возврат Истина;
	ИначеЕсли Найти(ИмяЭлемента, "Дата") > 0 Тогда
		Возврат Истина;
	Иначе
		Возврат Ложь;
	КонецЕсли;
...
&НаСервере
Функция ПеревестиВДату(Сведение)
	Возврат СтроковыеФункцииКлиентСервер.СтрокаВДату(Сведение);
КонецФункции
...
Функция ПолучитьОписаниеТиповДаты(ЧастиДаты)
	...
	Возврат Новый ОписаниеТипов(Массив, , , Новый КвалификаторыДаты(ЧастиДаты));
КонецФункции
...
Функция ПолучитьОписаниеТиповСтроки(ДлинаСтроки)
	...
	Возврат Новый ОписаниеТипов(Массив, , Новый КвалификаторыСтроки(ДлинаСтроки, ДопустимаяДлина.Переменная));
КонецФункции 
#КонецОбласти