...
Процедура ПриДобавленииКлассификаторов(Классификаторы) Экспорт
    ...
    // ЗУПК
	Описание.Наименование = НСтр("ru = 'Классификатор валют'");
    // ЗУПК
...
Процедура ДобавитьРазрешения(Разрешения) Экспорт
	...
	Если ИспользоватьАльтернативныйСервер Тогда
		Протокол = "HTTPS";
		Адрес = "nationalbank.kz";
		Порт = Неопределено;
		Описание = НСтр("ru = 'Загрузка курсов валют с сайта Национального банка РК.'");
		Разрешения.Добавить( 
...
Функция ЗагрузитьКурсыВалютПоПараметрам(Знач Валюты, Знач НачалоПериодаЗагрузки, Знач ОкончаниеПериодаЗагрузки, ПриЗагрузкеВозниклиОшибки = Ложь)
    ...
    // УТК
	Если Истина Тогда
		Возврат ЗагрузитьКурсыВалютПоПараметрамРК(Валюты, НачалоПериодаЗагрузки, ОкончаниеПериодаЗагрузки, ПриЗагрузкеВозниклиОшибки);
	КонецЕсли;
   	// УТК
   	// ЗУПК
...
//
Функция ЗагрузитьКурсыВалютПоПараметрамРК(Знач Валюты, Знач НачалоПериодаЗагрузки, Знач ОкончаниеПериодаЗагрузки, ПриЗагрузкеВозниклиОшибки = Ложь)
	
	СостояниеЗагрузки = Новый Массив;
	
	СерверИсточник = "www.nationalbank.kz";
	
	Адрес = "rss/get_rates.cfm?fdate=";
	
	ТекущийПериодЗагрузки = НачалоПериодаЗагрузки;
	
	Пока ТекущийПериодЗагрузки <= ОкончаниеПериодаЗагрузки Цикл
		
		ТМП = Формат(ТекущийПериодЗагрузки, "ДФ=dd.MM.yyyy");
		
		ФайлНаВебСервере = "https://" + СерверИсточник + "/" + Адрес + ТМП;  ВалютыЗагружаемыеИзИнтернета = ВалютыЗагружаемыеИзИнтернета();
		
		Результат = ПолучениеФайловИзИнтернета.СкачатьФайлНаСервере(ФайлНаВебСервере);
		Если Результат.Статус Тогда
			ПоясняющееСообщение = ЗагрузитьКурсВалютыИзФайлаРК(Валюты, Результат.Путь, ТекущийПериодЗагрузки, Неопределено) + Символы.ПС;
			УдалитьФайлы(Результат.Путь);
			СтатусОперации = ПустаяСтрока(ПоясняющееСообщение);
		Иначе
			ПоясняющееСообщение = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		...
		СостояниеЗагрузки.Добавить(Новый Структура("Дата,СтатусОперации,Сообщение", ТекущийПериодЗагрузки, СтатусОперации, ПоясняющееСообщение));
		
		ТекущийПериодЗагрузки = ТекущийПериодЗагрузки + 86400;
...
// только те данные, которые удовлетворяют периоду (НачалоПериодаЗагрузки, ОкончаниеПериодаЗагрузки).
//
Функция ЗагрузитьКурсВалютыИзФайлаРК(Знач Валюты, Знач ПутьКФайлу, Знач НачалоПериодаЗагрузки, Знач ОкончаниеПериодаЗагрузки)
	...
	ЧислоЗагружаемыхВалютВсего = Валюты.Количество();
	ЧислоЗагруженныхВалют = 0;
	
	ВалютыЗагруженные = Новый Массив;
	...
	Если ОбъектXDTO.Свойства().Получить("item") <> Неопределено Тогда
		СписокXDTO = ОбъектXDTO.item;
		ТаблицаКурсовВалют = Новый ТаблицаЗначений;
		ТаблицаКурсовВалют.Колонки.Добавить("title");
		ТаблицаКурсовВалют.Колонки.Добавить("description");
		ТаблицаКурсовВалют.Колонки.Добавить("quant");
		Для Каждого СтрокаXDTO Из СписокXDTO Цикл
			ЗаполнитьЗначенияСвойств(ТаблицаКурсовВалют.Добавить(), СтрокаXDTO);
		КонецЦикла;
		
		НаборЗаписей = РегистрыСведений.КурсыВалют.СоздатьНаборЗаписей();
		НаборЗаписей.Отбор.Период.Установить(ТекущийПериодЗагрузки);
		...
		Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.ДатыЗапретаИзменения") Тогда
			...
			Если МодульДатыЗапретаИзмененияСлужебный.ЗапретИзмененияПроверяется(Метаданные.РегистрыСведений.КурсыВалют) Тогда
				...
				Если Не Записывать Тогда
					Если ДатаЗапрета = Неопределено Тогда
						ДатаЗапрета = ТекущийПериодЗагрузки;
					Иначе
						ДатаЗапрета = Макс(ДатаЗапрета, ТекущийПериодЗагрузки);
					КонецЕсли;
				КонецЕсли;
			КонецЕсли;
		КонецЕсли;
		...
		Если Записывать Тогда
			Для Каждого Валюта Из Валюты Цикл
				НайденнаяСтрокаКурса = ТаблицаКурсовВалют.Найти(Валюта.Валюта.Наименование, "title");
				Если НайденнаяСтрокаКурса <> Неопределено Тогда
					НаборЗаписей.Отбор.Валюта.Установить(Валюта.Валюта);
					
					Запись = НаборЗаписей.Добавить();
					Запись.Валюта = Валюта.Валюта;
					Запись.Период = ТекущийПериодЗагрузки;
					Запись.Курс = Число(НайденнаяСтрокаКурса.description);
					Запись.Кратность = Число(НайденнаяСтрокаКурса.quant);
					
					НаборЗаписей.Записать();
					
					//Удаляем текущую запись, чтобы она не попадала в следующую итерацию
					НаборЗаписей.Удалить(0);
					
					ВалютыЗагруженные.Добавить(Валюта);
					
					ЧислоЗагруженныхВалют = ЧислоЗагруженныхВалют + 1;
				КонецЕсли;
			КонецЦикла;	
		КонецЕсли;
		
	ИначеЕсли ОбъектXDTO.Свойства().Получить("info") <> Неопределено Тогда
		ДополнительнаяИнформация = ОбъектXDTO.info;
	КонецЕсли;
	...
	Если ЧислоЗагружаемыхВалютВсего = ЧислоЗагруженныхВалют Тогда
		ПояснениеОЗагрузке = "";
	ИначеЕсли ЧислоЗагруженныхВалют = 0 Тогда
		ПояснениеОЗагрузке = НСтр("ru = 'Курсы валют, на дату %2, не загружены. Дополнительная информация сервера: %1'");
	Иначе
		ПояснениеОЗагрузке = НСтр("ru = 'На дату %2, не удалось загрузить курс следующих валют: %3'");
	КонецЕсли;
	
	СписокВалют = "";
	Для Каждого Валюта Из Валюты Цикл
		Если ВалютыЗагруженные.Найти(Валюта) = Неопределено Тогда
			Если ЗначениеЗаполнено(СписокВалют) Тогда
				СписокВалют = СписокВалют + ", ";
			КонецЕсли;
			СписокВалют = СписокВалют + Валюта.Валюта.Наименование + "(" + Валюта.Валюта.НаименованиеПолное + ")";
		КонецЕсли;
	КонецЦикла;
	...
	ПояснениеОЗагрузке = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
									ПояснениеОЗагрузке,
									ДополнительнаяИнформация,
									Формат(ТекущийПериодЗагрузки, "ДЛФ=ДД"),
									СписокВалют);
	
	Если ДатаЗапрета <> Неопределено Тогда
		ПояснениеОЗагрузке = ПояснениеОЗагрузке + Символы.ПС + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Загрузка курсов валют ограничена датой запрета изменений %1.
			|Курсы запрещенного периода были пропущены при загрузке.'"), Формат(ДатаЗапрета, "ДЛФ=D"));
	КонецЕсли;
...
Функция КодыВалютЗагружаемыхИзИнтернета() Экспорт
	...
	// ЗУПК
	Если ТипЗнч(КлассификаторТаблица) = Тип("Массив")
		И КлассификаторТаблица.Количество() <> 0 Тогда 
		НоваяТаблицаКлассификатора = Новый ТаблицаЗначений;
		
		ПерваяСтруктура = КлассификаторТаблица[0];
		Для Каждого Свойство Из ПерваяСтруктура Цикл
			НоваяТаблицаКлассификатора.Колонки.Добавить(Свойство.Ключ);
		КонецЦикла;
		
		Для Каждого Структура Из КлассификаторТаблица Цикл
			НоваяСтрока = НоваяТаблицаКлассификатора.Добавить();
			ЗаполнитьЗначенияСвойств(НоваяСтрока, Структура);
		КонецЦикла;
		
		НоваяТаблицаКлассификатора.Индексы.Добавить("currencyCode");
		НоваяТаблицаКлассификатора.Индексы.Добавить("downloadFromInternet");
		НайденныеСтроки = НоваяТаблицаКлассификатора.НайтиСтроки(Новый Структура("downloadFromInternet", "истина"));
		ЗагружаемыеПоКлассификатору = НоваяТаблицаКлассификатора.Скопировать(НайденныеСтроки, "currencyCode").ВыгрузитьКолонку("currencyCode");
	Иначе
...
Функция КлассификаторВалют() Экспорт
	...
	Иначе
		...
		Результат.Колонки.Добавить("Code", Новый ОписаниеТипов("Строка", ,Новый КвалификаторыСтроки(3)));
		Результат.Колонки.Добавить("CodeSymbol", Новый ОписаниеТипов("Строка", ,Новый КвалификаторыСтроки(3)));
		Результат.Колонки.Добавить("Name", Новый ОписаниеТипов("Строка", ,Новый КвалификаторыСтроки(50)));
		Результат.Колонки.Добавить("Description", Новый ОписаниеТипов("Строка", ,Новый КвалификаторыСтроки(50)));
		Результат.Колонки.Добавить("RBCLoading", Новый ОписаниеТипов("Булево"));
		Результат.Колонки.Добавить("NumerationItemOptions");
		
		Для Каждого СтрокаКлассификатор Из КлассификаторJSON Цикл
			
			СтрокаТаблицаСтран = Результат.Добавить();
			
			СтрокаТаблицаСтран.Code						 = ?(СтрокаКлассификатор.Свойство("currencyCode"),
															 СтрокаКлассификатор.currencyCode, "");
			СтрокаТаблицаСтран.CodeSymbol				 = ?(СтрокаКлассификатор.Свойство("currencyLetter"),
															 СтрокаКлассификатор.currencyLetter, "");
			СтрокаТаблицаСтран.Name						 = ?(СтрокаКлассификатор.Свойство("currencyName"),
															 СтрокаКлассификатор.currencyName, "");
			СтрокаТаблицаСтран.Description				 = "";
			СтрокаТаблицаСтран.RBCLoading				 = ?(СтрокаКлассификатор.Свойство("downloadFromInternet"),
															 СтрокаКлассификатор.downloadFromInternet, "");
			СтрокаТаблицаСтран.NumerationItemOptions	 = ?(СтрокаКлассификатор.Свойство("numerationItemOptions"),
															 СтрокаКлассификатор.numerationItemOptions, Ложь);
