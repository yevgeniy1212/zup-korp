...
Процедура ПриОткрытии(Отказ)
	Если СтрНачинаетсяС(ЭтаФорма.ВладелецФормы.ИмяФормы, "Отчет.РегламентированныйОтчетСтатистика")
		Или СтрНачинаетсяС(ЭтаФорма.ВладелецФормы.ИмяФормы, "ВнешнийОтчет.РегламентированныйОтчетСтатистика") Тогда 
...
#Область ФункционалОсновногоОкна
...
Процедура Обновить(Команда)
	...
	ОповещениеОЗавершении = Новый ОписаниеОповещения("ОбновитьЗавершение", ЭтотОбъект, 
		ДополнительныеПараметры);
	
	Если РезультатПроверки.Свойство("Статус") Тогда
		ПараметрыОжидания = ДлительныеОперацииКлиент.ПараметрыОжидания(ЭтаФорма.ВладелецФормы);
	...
	Иначе
		АдресРезультата = ПоместитьВоВременноеХранилище(РезультатПроверки, ЭтаФорма.ВладелецФормы.УникальныйИдентификатор);
...
&НаКлиенте
Процедура ПолеТабличногоДокументаКСПриАктивизации(Элемент)
	
	Если Элементы.Группа2.Видимость
		И ТипЗнч(ПолеТабличногоДокументаКС.ВыделенныеОбласти) = Тип("ВыделенныеОбластиТабличногоДокумента") 
		И Не ПолеТабличногоДокументаКС.ТекущаяОбласть.Расшифровка = Неопределено
		Тогда
		ИнтервалОжидания = ?(ПолучитьСкоростьКлиентскогоСоединения() = СкоростьКлиентскогоСоединения.Низкая, 1, 0.5);
		ПодключитьОбработчикОжидания("Подключаемый_ОбновитьРасшифровкуПриАктивизации", ИнтервалОжидания, Истина);
	КонецЕсли;
	
КонецПроцедуры

&НаКлиенте
Процедура Подключаемый_ОбновитьРасшифровкуПриАктивизации()
	
	Элемент = ПолеТабличногоДокументаКС;
	ТекущееПолеТабличногоДокументаКС = Элемент.ТекущаяОбласть;
	Если НЕ Элемент.ТекущаяОбласть.Расшифровка = Неопределено Тогда
		Показать(Элемент.ТекущаяОбласть.Расшифровка);
	КонецЕсли;
