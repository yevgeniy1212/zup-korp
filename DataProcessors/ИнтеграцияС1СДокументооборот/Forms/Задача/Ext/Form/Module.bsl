...
Функция ПолучитьЗадачиБизнесПроцесса(Прокси)
	...
	ОбъектXDTO.objectID = ИнтеграцияС1СДокументооборотБазоваяФункциональность.СоздатьObjectID(Прокси, ПроцессID, ПроцессТип);
...
Процедура ЗаполнитьXDTOПоОбъектномуРеквизиту(Прокси, ИмяРеквизита, ИмяСвойстваОбъекта, ОбъектXDTO)
	...
	СвойствоОбъекта.ObjectID = ИнтеграцияС1СДокументооборотБазоваяФункциональность.СоздатьObjectID(Прокси,
...
Функция ЗаписатьОбъект(РезультатВыполненияЗадачи = Неопределено)
	...
	ПроцессXDTO.objectID = ИнтеграцияС1СДокументооборотБазоваяФункциональность.СоздатьObjectID(Прокси, ПроцессID, ПроцессТип);
	...
	Если РезультатВыполненияЗадачи <> Неопределено Тогда
		...
		Если РезультатВыполненияЗадачи.ВыбранныйИсполнитель = "currentUser" Тогда
			Обработки.ИнтеграцияС1СДокументооборот.СоздатьУчастникаБизнесПроцесса(
				Прокси, Задача.performer, ЭтотОбъект, "ТекущийПользователь", "ОбъектАдресации");
		Иначе
			Обработки.ИнтеграцияС1СДокументооборот.СоздатьУчастникаБизнесПроцесса(
				Прокси, Задача.performer, ЭтотОбъект, "Исполнитель", "ОбъектАдресации");
		КонецЕсли;
