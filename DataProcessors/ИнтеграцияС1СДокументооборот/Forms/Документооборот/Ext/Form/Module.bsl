...
Процедура ЗагрузитьДеревоБизнесПроцессовИЗадач()
	...
	Target = ИнтеграцияС1СДокументооборотБазоваяФункциональность.СоздатьОбъект(Прокси, "ExternalObject");
	...
	СписокВнешнихОбъектов.Добавить(Target);
...
Процедура СкопироватьСуществующийФайлВПапку(ИдентификаторФайла)
	...
	Запрос.file.externalObject = ИнтеграцияС1СДокументооборотБазоваяФункциональность.СоздатьОбъект(
		Прокси,
		"ExternalObject");
...
Функция СоздатьФайлВПапкеИзФайлаНаДиске(ПараметрыСоздания)
	...
	Запрос.file.externalObject = ИнтеграцияС1СДокументооборотБазоваяФункциональность.СоздатьОбъект(Прокси, "ExternalObject");
	Запрос.file.externalObject.name = Строка(ВнешнийОбъект);
	Запрос.file.externalObject.ID = Строка(ВнешнийОбъект.УникальныйИдентификатор());
	Запрос.file.externalObject.type = ВнешнийОбъект.Метаданные().ПолноеИмя();
