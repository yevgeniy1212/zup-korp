...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Параметры.Свойство("ДанныеПоЗадаче") Тогда
		...
		Заголовок = Параметры.ДанныеПоЗадаче.Заголовок;
...
Процедура ВыполнитьДействиеЗадачиНаКлиентеВФорме(РезультатВариантаНомер)
	...
	ИнтеграцияС1СДокументооборотБазоваяФункциональностьКлиент.ВыполнитьЗапросАсинхронно(
		ЭтотОбъект,
		НачатьВыполнениеДействияЗадачиНаСервере(РезультатДействия),
		Новый ОписаниеОповещения("ВыполнитьДействиеЗадачиНаКлиентеВФормеЗавершение", ЭтотОбъект),
		Истина,
...
Процедура ВыполнитьДействиеЗадачиНаКлиентеВФормеЗавершение(Результат, ПараметрыОповещения) Экспорт
	...
	Если Результат.Статус = "Выполнено" Тогда
		...
		ЧтениеXML.УстановитьСтроку(Результат.РезультатДлительнойОперации);
		ЧтениеXML.Прочитать();
		Если ЧтениеXML.Имя = "DMError" Тогда
			ЧтениеXML.Прочитать();
			ЧтениеXML.Прочитать();
			ПоказатьПредупреждение(, СокрЛП(ЧтениеXML.Значение));
		Иначе
			ПараметрыОповещения = Новый Структура;
			ПараметрыОповещения.Вставить("ID", ID);
			ПараметрыОповещения.Вставить("Тип", Тип);
...
Процедура ОбновитьФормуЗавершение(Результат, ПараметрыОповещения) Экспорт
	...
	Если Результат.Статус = "Выполнено" Тогда
		...
		ОбновитьФормуЗавершениеНаСервере(Результат.РезультатДлительнойОперации);
		ИнтеграцияС1СДокументооборот3Клиент.РазвернутьДеревоПриложений(ЭтотОбъект, СостояниеДереваПриложений);
...
&НаСервере
Функция НачатьВыполнениеДействияЗадачиНаСервере(РезультатДействия)
	...
	Попытка
		...
		Запрос = ИнтеграцияС1СДокументооборот3.ВыполнитьДействиеЗадачиЗапрос(
			Прокси,
			ДействиеЗадачи,
			РезультатДействия,
...
&НаСервере
Процедура ОбновитьФормуЗавершениеНаСервере(ЗадачаXDTOСтрока)
