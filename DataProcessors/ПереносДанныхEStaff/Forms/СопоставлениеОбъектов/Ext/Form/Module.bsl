...
Процедура ПроверитьОднофамильцев(СтандартнаяОбработка)
	...
	ПараметрыПоиска.Ссылка = ТекущиеДанные.ОбъектСсылка;
	ПараметрыПоиска.Фамилия = СтруктураФИО.Фамилия;
	ПараметрыПоиска.Имя = СтруктураФИО.Имя;
	ПараметрыПоиска.Отчество = СтруктураФИО.Отчество;
	
	ПараметрыПроверки = СотрудникиВызовСервера.ПодобратьСписокФизЛиц(ПараметрыПоиска);
	...
	Оповещение = Новый ОписаниеОповещения("ПроверкаОднофамильцевЗавершение", ЭтотОбъект,
		Новый Структура("ТекущаяСтрока", Элементы.ТаблицаСоответствия.ТекущаяСтрока));
	ОткрытьФорму(
