...
Функция ПолучитьДанныеОНачальныхНастройках()
	...
	// ЗУПК
	|	ЕСТЬNULL(УчетнаяПолитикаПоНалоговомуУчету.ПлательщикОППВ, ЛОЖЬ) КАК ПлательщикОППВ,
	|	ЕСТЬNULL(НастройкиВзаиморасчетовПоПрочимДоходам.ИспользоватьВедомостиДляВыплатыПрочихДоходов, ЛОЖЬ) КАК ИспользоватьВедомостиДляВыплатыПрочихДоходов,
	// ЗУПК
	...
	// ЗУПК
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.УчетнаяПолитикаПоНалоговомуУчету.СрезПоследних(, ) КАК УчетнаяПолитикаПоНалоговомуУчету
	|		ПО Организации.Ссылка = УчетнаяПолитикаПоНалоговомуУчету.Организация
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.НастройкиВзаиморасчетовПоПрочимДоходам КАК НастройкиВзаиморасчетовПоПрочимДоходам
	|		ПО ИСТИНА
	// ЗУПК
	|
	|УПОРЯДОЧИТЬ ПО
...
Процедура ЗаписатьНастройки(Параметры, АдресХранилища) Экспорт
	...
	Если Не ПараметрыПрограммы.ИспользоватьНесколькоОрганизаций Тогда
		...
		Если Организация <> Неопределено Тогда
			...
			НаборЗаписей.Отбор.Организация.Установить(Организация);
			ЗаполнитьЗначенияСвойств(НаборЗаписей.Добавить(), ПараметрыОрганизации.УчетнаяПолитикаПоНалоговомуУчету);
			НаборЗаписей[0].Организация = Организация;
			Если Не ЗначениеЗаполнено(НаборЗаписей[0].Период) Тогда
				НаборЗаписей[0].Период = Дата(2010,1,1);
			КонецЕсли;
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("ЗарплатаКадрыПриложения.Медицина") Тогда
		...
		МодульМедицина.ЗаписатьНастройкиНачальнаяНастройкаПрограммы(Параметры);
	КонецЕсли;
	...
	ЗаполнитьЗначенияСвойств(НаборЗаписей.Добавить(), ПараметрыОрганизации.НастройкиВзаиморасчетовПоПрочимДоходам);
	НаборЗаписей.Записать();
	// ЗУПК		
