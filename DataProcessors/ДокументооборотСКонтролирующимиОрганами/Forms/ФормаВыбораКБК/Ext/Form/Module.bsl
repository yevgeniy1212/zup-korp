...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Элементы.ВидОтображения.Видимость = 
		НЕ ЗапретитьВыборКБК
		И ТипСверки <> Перечисления.ТипыСверокИОН.ВЦеломПоОрганизации;
		
	Элементы.ОчиститьПоле.Видимость = Параметры.Свойство("ВыборИзФормыУведомления");
...
Процедура ОчиститьПоле(Команда)
	Закрыть(Новый Структура("Значение, Наименование"));
КонецПроцедуры
...
Процедура ВернутьРезультат()
	...
	Иначе
		...
		// Запрещен выбор налога для типа "В целом по организации"
		Если ТипСверки = ПредопределенноеЗначение("Перечисление.ТипыСверокИОН.ВЦеломПоОрганизации")
			И НЕ СтроковыеФункцииКлиентСервер.ТолькоЦифрыВСтроке(ТекущиеДанные.Значение, , Ложь) Тогда
			Возврат;
		КонецЕсли;
...
Процедура ОпределитьВидОтображенияПриСозданииНаСервере()
	...
	Если ЗапретитьВыборКБК Тогда
		...
		ЗапомнитьВидОтображения();
	ИначеЕсли ТипСверки = Перечисления.ТипыСверокИОН.ВЦеломПоОрганизации Тогда
		ВидОтображения = "НалогИКБК";
		ЗапомнитьВидОтображения();
	Иначе
