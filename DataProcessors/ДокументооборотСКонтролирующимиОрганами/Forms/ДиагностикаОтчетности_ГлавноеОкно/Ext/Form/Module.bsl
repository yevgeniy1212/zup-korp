...
Функция ПрименитьСтиль(Объект, ИмяМакета="ДиагностикаОтчетности_Стили")
	...
	Обработка = ДокументооборотСКО.ПолучитьОбработкуЭДО();
	...
	МассивВсехТипов.Добавить("ГруппаФормы");
	МассивВсехТипов.Добавить("ДекорацияФормы");
	МассивВсехТипов.Добавить("КнопкаФормы");
	МассивВсехТипов.Добавить("ТаблицаФормы");
	МассивВсехТипов.Добавить("ПолеФормы");
	
	ВсеТипы = Новый Соответствие;
	Для каждого ЭлементМассиваТипов Из МассивВсехТипов Цикл
		ВсеТипы.Вставить(Тип(ЭлементМассиваТипов), ЭлементМассиваТипов);
	КонецЦикла;
...
Процедура ЗаполнитьСводку(РезультатыДиагностики, ВсегоОшибок, ВсегоПредупреждений)
	...
	ИначеЕсли НЕ РезультатыДиагностики.Отчет.Проверки.Свойство("ПроверкаКриптоопераций") Тогда
		Если КриптографияЭДКОКлиентСервер.ЭтоПодписьСервиса(РезультатыДиагностики.Параметры) Тогда
			СсылкаГруппы = ДобавитьНадпись("Проверка криптоопераций не производится при ключах в программе", 1);
			ВсегоПредупреждений = ВсегоПредупреждений + 1;
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Для Каждого Эл Из Элементы.Организация.СписокВыбора Цикл
		МестоХраненияКлюча = КриптографияЭДКОКлиентСервер.КонтекстМоделиХраненияКлюча(Эл.Значение.Ссылка);
		...
		ИначеЕсли КриптографияЭДКОКлиентСервер.ЭтоОблачнаяПодпись(МестоХраненияКлюча) Тогда
			Эл.Представление = Строка(Эл.Значение) + ", 1С:DSS";
		КонецЕсли;
	КонецЦикла;
...
Процедура ОбновитьЭлементыПриИзмененииОрганизации()
	...
	Если ЗначениеЗаполнено(ОсновнаяУчетнаяЗапись) И КриптографияЭДКОКлиентСервер.ЭтоПодписьСервиса(ОсновнаяУчетнаяЗапись) Тогда
...
Функция ЗаполнитьОбщиеПараметры(ПараметрыСервера)
	...
	КриптографияЭДКОКлиентСервер.КонтекстМоделиХраненияКлюча(Организация.УчетнаяЗаписьОбмена, ПараметрыСервера); 
...
Функция ПоместитьИнформациюОбУчетнойЗаписиВОтчет()
	...
	ОрганизацияКлиент = РезультатыДиагностики.Параметры.Организация;
	УчетнаяЗаписьОбменаКлиент = РезультатыДиагностики.Параметры.УчетнаяЗаписьОбмена;
	МестоХраненияКлюча = КриптографияЭДКОКлиентСервер.КонтекстМоделиХраненияКлюча(РезультатыДиагностики.Параметры);
	...
	РезультатыДиагностики.Отчет.Абонент.УчетнаяЗапись.Вставить("МодельХраненияЗакрытогоКлюча", КриптографияЭДКОКлиентСервер.МодельХраненияЗакрытогоКлюча(МестоХраненияКлюча));
...
Функция ПолучитьИнформациюФайлаАвтонастройки()
	...
	ПараметрыФункции.Вставить("ДействиеОблачнойПодписи", "");
	ПараметрыФункции.Вставить("ОблачныйСертификат", Неопределено);
	ПараметрыФункции.Вставить("СерверОблачнойПодписи", "");
...
Процедура НачатьВызовPKCS7ПолучитьСвойстваПолучателейЗавершение(Результат, ПараметрыВызова, ВхПараметры) Экспорт
	...
	Если ТипЗнч(Результат) = Тип("Строка") Тогда
		РезультатПриведенный = СтрЗаменить(Результат, "\u0000", ""); // обход ошибки компоненты
		СвойстваПолучателей = JSONParse(РезультатПриведенный);
	...
	Для Каждого ВременныйФайл Из ПараметрыВызова Цикл 
		...
		Если НЕ ЭтоВеб И НЕ ЭтоLinuxИлиMacOS Тогда
		...
		Иначе
			...
			Если Доверенность.ПолномочияПредставителя.Количество() = 0 Тогда
				Если ТипЗнч(Доверенность) <> Тип("СправочникСсылка.МашиночитаемыеДоверенностиФНС")
					И ТипЗнч(Доверенность) <> Тип("СправочникСсылка.МашиночитаемыеДоверенностиРаспределенныйРеестр") Тогда
...
Функция ПолучитьДанныеОрганизацииСервер(Организация) Экспорт
	...
	Если ЭтоЮрЛицо Тогда 
		...
		СтруктураДанныхОрганизации.Вставить("КрупнейшийНалогоплательщик", ЭтоКрупнейший И НЕ ПустаяСтрока(КодНОКрупнейшего));
	Иначе
...
Функция ПолучитьСписокИзменившихсяРеквизитовПодключенияК1СОтчетности(ОрганизацияСсылка, УчетнаяЗаписьОбменаСсылка)
	...
	ЭтоЮрЛицо = РегламентированнаяОтчетностьПереопределяемый.ЭтоЮридическоеЛицо(ОрганизацияСсылка);
	Если ЭтоЮрЛицо Тогда
...
Функция ВыполнитьПроверкуПодключений() //2
	...
	Адрес = КонтекстЭДОКлиент.СерверПриемаОтчетностиФСС(ВернутьТипДокументовФСС(2));
	Порт = 443;
	Урл = ПолучитьЗаменяемоеЗначениеТестовогоСервера1С("/ws-insurer-crypto-v20/FileOperationsLnService?wsdl", "{FSS}/ws-insurer-crypto-v20/FileOperationsLnService?wsdl");
...
Функция ВыполнитьПроверкуКомпонентов() //3
	...
	Если КриптографияЭДКОКлиентСервер.ЭтоЛокальнаяПодпись(РезультатыДиагностики.Параметры) Тогда
	...
	Иначе
		...
		РезультатыДиагностики.Отчет.Проверки.ПроверкаКомпонентов.Криптокомпонент = Новый Структура("Подключен, КодОшибки, ОписаниеОшибки, Состояние");
		РезультатыДиагностики.Отчет.Проверки.ПроверкаКомпонентов.Криптокомпонент.Подключен = Истина;
...
Функция ВыполнитьПроверкуСертификатов() //4
	...
	Для Каждого Сертификат из РезультатыДиагностики.Автонастройка.СертификатыCA Цикл
		НоваяСтрока = Новый Структура("Отпечаток", Сертификат.Ключ);
		КриптографияЭДКОКлиентСервер.ЗаполнитьМестоХраненияКлюча(МестоХраненияКлюча, НоваяСтрока);
		СписокСертификатовДляПоиска.Добавить(НоваяСтрока);
	КонецЦикла;
...
Процедура ВыполнитьПоискВНайденныхСертификатах(Результат, ВхПараметры) Экспорт
	...
	Иначе
		...
		Хранилище.Вставить("ЭтоЛокальноеХранилище", КриптографияЭДКОКлиентСервер.ЭтоЛокальнаяПодпись(МестоХраненияКлюча));
...
Функция ВыполнитьПроверкуКриптоопераций(ИнтерактивныеУведомления = Ложь, ВходящийКонтекст = Неопределено) //5
	...
	СписокСертификатовДляПоиска = Новый Массив;
	МестоХраненияКлюча = КриптографияЭДКОКлиентСервер.КонтекстМоделиХраненияКлюча(РезультатыДиагностики.Параметры);
	НоваяСтрока = Новый Структура("Отпечаток", РезультатыДиагностики.Параметры.УчетнаяЗаписьОбмена.СертификатДляШифрования);
	КриптографияЭДКОКлиентСервер.ЗаполнитьМестоХраненияКлюча(МестоХраненияКлюча, НоваяСтрока);
	СписокСертификатовДляПоиска.Добавить(НоваяСтрока);
...
Функция НайтиСертификатИПерейтиКШифрованию(Результат, ВходящийКонтекст) Экспорт
	...
	Если Результат.Выполнено Тогда 
		...
		Если Результат.Сертификаты.Количество() > 0 Тогда 
			...
			ОтпечатокСертификата = Новый Структура("Отпечаток, Поставщик, СерийныйНомер, Сертификат");
			ЗаполнитьЗначенияСвойств(ОтпечатокСертификата, Сертификат);
			КриптографияЭДКОКлиентСервер.КонтекстМоделиХраненияКлюча(МестоХраненияКлюча, ОтпечатокСертификата);
...
Процедура ВыполнитьТестовыеОперацииЗашифровать(Результат, ВходящийКонтекст) Экспорт
	...
	Иначе
		...
		Если КриптографияЭДКОКлиентСервер.ЭтоЛокальнаяПодпись(МестоХраненияКлюча) Тогда
			РезультатыПодключенияКК.Криптокомпонент.НачатьПолучениеСписокОшибок(Оповещение);
		Иначе
			ВыполнитьОбработкуОповещения(Оповещение, "Ошибка");
		КонецЕсли;
...
Функция ОтправитьОтчетНаСервере(ОтчетДиагностики, ОписаниеОшибки = "", Знач ТолькоСформировать = Ложь)
	...
	Отчет_Абонент_УчетнаяЗапись.МодельХраненияЗакрытогоКлюча = XMLСтрока(ОтчетДиагностики.Абонент.УчетнаяЗапись.МодельХраненияЗакрытогоКлюча);
	...
	Для Каждого  Направление из ТекущаяПроверка.ИнформацияДоверенности Цикл
		...
		Если Направление.Доверенность <> Неопределено Тогда 
			...
			РеквизитыДоверенности.Номер = ?(ТипЗнч(Направление.Доверенность) = Тип("Структура")
				И Направление.Доверенность.Свойство("НомерДоверенности"), Направление.Доверенность.НомерДоверенности,
				Направление.Доверенность.НомерДовер);
...
Функция ПолучитьУчетнуюЗапись(Организация)
	...
	Если ЗначениеЗаполнено(УчетнаяЗапись) Тогда 
		...
		КриптографияЭДКОКлиентСервер.КонтекстМоделиХраненияКлюча(УчетнаяЗапись, ПоляУчетнойЗаписи);
		
		Результат = ПоляУчетнойЗаписи;
