...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	СтраницаЖурнала   = Сведения.СтраницаЖурнала;
	
	Если СтраницаЖурнала = Перечисления.СтраницыЖурналаОтчетность.Отчеты Тогда
		Заголовок = НСтр("ru = 'Печать протоколов'");
	Иначе
	...
	Для Каждого ПечатныйДокумент Из ПечатныеДокументы Цикл
		...
		Если ПечатныйДокумент.Значение <> Неопределено Тогда 
			...
			Если ТипЗнч(ПечатныйДокумент.Значение) = Тип("Структура") Тогда
				...
				НовСтр2.ИмяФайлаБезРасширения 	= ИмяФайлаБезРасширения(ПечатныйДокумент.Представление, Сведения);
...
Процедура КомандаПечать(Команда)
	...
	Для Каждого Эл Из СписокОбъектовДляПечати Цикл
		...
		Для Сч = 1 По КоличествоКопий Цикл
			...
			ЭлТабличныйДокумент.Автомасштаб = Истина;
...
&НаСервере
Функция ИмяФайлаБезРасширения(Представление, Сведения)
	
	ПодстрокиИмени = Новый Массив;
	// Представление протокола
	ПодстрокиИмени.Добавить(Представление);
	...
	Иначе
		ПодстрокиИмени.Добавить(Формат(Сведения.Дата, "ДФ=dd.MM.yyyy"));
	КонецЕсли;
...
Процедура СписокЛистовПриАктивизацииСтроки(Элемент)
	...
	Если Элемент.ТекущиеДанные <> Неопределено И ЗначениеЗаполнено(Элемент.ТекущиеДанные.ТабличныйДокумент) Тогда 
...
Процедура ОсвежитьВыбранныйЛист(АдресТД, ИДСтроки=0)
	...
	Если ЗначениеЗаполнено(АдресТД) Тогда
		...
		ВыводимыйТаблДок.АвтоМасштаб = Истина;
...
Функция ПолучитьМассивОписанийФайловВыгрузки(ВФормате)
	...
	Для Каждого Эл Из СписокОбъектовДляПечати Цикл
		Если Эл.Пометка Тогда
			...
			КороткоеИмяФайла      = Формат(Номер, "ЧГ=0") + ". " + ИмяОбъктаБезДвоеточий + "." + СоответствиеФорматаРасширению[ВФормате];
...
Процедура ВыгрузитьНаДискЗавершение(Результат, ВФормате) Экспорт
	...
	Иначе
		...
		#Иначе
			ЭтоВеб = Ложь;
		#КонецЕсли
		Если ЭтоВеб ИЛИ МассивОписанийПолучаемыеФайлы.Количество() > 1 Тогда
		...
		Иначе
			...
			Шаблон = "Файл в формате (*.%1)|*.%1";
			Фильтр = СтрШаблон(Шаблон, НРег(Строка(ВФормате)));
