...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	МестоХраненияКлюча = КриптографияЭДКОКлиентСервер.КонтекстМоделиХраненияКлюча(ЦиклОбмена);
...
Процедура КомандаПодписиПроверить(Команда)
	...
	Иначе
		МестоХраненияКлюча = КриптографияЭДКОКлиентСервер.КонтекстМоделиХраненияКлюча(Результат.РеквизитыУчетнойЗаписи);
		...
		//КриптографияЭДКОКлиент.ПроверитьПодписьPKCS7(ОписаниеОповещения, ЭтоЭлектроннаяПодписьВМоделиСервиса, АдресФайлаЭЦП, АдресФайлаДИВ,, Ложь);
		КриптографияЭДКОКлиент.ПроверитьПодписьPKCS7(ОписаниеОповещения, МестоХраненияКлюча, АдресФайлаЭЦП, АдресФайлаДИВ,, Ложь);
	КонецЕсли;
...
Процедура ОформитьСтрокиПодписей()
	...
	Для Каждого ЭлементСтроки Из ПодписиПредставление Цикл 
		...
		Если ЗначениеЗаполнено(ОтпечатокСертификата) Тогда
			...
			НовыйСертификат = Новый Структура("Отпечаток", ОтпечатокСертификата, МестоХраненияКлюча);
			КриптографияЭДКОКлиентСервер.КонтекстМоделиХраненияКлюча(МестоХраненияКлюча, НовыйСертификат);
			СертификатыДляПоиска.Добавить(НовыйСертификат);
		Иначе
...
Процедура УдалитьСертификатЕслиНеИспользуется(ТранспортноеСообщение, ОтпечатокСертификата)
	...
	Если ЗначениеЗаполнено(ИмяФайлаСертификата) Тогда
		...
		Если МассивФайлов.Количество() > 0 Тогда
			ПрисоединенныйФайлОбъект = МассивФайлов[0].ПолучитьОбъект();
...
Процедура ВыбратьИзМенюКомандаПодписиДобавитьЗавершение(РезультатВыбора, ДополнительныеПараметры) Экспорт
	...
	Иначе
		КриптографияЭДКОКлиент.ВыбратьСертификат(
			ОписаниеОповещения, МестоХраненияКлюча);
	КонецЕсли;
...
Процедура УдалитьПодписиНаСервере(ТекущаяСтрока)
	...
	ТранспортноеСообщениеПодписи = ПодписиТекущаяСтрока.ТранспортноеСообщение;
	ИмяФайлаПодписи = ПодписиТекущаяСтрока.ИмяФайла;
...
Функция ПолучитьДИВНаСервере(ИмяФайлаТекПодписи, ИспользоватьВК = Ложь)
	...
	РеквизитыУчетнойЗаписи = ЭлектроннаяПодписьВМоделиСервисаБРОВызовСервера.РеквизитыУчетнойЗаписи(ТранспортноеСообщение.УчетнаяЗапись);
	
	Возврат Новый Структура("АдресФайлаДИВ, АдресФайлаЭЦП, РеквизитыУчетнойЗаписи", АдресФайлаДИВ, АдресФайлаЭЦП, РеквизитыУчетнойЗаписи);
