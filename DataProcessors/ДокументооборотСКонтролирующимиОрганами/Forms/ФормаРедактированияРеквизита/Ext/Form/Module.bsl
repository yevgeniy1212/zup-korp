...
Процедура КнопкаОК(Команда)
	...
	Если ИмяРеквизита = "КодРосстата" Тогда
		...
		Если СтрДлина(СокрЛП(ЗначениеРеквизита))< 5 Тогда
			ОбщегоНазначенияКлиент.СообщитьПользователю(НСтр("ru = 'Код органа Росстата должен состоять как минимум из 4 цифр'"), ,"ЗначениеРеквизита");
	...
	ИначеЕсли ИмяРеквизита = "КодПФР" Тогда
		...
		Если СтрДлина(СокрЛП(ЗначениеРеквизита))<> 7 Тогда
			ОбщегоНазначенияКлиент.СообщитьПользователю(НСтр("ru = 'Код отделения ПФР должен состоять из 6 цифр'"), ,"КодПФР");
			ВведеноВерноеЗначение = Ложь;
		КонецЕсли;
