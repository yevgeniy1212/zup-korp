...
Функция КомпонентаКриптографииУстановлена(ОповещениеЗавершения)
	...
	ВхПараметры = Новый Структура("ЭтоЗаявлениеНаПодключение", Истина);
...
Процедура НачатьПодключениеВнешнейКомпонентыЗавершение(Результат, ВхПараметры) Экспорт
	Если Результат Тогда 
		Первичное = ВхПараметры<>Неопределено и ВхПараметры.Свойство("ЭтоЗаявлениеНаПодключение") и ВхПараметры.ЭтоЗаявлениеНаПодключение;
		ВыполнитьОбработкуОповещения(ВхПараметры.ОповещениеЗавершения, ?(Первичное, 0, 1));
	Иначе
		Если ВхПараметры<>Неопределено и ВхПараметры.Свойство("ЭтоЗаявлениеНаПодключение") и ВхПараметры.ЭтоЗаявлениеНаПодключение Тогда 
			ВхПараметры.ЭтоЗаявлениеНаПодключение = Ложь;
...
&НаКлиенте
Процедура ОбщееДействиеНастройкаНерасшифровываемых(Элемент, НавигационнаяСсылкаФорматированнойСтроки, СтандартнаяОбработка)
	...
	ФормаРезультат = ПолучитьФорму(КонтекстЭДОКлиент.ПутьКОбъекту + ".Форма.Нерасшифровываемые_УправлениеОшибкамиОбмена", ПараметрыОткрытияФормы, ЭтотОбъект.ВладелецФормы);
...
Процедура ОбщееДействиеИнформацияОПодписке(Элемент, НавигационнаяСсылкаФорматированнойСтроки, СтандартнаяОбработка) Экспорт
	...
	Если Элемент = "КПНастроить" Тогда 		
		ИнтернетПоддержкаПользователейКлиент.ОткрытьСтраницуИнтегрированногоСайта("http://its.1c.ru/bmk/elreps/subscr");
...
&НаКлиенте
Процедура ПолучитьКонтекстЭДОИНастроитьНерасшифровываемые(Результат, ДополнительныеПараметры) Экспорт
	
	Если Результат.Свойство("КонтекстЭДО") Тогда 
		КонтекстЭДОКлиент = Результат.КонтекстЭДО;
		ОбщееДействиеНастройкаНерасшифровываемых(Неопределено, ДополнительныеПараметры, Неопределено);
	КонецЕсли;
...
&НаКлиенте
Процедура ДействиеНастройкаРасшифровки(Результат, ВхКонтекст) Экспорт
	
	Если КонтекстЭДОКлиент = Неопределено Тогда 		
		ОписаниеОповещения = Новый ОписаниеОповещения("ПолучитьКонтекстЭДОИНастроитьНерасшифровываемые", ЭтотОбъект, ВхКонтекст);
		ДокументооборотСКОКлиент.ПолучитьКонтекстЭДО(ОписаниеОповещения);
		Возврат;
	КонецЕсли;
	
	ОбщееДействиеНастройкаНерасшифровываемых(Неопределено, ВхКонтекст, Неопределено);
