...
Процедура ПроверитьЗаполнениеДокумента(ВыполняемоеОповещение)
	...
	ИтогПроверки = НЕ ЕстьОшибкаЗаполненияУЦ();
	...
	ИначеЕсли КриптографияЭДКОКлиентСервер.ЭтоОблачнаяПодпись(МестоХраненияКлюча) Тогда
		Если НЕ ЗначениеЗаполнено(ОбработкаЗаявленийАбонентаКлиентСервер.ПолучитьПараметрПодключения(Объект, "УчетнаяЗапись")) 
				И НЕ ОбработкаЗаявленийАбонентаКлиентСервер.ПолучитьПараметрПодключения(Объект, "СоздатьНовуюУчетнуюЗапись") Тогда
			ОбщегоНазначенияКлиент.СообщитьПользователю(НСтр("ru = 'Не выбрана учетная запись облачной подписи. Воспользуйтесь мастером подключения для отправки заявления.'"));
			ИтогПроверки = Ложь;
		КонецЕсли;
	Иначе
	...
	Если Объект.ПодатьЗаявкуНаСертификатДляФСРАР Тогда
		...
		Если ИтогПроверки = Истина Тогда
			...
			Если Объект.КодРегионаФСРАР <> КодРегионаЮрАдреса И ЗначениеЗаполнено(КодРегионаЮрАдреса) Тогда
				НаименованиеРегионаЮрАдреса = НаименованиеСубъектаРФПоКодуСубъекта(Число(КодРегионаЮрАдреса));
...
Процедура СформироватьТаблицуИзмененныхДанных()
	...
	ДобавитьВТаблицу(Изменения.КраткоеНаименование, Объект.КраткоеНаименование,,,,,ЭтоЗаявлениеТолькоНаПереход);
	ДобавитьВТаблицу(Изменения.ИНН, Объект.ИНН, Ложь,,,,ЭтоЗаявлениеТолькоНаПереход); // ИНН запрещено менять
	ДобавитьВТаблицу(Изменения.КПП, Объект.КПП,,,,,ЭтоЗаявлениеТолькоНаПереход);
	ДобавитьВТаблицу(Изменения.ОГРН, Объект.ОГРН,,,,,ЭтоЗаявлениеТолькоНаПереход);
	ДобавитьВТаблицу(Изменения.РегНомерПФР, Объект.РегНомерПФР,,,,,ЭтоЗаявлениеТолькоНаПереход);
	...
	Если ЭтоАдресПоФИАСу Тогда
		...
		ДобавитьВТаблицу(Изменения.АдресЮридический,ПредставлениеАдреса,,,,,ЭтоЗаявлениеТолькоНаПереход);
		
		Область = КонтекстЭДОСервер.ПолеСертификата_2_5_4_8(Объект.АдресЮридический);
		ДобавитьВТаблицу(Изменения.Область,Область,,,,,ЭтоЗаявлениеТолькоНаПереход);
		
		Город = КонтекстЭДОСервер.ПолеСертификата_2_5_4_7(Объект.АдресЮридический);
		ДобавитьВТаблицу(Изменения.Город,Город,,,,,ЭтоЗаявлениеТолькоНаПереход);
		
		Улица = КонтекстЭДОСервер.ПолеСертификата_2_5_4_9(Объект.АдресЮридический);
		ДобавитьВТаблицу(Изменения.Улица,Улица,,,,,ЭтоЗаявлениеТолькоНаПереход);
	...
	ДобавитьВТаблицу(Изменения.КодРегионаФСРАР, Объект.КодРегионаФСРАР,,,,,ЭтоЗаявлениеТолькоНаПереход);
	ДобавитьВТаблицу(Изменения.ТелефонОсновной, Объект.ТелефонОсновной,,,,,ЭтоЗаявлениеТолькоНаПереход);
	ДобавитьВТаблицу(Изменения.ЭлектроннаяПочта, Объект.ЭлектроннаяПочта,,,,,ЭтоЗаявлениеТолькоНаПереход); 
	...
	ДобавитьВТаблицу(Изменения.ВладелецЭЦП, ФИОВладельцаЭЦП,,,,,ЭтоЗаявлениеТолькоНаПереход);
	ДобавитьВТаблицу(Изменения.ВладелецЭЦПФИО, ФИОВладельцаЭЦП,,,,,ЭтоЗаявлениеТолькоНаПереход);
	
	Если Объект.ТипОрганизации Тогда
		ДобавитьВТаблицу(Изменения.ВладелецЭЦПДолжность, Объект.ВладелецЭЦПДолжность,,,,,ЭтоЗаявлениеТолькоНаПереход);
	КонецЕсли;
		
	ДобавитьВТаблицу(Изменения.ВладелецЭЦППодразделение, Объект.ВладелецЭЦППодразделение,,,,,ЭтоЗаявлениеТолькоНаПереход);
	ДобавитьВТаблицу(Изменения.ВладелецЭЦПСНИЛС, Объект.ВладелецЭЦПСНИЛС,,,,,ЭтоЗаявлениеТолькоНаПереход);
	Если Объект.ТипОрганизации Тогда
		ДобавитьВТаблицу(Изменения.ВладелецЭЦПИНН, Объект.ВладелецЭЦПИНН,,,,,ЭтоЗаявлениеТолькоНаПереход);
	КонецЕсли;
	ДобавитьВТаблицу(Изменения.ВладелецЭЦПДатаРождения, Формат(Объект.ВладелецЭЦПДатаРождения, "ДЛФ=D"),,,,,ЭтоЗаявлениеТолькоНаПереход);
	ДобавитьВТаблицу(Изменения.ВладелецЭЦППол, Объект.ВладелецЭЦППол,,,,,ЭтоЗаявлениеТолькоНаПереход);
	ДобавитьВТаблицу(Изменения.ВладелецЭЦПГражданство, Объект.ВладелецЭЦПГражданство,,,,,ЭтоЗаявлениеТолькоНаПереход);
	ДобавитьВТаблицу(Изменения.ВладелецЭЦПВидДокумента, Объект.ВладелецЭЦПВидДокумента, ,,,,ЭтоЗаявлениеТолькоНаПереход);
	ДобавитьВТаблицу(Изменения.ВладелецЭЦПСерияДокумента, Строка(Объект.ВладелецЭЦПСерияДокумента),,,,,ЭтоЗаявлениеТолькоНаПереход);
	ДобавитьВТаблицу(Изменения.ВладелецЭЦПНомерДокумента, Строка(Объект.ВладелецЭЦПНомерДокумента),,,,,ЭтоЗаявлениеТолькоНаПереход);
	ДобавитьВТаблицу(Изменения.ВладелецЭЦПКемВыданДокумент, Объект.ВладелецЭЦПКемВыданДокумент,,,,,ЭтоЗаявлениеТолькоНаПереход);
	ДобавитьВТаблицу(Изменения.ВладелецЭЦПДатаВыдачиДокумента, Объект.ВладелецЭЦПДатаВыдачиДокумента,,,,,ЭтоЗаявлениеТолькоНаПереход);
	ДобавитьВТаблицу(Изменения.ВладелецЭЦПМестоРождения, Объект.ВладелецЭЦПМестоРождения,,,,,ЭтоЗаявлениеТолькоНаПереход);
	ДобавитьВТаблицу(Изменения.ВладелецЭЦПКемВыданДокумент, Объект.ВладелецЭЦПКемВыданДокумент,,,,,ЭтоЗаявлениеТолькоНаПереход);
	Если ЭлектронныйДокументооборотСКонтролирующимиОрганамиВызовСервера.ЭтоПаспортРФ(Объект.ВладелецЭЦПВидДокумента) Тогда
		ДобавитьВТаблицу(Изменения.ВладелецЭЦПКодПодразделения, Объект.ВладелецЭЦПКодПодразделения,,,,,ЭтоЗаявлениеТолькоНаПереход);
	КонецЕсли;
	...
	Если ОператорПоддерживаетСМСУведомление Тогда
		ДобавитьВТаблицу(Изменения.ТелефонМобильный, Объект.ТелефонМобильный,,,,,ЭтоЗаявлениеТолькоНаПереход);
	КонецЕсли;
	...
	Если НЕ ЭтоЗаявлениеТолькоНаПереход Тогда
		...
		СдаватьВСФР = СтрокиСПФР.Количество() > 0 ИЛИ СтрокиСФСС.Количество() > 0;;
		
		ДобавитьВТаблицу(Изменения.СдаватьВПФР, СдаватьВСФР);
		Если СдаватьВСФР Тогда
