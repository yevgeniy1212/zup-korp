	...
	Для Каждого СтрУровня1 Из ЭлементыУровня1 Цикл
		...
		Для Каждого СтрУровня2 Из ЭлементыУровня2 Цикл
			Если ЗначениеЗаполнено(СтрУровня2.Сертификат) Тогда
				НовыйСертификат = Новый Структура("Отпечаток", СтрУровня2.Сертификат);
				КриптографияЭДКОКлиентСервер.КонтекстМоделиХраненияКлюча(МестоХраненияКлюча, НовыйСертификат);
				СертификатыСоответствие.Добавить(НовыйСертификат);
			КонецЕсли;
		КонецЦикла;
	КонецЦикла;
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	МестоХраненияКлюча = КриптографияЭДКОКлиентСервер.КонтекстМоделиХраненияКлюча(Параметры.Организация);
...
Процедура Выбрать(Команда)
	...
	СвойстваСертификата = ОбщегоНазначенияКлиент.СкопироватьРекурсивно(СвойстваСертификатов[ВРег(ТекДанные.Сертификат)], Ложь);
...
Процедура ОткрытьСертификат(Команда)
	...
	Если Элементы.Сертификаты.ТекущиеДанные <> Неопределено Тогда
		НовыйСертификат = Новый Структура("Отпечаток", Элементы.Сертификаты.ТекущиеДанные.Сертификат);
		КриптографияЭДКОКлиентСервер.КонтекстМоделиХраненияКлюча(МестоХраненияКлюча, НовыйСертификат);
...
Процедура ПриОткрытииЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	Для Каждого НайденныйСертификат Из Результат.Сертификаты Цикл
		СвойстваСертификатов.Вставить(ВРег(НайденныйСертификат.Отпечаток), НайденныйСертификат);
	КонецЦикла;
	...
	Если Сертификаты.ПолучитьЭлементы().Количество() = 0 Тогда
		ПоказатьПредупреждение(, НСтр("ru = 'Не определено ни одного сертификата ответственого лица
	...
	Для Каждого СтрУровня1 Из ЭлементыУровня1 Цикл
		...
		Для Каждого СтрУровня2 Из ЭлементыУровня2 Цикл
			...
			Если СвойстваСертификата = Неопределено Тогда
				...
				ДанныеСтроки.СертификатПредставление = НСтр("ru = 'Отпечаток: '") + Сертификат + Символы.ПС + 
					НСтр("ru = 'Сертификат недоступен текущему пользователю операционной системы!'");
				Продолжить;
			КонецЕсли;
			
			ПредставлениеСертификата = СокрЛП(СвойстваСертификата.Наименование) + Символы.ПС +
				НСтр("ru = 'Действует с '") + СвойстваСертификата.ДействителенС + НСтр("ru = ' по '") + СвойстваСертификата.ДействителенПо;
			...
			Если НЕ СвойстваСертификата.ИспользоватьДляПодписи Тогда
				ДанныеСтроки.ЦветТекста = "ОсобыйЦвет";
				ДанныеСтроки.СертификатПредставление = ПредставлениеСертификата + Символы.ПС +
					НСтр("ru = 'Сертификат не предназначен для подписания!'");
			ИначеЕсли НЕ (ТипЗнч(СвойстваСертификата.ДействителенПо) = Тип("Строка") 
				...
				Если ОбщегоНазначенияКлиент.ДатаСеанса() > СвойстваСертификата.ДействителенПо Тогда
					...
					ДанныеСтроки.СертификатПредставление = ПредставлениеСертификата + Символы.ПС +
						НСтр("ru = 'Период действия сертификата истек!'");
				ИначеЕсли ОбщегоНазначенияКлиент.ДатаСеанса() < СвойстваСертификата.ДействителенС Тогда
					ДанныеСтроки.ЦветТекста = "СерыйЦвет";
					ДанныеСтроки.СертификатПредставление = ПредставлениеСертификата + Символы.ПС +
						НСтр("ru = 'Период действия сертификата еще не наступил!'");
				КонецЕсли;
			Иначе
				ДанныеСтроки.ЦветТекста = Неопределено;
				ДанныеСтроки.СертификатПредставление = ПредставлениеСертификата;
