...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	АктивизироватьСтраницуОтправка(ЭтотОбъект);
...
&НаКлиенте
Процедура ПодсказкаПревышеноКоличествоПопытокОбработкаНавигационнойСсылки(Элемент, НавигационнаяСсылкаФорматированнойСтроки, СтандартнаяОбработка)
...
&НаКлиенте
Процедура НетЛогинаНажатие(Элемент)
...
&НаКлиенте
Процедура ЗадатьЛогинИПарольИТССейчас(Команда)
	
	ОписаниеОповещения = Новый ОписаниеОповещения(
		"ПослеПодключенияИнтернетПоддержкиПользователей", 
		ЭтотОбъект); 
		
	МодульИнтернетПоддержкаПользователейКлиент = ЭлектронныйДокументооборотСКонтролирующимиОрганамиКлиент.МодульИнтернетПоддержкаПользователейКлиент();
...
Процедура АктивизироватьСтраницуОтправка(ЭтотОбъект)
	
	Элементы = ЭтотОбъект.Элементы;
	
	ЭтотОбъект.Заголовок = НСтр("ru = 'Пожалуйста, подождите...'"); 
	АктивизироватьСтраницу(ЭтотОбъект, Элементы.Отправка);
	
КонецПроцедуры

&НаКлиентеНаСервереБезКонтекста
Процедура АктивизироватьСтраницуНеверныйЛогинИлиПароль(ЭтотОбъект)
	
	Элементы = ЭтотОбъект.Элементы;
	
	ЭтотОбъект.Заголовок = НСтр("ru = 'Авторизуйтесь на Портале 1С:ИТС'"); 
	АктивизироватьСтраницу(ЭтотОбъект, Элементы.НеверныйЛогинИлиПароль);
	Элементы.ЗадатьСейчас.КнопкаПоУмолчанию = Истина;
	
КонецПроцедуры

&НаКлиентеНаСервереБезКонтекста
Процедура АктивизироватьСтраницуПревышеноКоличествоПопыток(ЭтотОбъект)
	
	Элементы = ЭтотОбъект.Элементы;
	
	ЭтотОбъект.Заголовок = НСтр("ru = 'Ошибка авторизации на Портале 1С:ИТС'"); 
	АктивизироватьСтраницу(ЭтотОбъект, Элементы.ПревышеноКоличествоПопыток);
...
&НаКлиенте
Процедура ПослеПодключенияИнтернетПоддержкиПользователей(Результат, ВходящийКонтекст) Экспорт
...
&НаСервере
Функция ПроверитьОрганизаицю(Отказ)
	...
	Если ИспользуетсяОднаОрганизация И НЕ ЗначениеЗаполнено(ОрганизацияПоУмолчанию) Тогда
		Отказ = Истина;
		Сообщить(НСтр("ru = 'Перед началом подключения создайте организацию, которую требуется подключить к 1С-Отчетности'"));
		Возврат Ложь;
	КонецЕсли;
	
	Если ИспользуетсяОднаОрганизация И ЗначениеЗаполнено(ОрганизацияПоУмолчанию) И НЕ ЗначениеЗаполнено(Организация) Тогда
...
Процедура Инициализация(Отказ)
	...
	Если НЕ ПроверитьОрганизаицю(Отказ) Тогда
		Возврат;
	КонецЕсли;
	...
	НеПредлагатьУстановкуРасширения = ДоступнаЭПВМоделиСервиса И ОбщегоНазначения.ЭтоВебКлиент() И НЕ ЭтоИнтеграцияСБанком;
...
Функция ОткрытьВторичноеЗаявление()
	...
	ЗапретитьОткрытиеВторичного = 
		ИспользуетсяОднаОрганизация
		И ЗначениеЗаполнено(ОрганизацияПоУмолчанию)
		И НЕ ЗначениеЗаполнено(УчетнаяЗаписьОрганизации);
	...
	ОткрытьВторичноеЗаявление = 
		...
		И НЕ ЗапретитьОткрытиеВторичного; 
...
Процедура НачатьОткрытьЗаявлениеНаПодключениеУпрощенное()
	...
	Если ЗначениеЗаполнено(ЗначениеКопирования) И ЗаявлениеОтправлено Тогда
		
		НачатьОткрытьЗаявлениеНаПодключениеУпрощенное_НачатьПоискСертификатов();
		
	Иначе
		
		// Получение тикета для сохраненных логина и пароля.
		// Внимание! Если в базе в модели сервиса отлаживаемся из-под админа, то тикет будет невозможно получить.
		// Получать тикет только в нужной области под нужным пользователем.
	
		РезультатПолученияТикета = КонтекстЭДОКлиент.ТикетАутентификацииНаПорталеПоддержки();
		
		Если РезультатПолученияТикета = Неопределено Тогда

			
			ПараметрыЗакрытияБублика.Параметры.Вставить("ЗапрашиватьРегНомер", Истина);
			ПараметрыЗакрытияБублика.Параметры.Вставить("ПоказыватьПодсказкуРегНомера", Ложь);
			НачатьОткрытьЗаявлениеНаПодключениеУпрощенное_НачатьПоискСертификатов();
			
		ИначеЕсли ЗначениеЗаполнено(РезультатПолученияТикета.Тикет) Тогда
			
			НачатьОткрытьЗаявлениеНаПодключениеУпрощенное_НачатьПоискСертификатов();
			
		Иначе
			
			ИгнорироватьОшибку = Истина; // Временно
			
			// Если не удалось получить тикет, обработать ошибку.
			Если НЕ ИгнорироватьОшибку И РезультатПолученияТикета.КодОшибки = "НеверныйЛогинИлиПароль" Тогда
				
				ПараметрыРаботыКлиента = СтандартныеПодсистемыКлиент.ПараметрыРаботыКлиента().ИнтернетПоддержкаПользователей;
				ДоступноПодключениеИнтернетПоддержки = ПараметрыРаботыКлиента.ДоступноПодключениеИнтернетПоддержки;
				
				Если ДоступноПодключениеИнтернетПоддержки Тогда
					
					АктивизироватьСтраницуНеверныйЛогинИлиПароль(ЭтотОбъект);
					
				Иначе
					

					ПараметрыЗакрытияБублика.Параметры.Вставить("ЗапрашиватьРегНомер", Истина);
					ПараметрыЗакрытияБублика.Параметры.Вставить("ПоказыватьПодсказкуРегНомера", Истина);
					НачатьОткрытьЗаявлениеНаПодключениеУпрощенное_НачатьПоискСертификатов();
					
				КонецЕсли;
				
			ИначеЕсли НЕ ИгнорироватьОшибку И РезультатПолученияТикета.КодОшибки = "ПревышеноКоличествоПопыток" Тогда
				
				Элементы.ПодсказкаПревышеноКоличествоПопыток.Заголовок = Новый ФорматированнаяСтрока(
					НСтр("ru = 'Подключение к 1С-Отчетности временно невозможно, т.к. не удалось авторизоваться на Портале 1С:ИТС.'"),Символы.ПС,
					НСтр("ru = 'Ответ сервера: превышено количество попыток авторизации.'"),Символы.ПС,
					КонтекстЭДОКлиент.ТекстДляПревышеноКоличествоПопытокПолученияТикета());

				
				АктивизироватьСтраницуПревышеноКоличествоПопыток(ЭтотОбъект);
				
			Иначе
				
				// Другая ошибка, находится в РезультатПолученияТикета.СообщениеОбОшибке
				ПараметрыЗакрытияБублика.Параметры.Вставить("ЗапрашиватьРегНомер", Истина);
				ПараметрыЗакрытияБублика.Параметры.Вставить("ПоказыватьПодсказкуРегНомера", Истина);
				НачатьОткрытьЗаявлениеНаПодключениеУпрощенное_НачатьПоискСертификатов();
...
&НаКлиенте
Процедура НачатьОткрытьЗаявлениеНаПодключениеУпрощенное_НачатьПоискСертификатов()
...
Процедура НачатьОткрытьЗаявлениеНаПодключениеУпрощенное_ПослеПоискаКриптопровайдеров(Результат, ВходящийКонтекст) Экспорт
	...
	ПоказатьОшибку = ЗначениеЗаполнено(ОписаниеОшибки) И НЕ ДоступнаЭПВМоделиСервиса И НЕ ДоступнаОблачнаяПодпись;
	ПоказатьФормуКонфликтаКриптопровайдеров  = ViPNetCSPУстановлен И CryptoProCSPУстановлен И НЕ ДоступнаЭПВМоделиСервиса И НЕ ИгнорироватьКонфликт И НЕ ДоступнаОблачнаяПодпись;
	ПоказатьФормуОтсутствияКриптопровайдеров = 
		НЕ ViPNetCSPУстановлен 
		И НЕ CryptoProCSPУстановлен 
		И (НЕ ДоступнаЭПВМоделиСервиса И НЕ ЭтоИнтеграцияСБанком И НЕ ДоступнаОблачнаяПодпись ИЛИ ЭтоИнтеграцияСБанком)
		И КомпонентаДляРаботыСКриптографиейПодключена;
	...
	ИначеЕсли ПоказатьФормуОтсутствияКриптопровайдеров Тогда
		...
		Если ЭтоИнтеграцияСБанком Тогда
			ПараметрыЗакрытияБублика.ИмяФормы = ПутьКОбъекту + ".Форма.Мастер_ОтсуствиеКриптоПроУСН";
			ПараметрыЗакрытияБублика.РежимОткрытияОкнаФормы = РежимОткрытияОкнаФормы.Независимый;
			ПерейтиКОткрытиюФормы();
		Иначе
...
Процедура ДобавитьВПараметрыКриптопровайдеров()
	...
	Истекла = ОбработкаЗаявленийАбонентаКлиент.ЛицензииКриптоПроИстекла(ЭтотОбъект);
	ПараметрыЗакрытияБублика.Параметры.Вставить("ЛицензииКриптоПроИстекла", Истекла);
...
Процедура ОткрытьЗаявлениеНаИзменение()
	...
	ПараметрыЗакрытияБублика.Параметры.Вставить("НовыйСертификат", НовыйСертификат);
	ПерейтиКОткрытиюФормы();
