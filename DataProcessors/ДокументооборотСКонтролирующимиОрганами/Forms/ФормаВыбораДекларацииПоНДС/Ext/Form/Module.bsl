...
Процедура СформироватьТаблицу()
	...
	Запрос.Текст = 
		...
		|	ПоследниеОтправки.Отчет КАК Отчет,
		...
		|	ЖурналОтчетовСтатусы.ФинансовыйПериод КАК ФинансовыйПериод,
		|	ЖурналОтчетовСтатусы.ДатаНачала КАК ДатаНачала
		|ИЗ
		...
		|			ИЛИ ЖурналОтчетовСтатусы.Статус = ""Сдано, требует уточнения"")
