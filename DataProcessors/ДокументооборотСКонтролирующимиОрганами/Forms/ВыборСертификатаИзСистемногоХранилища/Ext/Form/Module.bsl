...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ОтпечаткиЗапрещенныхСертификатов = Параметры.ОтпечаткиЗапрещенныхСертификатов;
	СообщениеПриВыбореЗапрещенного   = Параметры.СообщениеПриВыбореЗапрещенного;
	...
	УстановитьНачальноеЗначениеВыбора(Параметры);
	УправлениеЭУПриСозданииНаСервере();
...
&НаСервере
Процедура УправлениеЭУПриСозданииНаСервере()
	
	// определяем с каким хранилищем будем работать
	ОбрабатываемоеХранилище = ?(ЗначениеЗаполнено(Хранилище), Врег(Хранилище), "MY");
	...
	Иначе
		Элементы.ПоказыватьПросроченные.Пометка = (ПоказыватьПросроченные = Истина);
	КонецЕсли;
...
&НаСервере
Процедура УстановитьНачальноеЗначениеВыбора(Параметры)
...
Процедура УстановитьУсловноеОформление()
	...
	Для Каждого ЭлементУсловногоОформления Из УсловноеОформление.Элементы Цикл
		Для Каждого Эл Из ЭлементУсловногоОформления.Отбор.Элементы Цикл
			Если ТипЗнч(Эл.ПравоеЗначение) = Тип("СтандартнаяДатаНачала") Тогда
...
Процедура ПолучитьСертификатыЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	СформироватьПолнуюТаблицуСертификатов(СписокСертификаты);
	ПостОбработкаПолнойТаблицыСертификатовНаСервере();
	ИзменитьОформлениеКнопкиПоказыватьПросроченные();
	ПереформироватьТаблицуСертификатов();
...
&НаКлиенте
Процедура СформироватьПолнуюТаблицуСертификатов(СписокСертификаты)
...
&НаКлиенте
Процедура ИзменитьОформлениеКнопкиПоказыватьПросроченные()
...
Процедура ПересчитатьТаблицуСертификатов()
	...
	Для Каждого Серт Из ПолнаяТаблицаСертификатов Цикл
		...
		ЗапрещенКВыбору = Ложь;
		РезультатПоиска = ОтпечаткиЗапрещенныхСертификатов.НайтиПоЗначению(Серт.Отпечаток);
		
		НовСтр = Сертификаты.Добавить();
		ЗаполнитьЗначенияСвойств(НовСтр, Серт);
		
		НовСтр.ИННВладельца = ДокументооборотСКОКлиентСервер.ИННИзСертификата(Серт.ВладелецСтруктура,,Ложь);
		
		Если РезультатПоиска <> Неопределено Тогда
			НовСтр.ЗапрещенКВыбору = Истина;
		КонецЕсли;
...
&НаКлиенте
Процедура ПометитьВыбранныеСертификаты()
...
Процедура ВыбратьСертификат(парамМножественныйВыбор = Неопределено)
	...
	Если ПризнакВыбораНесколькихСтрок Тогда
		ВыбратьСертификат_МножественныйВыбор();
	Иначе
		ВыбратьСертификат_ОдиночныйВыбор();
	КонецЕсли;
	
КонецПроцедуры

&НаКлиенте
Процедура ВыбратьСертификат_ОдиночныйВыбор()
	
	ТекДанные = Элементы.Сертификаты.ТекущиеДанные;
	
	Если ТекДанные.ЗапрещенКВыбору Тогда
		ОписаниеОповещения = Новый ОписаниеОповещения();
		ПоказатьПредупреждение(ОписаниеОповещения, СообщениеПриВыбореЗапрещенного);
		Возврат;
	КонецЕсли;
	...
	Если СрокИстек ИЛИ СрокНеНачался Тогда
		...
		ОписаниеОповещения = Новый ОписаниеОповещения("ПослеОтветаНаВопросОПросроченных", ЭтотОбъект, ДополнительныеПараметры);
...
&НаКлиенте
Процедура ВыбратьСертификат_МножественныйВыбор()
	...
	Для Каждого СтрСертификат Из Сертификаты Цикл
		...
		ТекДанные.Добавить(СвойстваСертификата);
	...
	Если ОдинИзСертификатовПросрочен Тогда
		ДополнительныеПараметры = Новый Структура("ТекДанные", ТекДанные);
		ОписаниеОповещения = Новый ОписаниеОповещения("ПослеОтветаНаВопросОПросроченных", ЭтотОбъект, ДополнительныеПараметры);
		ТекстВопроса = "Среди выбранных сертификатов есть истекшие.
...
Функция СвойстваСертификата(ТекДанные)
	...
	КриптографияЭДКОКлиентСервер.КонтекстМоделиХраненияКлюча(Ложь, СвойстваСертификата);
...
&НаКлиенте
Процедура ПослеОтветаНаВопросОПросроченных(РезультатВопроса, ДополнительныеПараметры) Экспорт
