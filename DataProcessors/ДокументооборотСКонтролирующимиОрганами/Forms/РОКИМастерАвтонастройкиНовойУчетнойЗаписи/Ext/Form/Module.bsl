...
Процедура КонтейнерНачалоВыбора(Элемент)
	...
	ПолучатьУникальныеИмена = ОбщегоНазначенияЭДКОКлиент.ЭтоКриптопровайдерLinux(); // в Linux удается получить только уникальные имена
	КриптографияЭДКОКлиент.ПолучитьКонтейнерыКлючей(Оповещение, ПолучатьУникальныеИмена);
...
Процедура ПрименитьДанныеСтраницы2(ВыполняемоеОповещение)
	...
	Если НЕ ФайлНастроекРасшифрован Тогда
		...
		КриптографияЭДКОКлиент.РасшифроватьPKCS7(ОписаниеОповещения, ФайлНастроекАдресДанных,, МестоХраненияКлюча);
...
Функция ПрименитьДанныеСтраницы6(ВыполняемоеОповещение)
	...
	КонтекстЭДОКлиент.ПрименитьИзмененныеНастройки(
		...
		Истина,
		ОписаниеОповещения,
		?(СопоставитьУчетнуюЗаписьОрганизации, Организация, Неопределено));
...
Процедура ЗарегистрироватьСертификатСПривязкойКЗакрытомуКлючуПоИмениФайла(ВыполняемоеОповещение, СертификатИмяФайла, СертификатКонтейнер)
	...
	ИспользоватьУстановитьСертификатВКонтейнерИХранилище = Истина;
	
	ДополнительныеПараметры = Новый Структура("ВыполняемоеОповещение, СертификатКонтейнер", ВыполняемоеОповещение, СертификатКонтейнер);
	
	Если ИспользоватьУстановитьСертификатВКонтейнерИХранилище ИЛИ ОбщегоНазначенияЭДКОКлиент.ЭтоКриптопровайдерLinux() Тогда
		ОписаниеОповещения = Новый ОписаниеОповещения(
			"ЗарегистрироватьСертификатСПривязкойКЗакрытомуКлючуПоИмениФайлаПослеСвязыванияСертификатаСКонтейнеромКлючей",
