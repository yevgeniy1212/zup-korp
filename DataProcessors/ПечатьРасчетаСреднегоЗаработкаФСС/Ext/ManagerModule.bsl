...
Процедура ВывестиТабличныйДокументРасчетаСреднегоЗаработка(ДанныеДокумента, ОбъектыПечати, ТабличныйДокумент, ВыводитьЗаголовок, НомерТаблицыРазделителя)
	...
	ОбластиМакета = ОбластиМакета(
		ДанныеДокумента.ПараметрыРасчета.ИспользоватьДниБолезниУходаЗаДетьми,
		ДанныеДокумента.ПараметрыРасчета.ПрименятьПредельнуюВеличину И НЕ ДанныеДокумента.ПараметрыРасчета.НеполныйРасчетныйПериод И НЕ ДанныеДокумента.ПараметрыРасчета.ПрименяетсяФЗ_20_2023);
	...
	Если ДанныеДокумента.ПараметрыРасчета.НеполныйРасчетныйПериод
...
Процедура ВывестиТаблицуЗаработка(ТабличныйДокумент, ОбластиМакета, ДанныеДокумента)
	...
	Если ПараметрыРасчета.УчитыватьДниПриостановленияТД Тогда
		...
		Если РасчетныйГодМакс >= 2022
			...
			Или (РасчетныйГодМакс = 0 И Год(ДанныеДокумента.ПараметрыРасчета.ДатаНачалаСобытия) >= 2023) Тогда
