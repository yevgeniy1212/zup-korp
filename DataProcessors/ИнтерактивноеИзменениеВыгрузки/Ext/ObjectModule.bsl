...
Функция СоставУкрупненнойГруппыМетаданных(ПолноеИмяМетаданных) Экспорт
	...
	ИначеЕсли ПолноеИмяМетаданных = ИдентификаторВсехДокументов() Тогда
		...
		// ЗУПК
		ИсключитьОтключенныеДокументы(ТаблицыСостава);
		// ЗУПК
...
// ЗУПК
Процедура ИсключитьОтключенныеДокументы(ТаблицыСостава)
	
	Если ПолучитьФункциональнуюОпцию("ИспользоватьРезервыПоОплатеТруда") = Ложь Тогда
		Индекс = ТаблицыСостава.Найти("Документ.РезервыПоОплатеТруда");
		Если Индекс <> Неопределено Тогда
			ТаблицыСостава.Удалить(Индекс);
		КонецЕсли;
	КонецЕсли;
	Если ПолучитьФункциональнуюОпцию("ИспользоватьВедомостиДляВыплатыПрочихДоходов") = Ложь Тогда
		Индекс = ТаблицыСостава.Найти("Документ.ВедомостьПрочихДоходовВКассу");
		Если Индекс <> Неопределено Тогда
			ТаблицыСостава.Удалить(Индекс);
		КонецЕсли;
	КонецЕсли;
	Если ПолучитьФункциональнуюОпцию("ИспользоватьРегистрациюПрочихВыплат") = Ложь Тогда
		Индекс = ТаблицыСостава.Найти("Документ.ВедомостьНаВозвратПрочихВыплат");
		Если Индекс <> Неопределено Тогда
			ТаблицыСостава.Удалить(Индекс);
		КонецЕсли;
		Индекс = ТаблицыСостава.Найти("Документ.РегистрацияПрочихВыплат");
		Если Индекс <> Неопределено Тогда
			ТаблицыСостава.Удалить(Индекс);
		КонецЕсли;
	КонецЕсли;
