...
Процедура ВыгрузитьЖурналДляПередачиВТехподдержку(Команда)
	...
	ПараметрыСохранения.Диалог.Фильтр = НСтр("ru = 'Данные журнала обмена'") + "(*.txt)|*.txt";
	ПараметрыСохранения.Диалог.ПолноеИмяФайла = "Log";
	ФайловаяСистемаКлиент.СохранитьФайл(Неопределено, ВыгрузкаЖурналаРегистрации(), "Log.txt", ПараметрыСохранения);
