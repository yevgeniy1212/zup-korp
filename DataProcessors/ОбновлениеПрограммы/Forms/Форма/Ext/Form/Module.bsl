...
&НаКлиенте
Процедура ДекорацияЗагрузитьДистрибутивКомпонентаПлатформыОбработкаНавигационнойСсылки(
	Элемент,
	НавигационнаяСсылкаФорматированнойСтроки,
	СтандартнаяОбработка)
...
&НаКлиенте
Процедура ДекорацияИнструкцияУстановкаЗавершенаОбработкаНавигационнойСсылки(
	Элемент,
	НавигационнаяСсылкаФорматированнойСтроки,
	СтандартнаяОбработка)
...
Процедура ОтобразитьИнформациюОДоступномОбновлении(ЗаполнитьНачальныеНастройки = Ложь)
	...
	Иначе
		...
		Элементы.НадписьЕстьРасширения.Видимость = (ЭтоАдминистраторСистемы
			И ДоступноОбновлениеКонфигурации
...
Процедура ЗаполнитьНачальныеНастройкиИОтображениеУстановкиКомпонентов()
	...
	ОбновитьПлатформу = ОбновлениеПлатформы <> Неопределено
		И Не ПустаяСтрока(ОбновлениеПлатформы.Версия)
...
Процедура ОтобразитьКраткуюИнформациюОДоступномОбновлении()
	...
	Если (ОбновлениеКомКонф = Неопределено
		...
		Или Не ОбновлениеКомПл.ДоступноОбновление) Тогда
...
Процедура ПриОбновленииПрогрессаПолученияИУстановкиОбновлений(Прогресс, ДополнительныеПараметры) Экспорт
	...
	ОписательСостояния = Неопределено;
	Если Не СтруктураПрогресса.Свойство("ДополнительныеПараметры", ОписательСостояния) Тогда
		Возврат;
	КонецЕсли;
	
	КонтекстОбновления = Неопределено;
	Если Не ОписательСостояния.Свойство("ДопПараметры", КонтекстОбновления)
		Или КонтекстОбновления = Неопределено Тогда
		Возврат;
	КонецЕсли;
...
Процедура ПриЗавершенииПолученияИУстановкиОбновлений(Результат, ДополнительныеПараметры) Экспорт
	...
	Если ОписательСостояния.КодСостояния = "Ошибка" Тогда
		...
		Если КонтекстОбновления.Свойство("СообщениеТехподдержке")
			И Не ПустаяСтрока(КонтекстОбновления.СообщениеТехподдержке) Тогда
			Вложения.Добавить(
				Новый Структура(
					"Представление, ВидДанных, Данные",
					НСтр("ru = 'Информация об ошибках.txt'"),
...
Процедура ОтобразитьВнутреннююОшибку(
	СообщениеОбОшибке,
	ЗаголовокКнопкиДалее = Неопределено,
	ПодробноеОписаниеВнутрОшибки = "")
	...
	Если ВРег(Лев(Сообщение, 6)) <> "<BODY>" Тогда
		...
		ПредставлениеHTML =
			ИнтернетПоддержкаПользователейКлиентСервер.ПодставитьДомен(
				...
				ИнтернетПоддержкаПользователей.НастройкиСоединенияССерверами().ДоменРасположенияСерверовИПП);
		Элементы.ДекорацияСообщениеСервиса.Заголовок =
			ИнтернетПоддержкаПользователейКлиентСервер.ФорматированныйЗаголовок(ПредставлениеHTML);
	Иначе
		Элементы.ДекорацияСообщениеСервиса.Заголовок =
			ИнтернетПоддержкаПользователейКлиентСервер.ФорматированныйЗаголовок(Сообщение);
	КонецЕсли;
...
Функция ТекстИнструкцииПоПереходуНаНовуюВерсиюПлатформы(ПараметрСоздатьРезервнуюКопию)
	...
	Если ВнедренаПодсистемаРезервногоКопирования
		...
		Если ЭтоБазоваяВерсияКонфигурации Тогда
			Строки.Добавить(
				НСтр("ru = 'информационной базы и запущен сеанс работы с программой на новой версии платформы.<body>'"));
		Иначе
			Строки.Добавить(
	...
	Иначе
		...
		Если ВнедренаПодсистемаРезервногоКопирования Тогда
			Строки.Добавить(
				НСтр("ru = '<br />Перед началом работы на новой версии платформы рекомендуется создать резервную копию.</body>'"));
		Иначе
			Строки.Добавить(
				НСтр("ru = '<br />Перед началом работы на новой версии платформы рекомендуется <a href=""open:RCInstruction"">создать резервную копию</a>.</body>'"));
		КонецЕсли;
		
	КонецЕсли;
	
	Возврат ИнтернетПоддержкаПользователейКлиентСервер.ФорматированныйЗаголовок(
		СтрСоединить(Строки, Символы.ПС + "<br />"));
...
&НаСервере
Процедура ПриИзмененииРасписанияУстановкиИсправленийНаСервере(Расписание)
...
&НаСервере
Функция ИнициализацияПолученияОбновленийНаСервере(ПараметрыКлиентаИПП)
	...
	Иначе
		...
		Если УстановитьИсправления Тогда
			ИдентификаторыПолучаемыхИсправлений = ?(
				ОбновитьКонфигурацию,
				ВыбранныеИсправленияНоваяВерсия,
				ВыбранныеИсправленияТекущаяВерсия).ВыгрузитьЗначения();
