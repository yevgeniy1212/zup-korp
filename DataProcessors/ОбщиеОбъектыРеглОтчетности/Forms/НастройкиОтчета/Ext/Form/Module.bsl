...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	// ЗУПК
	мПараметры.Вставить("СписокСтруктурныхЕдиниц",               Ложь);
	// ЗУПК
    ...
    // ЗУПК
	Если мПараметры.СписокСтруктурныхЕдиниц Тогда
		Элементы.ГруппаСтруктурныеЕдиницы.Видимость = Истина;
		ЭтаФорма["СтруктурныеЕдиницы"] = Параметры.СтруктурныеЕдиницы;
		ЭтаФорма["мСписокСтруктурныхЕдиниц"] = Параметры.СтруктурныеЕдиницы;
		Если мПараметры.РежимВыбораСтруктурныхЕдиниц = "ПоНалогоплательщику" Тогда
			ЭтаФорма["НалоговыйКомитет"] = Параметры.НалоговыйКомитет;
		КонецЕсли; 
	КонецЕсли;
    // ЗУПК
    ...
    // ЗУПК
	ИначеЕсли Параметры.мПараметры.Свойство("ИмяОтчета")
		И ОсновнаяФормаОтчетностьРКСервер(ИмяОсновнойФормыРК,Параметры.мПараметры.ИмяОтчета) Тогда
		
		мПараметры.Вставить("ИмяОтчета", Параметры.мПараметры.ИмяОтчета);
		ЭтаФорма.УсловноеОформление.Элементы.Получить(0).Отбор.Элементы[0].ПравоеЗначение.Добавить(ИмяОсновнойФормыРК);
    // ЗУПК
...
Процедура ПриИзмененииФлажка(Элемент)
	...
	ИначеЕсли ТекЭлемент = "ВыгрузитьСтраницу" Тогда
        ...
        // ЗУПК
        Если РазрешитьНастройкуВыгрузкиОтчетов(мПараметры.ИмяОтчета) Тогда
	...
	ИначеЕсли ТекКолонка = "ВыводНаПечать" Тогда
		...
		Для Каждого СтрокаУровня1 Из Дерево.ПолучитьЭлементы() Цикл
			...
			Если СтрокаУровня1.ИмяСтраницы = "Титульный"
                ...
                ИЛИ ОсновнаяФормаОтчетностьРККлиет(СтрокаУровня1)
...
Процедура СохранитьДанные()
	...
	// ЗУПК
	Если мПараметры.СписокСтруктурныхЕдиниц Тогда
		ВладелецФормы.мСписокСтруктурныхЕдиниц = ЭтаФорма["мСписокСтруктурныхЕдиниц"];
	КонецЕсли;
	// ЗУПК
...
// ЗУПК
&НаКлиенте
Функция ОсновнаяФормаОтчетностьРККлиет(СтрокаУровня1)
	
	Итог = Ложь;
	
	Если (СтрЧислоВхождений(СтрокаУровня1.ИмяСтраницы,"_")) = 0  Тогда
		
		Если (СтрЧислоВхождений(мПараметры.ИмяОтчета, "РегламентированныйОтчет300Форма") > 0)
			ИЛИ (СтрЧислоВхождений(мПараметры.ИмяОтчета, "РегламентированныйОтчет200Форма") > 0)
			ИЛИ (СтрЧислоВхождений(мПараметры.ИмяОтчета, "РегламентированныйОтчет210Форма") > 0)
			ИЛИ (СтрЧислоВхождений(мПараметры.ИмяОтчета, "РегламентированныйОтчет701Форма") > 0)
			ИЛИ (СтрЧислоВхождений(мПараметры.ИмяОтчета, "РегламентированныйОтчет70101Форма") > 0)
			ИЛИ (СтрЧислоВхождений(мПараметры.ИмяОтчета, "РегламентированныйОтчетСтатистика1ТКвартальная") > 0)
			ИЛИ (СтрЧислоВхождений(мПараметры.ИмяОтчета, "РегламентированныйОтчетСтатистика1ТЗП") > 0)
			ИЛИ (СтрЧислоВхождений(мПараметры.ИмяОтчета, "РегламентированныйОтчетСтатистика2ТПРОФ") > 0) 
			ИЛИ (СтрЧислоВхождений(мПараметры.ИмяОтчета, "РегламентированныйОтчетСтатистика1ТГодовая") > 0)
			ИЛИ (СтрЧислоВхождений(мПараметры.ИмяОтчета, "РегламентированныйОтчетСтатистика2ТОплатаТруда") > 0)
			ИЛИ (СтрЧислоВхождений(мПараметры.ИмяОтчета, "РегламентированныйОтчетВУСписокПризывников") > 0)
			ИЛИ (СтрЧислоВхождений(мПараметры.ИмяОтчета, "РегламентированныйОтчетВУСписокПринятыхУволенных") > 0)
			ИЛИ (СтрЧислоВхождений(мПараметры.ИмяОтчета, "РегламентированныйОтчетВУСведенияОЧисленностиПризывников") > 0) Тогда
...
&НаСервере
Функция ОсновнаяФормаОтчетностьРКСервер(ИмяОсновнойФормы,ИмяОтчета)
	
	ОтчетРК = Ложь;
	
	Если (СтрЧислоВхождений(ИмяОтчета, "РегламентированныйОтчет300Форма") > 0) Тогда
		
		ОтчетРК = Истина;
		ИмяОсновнойФормы = "Форма300Страница1";
		
	ИначеЕсли (СтрЧислоВхождений(ИмяОтчета, "РегламентированныйОтчет200Форма") > 0) Тогда
		
		ОтчетРК = Истина;
		ИмяОсновнойФормы = "Форма200Страница1";
		
	ИначеЕсли (СтрЧислоВхождений(ИмяОтчета, "РегламентированныйОтчет210Форма") > 0) Тогда
		
		ОтчетРК = Истина;
		ИмяОсновнойФормы = "Форма210Страница1";
		
	ИначеЕсли (СтрЧислоВхождений(ИмяОтчета, "РегламентированныйОтчет701Форма") > 0) Тогда
		
		ОтчетРК = Истина;
		ИмяОсновнойФормы = "Форма701Страница1";
		
	ИначеЕсли (СтрЧислоВхождений(ИмяОтчета, "РегламентированныйОтчет70101Форма") > 0) Тогда
		
		ОтчетРК = Истина;
		ИмяОсновнойФормы = "Форма70101Страница1";
		
	ИначеЕсли (СтрЧислоВхождений(ИмяОтчета, "РегламентированныйОтчетСтатистика1ТКвартальная") > 0) Тогда
		
		ОтчетРК = Истина;
		ИмяОсновнойФормы = "Форма1ТСтраница1";
		
	ИначеЕсли (СтрЧислоВхождений(ИмяОтчета, "РегламентированныйОтчетСтатистика1ТЗП") > 0) Тогда
		
		ОтчетРК = Истина;
		ИмяОсновнойФормы = "Форма1ТСтраница1";
		
	ИначеЕсли (СтрЧислоВхождений(ИмяОтчета, "РегламентированныйОтчетСтатистика2ТПРОФ") > 0) Тогда
		
		ОтчетРК = Истина;
		ИмяОсновнойФормы = "Форма2ТСтраница1";
		
	ИначеЕсли (СтрЧислоВхождений(ИмяОтчета, "РегламентированныйОтчетСтатистика1ТГодовая") > 0) Тогда
		
		ОтчетРК = Истина;
		ИмяОсновнойФормы = "Форма1ТСтраница1";
	
	ИначеЕсли (СтрЧислоВхождений(ИмяОтчета, "РегламентированныйОтчетСтатистика2ТОплатаТруда") > 0) Тогда
		
		ОтчетРК = Истина;
		ИмяОсновнойФормы = "Форма2ТСтраница1";
	
	ИначеЕсли (СтрЧислоВхождений(ИмяОтчета, "РегламентированныйОтчетВУСписокПризывников") > 0) Тогда
		
		ОтчетРК = Истина;
		ИмяОсновнойФормы = "ФормаСтраница1";
		
	ИначеЕсли (СтрЧислоВхождений(ИмяОтчета, "РегламентированныйОтчетВУСписокПринятыхУволенных") > 0) Тогда
		
		ОтчетРК = Истина;
		ИмяОсновнойФормы = "ФормаСтраница1";
		
	ИначеЕсли (СтрЧислоВхождений(ИмяОтчета, "РегламентированныйОтчетВУСведенияОЧисленностиПризывников") > 0) Тогда
...
Процедура ПослеВыбораСпискаСтруктурныхЕдиниц(Результат, Параметры) Экспорт
	...
	Если Результат <> Неопределено Тогда
		ЭтаФорма["СтруктурныеЕдиницы"] = Результат;
		ЭтаФорма["мСписокСтруктурныхЕдиниц"] = Результат;
		//мСписокРегистраций = Результат.СписокРегистраций;
	КонецЕсли;
	
КонецПроцедуры

&НаКлиенте
Процедура Подключаемый_СтруктурныеЕдиницыНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	
	СтандартнаяОбработка = Ложь;
	
	ПараметрыФормы = Новый Структура;
	ПараметрыФормы.Вставить("НачальныйСписокСтруктурныхЕдиниц", ЭтаФорма["мСписокСтруктурныхЕдиниц"]);
	ПараметрыФормы.Вставить("Налогоплательщик", ВладелецФормы.СтруктураРеквизитовФормы.Организация);
	ПараметрыФормы.Вставить("мДатаКонцаПериодаОтчета",ВладелецФормы.СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета);
	ПараметрыФормы.Вставить("РежимВыбораСтруктурныхЕдиниц", мПараметры.РежимВыбораСтруктурныхЕдиниц);  
	ПараметрыФормы.Вставить("НалоговыйКомитет", ЭтаФорма["НалоговыйКомитет"]);
	
	ОписаниеОповещения = Новый ОписаниеОповещения("ПослеВыбораСпискаСтруктурныхЕдиниц",ЭтаФорма);
		
	ОткрытьФорму("Обработка.ОбщиеОбъектыРеглОтчетности.Форма.ФормаВыбораСтруктурныхЕдиниц", ПараметрыФормы,,,,, ОписаниеОповещения);
	
КонецПроцедуры

&НаСервереБезКонтекста
Функция РазрешитьНастройкуВыгрузкиОтчетов(ИмяОтчета) 
    
    Возврат РегламентированнаяОтчетностьРК.РазрешитьНастройкуВыгрузкиОтчетов(ИмяОтчета);
