...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Не ТолькоПросмотр И ОбщегоНазначения.ПодсистемаСуществует("ЗарплатаКадрыКорпоративнаяПодсистемы.ФинансовыйУчет") Тогда 
		МодульФинансовыйУчет = ОбщегоНазначения.ОбщийМодуль("ФинансовыйУчет");
		ТолькоПросмотр = Не МодульФинансовыйУчет.МожноРедактироватьФинансовыйУчет();
	КонецЕсли; 
	...
	Элементы.НаборЗаписейАналитикаРасходов.ОграничениеТипа = Новый ОписаниеТипов("СправочникСсылка.Проекты");
...
Процедура КомандаОК(Команда)
	РедактированиеПериодическихСведенийКлиент.ОповеститьОЗавершении(ЭтаФорма, "ФинансовыйУчетПоУмолчанию", СтруктураОбъектовВладельцев);
КонецПроцедуры
