...
Процедура ПриДобавленииКлассификаторов(Классификаторы) Экспорт
    ...
    // ЗУПК
    Описание.Наименование = НСтр("ru = 'Классификатор банков'");
    // ЗУПК
...
Процедура ПриЗагрузкеКлассификатора(Идентификатор, Версия, Адрес, Обработан, ДополнительныеПараметры) Экспорт
	...
	ЧтениеJSON.Закрыть();
	ЗагрузитьДанныеИзФайла(ДанныеКлассификатора);
...
Процедура ЗагрузитьДанные(Параметры)
	...
	КонецЦикла;
	Иначе
		Для Каждого ДанныеОрганизации Из Параметры.ДанныеКлассификатора.response Цикл 
			АктуальныеЗаписи.Добавить(ЗаписатьЭлементКлассификатораБанковРФ(ДанныеОрганизации));
		КонецЦикла;
	КонецЕсли;
	// Классификатор банков загружается полностью только во фреше  
...
Процедура ЗагрузитьДанныеИзФайла(ИмяФайла)
	...
	Если ИмяФайла.Свойство("response") Тогда
		Параметры = Новый Структура;
		Параметры.Вставить("ДанныеКлассификатора", ИмяФайла);
		Параметры.Вставить("ТекстСообщения", "");
		Параметры.Вставить("ЗагрузкаВыполнена", Неопределено);
		ЗагрузитьДанные(Параметры);
		УстановитьВерсиюКлассификатора();
...
Функция ЗаписатьЭлементКлассификатораБанковРФ(СведенияОБанке)
	...
	Попытка
		...
		КонецЕсли;
		БанкСсылка = НайтиБанк(СведенияОБанке.bankBIK, "");
		// ЗУПК
		...
		// ЗУПК
		Если БанкОбъект <> Неопределено Тогда
		...
		Если БанкОбъект.Код <> СведенияОБанке.bankBIK Тогда
			БанкОбъект.Код = СведенияОБанке.bankBIK;
		КонецЕсли;

		ОбновитьЗначениеРеквизита(БанкОбъект, "Наименование",
			БанкОбъект.Метаданные().СтандартныеРеквизиты.Наименование.Тип.ПривестиЗначение(СведенияОБанке.bankName));
