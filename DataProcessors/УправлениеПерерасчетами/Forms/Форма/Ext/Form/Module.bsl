...
Процедура ТаблицаЗарплатаВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	ИначеЕсли Поле.Имя = "ТаблицаЗарплатаПричинаПредставление" Тогда
...
Процедура Исправить(Команда)
	...
	Если ТекущиеДанные <> Неопределено Тогда
		Если Не ИсправлениеПересчетДоступны(ТекущиеДанные.Документ) Тогда
			Возврат;
		КонецЕсли;
		ИсправлениеДокументовЗарплатаКадрыКлиент.ИсправитьДокумент(ТекущиеДанные.Документ, ИмяОбъектаМетаданных(ТекущиеДанные.Документ));
	КонецЕсли;
		...
		Для каждого ИдентификаторУдаляемойСтроки Из УдаляемыеСтроки Цикл
			...
			Если ТекущиеДанные <> Неопределено Тогда
				...
				Если ОбщегоНазначения.ПодсистемаСуществует("ЗарплатаКадрыПриложения.ГосударственнаяСлужба") Тогда
					МодульГосударственнаяСлужба = ОбщегоНазначения.ОбщийМодуль("РасчетДенежногоДовольствия");
					МодульГосударственнаяСлужба.УдалитьПерерасчетыТаблицыДенежногоДовольствия(Организация, ТекущиеДанные.Документ, ТекущиеДанные.Сотрудник);
				КонецЕсли;
			КонецЕсли; 
...
Процедура ЗаполнитьПерерасчетыЗарплаты(Сотрудник = Неопределено, Период = Неопределено)
	...
	Запрос.Текст =
		...
		|	МАКСИМУМ(ДанныеСотрудников.Сотрудник),
		...
		|		ПО ПерерасчетУдержаний.ФизическоеЛицо = ДанныеСотрудников.ФизическоеЛицо
		|			И ПерерасчетУдержаний.Организация.ГоловнаяОрганизация = ДанныеСотрудников.ГоловнаяОрганизация
		|			И (ПерерасчетУдержаний.ПериодДействия МЕЖДУ ДанныеСотрудников.ДатаНачала И ДанныеСотрудников.ДатаОкончания)
		|ГДЕ
		...
		Пока Выборка.СледующийПоЗначениюПоля("Сотрудник") Цикл
			...
			Пока Выборка.СледующийПоЗначениюПоля("ПериодДействия") Цикл
				...
				Пока Выборка.СледующийПоЗначениюПоля("ДокументНачисления") Цикл
					...
					УстановитьПредставлениеПричинПерерасчета(НоваяСтрока);
...
Процедура ЗаполнитьПерерасчетыСреднегоЗаработка()
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("ЗарплатаКадрыПриложения.ГосударственнаяСлужба") Тогда
		МодульГосударственнаяСлужба = ОбщегоНазначения.ОбщийМодуль("РасчетДенежногоДовольствия");
		МодульГосударственнаяСлужба.ЗаполнитьПерерасчетыДенежногоДовольствия(ТаблицаСреднийЗаработок, Организация);
	КонецЕсли;
...
Процедура ОткрытьФормуОбъекта(СсылкаНаОбъект, ПараметрыОткрытия = Неопределено)
	...
	ОткрытьФорму(ПолноеИмяОбъектаМетаданных(СсылкаНаОбъект) + ".ФормаОбъекта", ПараметрыОткрытия, ЭтаФорма);
...
&НаСервереБезКонтекста
Функция ИсправлениеПересчетДоступны(Ссылка)
	
	Отказ = Ложь;
	ИсправлениеДокументовЗарплатаКадры.ПроверитьНаличиеИсправленияСторнирования(Ссылка,,Отказ);
	Возврат Не Отказ;
...
&НаСервереБезКонтекста
Функция ПолноеИмяОбъектаМетаданных(Ссылка)
...
&НаСервере
Процедура УстановитьПредставлениеПричинПерерасчета(СтрокаПерерасчета)

	МассивПричинПерерасчета = ОбщегоНазначения.ВыгрузитьКолонку(СтрокаПерерасчета.Причина, "Представление");
	СтрокаПерерасчета.ПричинаПредставление = СтрСоединить(МассивПричинПерерасчета, "; ");
