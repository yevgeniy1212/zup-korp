...
Процедура УстановитьЗащитуИЦветФонаОбластейАвторассчитываемыхПоказателей()
	...
	Для Гр = 1 По 5 Цикл
		...
		мФормаОтчета.Области["П0000100001" + Графа].ЦветФона = ЦветФонаЗначение;
		мФормаОтчета.Области["П0000100001" + Графа].Защита   = Не ОтклАвтоРасчетЗначение;
		
		мФормаОтчета.Области["П0000100012" + Графа].ЦветФона = ЦветФонаЗначение;
		мФормаОтчета.Области["П0000100012" + Графа].Защита   = Не ОтклАвтоРасчетЗначение;
	КонецЦикла;
...
Процедура Расчет(Форма, ИмяТекущейСтраницыПанели = "", ОбластьИмя = "")
	...
	Для Гр = 1 По 5 Цикл
		...
		Области["П0000100012" + Графа].Значение
			= Области["П0000100013" + Графа].Значение
			+ Области["П0000100016" + Графа].Значение
			+ Области["П0000100017" + Графа].Значение
			+ Области["П0000100018" + Графа].Значение;
		
		Области["П0000100001" + Графа].Значение
			= Области["П0000100002" + Графа].Значение
			+ Области["П0000100004" + Графа].Значение
			+ Области["П0000100006" + Графа].Значение
			+ Области["П0000100008" + Графа].Значение
			+ Области["П0000100010" + Графа].Значение
			+ Области["П0000100012" + Графа].Значение;
...
Функция УниверсальнаяВыгрузкаXMLПроверкаДанныхНаСервере(ТекстВыгрузки, ПараметрыВыгрузки) Экспорт
	...
	РегламентированнаяОтчетность.ИзменитьВерсиюФорматаСПроверкойСовместимостиШаблонов(
		ЭтотОбъект, СтруктураРеквизитовФормы["ИмяМакетаАтрибутовВыгрузки"], СтруктураРеквизитовФормы["ИмяМакетаШаблона"], ПараметрыВыгрузки);
	ЗаполнитьДанными(ДеревоВыгрузки, ПараметрыВыгрузки);
	Если НЕ РегламентированнаяОтчетность.ПроверитьДанные(ЭтотОбъект, ДеревоВыгрузки) Тогда
		КоллекцияСписковВыбора = "";
		Возврат Ложь;
...
Функция ПечатьНаСервере(ВидПечати)
	...
	СтруктураРеквизитовФормы.мПечатныеФормы.Добавить(ПечатнаяФорма, "Форма ЗП-физическая культура и спорт");
