...
Процедура ПодготовитьДанныеОтчетаПоШаблону(ДанныеШаблонаОтчета, Отказ = Ложь, ЗагружатьСписки = Истина);
	...
	Исключение
		...
		ОбщегоНазначения.СообщитьПользователю(НСтр("ru='Не удалось получить текст XML-шаблона:%1'"), Символы.ПС + ОписаниеОшибки());
		Отказ = Истина;
		Возврат;
...
Процедура ПрименитьПараметрыЯчейки(Форма, ЦелеваяЯчейка, ПараметрыЯчейки, ПараметрыКолонки, ПараметрыСтроки, ЭлементДереваСтрокаРаздела)
	...
	ИначеЕсли ПараметрыЯчейки["inputType"] = 1 Тогда // обязательное заполнение
		...
		Если ЦелеваяЯчейка.ТипЗначения.СодержитТип(Тип("Число"))
			...
			И Форма.РасчитываемыеЯчейки.НайтиСтроки(Новый Структура("ЗависимоеЗначение", ЦелеваяЯчейка.Имя)).Количество() = 1 Тогда 
...
Процедура ПриОткрытии(Отказ)
	...
	Если ГодОтчетаНеВСпискеШаблона Тогда
		ПериодГод = Формат(Год(СтруктураРеквизитовФормы.мДатаНачалаПериодаОтчета), "ЧГ=");
		ОО = Новый ОписаниеОповещения("ПриОткрытииВопросНеактуальныйГодЗавершение", ЭтотОбъект, Неопределено);
		ПоказатьВопрос(ОО, "Данный шаблон не применим для " + ПериодГод + " года. Продолжить?", РежимДиалогаВопрос.ДаНет);
		Возврат;
	Иначе
		ПриОткрытииВопросНеактуальныйГодЗавершение(КодВозвратаДиалога.Да, Неопределено);
	КонецЕсли;
КонецПроцедуры

&НаКлиенте
Процедура ПриОткрытииВопросНеактуальныйГодЗавершение(РезультатВопроса, ДополнительныеПараметры) Экспорт
