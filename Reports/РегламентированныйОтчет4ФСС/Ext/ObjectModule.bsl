...
Функция СформироватьАвтоматическиРеглОтчет(ПараметрыАвтоформирования, КорректирующийОтчет) Экспорт
	...
	Если ВРег(СокрЛП(ИмяФормы)) = ВРег("ФормаОтчета2022Кв2") Тогда
		
		РегламентированнаяОтчетностьКлиентСервер.ИзменитьВариантыЗаполненияПоказателейТаблицы2Формы4ФСС(
		ДанныеРеглОтчета.СтруктураВариантыЗаполнения.Таблица2[0], ДанныеРеглОтчета.СтруктураРеквизитовФормы);
		
	ИначеЕсли ВРег(СокрЛП(ИмяФормы)) = ВРег("ФормаОтчета2017Кв3") Тогда
		
		РегламентированнаяОтчетностьКлиентСервер.ИзменитьВариантыЗаполненияПоказателейТаблицы1_1Формы4ФСС(
		ДанныеРеглОтчета.СтруктураВариантыЗаполнения.Таблица1_1[0], ДанныеРеглОтчета.СтруктураРеквизитовФормы);
		
		РегламентированнаяОтчетностьКлиентСервер.ИзменитьВариантыЗаполненияПоказателейТаблицы91Формы4ФСС(
		ДанныеРеглОтчета.СтруктураВариантыЗаполнения.Таблица91[0], ДанныеРеглОтчета.СтруктураРеквизитовФормы);
...
Процедура СформироватьСтруктуруРеквизитовФормы(ИмяФормы, СтруктураРеквизитовФормы) Экспорт
	...
	Если ВРег(СокрЛП(ИмяФормы)) = ВРег("ФормаОтчета2022Кв2") Тогда
		...
		СтруктураРеквизитовФормы.Вставить("мВерсияФормы", "11/06/2022");
		
		СтруктураРеквизитовФормы.Вставить("ИдГруппы92", "П0009200001");
		СтруктураРеквизитовФормы.Вставить("ИдГруппы11", "П0001100001");
		СтруктураРеквизитовФормы.Вставить("ИдГруппы2",  "П0000200001");
	...
	ИначеЕсли ВРег(СокрЛП(ИмяФормы)) = ВРег("ФормаОтчета2017Кв3") Тогда
		...
		СтруктураРеквизитовФормы.Вставить("ИдГруппы91",       "П0009100001");
		
		СтруктураРеквизитовФормы.Вставить("ВариантЗаполненияП000910000107", "0");
		СтруктураРеквизитовФормы.Вставить("ВариантЗаполненияП000910000108", "0");
...
Функция ЗаписатьДокумент(ИмяФормы, ПараметрыЗаписи) Экспорт
	...
	КПП = Неопределено;
	Если ПараметрыЗаписи.РазделыОтчета.Свойство("Титульный")
		И ПараметрыЗаписи.РазделыОтчета.Титульный.Свойство("Данные") Тогда
		ДанныеТитульногоЛиста = ПараметрыЗаписи.РазделыОтчета.Титульный.Данные;
		Если ДанныеТитульногоЛиста.Свойство("КПП", КПП) И ТипЗнч(КПП) = Тип("Строка") Тогда
			ДокументРегламентированныйОтчет.КПП = СокрЛП(КПП);
		КонецЕсли;
	КонецЕсли;
...
Процедура УстановитьРазмерСтраховогоТарифа(ИмяФормы, СтруктураРеквизитовФормы, СтруктураДанныхТаблица1) Экспорт
	...
	Если (СтруктураРеквизитовФормы.мВПрограммеИзмененаОрганизация)
		...
		Если ВРег(СокрЛП(ИмяФормы)) = ВРег("ФормаОтчета2022Кв2") Тогда
			...
			СтруктураДанныхТаблица1.П000010000803 = СтруктураДанныхТаблица1.П000010000503;
			
		ИначеЕсли ВРег(СокрЛП(ИмяФормы)) = ВРег("ФормаОтчета2017Кв3")
	...
	Для Инд = 0 По МакетСоставаПоказателей.Области.Количество() - 1 Цикл
		...
		Для Ном = ТекОбласть.Верх По ТекОбласть.Низ Цикл
			...
			Если чВариантЗаполнения <> 0
				ИЛИ ИмяСтраницы = "Титульный" ИЛИ (ВРег(СокрЛП(ИмяФормы)) = ВРег("ФормаОтчета2022Кв2")
				И (ИмяСтраницы = "Таблица92" ИЛИ ИмяСтраницы = "Таблица4")) Тогда
...
Процедура ПоказатьПериод(ИмяФормы, СтруктураДанныхТитульный, СтруктураРеквизитовФормы) Экспорт
	...
	Если ТипЗнч(СтруктураДанныхТитульный) = Тип("ТабличныйДокумент") Тогда
		...
		Иначе// "ФормаОтчета2017Кв3", "ФормаОтчета2017Кв1"
...
Функция СтруктураСтраницОтчета(ИмяФормы) Экспорт
	...
	Если ВРег(СокрЛП(ИмяФормы)) = ВРег("ФормаОтчета2022Кв2") Тогда
		...
		СтрокаУровня1.Представление      = "Платежи";
		СтрокаУровня1.ОриентацияСтраницы = "Портрет";
	...
	Иначе// "ФормаОтчета2017Кв3", "ФормаОтчета2017Кв1"
		...
		Если ВРег(СокрЛП(ИмяФормы)) = ВРег("ФормаОтчета2017Кв3") Тогда
			...
			СтрокаУровня1.Представление      = "Самостоятельные классификационные единицы / источники финансирования";
			СтрокаУровня1.ОриентацияСтраницы = "Ландшафт";
			СтрокаУровня1.ВыводНаПечать      = 0;
			СтрокаУровня1.ПоказатьСтраницу   = 1;
...
Процедура ЗаполнитьСведенияОбОрганизации(ИмяФормы,
	...
	Если НЕ ВПрограммеИзмененаДатаПодписи Тогда
		...
		Сведения.Добавить("", "НомерСтрахователяФСС");
	...
	Если НЕ ВПрограммеИзмененаДатаПодписи Тогда
		...
		ОргСведения.Свойство("НомерСтрахователяФСС", РегистрационныйНомерФСС);
		Если НЕ ЗначениеЗаполнено(РегистрационныйНомерФСС) Тогда
...
Функция РазделыРеглОтчета(ИмяФормы) Экспорт
	...
	Если ВРег(СокрЛП(ИмяФормы)) = ВРег("ФормаОтчета2022Кв2") Тогда
		...
		ДанныеДопСтрок.Вставить("Таблица92", Новый ТаблицаЗначений);
		ДанныеДопСтрок.Таблица92.Колонки.Добавить("П000920000101",
			Новый ОписаниеТипов("Число", Новый КвалификаторыЧисла(6, 0, ДопустимыйЗнак.Любой)));
		ДанныеДопСтрок.Таблица92.Колонки.Добавить("П000920000102",
			Новый ОписаниеТипов("Строка", , Новый КвалификаторыСтроки(10)));
		ДанныеДопСтрок.Таблица92.Колонки.Добавить("П000920000103",
			Новый ОписаниеТипов("Дата", , , Новый КвалификаторыДаты(ЧастиДаты.Дата)));
		ДанныеДопСтрок.Таблица92.Колонки.Добавить("П000920000104",
		...
		ДанныеДопСтрок.Таблица1_1.Колонки.Добавить("П000110000115",
			Новый ОписаниеТипов("Число", Новый КвалификаторыЧисла(16, 2, ДопустимыйЗнак.Любой)));
		ДанныеДопСтрок.Таблица1_1.Колонки.Добавить("П000110000116",
			Новый ОписаниеТипов("Число", Новый КвалификаторыЧисла(16, 2, ДопустимыйЗнак.Любой)));
		ДанныеДопСтрок.Таблица1_1.Колонки.Добавить("П000110000117",
			Новый ОписаниеТипов("Число", Новый КвалификаторыЧисла(16, 2, ДопустимыйЗнак.Любой)));
		ДанныеДопСтрок.Таблица1_1.Колонки.Добавить("П000110000118",
			Новый ОписаниеТипов("Число", Новый КвалификаторыЧисла(16, 2, ДопустимыйЗнак.Любой)));
		ДанныеДопСтрок.Таблица1_1.Колонки.Добавить("П000110000119",
			Новый ОписаниеТипов("Число", Новый КвалификаторыЧисла(16, 2, ДопустимыйЗнак.Любой)));
		ДанныеДопСтрок.Таблица1_1.Колонки.Добавить("П000110000120",
			Новый ОписаниеТипов("Число", Новый КвалификаторыЧисла(16, 2, ДопустимыйЗнак.Любой)));
		ДанныеДопСтрок.Таблица1_1.Колонки.Добавить("П000110000121",
			Новый ОписаниеТипов("Число", Новый КвалификаторыЧисла(16, 2, ДопустимыйЗнак.Любой)));
		ДанныеДопСтрок.Таблица1_1.Колонки.Добавить("П000110000122",
			Новый ОписаниеТипов("Число", Новый КвалификаторыЧисла(16, 2, ДопустимыйЗнак.Любой)));
		ДанныеДопСтрок.Таблица1_1.Колонки.Добавить("П000110000123",
			Новый ОписаниеТипов("Число", Новый КвалификаторыЧисла(16, 2, ДопустимыйЗнак.Любой)));
		ДанныеДопСтрок.Таблица1_1.Колонки.Добавить("П000110000124",
			Новый ОписаниеТипов("Число", Новый КвалификаторыЧисла(16, 2, ДопустимыйЗнак.Любой)));
		
		ДанныеДопСтрок.Вставить("Таблица2", Новый ТаблицаЗначений);
		ДанныеДопСтрок.Таблица2.Колонки.Добавить("П000020000101",
			Новый ОписаниеТипов("Число", Новый КвалификаторыЧисла(6, 0, ДопустимыйЗнак.Любой)));
		ДанныеДопСтрок.Таблица2.Колонки.Добавить("П000020000102",
			Новый ОписаниеТипов("Строка", , Новый КвалификаторыСтроки(10)));
		ДанныеДопСтрок.Таблица2.Колонки.Добавить("П000020000103",
			Новый ОписаниеТипов("Строка", , Новый КвалификаторыСтроки(12)));
		ДанныеДопСтрок.Таблица2.Колонки.Добавить("П000020000104",
			Новый ОписаниеТипов("Строка", , Новый КвалификаторыСтроки(9)));
		ДанныеДопСтрок.Таблица2.Колонки.Добавить("П000020000105",
			Новый ОписаниеТипов("Строка", , Новый КвалификаторыСтроки(8)));
		ДанныеДопСтрок.Таблица2.Колонки.Добавить("П000020000106",
			Новый ОписаниеТипов("Число", Новый КвалификаторыЧисла(6, 0, ДопустимыйЗнак.Неотрицательный)));
		ДанныеДопСтрок.Таблица2.Колонки.Добавить("П000020000107",
			Новый ОписаниеТипов("Число", Новый КвалификаторыЧисла(6, 0, ДопустимыйЗнак.Неотрицательный)));
		ДанныеДопСтрок.Таблица2.Колонки.Добавить("П000020000108",
			Новый ОписаниеТипов("Число", Новый КвалификаторыЧисла(16, 2, ДопустимыйЗнак.Любой)));
		ДанныеДопСтрок.Таблица2.Колонки.Добавить("П000020000109",
			Новый ОписаниеТипов("Число", Новый КвалификаторыЧисла(16, 2, ДопустимыйЗнак.Любой)));
		ДанныеДопСтрок.Таблица2.Колонки.Добавить("П000020000110",
			Новый ОписаниеТипов("Число", Новый КвалификаторыЧисла(16, 2, ДопустимыйЗнак.Любой)));
		ДанныеДопСтрок.Таблица2.Колонки.Добавить("П000020000111",
			Новый ОписаниеТипов("Число", Новый КвалификаторыЧисла(16, 2, ДопустимыйЗнак.Любой)));
		ДанныеДопСтрок.Таблица2.Колонки.Добавить("П000020000112",
			Новый ОписаниеТипов("Число", Новый КвалификаторыЧисла(16, 2, ДопустимыйЗнак.Любой)));
		ДанныеДопСтрок.Таблица2.Колонки.Добавить("П000020000113",
			Новый ОписаниеТипов("Число", Новый КвалификаторыЧисла(16, 2, ДопустимыйЗнак.Любой)));
		ДанныеДопСтрок.Таблица2.Колонки.Добавить("П000020000114",
			Новый ОписаниеТипов("Число", Новый КвалификаторыЧисла(16, 2, ДопустимыйЗнак.Любой)));
		ДанныеДопСтрок.Таблица2.Колонки.Добавить("П000020000115",
			Новый ОписаниеТипов("Число", Новый КвалификаторыЧисла(16, 2, ДопустимыйЗнак.Любой)));
		ДанныеДопСтрок.Таблица2.Колонки.Добавить("П000020000116",
			Новый ОписаниеТипов("Число", Новый КвалификаторыЧисла(3, 1, ДопустимыйЗнак.Неотрицательный)));
		ДанныеДопСтрок.Таблица2.Колонки.Добавить("П000020000117",
			Новый ОписаниеТипов("Число", Новый КвалификаторыЧисла(3, 1, ДопустимыйЗнак.Неотрицательный)));
		ДанныеДопСтрок.Таблица2.Колонки.Добавить("П000020000118",
			Новый ОписаниеТипов("Число", Новый КвалификаторыЧисла(3, 1, ДопустимыйЗнак.Неотрицательный)));
		ДанныеДопСтрок.Таблица2.Колонки.Добавить("П000020000119",
			Новый ОписаниеТипов("Число", Новый КвалификаторыЧисла(5, 3, ДопустимыйЗнак.Неотрицательный)));
		...
		РазделыОтчета.Вставить("Таблица92",
		Новый Структура("МногостраничностьВРазделе, Данные, ДанныеДопСтрок, ИдентификаторДопСтрок",
		Ложь, Новый Структура, ДанныеДопСтрок.Таблица92, "П0009200001"));
		...
		Новый Структура("МногостраничностьВРазделе, Данные, ДанныеДопСтрок, ИдентификаторДопСтрок",
		Ложь, Новый Структура, ДанныеДопСтрок.Таблица2, "П0000200001"));
		
		РазделыОтчета.Вставить("Таблица3", Новый Структура("МногостраничностьВРазделе, Данные", Ложь, Новый Структура));
		
		РазделыОтчета.Вставить("Таблица4", Новый Структура("МногостраничностьВРазделе, Данные", Ложь, Новый Структура));
	...
	Иначе// "ФормаОтчета2017Кв3", "ФормаОтчета2017Кв1"
		...
		Если ВРег(СокрЛП(ИмяФормы)) = ВРег("ФормаОтчета2017Кв3") Тогда
			...
			ДанныеДопСтрок.Вставить("Таблица91", Новый ТаблицаЗначений);
			
			ДанныеДопСтрок.Таблица91.Колонки.Добавить("П000910000101",
				Новый ОписаниеТипов("Число", Новый КвалификаторыЧисла(6, 0, ДопустимыйЗнак.Любой)));
			ДанныеДопСтрок.Таблица91.Колонки.Добавить("П000910000102",
				Новый ОписаниеТипов("Строка", , Новый КвалификаторыСтроки(8)));
			ДанныеДопСтрок.Таблица91.Колонки.Добавить("П000910000103",
				Новый ОписаниеТипов("Число", Новый КвалификаторыЧисла(2, 0, ДопустимыйЗнак.Неотрицательный)));
			ДанныеДопСтрок.Таблица91.Колонки.Добавить("П000910000104",
				Новый ОписаниеТипов("Число", Новый КвалификаторыЧисла(3, 1, ДопустимыйЗнак.Неотрицательный)));
			ДанныеДопСтрок.Таблица91.Колонки.Добавить("П000910000105",
				Новый ОписаниеТипов("Число", Новый КвалификаторыЧисла(2, 0, ДопустимыйЗнак.Неотрицательный)));
			ДанныеДопСтрок.Таблица91.Колонки.Добавить("П000910000106",
				Новый ОписаниеТипов("Число", Новый КвалификаторыЧисла(2, 0, ДопустимыйЗнак.Неотрицательный)));
			ДанныеДопСтрок.Таблица91.Колонки.Добавить("П000910000107",
				Новый ОписаниеТипов("Число", Новый КвалификаторыЧисла(16, 2, ДопустимыйЗнак.Любой)));
			ДанныеДопСтрок.Таблица91.Колонки.Добавить("П000910000108",
				Новый ОписаниеТипов("Число", Новый КвалификаторыЧисла(16, 2, ДопустимыйЗнак.Любой)));
			ДанныеДопСтрок.Таблица91.Колонки.Добавить("П000910000109",
				Новый ОписаниеТипов("Число", Новый КвалификаторыЧисла(16, 2, ДопустимыйЗнак.Любой)));
			ДанныеДопСтрок.Таблица91.Колонки.Добавить("П000910000110",
				Новый ОписаниеТипов("Число", Новый КвалификаторыЧисла(16, 2, ДопустимыйЗнак.Любой)));
			ДанныеДопСтрок.Таблица91.Колонки.Добавить("П000910000111",
				Новый ОписаниеТипов("Число", Новый КвалификаторыЧисла(16, 2, ДопустимыйЗнак.Любой)));
			ДанныеДопСтрок.Таблица91.Колонки.Добавить("П000910000112",
				Новый ОписаниеТипов("Число", Новый КвалификаторыЧисла(16, 2, ДопустимыйЗнак.Любой)));
			ДанныеДопСтрок.Таблица91.Колонки.Добавить("П000910000113",
				Новый ОписаниеТипов("Число", Новый КвалификаторыЧисла(16, 2, ДопустимыйЗнак.Любой)));
			ДанныеДопСтрок.Таблица91.Колонки.Добавить("П000910000114",
				Новый ОписаниеТипов("Число", Новый КвалификаторыЧисла(16, 2, ДопустимыйЗнак.Любой)));
			ДанныеДопСтрок.Таблица91.Колонки.Добавить("П000910000115",
				Новый ОписаниеТипов("Число", Новый КвалификаторыЧисла(16, 2, ДопустимыйЗнак.Любой)));
			ДанныеДопСтрок.Таблица91.Колонки.Добавить("П000910000116",
				Новый ОписаниеТипов("Число", Новый КвалификаторыЧисла(16, 2, ДопустимыйЗнак.Любой)));
			
			РазделыОтчета.Вставить("Таблица91",
				Новый Структура("МногостраничностьВРазделе, Данные, ДанныеДопСтрок, ИдентификаторДопСтрок",
								Ложь, Новый Структура, ДанныеДопСтрок.Таблица91, "П0009100001"));
...
Процедура ЗаполнитьРеглОтчет(ИмяФормы,
	...
	Если ВРег(СокрЛП(ИмяФормы)) = ВРег("ФормаОтчета2022Кв2")
		ИЛИ ВРег(СокрЛП(ИмяФормы)) = ВРег("ФормаОтчета2017Кв3") Тогда
		
		ПараметрыОтчета.Вставить("СообщенияПослеАвтозаполнения", ДанныеРеглОтчета.СтруктураРеквизитовФормы.СообщенияПослеАвтозаполнения);
	...
	Если ВРег(СокрЛП(ИмяФормы)) = ВРег("ФормаОтчета2022Кв2") Тогда
		...
		Контейнер.Вставить("Таблица92",  РазделыОтчета.Таблица92.Данные);
		Контейнер.Вставить("Таблица92_ДанныеДопСтрок", РазделыОтчета.Таблица92.ДанныеДопСтрок);
		...
		Контейнер.Вставить("Таблица2_ДанныеДопСтрок", РазделыОтчета.Таблица2.ДанныеДопСтрок);
		Контейнер.Вставить("Таблица3",   РазделыОтчета.Таблица3.Данные);
		Контейнер.Вставить("Таблица4",   РазделыОтчета.Таблица4.Данные);
	...
	Иначе// "ФормаОтчета2017Кв3", "ФормаОтчета2017Кв1"
		...
		Если ВРег(СокрЛП(ИмяФормы)) = ВРег("ФормаОтчета2017Кв3") Тогда
			...
			Контейнер.Вставить("Таблица91", РазделыОтчета.Таблица91.Данные);
			Контейнер.Вставить("Таблица91_ДанныеДопСтрок", РазделыОтчета.Таблица91.ДанныеДопСтрок);
	...
	Иначе
		...
		Если ВРег(СокрЛП(ИмяФормы)) = ВРег("ФормаОтчета2022Кв2") Тогда
			...
			Если РазделыОтчета.Таблица92.ДанныеДопСтрок.Количество() = 0 Тогда
				РазделыОтчета.Таблица92.ДанныеДопСтрок.Добавить();
			КонецЕсли;
			...
			Если РазделыОтчета.Таблица2.ДанныеДопСтрок.Количество() = 0 Тогда
				РазделыОтчета.Таблица2.ДанныеДопСтрок.Добавить();
			КонецЕсли;
		...
		Иначе// "ФормаОтчета2017Кв3", "ФормаОтчета2017Кв1"
			...
			Если ВРег(СокрЛП(ИмяФормы)) = ВРег("ФормаОтчета2017Кв3") Тогда
				...
				Если РазделыОтчета.Таблица91.ДанныеДопСтрок.Количество() = 0 Тогда
					РазделыОтчета.Таблица91.ДанныеДопСтрок.Добавить();
				КонецЕсли;
		...
		Для Каждого СтраницаОтчета Из ДанныеРеглОтчета.ДеревоСтраницОтчета.Строки Цикл
			...
			Если ВРег(СокрЛП(ИмяФормы)) = ВРег("ФормаОтчета2022Кв2") Тогда
				...
				Если СтраницаОтчета.ИмяСтраницы = "Таблица92" Тогда
					
					ПрефиксПоказателейРаздела = "П00092";
					
					Для Каждого Показатель Из РазделыОтчета.Таблица92.Данные Цикл
				...
				ИначеЕсли СтраницаОтчета.ИмяСтраницы = "Таблица1_1" Тогда
				...
				ИначеЕсли СтраницаОтчета.ИмяСтраницы = "Таблица2" Тогда
					
					ПрефиксПоказателейРаздела = "П00002";
					
					Для Каждого Показатель Из РазделыОтчета.Таблица2.Данные Цикл
				...
				ИначеЕсли СтраницаОтчета.ИмяСтраницы = "Таблица3" Тогда
					
					Для Каждого Показатель Из РазделыОтчета.Таблица3.Данные Цикл
			...
			Иначе// "ФормаОтчета2017Кв3", "ФормаОтчета2017Кв1"
				...
				ИначеЕсли ВРег(СокрЛП(ИмяФормы)) = ВРег("ФормаОтчета2017Кв3") И СтраницаОтчета.ИмяСтраницы = "Таблица91" Тогда
					
					ПрефиксПоказателейРаздела = "П00091";
					
					Для Каждого Показатель Из РазделыОтчета.Таблица91.Данные Цикл
...
Процедура ЗаполнитьСтруктурыДанныхРазделовЗначениямиИзДопСтрок(ИмяФормы, Контейнер) Экспорт
	...
	Если ВРег(СокрЛП(ИмяФормы)) = ВРег("ФормаОтчета2022Кв2") Тогда
		...
		РегламентированнаяОтчетность.ЗаполнитьСтруктуруДанныхРазделаЗначениямиИзДопСтрок(
			Контейнер.Таблица92, Контейнер.Таблица92_ДанныеДопСтрок);
		
		РегламентированнаяОтчетность.ЗаполнитьСтруктуруДанныхРазделаЗначениямиИзДопСтрок(
			Контейнер.Таблица1_1, Контейнер.Таблица1_1_ДанныеДопСтрок);
		
		РегламентированнаяОтчетность.ЗаполнитьСтруктуруДанныхРазделаЗначениямиИзДопСтрок(
			Контейнер.Таблица2, Контейнер.Таблица2_ДанныеДопСтрок);
	...
	Иначе// "ФормаОтчета2017Кв3", "ФормаОтчета2017Кв1"
		...
		Если ВРег(СокрЛП(ИмяФормы)) = ВРег("ФормаОтчета2017Кв3") Тогда
			...
			РегламентированнаяОтчетность.ЗаполнитьСтруктуруДанныхРазделаЗначениямиИзДопСтрок(
				Контейнер.Таблица91, Контейнер.Таблица91_ДанныеДопСтрок);
...
Процедура РассчитатьИтоги(ИмяФормы, Контейнер, ПериодОтчетаВходитВПервыйКвартал)
	...
	Если ВРег(СокрЛП(ИмяФормы)) = ВРег("ФормаОтчета2022Кв2") Тогда
		...
		ТаблицаБлока = Контейнер["Таблица2_ДанныеДопСтрок"];
		Для Ном = 1 По ТаблицаБлока.Количество() Цикл
			Постфикс = Формат(Ном, "ЧГ=");
			СтрБлока = ТаблицаБлока[Ном - 1];
		...
		// Таблица 3.
		//
		СтруктураДанныхРаздела = Контейнер["Таблица3"];
	...
	Иначе// "ФормаОтчета2017Кв3", "ФормаОтчета2017Кв1"
		...
		Если ПериодОтчетаВходитВПервыйКвартал Тогда
			...
			Если ВРег(СокрЛП(ИмяФормы)) = ВРег("ФормаОтчета2017Кв3") Тогда
				...
				СтруктураДанныхРаздела = Контейнер["Таблица91"];
				ТаблицаБлока = Контейнер["Таблица91_ДанныеДопСтрок"];
				Для Ном = 1 По ТаблицаБлока.Количество() Цикл
					Постфикс = Формат(Ном, "ЧГ=");
					СтрБлока = ТаблицаБлока[Ном - 1];
...
Функция СоздатьНовыйПотокXML(ИмяФормы, КодировкаФайлаВыгрузки)
	...
	Если ВРег(СокрЛП(ИмяФормы)) = ВРег("ФормаОтчета2022Кв2") Тогда
		...
		ПотокXML.ЗаписатьБезОбработки(Символы.ПС + "<?F4FORM version=""0.94""?>");
		
	ИначеЕсли ВРег(СокрЛП(ИмяФормы)) = ВРег("ФормаОтчета2017Кв3") Тогда
		
		ПотокXML.ЗаписатьБезОбработки(Символы.ПС + "<?F4FORM version=""0.93""?>");
...
Функция ЗаписатьУзелДереваВXML(ИмяФормы, СтрокаДерева, ПотокXML, ПараметрыВыгрузки)
	...
	Если ТипЗнч(СтрокаДерева) = Тип("ДеревоЗначений") Тогда
		...
		Если ВРег(СокрЛП(ИмяФормы)) = ВРег("ФормаОтчета2022Кв2") Тогда
			...
			ПотокXML.ЗаписатьАтрибут("xsd:schemaLocation", "http://fz122.fss.ru http://fz122.fss.ru/doc/f4form_2022_2.xsd");
...
Процедура ЗаполнитьСодержательнуюЧасть(ИмяФормы, ДанныеОтчета, ПараметрыВыгрузки, УзелРодитель)
	...
	Если ВРег(СокрЛП(ИмяФормы)) = ВРег("ФормаОтчета2022Кв2") Тогда
		...
		Таблица1 = ОбщегоНазначения.СкопироватьРекурсивно(ДанныеОтчета.ПолеТабличногоДокументаТаблица1);
		...
		ЗаполнитьДаннымиУзел(ИмяФормы, ДанныеОтчета.ПолеТабличногоДокументаТаблица3, Узел_F4INFO);
		ЗаполнитьДаннымиУзел(ИмяФормы, ДанныеОтчета.ПолеТабличногоДокументаТаблица4, Узел_F4INFO);
		
		// Выгрузка таблицы "Платежи" (многострочная).
		//
		// Особая обработка: случай отчета, сохраненного без раздела "Платежи".
		Таблица92 = Неопределено;
		ДанныеОтчета.Свойство("ПолеТабличногоДокументаТаблица92", Таблица92);
		Если ТипЗнч(Таблица92) = Тип("Структура") Тогда
			УзелPAYM_ORDERШаблон = ПолучитьПодчиненныйЭлемент(ИмяФормы, Узел_F4INFO, "PAYM_ORDER");
			НомСтр = 1;
			Пока РегламентированнаяОтчетностьКлиентСервер.СвойствоОпределено(
				Таблица92, "П000920000102_" + Формат(НомСтр, "ЧГ=")) Цикл
				Если ЗначениеЗаполнено(Таблица92["П000920000102_" + Формат(НомСтр, "ЧГ=")]) Тогда
					УзелPAYM_ORDER = СкопироватьУзел(ИмяФормы, Узел_F4INFO, УзелPAYM_ORDERШаблон);
					ВыгрузитьСтрокуМногострочнойЧасти(ИмяФормы, Таблица92, УзелPAYM_ORDER, НомСтр);
				КонецЕсли;
				НомСтр = НомСтр + 1;
			КонецЦикла;
		КонецЕсли;
		...
		//
		УзелF4CALCШаблон = ПолучитьПодчиненныйЭлемент(ИмяФормы, УзелРодитель, "F4CALC");
		
		Таблица1_1 = ОбщегоНазначения.СкопироватьРекурсивно(ДанныеОтчета.ПолеТабличногоДокументаТаблица1_1);
		// Всего.
		ВыгружатьСпецсимволы_Таблица1_1 = Ложь;
		Для Каждого Показатель Из Таблица1_1 Цикл
			Если СтрНачинаетсяС(Показатель.Ключ, "П00011") И СтрДлина(Показатель.Ключ) >= 13
				И ЗначениеЗаполнено(Показатель.Значение) Тогда
				ВыгружатьСпецсимволы_Таблица1_1 = Истина;
				Прервать;
			КонецЕсли;
		КонецЦикла;
		Всего_Данные = Новый Структура;
		Для НомГрафы = 2 По 24 Цикл
			...
			Если НомГрафы <= 4 Тогда
				Всего_Данные.Вставить(ВиртуальноеИмяПоказателя, ?(ВыгружатьСпецсимволы_Таблица1_1, "X", ""));
			ИначеЕсли НомГрафы <= 8 Тогда
				Всего_Данные.Вставить(ВиртуальноеИмяПоказателя, 0);
			Иначе
				РеальноеИмяПоказателя = "П00011ВСЕГО" + НомГрафыСтр;
				Всего_Данные.Вставить(ВиртуальноеИмяПоказателя, Таблица1_1[РеальноеИмяПоказателя]);
			КонецЕсли;
		КонецЦикла;
		УзелF4CALC = СкопироватьУзел(ИмяФормы, УзелРодитель, УзелF4CALCШаблон);
		ВыгрузитьСтрокуМногострочнойЧасти(ИмяФормы, Всего_Данные, УзелF4CALC, 1);
		// Основной ВЭД.
		...
		КонецЦикла;
		УзелF4CALC = СкопироватьУзел(ИмяФормы, УзелРодитель, УзелF4CALCШаблон);
		ВыгрузитьСтрокуМногострочнойЧасти(ИмяФормы, ОсновнойВЭД_Данные, УзелF4CALC, 1);
		// СКЕ.
		НомСтр = 1;
		Пока РегламентированнаяОтчетностьКлиентСервер.СвойствоОпределено(
			...
			Если ЗначениеЗаполнено(Таблица1_1["П000110000103_" + Формат(НомСтр, "ЧГ=")]) Тогда
				...
				УзелF4CALC = СкопироватьУзел(ИмяФормы, УзелРодитель, УзелF4CALCШаблон);
				ВыгрузитьСтрокуМногострочнойЧасти(ИмяФормы, Таблица1_1, УзелF4CALC, НомСтр);
			КонецЕсли;
			НомСтр = НомСтр + 1;
		КонецЦикла;
		...
		Пока РегламентированнаяОтчетностьКлиентСервер.СвойствоОпределено(
			...
			Если ЗначениеЗаполнено(Таблица2["П000020000102_" + Формат(НомСтр, "ЧГ=")]) Тогда
				...
				ВыгрузитьСтрокуМногострочнойЧасти(ИмяФормы, Таблица2, УзелF4DOGOV, НомСтр);
			КонецЕсли;
			НомСтр = НомСтр + 1;
		КонецЦикла;
	...
	Иначе// "ФормаОтчета2017Кв3", "ФормаОтчета2017Кв1"
		...
		Если ВРег(СокрЛП(ИмяФормы)) = ВРег("ФормаОтчета2017Кв3")
			И ДанныеОтчета.Свойство("ПолеТабличногоДокументаТаблица91") Тогда
			
			// СКЕ / ИФ.
			Таблица91 = ДанныеОтчета.ПолеТабличногоДокументаТаблица91;
			УзелF4DECODEШаблон = ПолучитьПодчиненныйЭлемент(ИмяФормы, УзелРодитель, "F4DECODE");
			НомСтр = 1;
			Пока РегламентированнаяОтчетностьКлиентСервер.СвойствоОпределено(
				Таблица91, "П000910000101_" + Формат(НомСтр, "ЧГ=")) Цикл
				Если ЗначениеЗаполнено(Таблица91["П000910000102_" + Формат(НомСтр, "ЧГ=")]) Тогда
					// Заполнен "ОКВЭД".
					УзелF4DECODE = СкопироватьУзел(ИмяФормы, УзелРодитель, УзелF4DECODEШаблон);
					ВыгрузитьСтрокуМногострочнойЧасти(ИмяФормы, Таблица91, УзелF4DECODE, НомСтр);
				КонецЕсли;
				НомСтр = НомСтр + 1;
			КонецЦикла;
...
Функция ВыгрузкаВозможна(ИмяФормы,
	...
	Если НЕ ЗначениеЗаполнено(ПараметрыВыгрузки.НомерСтрахователя) Тогда
		...
		РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(РезультатВыгрузки,
		НСтр("ru='Не задан номер страхователя'"), "Титульный", "ОргФССКод");
		СтатусПроверки = Ложь;
		
	ИначеЕсли НЕ СтрДлина(СокрЛП(ПараметрыВыгрузки.НомерСтрахователя)) = 10 Тогда
		
		РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(РезультатВыгрузки,
		НСтр("ru='Длина регистрационного номера страхователя должна быть равна 10-ти символам'"), "Титульный", "ОргФССКод");
		СтатусПроверки = Ложь;
	...
	Если НЕ ЗначениеЗаполнено(ПараметрыВыгрузки.КодПодчиненности) Тогда
		...
		РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(РезультатВыгрузки,
		НСтр("ru='Не задан код подчиненности'"), "Титульный", "КодПодчиненности");
		СтатусПроверки = Ложь;
		
	ИначеЕсли СтрДлина(ПараметрыВыгрузки.КодПодчиненности) <> 5 Тогда
		
		РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(РезультатВыгрузки,
		НСтр("ru='Задан код подчиненности, не состоящий из 5 цифр'"), "Титульный", "КодПодчиненности");
		СтатусПроверки = Ложь;
		
	Иначе
		
		ПоследнийСимвол = Прав(ПараметрыВыгрузки.КодПодчиненности, 1);
		Если ПоследнийСимвол <> "1" И ПоследнийСимвол <> "2" И ПоследнийСимвол <> "3" И ПоследнийСимвол <> "4" Тогда
			РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(РезультатВыгрузки,
	...
	Если ПустаяСтрока(ПараметрыВыгрузки.ИНН)
		...
		РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(РезультатВыгрузки,
		НСтр("ru='Не задан ИНН организации'"), "Титульный", "ИНН");
		СтатусПроверки = Ложь;
	КонецЕсли;
	
	Если РегламентированнаяОтчетностьПереопределяемый.ЭтоЮридическоеЛицо(
		СтруктураРеквизитовФормы.мСохраненныйДок.Организация) И СтрДлина(ПараметрыВыгрузки.КПП) <> 9 Тогда
		РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(РезультатВыгрузки,
		НСтр("ru='Неверно задан КПП организации'"), "Титульный", "КПП");
		СтатусПроверки = Ложь;
	КонецЕсли;
	...
	Если НЕ ЗначениеЗаполнено(ПараметрыВыгрузки.ОргДиректор) Тогда
		РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(РезультатВыгрузки,
		НСтр("ru='На титульном листе не заданы Ф.И.О. руководителя организации, индивидуального предпринимателя, "
		+ "физического лица, представителя плательщика страховых взносов (страхователя)'"), "Титульный", "ОргПодписантФамилия");
		СтатусПроверки = Ложь;
	КонецЕсли;
	...
	Если НЕ ПустаяСтрока(СокрЛП(СтрЗаменить(ПараметрыВыгрузки.ОКВЭД, ".", "")))
		...
		РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(РезультатВыгрузки,
		НСтр("ru='На титульном листе указано некорректное значение кода вида деятельности по ОКВЭД'"), "Титульный", "ОКВЭД");
		СтатусПроверки = Ложь;
	КонецЕсли;
	...
	Если ВРег(СокрЛП(ИмяФормы)) = ВРег("ФормаОтчета2022Кв2") Тогда
		...
		//
		// Особая обработка: случай отчета, сохраненного без раздела "Платежи".
		ДанныеТаблица92 = Неопределено;
		ДанныеОтчета.Свойство("ПолеТабличногоДокументаТаблица92", ДанныеТаблица92);
		Если ТипЗнч(ДанныеТаблица92) = Тип("Структура") Тогда
			
			НомСтроки = 1;
			Пока ДанныеТаблица92.Свойство("П000920000102_" + Формат(НомСтроки, "ЧГ=")) Цикл
				НомерСтроки = Формат(НомСтроки, "ЧГ=");
				
				СтрокаЗаполнена
				= ЗначениеЗаполнено(ДанныеТаблица92["П000920000102_" + НомерСтроки])
				ИЛИ ЗначениеЗаполнено(ДанныеТаблица92["П000920000103_" + НомерСтроки])
				ИЛИ ДанныеТаблица92["П000920000104_" + НомерСтроки] <> 0;
				
				Если СтрокаЗаполнена Тогда
					
					ИмяПок = "П000920000102_" + НомерСтроки;
					Если НЕ ЗначениеЗаполнено(ДанныеТаблица92[ИмяПок]) Тогда
						РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(РезультатВыгрузки, НСтр("ru='Платежи: в строке "
						+ НомерСтроки + " не указан номер платежного поручения'"), "Таблица92", ИмяПок);
						СтатусПроверки = Ложь;
					КонецЕсли;
					
					ИмяПок = "П000920000103_" + НомерСтроки;
					Если НЕ ЗначениеЗаполнено(ДанныеТаблица92[ИмяПок]) Тогда
						РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(РезультатВыгрузки, НСтр("ru='Платежи: в строке "
						+ НомерСтроки + " не указана дата платежного поручения'"), "Таблица92", ИмяПок);
						СтатусПроверки = Ложь;
					КонецЕсли;
					
					ИмяПок = "П000920000104_" + НомерСтроки;
					Если ДанныеТаблица92[ИмяПок] = 0 Тогда
						РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(РезультатВыгрузки, НСтр("ru='Платежи: в строке "
						+ НомерСтроки + " не указана сумма платежа'"), "Таблица92", ИмяПок);
						СтатусПроверки = Ложь;
					КонецЕсли;
		...
		Если ДанныеТаблица1["П000010000503"] = 0 Тогда
			РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(РезультатВыгрузки, НСтр("ru='Таблица 1: "
			+ "строка 5 должна быть заполнена'"), "Таблица1", "П000010000503");
			СтатусПроверки = Ложь;
		КонецЕсли;
		
		// Таблица 1.1.
		//
		ЗаполненаТаблица1_1 = Ложь;
		ДанныеТаблица1_1 = ДанныеОтчета.ПолеТабличногоДокументаТаблица1_1;
		ПрефиксПоказателейРаздела = "П00011";
		Для Каждого Показатель Из ДанныеТаблица1_1 Цикл
			Если СтрНачинаетсяС(Показатель.Ключ, "П00011") И СтрДлина(Показатель.Ключ) >= 13
				И ЗначениеЗаполнено(Показатель.Значение) Тогда
				ЗаполненаТаблица1_1 = Истина;
				Прервать;
			КонецЕсли;
		КонецЦикла;
		
		Если ЗаполненаТаблица1_1 Тогда
			
			ИмяПок = "П00011ОСВЭД02";
			Если НЕ ЗначениеЗаполнено(ДанныеТаблица1_1[ИмяПок]) Тогда
				РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(РезультатВыгрузки, НСтр("ru='Таблица 1.1: "
				+ "в строке ""основной ВЭД"" не указано наименование ВЭД организации'"), "Таблица1_1", ИмяПок);
				СтатусПроверки = Ложь;
			КонецЕсли;
			
			ИмяПок = "П00011ОСВЭД03";
			ЗначПок = СокрЛП(ДанныеТаблица1_1[ИмяПок]);
			Если НЕ ЗначениеЗаполнено(ЗначПок) Тогда
				РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(РезультатВыгрузки, НСтр("ru='Таблица 1.1: "
				+ "в строке ""основной ВЭД"" не указан код по ОКВЭД организации'"), "Таблица1_1", ИмяПок);
				СтатусПроверки = Ложь;
			ИначеЕсли НЕ РегламентированнаяОтчетностьКлиентСервер.ОКВЭДСоответствуетТребованиям(ЗначПок) Тогда
				РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(РезультатВыгрузки, НСтр("ru='Таблица 1.1: "
				+ "в строке ""основной ВЭД"" указано некорректное значение кода по ОКВЭД организации'"), "Таблица1_1", ИмяПок);
				СтатусПроверки = Ложь;
			КонецЕсли;
			
			ИмяПок = "П00011ОСВЭД04";
			Если НЕ ЗначениеЗаполнено(ДанныеТаблица1_1[ИмяПок]) Тогда
				РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(РезультатВыгрузки, НСтр("ru='Таблица 1.1: "
				+ "в строке ""основной ВЭД"" не указано наименование организации'"), "Таблица1_1", ИмяПок);
				СтатусПроверки = Ложь;
			КонецЕсли;
			
			ИмяПок = "П00011ОСВЭД09";
			Если ДанныеТаблица1_1[ИмяПок] < ДанныеТаблица1_1["П00011ОСВЭД11"] + ДанныеТаблица1_1["П00011ОСВЭД13"]
				+ ДанныеТаблица1_1["П00011ОСВЭД15"] + ДанныеТаблица1_1["П00011ОСВЭД17"] Тогда
				РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(РезультатВыгрузки, НСтр("ru='Таблица 1.1: "
				+ "в строке ""основной ВЭД"" значение в графе 9 должно быть больше или равно сумме значений в графах 11, 13, 15, 17'"),
				"Таблица1_1", ИмяПок);
				СтатусПроверки = Ложь;
			КонецЕсли;
			
			ИмяПок = "П00011ОСВЭД10";
			Если ДанныеТаблица1_1[ИмяПок] < ДанныеТаблица1_1["П00011ОСВЭД12"] + ДанныеТаблица1_1["П00011ОСВЭД14"]
				+ ДанныеТаблица1_1["П00011ОСВЭД16"] + ДанныеТаблица1_1["П00011ОСВЭД18"] Тогда
				РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(РезультатВыгрузки, НСтр("ru='Таблица 1.1: "
				+ "в строке ""основной ВЭД"" значение в графе 10 должно быть больше или равно сумме значений в графах 12, 14, 16, 18'"),
				"Таблица1_1", ИмяПок);
				СтатусПроверки = Ложь;
			КонецЕсли;
			
			ИмяПок = "П00011ОСВЭД19";
			Если ДанныеТаблица1_1[ИмяПок] < ДанныеТаблица1_1["П00011ОСВЭД20"] + ДанныеТаблица1_1["П00011ОСВЭД21"] Тогда
				РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(РезультатВыгрузки, НСтр("ru='Таблица 1.1: "
				+ "в строке ""основной ВЭД"" значение в графе 19 должно быть больше или равно сумме значений в графах 20, 21'"),
				"Таблица1_1", ИмяПок);
				СтатусПроверки = Ложь;
			КонецЕсли;
			
			ЗаполненаХотяБыОднаСКЕ = Ложь;
			НомСтроки = 1;
			Пока ДанныеТаблица1_1.Свойство("П000110000103_" + Формат(НомСтроки, "ЧГ=")) Цикл
				НомерСтроки = Формат(НомСтроки, "ЧГ=");
				
				СтрокаЗаполнена
				= ЗначениеЗаполнено(ДанныеТаблица1_1["П000110000102_" + НомерСтроки])
				ИЛИ ЗначениеЗаполнено(ДанныеТаблица1_1["П000110000103_" + НомерСтроки])
				ИЛИ ЗначениеЗаполнено(ДанныеТаблица1_1["П000110000104_" + НомерСтроки])
				ИЛИ ДанныеТаблица1_1["П000110000105_" + НомерСтроки] <> 0
				ИЛИ ДанныеТаблица1_1["П000110000109_" + НомерСтроки] <> 0
				ИЛИ ДанныеТаблица1_1["П000110000110_" + НомерСтроки] <> 0
				ИЛИ ДанныеТаблица1_1["П000110000111_" + НомерСтроки] <> 0
				ИЛИ ДанныеТаблица1_1["П000110000112_" + НомерСтроки] <> 0
				ИЛИ ДанныеТаблица1_1["П000110000113_" + НомерСтроки] <> 0
				ИЛИ ДанныеТаблица1_1["П000110000114_" + НомерСтроки] <> 0
				ИЛИ ДанныеТаблица1_1["П000110000115_" + НомерСтроки] <> 0
				ИЛИ ДанныеТаблица1_1["П000110000116_" + НомерСтроки] <> 0
				ИЛИ ДанныеТаблица1_1["П000110000117_" + НомерСтроки] <> 0
				ИЛИ ДанныеТаблица1_1["П000110000118_" + НомерСтроки] <> 0
				ИЛИ ДанныеТаблица1_1["П000110000119_" + НомерСтроки] <> 0
				ИЛИ ДанныеТаблица1_1["П000110000120_" + НомерСтроки] <> 0
				ИЛИ ДанныеТаблица1_1["П000110000121_" + НомерСтроки] <> 0
				ИЛИ ДанныеТаблица1_1["П000110000122_" + НомерСтроки] <> 0
				ИЛИ ДанныеТаблица1_1["П000110000123_" + НомерСтроки] <> 0
				ИЛИ ДанныеТаблица1_1["П000110000124_" + НомерСтроки] <> 0;
				
				ЗаполненаХотяБыОднаСКЕ = ЗаполненаХотяБыОднаСКЕ ИЛИ СтрокаЗаполнена;
				
				Если СтрокаЗаполнена Тогда
					
					ИмяПок = "П000110000102_" + НомерСтроки;
					Если НЕ ЗначениеЗаполнено(ДанныеТаблица1_1[ИмяПок]) Тогда
						РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(РезультатВыгрузки, НСтр("ru='Таблица 1.1: в строке "
						+ НомерСтроки + " не указано наименование ВЭД СКЕ'"), "Таблица1_1", ИмяПок);
						СтатусПроверки = Ложь;
					КонецЕсли;
					
					ИмяПок = "П000110000103_" + НомерСтроки;
					ЗначПок = СокрЛП(ДанныеТаблица1_1[ИмяПок]);
					Если НЕ ЗначениеЗаполнено(ЗначПок) Тогда
						РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(РезультатВыгрузки, НСтр("ru='Таблица 1.1: в строке "
						+ НомерСтроки + " не указан код по ОКВЭД СКЕ'"), "Таблица1_1", ИмяПок);
						СтатусПроверки = Ложь;
					ИначеЕсли НЕ РегламентированнаяОтчетностьКлиентСервер.ОКВЭДСоответствуетТребованиям(ЗначПок) Тогда
						РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(РезультатВыгрузки, НСтр("ru='Таблица 1.1: в строке "
						+ НомерСтроки + " указано некорректное значение кода по ОКВЭД СКЕ'"), "Таблица1_1", ИмяПок);
						СтатусПроверки = Ложь;
					КонецЕсли;
					
					ИмяПок = "П000110000104_" + НомерСтроки;
					Если НЕ ЗначениеЗаполнено(ДанныеТаблица1_1[ИмяПок]) Тогда
						РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(РезультатВыгрузки, НСтр("ru='Таблица 1.1: в строке "
						+ НомерСтроки + " не указано наименование структурного подразделения - СКЕ'"), "Таблица1_1", ИмяПок);
						СтатусПроверки = Ложь;
					КонецЕсли;
					
					ИмяПок = "П000110000109_" + НомерСтроки;
					Если ДанныеТаблица1_1[ИмяПок]
						< ДанныеТаблица1_1["П000110000111_" + НомерСтроки] + ДанныеТаблица1_1["П000110000113_" + НомерСтроки]
						+ ДанныеТаблица1_1["П000110000115_" + НомерСтроки] + ДанныеТаблица1_1["П000110000117_" + НомерСтроки] Тогда
						РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(РезультатВыгрузки, НСтр("ru='Таблица 1.1: в строке "
						+ НомерСтроки + " значение в графе 9 должно быть больше или равно сумме значений в графах 11, 13, 15, 17'"),
						"Таблица1_1", ИмяПок);
						СтатусПроверки = Ложь;
					КонецЕсли;
					
					ИмяПок = "П000110000110_" + НомерСтроки;
					Если ДанныеТаблица1_1[ИмяПок]
						< ДанныеТаблица1_1["П000110000112_" + НомерСтроки] + ДанныеТаблица1_1["П000110000114_" + НомерСтроки]
						+ ДанныеТаблица1_1["П000110000116_" + НомерСтроки] + ДанныеТаблица1_1["П000110000118_" + НомерСтроки] Тогда
						РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(РезультатВыгрузки, НСтр("ru='Таблица 1.1: в строке "
						+ НомерСтроки + " значение в графе 10 должно быть больше или равно сумме значений в графах 12, 14, 16, 18'"),
						"Таблица1_1", ИмяПок);
						СтатусПроверки = Ложь;
					КонецЕсли;
					
					ИмяПок = "П000110000119_" + НомерСтроки;
					Если ДанныеТаблица1_1[ИмяПок]
						< ДанныеТаблица1_1["П000110000120_" + НомерСтроки] + ДанныеТаблица1_1["П000110000121_" + НомерСтроки] Тогда
						РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(РезультатВыгрузки, НСтр("ru='Таблица 1.1: в строке "
						+ НомерСтроки + " значение в графе 19 должно быть больше или равно сумме значений в графах 20, 21'"),
						"Таблица1_1", ИмяПок);
						СтатусПроверки = Ложь;
					КонецЕсли;
			...
			Если НЕ ЗаполненаХотяБыОднаСКЕ Тогда
				
				ИмяПок = ?(ЗначениеЗаполнено(ДанныеТаблица1_1["П000110000101_1"]), "П000110000102_1", "П000110000101_1");
				РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(РезультатВыгрузки, НСтр(
				"ru='Таблица 1.1 должна содержать сведения по выделенным самостоятельным классификационным единицам'"),
				"Таблица1_1", ИмяПок);
				СтатусПроверки = Ложь;
		...
		НомСтроки = 1;
		Пока ДанныеТаблица2.Свойство("П000020000102_" + Формат(НомСтроки, "ЧГ=")) Цикл
			НомерСтроки = Формат(НомСтроки, "ЧГ=");
			
			СтрокаЗаполнена
			= ЗначениеЗаполнено(ДанныеТаблица2["П000020000102_" + НомерСтроки])
			ИЛИ ЗначениеЗаполнено(ДанныеТаблица2["П000020000103_" + НомерСтроки])
			ИЛИ ЗначениеЗаполнено(ДанныеТаблица2["П000020000104_" + НомерСтроки])
			ИЛИ ЗначениеЗаполнено(ДанныеТаблица2["П000020000105_" + НомерСтроки])
			ИЛИ ДанныеТаблица2["П000020000106_" + НомерСтроки] <> 0
			ИЛИ ДанныеТаблица2["П000020000107_" + НомерСтроки] <> 0
			ИЛИ ДанныеТаблица2["П000020000108_" + НомерСтроки] <> 0
			ИЛИ ДанныеТаблица2["П000020000109_" + НомерСтроки] <> 0
			ИЛИ ДанныеТаблица2["П000020000110_" + НомерСтроки] <> 0
			ИЛИ ДанныеТаблица2["П000020000111_" + НомерСтроки] <> 0
			ИЛИ ДанныеТаблица2["П000020000112_" + НомерСтроки] <> 0
			ИЛИ ДанныеТаблица2["П000020000113_" + НомерСтроки] <> 0
			ИЛИ ДанныеТаблица2["П000020000114_" + НомерСтроки] <> 0
			ИЛИ ДанныеТаблица2["П000020000115_" + НомерСтроки] <> 0
			ИЛИ ДанныеТаблица2["П000020000116_" + НомерСтроки] <> 0
			ИЛИ ДанныеТаблица2["П000020000117_" + НомерСтроки] <> 0
			ИЛИ ДанныеТаблица2["П000020000118_" + НомерСтроки] <> 0
			ИЛИ ДанныеТаблица2["П000020000119_" + НомерСтроки] <> 0;
			
			Если СтрокаЗаполнена Тогда
				
				ИмяПок = "П000020000102_" + НомерСтроки;
				Если НЕ ЗначениеЗаполнено(ДанныеТаблица2[ИмяПок]) Тогда
					РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(РезультатВыгрузки, НСтр("ru='Таблица 2: в строке "
					+ НомерСтроки + " не указан регистрационный номер принимающей организации в территориальном органе ФСС РФ'"),
					"Таблица2", ИмяПок);
					СтатусПроверки = Ложь;
				ИначеЕсли СтрДлина(СокрЛП(ДанныеТаблица2[ИмяПок])) <> 10 Тогда
					РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(РезультатВыгрузки, НСтр("ru='Таблица 2: в строке "
					+ НомерСтроки + " указан регистрационный номер принимающей организации в территориальном органе ФСС РФ, "
					+ "не состоящий из 10 символов'"), "Таблица2", ИмяПок);
					СтатусПроверки = Ложь;
				КонецЕсли;
				
				ИмяПок = "П000020000103_" + НомерСтроки;
				Если НЕ ЗначениеЗаполнено(ДанныеТаблица2[ИмяПок]) Тогда
					РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(РезультатВыгрузки, НСтр("ru='Таблица 2: в строке "
					+ НомерСтроки + " не указан ИНН принимающей организации'"), "Таблица2", ИмяПок);
					СтатусПроверки = Ложь;
				ИначеЕсли СтрДлина(СокрЛП(ДанныеТаблица2[ИмяПок])) <> 10 И СтрДлина(СокрЛП(ДанныеТаблица2[ИмяПок])) <> 12 Тогда
					РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(РезультатВыгрузки, НСтр("ru='Таблица 2: в строке "
					+ НомерСтроки + " указан ИНН принимающей организации, не состоящий из 10 или 12 символов'"), "Таблица2", ИмяПок);
					СтатусПроверки = Ложь;
				ИначеЕсли СтрДлина(СокрЛП(ДанныеТаблица2[ИмяПок])) = 12
					И ЗначениеЗаполнено(ДанныеТаблица2["П000020000104_" + НомерСтроки]) Тогда
					
					РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(РезультатВыгрузки, НСтр("ru='Таблица 2: в строке "
					+ НомерСтроки + " КПП принимающей организации не должен быть указан, если ИНН состоит из 12 символов'"),
					"Таблица2", "П000020000104_" + НомерСтроки);
					СтатусПроверки = Ложь;
					
				ИначеЕсли СтрДлина(СокрЛП(ДанныеТаблица2[ИмяПок])) = 10
					И ЗначениеЗаполнено(ДанныеТаблица2["П000020000104_" + НомерСтроки])
					И СтрДлина(СокрЛП(ДанныеТаблица2["П000020000104_" + НомерСтроки])) <> 9 Тогда
					
					РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(РезультатВыгрузки, НСтр("ru='Таблица 2: в строке "
					+ НомерСтроки + " указан КПП принимающей организации, не состоящий из 9 символов'"),
					"Таблица2", "П000020000104_" + НомерСтроки);
					СтатусПроверки = Ложь;
					
				КонецЕсли;
				
				ИмяПок = "П000020000105_" + НомерСтроки;
				ЗначПок = СокрЛП(ДанныеТаблица2[ИмяПок]);
				Если ЗначениеЗаполнено(ЗначПок)
					И НЕ РегламентированнаяОтчетностьКлиентСервер.ОКВЭДСоответствуетТребованиям(ЗначПок) Тогда
					РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(РезультатВыгрузки, НСтр("ru='Таблица 2: в строке "
					+ НомерСтроки + " указано некорректное значение кода по ОКВЭД принимающей организации'"), "Таблица2", ИмяПок);
					СтатусПроверки = Ложь;
				КонецЕсли;
				
				ИмяПок = "П000020000106_" + НомерСтроки;
				Если ДанныеТаблица2[ИмяПок] < ДанныеТаблица2["П000020000107_" + НомерСтроки] Тогда
					РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(РезультатВыгрузки, НСтр("ru='Таблица 2: в строке "
					+ НомерСтроки + " значение в графе 6 должно быть больше или равно значению в графе 7'"),
					"Таблица2", ИмяПок);
					СтатусПроверки = Ложь;
				КонецЕсли;
				ИмяПок = "П000020000108_" + НомерСтроки;
				Если ДанныеТаблица2[ИмяПок] < ДанныеТаблица2["П000020000109_" + НомерСтроки] Тогда
					РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(РезультатВыгрузки, НСтр("ru='Таблица 2: в строке "
					+ НомерСтроки + " значение в графе 8 должно быть больше или равно значению в графе 9'"),
					"Таблица2", ИмяПок);
					СтатусПроверки = Ложь;
				КонецЕсли;
				ИмяПок = "П000020000110_" + НомерСтроки;
				Если ДанныеТаблица2[ИмяПок] < ДанныеТаблица2["П000020000111_" + НомерСтроки] Тогда
					РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(РезультатВыгрузки, НСтр("ru='Таблица 2: в строке "
					+ НомерСтроки + " значение в графе 10 должно быть больше или равно значению в графе 11'"),
					"Таблица2", ИмяПок);
					СтатусПроверки = Ложь;
				КонецЕсли;
				ИмяПок = "П000020000112_" + НомерСтроки;
				Если ДанныеТаблица2[ИмяПок] < ДанныеТаблица2["П000020000113_" + НомерСтроки] Тогда
					РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(РезультатВыгрузки, НСтр("ru='Таблица 2: в строке "
					+ НомерСтроки + " значение в графе 12 должно быть больше или равно значению в графе 13'"),
					"Таблица2", ИмяПок);
					СтатусПроверки = Ложь;
				КонецЕсли;
				ИмяПок = "П000020000114_" + НомерСтроки;
				Если ДанныеТаблица2[ИмяПок] < ДанныеТаблица2["П000020000115_" + НомерСтроки] Тогда
					РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(РезультатВыгрузки, НСтр("ru='Таблица 2: в строке "
					+ НомерСтроки + " значение в графе 14 должно быть больше или равно значению в графе 15'"),
					"Таблица2", ИмяПок);
					СтатусПроверки = Ложь;
				КонецЕсли;
				
				ИмяПок = "П000020000108_" + НомерСтроки;
				Если ДанныеТаблица2[ИмяПок] < ДанныеТаблица2["П000020000110_" + НомерСтроки]
					+ ДанныеТаблица2["П000020000112_" + НомерСтроки] + ДанныеТаблица2["П000020000114_" + НомерСтроки] Тогда
					РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(РезультатВыгрузки, НСтр("ru='Таблица 2: в строке "
					+ НомерСтроки + " значение в графе 8 должно быть больше или равно сумме значений в графах 10, 12, 14'"),
					"Таблица2", ИмяПок);
					СтатусПроверки = Ложь;
				КонецЕсли;
				ИмяПок = "П000020000109_" + НомерСтроки;
				Если ДанныеТаблица2[ИмяПок] < ДанныеТаблица2["П000020000111_" + НомерСтроки]
					+ ДанныеТаблица2["П000020000113_" + НомерСтроки] + ДанныеТаблица2["П000020000115_" + НомерСтроки] Тогда
					РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(РезультатВыгрузки, НСтр("ru='Таблица 2: в строке "
					+ НомерСтроки + " значение в графе 9 должно быть больше или равно сумме значений в графах 11, 13, 15'"),
					"Таблица2", ИмяПок);
					СтатусПроверки = Ложь;
				КонецЕсли;
		...
		// Таблица 3.
		//
		ДанныеТаблица3 = ДанныеОтчета.ПолеТабличногоДокументаТаблица3;
		
		Если ДанныеТаблица3["П000030000103"] < ДанныеТаблица3["П000030000203"] Тогда
			РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(РезультатВыгрузки, НСтр("ru='Таблица 3: значение "
			+ "в строке 1 должно быть больше или равно значению в строке 2'"), "Таблица3", "П000030000103");
			СтатусПроверки = Ложь;
		КонецЕсли;
		
		Если ДанныеТаблица3["П000030000403"] < ДанныеТаблица3["П000030000503"] Тогда
			РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(РезультатВыгрузки, НСтр("ru='Таблица 3: значение "
			+ "в строке 4 должно быть больше или равно значению в строке 5'"), "Таблица3", "П000030000403");
			СтатусПроверки = Ложь;
		КонецЕсли;
		
		// Таблица 4.
		//
		ДанныеТаблица4 = ДанныеОтчета.ПолеТабличногоДокументаТаблица4;
		
		Если ДанныеТаблица4["П000040000104"] < ДанныеТаблица4["П000040000105"] + ДанныеТаблица4["П000040000106"]
			+ ДанныеТаблица4["П000040000107"] + ДанныеТаблица4["П000040000108"] + ДанныеТаблица4["П000040000109"]
			+ ДанныеТаблица4["П000040000110"] + ДанныеТаблица4["П000040000111"] Тогда
			РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(РезультатВыгрузки, НСтр("ru='Таблица 4: значение "
			+ "в графе 4 должно быть больше или равно сумме значений в графах 5-11'"), "Таблица4", "П000040000104");
			СтатусПроверки = Ложь;
		КонецЕсли;
	...
	Иначе// "ФормаОтчета2017Кв3", "ФормаОтчета2017Кв1"
		...
		Если ВРег(СокрЛП(ИмяФормы)) = ВРег("ФормаОтчета2017Кв3")
			...
			Для каждого ИмяПоказателя Из ИменаПроверяемыхПоказателей Цикл
				...
				Если ДанныеТаблица2.Свойство(ИмяПоказателя, ЗначениеПоказателя) И ЗначениеЗаполнено(ЗначениеПоказателя) Тогда
					РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(РезультатВыгрузки, НСтр(
					"ru='В 2021 году показатели строки 15 в таблице 2 не заполняются (информация опубликована на сайте ФСС РФ: "
					+ "http://fss.ru/ru/news/567885/586264.shtml).'"), "Таблица2", ИмяПоказателя);
					СтатусПроверки = Ложь;
					Прервать;
				КонецЕсли;
		...
		Если ВРег(СокрЛП(ИмяФормы)) = ВРег("ФормаОтчета2017Кв3")
			...
			ДанныеТаблица3_4 = ДанныеОтчета.ПолеТабличногоДокументаТаблица3_4;
			...
			Для каждого ИмяПоказателя Из ИменаПроверяемыхПоказателей Цикл
				...
				ЗначениеПоказателя = Неопределено;
				Если ДанныеТаблица3_4.Свойство(ИмяПоказателя, ЗначениеПоказателя) И ЗначениеЗаполнено(ЗначениеПоказателя) Тогда
					РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(РезультатВыгрузки, НСтр(
					"ru='В 2021 году таблица 3 не заполняется и не представляется (информация опубликована на сайте ФСС РФ: "
					+ "http://fss.ru/ru/news/567885/586264.shtml).'"), "Таблица3_4", ИмяПоказателя);
					СтатусПроверки = Ложь;
					Прервать;
				КонецЕсли;
		...
		Если ВРег(СокрЛП(ИмяФормы)) = ВРег("ФормаОтчета2017Кв3")
			...
			// СКЕ / ИФ.
			ДанныеТаблица91 = ДанныеОтчета.ПолеТабличногоДокументаТаблица91;
			НомСтроки = 1;
			Пока ДанныеТаблица91.Свойство("П000910000101_" + Формат(НомСтроки, "ЧГ=")) Цикл
				
				НомерСтроки = Формат(НомСтроки, "ЧГ=");
				
				СтрокаЗаполнена = Ложь;
				
				ОКВЭД = СокрЛП(ДанныеТаблица91["П000910000102_" + НомерСтроки]);
				
				КлассПрофРиска         = ДанныеТаблица91["П000910000103_" + НомерСтроки];
				СтрахТариф             = ДанныеТаблица91["П000910000104_" + НомерСтроки];
				СкидкаКСтрахТарифу     = ДанныеТаблица91["П000910000105_" + НомерСтроки];
				НадбавкаКСтрахТарифу   = ДанныеТаблица91["П000910000106_" + НомерСтроки];
				
				ВыплРасчПериодВсего    = ДанныеТаблица91["П000910000107_" + НомерСтроки];
				ВыплРасчПериодИнвалиды = ДанныеТаблица91["П000910000108_" + НомерСтроки];
				Выпл1МесяцВсего        = ДанныеТаблица91["П000910000109_" + НомерСтроки];
				Выпл1МесяцИнвалиды     = ДанныеТаблица91["П000910000110_" + НомерСтроки];
				Выпл2МесяцВсего        = ДанныеТаблица91["П000910000111_" + НомерСтроки];
				Выпл2МесяцИнвалиды     = ДанныеТаблица91["П000910000112_" + НомерСтроки];
				Выпл3МесяцВсего        = ДанныеТаблица91["П000910000113_" + НомерСтроки];
				Выпл3МесяцИнвалиды     = ДанныеТаблица91["П000910000114_" + НомерСтроки];
				
				ВыплНеНачислСВ  = ДанныеТаблица91["П000910000115_" + НомерСтроки];
				ЗачтеноРасходов = ДанныеТаблица91["П000910000116_" + НомерСтроки];
				
				Если ЗначениеЗаполнено(ОКВЭД) ИЛИ ЗначениеЗаполнено(КлассПрофРиска) ИЛИ ЗначениеЗаполнено(СтрахТариф)
					ИЛИ ЗначениеЗаполнено(СкидкаКСтрахТарифу) ИЛИ ЗначениеЗаполнено(НадбавкаКСтрахТарифу)
					ИЛИ ЗначениеЗаполнено(ВыплРасчПериодВсего) ИЛИ ЗначениеЗаполнено(ВыплРасчПериодИнвалиды)
					ИЛИ ЗначениеЗаполнено(Выпл1МесяцВсего) ИЛИ ЗначениеЗаполнено(Выпл1МесяцИнвалиды)
					ИЛИ ЗначениеЗаполнено(Выпл2МесяцВсего) ИЛИ ЗначениеЗаполнено(Выпл2МесяцИнвалиды)
					ИЛИ ЗначениеЗаполнено(Выпл3МесяцВсего) ИЛИ ЗначениеЗаполнено(Выпл3МесяцИнвалиды)
					ИЛИ ЗначениеЗаполнено(ВыплНеНачислСВ) ИЛИ ЗначениеЗаполнено(ЗачтеноРасходов) Тогда
					
					Если НЕ ЗначениеЗаполнено(ОКВЭД) Тогда
						РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(РезультатВыгрузки, НСтр(
						"ru='В разделе ""Самостоятельные классификационные единицы / источники финансирования"", в строке "
						+ НомерСтроки + " не указан код по ОКВЕД.'"), "Таблица91", "П000910000102_" + НомерСтроки);
						СтатусПроверки = Ложь;
					ИначеЕсли НЕ РегламентированнаяОтчетностьКлиентСервер.ОКВЭДСоответствуетТребованиям(ОКВЭД) Тогда
						РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(РезультатВыгрузки, НСтр(
						"ru='В разделе ""Самостоятельные классификационные единицы / источники финансирования"", в строке "
						+ НомерСтроки + " указано некорректное значение кода по ОКВЕД.'"), "Таблица91", "П000910000102_" + НомерСтроки);
						СтатусПроверки = Ложь;
					КонецЕсли;
					
					Если КлассПрофРиска = 0 Тогда
						РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(РезультатВыгрузки, НСтр(
						"ru='В разделе ""Самостоятельные классификационные единицы / источники финансирования"", в строке "
						+ НомерСтроки + " не указан класс профессионального риска.'"), "Таблица91", "П000910000103_" + НомерСтроки);
						СтатусПроверки = Ложь;
					КонецЕсли;
					
					Если СтрахТариф = 0 Тогда
						РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(РезультатВыгрузки, НСтр(
						"ru='В разделе ""Самостоятельные классификационные единицы / источники финансирования"", в строке "
						+ НомерСтроки + " не указан страховой тариф.'"), "Таблица91", "П000910000104_" + НомерСтроки);
						СтатусПроверки = Ложь;
					КонецЕсли;
					
					Если ВыплРасчПериодВсего < Выпл1МесяцВсего + Выпл2МесяцВсего + Выпл3МесяцВсего Тогда
						РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(РезультатВыгрузки, НСтр(
						"ru='В разделе ""Самостоятельные классификационные единицы / источники финансирования"", в строке "
						+ НомерСтроки + " значение в графе 7 должно быть больше или равно сумме значений в графах 9, 11, 13.'"),
						"Таблица91", "П000910000107_" + НомерСтроки);
						СтатусПроверки = Ложь;
					КонецЕсли;
					Если ВыплРасчПериодИнвалиды < Выпл1МесяцИнвалиды + Выпл2МесяцИнвалиды + Выпл3МесяцИнвалиды Тогда
						РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(РезультатВыгрузки, НСтр(
						"ru='В разделе ""Самостоятельные классификационные единицы / источники финансирования"", в строке "
						+ НомерСтроки + " значение в графе 8 должно быть больше или равно сумме значений в графах 10, 12, 14.'"),
						"Таблица91", "П000910000108_" + НомерСтроки);
						СтатусПроверки = Ложь;
					КонецЕсли;
					Если ВыплРасчПериодВсего < ВыплРасчПериодИнвалиды Тогда
						РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(РезультатВыгрузки, НСтр(
						"ru='В разделе ""Самостоятельные классификационные единицы / источники финансирования"", в строке "
						+ НомерСтроки + " значение в графе 7 должно быть больше или равно значению в графе 8.'"),
						"Таблица91", "П000910000107_" + НомерСтроки);
						СтатусПроверки = Ложь;
					КонецЕсли;
					Если Выпл1МесяцВсего < Выпл1МесяцИнвалиды Тогда
						РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(РезультатВыгрузки, НСтр(
						"ru='В разделе ""Самостоятельные классификационные единицы / источники финансирования"", в строке "
						+ НомерСтроки + " значение в графе 9 должно быть больше или равно значению в графе 10.'"),
						"Таблица91", "П000910000109_" + НомерСтроки);
						СтатусПроверки = Ложь;
					КонецЕсли;
					Если Выпл2МесяцВсего < Выпл2МесяцИнвалиды Тогда
						РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(РезультатВыгрузки, НСтр(
						"ru='В разделе ""Самостоятельные классификационные единицы / источники финансирования"", в строке "
						+ НомерСтроки + " значение в графе 11 должно быть больше или равно значению в графе 12.'"),
						"Таблица91", "П000910000111_" + НомерСтроки);
						СтатусПроверки = Ложь;
					КонецЕсли;
					Если Выпл3МесяцВсего < Выпл3МесяцИнвалиды Тогда
						РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(РезультатВыгрузки, НСтр(
						"ru='В разделе ""Самостоятельные классификационные единицы / источники финансирования"", в строке "
						+ НомерСтроки + " значение в графе 13 должно быть больше или равно значению в графе 14.'"),
						"Таблица91", "П000910000113_" + НомерСтроки);
						СтатусПроверки = Ложь;
					КонецЕсли;
...
Функция СформироватьСтруктуруПараметров(ИмяФормы,
	...
	ПараметрыВыгрузки.Вставить("ИмяФайла", ПараметрыВыгрузки.НомерСтрахователя
	+ "_" + ПараметрыВыгрузки.РасчПериод + "_" + ПараметрыВыгрузки.ОтчПериод + ".xml");
	...
	ЧастичноеФинансированиеИзБюдж = ?(ЗначениеЗаполнено(ДанныеОтчета.ПолеТабличногоДокументаТаблица1.П000010000001), 1, 0);
	...
	Если ВРег(СокрЛП(ИмяФормы)) = ВРег("ФормаОтчета2022Кв2") Тогда
		...
		ПараметрыВыгрузки.Вставить("Адрес1", СокрЛП(Титульный.Адрес1));
		ПараметрыВыгрузки.Вставить("Адрес2", СокрЛП(Титульный.Адрес2));
		ПараметрыВыгрузки.Вставить("Адрес3", СокрЛП(Титульный.Адрес3));
		ПараметрыВыгрузки.Вставить("Адрес4", СокрЛП(Титульный.Адрес4));
		ПараметрыВыгрузки.Вставить("Адрес6", СокрЛП(Титульный.Адрес6));
		ПараметрыВыгрузки.Вставить("Адрес7", СокрЛП(Титульный.Адрес7));
		ПараметрыВыгрузки.Вставить("Адрес8", СокрЛП(Титульный.Адрес8));
		ПараметрыВыгрузки.Вставить("Адрес9", СокрЛП(Титульный.Адрес9));
		
		ПараметрыВыгрузки.Вставить("ЧислФизЛицВыпл", СтрокаВЧисло(ИмяФормы, Титульный.ЧислФизЛицВыпл));
	...
	ИначеЕсли ВРег(СокрЛП(ИмяФормы)) = ВРег("ФормаОтчета2017Кв3")
		...
		Табл2Стр16Месяц1 = СуммаПоказателейМСЧ(ИмяФормы, ДанныеОтчета.ПолеТабличногоДокументаТаблица2, "П000020016101");
		Табл2Стр16Месяц2 = СуммаПоказателейМСЧ(ИмяФормы, ДанныеОтчета.ПолеТабличногоДокументаТаблица2, "П000020016201");
		Табл2Стр16Месяц3 = СуммаПоказателейМСЧ(ИмяФормы, ДанныеОтчета.ПолеТабличногоДокументаТаблица2, "П000020016301");
...
Функция ИзвлечьСтруктуруXML(ИмяФормы) Экспорт
	...
	Если ВРег(СокрЛП(ИмяФормы)) = ВРег("ФормаОтчета2022Кв2") Тогда
		...
		Макет = ЭтотОбъект.ПолучитьМакет("СхемаВыгрузкиXML094");
...
Функция ПечатныйБланк(ПараметрыПечати) Экспорт
	...
	Если НЕ ПараметрыПечати.Свойство("ДанныеРазделов", ДанныеРазделов) Тогда
		...
		Если ВРег(СокрЛП(ИмяФормы)) = ВРег("ФормаОтчета2022Кв2") Тогда
			...
			// Особая обработка: случай отчета, сохраненного без раздела "Платежи".
			Если ПоказателиОтчета.Свойство("ПолеТабличногоДокументаТаблица92") Тогда
				ДанныеРазделов.Вставить("Таблица92", ПоказателиОтчета.ПолеТабличногоДокументаТаблица92);
				ДанныеРазделов.Вставить(СтруктураРеквизитовФормы.ИдГруппы92,
					ДанныеМногострочныхРазделов[СтруктураРеквизитовФормы.ИдГруппы92]);
			КонецЕсли;
			
			ДанныеРазделов.Вставить("Таблица3",   ПоказателиОтчета.ПолеТабличногоДокументаТаблица3);
			ДанныеРазделов.Вставить("Таблица4",   ПоказателиОтчета.ПолеТабличногоДокументаТаблица4);
			ДанныеРазделов.Вставить(СтруктураРеквизитовФормы.ИдГруппы11,
				ДанныеМногострочныхРазделов[СтруктураРеквизитовФормы.ИдГруппы11]);
			ДанныеРазделов.Вставить(СтруктураРеквизитовФормы.ИдГруппы2, 
				ДанныеМногострочныхРазделов[СтруктураРеквизитовФормы.ИдГруппы2]);
		...
		Иначе// "ФормаОтчета2017Кв3", "ФормаОтчета2017Кв1"
			...
			Если ВРег(СокрЛП(ИмяФормы)) = ВРег("ФормаОтчета2017Кв3") Тогда
				...
				ДанныеРазделов.Вставить("Таблица91", ПоказателиОтчета.ПолеТабличногоДокументаТаблица91);
				ДанныеРазделов.Вставить(СтруктураРеквизитовФормы.ИдГруппы91,
					ДанныеМногострочныхРазделов[СтруктураРеквизитовФормы.ИдГруппы91]);
	...
	Для Каждого СтраницаОтчета Из ДеревоВыбранныхСтраниц.Строки Цикл
		...
		// Особая обработка: случай отчета, сохраненного без раздела "Платежи".
		Если СтраницаОтчета.ИмяСтраницы = "Таблица92" И НЕ ДанныеРазделов.Свойство("Таблица92") Тогда
...
Процедура ВывестиНаПечатьЛист(ИмяФормы,
	...
	Если ВРег(СокрЛП(ИмяФормы)) = ВРег("ФормаОтчета2022Кв2") Тогда
		...
		СоответствиеРазделовМакетам.Вставить("Таблица92",  Префикс + "Таблица92");
		...
		СоответствиеРазделовМакетам.Вставить("Таблица3",   Префикс + "Таблица3");
		СоответствиеРазделовМакетам.Вставить("Таблица4",   Префикс + "Таблица4");
		...
		Если СтруктураПараметров.ИмяСтраницы = "Таблица92" Тогда
			
			ИдГруппы = СтруктураРеквизитовФормы.ИдГруппы92;
			...
			Если ИмяОбластиПоследнейДопСтроки = "МногострочнаяЧасть" Тогда
				ИмяОбластиПоследнейДопСтроки
				= ОбластьМнгЧасти.ПолучитьОбласть(ВысотаОбластиМнгЧасти, , ВысотаОбластиМнгЧасти).Области[1].Имя;
			КонецЕсли;
		...
		ИначеЕсли СтруктураПараметров.ИмяСтраницы = "Таблица1_1"
			ИЛИ СтруктураПараметров.ИмяСтраницы = "Таблица2" Тогда
			
			Если СтруктураПараметров.ИмяСтраницы = "Таблица1_1" Тогда
				ИдГруппы = СтруктураРеквизитовФормы.ИдГруппы11;
			ИначеЕсли СтруктураПараметров.ИмяСтраницы = "Таблица2" Тогда
				ИдГруппы = СтруктураРеквизитовФормы.ИдГруппы2;
			КонецЕсли;
			...
			Если КолвоСтрокВТаблДок > КолвоСтрокВПечатнойФорме Тогда
				...
				НомерВерхЯчейкиОбласти
				= МакетПечатнойФормы.Область(ИдГруппы + "01_" + Формат(КолвоСтрокВПечатнойФорме, "ЧГ=")).Верх;
				НомерНижнЯчейкиОбласти = МакетПечатнойФормы.Область(ИдГруппы + Формат(
				ДанныеРазделов[ИдГруппы].Колонки.Количество(), "ЧЦ=2; ЧВН=") + "_" + Формат(КолвоСтрокВПечатнойФорме, "ЧГ=")).Низ;
				ВставляемаяОбласть = МакетПечатнойФормы.Область(НомерВерхЯчейкиОбласти, , НомерНижнЯчейкиОбласти);
				...
				Для Каждого Область Из МакетПечатнойФормы.ПолучитьОбласть(
					...
					Если НЕ НайденнаяОбласть = Неопределено Тогда
						...
						= Лев(НайденнаяОбласть.Имя, СтрНайти(НайденнаяОбласть.Имя, "_") - 1) + "_" + Формат(КолвоСтрокВТаблДок, "ЧЦ=3");
					КонецЕсли;
				КонецЦикла;
				...
				Для Ном = 0 По КолвоДопСтрок - 1 Цикл
					Для Каждого Область Из МакетПечатнойФормы.ПолучитьОбласть(
						...
						Если НЕ НайденнаяОбласть = Неопределено Тогда 
							НайденнаяОбласть.Имя
							= Лев(Область.Имя, СтрНайти(Область.Имя, "_") - 1) + "_" + Формат(КолвоСтрокВПечатнойФорме + Ном, "ЧЦ=3");
						КонецЕсли;
					КонецЦикла;
				КонецЦикла;
	...
	Иначе// "ФормаОтчета2017Кв3", "ФормаОтчета2017Кв1"
		...
		Если ВРег(СокрЛП(ИмяФормы)) = ВРег("ФормаОтчета2017Кв3") Тогда
			...
			СоответствиеРазделовМакетам.Вставить("Таблица91", Префикс + "Таблица91");
		...
		ИначеЕсли ВРег(СокрЛП(ИмяФормы)) = ВРег("ФормаОтчета2017Кв3") И СтруктураПараметров.ИмяСтраницы = "Таблица91" Тогда
			
			ИдГруппы = СтруктураРеквизитовФормы.ИдГруппы91;
...
Процедура ЗаполнитьПечатнуюФорму(ИмяФормы,
	...
	Для Каждого Показатель Из ДанныеРазделов[ПараметрыПечати.ИмяСтраницы] Цикл
		...
		Если ПараметрыПечати.ИмяСтраницы = "Титульный" Тогда
			...
			ИначеЕсли СтрЧислоВхождений(Показатель.Ключ, "ЧислФизЛицВыпл") > 0
				  ИЛИ СтрЧислоВхождений(Показатель.Ключ, "ЧислРаботников") > 0
			...
			ИначеЕсли СтрЧислоВхождений(Показатель.Ключ, "НомерКорректировки") > 0
				...
				Для Ном = 1 По СтрДлина(Показатель.Значение) Цикл
					...
					Если ВРег(СокрЛП(ИмяФормы)) = ВРег("ФормаОтчета2022Кв2")
						И СтрЧислоВхождений(Показатель.Ключ, "ОКВЭД") > 0 И (Ном = 3 ИЛИ Ном = 6) Тогда
	...
	Если МакетПечатнойФормы.Области.Найти("ДатаПодписиПолная") <> Неопределено Тогда
