...
Процедура ЗаполнитьСтруктуруЗависимыхЯчеекДляАвторасчета()
	...
	СтруктураЗависимыхЯчеекДляАвторасчета.Вставить("П000070009003", "П000070014003;П000070008003; П000070010003;П000070011003;П000070012003;П000070013003;"
																   ...
																   + "П000070012101;П000070002101;П000070002201;П000070002301;П000070012201;П000070012301");
	...
	Если СписокСохранения.Свойство("ДанныеВариантовАвтоЗаполнения", ДанныеВариановАвтоЗаполнения) Тогда
		...
		Если НЕ ДанныеВариановАвтоЗаполнения = Неопределено Тогда
			...
			Для Каждого ЭлементСтруктуры Из ДанныеВариановАвтоЗаполнения Цикл
				...
				//Сделано для восстановления данных из 2.0, т.к. в релизе 2.0.34 была допущена ошибка:
				//В макете "СоставПоказателей2012Кв1" была лишняя секция "Раздел1_6".
				Если ЭлементСтруктуры.Ключ = "Раздел1_6" Тогда 
					Продолжить;
				КонецЕсли;
...
Процедура ВосстановитьСведенияОбОрганизацииИзТитульногоЛистаСтарогоОбразца(ПоказателиОтчета)
	...
	Для Каждого Показатель Из ПоказателиТаблПоле Цикл
		...
		ИначеЕсли ИмяПоказателя = "КодПодчиненности_1" Тогда
			...
			КонецЦикла;
		ИначеЕсли ИмяПоказателя = "КодОрганаФСС1" Тогда
			СтруктураДанныхТитульный.Вставить("КодОрганаФСС");
			Для Ном = 1 По 4 Цикл
				СтруктураДанныхТитульный.КодОрганаФСС = СокрЛП(СтруктураДанныхТитульный.КодОрганаФСС) + ПоказателиТаблПоле["КодОрганаФСС" + Ном];
				СтруктураДанныхТитульный.Удалить("КодОрганаФСС" + Ном);
			КонецЦикла;
...
Процедура СкорректироватьМакетРаздела2_6_7(ИмяГруппы, СтруктураГруппыКоличество) Экспорт
	...
	Если ИмяГруппы = СтруктураРеквизитовФормы.ИдМСЧ_Раздел267_1 Тогда
		...
		ОбъединяемаяОбласть = ТабличныйДокумент.Область(ВерхСтроки5, 2, ПоследняяЯчейкаМСЧ.Низ, 3);
		ОбъединяемаяОбласть.Объединить();
		
		ОбъединяемаяОбласть = ТабличныйДокумент.Область(ВерхСтроки5, 4, ПоследняяЯчейкаМСЧ.Низ, 4);
		ОбъединяемаяОбласть.Объединить();
		
		ОбъединяемаяОбласть = ТабличныйДокумент.Область(ВерхСтроки5, 5, ПоследняяЯчейкаМСЧ.Низ, 5);
		ОбъединяемаяОбласть.Объединить();
		ОбъединяемаяОбласть.Имя = "П000070005003";
		
	ИначеЕсли ИмяГруппы = СтруктураРеквизитовФормы.ИдМСЧ_Раздел267_2 ИЛИ ИмяГруппы = СтруктураРеквизитовФормы.ИдМСЧ_Раздел267_3 Тогда
...
Процедура ОчисткаРегламентированногоОтчетаНаСервере(ИмяСтраницы, ЗапускатьОтдельнуюОбработкуПослеОчисткиЛиста, РежимОчистки, Многостраничность, КолВоСтраницВРазделе) Экспорт
	...
	Если ИмяСтраницы = "Раздел1_1" Тогда
		...
		ВывестиМакетВТаблДокНаСервере(ИмяСтраницы, 1);
...
Процедура ВывестиНаПечатьЛист(Парам1, ОбщиеРеквизитыРазделов, СтрПоля = Неопределено)
	...
	СоответствиеРазделовМакетам.Вставить("Раздел1_1",    "ПечатнаяФормаОтчета2012Кв1_Раздел1_1");
	СоответствиеРазделовМакетам.Вставить("Раздел1_2",    "ПечатнаяФормаОтчета2012Кв1_Раздел1_2");
	СоответствиеРазделовМакетам.Вставить("Раздел1_3",    "ПечатнаяФормаОтчета2012Кв1_Раздел1_3");
	СоответствиеРазделовМакетам.Вставить("Раздел1_31",   "ПечатнаяФормаОтчета2012Кв1_Раздел1_31");
	СоответствиеРазделовМакетам.Вставить("Раздел1_4",    "ПечатнаяФормаОтчета2012Кв1_Раздел1_4");
	СоответствиеРазделовМакетам.Вставить("Раздел1_41",   "ПечатнаяФормаОтчета2012Кв1_Раздел1_41");
	СоответствиеРазделовМакетам.Вставить("Раздел1_42",   "ПечатнаяФормаОтчета2012Кв1_Раздел1_42");
	СоответствиеРазделовМакетам.Вставить("Раздел1_43",   "ПечатнаяФормаОтчета2012Кв1_Раздел1_43");
	СоответствиеРазделовМакетам.Вставить("Раздел1_44",   "ПечатнаяФормаОтчета2012Кв1_Раздел1_44");
	СоответствиеРазделовМакетам.Вставить("Раздел1_5",    "ПечатнаяФормаОтчета2012Кв1_Раздел1_5");
	СоответствиеРазделовМакетам.Вставить("Раздел2_6_7",  "ПечатнаяФормаОтчета2012Кв1_Раздел2_6_7");
	СоответствиеРазделовМакетам.Вставить("Раздел2_8_9",  "ПечатнаяФормаОтчета2012Кв1_Раздел2_8_9");
...
Функция СформироватьСтруктуруПараметров()
	...
	ПараметрыВыгрузки.Вставить("КодФонда", Титульный.КодОрганаФСС); // Используется для контроля заполнения значения
...
Функция ИзвлечьСтруктуруXML()
	...
	Макет = ОбъектОтчета(ЭтаФорма.ИмяФормы).ПолучитьМакет("СхемаВыгрузкиXML04");
...
Функция ПроверитьВозможностьВыгрузки(ПараметрыВыгрузки, ДеревоВыгрузки)
	...
	Если НЕ ЗначениеЗаполнено(ПараметрыВыгрузки.КодФонда) Тогда
		РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма, "На титульном листе не задано значение кода органа ФСС, в который представляется отчет!", "Титульный", "КодОрганаФСС");
		СтатусПроверки = Ложь;
	КонецЕсли;
