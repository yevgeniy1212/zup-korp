...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	СтруктураРеквизитовФормы.мСоставПоказателей 	   = "СоставПоказателей2013Кв2";
	...
	// Версия формы
	СтруктураРеквизитовФормы.мВерсияФормы = "24/06/2013";
...
Процедура Инициализация(БезОткрытияФормы = Ложь) Экспорт
	...
	ОтобразитьНомерКорректировки(ЭтаФорма, НомерКорректировки);
...
Процедура СформироватьСпискиВыбора()
	...
	МакетСоставаПоказателей = ОбъектОтчета(ЭтаФорма.ИмяФормы).ПолучитьМакет("СпискиВыбора2013Кв2");
...
Процедура ВосстановитьСохраненныеДанные()
	...
	Для Каждого Раздел Из мСвойстваРазделовДекларации.ПолучитьЭлементы() Цикл	
		...
		Если НЕ Раздел.МногостраничностьВРазделе Тогда
			...
			Если ВерсияФормы <> СтруктураРеквизитовФормы.мВерсияФормы
			   И (Раздел.ИмяСтраницы = "Раздел1_1" ИЛИ Раздел.ИмяСтраницы = "Раздел2_6_7") Тогда
			   
			   Если Раздел.ИмяСтраницы = "Раздел1_1" Тогда
				   
				   П0000100163.Добавить();
				   П0000100164.Добавить();
				   П0000100165.Добавить();
				   
			   ИначеЕсли Раздел.ИмяСтраницы = "Раздел2_6_7" Тогда
...
Функция ОпределитьТабличноеПолеПоИдМногострочногоРаздела(ИдГруппы, ИдГрупп) Экспорт
	...
	Если ИдГруппы = ИдГрупп.ИдМСЧ_Раздел11_1 ИЛИ ИдГруппы = ИдГрупп.ИдМСЧ_Раздел11_2 ИЛИ ИдГруппы = ИдГрупп.ИдМСЧ_Раздел11_3 Тогда
		
		ТекТабличноеПоле = "Раздел1_1";
		
	ИначеЕсли ИдГруппы = ИдГрупп.ИдМСЧ_Раздел267_1 ИЛИ ИдГруппы = ИдГрупп.ИдМСЧ_Раздел267_2 ИЛИ ИдГруппы = ИдГрупп.ИдМСЧ_Раздел267_3 Тогда	
...
Процедура СкорректироватьМакетРаздела1_1(ИмяГруппы, СтруктураГруппыКоличество) Экспорт
	...
	Если ИмяГруппы <> СтруктураРеквизитовФормы.ИдМСЧ_Раздел11_1 И ИмяГруппы <> СтруктураРеквизитовФормы.ИдМСЧ_Раздел11_2 И ИмяГруппы <> СтруктураРеквизитовФормы.ИдМСЧ_Раздел11_3 Тогда
		Возврат;
	КонецЕсли;
...
Процедура ВставитьДополнительнуюСтроку(ТекущаяСтраница, ИмяГруппы)
	...
	СпециальногоДобавленияСтроки(ТекТабличноеПоле, ТекТабличноеПолеИмя, ИмяГруппы, НомерВерхЯчейкиОбласти, НомерНижнЯчейкиОбласти, СтруктураГруппы, НомерПоследнейСтроки, ВерхОбластиМногострочнойЧасти, ВысотаСтроки, ИмяОбластиМСЧ);
...
Процедура УдалитьДополнительнуюСтрокуНаСервере(СпециальноеДобавление = Ложь, ИмяКолонкиНомера = Неопределено) Экспорт
	...
	Если ИмяГруппы = СтруктураРеквизитовФормы.ИдМСЧ_Раздел11_1 ИЛИ ИмяГруппы = СтруктураРеквизитовФормы.ИдМСЧ_Раздел11_2 ИЛИ ИмяГруппы = СтруктураРеквизитовФормы.ИдМСЧ_Раздел11_3
	 ИЛИ ИмяГруппы = СтруктураРеквизитовФормы.ИдМСЧ_Раздел267_1 ИЛИ ИмяГруппы = СтруктураРеквизитовФормы.ИдМСЧ_Раздел267_2 ИЛИ ИмяГруппы = СтруктураРеквизитовФормы.ИдМСЧ_Раздел267_3 Тогда
...
Процедура УдалитьОбычнуюДополнительнуюСтроку(ПараметрыУдаления)
	...
	Если СтруктураГруппы.Количество() > ИсходноеКоличествоСтрокГруппы Тогда
		...
		МногострочностьВРазделе = РегламентированнаяОтчетностьКлиентСервер.ПолучитьСвойствоРаздела(ЭтаФорма, ТекущаяСтраница, "МногострочностьВРазделе");
...
Процедура ДобавитьНеобходимоеКоличествоДопСтрокНаСервере(ТекТабличноеПоле, ТекТабличноеПолеИмя, ИсходноеКоличествоСтрокГруппы, ТекущееКоличествоСтрокГруппы, ИмяГруппы, СтруктураГруппы) Экспорт
	...
	ОбластьМСЧ = ТекТабличноеПоле.Область(ВерхОбластиМногострочнойЧасти, , ВерхОбластиМногострочнойЧасти + МаксимальноеКоличествоСтрокГруппы - 1, );
...
Процедура ОчисткаРегламентированногоОтчетаНаСервере(ИмяСтраницы, ЗапускатьОтдельнуюОбработкуПослеОчисткиЛиста, РежимОчистки, Многостраничность, КолВоСтраницВРазделе) Экспорт
	...
	Если ИмяСтраницы = "Раздел1_1" Тогда
...
Процедура ВывестиНаПечатьЛист(Парам1, ОбщиеРеквизитыРазделов, СтрПоля = Неопределено)
	...
	СоответствиеРазделовМакетам.Вставить("Титульный",    "ПечатнаяФормаОтчета2013Кв2_Титульный");
	СоответствиеРазделовМакетам.Вставить("Раздел1_1",    "ПечатнаяФормаОтчета2013Кв2_Раздел1_1");
	СоответствиеРазделовМакетам.Вставить("Раздел1_2",    "ПечатнаяФормаОтчета2013Кв2_Раздел1_2");
	СоответствиеРазделовМакетам.Вставить("Раздел1_3",    "ПечатнаяФормаОтчета2013Кв2_Раздел1_3");
	СоответствиеРазделовМакетам.Вставить("Раздел1_31",   "ПечатнаяФормаОтчета2013Кв2_Раздел1_31");
	СоответствиеРазделовМакетам.Вставить("Раздел1_4",    "ПечатнаяФормаОтчета2013Кв2_Раздел1_4");
	СоответствиеРазделовМакетам.Вставить("Раздел1_41",   "ПечатнаяФормаОтчета2013Кв2_Раздел1_41");
	СоответствиеРазделовМакетам.Вставить("Раздел1_42",   "ПечатнаяФормаОтчета2013Кв2_Раздел1_42");
	СоответствиеРазделовМакетам.Вставить("Раздел1_43",   "ПечатнаяФормаОтчета2013Кв2_Раздел1_43");
	СоответствиеРазделовМакетам.Вставить("Раздел1_44",   "ПечатнаяФормаОтчета2013Кв2_Раздел1_44");
	СоответствиеРазделовМакетам.Вставить("Раздел1_5",    "ПечатнаяФормаОтчета2013Кв2_Раздел1_5");
	СоответствиеРазделовМакетам.Вставить("Раздел2_6_7",  "ПечатнаяФормаОтчета2013Кв2_Раздел2_6_7");
	СоответствиеРазделовМакетам.Вставить("Раздел2_8_9",  "ПечатнаяФормаОтчета2013Кв2_Раздел2_8_9");
	СоответствиеРазделовМакетам.Вставить("Раздел2_10",   "ПечатнаяФормаОтчета2013Кв2_Раздел2_10");
		...
		ИначеЕсли Парам1.ИмяСтраницы = "Раздел2_6_7" Тогда
			...
			Для НомГруппы = 1 По 3 Цикл
				...
				Если КолвоСтрокВТаблДок > КолвоСтрокВПечатнойФорме Тогда
					...
					ИначеЕсли ИдГруппы = СтруктураРеквизитовФормы.ИдМСЧ_Раздел267_2 ИЛИ ИдГруппы = СтруктураРеквизитовФормы.ИдМСЧ_Раздел267_3 Тогда
	...
	Пока РегламентированнаяОтчетностьКлиентСервер.СвойствоОпределено(ДанныеМСЧ, ИмяПоказателяНомераПП + "_" + Формат(НомСтр, "ЧГ=0")) Цикл
		...
		Если ЗначениеЗаполнено(НомерПП) ИЛИ ЗначениеЗаполнено(ДатаПП) ИЛИ ЗначениеЗаполнено(СуммаПП) Тогда
			Если НЕ (ЗначениеЗаполнено(НомерПП) И ЗначениеЗаполнено(ДатаПП) И ЗначениеЗаполнено(СуммаПП)) Тогда
				ТекстСообщенияОбОшибке = "Не полностью заполнены сведения о платежном поручении " + ОписаниеРаздела + "!";
				РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма, ТекстСообщенияОбОшибке, ИмяРаздела, ИмяПоказателяДатыПП + Посфикс);
				СтатусПроверки = Ложь;
			КонецЕсли;
		КонецЕсли;
...
Функция ИзвлечьСтруктуруXML()
	...
	Макет = ОбъектОтчета(ЭтаФорма.ИмяФормы).ПолучитьМакет("СхемаВыгрузкиXML05_2013");
...
Функция ВыгрузкаВозможна(ПараметрыВыгрузки, ДеревоВыгрузки)
	...
	Если ЗначениеЗаполнено(ПараметрыВыгрузки.ОКВЭДРаздел1) И (ПараметрыВыгрузки.ОКВЭДРаздел1 <> ПараметрыВыгрузки.ОКВЭДРаздел2) Тогда
		РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма, "Различаются значения кодов по ОКВЭД раздела I и раздела II!", "Раздел1_1", "ОКВЭД1");
		СтатусПроверки = Ложь;
	КонецЕсли;
	
	ВозможныеЗначенияШифра1 = СтроковыеФункцииКлиентСервер.РазложитьСтрокуВМассивПодстрок("071,032,041,042,043,061,051,081,091,101,121,131,141,151,161,171");
...
Процедура Расшифровать(Команда)
	...
	Если СтрЧислоВхождений(Элементы.ТабличныйДокумент.ТекущаяОбласть.Имя, СтруктураРеквизитовФормы.мИдГруппы1) > 0 Тогда
		Раздел31ФИО = ТабличныйДокумент.Области[СтруктураРеквизитовФормы.мИдГруппы1 + "02_" + Сред(Элементы.ТабличныйДокумент.ТекущаяОбласть.Имя, СтрНайти(Элементы.ТабличныйДокумент.ТекущаяОбласть.Имя, "_") + 1)].Значение;
	КонецЕсли;
