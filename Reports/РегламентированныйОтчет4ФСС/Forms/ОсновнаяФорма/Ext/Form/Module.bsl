...
Процедура ПоказатьПериод(Форма)
	...
	ФормаУтратилаСилу = ГодПериода >= 2023;
	...
	Иначе
		...
		НСтр("ru='С 01.01.2023 Постановлением Правления ПФР от 31.10.2022 N 245п введена единая форма ЕФС-1'"),
...
Процедура ИзменитьПериод(Форма, Шаг)
	...
	Если КонецДня(Форма.мДатаКонцаПериодаОтчета) > КонецДня('20220331') Тогда
	...
	ИначеЕсли Год(Форма.мДатаКонцаПериодаОтчета) >= 2010 Тогда
