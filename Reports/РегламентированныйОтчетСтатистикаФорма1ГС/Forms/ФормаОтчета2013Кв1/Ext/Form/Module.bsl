...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	//
	// Версия формы
	СтруктураРеквизитовФормы.мВерсияФормы = "28/07/2014";
...
Процедура Расчет(Форма, ИмяСтраницыПанели = "", ОбластьИмя = "")
	...
	Для Стр = 301 По 323 Цикл
		Области["П03"+Стр+"03"].Значение = Области["П03"+Стр+"04"].Значение + Области["П03"+Стр+"07"].Значение + Области["П03"+Стр+"10"].Значение
										 + Области["П03"+Стр+"11"].Значение + Области["П03"+Стр+"12"].Значение + Области["П03"+Стр+"13"].Значение
										 + Области["П03"+Стр+"14"].Значение + Области["П03"+Стр+"15"].Значение + Области["П03"+Стр+"16"].Значение
										 + Области["П03"+Стр+"17"].Значение;
	КонецЦикла;
	
	Для Стр = 101 По 121 Цикл
		Области["П01"+Стр+"04"].Значение = Области["П01"+Стр+"06"].Значение + Области["П01"+Стр+"07"].Значение + Области["П01"+Стр+"08"].Значение
										 + Области["П01"+Стр+"09"].Значение + Области["П01"+Стр+"10"].Значение + Области["П01"+Стр+"11"].Значение;
		Области["П01"+Стр+"12"].Значение = Области["П01"+Стр+"13"].Значение + Области["П01"+Стр+"14"].Значение + Области["П01"+Стр+"15"].Значение
										 + Области["П01"+Стр+"16"].Значение + Области["П01"+Стр+"18"].Значение + Области["П01"+Стр+"19"].Значение;
	КонецЦикла;
...
Функция ПроверитьКонтрольныеСоотношенияВОтчете() Экспорт
	...
	Нарушение = "Приказ Росстата от 18.07.2013 № 285";
	
	// Раздел 1
	
	МассивБ = СтроковыеФункцииКлиентСервер.РазложитьСтрокуВМассивПодстрок("3;4;4;6;7;8;9;10;11;16", ";");
	МассивМ = СтроковыеФункцииКлиентСервер.РазложитьСтрокуВМассивПодстрок("4;5;12;13;14;15;16;18;19;17", ";");
	
	Для Стр = 101 По 121 Цикл
		Для Инд = 0 По МассивБ.Количество() - 1 Цикл 
			ЛеваяЧасть  = Показатели["П01" + Стр + Формат(Число(МассивБ[Инд]), "ЧЦ=2; ЧВН=; ЧН=00")];
			ПраваяЧасть  = Показатели["П01" + Стр + Формат(Число(МассивМ[Инд]), "ЧЦ=2; ЧВН=; ЧН=00")];
			ТекстФормулы = "гр." + МассивБ[Инд] + " >= гр." + МассивМ[Инд] + " по стр." + Стр;
			Описание = "гр." + МассивБ[Инд] + " < гр." + МассивМ[Инд] + " по стр." + Стр;
...
Процедура ЗагрузитьПодготовленныеДанные()
	...
	ТекТабличноеПоле.Вывести(
		Отчеты[Сред(Лев(ЭтаФорма.ИмяФормы, СтрНайти(ЭтаФорма.ИмяФормы, ".Форма.") - 1), 7)].ПолучитьМакет("ФормаОтчета2013Кв1_ФормаОтчета"));
