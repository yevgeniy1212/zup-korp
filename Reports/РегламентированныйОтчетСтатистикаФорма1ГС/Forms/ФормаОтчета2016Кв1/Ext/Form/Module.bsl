...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	//
	// Версия формы
	СтруктураРеквизитовФормы.мВерсияФормы = "08/07/2016";
...
Процедура Инициализация(БезОткрытияФормы = Ложь) Экспорт
	...
	Если ПолеТабличногоДокументаФормаОтчета.Области.ОргУровеньГосОргана.Значение = "Федеральный" Тогда
		ИмяМакета = "АтрибВыгрузкиXML2016Кв1_фед";
	Иначе
		ИмяМакета = "АтрибВыгрузкиXML2016Кв1_рег";
	КонецЕсли;
...
Процедура Расчет(Форма, ИмяСтраницыПанели = "", ОбластьИмя = "")
	...
	Для Стр = 301 По 323 Цикл
		Области["П03"+Стр+"03"].Значение = Области["П03"+Стр+"04"].Значение + Области["П03"+Стр+"06"].Значение + Области["П03"+Стр+"08"].Значение
										 + Области["П03"+Стр+"09"].Значение + Области["П03"+Стр+"10"].Значение + Области["П03"+Стр+"11"].Значение
										 + Области["П03"+Стр+"12"].Значение + Области["П03"+Стр+"13"].Значение + Области["П03"+Стр+"14"].Значение;
	КонецЦикла;
...
Функция УниверсальнаяВыгрузкаXMLПроверкаДанныхНаСервере(ТекстВыгрузки, ПараметрыВыгрузки) Экспорт
	...
	Если ПолеТабличногоДокументаФормаОтчета.Области.ОргУровеньГосОргана.Значение = "Федеральный" Тогда
		ИмяМакета = "АтрибВыгрузкиXML2016Кв1_фед";
		ИмяШаблона = "ШаблонРосстата2016Кв1_фед";
	Иначе
		ИмяМакета = "АтрибВыгрузкиXML2016Кв1_рег";
		ИмяШаблона = "ШаблонРосстата2016Кв1_рег";
	КонецЕсли;
	РегламентированнаяОтчетность.ИзменитьВерсиюФорматаСПроверкойСовместимостиШаблонов(ЭтотОбъект, ИмяМакета, ИмяШаблона, ПараметрыВыгрузки);
	ЗаполнитьДанными(ДеревоВыгрузки, ПараметрыВыгрузки);
	Если НЕ РегламентированнаяОтчетность.ПроверитьДанные(ЭтаФорма, ДеревоВыгрузки) Тогда
...
Функция ПолучитьПараметрыИнициализацииУниверсальногоОтчета()
	...
	Иначе
		...
		ИмяШаблона = "ШаблонРосстата2016Кв1_рег";
	КонецЕсли;
	Возврат УниверсальныйОтчетСтатистики.ПолучитьПараметрыИнициализацииУниверсальногоОтчета(ЭтаФорма, ИмяМакета, ИмяШаблона, "СхемаВыгрузкиXML2016Кв1", Истина, Ложь);
КонецФункции
