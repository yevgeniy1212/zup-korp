...
Функция СформироватьСтруктуруПараметров(ИмяФормы,
	...
	ПараметрыВыгрузки.Вставить("ОКТМО", КодМуниципальногоОбразованияПоОКТМО(СокрЛП(Титульный.ОКТМО)));
		...
		Если ЗначениеЗаполнено(ПараметрыВыгрузки.ФормРеорг) Тогда
			...
			ИначеЕсли ПараметрыВыгрузки.ФормРеорг <> "0" Тогда
				...
				Иначе
					...
					Если Символы56КПП = "50" Тогда
						СообщитьВТС(ИмяФормы, РезультатВыгрузки,
						"5-й и 6-й символы КПП " + ЧастьСообщения + " не должны быть равны ""50"".",
						"Титульный", "КППРеорганизованнойОрганизации");
						СтатусПроверки = Ложь;
					КонецЕсли;
				КонецЕсли;
	...
	Если Узел.Формат = "T" ИЛИ Узел.Формат = "Т" Тогда // Учтем оба варианта: кириллицу и латиницу.
		Если ЗначениеЗаполнено(ЗначениеПоказателя) Тогда
			Если ТипЗнч(ЗначениеПоказателя) = Тип("Дата") Тогда
				ЗначениеПоказателяСтр = Формат(ЗначениеПоказателя, "ДФ=дд.ММ.гггг");
			Иначе
...
Процедура ЗаполнитьПечатнуюФорму(ИмяФормы,
	...
	Для Каждого Показатель Из Показатели Цикл
		Если СтрНайти(Показатель.Имя, "ОКТМО") > 0 Тогда
			Показатель.Значение = КодМуниципальногоОбразованияПоОКТМО(Показатель.Значение);
...
Функция КодМуниципальногоОбразованияПоОКТМО(Знач ОКТМО) Экспорт
	
	ОКТМО = СокрЛП(ОКТМО);
	
	Если СтрДлина(ОКТМО) >= 8 Тогда
		ОКТМО = Лев(ОКТМО, 8);
	КонецЕсли;
