...
Функция КодПоМестуДляЮрЛицаПослеВыбораКодаИФНС(СсылкаНаРегистрациюВНО)
	...
	Если СтруктураРеквизитовФормы.Организация
		...
		СтруктураПараметров
		= Новый Структура("Организация, ДатаЗначения", СтруктураРеквизитовФормы.Организация, ДатаПодписи);
		КодПоМесту = ?(РегламентированнаяОтчетность.ОрганизацияЯвляетсяКрупнейшимНалогоплательщиком(СтруктураПараметров),
		"213", "214");
...
Процедура ПроверитьКС_3_6(СтруктураРезультата)
	...
	Заголовки.Вставить("Комментарий"		, 
		НСтр("ru='Отключено по многочисленным обращениям пользователей'"));
	...
	Если Истина Или Документирование Тогда
		Возврат;
	КонецЕсли;
...
Процедура ПроверитьКС_3_7(СтруктураРезультата)
	...
	Заголовки.Вставить("Условие"			, НСтр("ru='Отключено для 2020-2021г в связи с короновирусом'"));
	Заголовки.Вставить("Комментарий"		, НСтр("ru=''"));
