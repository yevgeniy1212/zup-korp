...
Процедура Расчет(Форма, ИмяТекущейСтраницыПанели = "", ОбластьИмя = "")
	...
	Для Гр = 3 По 19 Цикл
		...
		СуммаПоСтр = 0;
		Для Стр = 317 По 320 Цикл
			СуммаПоСтр = СуммаПоСтр + Области["П0030010" + Стр + Графа].Значение;
		КонецЦикла;
		Области["П0030010316" + Графа].Значение = СуммаПоСтр;
		
		СуммаПоСтр = 0;
		Для Стр = 312 По 315 Цикл
			СуммаПоСтр = СуммаПоСтр + Области["П0030010" + Стр + Графа].Значение;
		КонецЦикла;
		Области["П0030010311" + Графа].Значение = СуммаПоСтр;
		
		СуммаПоСтр = 0;
		Для Стр = 308 По 310 Цикл
			СуммаПоСтр = СуммаПоСтр + Области["П0030010" + Стр + Графа].Значение;
		КонецЦикла;
		Области["П0030010307" + Графа].Значение = СуммаПоСтр;
		
		СуммаПоСтр = 0;
		Для Стр = 304 По 306 Цикл
			СуммаПоСтр = СуммаПоСтр + Области["П0030010" + Стр + Графа].Значение;
		КонецЦикла;
		Области["П0030010303" + Графа].Значение = СуммаПоСтр;
		
		Области["П0030010302" + Графа].Значение
			= Области["П0030010303" + Графа].Значение
			+ Области["П0030010307" + Графа].Значение
			+ Области["П0030010311" + Графа].Значение
			+ Области["П0030010316" + Графа].Значение;
		
		Области["П0030010321" + Графа].Значение
			= Области["П0030010301" + Графа].Значение
			+ Области["П0030010302" + Графа].Значение;
		
	КонецЦикла;
	
	Для Стр = 301 По 321 Цикл
		СуммаПоГрафам = 0;
		Для Гр = 13 По 19 Цикл
			Графа = Формат(Гр, "ЧЦ=2; ЧВН=");
			СуммаПоГрафам = СуммаПоГрафам + Области["П0030010" + Стр + Графа].Значение;
		КонецЦикла;
		Области["П0030010" + Стр + "12"].Значение = СуммаПоГрафам;
	КонецЦикла;
	...
	Для Гр = 3 По 9 Цикл
		...
		Области["П0040010403" + Графа].Значение
			= Области["П0040010404" + Графа].Значение
			+ Области["П0040010405" + Графа].Значение
			+ Области["П0040010406" + Графа].Значение;
			
		Области["П0040010407" + Графа].Значение
			= Области["П0040010408" + Графа].Значение
			+ Области["П0040010409" + Графа].Значение
			+ Области["П0040010410" + Графа].Значение;
			
		Области["П0040010411" + Графа].Значение
			= Области["П0040010412" + Графа].Значение
			+ Области["П0040010413" + Графа].Значение
			+ Области["П0040010414" + Графа].Значение
			+ Области["П0040010415" + Графа].Значение;
			
		Области["П0040010416" + Графа].Значение
			= Области["П0040010417" + Графа].Значение
			+ Области["П0040010418" + Графа].Значение
			+ Области["П0040010419" + Графа].Значение
			+ Области["П0040010420" + Графа].Значение;
			
		Области["П0040010421" + Графа].Значение
			= Области["П0040010422" + Графа].Значение
			+ Области["П0040010423" + Графа].Значение;
		
		Области["П0040010402" + Графа].Значение
			= Области["П0040010403" + Графа].Значение
			+ Области["П0040010407" + Графа].Значение
			+ Области["П0040010411" + Графа].Значение
			+ Области["П0040010416" + Графа].Значение;
	...
	Для Гр = 4 По 18 Цикл
		...
		Области["П0050010507" + Графа].Значение
			= Области["П0050010508" + Графа].Значение
			+ Области["П0050010509" + Графа].Значение
			+ Области["П0050010510" + Графа].Значение;
			
		Области["П0050010511" + Графа].Значение
			= Области["П0050010512" + Графа].Значение
			+ Области["П0050010513" + Графа].Значение
			+ Области["П0050010514" + Графа].Значение
			+ Области["П0050010515" + Графа].Значение;
			
		Области["П0050010516" + Графа].Значение
			= Области["П0050010517" + Графа].Значение
			+ Области["П0050010518" + Графа].Значение
			+ Области["П0050010519" + Графа].Значение
			+ Области["П0050010520" + Графа].Значение;
		
		Области["П0050010521" + Графа].Значение
			= Области["П0050010522" + Графа].Значение
			+ Области["П0050010523" + Графа].Значение;
	...
	Для Стр = 501 По 523 Цикл
		СуммаПоГрафам = 0;
		Для Гр = 4 По 16 Цикл
			Если Гр = 6 Или Гр = 10 Или Гр = 11 Или Гр = 12 Тогда
				Продолжить;
			КонецЕсли;
			Графа = Формат(Гр, "ЧЦ=2; ЧВН=");
			СуммаПоГрафам = СуммаПоГрафам + Области["П0050010" + Стр + Графа].Значение;
		КонецЦикла;
		Области["П0050010" + Стр + "03"].Значение = СуммаПоГрафам;
	КонецЦикла;
	...
	Для Гр = 3 По 6 Цикл
		...
		Области["П0060010603" + Графа].Значение
			= Области["П0060010604" + Графа].Значение
			+ Области["П0060010605" + Графа].Значение
			+ Области["П0060010606" + Графа].Значение;
		
		Области["П0060010607" + Графа].Значение
			= Области["П0060010608" + Графа].Значение
			+ Области["П0060010609" + Графа].Значение
			+ Области["П0060010610" + Графа].Значение;
		
		Области["П0060010611" + Графа].Значение
			= Области["П0060010612" + Графа].Значение
			+ Области["П0060010613" + Графа].Значение
			+ Области["П0060010614" + Графа].Значение
			+ Области["П0060010615" + Графа].Значение;
		
		Области["П0060010616" + Графа].Значение
			= Области["П0060010617" + Графа].Значение
			+ Области["П0060010618" + Графа].Значение
			+ Области["П0060010619" + Графа].Значение
			+ Области["П0060010620" + Графа].Значение;
		
		Области["П0060010621" + Графа].Значение
			= Области["П0060010622" + Графа].Значение
			+ Области["П0060010623" + Графа].Значение;
...
Функция ПечатьНаСервере(ВидПечати)
	...
	ПечатнаяФорма = РегламентированнаяОтчетность.НоваяСтраница();
	ПечатнаяФорма.Вывести(ТабличныйДокумент.ПолучитьОбласть("ОсновнаяЧасть5_2"));
	ПечатнаяФорма.Вывести(ТабличныйДокумент.ПолучитьОбласть("ОсновнаяЧасть5_4"));
	СтруктураРеквизитовФормы.мПечатныеФормы.Добавить(ПечатнаяФорма, "Форма 1-Т (ГС)");
