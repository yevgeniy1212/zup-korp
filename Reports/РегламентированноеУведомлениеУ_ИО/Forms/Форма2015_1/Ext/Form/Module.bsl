...
Процедура СобратьДанныеСТекущейСтраницы()
	Если UID_СведУч <> UID_Пустой И ТекущийТипСтраницы = 4 Тогда
...
Процедура УстановитьДоступностьКнопок()
	...
	Если ТипСтраницы = 0 И Элементы.Разделы.ТекущиеДанные.ПолучитьЭлементы().Количество() > 0 Тогда 
		ТипСтраницы = Элементы.Разделы.ТекущиеДанные.ПолучитьЭлементы()[0].ТипСтраницы;
	КонецЕсли;
	
	КМенюРО = Элементы.Разделы.КонтекстноеМеню;
	Если ТипСтраницы = 2 Или ТипСтраницы = 3 Или ТипСтраницы = 4 Или ТипСтраницы = 6 Или ТипСтраницы = 7 Тогда
		КМенюРО.Видимость = Истина;
		КМенюРО.ПодчиненныеЭлементы.РазделыКонтекстноеМенюДобавитьСтраницу.Видимость = Истина;
		КМенюРО.ПодчиненныеЭлементы.РазделыКонтекстноеМенюУдалитьСтраницу.Видимость = (Элементы.Разделы.ТекущиеДанные.ПолучитьЭлементы().Количество() = 0);
		КМенюРО.ПодчиненныеЭлементы.РазделыКонтекстноеМенюДобавитьСтраницу.Доступность = Истина;
		Кво = Элементы.Разделы.ТекущиеДанные.ПолучитьЭлементы().Количество();
		Если Кво = 0 Тогда
			Кво = Элементы.Разделы.ТекущиеДанные.ПолучитьРодителя().ПолучитьЭлементы().Количество();
		КонецЕсли;
		КМенюРО.ПодчиненныеЭлементы.РазделыКонтекстноеМенюУдалитьСтраницу.Доступность = (Кво > 1);
	Иначе
...
Процедура СформироватьМакетНаСервере()
	...
	Макет = УведомлениеОСпецрежимахНалогообложения.ПолучитьМакетТабличногоДокумента(ЭтотОбъект, "Форма2015_1");
...
Процедура РазделыПриАктивизацииСтроки(Элемент)
	...
	Если Элемент.ТекущиеДанные.ТипСтраницы = 0 Тогда
		...
		Если ТекущийТипСтраницы = 4 Тогда 
			UID_СведУч = ПодчиненныеЭлементыДерева[0].UID;
		КонецЕсли;
...
Процедура ПересчитатьДанныеЦепочек()
	НомерУчастника =НРег(ПредставлениеУведомления.Области["НомерУчастника"].Значение);
	Если Не ЗначениеЗаполнено(НомерУчастника) Или Не СтрНачинаетсяС(НомерУчастника, "ио-") Тогда 
	...
	ЛистыА = СведИО.НайтиСтроки(Новый Структура("ИО_Номер", ИО_Номер));
	Если ЛистыА.Количество() = 0 Тогда 
		ОбщегоНазначенияКлиент.СообщитьПользователю(НСтр("ru='Не найден соответствующий лист А'"));
		Возврат;
	ИначеЕсли ЛистыА.Количество() > 1 Тогда 
		ОбщегоНазначенияКлиент.СообщитьПользователю(НСтр("ru='Найдено более одного листа А с указанным номером'"));
		Возврат;
	КонецЕсли;
	
	ОписаниеОповещения = Новый ОписаниеОповещения("ПересчитатьДанныеЦепочекЗавершение", ЭтотОбъект, Новый Структура("НомерУчастника, ЛистыА", НомерУчастника, ЛистыА));
...
Процедура ПересчитатьДанныеЦепочекЗавершение(РезультатВопроса, ДополнительныеПараметры) Экспорт
	Если РезультатВопроса = КодВозвратаДиалога.Да Тогда
		...
		ДополнительныеПараметры.ЛистыА[0].ДоляУчастия1 = ОбщаяДоляУчастия;
		Модифицированность = Истина;
	КонецЕсли;
КонецПроцедуры
...
Процедура ПересчитатьДанныеЦепочекНаСервере(НомерУчастника, ОбщаяДоляУчастия)
	...
	Для Каждого Стр Из СведУч Цикл 
		Если НРег(Стр.НомерУчастника) <> НомерУчастника Тогда
			Продолжить;
		КонецЕсли;
		
		МнгСтр = МногострочнаяЧасть.НайтиСтроки(Новый Структура("UID", Стр.UID));
		ДУчИзн = Число(МнгСтр[0].ДоляПрУчастия);
		Для Инд = 1 По МнгСтр.ВГраница() Цикл 
			ДУчИзн = ДУчИзн * Число(МнгСтр[Инд].ДоляПрУчастия) / 100;
			МнгСтр[Инд].ДоляКосвУчастия = ДУчИзн;
		КонецЦикла;
		
		Стр.ДоляКосвУчастияГр = ДУчИзн;
		ОбщаяДоляУчастия = ОбщаяДоляУчастия + ДУчИзн;
	КонецЦикла;
	
	Для Каждого Стр Из СведУч Цикл 
		Если НРег(Стр.НомерУчастника) = НомерУчастника Тогда
			Стр.ДоляКосвУчастия = ОбщаяДоляУчастия;
		КонецЕсли;
	КонецЦикла;
		...
		Если Автонумерация = Истина Тогда 
			Если Лев(Результат, 3) = "ИО-" Тогда 
				Если СведИО.НайтиСтроки(Новый Структура("ИО_Номер", ЧисловойНомер)).Количество() = 0 Тогда 
					ОбщегоНазначенияКлиент.СообщитьПользователю("Не найдена иностранная организация с номером " + ЧисловойНомер);
					Возврат;
				КонецЕсли;
			ИначеЕсли Лев(Результат, 3) = "РО-" Тогда
				Если СведРО.НайтиСтроки(Новый Структура("РО_Номер", ЧисловойНомер)).Количество() = 0 Тогда 
					ОбщегоНазначенияКлиент.СообщитьПользователю("Не найдена российская организация с номером " + ЧисловойНомер);
					Возврат;
				КонецЕсли;
			ИначеЕсли Лев(Результат, 3) = "ИС-" Тогда
				Если СведИС.НайтиСтроки(Новый Структура("НомерСтруктуры", ЧисловойНомер)).Количество() = 0 Тогда 
					ОбщегоНазначенияКлиент.СообщитьПользователю("Не найдена иностранная структура с номером " + ЧисловойНомер);
					Возврат;
				КонецЕсли;
...
Функция СформироватьВыгрузкуИПолучитьДанные() Экспорт 
	...
	СтруктураВыгрузки.Вставить("ИмяФайла", "1120411_5.01000_01.tif");
...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	Если ИмяСобытия = "УведомлениеОСпецрежимахНалогообложения_НавигацияПоОшибкам" Тогда 
		...
		ИначеЕсли Параметр.ИмяСтраницы = "СведИО" Тогда 
			ТекущийТипСтраницы = 2;
		ИначеЕсли Параметр.ИмяСтраницы = "СведРО" Тогда 
			ТекущийТипСтраницы = 3;
		ИначеЕсли Параметр.ИмяСтраницы = "СведИС" Тогда 
			ТекущийТипСтраницы = 6;
		ИначеЕсли Параметр.ИмяСтраницы = "СведИСУч" Тогда 
			ТекущийТипСтраницы = 7;
		ИначеЕсли Параметр.ИмяСтраницы = "СведУч" Тогда 
			ТекущийТипСтраницы = 4;
		КонецЕсли;
