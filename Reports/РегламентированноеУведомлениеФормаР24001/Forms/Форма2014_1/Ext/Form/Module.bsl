...
Процедура ВывестиМногострочнуюЧасть(МногострочнаяЧасть)
	...
	Если ТекущееИДНаименования = "ЛистЕ1" Тогда 
		Макет = УведомлениеОСпецрежимахНалогообложения.ПолучитьМакетТабличногоДокумента(ЭтотОбъект, "Форма2014_1_Страница7");
	ИначеЕсли ТекущееИДНаименования = "ЛистЕ2" Тогда
		Макет = УведомлениеОСпецрежимахНалогообложения.ПолучитьМакетТабличногоДокумента(ЭтотОбъект, "Форма2014_1_Страница8");
	КонецЕсли;
...
Функция СформироватьВыгрузкуИПолучитьДанные() Экспорт 
	...
	СтруктураВыгрузки.Вставить("ИмяФайла", "1112502_5.01000_04.tif");
