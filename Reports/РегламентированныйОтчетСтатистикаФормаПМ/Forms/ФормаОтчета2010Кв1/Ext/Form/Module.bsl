...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	//
	// Версия формы
	СтруктураРеквизитовФормы.мВерсияФормы = "23/12/2009";
	// Код отчета, равен коду по ОКУД
	СтруктураРеквизитовФормы.мКодОтчета   = "0601013";	
	
	СтруктураРеквизитовФормы.мПечатныеФормы = Новый СписокЗначений;
	
	ПолеТабличногоДокументаФормаОтчета.Вывести(ОбъектОтчета(ЭтаФорма.ИмяФормы).ПолучитьМакет("ФормаОтчета2010Кв1_ФормаОтчета"));
...
Процедура ПолеТабличногоДокументаФормаОтчетаВыбор(Элемент, Область, СтандартнаяОбработка)
	...
	Иначе
		...
		массивСрок = РегламентированнаяОтчетностьКлиентСервер.РазобратьСтрокуВМассивПоРазделителю("П040002201.П040002301");
		Если массивСрок.Найти(Область.Имя) <> Неопределено Тогда 
			СтандартнаяОбработка = Ложь;
			Для Идн = 2 По 3 Цикл
				Если ("П040002"+Идн+"01") = Область.Имя Тогда
					ЭтаФорма.ПолеТабличногоДокументаФормаОтчета.Области["П040002"+Идн+"01"].Значение = 
					?(ЭтаФорма.ПолеТабличногоДокументаФормаОтчета.Области["П040002"+Идн+"01"].Значение = "", "V", "");
				Иначе	 
					ЭтаФорма.ПолеТабличногоДокументаФормаОтчета.Области["П040002"+Идн+"01"].Значение = "";
				КонецЕсли;
			КонецЦикла;
...
Процедура Расчет(Форма, ИмяСтраницыПанели = "") Экспорт
	...
	мФормаОтчета.Области.П010000109.Значение = мФормаОтчета.Области.П010000209.Значение + мФормаОтчета.Области.П010000309.Значение + мФормаОтчета.Области.П010000409.Значение;
	мФормаОтчета.Области.П010000110.Значение = мФормаОтчета.Области.П010000210.Значение + мФормаОтчета.Области.П010000310.Значение + мФормаОтчета.Области.П010000410.Значение;

	мФормаОтчета.Области.П010000111.Значение = мФормаОтчета.Области.П010000211.Значение + мФормаОтчета.Области.П010000311.Значение + мФормаОтчета.Области.П010000411.Значение + мФормаОтчета.Области.П010000511.Значение;
	мФормаОтчета.Области.П010000112.Значение = мФормаОтчета.Области.П010000212.Значение + мФормаОтчета.Области.П010000312.Значение + мФормаОтчета.Области.П010000412.Значение + мФормаОтчета.Области.П010000512.Значение;
	мФормаОтчета.Области.П010000113.Значение = мФормаОтчета.Области.П010000213.Значение + мФормаОтчета.Области.П010000313.Значение + мФормаОтчета.Области.П010000413.Значение + мФормаОтчета.Области.П010000513.Значение;
	мФормаОтчета.Области.П010000114.Значение = мФормаОтчета.Области.П010000214.Значение + мФормаОтчета.Области.П010000314.Значение + мФормаОтчета.Области.П010000414.Значение + мФормаОтчета.Области.П010000514.Значение;
...
Функция ЗагрузкаОтчетаИзФайлаНаСервере(ДанныеДляЗагрузки)
	...
	ДеревоДляЗагрузки = РегламентированнаяОтчетность.ПолучитьДеревоВыгрузки(ЭтаФорма, "СхемаВыгрузкиXML2010Кв1");
...
Функция ПроверитьПоказатели() Экспорт
	...
	РегламентированнаяОтчетность.СформироватьСтруктуруПоказателей(ЭтаФорма);
	ДеревоВыгрузки = РегламентированнаяОтчетность.ПолучитьДеревоВыгрузки(ЭтаФорма, "СхемаВыгрузкиXML2010Кв1");
...
Функция СформироватьСтруктуруПараметров()
	...
	ПараметрыВыгрузки.Вставить("Зн_П040002201", ?(ПустаяСтрока(ТабДокумент.П040002201), "0", "1"));
	ПараметрыВыгрузки.Вставить("Зн_П040002301", ?(ПустаяСтрока(ТабДокумент.П040002301), "0", "1"));
