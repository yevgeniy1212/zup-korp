...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	//
	// Версия формы
	СтруктураРеквизитовФормы.мВерсияФормы = "25/08/2017";
...
Процедура Расчет(Форма, ИмяСтраницыПанели = "") Экспорт
	...
	Области.П020304.Значение = Области.П020404.Значение
								 + Области.П020504.Значение
								 + Области.П020604.Значение;
	
	Области.П020704.Значение = Области.П020804.Значение
								 + Области.П020904.Значение
								 + Области.П021004.Значение
								 + Области.П021104.Значение;
								 
	Области.П021204.Значение = Области.П021304.Значение
								 + Области.П021404.Значение
								 + Области.П021504.Значение
								 + Области.П021604.Значение;
...
Функция ПроверитьВозможностьВыгрузки(ПараметрыВыгрузки, ДеревоВыгрузки)
	...
	Если ПустаяСтрока(ПараметрыВыгрузки.ОКВЭД) Тогда
		РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма, "Не задан код вида деятельности по ОКВЭД2!", ИмяПоляТаблДокумента, "ОргКодОКВЭД");
		СтатусПроверки = Ложь;
	КонецЕсли;
