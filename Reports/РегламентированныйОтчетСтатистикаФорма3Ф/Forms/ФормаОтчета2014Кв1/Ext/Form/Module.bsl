...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	//
	// Версия формы.
	СтруктураРеквизитовФормы.мВерсияФормы = "20/02/2014"; 
...
Функция ПроверитьКонтрольныеСоотношенияВОтчете() Экспорт
	...
	Зн01 = Показатели["П01000103"];
	Зн02 = Показатели["П01000203"];
	Зн03 = Показатели["П01000303"];
	Зн07 = Показатели["П01000703"];
	Зн09 = Показатели["П01000903"];
	Зн10 = Показатели["П01001003"];
	Зн11 = Показатели["П01001103"];
	ТекстФормулы = "стр. 01 гр. 1 >= стр. 02 гр.1";
	Описание     = "стр. 01 гр. 1 < стр. 02 гр.1";
	Расшифровка = Строка(Зн01) + " >= " + Строка(Зн02);
	Если Зн01 >= Зн02 Тогда
		ЭтоОшибка = Ложь;
	...
	ТекстФормулы = "стр. 01 гр. 1 >= стр. 03 гр.1";
	Описание     = "стр. 01 гр. 1 < стр. 03 гр.1";
	Расшифровка = Строка(Зн01) + " >= " + Строка(Зн03);
	Если Зн01 >= Зн03 Тогда
		ЭтоОшибка = Ложь;
		ОбщееЧислоБезОшибок = ОбщееЧислоБезОшибок + 1;
	Иначе
		ЭтоОшибка = Истина;
	...
	ТекстФормулы = "стр. 07 гр. 1 >= стр. 02 гр.1";
	Описание     = "стр. 07 гр. 1 < стр. 02 гр.1";
	Расшифровка = Строка(Зн07) + " >= " + Строка(Зн02);
	Если Зн07 >= Зн02 Тогда
		ЭтоОшибка = Ложь;
		ОбщееЧислоБезОшибок = ОбщееЧислоБезОшибок + 1;
	Иначе
		ЭтоОшибка = Истина;
	...
	ТекстФормулы = "стр. 01 гр. 1 >= стр. 09 гр.1";
	Описание     = "стр. 01 гр. 1 < стр. 09 гр.1";
	Расшифровка = Строка(Зн01) + " >= " + Строка(Зн09);
	Если Зн01 >= Зн09 Тогда
		ЭтоОшибка = Ложь;
		ОбщееЧислоБезОшибок = ОбщееЧислоБезОшибок + 1;
	Иначе
		ЭтоОшибка = Истина;
	...
	ТекстФормулы = "стр. 01 гр. 1 >= стр. 10 гр.1";
	Описание     = "стр. 01 гр. 1 < стр. 10 гр.1";
	Расшифровка = Строка(Зн01) + " >= " + Строка(Зн10);
	Если Зн01 >= Зн10 Тогда
		ЭтоОшибка = Ложь;
		ОбщееЧислоБезОшибок = ОбщееЧислоБезОшибок + 1;
	Иначе
		ЭтоОшибка = Истина;
	...
	ТекстФормулы = "стр. 01 гр. 1 >= стр. 11 гр.1";
	Описание     = "стр. 01 гр. 1 < стр. 11 гр.1";
	Расшифровка = Строка(Зн01) + " >= " + Строка(Зн11);
	Если Зн01 >= Зн11 Тогда
		ЭтоОшибка = Ложь;
		ОбщееЧислоБезОшибок = ОбщееЧислоБезОшибок + 1;
	Иначе
		ЭтоОшибка = Истина;
