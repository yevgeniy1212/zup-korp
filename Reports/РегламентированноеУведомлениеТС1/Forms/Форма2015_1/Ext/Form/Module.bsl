...
Процедура ЗаполнитьАдресБСП(Лист, Адрес)
	Попытка
		АдресСтруктурой = РаботаСАдресами.СведенияОбАдресе(Адрес);
	Исключение
		Возврат;
	КонецПопытки;
...
Процедура ПолучитьПутьФайлЗавершение(ВыбранныеФайлы, ВходящийКонтекст) Экспорт
	Если ТипЗнч(ВыбранныеФайлы) = Тип("Массив") И ВыбранныеФайлы.Количество() > 0 Тогда 
		ИмяФайла = ВРег(ВыбранныеФайлы[0].ПолноеИмя);
		...
		Если ИндексНачала = 0 Тогда
			ОбщегоНазначенияКлиент.СообщитьПользователю("Имя файла должно начинаться с UT_UVTORGSB");
			Возврат;
		КонецЕсли;
		...
		Исключение
			ОбщегоНазначенияКлиент.СообщитьПользователю("Не удалось загрузить файл:" + ИмяФайла + " не соответсвует формату");
		КонецПопытки;
	КонецЕсли;
КонецПроцедуры
...
Процедура СоздатьНаОснованииУведомления(Команда)
	ОткрытьФорму("Отчет." + СтроковыеФункцииКлиентСервер.РазложитьСтрокуВМассивПодстрок(ИмяФормы, ".")[1] + ".Форма.ФормаВыбораОтправленных",
		Новый Структура("Организация", Объект.Организация), ЭтотОбъект,,,,,РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
КонецПроцедуры
