...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	//
	// Версия формы
	СтруктураРеквизитовФормы.мВерсияФормы = "03/07/2017";
...
Процедура СформироватьСпискиВыбора()
	...
	РегламентированнаяОтчетность.ДобавитьСтрокуОписанияВвода(СвойстваПоказателей, "04", 512, , "Выбор признака пола", КоллекцияСписковВыбора["Пол"]);
	РегламентированнаяОтчетность.ДобавитьСтрокуОписанияВвода(СвойстваПоказателей, "06", 512, , "Выбор признака образования", КоллекцияСписковВыбора["Образование"]);
	РегламентированнаяОтчетность.ДобавитьСтрокуОписанияВвода(СвойстваПоказателей, "13", 512, , "Выбор признака категори персонала", КоллекцияСписковВыбора["КатегорииПерсонала"]);
	РегламентированнаяОтчетность.ДобавитьСтрокуОписанияВвода(СвойстваПоказателей, "01", 512, , "Выбор профессии", КоллекцияСписковВыбора["Профессии"]);
...
Процедура ВосстановитьСохраненныеДанные()
	...
	СписокСохранения.Свойство( "мАвтоВыборКодов", СтруктураРеквизитовФормы.мАвтоВыборКодов);
	Если СтруктураРеквизитовФормы.мАвтоВыборКодов = Неопределено Тогда 
		СтруктураРеквизитовФормы.мАвтоВыборКодов = Ложь;
	КонецЕсли;
	
	СписокСохранения.Вставить("мАвтоВыборКодов", СтруктураРеквизитовФормы.мАвтоВыборКодов);
...
Процедура ПоказатьПериод()
	...
	ПолеТабличногоДокументаФормаОтчета.Области.ПериодОтчетаРаздел1.Текст = ПериодОтчета;
	...
	ЗаголовокГр7 = "Стаж работы в данной организации на "+Формат(СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета, "ДФ=dd.MM.yyyy")+" (лет, с одним десятичным знаком)";
	ЗаголовокГр8_11 = "Суммы начисленной заработной платы за октябрь "+Формат(СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета, "ДФ=yyyy")+" г., рублей";
	ЗаголовокГр12 = "Фактичес-"+Символы.ПС+"ки отрабо-"+Символы.ПС+"тано за октябрь "+Формат(СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета, "ДФ=yyyy")+" г., часов";
	...
	ПолеТабличногоДокументаФормаОтчета.Области.Гр12_3.Текст = ЗаголовокГр12;
	...
	ПолеТабличногоДокументаФормаОтчета.Области.Гр12_4.Текст = ЗаголовокГр12;
КонецПроцедуры
...
Процедура ПолеТабличногоДокументаФормаОтчетаВыбор(Элемент, Область, СтандартнаяОбработка)
	...
	ИначеЕсли СтрНайти(Область.Имя, "ВывестиПриложениеНаПечать") > 0 Тогда
		...
		КонецЕсли;
		Модифицированность = Истина;
	ИначеЕсли Лев(Область.Имя, 6) = "П0204_" Тогда 
		ПараметрВводаПоля = РегламентированнаяОтчетностьКлиентСервер.НайтиСвойстваПоказателя(ЭтаФорма, "04");
	ИначеЕсли Лев(Область.Имя, 6) = "П0206_" Тогда
		ПараметрВводаПоля = РегламентированнаяОтчетностьКлиентСервер.НайтиСвойстваПоказателя(ЭтаФорма, "06");
	ИначеЕсли Лев(Область.Имя, 6) = "П0213_" Тогда
		ПараметрВводаПоля = РегламентированнаяОтчетностьКлиентСервер.НайтиСвойстваПоказателя(ЭтаФорма, "13");
	ИначеЕсли Лев(Область.Имя, 6) = "П0202_" Или Лев(Область.Имя, 6) = "П0203_" Тогда
		ПараметрВводаПоля = РегламентированнаяОтчетностьКлиентСервер.НайтиСвойстваПоказателя(ЭтаФорма, "01");
	КонецЕсли;
...
Процедура ВвестиПоказатель(Элемент, Область, КодПоказателя, Знач ЭтоСумма, Знач ТекстВыбора, ТаблицаВыбора, СтандартнаяОбработка, КолонкаПоиска = "Код") Экспорт
	...
	ИсхОбласть = ПолеТабличногоДокументаФормаОтчета.Области[СтрЗаменить(Область.Имя, "П0202_", "П0203_")];
...
Процедура ВвестиПоказательЗавершение(РезультатВыбора, ДополнительныеПараметры) Экспорт
	...
	Если Лев(Область.Имя, 6) = "П0203_" Тогда 
		ВыборНазв = ?(ЗначениеЗаполнено(ВыборКод),СокрЛП(РезультатВыбора["Название"]),"");
		ИсхОбласть = ПолеТабличногоДокументаФормаОтчета.Области[СтрЗаменить(Область.Имя, "П0203_", "П0202_")];
...
Процедура Расчет(Форма, ИмяСтраницыПанели = "") Экспорт
	...
	Области.П030403.Значение = Области.П030503.Значение + Области.П030603.Значение + Области.П030703.Значение + Области.П030803.Значение;
	
	Для Инд = 1 По 64 Цикл
		Области["П0208_"+Инд].Значение = Области["П0209_"+Инд].Значение + Области["П0210_"+Инд].Значение + Области["П0211_"+Инд].Значение;
	КонецЦикла;
КонецПроцедуры
...
Функция ПолучитьПараметрыИнициализацииУниверсальногоОтчета()
	Возврат УниверсальныйОтчетСтатистики.ПолучитьПараметрыИнициализацииУниверсальногоОтчета(ЭтаФорма, "АтрибВыгрузкиXML2017Кв1", "ШаблонРосстата2017Кв1", "СхемаВыгрузкиXML2017Кв1");
КонецФункции
