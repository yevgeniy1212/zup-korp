...
Функция ЗаписатьДокумент(ИмяФормы, ПараметрыЗаписи) Экспорт
	...
	Если НЕ ЗначениеЗаполнено(ПараметрыЗаписи.РазделыОтчета.Титульный.Данные.ДатаПериод) Тогда
		...
		Если Год(ПараметрыЗаписи.РазделыОтчета.Титульный.Данные.ДатаПериод) > 2020 Тогда
			ПараметрыЗаписи.РазделыОтчета.Титульный.Данные.ДатаПериод
			= НачалоДня(Дата(ПараметрыЗаписи.РазделыОтчета.Титульный.Данные.ДатаПериод - 1));
		КонецЕсли;
	КонецЕсли;
