...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	СтруктураРеквизитовФормы.ДатаРегистрацииОрганизации = Параметры.ДатаРегистрацииОрганизации;
...
&НаСервере
Процедура ЗаполнитьДатуПериода()

	ОтчетныйГод = СтрЗаменить(Формат(Год(СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета), "ЧРГ=%"), "%", "") + НСтр("ru=' г.      '");
	ТекущийГод  = СтрЗаменить(Формат(Год(СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета) + 1, "ЧРГ=%"), "%", "") + НСтр("ru=' г.      '");
	
	СтруктураРеквизитовФормы.ДатаПериод = Новый СписокЗначений;
	
	Если Год(СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета) < 2020 Тогда
		СтруктураРеквизитовФормы.ДатаПериод.Добавить(1, НСтр("ru=' 1 января '") + ТекущийГод);
	Конецесли;
	СтруктураРеквизитовФормы.ДатаПериод.Добавить(12, НСтр("ru=' 1 декабря '") + ОтчетныйГод);
	СтруктураРеквизитовФормы.ДатаПериод.Добавить(11, НСтр("ru=' 1 ноября '") + ОтчетныйГод);
	СтруктураРеквизитовФормы.ДатаПериод.Добавить(10, НСтр("ru=' 1 октября '") + ОтчетныйГод);
	СтруктураРеквизитовФормы.ДатаПериод.Добавить(9, НСтр("ru=' 1 сентября '") + ОтчетныйГод);
	СтруктураРеквизитовФормы.ДатаПериод.Добавить(8, НСтр("ru=' 1 августа '") + ОтчетныйГод);
	СтруктураРеквизитовФормы.ДатаПериод.Добавить(7, НСтр("ru=' 1 июля '") + ОтчетныйГод);
	СтруктураРеквизитовФормы.ДатаПериод.Добавить(6, НСтр("ru=' 1 июня '") + ОтчетныйГод);
	СтруктураРеквизитовФормы.ДатаПериод.Добавить(5, НСтр("ru=' 1 мая '") + ОтчетныйГод);
	СтруктураРеквизитовФормы.ДатаПериод.Добавить(4, НСтр("ru=' 1 апреля '") + ОтчетныйГод);
	СтруктураРеквизитовФормы.ДатаПериод.Добавить(3, НСтр("ru=' 1 марта '") + ОтчетныйГод);
	СтруктураРеквизитовФормы.ДатаПериод.Добавить(2, НСтр("ru=' 1 февраля '") + ОтчетныйГод);
	
	ПоСостояниюНаДату = НачалоГода(ДобавитьМесяц(СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета, 1));
	
	Если ЗначениеЗаполнено(СтруктураРеквизитовФормы.ДатаРегистрацииОрганизации) Тогда
		
		ДатаРегистрацииОрганизации = СтруктураРеквизитовФормы.ДатаРегистрацииОрганизации;
		
	Иначе
		
		ДатаРегистрацииОрганизации = ДатаРегистрацииОрганизации(СтруктураРеквизитовФормы.Организация);
		
	КонецЕсли;	
	 	 	
	Если ЗначениеЗаполнено(ДатаРегистрацииОрганизации)
	   И Год(ДатаРегистрацииОрганизации) = Год(СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета)
	   И Месяц(ДатаРегистрацииОрганизации) < 12
	   И ((НЕ ЗначениеЗаполнено(СтруктураРеквизитовФормы.ДатаРегистрацииОрганизации)
	   И Год(ДатаРегистрацииОрганизации) = Год(ДатаПодписи))
	   ИЛИ ЗначениеЗаполнено(СтруктураРеквизитовФормы.ДатаРегистрацииОрганизации)) Тогда
	   	   	   
	   ПоСостояниюНаДату = НачалоМесяца(ДобавитьМесяц(ДатаРегистрацииОрганизации, 1));
	                                                                                                                                        	   
	КонецЕсли;   
	
	Если Год(ПоСостояниюНаДату) > 2020 Тогда
		ПоСостояниюНаДату = НачалоМесяца(ДобавитьМесяц(ПоСостояниюНаДату, - 1));
	Конецесли;
	
	ТабличныйДокумент.Области.ДатаПериод.Значение = СокрЛП(СтруктураРеквизитовФормы.ДатаПериод.НайтиПоЗначению(Месяц(ПоСостояниюНаДату)).Представление);
...
Функция ДатаРегистрацииОрганизации(ВыбОрганизация)
	...
	Если ЗначениеЗаполнено(ОргСведения)
	   И ОргСведения.Свойство("ДатаРегистрации") Тогда
...
Процедура ВосстановитьСохраненныеДанные()
	...
	Если ЭтаФорма["СтруктураДанных" + ИмяТекРаздела].Свойство("ДатаПериод", ПоСостояниюНаДату) Тогда
		
		Если ТипЗнч(ПоСостояниюНаДату) = Тип("Дата") Тогда
	...
	Если НЕ РегламентированнаяОтчетностьВызовСервера.ЭтоЮридическоеЛицо(СтруктураРеквизитовФормы.Организация) Тогда
		...
		ТабличныйДокумент.Области.ОргДиректор.Очистить();
	КонецЕсли;
...
Процедура ВосстановитьСведенияОбОрганизацииИзТитульногоЛистаСтарогоОбразца(ПоказателиОтчета, ВидДок)
	...
	Для Каждого Показатель Из ПоказателиТаблПоле Цикл
		...
		ИначеЕсли ИмяПоказателя = "ДатаПериод1" Тогда
			...
			СтруктураДанныхТитульный.ДатаПериод = СокрЛП(СтруктураРеквизитовФормы.ДатаПериод.НайтиПоЗначению(Месяц(СтруктураДанныхТитульный.ДатаПериод)).Представление);
...
Процедура ЗаполнитьСведенияОбОрганизации(ВПрограммеИзмененаДатаПодписи = Ложь)	
	...
	ОбъектОтчета(ЭтаФорма.ИмяФормы).ЗаполнитьСведенияОбОрганизации(
...
Функция ОбновитьНастройкиСтраниц()
	...
	ЗначениеВДанныеФормы(ОбъектОтчета(ЭтаФорма.ИмяФормы).СтруктураСтраницОтчета(СтруктураРеквизитовФормы.мВыбраннаяФорма),
						 ЭтаФорма["мДеревоСтраницОтчета"]);
...
Процедура ЗаполнитьСведенияНаСервере()
	...
	Если НЕ ЗначениеЗаполнено(СтруктураРеквизитовФормы.ДатаРегистрацииОрганизации) Тогда
		ЗаполнитьДатуПериода();
	КонецЕсли;
...
Процедура ТабличныйДокументВыбор(Элемент, Область, СтандартнаяОбработка)
	...
	Если ИмяЯчейки = "КодИМНС"
	 ИЛИ ИмяЯчейки = "ОргИМНС" Тогда
	...
	ИначеЕсли ИмяЯчейки = "ДатаПериод" Тогда
		
		СтандартнаяОбработка = Ложь;
						
		НачальноеЗначение = СтруктураРеквизитовФормы.ДатаПериод.НайтиПоЗначению(НомерМесяца(СтруктураРеквизитовФормы, Область.Значение));
		Если НачальноеЗначение = Неопределено Тогда
			НачальноеЗначение = СтруктураРеквизитовФормы.ДатаПериод[0];
		КонецЕсли;
		
		ОписаниеОповещения = Новый ОписаниеОповещения("ПоказатьВыборИзСпискаЗавершение", ЭтотОбъект, Область.Имя);
		ПоказатьВыборИзСписка(ОписаниеОповещения, СтруктураРеквизитовФормы.ДатаПериод, , НачальноеЗначение);
...
&НаКлиентеНаСервереБезКонтекста
Функция НомерМесяца(СтруктураРеквизитовФормы, МесяцПрописью)
	
	НомерМесяца = 0;
		
	Для Каждого Месяц Из СтруктураРеквизитовФормы.ДатаПериод Цикл
		
		Если СтрЧислоВхождений(СокрЛП(Месяц.Представление), СокрЛП(МесяцПрописью)) > 0  Тогда
			
			НомерМесяца = Месяц.Значение;
...
&НаКлиенте
Процедура ПоказатьВыборИзСпискаЗавершение(РезультатВыбора, ДополнительныеПараметры) Экспорт
	
	Если НЕ РезультатВыбора = Неопределено Тогда
		
		ТабличныйДокумент.Области[ДополнительныеПараметры].Значение = СокрЛП(РезультатВыбора.Представление);
...
Функция Сохранить(Автосохранение = Ложь, КодИФНС, КПП, Вариант,
	...
	ЭтаФорма["СтруктураДанныхТитульный"].ДатаПериод
		= ДатаНаКоторуюУказанаСреднесписочнаяЧисленность(ЭтаФорма["СтруктураДанныхТитульный"].ДатаПериод);
	...
	Если ОбъектОтчета.ЗаписатьДокумент(СтруктураРеквизитовФормы.мВыбраннаяФорма,
									   ПараметрыЗаписи) = Ложь Тогда
...
&НаСервере
Функция ДатаНаКоторуюУказанаСреднесписочнаяЧисленность(ДатаПериод)
	
	Возврат Дата(Лев(Прав(СокрЛП(ДатаПериод), 7), 4)
		  + Формат(НомерМесяца(СтруктураРеквизитовФормы, ДатаПериод), "ЧЦ=2; ЧВН=") + "01");
		...
		Если Область.ТипОбласти = ТипОбластиЯчеекТабличногоДокумента.Прямоугольник И Область.СодержитЗначение <> Неопределено И Область.СодержитЗначение Тогда
			...
			Если ПараметрыПечати.ИмяСтраницы = "Титульный" Тогда
				...
				ИначеЕсли СтрЧислоВхождений(Область.Имя, "ДатаПериод") > 0
					...
					Если СтрЧислоВхождений(Область.Имя, "ДатаПериод") > 0 Тогда
												
						Дата = Формат(ДатаНаКоторуюУказанаСреднесписочнаяЧисленность(ТабличныйДокумент.Области.ДатаПериод.Значение), "ДФ=ддММгггг");
																		
						Для Ном = 1 По СтрДлина(Дата) Цикл
							МакетПечатнойФормы.Области["ДатаПериод" + Ном].Значение = Сред(Дата, Ном, 1);		
						КонецЦикла;
...
Процедура ЗаполнитьАвтоНаСервере()
	...
	Если ЗначениеЗаполнено(СтруктураДанныхТитульный.ДатаПериод) Тогда
		
		ДатаНаКоторуюУказанаСреднесписочнаяЧисленность = ДатаНаКоторуюУказанаСреднесписочнаяЧисленность(СтруктураДанныхТитульный.ДатаПериод);
		
		Если Год(ДатаНаКоторуюУказанаСреднесписочнаяЧисленность) = Год(СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета) Тогда
		
			ПараметрыОтчета.Вставить("ДатаНачалаПериодаОтчета", НачалоМесяца(ДобавитьМесяц(ДатаНаКоторуюУказанаСреднесписочнаяЧисленность, -1)));
			ПараметрыОтчета.Вставить("ДатаКонцаПериодаОтчета",  КонецМесяца(ДобавитьМесяц(ДатаНаКоторуюУказанаСреднесписочнаяЧисленность, -1)));
	...
	РегламентированнаяОтчетностьПереопределяемый.ЗаполнитьОтчет(ИмяОтчета(ЭтаФорма.ИмяФормы),
