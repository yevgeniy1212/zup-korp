...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если НЕ ЗначениеЗаполнено(мДатаНачалаПериодаОтчета) И НЕ ЗначениеЗаполнено(мДатаКонцаПериодаОтчета) Тогда
		мДатаКонцаПериодаОтчета = КонецМесяца(Дата('20230404'));
		мДатаКонцаПериодаОтчета = НачалоМесяца(Дата('20230404'));
	КонецЕсли;
...
Процедура УстановитьПредыдущийПериод(Команда)
	...
	Если мДатаКонцаПериодаОтчета = КонецМесяца(Дата(2023,4,1)) Тогда
		Шаг = -1;
		мПериодичность = ПредопределенноеЗначение("Перечисление.Периодичность.Месяц");
	ИначеЕсли мДатаКонцаПериодаОтчета > КонецМесяца(Дата(2023,4,1)) Тогда
		Шаг = -12;
...
Процедура УстановитьСледующийПериод(Команда)
	...
	Если мДатаКонцаПериодаОтчета = КонецМесяца(Дата(2023,3,1)) Тогда
		Шаг = 1;
		мПериодичность = ПредопределенноеЗначение("Перечисление.Периодичность.Год");
	ИначеЕсли мДатаКонцаПериодаОтчета >= КонецМесяца(Дата(2023,4,1)) Тогда
		Шаг = 12;
