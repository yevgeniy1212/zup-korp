...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	СтруктураРеквизитовФормы.мВерсияФормы = "22/03/2016";
...
Процедура Расчет(ПолеТабличногоДокументаФормаОтчета) 
	...
	ПолеТабличногоДокументаФормаОтчета.Области["П012203"].Значение = Гр3;
	ПолеТабличногоДокументаФормаОтчета.Области["П012204"].Значение = Гр4;
КонецПроцедуры
...
Процедура ОтправитьВКонтролирующийОрган(Команда)
	...
	Если НачалоМесяца(СтруктураРеквизитовФормы.мДатаНачалаПериодаОтчета) <> НачалоМесяца(Дата("20170401")) Тогда 
		ОбщегоНазначенияКлиент.СообщитьПользователю("Отправка поддерживается только за апрель 2017 года");
		Возврат;
	КонецЕсли;
