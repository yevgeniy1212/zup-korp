...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	//
	// Версия формы
	СтруктураРеквизитовФормы.мВерсияФормы = "01/02/2013";
...
Функция ПечатьНаСервере(ВидПечати)
	...
	Для Каждого Обл Из ОбластиСтроки Цикл
		...
		СтруктураРеквизитовФормы.мПечатныеФормы.Добавить(ПечатнаяФорма, "Форма П-4 (НЗ)");
	КонецЦикла;
...
Функция ПроверитьВозможностьВыгрузки(ПараметрыВыгрузки, ДеревоВыгрузки)
	...
	Если НЕ (ТабДокумент.П0001000201 <= ТабДокумент.П0001001301 + ТабДокумент.П0001000801) Тогда
		...
		СтатусПроверки = Ложь;
	КонецЕсли;
	Если НЕ (ТабДокумент.П0001002101 <= ТабДокумент.П0001001301) Тогда
		РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма, "Не выполнено соотношение: ""строка 21 <= строки 13""", ИмяПоляТаблДокумента, "П0001002101");
		СтатусПроверки = Ложь;
	КонецЕсли;
	Если НЕ (ТабДокумент.П0001000101 + ТабДокумент.П0001000201 + ТабДокумент.П0001000301 + ТабДокумент.П0001000501
		...
		СтатусПроверки = Ложь;
	КонецЕсли;
	Если НЕ (ТабДокумент.П0001002201 <= ТабДокумент.П0001001301) Тогда
		РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма, "Не выполнено соотношение: ""строка 22 <= строки 13""", ИмяПоляТаблДокумента, "П0001002201");
		СтатусПроверки = Ложь;
	КонецЕсли;
	Если ЗначениеЗаполнено(ТабДокумент.П0001000701) И НЕ ЗначениеЗаполнено(ТабДокумент.П0001000601) Тогда
		РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма, "Строка 06 графа 1 должна быть заполнена! (есть значение в строке 07 графа 1)", ИмяПоляТаблДокумента, "П0001000601");
		СтатусПроверки = Ложь;
	КонецЕсли;
	...
	Если НЕ (ТабДокумент.П0001000501 <= ТабДокумент.П0001001301 + ТабДокумент.П0001000801) Тогда
		...
		СтатусПроверки = Ложь;
	КонецЕсли;
	Если НЕ (ТабДокумент.П0001002001 <= ТабДокумент.П0001001301) Тогда
		РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма, "Не выполнено соотношение: ""строка 20 <= строки 13""", ИмяПоляТаблДокумента, "П0001002001");
		СтатусПроверки = Ложь;
	КонецЕсли;
