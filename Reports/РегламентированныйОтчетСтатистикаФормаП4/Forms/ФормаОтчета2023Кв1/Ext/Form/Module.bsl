...
Процедура СформироватьСтруктуруРеквизитовФормы()
	...
	Если Периодичность = Перечисления.Периодичность.Месяц Тогда
		ИмяМакетаАтр = "АтрибВыгрузкиXML2023Кв1_месяц";
		ИмяШаблона   = "ШаблонРосстата2023Кв1_месяц";
	Иначе
		ИмяМакетаАтр = "АтрибВыгрузкиXML2023Кв1_квартал";
		ИмяШаблона   = "ШаблонРосстата2023Кв1_квартал";
	КонецЕсли;
...
Процедура ОчиститьТабличноеПоле(Форма)
	...
	ОблОтчетПрошлогоПериода = Форма.ПолеТабличногоДокументаФормаОтчета.Области.Найти("ВыбратьОтчетПрошлогоПериода");
	Если ОблОтчетПрошлогоПериода <> Неопределено Тогда
		ОблОтчетПрошлогоПериода.Значение = "Выбор отчета предыдущего периода";
	КонецЕсли;
...
Функция ПечатьНаСервере(ВидПечати)
	...
	Для Каждого Обл Из ПолеТабличногоДокументаФормаОтчета.Области Цикл
		Если Обл.ТипОбласти = ТипОбластиЯчеекТабличногоДокумента.Строки 
			И (Обл.Имя <> "ПолеНомерКорректировки" И Обл.Имя <> "ШапкаОбособленныеПодразделения" И Обл.Имя <> "ДанныеПредыдущегоПериода") Тогда
