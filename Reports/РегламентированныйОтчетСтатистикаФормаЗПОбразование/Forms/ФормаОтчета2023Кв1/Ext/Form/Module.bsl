...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	СтруктураРеквизитовФормы.мКодОтчета   = "0606048";
...
Процедура СформироватьСтруктуруРеквизитовФормы()
	...
	СтруктураРеквизитовФормы["ИмяМакетаТабличногоДокумента"] = "ФормаОтчета2023Кв1_ФормаОтчета" + ПостфиксМакетов;
	СтруктураРеквизитовФормы["ИмяМакетаАтрибутовВыгрузки"] = "АтрибВыгрузкиXML2023Кв1" + ПостфиксМакетов;
	СтруктураРеквизитовФормы["ИмяМакетаСхемыВыгрузки"] = "СхемаВыгрузкиXML2023Кв1" + ПостфиксМакетов;
	СтруктураРеквизитовФормы["ИмяМакетаШаблона"] = "ШаблонРосстата2023Кв1" + ПостфиксМакетов;
	СтруктураРеквизитовФормы["ИмяМакетаСписков"] = "Списки2023Кв1";
	СтруктураРеквизитовФормы["ИмяМакетаСоставаПоказателей"] = "СоставПоказателей2023Кв1" + ПостфиксМакетов;
...
Процедура Расчет(Форма, ИмяСтраницыПанели = "", ОбластьИмя = "")
	...
	КвоСтр = 28;
	Если Форма.СтруктураРеквизитовФормы.мПериодичность = ПредопределенноеЗначение("Перечисление.Периодичность.Квартал") Тогда
		КвоКол = 12;
	Иначе
		КвоКол = 11;
	КонецЕсли;
	
	НомСтрокИскл = "06,09,10,12,13,15,16,20,22";
		...
		Если ТекГрафа = "" ИЛИ СтрНайти(СтрРассчитатьГрафы, СтрНомКол) > 0 Тогда
			...
			Для ИндСтр = 0 По КвоСтр - 2 Цикл // без первой строки
				...
				Если СтрНайти(НомСтрокИскл, СтрНомСтр) = 0 Тогда
					Обл = мФормаОтчета.Области.Найти("П00000000" + СтрНомСтр + СтрНомКол);
					Если Обл <> Неопределено Тогда
						СуммаПоГрафе = СуммаПоГрафе + Обл.Значение;
					КонецЕсли;
				КонецЕсли;
				
			КонецЦикла;
			
			//Расчет: стр.01 = сумме строк 02-05, 07,08,11,14,17-19,21,23-28
...
Функция ПечатьНаСервере(ВидПечати)
	...
	СтруктураРеквизитовФормы.мПечатныеФормы.Добавить(ПечатнаяФорма, "Форма ЗП-образование");
...
Процедура ФормироватьСтруктуруСтраницОтчета() Экспорт
	...
	СтрокаУровня1.Представление            = "Статистика: Форма ЗП-образование";
...
Функция СформироватьСтруктуруПараметров() Экспорт 
	...
	СтруктураРеквизитовФормы.мПериодичность =
		ОбщегоНазначения.ЗначениеРеквизитаОбъекта(СтруктураРеквизитовФормы.мСохраненныйДок, "Периодичность");
