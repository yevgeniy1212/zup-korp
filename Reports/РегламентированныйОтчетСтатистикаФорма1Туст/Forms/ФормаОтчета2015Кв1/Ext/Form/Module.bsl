...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	//
	// Версия формы.
	СтруктураРеквизитовФормы.мВерсияФормы = "17/08/2015";
...
Функция ПроверитьКонтрольныеСоотношенияВОтчете() Экспорт
	...
	Нарушение = "Приказ Росстата от 03.08.2015 № 357";
	
	Для Стр = 1 По 13 Цикл
		
		СтрСтр = "П01" + Формат(Стр, "ЧЦ=2; ЧВН=; ЧГ=;");
		ЗнГр3 = Показатели[СтрСтр + "03"];
		ЗнГр4 = Показатели[СтрСтр + "04"];
		ЗнГр5 = Показатели[СтрСтр + "05"];
		
		ТекстФормулы = "по стр. " + Формат(Стр, "ЧЦ=2; ЧВН=; ЧГ=;") + " гр. 3 >= гр.4";
		Описание     = "по стр. " + Формат(Стр, "ЧЦ=2; ЧВН=; ЧГ=;") + " гр. 3 < гр.4";
		Расшифровка = Строка(ЗнГр3) + " >= " + Строка(ЗнГр4);
		Если ЗнГр3 >= ЗнГр4 Тогда
			ЭтоОшибка = Ложь;
		...
		ТекстФормулы = "по стр. " + Формат(Стр, "ЧЦ=2; ЧВН=; ЧГ=;") + " гр. 3 > гр.5";
		Описание     = "по стр. " + Формат(Стр, "ЧЦ=2; ЧВН=; ЧГ=;") + " гр. 3 <= гр.5";
		Расшифровка = Строка(ЗнГр3) + " > " + Строка(ЗнГр5);
		Если ЗнГр3 > ЗнГр5 Или ЗнГр3 = 0 Тогда
			ЭтоОшибка = Ложь;
			ОбщееЧислоБезОшибок = ОбщееЧислоБезОшибок + 1;
		Иначе
			ЭтоОшибка = Истина;
	...
	Для Гр = 3 По 5 Цикл 
		Зн01 = Показатели["П01010" + Гр];
		Зн02 = Показатели["П01020" + Гр];
		ТекстФормулы = "по гр. " + Гр + " стр. 1 > стр.2";
		Описание     = "по гр. " + Гр + " стр. 1 <= стр.2";
		Расшифровка = Строка(Зн01) + " > " + Строка(Зн02);
		Если Зн01 > Зн02 Или Зн01 = 0 Тогда
			ЭтоОшибка = Ложь;
			ОбщееЧислоБезОшибок = ОбщееЧислоБезОшибок + 1;
		Иначе
			ЭтоОшибка = Истина;
		...
		Для Стр = 3 По 13 Цикл
			ЗнСр = Показатели["П01" + Формат(Стр, "ЧЦ=2; ЧВН=; ЧГ=;") + "0" + Гр];
			ТекстФормулы = "по гр. " + Гр + " стр. 2 >= стр." + Стр;
			Описание     = "по гр. " + Гр + " гр. 2 < стр." + Стр;
			Расшифровка = Строка(Зн02) + " > " + Строка(ЗнСр);
			Если Зн02 >= ЗнСр Тогда
				ЭтоОшибка = Ложь;
				ОбщееЧислоБезОшибок = ОбщееЧислоБезОшибок + 1;
			Иначе
				ЭтоОшибка = Истина;
	...
	Для Гр = 3 По 4 Цикл 
		Зн29 = Показатели["П02290" + Гр];
		Зн01 = Показатели["П01010" + Гр];
		Для Стр = 20 По 28 Цикл
			ЗнСр = Показатели["П02" + Стр + "0" + Гр];
			
			ТекстФормулы = "по гр." + Гр + " стр. 30 >= стр." + Стр;
			Описание     = "по гр." + Гр + " стр. 30 < стр." + Стр;
			Расшифровка = Строка(Зн29) + " >= " + Строка(ЗнСр);
			
			Если Зн29 >= ЗнСр Тогда
				ЭтоОшибка = Ложь;
				ОбщееЧислоБезОшибок = ОбщееЧислоБезОшибок + 1;
			Иначе
				ЭтоОшибка = Истина;
		...
		Для Стр = 20 По 29 Цикл
			...
			ТекстФормулы = "по гр." + Гр + " стр."+Стр+" <= стр.01";
			Описание     = "по гр." + Гр + " стр."+Стр+" > стр.01";
			Расшифровка = Строка(Зн01) + " >= " + Строка(ЗнСр);
			
			Если Зн01 >= ЗнСр Тогда
				ЭтоОшибка = Ложь;
				ОбщееЧислоБезОшибок = ОбщееЧислоБезОшибок + 1;
			Иначе
				ЭтоОшибка = Истина;
		...
		ЗнСр = Показатели["П02260" + Гр]+Показатели["П02270" + Гр]+Показатели["П02280" + Гр];
		ТекстФормулы = "по гр." + Гр + " стр. 30 >= стр.26+стр.27+стр.28";
		Описание     = "по гр." + Гр + " стр. 30 < стр.26+стр.27+стр.28";
		Расшифровка = Строка(Зн29) + " >= " + Строка(Показатели["П02260" + Гр]) + "+" + Показатели["П02270" + Гр] + "+" + Показатели["П02280" + Гр];
		Если Зн29 >= ЗнСр Тогда
			ЭтоОшибка = Ложь;
			ОбщееЧислоБезОшибок = ОбщееЧислоБезОшибок + 1;
		Иначе
	...
	Зн01 = Показатели["П010103"];
	Зн30 = Показатели["П023003"];
	ТекстФормулы = "по гр.3 стр. 31 <= стр.01";
	Описание     = "по гр.3 стр. 31 > стр.01";
	Расшифровка = Строка(Зн30) + " <= " + Строка(Зн01);
	Если Зн30 <= Зн01 Тогда
		ЭтоОшибка = Ложь;
		ОбщееЧислоБезОшибок = ОбщееЧислоБезОшибок + 1;
	Иначе
		ЭтоОшибка = Истина;
	...
	Зн30_3 = Показатели["П023003"];
	Зн30_5 = Показатели["П023005"];
	Зн31_3 = Показатели["П023103"];
	Зн31_5 = Показатели["П023105"];
	ТекстФормулы = "по гр.3 стр. 30 >= стр.31";
	Описание     = "по гр.3 стр. 30 < стр.31";
	Расшифровка = Строка(Зн30_3) + " >= " + Строка(Зн31_3);
	Если Зн30_3 >= Зн31_3 Тогда
		ЭтоОшибка = Ложь;
		ОбщееЧислоБезОшибок = ОбщееЧислоБезОшибок + 1;
	Иначе
		ЭтоОшибка = Истина;
	...
	ТекстФормулы = "по гр.5 стр. 30 >= стр.31";
	Описание     = "по гр.5 стр. 30 < стр.31";
	Расшифровка = Строка(Зн30_5) + " >= " + Строка(Зн31_5);
	Если Зн30_5 >= Зн31_5 Тогда
		ЭтоОшибка = Ложь;
		ОбщееЧислоБезОшибок = ОбщееЧислоБезОшибок + 1;
	Иначе
		ЭтоОшибка = Истина;
	...
	ЗнЛ = Показатели["П010103"] - Показатели["П010104"];
	ЗнП = Показатели["П010203"] - Показатели["П010204"];
	ТекстФормулы = "(гр.3 - гр.4) стр. 01 >= (гр.3 - гр.4) стр. 02";
	Описание     = "(гр.3 - гр.4) стр. 01 < (гр.3 - гр.4) стр. 02";
	Расшифровка = Строка(ЗнЛ) + " >= " + Строка(ЗнП);
	Если ЗнЛ >= ЗнП Тогда
		ЭтоОшибка = Ложь;
		ОбщееЧислоБезОшибок = ОбщееЧислоБезОшибок + 1;
	Иначе
	...
	ЗнЛ = Показатели["П010203"] - Показатели["П010204"];
	Для Стр = 3 по 13 Цикл
		СтрСтр = Прав("0"+Стр, 2);
		ЗнП = Показатели["П01"+СтрСтр+"03"] - Показатели["П01"+СтрСтр+"04"];
		ТекстФормулы = "(гр.3 - гр.4) стр. 02 >= (гр.3 - гр.4) стр. "+СтрСтр;
		Описание     = "(гр.3 - гр.4) стр. 02 < (гр.3 - гр.4) стр. "+СтрСтр;
		Расшифровка = Строка(ЗнЛ) + " >= " + Строка(ЗнП);
		Если ЗнЛ >= ЗнП Тогда
			ЭтоОшибка = Ложь;
			ОбщееЧислоБезОшибок = ОбщееЧислоБезОшибок + 1;
	...
	ЗнЛ = Показатели["П022903"] - Показатели["П022904"];
	Для Стр = 20 по 28 Цикл
		СтрСтр = ""+Стр;
		ЗнП = Показатели["П02"+СтрСтр+"03"] - Показатели["П02"+СтрСтр+"04"];
		ТекстФормулы = "(гр.3 - гр.4) стр. 29 >= (гр.3 - гр.4) стр. "+СтрСтр;
		Описание     = "(гр.3 - гр.4) стр. 29 < (гр.3 - гр.4) стр. "+СтрСтр;
		Расшифровка = Строка(ЗнЛ) + " >= " + Строка(ЗнП);
		Если ЗнЛ >= ЗнП Тогда
			ЭтоОшибка = Ложь;
			ОбщееЧислоБезОшибок = ОбщееЧислоБезОшибок + 1;
	...
	Для Стр = 20 по 29 Цикл
		СтрСтр = ""+Стр;
		ЗнП = Показатели["П02"+СтрСтр+"03"] - Показатели["П02"+СтрСтр+"04"];
		ТекстФормулы = "(гр.3 - гр.4) стр. 01 >= (гр.3 - гр.4) стр. "+СтрСтр;
		Описание     = "(гр.3 - гр.4) стр. 01 < (гр.3 - гр.4) стр. "+СтрСтр;
		Расшифровка = Строка(ЗнЛ) + " >= " + Строка(ЗнП);
		Если ЗнЛ >= ЗнП Тогда
			ЭтоОшибка = Ложь;
			ОбщееЧислоБезОшибок = ОбщееЧислоБезОшибок + 1;
	...
	Для Гр = 3 По 4 Цикл
		ЗнЛ = Показатели["П01010"+Гр];
		Для Стр = 20 по 29 Цикл
			СтрСтр = ""+Стр;
			ЗнП = Показатели["П02"+СтрСтр+"0"+Гр];
			ТекстФормулы = "стр. 01 >= стр. "+СтрСтр+" по графе "+Гр;
			Описание     = "стр. 01 < стр. "+СтрСтр+" по графе "+Гр;
			Расшифровка = Строка(ЗнЛ) + " >= " + Строка(ЗнП);
			Если ЗнЛ >= ЗнП Тогда
				ЭтоОшибка = Ложь;
				ОбщееЧислоБезОшибок = ОбщееЧислоБезОшибок + 1;
