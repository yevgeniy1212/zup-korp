...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	//
	// Версия формы.
	СтруктураРеквизитовФормы.мВерсияФормы = "17/01/2014";
...
Функция ПроверитьКонтрольныеСоотношенияВОтчете() Экспорт
	...
	Для Стр = 1 По 17 Цикл
		Если Стр >= 10 И Стр <= 14 Тогда 
			Продолжить;
		КонецЕсли;
	...
	Для Стр = 20 По 30 Цикл
		Если Стр >= 28 И Стр <= 29 Тогда 
			Продолжить;
		КонецЕсли;
	...
	МассивСравнения.Добавить(2);
	МассивСравнения.Добавить(15);
	МассивСравнения.Добавить(16);
	МассивСравнения.Добавить(17);
	
	Для Гр = 3 По 5 Цикл
		Зн01 = Показатели["П01010" + Гр];
		Для Каждого Элт Из МассивСравнения Цикл
			ЗнСр = Показатели["П01" + Формат(Элт, "ЧЦ=2; ЧВН=; ЧГ=;") + "0" + Гр];
			
			ТекстФормулы = "по гр." + Гр + " стр. 01 > стр." + Формат(Элт, "ЧЦ=2; ЧВН=; ЧГ=;");
			Описание     = "по гр." + Гр + " стр. 01 <= стр." + Формат(Элт, "ЧЦ=2; ЧВН=; ЧГ=;");
			Расшифровка = Строка(Зн01) + " > " + Строка(ЗнСр);
			
			Если Гр >= 4 И Зн01 = 0 И ЗнСр = 0 Тогда 
				Продолжить;
			КонецЕсли;
	...
	Для Гр = 3 По 5 Цикл
		...
		Для Каждого Элт Из МассивСравнения Цикл
			ЗнСр = Показатели["П01" + Формат(Элт, "ЧЦ=2; ЧВН=; ЧГ=;") + "0" + Гр];
			ТекстФормулы = "по гр." + Гр + " стр. 02 >= стр." + Формат(Элт, "ЧЦ=2; ЧВН=; ЧГ=;");
			Описание     = "по гр." + Гр + " стр. 02 < стр." + Формат(Элт, "ЧЦ=2; ЧВН=; ЧГ=;");
	...
	Для Гр = 3 По 4 Цикл 
		Зн30 = Показатели["П02300" + Гр];
		Зн01 = Показатели["П01010" + Гр];
		Для Стр = 20 По 27 Цикл
			ЗнСр = Показатели["П02" + Стр + "0" + Гр];
			
			ТекстФормулы = "по гр." + Гр + " стр. 30 >= стр." + Стр;
			Описание     = "по гр." + Гр + " стр. 30 < стр." + Стр;
			Расшифровка = Строка(Зн30) + " >= " + Строка(ЗнСр);
			
			Если Зн30 >= ЗнСр Тогда
				ЭтоОшибка = Ложь;
				ОбщееЧислоБезОшибок = ОбщееЧислоБезОшибок + 1;
			Иначе
				ЭтоОшибка = Истина;
		...
		ЗнСр = Показатели["П02250" + Гр]+Показатели["П02260" + Гр]+Показатели["П02270" + Гр];
		ТекстФормулы = "по гр." + Гр + " стр. 30 >= стр.25+стр.26+стр.27";
		Описание     = "по гр." + Гр + " стр. 30 < стр.25+стр.26+стр.27";
		Расшифровка = Строка(Зн30) + " >= " + Строка(Показатели["П02250" + Гр]) + "+" + Показатели["П02260" + Гр] + "+" + Показатели["П02270" + Гр];
		Если Зн30 >= ЗнСр Тогда
			ЭтоОшибка = Ложь;
			ОбщееЧислоБезОшибок = ОбщееЧислоБезОшибок + 1;
		Иначе
		...
		ТекстФормулы = "по гр." + Гр + " стр. 30 <= стр.01";
		Описание     = "по гр." + Гр + " стр. 30 > стр.01";
	...
	Зн31 = Показатели["П023103"];
	ТекстФормулы = "по гр.3 стр. 31 <= стр.01";
	Описание     = "по гр.3 стр. 31 > стр.01";
	Расшифровка = Строка(Зн31) + " <= " + Строка(Зн01);
	Если Зн31 <= Зн01 Тогда
		ЭтоОшибка = Ложь;
		ОбщееЧислоБезОшибок = ОбщееЧислоБезОшибок + 1;
	Иначе
		ЭтоОшибка = Истина;
	...
	Зн32_3 = Показатели["П023203"];
	Зн32_5 = Показатели["П023205"];
	ТекстФормулы = "по гр.3 стр. 31 >= стр.32";
	Описание     = "по гр.3 стр. 31 < стр.32";
	Расшифровка = Строка(Зн31_3) + " >= " + Строка(Зн32_3);
	Если Зн31_3 >= Зн32_3 Тогда
		ЭтоОшибка = Ложь;
		ОбщееЧислоБезОшибок = ОбщееЧислоБезОшибок + 1;
	Иначе
		ЭтоОшибка = Истина;
	...
	ТекстФормулы = "по гр.5 стр. 31 >= стр.32";
	Описание     = "по гр.5 стр. 31 < стр.32";
	Расшифровка = Строка(Зн31_5) + " >= " + Строка(Зн32_5);
	Если Зн31_5 >= Зн32_5 Тогда
		ЭтоОшибка = Ложь;
		ОбщееЧислоБезОшибок = ОбщееЧислоБезОшибок + 1;
	Иначе
		ЭтоОшибка = Истина;
