...
Функция ПроверитьКонтрольныеСоотношенияВОтчете() Экспорт
	...
	Нарушение = "Приказ Росстата от 24.09.2014 № 580";
	
	Для Стр = 1 По 16 Цикл
		
		Если Стр = 13 Или Стр = 14 Тогда 
			Продолжить;
		КонецЕсли;
	...
	Для Гр = 3 По 4 Цикл 
		...
		ЗнСр = Показатели["П02260" + Гр]+Показатели["П02270" + Гр]+Показатели["П02280" + Гр]+Показатели["П02290" + Гр];
		ТекстФормулы = "по гр." + Гр + " стр. 30 >= стр.26+стр.27+стр.28+стр.29";
		Описание     = "по гр." + Гр + " стр. 30 < стр.26+стр.27+стр.28+стр.29";
		Расшифровка = Строка(Зн30) + " >= " + Строка(Показатели["П02260" + Гр]) + "+" + Показатели["П02270" + Гр] + "+" + Показатели["П02280" + Гр] + "+" + Показатели["П02290" + Гр];
		Если Зн30 >= ЗнСр Тогда
			ЭтоОшибка = Ложь;
			ОбщееЧислоБезОшибок = ОбщееЧислоБезОшибок + 1;
		Иначе
