...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ПолеВыбораПериодичностиПоказаПериода = ПредставлениеПериода(_Период, КонецКвартала(_Период), "ФП = Истина" );
...
Процедура СформироватьСпискиВыбора()
	МакетСписковВыбора = УведомлениеОСпецрежимахНалогообложения.ПолучитьМакетТабличногоДокумента(ЭтотОбъект, "СпискиВыбора2020_1");
	Список = МакетСписковВыбора.Область("СписокКодовВидовОпераций");
	ТС = Список.Верх;
	Пока ТС <= Список.Низ Цикл
		...
		НовСтр = СпискиВыбораКодов.Добавить();
		НовСтр.Код = Код;
...
Процедура _РеестрДокПОбНЛКодОперНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	ПараметрыФормы.Вставить("ТаблицаЗначений",    СпискиВыбораКодов);
...
Процедура ДобавитьТиповойДоговор(Команда)
	...
	ПоказатьВводСтроки(ОО, Неопределено, "Имя файла типового документа (при наличии)", 255);
КонецПроцедуры
...
Процедура _РеестрДокПОбНЛПриАктивизацииСтроки(Элемент)
	...
	Если Элементы._РеестрДокПОбНЛ.ТекущиеДанные <> Неопределено
		...
		Если Элементы._ПредТипДогТек.Видимость = Истина И ПредТипДогТек.ПолучитьЭлементы().Количество() < 100 Тогда 
...
Процедура _ПредТипДогТекВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	ИначеЕсли Поле.Имя = "_ПредТипДогТекОписаниеКА" Тогда
		...
		Если УровеньГруппировки = 0 Тогда
			...
			ОО = Новый ОписаниеОповещения("ДобавитьТиповойДоговорЗавершение", ЭтотОбъект, ДополнительныеПараметры);
			ПоказатьВводСтроки(ОО, ТекущиеДанные.ИмяФайлДог, "Имя файла типового документа (при наличии)");
...
Функция ЗаполнитьАвтоНаСервере()
	...
	ПараметрыОтчета.Вставить("ПериодОтчета", _Период);
...
Процедура ЗагрузитьПодготовленныеДанные()
	...
	Для Каждого СтрКодВид Из СтруктураДанных.Контейнер.Реестр.Строки Цикл 
		...
		ОтборПоКоду = СпискиВыбораКодов.НайтиСтроки(СтруктураПоискаПоКоду);
...
Процедура ИзменитьПериод(Шаг)
	_Период = НачалоКвартала(ДобавитьМесяц(_Период, 3 * Шаг));
	ПолеВыбораПериодичностиПоказаПериода = ПредставлениеПериода(_Период, КонецКвартала(_Период), "ФП = Истина" );
КонецПроцедуры
