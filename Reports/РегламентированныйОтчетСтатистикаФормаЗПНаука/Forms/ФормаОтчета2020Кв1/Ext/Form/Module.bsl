...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	//
	// Версия формы
	СтруктураРеквизитовФормы.мВерсияФормы = "24/07/2019";
	...
	Если Параметры.мПериодичность = Неопределено Тогда 
		СтруктураРеквизитовФормы.мПериодичность = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Параметры.мСохраненныйДок, "Периодичность");
	Иначе
		СтруктураРеквизитовФормы.мПериодичность = Параметры.мПериодичность;
	КонецЕсли;
	ПолеТабличногоДокументаФормаОтчета.Вывести(ОбъектОтчета(ЭтаФорма.ИмяФормы).ПолучитьМакет("ФормаОтчета2020Кв1_ФормаОтчета"+ПостфиксМакетов()));
...
Процедура Инициализация(БезОткрытияФормы = Ложь) Экспорт
	...
	РегламентированнаяОтчетность.ОбновитьШаблоныЧерезВебСервисНаСервере(ЭтотОбъект, "АтрибВыгрузкиXML2020Кв1"+ПостфиксМакетов());
...
Процедура СформироватьСпискиВыбора()
	...
	МакетСоставаПоказателей = ОбъектОтчета(ЭтаФорма.ИмяФормы).ПолучитьМакет("Список2019кв1");
			...
			Для ИндСтр = 0 По КвоСтр - 3 Цикл // без первой и второй строки
				...
				Если ТекСтрока = "" ИЛИ СтрНомСтр = ТекСтрока Тогда
					Если СтрНомКол = "05" Тогда // расчет: графа 5 = сумме граф 9-11 по текущей строке
						Области["П00000000" + СтрНомСтр + СтрНомКол].Значение = Области["П00000000" + СтрНомСтр + "09"].Значение
					...
					ИначеЕсли СтрНомКол = "03" Тогда // расчет: графа 3 = сумме граф 6-8 по текущей строке
						Области["П00000000" + СтрНомСтр + СтрНомКол].Значение = Области["П00000000" + СтрНомСтр + "06"].Значение
...
Функция УниверсальнаяВыгрузкаXMLНаСервере(ТекстВыгрузки, ПараметрыВыгрузки) Экспорт
	...
	ПараметрыВыгрузки = СформироватьСтруктуруПараметров();
	ДеревоВыгрузки = РегламентированнаяОтчетность.ПолучитьДеревоВыгрузки(ЭтаФорма, "СхемаВыгрузкиXML2020Кв1");
	РегламентированнаяОтчетность.ИзменитьВерсиюФорматаСПроверкойСовместимостиШаблонов(ЭтотОбъект, "АтрибВыгрузкиXML2020Кв1"+ПостфиксМакетов(), "ШаблонРосстата2020Кв1"+ПостфиксМакетов(), ПараметрыВыгрузки);
	Если НЕ ПроверитьВозможностьВыгрузки(ПараметрыВыгрузки, ДеревоВыгрузки) Тогда
		Возврат Ложь;
	КонецЕсли;
	ЗаполнитьДанными(ДеревоВыгрузки, ПараметрыВыгрузки);
...
Функция СформироватьСтруктуруПараметров() Экспорт 
	...
	ДопАтрибуты = РегламентированнаяОтчетность.СформироватьСтруктуруДопАтрибутов(ЭтаФорма.ИмяФормы, "АтрибВыгрузкиXML2020Кв1"+ПостфиксМакетов());
...
Функция ПроверитьКонтрольныеСоотношенияИзШаблонаВФоне()
	...
	ПараметрыПроверкиКонтрольныхСоотношений.Вставить("ШаблонРосстата", "ШаблонРосстата2020Кв1" + ПостфиксМакетов());
	ПараметрыПроверкиКонтрольныхСоотношений.Вставить("Атрибуты", "АтрибВыгрузкиXML2020Кв1" + ПостфиксМакетов());
