...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	//
	// Версия формы
	СтруктураРеквизитовФормы.мВерсияФормы = "01/02/2014";
...
Процедура Расчет(Форма, ИмяСтраницыПанели = "", ОбластьИмя = "")
	...
	Для ИндКол = 0 По КвоКол - 1 Цикл
		...
		Если ТекГрафа = "" ИЛИ СтрНайти(СтрРассчитатьГрафы, СтрНомКол) > 0 Тогда
			...
			СуммаПоГрафеСтр02 = мФормаОтчета.Области["П0000000003" + СтрНомКол].Значение
							  + мФормаОтчета.Области["П0000000004" + СтрНомКол].Значение
							  + мФормаОтчета.Области["П0000000005" + СтрНомКол].Значение
							  + мФормаОтчета.Области["П0000000008" + СтрНомКол].Значение;
			
			//Расчет: стр.02 = сумме строк 03+04+05+08 по всем графам
			мФормаОтчета.Области["П0000000002" + СтрНомКол].Значение = СуммаПоГрафеСтр02;
			
			//Расчет: стр.01 = сумме строк 02+09+10+11+12+13+14 по всем графам
			мФормаОтчета.Области["П0000000001" + СтрНомКол].Значение = СуммаПоГрафеСтр02 + СуммаПоГрафе;
