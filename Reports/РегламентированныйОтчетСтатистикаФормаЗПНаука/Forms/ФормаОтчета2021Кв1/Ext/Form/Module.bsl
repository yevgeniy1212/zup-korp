...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	СтруктураРеквизитовФормы.мКодОтчета   = "0606047";
	...
	ПолеТабличногоДокументаФормаОтчета.Вывести(ОбъектОтчета(ИмяФормы).ПолучитьМакет(
		СтруктураРеквизитовФормы["ИмяМакетаТабличногоДокумента"] + ПостфиксМакетов()));
...
Процедура Инициализация(БезОткрытияФормы = Ложь) Экспорт
	...
	РегламентированнаяОтчетность.ОбновитьШаблоныЧерезВебСервисНаСервере(ЭтотОбъект,
		СтруктураРеквизитовФормы["ИмяМакетаАтрибутовВыгрузки"] + ПостфиксМакетов());
...
Процедура Расчет(Форма, ИмяСтраницыПанели = "", ОбластьИмя = "")
	...
	КвоСтр = 14;
	КвоКол = 11;
	НомСтрокИскл = "03,04,05,06,07,08";
	...
	Для ИндКол = 0 По КвоКол - 1 Цикл
		...
		Если ТекГрафа = "" ИЛИ СтрНайти(СтрРассчитатьГрафы, СтрНомКол) > 0 Тогда
			...
			Для ИндСтр = 0 По КвоСтр - 3 Цикл // без первой и второй строки
				...
				Если ТекСтрока = "" ИЛИ СтрНомСтр = ТекСтрока Тогда
					Если СтрНомКол = "05" Тогда // расчет: графа 5 = сумме граф 9-11 по текущей строке
						Области["П00000000" + СтрНомСтр + СтрНомКол].Значение
							= Области["П00000000" + СтрНомСтр + "09"].Значение
							+ Области["П00000000" + СтрНомСтр + "10"].Значение
							+ Области["П00000000" + СтрНомСтр + "11"].Значение;
					ИначеЕсли СтрНомКол = "03" Тогда // расчет: графа 3 = сумме граф 6-8 по текущей строке
						Области["П00000000" + СтрНомСтр + СтрНомКол].Значение
							= Области["П00000000" + СтрНомСтр + "06"].Значение
							+ Области["П00000000" + СтрНомСтр + "07"].Значение
							+ Области["П00000000" + СтрНомСтр + "08"].Значение;
					КонецЕсли;
				КонецЕсли;
				
				Если СтрНайти(НомСтрокИскл, СтрНомСтр) = 0 Тогда
				
					СуммаПоГрафе = СуммаПоГрафе + Области["П00000000" + СтрНомСтр + СтрНомКол].Значение;
				
				КонецЕсли;
				
			КонецЦикла;
			
			СуммаПоГрафеСтр02 = Области["П0000000003" + СтрНомКол].Значение
							  + Области["П0000000004" + СтрНомКол].Значение
							  + Области["П0000000005" + СтрНомКол].Значение
							  + Области["П0000000008" + СтрНомКол].Значение;
			
			//Расчет: стр.02 = сумме строк 03+04+05+08 по всем графам
			Области["П0000000002" + СтрНомКол].Значение = СуммаПоГрафеСтр02;
			
			//Расчет: стр.01 = сумме строк 02+09+10+11+12+13+14 по всем графам
			Области["П0000000001" + СтрНомКол].Значение = СуммаПоГрафеСтр02 + СуммаПоГрафе;
...
Функция ПечатьНаСервере(ВидПечати)
	...
	Если СтруктураРеквизитовФормы.мПериодичность = Перечисления.Периодичность.Квартал Тогда
		...
		СтруктураРеквизитовФормы.мПечатныеФормы.Добавить(ПечатнаяФорма, "Форма ЗП-наука");
...
Процедура ФормироватьСтруктуруСтраницОтчета() Экспорт
	...
	СтрокаУровня1.Представление            = "Статистика: Форма ЗП-наука";
...
Функция УниверсальнаяВыгрузкаXMLНаСервере(ТекстВыгрузки, ПараметрыВыгрузки) Экспорт
	...
	РегламентированнаяОтчетность.ИзменитьВерсиюФорматаСПроверкойСовместимостиШаблонов(ЭтотОбъект,
		СтруктураРеквизитовФормы["ИмяМакетаАтрибутовВыгрузки"] + ПостфиксМакетов(),
		СтруктураРеквизитовФормы["ИмяМакетаШаблона"] + ПостфиксМакетов(), ПараметрыВыгрузки);
	Если НЕ ПроверитьВозможностьВыгрузки(ПараметрыВыгрузки, ДеревоВыгрузки) Тогда
		Возврат Ложь;
	КонецЕсли;
	ЗаполнитьДанными(ДеревоВыгрузки, ПараметрыВыгрузки);
...
Функция ПроверитьКонтрольныеСоотношенияИзШаблонаВФоне()
	...
	ШаблонРосстатаИмя = СтруктураРеквизитовФормы["ИмяМакетаШаблона"] + ПостфиксМакетов();
	АтрибутыИмя		  = СтруктураРеквизитовФормы["ИмяМакетаАтрибутовВыгрузки"] + ПостфиксМакетов();
	
	ПараметрыПроверкиКонтрольныхСоотношений = Новый Структура;
	ПараметрыПроверкиКонтрольныхСоотношений.Вставить("ШаблонРосстата", ШаблонРосстатаИмя);
	ПараметрыПроверкиКонтрольныхСоотношений.Вставить("Атрибуты", АтрибутыИмя);
