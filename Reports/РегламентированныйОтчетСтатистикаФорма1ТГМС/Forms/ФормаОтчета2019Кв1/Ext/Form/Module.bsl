...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	//
	// Версия формы.
	СтруктураРеквизитовФормы.мВерсияФормы = "05/09/2018";
...
Функция СформироватьСтруктуруПараметров() Экспорт 
	...
	Если мДанныеОтчета.ПолеТабличногоДокументаФормаОтчета.ОргУровеньГосОргана = "Федеральный" Тогда
		ИмяАтрибутов = "АтрибВыгрузкиXML2019Кв1_фед";
	Иначе
		ИмяАтрибутов = "АтрибВыгрузкиXML2019Кв1_рег";
	КонецЕсли;
...
Функция ПолучитьПредидущийПериод() Экспорт 
	...
	Если Месяц(СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета) <> 3 Тогда 
		...
		Результат.мДатаНачалаПериодаОтчета = НачалоГода(Результат.мДатаКонцаПериодаОтчета);
		Возврат Результат;
	КонецЕсли;
	Возврат Неопределено;
КонецФункции
