...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	//
	// Версия формы.
	СтруктураРеквизитовФормы.мВерсияФормы = "22/07/2019";
...
Процедура ОчиститьТабличноеПолеНаСервере()
	...
	ВыбТабличноеПоле.Вывести(ОбъектОтчета(ЭтаФорма.ИмяФормы).ПолучитьМакет("ФормаОтчета2020Кв1_ФормаОтчета"));
...
Функция СформироватьСтруктуруПараметров() Экспорт 
	...
	Если мДанныеОтчета.ПолеТабличногоДокументаФормаОтчета.ОргУровеньГосОргана = "Федеральный" Тогда
		ИмяАтрибутов = "АтрибВыгрузкиXML2020Кв1_фед";
	Иначе
		ИмяАтрибутов = "АтрибВыгрузкиXML2020Кв1_рег";
	КонецЕсли;
...
Функция ПроверитьКонтрольныеСоотношенияИзШаблонаВФоне()
	...
	Если ТабДокумент.ОргУровеньГосОргана = "Федеральный" Тогда
		ИмяМакета = "АтрибВыгрузкиXML2020Кв1_фед";
		ИмяШаблона = "ШаблонРосстата2020Кв1_фед";
	Иначе
		ИмяМакета = "АтрибВыгрузкиXML2020Кв1_рег";
		ИмяШаблона = "ШаблонРосстата2020Кв1_рег";
	КонецЕсли;
