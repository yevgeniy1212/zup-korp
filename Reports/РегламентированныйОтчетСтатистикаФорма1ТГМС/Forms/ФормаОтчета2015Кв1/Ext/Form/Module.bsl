...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	СтруктураРеквизитовФормы.мКодОтчета   = "0606027";
...
Процедура ПолеТабличногоДокументаФормаОтчетаПриИзмененииСодержимогоОбласти(Элемент, Область)
	...
	Если Область.СодержитЗначение И Область.ТипЗначения.СодержитТип(Тип("Число")) Тогда
		Если Область.Значение = 0 И (Область.ТипЗначения.КвалификаторыЧисла.РазрядностьДробнойЧасти = 1
			Или Область.ТипЗначения.КвалификаторыЧисла.РазрядностьДробнойЧасти = 0) Тогда
...
Процедура Расчет(Форма, ИмяТекущейСтраницыПанели = "", ОбластьИмя = "")
	...
	// По всем графам (с 3 по 7): стр. 05 = сумме стр. с 01 по 04.
	Для НомКол = 3 По 7 Цикл
		ТекСумма = 0;
		Для НомСтр = 1 По 4 Цикл
			ТекСумма = ТекСумма + мФормаОтчета.Области["П000001" + Формат(НомСтр, "ЧЦ=4; ЧВН=; ЧГ=") + Формат(НомКол, "ЧЦ=2; ЧВН=; ЧГ=")].Значение;
		КонецЦикла;
		мФормаОтчета.Области["П0000010005" + Формат(НомКол, "ЧЦ=2; ЧВН=; ЧГ=")].Значение = ТекСумма;
	КонецЦикла;
...
Процедура УстановитьЗащитуИЦветФонаОбластейАвторассчитываемыхПоказателей()
	...
	Для НомКол = 3 По 7 Цикл
		...
		ЭтаФорма.ПолеТабличногоДокументаФормаОтчета.Области["П0000010005"
...
Процедура ОчиститьТабличноеПолеНаСервере()
	...
	СписокПоказателейНеПодлежащихОчистке.Добавить(ВыбТабличноеПоле.Области["ОргУровеньГосОргана"].Значение,   "ОргУровеньГосОргана");
	...
	СписокПоказателейНеПодлежащихОчистке.Добавить(ВыбТабличноеПоле.Области["ОргКодНазв2"].Значение,           "ОргКодНазв2");
	СписокПоказателейНеПодлежащихОчистке.Добавить(ВыбТабличноеПоле.Области["ОргКодЗнач2"].Значение,           "ОргКодЗнач2");
...
Функция ПечатьНаСервере(ВидПечати)
	...
	СтруктураПараметров.Вставить("НаименованиеЛиста",  "Форма 1-Т (ГМС)");
	СтруктураПараметров.Вставить("ОриентацияЛиста",    "Ландшафт");
	СтруктураПараметров.Вставить("ИмяСтраницы",        "1ТГМС");
	...
	// Дополнительно, для Формы 1-Т (ГМС):
	// удаление строки с Уровнем гос. органа (Титульный лист), необходимым для определения шаблона выгрузки (не выводится на печать).
...
Функция УниверсальнаяВыгрузка(ТекстВыгрузки = Неопределено, ПроверятьБлокировки = Истина, ИмяФайлаВыгрузки = "", ВыгружатьСОшибками = Ложь) Экспорт
	...
	Если НЕ УниверсальнаяВыгрузкаXMLПроверкаВозможностиВыгрузкиНаСервере(ТекстВыгрузки, ПараметрыВыгрузки) Тогда
		...
		// В 1-Т (ГМС) обязательно указание "Уровня гос. органа",
...
Функция ПроверитьВозможностьВыгрузки(ПараметрыВыгрузки, ДеревоВыгрузки)
	...
	// КОНТРОЛЬ показателей формы (Указания по заполнению Формы 1-Т (ГМС)).
	// По всем строкам:
	Для НомСтр = 1 По 5 Цикл
		...
		Если Не (ПараметрыВыгрузки["ПР00001" + Формат(НомСтр, "ЧЦ=4; ЧВН=; ЧГ=") + "03"]
			...
			РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма,
				"Не выполнено условие контроля: в стр. " + Формат(НомСтр, "ЧЦ=2; ЧВН=; ЧГ=") + " значение по гр. 3 должно быть больше или равно значению по гр. 4!",
				ИмяПоляТаблДокумента, "П000001" + Формат(НомСтр, "ЧЦ=4; ЧВН=; ЧГ=") + "03");
			СтатусПроверки = Ложь;
		КонецЕсли;
		// Гр. 3 >= гр. 5.
		Если Не (ПараметрыВыгрузки["ПР00001" + Формат(НомСтр, "ЧЦ=4; ЧВН=; ЧГ=") + "03"]
			  >= ПараметрыВыгрузки["ПР00001" + Формат(НомСтр, "ЧЦ=4; ЧВН=; ЧГ=") + "05"]) Тогда
			РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма,
				"Не выполнено условие контроля: в стр. " + Формат(НомСтр, "ЧЦ=2; ЧВН=; ЧГ=") + " значение по гр. 3 должно быть больше или равно значению по гр. 5!",
				ИмяПоляТаблДокумента, "П000001" + Формат(НомСтр, "ЧЦ=4; ЧВН=; ЧГ=") + "03");
			СтатусПроверки = Ложь;
		КонецЕсли;
		// Гр. 6 >= гр. 7.
		// (В шаблоне это соотношение проверяется при условии: гр. 7 <> 0. Делаем так же.)
		Если ПараметрыВыгрузки["ПР00001" + Формат(НомСтр, "ЧЦ=4; ЧВН=; ЧГ=") + "07"] <> 0 Тогда
			Если Не (ПараметрыВыгрузки["ПР00001" + Формат(НомСтр, "ЧЦ=4; ЧВН=; ЧГ=") + "06"]
				  >= ПараметрыВыгрузки["ПР00001" + Формат(НомСтр, "ЧЦ=4; ЧВН=; ЧГ=") + "07"]) Тогда
				РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма,
					"Не выполнено условие контроля: в стр. " + Формат(НомСтр, "ЧЦ=2; ЧВН=; ЧГ=") + " значение по гр. 6 должно быть больше или равно значению по гр. 7!",
	...
	КонецЦикла;
	// Стр. 02 ≠ 0.
	// (В шаблоне это соотношение проверяется иначе: стр. 02 > 0, для гр.: 3-6. Делаем так же.)
	Для НомКол = 3 По 6 Цикл
		Если Не (ПараметрыВыгрузки["ПР000010002" + Формат(НомКол, "ЧЦ=2; ЧВН=; ЧГ=")] > 0) Тогда
				РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма,
					"Не выполнено условие контроля: в стр. 02 значение по гр. " + Формат(НомКол, "ЧЦ=1; ЧВН=; ЧГ=") + " должно быть больше нуля!",
...
Функция СформироватьСтруктуруПараметров()
	...
	// Для ПОСЛЕДУЮЩЕГО (ПЕРЕД ВЫГРУЗКОЙ) контроля показателей формы (Указания ФСН по заполнению Формы 1-Т (ГМС)).
	Для НомКол = 3 По 7 Цикл
		Для НомСтр = 1 По 5 Цикл
			ПараметрыВыгрузки.Вставить("ПР00001" + Формат(НомСтр, "ЧЦ=4; ЧВН=; ЧГ=") + Формат(НомКол, "ЧЦ=2; ЧВН=; ЧГ="),
...
Функция СформироватьСтруктуруДопАтрибутов()
	...
	Макет = Отчеты[Сред(Лев(ЭтаФорма.ИмяФормы, СтрНайти(ЭтаФорма.ИмяФормы, ".Форма.") - 1), 7)].ПолучитьМакет("АтрибВыгрузкиXML2015Кв1");
