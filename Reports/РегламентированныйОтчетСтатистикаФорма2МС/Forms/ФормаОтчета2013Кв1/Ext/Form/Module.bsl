...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	// Версия формы.
	СтруктураРеквизитовФормы.мВерсияФормы = "09/01/2014";
...
Функция ПроверитьВозможностьВыгрузки(ПараметрыВыгрузки, ДеревоВыгрузки)
	...
	// Сумма стр. с 151 по 153 = стр. 109, гр. 4, разд. 1;
	Если Не (ПараметрыВыгрузки["ПР00001015103"] + ПараметрыВыгрузки["ПР00001015203"] + ПараметрыВыгрузки["ПР00001015303"]
		...
		РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(
			ЭтаФорма, "Не выполнено условие контроля: в справке 2 к разделу 1 сумма данных в строках с 151 по 153 должна быть равна данным в стр. 109 по графе 4 раздела 1!",
	...
	Если ПараметрыВыгрузки["ПР00001010904"] <> 0 Тогда
		...
		КонецЕсли;
	КонецЕсли;
	// Стр.165, 166 < стр.109, гр.3, разд.1.
	...
	Если Не (ПараметрыВыгрузки["ПР00003030106"]
		...
		СтатусПроверки = Ложь;
	КонецЕсли;
	// Сумма стр.301, 302, 304, 306, гр.7 = стр.109, гр.8, разд.1.
	Если Не (ПараметрыВыгрузки["ПР00003030107"]
		+ ПараметрыВыгрузки["ПР00003030207"]
		+ ПараметрыВыгрузки["ПР00003030407"]
		+ ПараметрыВыгрузки["ПР00003030607"]
		= ПараметрыВыгрузки["ПР00001010908"]) Тогда
		РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(
			ЭтаФорма, "Не выполнено условие контроля: сумма данных строк 301, 302, 304, 306 раздела 3 по графе 7 должна быть равна данным в стр. 109 раздела 1 по графе 8!",
			ИмяПоляТаблДокумента, "П000001010908");
		СтатусПроверки = Ложь;
	КонецЕсли;
	// Сумма стр.303, 305, гр.3 = стр.109, гр.9, разд.1.
	Если Не (ПараметрыВыгрузки["ПР00003030303"]
		+ ПараметрыВыгрузки["ПР00003030503"]
		= ПараметрыВыгрузки["ПР00001010909"]) Тогда
		РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(
			ЭтаФорма, "Не выполнено условие контроля: сумма данных строк 303, 305 раздела 3 по графе 3 должна быть равна данным в стр. 109 раздела 1 по графе 9!",
			ИмяПоляТаблДокумента, "П000001010909");
		СтатусПроверки = Ложь;
	КонецЕсли;
	// Сумма стр.303, 305, гр.4 = стр.109, гр.10, разд.1.
	Если Не (ПараметрыВыгрузки["ПР00003030304"]
		+ ПараметрыВыгрузки["ПР00003030504"]
		= ПараметрыВыгрузки["ПР00001010910"]) Тогда
		РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(
			ЭтаФорма, "Не выполнено условие контроля: сумма данных строк 303, 305 раздела 3 по графе 4 должна быть равна данным в стр. 109 раздела 1 по графе 10!",
			ИмяПоляТаблДокумента, "П000001010910");
		СтатусПроверки = Ложь;
	КонецЕсли;
	// Сумма стр.303, 305, гр.5 = стр.109, гр.11, разд.1.
	Если Не (ПараметрыВыгрузки["ПР00003030305"]
		+ ПараметрыВыгрузки["ПР00003030505"]
		= ПараметрыВыгрузки["ПР00001010911"]) Тогда
		РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(
			ЭтаФорма, "Не выполнено условие контроля: сумма данных строк 303, 305 раздела 3 по графе 5 должна быть равна данным в стр. 109 раздела 1 по графе 11!",
			ИмяПоляТаблДокумента, "П000001010911");
		СтатусПроверки = Ложь;
	КонецЕсли;
	// Сумма стр.303, 305, гр.6 = стр.109, гр.12, разд.1.
	Если Не (ПараметрыВыгрузки["ПР00003030306"]
		+ ПараметрыВыгрузки["ПР00003030506"]
		= ПараметрыВыгрузки["ПР00001010912"]) Тогда
		РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(
			ЭтаФорма, "Не выполнено условие контроля: сумма данных строк 303, 305 раздела 3 по графе 6 должна быть равна данным в стр. 109 раздела 1 по графе 12!",
			ИмяПоляТаблДокумента, "П000001010912");
		СтатусПроверки = Ложь;
	КонецЕсли;
	// Сумма стр.303, 305, гр.7 = стр.109, гр.13, разд.1.
	Если Не (ПараметрыВыгрузки["ПР00003030307"]
		+ ПараметрыВыгрузки["ПР00003030507"]
		= ПараметрыВыгрузки["ПР00001010913"]) Тогда
		РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(
			ЭтаФорма, "Не выполнено условие контроля: сумма данных строк 303, 305 раздела 3 по графе 7 должна быть равна данным в стр. 109 раздела 1 по графе 13!",
			ИмяПоляТаблДокумента, "П000001010913");
		СтатусПроверки = Ложь;
	КонецЕсли;
