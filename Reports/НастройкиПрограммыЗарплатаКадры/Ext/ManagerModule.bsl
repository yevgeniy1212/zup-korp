...
Процедура ПодготовитьДанныеПоРазделуОрганизация(ТаблицаПоказателей)
	...
	// ЗУПК
		"ИИН", "Настройки организации", "Главное", """""", 1, 1, 2, "Ссылка");
	...
	ДобавитьПоказатель(ТаблицаПоказателей, "РегистрСведений.НастройкиРасчетаРезервовОтпусков.СрезПоследних()", "МетодНачисленияРезерваОтпусков", 
		"Метод расчета резерва", "Настройки организации", "Резервы отпусков", "Значение(Перечисление.МетодыНачисленияРезервовПоОплатеТруда.ПустаяСсылка)", 1, 4, 3, "Организация");
...
Процедура ПодготовитьДанныеПоРазделуКадровыйУчет(ТаблицаПоказателей)
	...
	ДобавитьПоказатель(ТаблицаПоказателей, "РегистрСведений.НастройкиШтатногоРасписания", "ПредставлениеТарифовИНадбавок", 
		// ЗУПК
		"Способ отображения надбавок в печатной форме штатного расписания", "Кадровый учет", "Штатное расписание", "Значение(Перечисление.ПредставлениеТарифовИНадбавок.ПустаяСсылка)", 3, 2, 6);
		// ЗУПК
...
Процедура ПодготовитьДанныеПоРазделуРасчетЗарплаты(ТаблицаПоказателей)
	...
	// ЗУПК
	ДобавитьПоказатель(ТаблицаПоказателей, "РегистрСведений.НастройкиРасчетаЗарплатыРасширенный", "ИспользоватьРасчетыОтОбратного", 
		"Используются расчеты <от обратного>", "Расчет зарплаты", "", "Ложь", 4, 1, 1);
	...
	Если РаботаВБюджетномУчреждении Тогда
		ДобавитьПоказатель(ТаблицаПоказателей, "РегистрСведений.НастройкиВыплатПоДоговорамОпеки", "ИспользоватьВыплатыПоДоговорамОпеки", 
			"Регистрируются выплаты по договорам опеки", "Расчет зарплаты", "", "Ложь", 4, 1, 10);
		Если ПолучитьФункциональнуюОпцию("ИспользоватьВыплатыПоДоговорамОпеки") Тогда 
			ДобавитьПоказатель(ТаблицаПоказателей, "РегистрСведений.НастройкиВыплатПоДоговорамОпеки", "УчитыватьВозрастРебенкаПриВыплатахПоДоговорамОпеки", 
				"Учитывается возраст ребенка при выплатах по договорам опеки", "Расчет зарплаты", "", "Ложь", 4, 1, 10);
		КонецЕсли;	
	КонецЕсли;
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("ЗарплатаКадрыПриложения.Медицина") Тогда
		...
		МодульМедицина.ДобавитьПоказательОтчетаПоНастройкам(ТаблицаПоказателей);
	КонецЕсли;
	КонецЕсли;
	// ЗУПК
...
Процедура ПодготовитьДанныеПоРазделуУправлениеПерсоналом(ТаблицаПоказателей)
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("ЗарплатаКадрыКорпоративнаяПодсистемы.ФинансовыйУчет") Тогда
		...
		МодульФинансовыйУчет.ДобавитьПоказательИспользоватьФинансовыйУчет(ТаблицаПоказателей);
	КонецЕсли;
