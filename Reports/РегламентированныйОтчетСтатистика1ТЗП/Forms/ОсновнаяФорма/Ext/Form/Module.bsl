...
Процедура ОбработкаПериодичностьОтчета(Форма)
	...
	// Если текущая периодичность отчета не Месяц, тогда установим Месяц.
	Форма.ПолеВыбораПериодичность = Форма.ПеречислениеПериодичностьМесяц;
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если НЕ ЗначениеЗаполнено(мДатаНачалаПериодаОтчета) И НЕ ЗначениеЗаполнено(мДатаКонцаПериодаОтчета) Тогда
		мДатаКонцаПериодаОтчета  = КонецМесяца(ТекущаяДата());
		Если Месяц(мДатаКонцаПериодаОтчета) < 6 Тогда
			мДатаКонцаПериодаОтчета = КонецМесяца(Дата(Год(мДатаКонцаПериодаОтчета) - 1, 6, 1));
		КонецЕсли;
