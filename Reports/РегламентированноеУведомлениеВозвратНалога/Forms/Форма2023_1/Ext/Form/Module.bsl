...
Процедура ЗаполнитьНачальныеДанные() Экспорт
	...
	СвСчет = ДанныеУведомления["СвСчет"];
	...
	Иначе
		...
		Если Не ЗначениеЗаполнено(СведенияОбОрганизации.ИННФЛ) Тогда 
			...
			СвСчет.Вставить("КодВидДок", СведенияОбОрганизации.КодУдЛичн);
			СвСчет.Вставить("СерНомДок", СведенияОбОрганизации.СерияУдЛичн + " " + СведенияОбОрганизации.НомерУдЛичн);
			СвСчет.Вставить("ВыдДок", СведенияОбОрганизации.ОрганВыданУдЛичн);
			СвСчет.Вставить("ДатаДок", СведенияОбОрганизации.ДатаУдЛичн);
		КонецЕсли;
		
		СвСчет.Вставить("ПрПолуч", "2");
	КонецЕсли;
	
	СвСчет.БИК = СведенияОбОрганизации.БанкСчетБИКБанка;
	СвСчет.НомСч = СведенияОбОрганизации.БанкСчетНомер;
...
Процедура СформироватьДеревоСтраниц() Экспорт
	ЗначениеВРеквизитФормы(
		Отчеты[СтрРазделить(ИмяФормы, ".")[1]].СформироватьПустоеДеревоСтраниц_2023(Объект.Организация),
		"ДеревоСтраниц");
КонецПроцедуры
...
Процедура ПоказатьТекущуюСтраницу(ИмяМакета)
	...
	Если ТекущееИДНаименования = "Титульная" Тогда 
		...
		КонецЕсли;
	ИначеЕсли ТекущееИДНаименования = "СвСчет" И ЭтоЮЛ Тогда
...
Функция СформироватьВыгрузкуИПолучитьДанные() Экспорт 
	...
	СтруктураВыгрузки.Вставить("ИмяФайла", "1112542_5.02000_02.tif");
