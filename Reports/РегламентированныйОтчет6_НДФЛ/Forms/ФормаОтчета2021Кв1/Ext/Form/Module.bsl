...
Процедура СформироватьСпискиВыбора()
	...
	МакетСоставаПоказателей = ОбъектОтчета(ЭтаФорма.ИмяФормы).ПолучитьМакет("СпискиВыбора2021Кв1");
...
Функция ДополнительныйТекстПриДобавленииУдаленииСтрок(Знач ИмяГруппы)
	...
	Если ЗначениеЗаполнено(ИмяГруппы) И СтруктураРеквизитовФормы.СтруктураМногострочныхЧастей.Свойство(ИмяГруппы) Тогда
		...
		ИначеЕсли ИмяГруппы = СтруктураРеквизитовФормы.мИдГруппыП1М1 Тогда
			...
			ТекстВопроса = НСтр("ru='Удалить выбранные сведения по ставке налога?'");
...
Функция ПечатьРеестраНаСервере(ВидПечати, ЗаголовокПечатнойФормы)
	...
	Макет = ОбъектОтчета(ЭтаФорма.ИмяФормы).ПолучитьМакет("ПечатныйБланк2021Кв1_Реестр");
	...
	СтрКолонкиСуммирования = "ДохОбщ,ДохДив,Выч,НалИсч,ФиксАв,НалУд,НалПереч,НалНеУд,НалИзлУд,НалВозвр";
		...
		Для каждого Ветка Из ДеревоДанных.Строки Цикл
			...
			Для ИндСтрТЗ = 1 По ТЗ.Количество() - 1 Цикл
				...
				Если ИндСтрТЗ = ТЗ.Количество() - 1 Тогда
					...
					Обл = ПФ.Область(ОблВерх, 5, ОблВерх, 16);
...
Функция ВыполнитьДокументированиеНаСервере()
	...
	Возврат ОбъектОтчета.ВыполнитьДокументированиеОбъект_2021Кв1();
...
Процедура ВвестиПоказатель(Элемент, Область, КодПоказателя, Знач ЭтоСумма, Знач ТекстВыбора, ТаблицаВыбора, СтандартнаяОбработка, КолонкаПоиска = "Код") Экспорт
	...
	Если ТаблицаВыбора = Неопределено
		ИЛИ (СтруктураРеквизитовФормы.мАвтоВыборКодов И НЕ Область.Имя = "КодФормыРеорганизации") Тогда
		Возврат;
	КонецЕсли;
...
Процедура ВвестиПоказательЗавершение(РезультатВыбора, ДополнительныеПараметры) Экспорт
	...
	Если Область.Имя = "КодФормыРеорганизации" И НЕ РасчетСоставленЗаГод Тогда
