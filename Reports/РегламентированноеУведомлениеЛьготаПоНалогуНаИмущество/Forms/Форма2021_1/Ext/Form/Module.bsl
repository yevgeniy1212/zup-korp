...
Процедура ПредставлениеУведомленияВыбор(Элемент, Область, СтандартнаяОбработка)
	...
	ИначеЕсли Область.Имя = "СтруктЕдАкт" Тогда 
		СтандартнаяОбработка = Ложь;
...
Процедура ЗагрузитьПодготовленныеДанные()
	...
	Для Каждого КЗ Из СтруктураДанных.Контейнер.СведОбНО Цикл 
		...
		ДанныеМногостраничныхРазделов.СведОбНО.Добавить(СтруктураСтраницы);
		...
		Если ОтбСтроки.Количество() = 0 Тогда
			...
			ДанныеМногостраничныхРазделов.СведДокПодтвЛьг.Добавить(СведДокПодтвЛьг);
		Иначе
	...
	Если СтруктураДанных.Контейнер.СведОбНО.Количество() = 0 Тогда
		ДанныеМногостраничныхРазделов.СведОбНО.Добавить(ОбщегоНазначения.СтрокаТаблицыЗначенийВСтруктуру(СтруктураДанных.Контейнер.СведОбНО.Добавить()));
		ДанныеМногостраничныхРазделов.СведОбНО[0].Значение.УИД = Новый УникальныйИдентификатор;
		СведДокПодтвЛьг = ОбщегоНазначенияКлиентСервер.СкопироватьСтруктуру(СтраницаСведДокПодтвЛьг);
		СведДокПодтвЛьг.УИД = Новый УникальныйИдентификатор;
		СведДокПодтвЛьг.УИДРодителя = ДанныеМногостраничныхРазделов.СведОбНО[0].Значение.УИД;
		ДанныеМногостраничныхРазделов.СведДокПодтвЛьг.Добавить(СведДокПодтвЛьг);
	КонецЕсли;
	...
	СтрРег.Многостраничность = Истина;
	СтрРег.Многострочность = Истина;
	Для Каждого Стр Из ДанныеМногостраничныхРазделов.СведОбНО Цикл 
		СтрРегТр = СтрРег.ПолучитьЭлементы().Добавить();
		СтрРегТр.Наименование = "Стр. " + (ДанныеМногостраничныхРазделов.СведОбНО.Индекс(Стр) + 1);
		...
		Инд = 0;
		Для Каждого Стр2 Из ДанныеМногостраничныхРазделов.СведДокПодтвЛьг Цикл 
