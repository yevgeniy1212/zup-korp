...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	// Версия формы
	СтруктураРеквизитовФормы.мВерсияФормы = "15/01/2010";
...
Процедура ЗаполнитьСведенияОбОрганизации(ВПрограммеИзмененаДатаПодписи = Ложь, СменилсяПризнакПодписанта = Ложь)
	...
	Если (СтруктураРеквизитовФормы.мВПрограммеИзмененаОрганизация) ИЛИ (НЕ ЗначениеЗаполнено(РегламентированнаяОтчетностьКлиентСервер.ВернутьЗначениеПараметраСЛиста(ЭтаФорма, "Титульный", "ОКПО"))) Тогда
		ТабличныйДокумент.Области.ОКПО.Значение = ОКПО;
	КонецЕсли;
	
	Если (СтруктураРеквизитовФормы.мВПрограммеИзмененаОрганизация) ИЛИ (НЕ ЗначениеЗаполнено(РегламентированнаяОтчетностьКлиентСервер.ВернутьЗначениеПараметраСЛиста(ЭтаФорма, "Титульный", "ОКОПФ"))) Тогда
		ТабличныйДокумент.Области.ОКОПФ.Значение = ОКОПФ;
	КонецЕсли;
	
	Если (СтруктураРеквизитовФормы.мВПрограммеИзмененаОрганизация) ИЛИ (НЕ ЗначениеЗаполнено(РегламентированнаяОтчетностьКлиентСервер.ВернутьЗначениеПараметраСЛиста(ЭтаФорма, "Титульный", "ОКФС"))) Тогда
...
Процедура ЗаполнитьСодержательнуюЧасть(УзелРодитель)
	...
	// выгружаем раздел 1
	УзелРасчетПоНачисленнымИуплаченнымВзносам = ПолучитьПодчиненныйЭлемент(УзелРодитель, "РасчетПоНачисленнымИуплаченнымВзносам");
	ЗаполнитьДаннымиУзел(Раздел12, УзелРасчетПоНачисленнымИуплаченнымВзносам);
	
	// выгружаем раздел 2
	УзелРасчетБазыДляНачисленияВзносовПоДополнительномуТарифу = ПолучитьПодчиненныйЭлемент(УзелРодитель, "РасчетБазыДляНачисленияВзносовПоДополнительномуТарифу");
	ЗаполнитьДаннымиУзел(Раздел12, УзелРасчетБазыДляНачисленияВзносовПоДополнительномуТарифу);
	
	// выгружаем раздел 3
	УзелСведенияОзадолженностиПоВзносамПоДополнительномуТарифуПоСостояниюНаКонецГода = ПолучитьПодчиненныйЭлемент(УзелРодитель, "СведенияОзадолженностиПоВзносамПоДополнительномуТарифуПоСостояниюНа31.12.2009");
	ЗаполнитьДаннымиУзел(Раздел30, УзелСведенияОзадолженностиПоВзносамПоДополнительномуТарифуПоСостояниюНаКонецГода);
...
Процедура НачальнаяПодготовкаФормы()
	...
	Если СтруктураРеквизитовФормы.мСохраненныйДок = Неопределено Тогда
		...
		стрКодТарифа = ?(Год(СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета) >= 2011, "21", "01");
		ТабличныйДокумент.Области.Тариф.Значение = стрКодТарифа;
...
Функция ПроверитьВозможностьВыгрузки(ПараметрыВыгрузки, ДеревоВыгрузки)
	...
	Если ПустаяСтрока(ПараметрыВыгрузки.КодПоОКПО) Тогда
		РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма, "Не указан код ОКПО!", "Титульный", "ОКПО");
		СтатусПроверки = Ложь;
	КонецЕсли;
...
Процедура Расчет(Форма, ИмяТекущейСтраницыПанели = "", Знач ОбластьИмя = "")
	...
	Если ИмяТекущейСтраницыПанели = "Раздел12" Тогда
		...
		Если НЕ ЗначениеЗаполнено(ОбластьИмя) ИЛИ СтрЧислоВхождений(Форма.СтруктураЗависимыхЯчеекДляАвторасчета.П000100011403, ОбластьИмя) > 0 Тогда
			...
			П000100011103 = РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФорматуДелителя(Форма, Форма.ТабличныйДокумент.Области.П000100011103.Значение);
			П000100011203 = РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФорматуДелителя(Форма, Форма.ТабличныйДокумент.Области.П000100011203.Значение);
			П000100011303 = РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФорматуДелителя(Форма, Форма.ТабличныйДокумент.Области.П000100011303.Значение);
			
			П000100011403 = П000100011103 + П000100011203 + П000100011303;
			
			Форма.ТабличныйДокумент.Области.П000100011403.Значение = П000100011403;
		...
		Если НЕ ЗначениеЗаполнено(ОбластьИмя) ИЛИ СтрЧислоВхождений(Форма.СтруктураЗависимыхЯчеекДляАвторасчета.П000100013003, ОбластьИмя) > 0 Тогда
			...
			П000100010003 = РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФорматуДелителя(Форма, Форма.ТабличныйДокумент.Области.П000100010003.Значение);
			П000100011003 = РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФорматуДелителя(Форма, Форма.ТабличныйДокумент.Области.П000100011003.Значение);
			П000100012003 = РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФорматуДелителя(Форма, Форма.ТабличныйДокумент.Области.П000100012003.Значение);
		...
		Если НЕ ЗначениеЗаполнено(ОбластьИмя) ИЛИ СтрЧислоВхождений(Форма.СтруктураЗависимыхЯчеекДляАвторасчета.П000100014403, ОбластьИмя) > 0 Тогда
			...
			П000100014103 = РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФорматуДелителя(Форма, Форма.ТабличныйДокумент.Области.П000100014103.Значение);
			П000100014203 = РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФорматуДелителя(Форма, Форма.ТабличныйДокумент.Области.П000100014203.Значение);
			П000100014303 = РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФорматуДелителя(Форма, Форма.ТабличныйДокумент.Области.П000100014303.Значение);
			
			П000100014403 = П000100014103 + П000100014203 + П000100014303;
			
			Форма.ТабличныйДокумент.Области.П000100014403.Значение = П000100014403;
		...
		Если НЕ ЗначениеЗаполнено(ОбластьИмя) ИЛИ СтрЧислоВхождений(Форма.СтруктураЗависимыхЯчеекДляАвторасчета.П000100015003, ОбластьИмя) > 0 Тогда
			...
			П000100011003 = РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФорматуДелителя(Форма, Форма.ТабличныйДокумент.Области.П000100011003.Значение);
			П000100012003 = РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФорматуДелителя(Форма, Форма.ТабличныйДокумент.Области.П000100012003.Значение);
			П000100014003 = РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФорматуДелителя(Форма, Форма.ТабличныйДокумент.Области.П000100014003.Значение);
	...
	ИначеЕсли ИмяТекущейСтраницыПанели = "Раздел30" Тогда
		...
		Если НЕ ЗначениеЗаполнено(ОбластьИмя) ИЛИ СтрЧислоВхождений(Форма.СтруктураЗависимыхЯчеекДляАвторасчета.П000300053003, ОбластьИмя) > 0 Тогда
			...
			П000300051003 = РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФорматуДелителя(Форма, Форма.ТабличныйДокумент.Области.П000300051003.Значение);
			П000300052003 = РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФорматуДелителя(Форма, Форма.ТабличныйДокумент.Области.П000300052003.Значение);
...
Процедура СформироватьСпискиВыбора()
	...
	стрСписокТарифов = ?(Год(СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета) >= 2011, "КодыТарифов2011", "КодыТарифов");
	РегламентированнаяОтчетность.ДобавитьСтрокуОписанияВвода(СвойстваПоказателей, "Тариф", 2, , "Выбор тарифа", КоллекцияСписковВыбора[стрСписокТарифов]);
...
Функция СформироватьСтруктуруПараметров()
	...
	ПараметрыВыгрузки.Вставить("ОГРН", ПредставлениеЧисла(Титульный.ОГРН));
	
	ПараметрыВыгрузки.Вставить("ИНН", ПредставлениеЧисла(Титульный.ИНН));
	...
	ПараметрыВыгрузки.Вставить("КодПоОКПО", СокрЛП(Титульный.ОКПО));
	...
	//
	ПараметрыВыгрузки.Вставить("ИмяКлючевогоУзлаСодержательнойЧасти", "РАСЧЕТ_ПО_ВЗНОСАМ_ПО_ДОПОЛНИТЕЛЬНОМУ_ТАРИФУ_ЛЭВС");
