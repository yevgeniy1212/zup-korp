...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Объект.ВидУведомления = Перечисления.ВидыУведомленийОСпецрежимахНалогообложения.ЗаключениеТрудовогоДоговора;
	...
	ПоказатьТекущуюСтраницу("Титульная_2023_Заключение", Неопределено);
...
Процедура СформироватьДеревоСтраниц() Экспорт
	...
	Стр001.ИмяМакета = "Титульная_2023_Заключение";
...
Процедура ЗаполнитьНачальныеДанные() Экспорт
	...
	Иначе 
		СтрокаСведений = "ИННФЛ,ФИО,ТелДом,АдрПрописки,ОКВЭД,ОГРН,ОКТМО";
		СведенияОбОрганизации = РегламентированнаяОтчетностьВызовСервера.ПолучитьСведенияОбОрганизации(
			Объект.Организация, Объект.ДатаПодписи, СтрокаСведений);
		ДанныеУведомленияТитульный.Работодатель = СведенияОбОрганизации.ФИО + ", " 
			+ СведенияОбОрганизации.АдрПрописки + ", " + СведенияОбОрганизации.ИННФЛ + ", "
			+ СведенияОбОрганизации.ОГРН + ", " + СведенияОбОрганизации.ОКТМО;
		ДанныеУведомленияТитульный.Телефон = СведенияОбОрганизации.ТелДом;
		ДанныеУведомленияТитульный.ОКВЭД = СведенияОбОрганизации.ОКВЭД;
	КонецЕсли;
КонецПроцедуры
...
Процедура ПредставлениеУведомленияВыбор(Элемент, Область, СтандартнаяОбработка)
	...
	Если СтрНачинаетсяС(Область.Имя, "Флаг") Тогда 
		...
		Если ОТЧ.ПривестиЗначение(Сред(Область.Имя, 5)) <= 8 Тогда 
			Для Инд = 1 По 8 Цикл
