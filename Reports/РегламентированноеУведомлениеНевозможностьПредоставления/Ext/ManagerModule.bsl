...
Функция ПроверитьДокументСВыводомВТаблицу_Форма2015_1(Данные, УникальныйИдентификатор)
	...
	ТаблицаОшибок.Добавить(Документы.УведомлениеОСпецрежимахНалогообложения.ПолучитьПустуюСтруктуруНавигацииПоОшибкам(
		"Устаревшая редакция формы", "ОсновныеСведения", "_Номер"));
...
Процедура СоздатьУведомленияОНевозможностиПредоставления_2019(ВыборкаСтрока) Экспорт
	Попытка
		...
		Уведомление.ИмяФормы = "Форма2019_1";
		...
		Уведомление.ДатаПодписи = ВыборкаСтрока.Ссылка.ДатаПодписи;
		...
		_СвНевозмПред = Новый ТаблицаЗначений;
		_СвНевозмПред.Колонки.Добавить("КодПричины", Новый ОписаниеТипов("Строка"));
		_СвНевозмПред.Колонки.Добавить("УИД", Новый ОписаниеТипов("УникальныйИдентификатор"));
		_СвНевозмПред.Колонки.Добавить("Описание", Новый ОписаниеТипов("Строка"));
		_СвНевозмПред.Колонки.Добавить("Данные", Новый ОписаниеТипов("СписокЗначений"));
		_СвНевозмПред.Колонки.Добавить("Причина", Новый ОписаниеТипов("Строка"));
		_СвНевозмПред.Колонки.Добавить("ДокИнф", Новый ОписаниеТипов("ДеревоЗначений"));
		_СвНевозмПред.Колонки.Добавить("ДокументыПредставление", Новый ОписаниеТипов("Строка"));
		_СвНевозмПред.Колонки.Добавить("ПриложенныеФайлы", Новый ОписаниеТипов("ДеревоЗначений"));
		_СвНевозмПред.Колонки.Добавить("СканированныеДокументыОписание", Новый ОписаниеТипов("Строка"));
		_СвНевозмПред.Колонки.Добавить("_ИмяДокР", Новый ОписаниеТипов("ТаблицаЗначений"));
		
		ДеревоПриложенныеФайлыЭталон = Новый ДеревоЗначений;
		ДеревоПриложенныеФайлыЭталон.Колонки.Добавить("ИндексКартинки", Новый ОписаниеТипов("Число"));
		ДеревоПриложенныеФайлыЭталон.Колонки.Добавить("Документ", Новый ОписаниеТипов("Строка"));
		ДеревоПриложенныеФайлыЭталон.Колонки.Добавить("УИДДокумент", Новый ОписаниеТипов("УникальныйИдентификатор"));
		ДеревоПриложенныеФайлыЭталон.Колонки.Добавить("УИДФайл", Новый ОписаниеТипов("УникальныйИдентификатор"));
		ДеревоПриложенныеФайлыЭталон.Колонки.Добавить("ПрисоединенныйФайл", Новый ОписаниеТипов("СправочникСсылка.УведомлениеОСпецрежимахНалогообложенияПрисоединенныеФайлы"));
		
		Для Каждого Стр Из ДанныеОтчета.СобранныеДанныеРазделов._ДетализацияОтветаНаТребование Цикл 
			НовСтр = _СвНевозмПред.Добавить();
			ЗаполнитьЗначенияСвойств(НовСтр, Стр);
			НовСтр.УИД = Новый УникальныйИдентификатор;
		КонецЦикла;
		СтруктураУведомления.Вставить("_СвНевозмПред", _СвНевозмПред);
		...
		Запрос.УстановитьПараметр("РегламентированныйОтчет", ВыборкаСтрока.Ссылка);
		ТаблицаФайлов = Запрос.Выполнить().Выгрузить();
		
		Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.РаботаСФайлами") Тогда
			МодульРаботаСФайлами = ОбщегоНазначения.ОбщийМодуль("РаботаСФайлами");
			Для Каждого Стр Из СтруктураУведомления._СвНевозмПред Цикл
				НовоеДерево = ДеревоПриложенныеФайлыЭталон.Скопировать();
				Для Каждого Стр1 Из Стр.ПриложенныеФайлы.Строки Цикл
					НовСтр = НовоеДерево.Строки.Добавить();
					НовСтр.ИндексКартинки = 0;
					НовСтр.Документ = Стр1.Документ;
					НовСтр.УИДДокумент = Стр1.УИДДокумент;
					Для Каждого Стр2 Из Стр1.Строки Цикл
						НовСтрПодч = НовСтр.Строки.Добавить();
						НовСтрПодч.ИндексКартинки = 2;
						НовСтрПодч.Документ = Стр2.Документ;
						НовСтрПодч.УИДДокумент = Стр2.УИДДокумент;
						НовСтрПодч.УИДФайл = Стр2.УИДФайл;
						НайденныйФайл = ТаблицаФайлов.НайтиСтроки(Новый Структура("ВидДополнительногоФайла", Стр2.УИДДокумент + "@" + Стр2.УИДФайл));
						Если НайденныйФайл.Количество() = 1 Тогда 
							НайденныйФайл = НайденныйФайл[0];
...
Процедура КонвертацияИзРегламентированногоОтчета(ВыборкаСтрока) Экспорт
	Попытка
		Если ВыборкаСтрока.ВыбраннаяФорма = "ФормаОтчета2019Кв1" Тогда
			СоздатьУведомленияОНевозможностиПредоставления_2019(ВыборкаСтрока.Ссылка);
		КонецЕсли;
	Исключение
	КонецПопытки;
КонецПроцедуры
