...
Процедура Расчет(Форма, ИмяТекущейСтраницыПанели = "", ОбластьИмя = "")
	...
	Для Гр = 3 По 7 Цикл
		...
		КонецЦикла;
		Области["П0000201202" + Графа].Значение = СуммаПоСтр;
		
		Области["П0000201209" + Графа].Значение
			= Области["П0000201210" + Графа].Значение
	...
	Для Гр = 3 По 19 Цикл
		...
		СуммаПоСтр = 0;
		Для Стр = 303 По 307 Цикл
			СуммаПоСтр = СуммаПоСтр + Области["П0030010" + Стр + Графа].Значение;
		КонецЦикла;
		Области["П0030010302" + Графа].Значение = СуммаПоСтр;
		
		Области["П0030010309" + Графа].Значение
			= Области["П0030010301" + Графа].Значение
			+ Области["П0030010302" + Графа].Значение;
		
	КонецЦикла;
	
	Для Стр = 301 По 309 Цикл
		СуммаПоГрафам = 0;
	...
	Для Гр = 3 По 9 Цикл
		...
		СуммаПоСтр = 0;
		Для Стр = 403 По 407 Цикл
			СуммаПоСтр = СуммаПоСтр + Области["П0040010" + Стр + Графа].Значение;
		КонецЦикла;
		Области["П0040010402" + Графа].Значение = СуммаПоСтр;
	...
	Для Гр = 4 По 18 Цикл
		...
		СуммаПоСтр = 0;
		Для Стр = 503 По 507 Цикл
			СуммаПоСтр = СуммаПоСтр + Области["П0050010" + Стр + Графа].Значение;
		КонецЦикла;
		Области["П0050010502" + Графа].Значение = СуммаПоСтр;
	...
	Для Гр = 3 По 6 Цикл
		...
		СуммаПоСтр = 0;
		Для Стр = 603 По 607 Цикл
			СуммаПоСтр = СуммаПоСтр + Области["П0060010" + Стр + Графа].Значение;
		КонецЦикла;
		Области["П0060010602" + Графа].Значение = СуммаПоСтр;
...
Функция ПечатьНаСервере(ВидПечати)
	...
	СтруктураРеквизитовФормы.мПечатныеФормы.Добавить(ПечатнаяФорма, "Форма 1-Т (МС)");
	
	Для Часть = 1 По 4 Цикл
		ПечатнаяФорма = РегламентированнаяОтчетность.НоваяСтраница();
	...
	ПечатнаяФорма = РегламентированнаяОтчетность.НоваяСтраница();
	ПечатнаяФорма.Вывести(ТабличныйДокумент.ПолучитьОбласть("ОсновнаяЧасть6"));
	СтруктураРеквизитовФормы.мПечатныеФормы.Добавить(ПечатнаяФорма, "Форма 1-Т (МС)");
