...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	// Версия формы.
	СтруктураРеквизитовФормы.мВерсияФормы = "04/09/2018";
	...
	Для НомСтр = 101 По 124 Цикл
		...
		Для НомКол = 9 По 11 Цикл
			ТекСумма = ТекСумма + Области["П000001"
				...
				+ Формат(НомКол, "ЧЦ=2; ЧВН=; ЧГ=")].Значение;
		КонецЦикла;
		Области["П000001" + Формат(НомСтр, "ЧЦ=4; ЧВН=; ЧГ=") + "08"].Значение = ТекСумма;
	КонецЦикла;
...
Функция СформироватьСтруктуруПараметров() Экспорт 
	...
	Если ТабДокумент.ОргУровеньГосОргана = "Федеральный" Тогда
		ИмяМакета = "АтрибВыгрузкиXML2018Кв1_фед";
	Иначе
		ИмяМакета = "АтрибВыгрузкиXML2018Кв1_рег";
	КонецЕсли;
...
Функция ПолучитьПараметрыИнициализацииУниверсальногоОтчета()
	...
	Если ТабДокумент.ОргУровеньГосОргана = "Федеральный" Тогда
		ИмяМакета = "АтрибВыгрузкиXML2018Кв1_фед";
		ИмяШаблона = "ШаблонРосстата2018Кв1_фед";
	Иначе
		ИмяМакета = "АтрибВыгрузкиXML2018Кв1_рег";
		ИмяШаблона = "ШаблонРосстата2018Кв1_рег";
	КонецЕсли;
	Возврат УниверсальныйОтчетСтатистики.ПолучитьПараметрыИнициализацииУниверсальногоОтчета(ЭтаФорма, ИмяМакета, ИмяШаблона, "СхемаВыгрузкиXML2018Кв1", Истина, Ложь);
КонецФункции
