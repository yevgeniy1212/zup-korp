...
Функция ПроверитьВозможностьВыгрузки(ПараметрыВыгрузки, ДеревоВыгрузки)
	...
	// Сумма стр. с 151 по 152 = стр. 121, гр. 4, разд. 1;
	Если Не (ПараметрыВыгрузки["ПР00001015103"] + ПараметрыВыгрузки["ПР00001015203"] = ПараметрыВыгрузки["ПР00001012104"]) Тогда
		РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(
	...
	Для НомСтр = 201 По 221 Цикл
		...
		Если Не (ТекСумма = ПараметрыВыгрузки["ПР0000101" + Сред(Формат(НомСтр, "ЧЦ=4; ЧВН=; ЧГ="), 3) + "04"]) Тогда
			...
			СтатусПроверки = Ложь;
		КонецЕсли;
		// Сумма гр.9, 10 < гр. 4, разд.1 при гр.7, разд.1 ≠ 0.
		Если ПараметрыВыгрузки["ПР0000101" + Сред(Формат(НомСтр, "ЧЦ=4; ЧВН=; ЧГ="), 3) + "07"] <> 0 Тогда
			Если Не (ПараметрыВыгрузки["ПР00002" + Формат(НомСтр, "ЧЦ=4; ЧВН=; ЧГ=") + "09"]
				...
				РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(
					ЭтаФорма, "Не выполнено условие контроля: в стр. " + Формат(НомСтр, "ЧГ=")
					+ " раздела 2 сумма данных по графам 9, 10 должна быть меньше данных по графе 4 раздела 1, если данные по графе 7 раздела 1 не равны нулю!",
					ИмяПоляТаблДокумента, "П00000101" + Сред(Формат(НомСтр, "ЧЦ=4; ЧВН=; ЧГ="), 3) + "04");
				СтатусПроверки = Ложь;
			КонецЕсли;
		КонецЕсли;
		// Сумма гр.9, 10 = гр.4 (гр.5 + гр.6), разд.1 при гр.7, разд.1 = 0.
		Если ПараметрыВыгрузки["ПР0000101" + Сред(Формат(НомСтр, "ЧЦ=4; ЧВН=; ЧГ="), 3) + "07"] = 0 Тогда
			Если Не (ПараметрыВыгрузки["ПР00002" + Формат(НомСтр, "ЧЦ=4; ЧВН=; ЧГ=") + "09"]
				+ ПараметрыВыгрузки["ПР00002" + Формат(НомСтр, "ЧЦ=4; ЧВН=; ЧГ=") + "10"]
				= ПараметрыВыгрузки["ПР0000101" + Сред(Формат(НомСтр, "ЧЦ=4; ЧВН=; ЧГ="), 3) + "04"]) Тогда
				РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(
					ЭтаФорма, "Не выполнено условие контроля: в стр. " + Формат(НомСтр, "ЧГ=")
					+ " раздела 2 сумма данных по графам 9, 10 должна быть равна данным по графе 4 раздела 1, если данные по графе 7 раздела 1 равны нулю!",
					ИмяПоляТаблДокумента, "П00000101" + Сред(Формат(НомСтр, "ЧЦ=4; ЧВН=; ЧГ="), 3) + "04");
				СтатусПроверки = Ложь;
			КонецЕсли;
		КонецЕсли;
	...
	Если Не (ПараметрыВыгрузки["ПР00003030106"]
		...
		СтатусПроверки = Ложь;
	КонецЕсли;
	// Сумма стр.302, 304, гр.3 = стр.121, гр.8, разд.1.
	Если Не (ПараметрыВыгрузки["ПР00003030203"]
		+ ПараметрыВыгрузки["ПР00003030403"]
		= ПараметрыВыгрузки["ПР00001012108"]) Тогда
		РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(
			ЭтаФорма, "Не выполнено условие контроля: сумма данных строк 302, 304 раздела 3 по графе 3 должна быть равна данным в стр. 121 раздела 1 по графе 8!",
			ИмяПоляТаблДокумента, "П000001012108");
		СтатусПроверки = Ложь;
	КонецЕсли;
	// Сумма стр.302, 304, гр.4 = стр.121, гр.9, разд.1.
	Если Не (ПараметрыВыгрузки["ПР00003030204"]
		+ ПараметрыВыгрузки["ПР00003030404"]
		= ПараметрыВыгрузки["ПР00001012109"]) Тогда
		РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(
			ЭтаФорма, "Не выполнено условие контроля: сумма данных строк 302, 304 раздела 3 по графе 4 должна быть равна данным в стр. 121 раздела 1 по графе 9!",
			ИмяПоляТаблДокумента, "П000001012109");
		СтатусПроверки = Ложь;
	КонецЕсли;
	// Сумма стр.302, 304, гр.5 = стр.121, гр.10, разд.1.
	Если Не (ПараметрыВыгрузки["ПР00003030205"]
		+ ПараметрыВыгрузки["ПР00003030405"]
		= ПараметрыВыгрузки["ПР00001012110"]) Тогда
		РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(
			ЭтаФорма, "Не выполнено условие контроля: сумма данных строк 302, 304 раздела 3 по графе 5 должна быть равна данным в стр. 121 раздела 1 по графе 10!",
			ИмяПоляТаблДокумента, "П000001012110");
		СтатусПроверки = Ложь;
	КонецЕсли;
	// Сумма стр.302, 304, гр.6 = стр.121, гр.11, разд.1.
	Если Не (ПараметрыВыгрузки["ПР00003030206"]
		+ ПараметрыВыгрузки["ПР00003030406"]
		= ПараметрыВыгрузки["ПР00001012111"]) Тогда
		РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(
			ЭтаФорма, "Не выполнено условие контроля: сумма данных строк 302, 304 раздела 3 по графе 6 должна быть равна данным в стр. 121 раздела 1 по графе 11!",
...
Функция СформироватьСтруктуруДопАтрибутов()
	...
	Макет = Отчеты[Сред(Лев(ЭтаФорма.ИмяФормы, СтрНайти(ЭтаФорма.ИмяФормы, ".Форма.") - 1), 7)].ПолучитьМакет("АтрибВыгрузкиXML2014Кв1");
