...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	СтруктураРеквизитовФормы.мВерсияФормы = "18/07/2019";
...
Процедура ПоказатьПериод()
	...
	ПолеТабличногоДокументаФормаОтчета.Области.ГодОтчета1.Значение
		= Формат(СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета, "ДФ = 'гггг'") + " году, человек";
	ПолеТабличногоДокументаФормаОтчета.Области.ГодОтчета2.Значение
		= "в " + Формат(СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета, "ДФ = 'гггг'") + " году, человек";
...
Процедура Расчет(Форма, ИмяСтраницыПанели = "", ОбластьИмя = "")
	...
	Для НомСтр = 101 По 124 Цикл
		...
		КонецЦикла;
		Области["П000001" + Формат(НомСтр, "ЧЦ=4; ЧВН=; ЧГ=") + "04"].Значение = ТекСумма;
	КонецЦикла;
	Для НомКол = 3 По 7 Цикл
		...
		Для НомСтр = 104 По 106 Цикл
			ТекСумма = ТекСумма + Области["П000001"
				...
				+ Формат(НомКол, "ЧЦ=2; ЧВН=; ЧГ=")].Значение;
		КонецЦикла;
		Области["П0000010103" + Формат(НомКол, "ЧЦ=2; ЧВН=; ЧГ=")].Значение = ТекСумма;
		ТекСумма = 0;
		Для НомСтр = 108 По 110 Цикл
			ТекСумма = ТекСумма + Области["П000001"
				...
				+ Формат(НомКол, "ЧЦ=2; ЧВН=; ЧГ=")].Значение;
		КонецЦикла;
		Области["П0000010107" + Формат(НомКол, "ЧЦ=2; ЧВН=; ЧГ=")].Значение = ТекСумма;
		ТекСумма = 0;
		Для НомСтр = 112 По 115 Цикл
			ТекСумма = ТекСумма + Области["П000001"
				...
				+ Формат(НомКол, "ЧЦ=2; ЧВН=; ЧГ=")].Значение;
		КонецЦикла;
		Области["П0000010111" + Формат(НомКол, "ЧЦ=2; ЧВН=; ЧГ=")].Значение = ТекСумма;
		ТекСумма = 0;
		Для НомСтр = 117 По 120 Цикл
			ТекСумма = ТекСумма + Области["П000001"
				...
				+ Формат(НомКол, "ЧЦ=2; ЧВН=; ЧГ=")].Значение;
		КонецЦикла;
		Области["П0000010116" + Формат(НомКол, "ЧЦ=2; ЧВН=; ЧГ=")].Значение = ТекСумма;
		Области["П0000010102" + Формат(НомКол, "ЧЦ=2; ЧВН=; ЧГ=")].Значение
			= Области["П0000010103" + Формат(НомКол, "ЧЦ=2; ЧВН=; ЧГ=")].Значение
			+ Области["П0000010107" + Формат(НомКол, "ЧЦ=2; ЧВН=; ЧГ=")].Значение
			+ Области["П0000010111" + Формат(НомКол, "ЧЦ=2; ЧВН=; ЧГ=")].Значение
			+ Области["П0000010116" + Формат(НомКол, "ЧЦ=2; ЧВН=; ЧГ=")].Значение;
		Области["П0000010121" + Формат(НомКол, "ЧЦ=2; ЧВН=; ЧГ=")].Значение
			= Области["П0000010122" + Формат(НомКол, "ЧЦ=2; ЧВН=; ЧГ=")].Значение
			+ Области["П0000010123" + Формат(НомКол, "ЧЦ=2; ЧВН=; ЧГ=")].Значение;
	КонецЦикла;
	...
	Для НомСтр = 132 По 135 Цикл
		ТекСумма = ТекСумма + Области["П000001"
			...
			+ "03"].Значение;
	КонецЦикла;
	Области["П000001013103"].Значение = ТекСумма;
	
	Для НомСтр = 201 По 221 Цикл
		ТекСумма = 0;
		Для НомКол = 5 По 9 Цикл
			ТекСумма = ТекСумма + Области["П000002"
				+ Формат(НомСтр, "ЧЦ=4; ЧВН=; ЧГ=")
				+ Формат(НомКол, "ЧЦ=2; ЧВН=; ЧГ=")].Значение;
		КонецЦикла;
		Области["П000002" + Формат(НомСтр, "ЧЦ=4; ЧВН=; ЧГ=") + "03"].Значение = ТекСумма;
	КонецЦикла;
	...
	Для НомКол = 3 По 9 Цикл
		...
		Для НомСтр = 204 По 206 Цикл
			ТекСумма = ТекСумма + Области["П000002"
				...
				+ Формат(НомКол, "ЧЦ=2; ЧВН=; ЧГ=")].Значение;
		КонецЦикла;
		Области["П0000020203" + Формат(НомКол, "ЧЦ=2; ЧВН=; ЧГ=")].Значение = ТекСумма;
		ТекСумма = 0;
		Для НомСтр = 208 По 210 Цикл
			ТекСумма = ТекСумма + Области["П000002"
				...
				+ Формат(НомКол, "ЧЦ=2; ЧВН=; ЧГ=")].Значение;
		КонецЦикла;
		Области["П0000020207" + Формат(НомКол, "ЧЦ=2; ЧВН=; ЧГ=")].Значение = ТекСумма;
		ТекСумма = 0;
		Для НомСтр = 212 По 215 Цикл
			ТекСумма = ТекСумма + Области["П000002"
				...
				+ Формат(НомКол, "ЧЦ=2; ЧВН=; ЧГ=")].Значение;
		КонецЦикла;
		Области["П0000020211" + Формат(НомКол, "ЧЦ=2; ЧВН=; ЧГ=")].Значение = ТекСумма;
		ТекСумма = 0;
		Для НомСтр = 217 По 220 Цикл
			ТекСумма = ТекСумма + Области["П000002"
				...
				+ Формат(НомКол, "ЧЦ=2; ЧВН=; ЧГ=")].Значение;
		КонецЦикла;
		Области["П0000020216" + Формат(НомКол, "ЧЦ=2; ЧВН=; ЧГ=")].Значение = ТекСумма;
		Области["П0000020202" + Формат(НомКол, "ЧЦ=2; ЧВН=; ЧГ=")].Значение
			= Области["П0000020203" + Формат(НомКол, "ЧЦ=2; ЧВН=; ЧГ=")].Значение
			+ Области["П0000020207" + Формат(НомКол, "ЧЦ=2; ЧВН=; ЧГ=")].Значение
			+ Области["П0000020211" + Формат(НомКол, "ЧЦ=2; ЧВН=; ЧГ=")].Значение
			+ Области["П0000020216" + Формат(НомКол, "ЧЦ=2; ЧВН=; ЧГ=")].Значение;
		Области["П0000020221" + Формат(НомКол, "ЧЦ=2; ЧВН=; ЧГ=")].Значение
			= Области["П0000020201" + Формат(НомКол, "ЧЦ=2; ЧВН=; ЧГ=")].Значение
			+ Области["П0000020202" + Формат(НомКол, "ЧЦ=2; ЧВН=; ЧГ=")].Значение;
	КонецЦикла;
	
	Для НомСтр = 301 По 305 Цикл
		ТекСумма = 0;
		Для НомКол = 4 По 6 Цикл
			ТекСумма = ТекСумма + Области["П000003"
				+ Формат(НомСтр, "ЧЦ=4; ЧВН=; ЧГ=")
				+ Формат(НомКол, "ЧЦ=2; ЧВН=; ЧГ=")].Значение;
		КонецЦикла;
		Области["П000003" + Формат(НомСтр, "ЧЦ=4; ЧВН=; ЧГ=") + "03"].Значение = ТекСумма;
	КонецЦикла;
...
Процедура ФормироватьСтруктуруСтраницОтчета() 
	...
	СтрокаУровня1.Представление            = "Титульная";
