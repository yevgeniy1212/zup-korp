...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	// Версия формы.
	СтруктураРеквизитовФормы.мВерсияФормы = "19/10/2017";
...
Функция СформироватьСтруктуруПараметров() Экспорт 
	...
	Если ТабДокумент.ОргУровеньГосОргана = "Федеральный" Тогда
		ИмяМакета = "АтрибВыгрузкиXML2017Кв1_фед";
	Иначе
		ИмяМакета = "АтрибВыгрузкиXML2017Кв1_рег";
	КонецЕсли;
...
Функция ПолучитьПараметрыИнициализацииУниверсальногоОтчета()
	...
	Если ТабДокумент.ОргУровеньГосОргана = "Федеральный" Тогда
		ИмяМакета = "АтрибВыгрузкиXML2017Кв1_фед";
		ИмяШаблона = "ШаблонРосстата2017Кв1_фед";
	Иначе
		ИмяМакета = "АтрибВыгрузкиXML2017Кв1_рег";
		ИмяШаблона = "ШаблонРосстата2017Кв1_рег";
	КонецЕсли;
	Возврат УниверсальныйОтчетСтатистики.ПолучитьПараметрыИнициализацииУниверсальногоОтчета(ЭтаФорма, ИмяМакета, ИмяШаблона, "СхемаВыгрузкиXML2015Кв1", Истина, Ложь);
КонецФункции
