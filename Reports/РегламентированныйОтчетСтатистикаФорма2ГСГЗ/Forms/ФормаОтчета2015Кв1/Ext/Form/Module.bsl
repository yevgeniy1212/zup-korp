...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ПолеТабличногоДокументаФормаОтчета.Вывести(Отчеты[Сред(Лев(ЭтаФорма.ИмяФормы, СтрНайти(
		ЭтаФорма.ИмяФормы, ".Форма.") - 1), 7)].ПолучитьМакет("ФормаОтчета2015Кв1_ФормаОтчета"));
...
Функция СформироватьСтруктуруПараметров() Экспорт 
	...
	Если ТабДокумент.ОргУровеньГосОргана = "Федеральный" Тогда
		ИмяМакета = "АтрибВыгрузкиXML2015Кв1_фед";
	Иначе
		ИмяМакета = "АтрибВыгрузкиXML2015Кв1_рег";
	КонецЕсли;
...
Функция ПолучитьПараметрыИнициализацииУниверсальногоОтчета()
	...
	Если ТабДокумент.ОргУровеньГосОргана = "Федеральный" Тогда
		ИмяМакета = "АтрибВыгрузкиXML2015Кв1_фед";
		ИмяШаблона = "ШаблонРосстата2015Кв1_фед";
	Иначе
		ИмяМакета = "АтрибВыгрузкиXML2015Кв1_рег";
		ИмяШаблона = "ШаблонРосстата2015Кв1_рег";
	КонецЕсли;
