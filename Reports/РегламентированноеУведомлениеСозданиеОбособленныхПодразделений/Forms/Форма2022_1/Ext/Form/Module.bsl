...
Функция СформироватьВыгрузкуИПолучитьДанные() Экспорт 
	...
	СтруктураВыгрузки.Вставить("ИмяФайла", "1111053_5.04000_04.tif");
...
Процедура ЗагрузитьИзXMLНаСервере(ПараметрыЗагрузкиXML)
	...
	Для Каждого Стр Из ДанныеМногостраничныхРазделов.СведОП Цикл 
		СведОП = Стр.Значение;
		
		СтрокиЗданий = МнгСтр.НайтиСтроки(Новый Структура("УИД", СведОП.УИД));
		Инд = 0;
		Для Каждого Элт Из СтрокиЗданий Цикл 
			...
			СведОП["Аддр_ЗданиеТип" + Инд] = Элт["Аддр_ЗданиеТип"];
			СведОП["Аддр_ЗданиеНомер" + Инд] = Элт["Аддр_ЗданиеНомер"];
		КонецЦикла;
	КонецЦикла;
КонецПроцедуры
