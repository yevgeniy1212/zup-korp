...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	// Версия формы.
	СтруктураРеквизитовФормы.мВерсияФормы = "27/01/2014";
...
Процедура ПоказатьПериод()
	...
	ПолеТабличногоДокументаФормаОтчета.Области.ПериодОтчета.Значение
		= "в " + НРег(Формат(СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета, "ДФ = 'гггг'")) + " году";
...
Процедура Расчет(Форма, ИмяСтраницыПанели = "", ОбластьИмя = "")
	...
	// Для Раздела 1.
	// Стр. 15 = сумме стр. с 16 по 18.
	Для НомКол = 2 По 12 Цикл
		ТекСумма = 0;
		Для НомСтр = 16 По 18 Цикл
			ТекСумма = ТекСумма + мФормаОтчета.Области["П000001"
				+ Формат(НомСтр, "ЧЦ=4; ЧВН=; ЧГ=")
				+ Формат(НомКол, "ЧЦ=2; ЧВН=; ЧГ=")].Значение;
		КонецЦикла;
		мФормаОтчета.Области["П0000010015" + Формат(НомКол, "ЧЦ=2; ЧВН=; ЧГ=")].Значение = ТекСумма;
	КонецЦикла;
	// Гр.1 = сумме гр. с 2 по 5.
	Для НомСтр = 1 По 18 Цикл
		...
		Для НомКол = 2 По 5 Цикл
			ТекСумма = ТекСумма + мФормаОтчета.Области["П000001"
				...
				+ Формат(НомКол, "ЧЦ=2; ЧВН=; ЧГ=")].Значение;
		КонецЦикла;
		мФормаОтчета.Области["П000001" + Формат(НомСтр, "ЧЦ=4; ЧВН=; ЧГ=") + "01"].Значение = ТекСумма;
	КонецЦикла;

	// Для Раздела 2.
	// Стр. 37 = стр. 34+ стр. 35 - стр. 36.
	Для НомКол = 2 По 3 Цикл
		мФормаОтчета.Области["П0000020037" + Формат(НомКол, "ЧЦ=2; ЧВН=; ЧГ=")].Значение
			= мФормаОтчета.Области["П0000020034" + Формат(НомКол, "ЧЦ=2; ЧВН=; ЧГ=")].Значение
			+ мФормаОтчета.Области["П0000020035" + Формат(НомКол, "ЧЦ=2; ЧВН=; ЧГ=")].Значение
			- мФормаОтчета.Области["П0000020036" + Формат(НомКол, "ЧЦ=2; ЧВН=; ЧГ=")].Значение;
	КонецЦикла;
	// Гр.1 = сумме гр.2 и 3.
	Для НомСтр = 34 По 37 Цикл
		ТекСумма = 0;
		Для НомКол = 2 По 3 Цикл
			ТекСумма = ТекСумма + мФормаОтчета.Области["П000002"
				...
				+ Формат(НомКол, "ЧЦ=2; ЧВН=; ЧГ=")].Значение;
		КонецЦикла;
		мФормаОтчета.Области["П000002" + Формат(НомСтр, "ЧЦ=4; ЧВН=; ЧГ=") + "01"].Значение = ТекСумма;
	КонецЦикла;
...
Функция ПечатьНаСервере(ВидПечати)
	...
	СтруктураПараметров.Вставить("НаименованиеЛиста",  "Форма 1 кадры");
	СтруктураПараметров.Вставить("ОриентацияЛиста",    "Ландшафт");
	СтруктураПараметров.Вставить("ИмяСтраницы",        "1Кадры");
...
Функция ПроверитьВозможностьВыгрузки(ПараметрыВыгрузки, ДеревоВыгрузки)
	...
	Если НЕ РегламентированнаяОтчетность.ПроверитьРазделыНаНаличиеИнформации(ЭтаФорма, ДеревоВыгрузки, ИмяПоляТаблДокумента) Тогда
		РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(
			ЭтаФорма, "Отчет не заполнен!",
			ИмяПоляТаблДокумента, "");
		СтатусПроверки = Ложь;
		Возврат СтатусПроверки;
	КонецЕсли;
	
	// КОНТРОЛЬ показателей формы (Указания по заполнению Формы 1 кадры).
	
	// В Разделе 1.
	
	// По гр. 1: стр. 01 > 0.
	Если Не (ПараметрыВыгрузки["ПР00001000101"] > 0) Тогда
		РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма,
			"Не выполнено условие контроля: по гр. 1 раздела 1 данные в стр. 01 должны быть больше нуля!",
			ИмяПоляТаблДокумента, "П000001000101");
		СтатусПроверки = Ложь;
	КонецЕсли;
	
	// По гр. 1: сумма стр. 04, 06, 10, 11, 12 = сумме стр. с 26 по 28, справки 2.
	ТекСумма1 = ПараметрыВыгрузки["ПР00001000401"] + ПараметрыВыгрузки["ПР00001000601"]
			  + ПараметрыВыгрузки["ПР00001001001"] + ПараметрыВыгрузки["ПР00001001101"]
			  + ПараметрыВыгрузки["ПР00001001201"];
	ТекСумма2 = ПараметрыВыгрузки["ПР00001002601"] + ПараметрыВыгрузки["ПР00001002701"]
			  + ПараметрыВыгрузки["ПР00001002801"];
	Если Не (ТекСумма1 = ТекСумма2) Тогда
		РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма,
			"Не выполнено условие контроля: по гр. 1 раздела 1 сумма данных в строках 04, 06, 10, 11, 12 должна быть равна сумме данных в строках с 26 по 28 справки 2!",
			ИмяПоляТаблДокумента, "");
		СтатусПроверки = Ложь;
	КонецЕсли;
	
	// Гр.1 = сумме гр. с 6 по 12.
	Для НомСтр = 1 По 18 Цикл
		ТекСумма = 0;
		Для НомКол = 6 По 12 Цикл
			ТекСумма = ТекСумма + ПараметрыВыгрузки["ПР00001"
				+ Формат(НомСтр, "ЧЦ=4; ЧВН=; ЧГ=")
				+ Формат(НомКол, "ЧЦ=2; ЧВН=; ЧГ=")];
		КонецЦикла;
		Если Не (ПараметрыВыгрузки["ПР00001" + Формат(НомСтр, "ЧЦ=4; ЧВН=; ЧГ=") + "01"] = ТекСумма) Тогда
			РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма,
				"Не выполнено условие контроля: в стр. " + Формат(НомСтр, "ЧЦ=2; ЧВН=; ЧГ=") + " раздела 1 данные по гр. 1 должны быть равны сумме данных по графам с 6 по 12!",
				ИмяПоляТаблДокумента, "П000001" + Формат(НомСтр, "ЧЦ=4; ЧВН=; ЧГ=") + "01");
			СтатусПроверки = Ложь;
		КонецЕсли;
	КонецЦикла;
	...
	Для НомКол = 1 По 12 Цикл
		...
		// Стр. 02 ≥ стр. 09.
		Если Не (ПараметрыВыгрузки["ПР000010002" + Формат(НомКол, "ЧЦ=2; ЧВН=; ЧГ=")]
			  >= ПараметрыВыгрузки["ПР000010009" + Формат(НомКол, "ЧЦ=2; ЧВН=; ЧГ=")]) Тогда
			РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма,
				"Не выполнено условие контроля: по графе " + Формат(НомКол, "ЧГ=")
				+ " раздела 1 данные в стр. 02 должны быть больше или равны данным в стр. 09!",
				ИмяПоляТаблДокумента, "П0000010002" + Формат(НомКол, "ЧЦ=2; ЧВН=; ЧГ="));
			СтатусПроверки = Ложь;
		КонецЕсли;
		
		// Стр. 02 ≤ суммы стр. 03, 09.
		Если Не (ПараметрыВыгрузки["ПР000010002" + Формат(НомКол, "ЧЦ=2; ЧВН=; ЧГ=")]
			  <= ПараметрыВыгрузки["ПР000010003" + Формат(НомКол, "ЧЦ=2; ЧВН=; ЧГ=")]
			   + ПараметрыВыгрузки["ПР000010009" + Формат(НомКол, "ЧЦ=2; ЧВН=; ЧГ=")]) Тогда
			РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма,
				"Не выполнено условие контроля: по графе " + Формат(НомКол, "ЧГ=")
				+ " раздела 1 данные в стр. 02 должны быть меньше или равны сумме данных в строках 03, 09!",
				ИмяПоляТаблДокумента, "П0000010002" + Формат(НомКол, "ЧЦ=2; ЧВН=; ЧГ="));
			СтатусПроверки = Ложь;
		КонецЕсли;
		
		// Стр. 03 ≤ суммы стр. 04, 06.
		Если Не (ПараметрыВыгрузки["ПР000010003" + Формат(НомКол, "ЧЦ=2; ЧВН=; ЧГ=")]
			  <= ПараметрыВыгрузки["ПР000010004" + Формат(НомКол, "ЧЦ=2; ЧВН=; ЧГ=")]
			   + ПараметрыВыгрузки["ПР000010006" + Формат(НомКол, "ЧЦ=2; ЧВН=; ЧГ=")]) Тогда
			РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма,
				"Не выполнено условие контроля: по графе " + Формат(НомКол, "ЧГ=")
				+ " раздела 1 данные в стр. 03 должны быть меньше или равны сумме данных в строках 04, 06!",
				ИмяПоляТаблДокумента, "П0000010003" + Формат(НомКол, "ЧЦ=2; ЧВН=; ЧГ="));
			СтатусПроверки = Ложь;
		КонецЕсли;
		
		// Стр. 05 ≤ стр. 04.
		Если Не (ПараметрыВыгрузки["ПР000010005" + Формат(НомКол, "ЧЦ=2; ЧВН=; ЧГ=")]
			  <= ПараметрыВыгрузки["ПР000010004" + Формат(НомКол, "ЧЦ=2; ЧВН=; ЧГ=")]) Тогда
			РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма,
				"Не выполнено условие контроля: по графе " + Формат(НомКол, "ЧГ=")
				+ " раздела 1 данные в стр. 05 должны быть меньше или равны данным в стр. 04!",
				ИмяПоляТаблДокумента, "П0000010005" + Формат(НомКол, "ЧЦ=2; ЧВН=; ЧГ="));
			СтатусПроверки = Ложь;
		КонецЕсли;
		
		// Стр. 07 ≤ стр. 06.
		Если Не (ПараметрыВыгрузки["ПР000010007" + Формат(НомКол, "ЧЦ=2; ЧВН=; ЧГ=")]
			  <= ПараметрыВыгрузки["ПР000010006" + Формат(НомКол, "ЧЦ=2; ЧВН=; ЧГ=")]) Тогда
			РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма,
				"Не выполнено условие контроля: по графе " + Формат(НомКол, "ЧГ=")
				+ " раздела 1 данные в стр. 07 должны быть меньше или равны данным в стр. 06!",
				ИмяПоляТаблДокумента, "П0000010007" + Формат(НомКол, "ЧЦ=2; ЧВН=; ЧГ="));
			СтатусПроверки = Ложь;
		КонецЕсли;
		
		// Стр. 08 ≤ стр. 03.
		Если Не (ПараметрыВыгрузки["ПР000010008" + Формат(НомКол, "ЧЦ=2; ЧВН=; ЧГ=")]
			  <= ПараметрыВыгрузки["ПР000010003" + Формат(НомКол, "ЧЦ=2; ЧВН=; ЧГ=")]) Тогда
			РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма,
				"Не выполнено условие контроля: по графе " + Формат(НомКол, "ЧГ=")
				+ " раздела 1 данные в стр. 08 должны быть меньше или равны данным в стр. 03!",
				ИмяПоляТаблДокумента, "П0000010008" + Формат(НомКол, "ЧЦ=2; ЧВН=; ЧГ="));
			СтатусПроверки = Ложь;
		КонецЕсли;
		
		// Стр. 09 ≤ суммы строк с 10 по 12.
		Если Не (ПараметрыВыгрузки["ПР000010009" + Формат(НомКол, "ЧЦ=2; ЧВН=; ЧГ=")]
			  <= ПараметрыВыгрузки["ПР000010010" + Формат(НомКол, "ЧЦ=2; ЧВН=; ЧГ=")]
			   + ПараметрыВыгрузки["ПР000010011" + Формат(НомКол, "ЧЦ=2; ЧВН=; ЧГ=")]
			   + ПараметрыВыгрузки["ПР000010012" + Формат(НомКол, "ЧЦ=2; ЧВН=; ЧГ=")]) Тогда
			РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма,
				"Не выполнено условие контроля: по графе " + Формат(НомКол, "ЧГ=")
				+ " раздела 1 данные в стр. 09 должны быть меньше или равны сумме данных в строках с 10 по 12!",
				ИмяПоляТаблДокумента, "П0000010009" + Формат(НомКол, "ЧЦ=2; ЧВН=; ЧГ="));
			СтатусПроверки = Ложь;
		КонецЕсли;
		
		// Стр. 13 ≤ стр. 09.
		Если Не (ПараметрыВыгрузки["ПР000010013" + Формат(НомКол, "ЧЦ=2; ЧВН=; ЧГ=")]
			  <= ПараметрыВыгрузки["ПР000010009" + Формат(НомКол, "ЧЦ=2; ЧВН=; ЧГ=")]) Тогда
			РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма,
				"Не выполнено условие контроля: по графе " + Формат(НомКол, "ЧГ=")
				+ " раздела 1 данные в стр. 13 должны быть меньше или равны данным в стр. 09!",
				ИмяПоляТаблДокумента, "П0000010013" + Формат(НомКол, "ЧЦ=2; ЧВН=; ЧГ="));
			СтатусПроверки = Ложь;
		КонецЕсли;
		
		// Стр. 14 ≤ стр. 13.
		Если Не (ПараметрыВыгрузки["ПР000010014" + Формат(НомКол, "ЧЦ=2; ЧВН=; ЧГ=")]
			  <= ПараметрыВыгрузки["ПР000010013" + Формат(НомКол, "ЧЦ=2; ЧВН=; ЧГ=")]) Тогда
			РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма,
				"Не выполнено условие контроля: по графе " + Формат(НомКол, "ЧГ=")
				+ " раздела 1 данные в стр. 14 должны быть меньше или равны данным в стр. 13!",
	...
	// В Справке 1 к Разделу 1.
	
	// Стр. 19, 20, 21 < стр. 01, гр. 1, разд. 1.
	Если Не (ПараметрыВыгрузки["ПР00001001901"] < ПараметрыВыгрузки["ПР00001000101"]) Тогда
		РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма,
			"Не выполнено условие контроля: в справке 1 данные в стр. 19 должны быть меньше данных в стр. 01 по гр. 1 раздела 1!",
			ИмяПоляТаблДокумента, "П000001001901");
		СтатусПроверки = Ложь;
	КонецЕсли;
	Если Не (ПараметрыВыгрузки["ПР00001002001"] < ПараметрыВыгрузки["ПР00001000101"]) Тогда
		РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма,
			"Не выполнено условие контроля: в справке 1 данные в стр. 20 должны быть меньше данных в стр. 01 по гр. 1 раздела 1!",
			ИмяПоляТаблДокумента, "П000001002001");
		СтатусПроверки = Ложь;
	КонецЕсли;
	Если Не (ПараметрыВыгрузки["ПР00001002101"] < ПараметрыВыгрузки["ПР00001000101"]) Тогда
		РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма,
			"Не выполнено условие контроля: в справке 1 данные в стр. 21 должны быть меньше данных в стр. 01 по гр. 1 раздела 1!",
			ИмяПоляТаблДокумента, "П000001002101");
		СтатусПроверки = Ложь;
	КонецЕсли;
	
	// Стр. 22, 23, 24, 25 ≤ стр.02, гр.1, разд.1.
	Если Не (ПараметрыВыгрузки["ПР00001002201"] <= ПараметрыВыгрузки["ПР00001000201"]) Тогда
		РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма,
			"Не выполнено условие контроля: в справке 1 данные в стр. 22 должны быть меньше или равна данным в стр. 02 по гр. 1 раздела 1!",
			ИмяПоляТаблДокумента, "П000001002201");
		СтатусПроверки = Ложь;
	КонецЕсли;
	Если Не (ПараметрыВыгрузки["ПР00001002301"] <= ПараметрыВыгрузки["ПР00001000201"]) Тогда
		РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма,
			"Не выполнено условие контроля: в справке 1 данные в стр. 23 должны быть меньше или равна данным в стр. 02 по гр. 1 раздела 1!",
			ИмяПоляТаблДокумента, "П000001002301");
		СтатусПроверки = Ложь;
	КонецЕсли;
	Если Не (ПараметрыВыгрузки["ПР00001002401"] <= ПараметрыВыгрузки["ПР00001000201"]) Тогда
		РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма,
			"Не выполнено условие контроля: в справке 1 данные в стр. 24 должны быть меньше или равна данным в стр. 02 по гр. 1 раздела 1!",
			ИмяПоляТаблДокумента, "П000001002401");
		СтатусПроверки = Ложь;
	КонецЕсли;
	Если Не (ПараметрыВыгрузки["ПР00001002501"] <= ПараметрыВыгрузки["ПР00001000201"]) Тогда
		РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма,
			"Не выполнено условие контроля: в справке 1 данные в стр. 25 должны быть меньше или равна данным в стр. 02 по гр. 1 раздела 1!",
			ИмяПоляТаблДокумента, "П000001002501");
		СтатусПроверки = Ложь;
	КонецЕсли;
	
	// В Справке 2 к Разделу 1.
	
	// Сумма стр. с 26 по 28 ≥ стр.02, гр.1, разд.1.
	Если Не (ПараметрыВыгрузки["ПР00001002601"] + ПараметрыВыгрузки["ПР00001002701"] + ПараметрыВыгрузки["ПР00001002801"]
		  >= ПараметрыВыгрузки["ПР00001000201"]) Тогда
		РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма,
			"Не выполнено условие контроля: в справке 2 сумма данных в строках c 26 по 28 должна быть больше или равна данным в стр. 02 по гр. 1 раздела 1!",
			ИмяПоляТаблДокумента, "П000001000201");
		СтатусПроверки = Ложь;
	КонецЕсли;
	
	// В Справке 3 к разделу 1.
	
	// Стр. с 29 по 33 ≤ стр.03, гр. 1, разд. 1.
	Если Не (ПараметрыВыгрузки["ПР00001002901"] <= ПараметрыВыгрузки["ПР00001000301"]) Тогда
		РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма,
			"Не выполнено условие контроля: в справке 3 данные в стр. 29 должны быть меньше или равна данным в стр. 03 по гр. 1 раздела 1!",
			ИмяПоляТаблДокумента, "П000001002901");
		СтатусПроверки = Ложь;
	КонецЕсли;
	Если Не (ПараметрыВыгрузки["ПР00001003001"] <= ПараметрыВыгрузки["ПР00001000301"]) Тогда
		РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма,
			"Не выполнено условие контроля: в справке 3 данные в стр. 30 должны быть меньше или равна данным в стр. 03 по гр. 1 раздела 1!",
			ИмяПоляТаблДокумента, "П000001003001");
		СтатусПроверки = Ложь;
	КонецЕсли;
	Если Не (ПараметрыВыгрузки["ПР00001003101"] <= ПараметрыВыгрузки["ПР00001000301"]) Тогда
		РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма,
			"Не выполнено условие контроля: в справке 3 данные в стр. 31 должны быть меньше или равна данным в стр. 03 по гр. 1 раздела 1!",
			ИмяПоляТаблДокумента, "П000001003101");
		СтатусПроверки = Ложь;
	КонецЕсли;
	Если Не (ПараметрыВыгрузки["ПР00001003201"] <= ПараметрыВыгрузки["ПР00001000301"]) Тогда
		РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма,
			"Не выполнено условие контроля: в справке 3 данные в стр. 32 должны быть меньше или равна данным в стр. 03 по гр. 1 раздела 1!",
			ИмяПоляТаблДокумента, "П000001003201");
		СтатусПроверки = Ложь;
	КонецЕсли;
	Если Не (ПараметрыВыгрузки["ПР00001003301"] <= ПараметрыВыгрузки["ПР00001000301"]) Тогда
		РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма,
			"Не выполнено условие контроля: в справке 3 данные в стр. 33 должны быть меньше или равна данным в стр. 03 по гр. 1 раздела 1!",
			ИмяПоляТаблДокумента, "П000001003301");
		СтатусПроверки = Ложь;
	КонецЕсли;
	
	// Сумма стр. с 32 по 33 ≥ стр.04 гр. 1, разд. 1.
	Если Не (ПараметрыВыгрузки["ПР00001003201"] + ПараметрыВыгрузки["ПР00001003301"]
		  >= ПараметрыВыгрузки["ПР00001000401"]) Тогда
		РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма,
			"Не выполнено условие контроля: в справке 3 сумма данных в строках c 32 по 33 должна быть больше или равна данным в стр. 04 по гр. 1 раздела 1!",
			ИмяПоляТаблДокумента, "П000001000401");
		СтатусПроверки = Ложь;
	КонецЕсли;
	
	// Сумма стр. с 30 по 31 ≥ стр. 06, гр. 1, разд. 1.
	Если Не (ПараметрыВыгрузки["ПР00001003001"] + ПараметрыВыгрузки["ПР00001003101"]
		  >= ПараметрыВыгрузки["ПР00001000601"]) Тогда
		РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма,
			"Не выполнено условие контроля: в справке 3 сумма данных в строках c 30 по 31 должна быть больше или равна данным в стр. 06 по гр. 1 раздела 1!",
			ИмяПоляТаблДокумента, "П000001000601");
		СтатусПроверки = Ложь;
	КонецЕсли;
