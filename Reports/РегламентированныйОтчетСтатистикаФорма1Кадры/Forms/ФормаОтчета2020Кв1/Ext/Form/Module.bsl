...
Процедура ПоказатьПериод()
	...
	ПолеТабличногоДокументаФормаОтчета.Области.ПериодОтчета.Значение = "ОБУЧЕНИИ) И ДОПОЛНИТЕЛЬНОМ ОБРАЗОВАНИИ РАБОТНИКОВ ОРГАНИЗАЦИЙ в "
...
Процедура Расчет(Форма, ИмяСтраницыПанели = "", ОбластьИмя = "")
	...
	Для Гр = 2 по 12 Цикл 
		...
		Области["П0113" + ГрСтр].Значение = Области["П0114" + ГрСтр].Значение
			+ Области["П0115" + ГрСтр].Значение + Области["П0116" + ГрСтр].Значение;
	КонецЦикла;
	
	Для Стр = 1 По 18 Цикл
...
Процедура УстановитьЗащитуИЦветФонаОбластейАвторассчитываемыхПоказателей()
	...
	Для Гр = 2 по 12 Цикл
		ГрСтр = Формат(Гр, "ЧЦ=2; ЧВН=");
		мФормаОтчета.Области["П0113" + ГрСтр].ЦветФона = ЦветФонаЗначение;
		мФормаОтчета.Области["П0113" + ГрСтр].Защита   = Не ОтклАвтоРасчетЗначение;
	КонецЦикла;
	
	Для Стр = 1 По 18 Цикл
		СтрСтр = Формат(Стр, "ЧЦ=2; ЧВН=");
		мФормаОтчета.Области["П01"+СтрСтр+"01"].ЦветФона = ЦветФонаЗначение;
		мФормаОтчета.Области["П01"+СтрСтр+"01"].Защита   = Не ОтклАвтоРасчетЗначение;
	КонецЦикла;
