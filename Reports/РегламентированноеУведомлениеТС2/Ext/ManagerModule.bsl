...
Процедура ЗаполнитьДаннымиТС2(ДокОбъект, Выборка)
	...
	Исключение
		...
		СтрОш = НСтр("ru = 'Не удалось получить ФИО подписанта'", ОбщегоНазначения.КодОсновногоЯзыка());
...
Процедура КонвертацияИзРегламентированногоОтчета(Выборка) Экспорт 
	Попытка
	...
	Исключение
		Ошибка = ИнформацияОбОшибке();
		СтрОш = НСтр("ru = 'Создание уведомления ТС-2'", ОбщегоНазначения.КодОсновногоЯзыка());
