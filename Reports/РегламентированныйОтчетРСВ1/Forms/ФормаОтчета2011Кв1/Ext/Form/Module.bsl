...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	СтруктураДанныхРаздел44    = Новый Структура;
...
Процедура ЗаполнитьСтруктуруЗависимыхЯчеекДляАвторасчета()
	...
	// Раздел 4.4.
	СтруктураЗависимыхЯчеекДляАвторасчета.Вставить("П000440043703", "П000440043503;П000440043603");
...
Процедура Расчет(Форма, ИмяТекущейСтраницыПанели = "", ОбластьИмя = "")
	...
	ИначеЕсли ИмяТекущейСтраницыПанели = "Раздел44" Тогда
		
    	Если НЕ ЗначениеЗаполнено(ОбластьИмя) ИЛИ СтрЧислоВхождений(Форма.СтруктураЗависимыхЯчеекДляАвторасчета.П000440043703, ОбластьИмя) > 0 Тогда
			П000440043503 = ОкруглитьЧислоПоФормату(Форма, Форма.ТабличныйДокумент.Области.П000440043503.Значение);
		    П000440043603 = ОкруглитьЧислоПоФормату(Форма, Форма.ТабличныйДокумент.Области.П000440043603.Значение);
    		Форма.ТабличныйДокумент.Области.П000440043703.Значение = ?(П000440043503 <> 0, (П000440043603 / П000440043503) * 100, 0);
		КонецЕсли;
...
Процедура РасчетВСтруктуреВсехРазделов(Форма, ИмяТекущейСтраницыПанели="", ОбластьИмя = "", НомерСтраницы = 0)
	...
	ИначеЕсли ИмяТекущейСтраницыПанели = "Раздел44" Тогда
		...
		Если НЕ ЗначениеЗаполнено(ОбластьИмя) ИЛИ СтрЧислоВхождений(Форма.СтруктураЗависимыхЯчеекДляАвторасчета.П000440043703, ОбластьИмя) > 0 Тогда
			П000440043503 = ОкруглитьЧислоПоФормату(Форма, Форма.СтруктураДанныхРаздел44.П000440043503);
			П000440043603 = ОкруглитьЧислоПоФормату(Форма, Форма.СтруктураДанныхРаздел44.П000440043603);
			Форма.СтруктураДанныхРаздел44.П000440043703 = ?(П000440043503 <> 0, (П000440043603 / П000440043503) * 100, 0);
		КонецЕсли;
	...
	ИначеЕсли ИмяТекущейСтраницыПанели = "" Тогда
		...
		РасчетВСтруктуреВсехРазделов(Форма, "Раздел44");
...
Процедура ОчиститьНаСервере()
	...
	СтруктураДанныхРаздел44.Очистить();
...
Функция СформироватьСтруктуруПараметров()
	...
	ПараметрыВыгрузки.Вставить("РегистрационныйНомерТФОМС", СокрЛП(Титульный.НомТФОМС));
...
Функция ИзвлечьСтруктуруXML()
	...
	Макет = ОбъектОтчета(ЭтаФорма.ИмяФормы).ПолучитьМакет("СхемаВыгрузки700_2011");
...
Процедура ЗаполнитьСодержательнуюЧасть(ПараметрыВыгрузки, УзелРодитель)
	...
	Раздел44  = мДанныеОтчета.ПолеТабличногоДокументаРаздел44;
	...
	УзелРасчетСоответствияУсловийНаПравоПримененияПонижТарифаОтдКатегориямиПлательщиковУСН = ПолучитьПодчиненныйЭлемент(УзелОснованияДляПримененияПониженногоТарифа, "РасчетСоответствияУсловийНаПравоПримененияПонижТарифаОтдКатегориямиПлательщиковУСН");
	ЗаполнитьДаннымиУзел(Раздел44, УзелРасчетСоответствияУсловийНаПравоПримененияПонижТарифаОтдКатегориямиПлательщиковУСН);
...
Процедура ВывестиНаПечатьЛист(Парам1, ОбщиеРеквизитыРазделов, СтрПоля = Неопределено)
	...
	СоответствиеРазделовМакетам.Вставить("Раздел12",    "ПечатнаяФормаОтчета2011Кв1_Раздел12");
	СоответствиеРазделовМакетам.Вставить("Раздел30",    "ПечатнаяФормаОтчета2011Кв1_Раздел30");
	СоответствиеРазделовМакетам.Вставить("Раздел41",    "ПечатнаяФормаОтчета2011Кв1_Раздел41");
	СоответствиеРазделовМакетам.Вставить("Раздел42",    "ПечатнаяФормаОтчета2011Кв1_Раздел42");
	СоответствиеРазделовМакетам.Вставить("Раздел43",    "ПечатнаяФормаОтчета2011Кв1_Раздел43");
	СоответствиеРазделовМакетам.Вставить("Раздел44",    "ПечатнаяФормаОтчета2011Кв1_Раздел44");
	СоответствиеРазделовМакетам.Вставить("Раздел50",    "ПечатнаяФормаОтчета2011Кв1_Раздел50");
...
Процедура ЗаполнитьАвтоНаСервере()
	...
	Контейнер.Вставить("Раздел44",  СтруктураДанныхРаздел44);
