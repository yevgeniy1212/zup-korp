...
Процедура ВывестиШапкуТ54(СтрокаСотрудника, ДокументРезультат, Макет)
	...
	Если НачалоГода(СтрокаСотрудника.СотрудникДатаУвольнения) > СтрокаСотрудника.ГодНачисления Тогда
		МакетШапки.Параметры.СотрудникДатаУвольнения = '00010101';
	КонецЕсли;
...
Процедура ВывестиШапкаПоПериодамТ54(СтрокаСотрудника, ДокументРезультат, Макет, ДополнительныеДанные)
	...
	Для каждого СтрокаТаблицы Из ДополнительныеДанные.ДанныеНачислений.ПорядокДопНачислений Цикл
		...
		Если (СтрокаТаблицы.НомерКолонки - ?(ЕстьБольничный, 1, 0)) >= 6 Тогда
			Продолжить;
		КонецЕсли;
...
Процедура ВывестиСотрудниковПодразделенияТ54а(СтрокиСотрудников, ДокументРезультат, Макет, ДополнительныеДанные, ДатаОтчета, УкомплектоватьРасчетнымиЛистами)
	...
	Для каждого СтрокаСотрудника Из СтрокиСотрудников Цикл
		...
		Если УкомплектоватьРасчетнымиЛистами Тогда
			...
			РасчетныйЛисток = ЗарплатаКадрыОтчеты.ДанныеРасчетныхЛистков(
	...
	Если МаксимальноеКоличествоСтрок > 0 Тогда
		...
		Для НомерСтроки = 0 По МаксимальноеКоличествоСтрок - 1 Цикл
			...
			Если НастройкиПечатныхФорм.УдалятьПрефиксыОрганизацииИИБИзНомеровКадровыхПриказов Тогда
				...
				Если Не СтрНачинаетсяС(МакетСтроки.Параметры.КадровыйПриказНомер, НСтр("ru = 'Тр. дог. №'"))
					И Не СтрНачинаетсяС(МакетСтроки.Параметры.КадровыйПриказНомер, НСтр("ru = 'Контр. №'")) Тогда
...
Функция КадроваяИстория(КлючВарианта, ПараметрыОтчетов)
	...
	Если СтрокиБезРеквизитов.Количество() > 0 Тогда
		...
		ПринятыеНаРаботуПоДоговорам = ДанныеРеестра.СкопироватьКолонки();
		...
		Для Каждого СтрокаБезРеквизитов Из СтрокиБезРеквизитов Цикл
			...
			Если РеквизитыРегистратора <> Неопределено Тогда
				...
				Если РеквизитыРегистратора.ПриемТрудовымДоговором = Истина Тогда
					ЗаполнитьЗначенияСвойств(ПринятыеНаРаботуПоДоговорам.Добавить(), СтрокаБезРеквизитов);
				Иначе
		...
		Если ПринятыеНаРаботуПоДоговорам.Количество() > 0 Тогда
			...
			Для Каждого СтрокаБезРеквизитов Из ПринятыеНаРаботуПоДоговорам Цикл
				
				ДанныеСотрудника = КадровыеДанныеСотрудниковПринятыхНаРаботуПоДоговорам.Найти(СтрокаБезРеквизитов.Сотрудник, "Сотрудник");
				Если ДанныеСотрудника <> Неопределено Тогда
					СтрокиСотрудника = ДанныеРеестра.НайтиСтроки(Новый Структура("НомерПриказа,Сотрудник,Регистратор",
						Неопределено, СтрокаБезРеквизитов.Сотрудник, СтрокаБезРеквизитов.Регистратор));
					Для Каждого СтрокаБезРеквизитов Из СтрокиСотрудника Цикл
						Если Не ЗначениеЗаполнено(ДанныеСотрудника.ПриказОПриемеВидДоговора)
							...
							СтрокаБезРеквизитов.НомерПриказа = СтрШаблон(НСтр("ru = 'Тр. дог. №%1'"), ДанныеСотрудника.ПриказОПриемеТрудовойДоговорНомер);
						Иначе
							СтрокаБезРеквизитов.НомерПриказа = СтрШаблон(НСтр("ru = 'Контр. №%1'"), ДанныеСотрудника.ПриказОПриемеТрудовойДоговорНомер);
						КонецЕсли;
						СтрокаБезРеквизитов.ДатаПриказа = ДанныеСотрудника.ПриказОПриемеТрудовойДоговорДата;
					КонецЦикла;
				КонецЕсли;
...
Функция ФактическиеОтпуска(КлючВарианта, ПараметрыОтчетов)
	...
	Запрос.Текст =
		...
		|	ЕСТЬNULL(ОтпускаСотрудников.Дата, РеестрОтпусков.ИсходныйДокумент.Дата) КАК ДатаПриказа,
		|	ЕСТЬNULL(ОтпускаСотрудников.Номер, РеестрОтпусков.ИсходныйДокумент.Номер) КАК НомерПриказа
		|ИЗ
