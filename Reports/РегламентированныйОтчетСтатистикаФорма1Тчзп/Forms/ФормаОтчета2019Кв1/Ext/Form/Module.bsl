...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	// Версия формы.
	СтруктураРеквизитовФормы.мВерсияФормы = "17/07/2019";
...
Процедура ПолеТабличногоДокументаФормаОтчетаВыбор(Элемент, Область, СтандартнаяОбработка)
	...
	Иначе
		Если Лев(Область.Имя, 3) = "П01" И (Прав(Область.Имя, 2) = "02" Или Прав(Область.Имя, 2) = "04") Тогда
...
Процедура Расчет(Форма, ИмяСтраницыПанели = "", ОбластьИмя = "")
	...
	Для Стр = 2 По 10 Цикл 
		Пфкс = "П01" + Формат(Стр, "ЧЦ=2; ЧВН=") + "0";
		Области[Пфкс + "6"].Значение = Области[Пфкс + "7"].Значение + Области[Пфкс + "8"].Значение + Области[Пфкс + "9"].Значение;
	КонецЦикла;
	...
	Для Гр = 5 По 10 Цикл 
		...
		Тот = 0;
		Для Стр = 2 По 10 Цикл 
			Тот = Тот + Области["П01" + Формат(Стр, "ЧЦ=2; ЧВН=") + ГрСтр].Значение;
		КонецЦикла;
		Области["П0101" + ГрСтр].Значение = Тот;
	КонецЦикла;
КонецПроцедуры
...
Процедура ВвестиПоказатель(Элемент, Область, КодПоказателя, Знач ЭтоСумма, Знач ТекстВыбора, ТаблицаВыбора, СтандартнаяОбработка, КолонкаПоиска = "Код") Экспорт
	...
	ИсходноеЗначение = ПолеТабличногоДокументаФормаОтчета.Области[Лев(Область.Имя, 5) + "04"].Значение;
...
Процедура ВвестиПоказательЗавершение(РезультатВыбора, ДополнительныеПараметры) Экспорт
	...
	ПолеТабличногоДокументаФормаОтчета.Области[Лев(Область.Имя, 5) + "02"].Значение = ?(ВыборКод <> "", ВыборНазв, "");
	ПолеТабличногоДокументаФормаОтчета.Области[Лев(Область.Имя, 5) + "04"].Значение = ВыборКод;
...
Процедура ДополнительныеПоказатели1Т(ТекСтруктура)
	...
	Для НомСтр = 2 По 10 Цикл
		ТекСтруктура.Свойство("П01" + Формат(НомСтр, "ЧЦ=2; ЧВН=; ЧГ=") + "04", ТекЗначение);
		...
		Для Инд = 5 По 10 Цикл 
			ТекСтруктура.Свойство("П01" + Формат(НомСтр, "ЧЦ=2; ЧВН=; ЧГ=") + Формат(Инд, "ЧЦ=2; ЧВН="), ТекЗначение);
...
Процедура ЗагрузитьПодготовленныеДанные()
	...
	ТекТабличноеПоле.Вывести(ОбъектОтчета(ЭтаФорма.ИмяФормы).ПолучитьМакет("ФормаОтчета2019Кв1_ФормаОтчета"));
...
Процедура Расшифровать(Команда)
	...
	Если СтрЧислоВхождений(Элементы.ПолеТабличногоДокументаФормаОтчета.ТекущаяОбласть.Имя, "П0001") > 0
	 ИЛИ СтрЧислоВхождений(Элементы.ПолеТабличногоДокументаФормаОтчета.ТекущаяОбласть.Имя, "П01") > 0 Тогда
		...
		ЯчейкаОКВЭД2 = "П01" + Лев(Прав(Элементы.ПолеТабличногоДокументаФормаОтчета.ТекущаяОбласть.Имя, 4), 2) + "04";
