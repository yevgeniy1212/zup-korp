...
Процедура ЗаполнитьЗначенияПоУмолчанию(ИмяФормы, СтруктураРеквизитовФормы, РазделыОтчета, ДатаПодписи) Экспорт
	...
	"НекоммерческаяОрганизация,ОснВидДеят2,ОКВЭД2,ДатаРегистрации,ОГРН,НаименованиеРегистрирующегоОргана");
	...
	Если ОргСведения.Свойство("НекоммерческаяОрганизация") Тогда
		...
		Если ТипЗнч(ОргСведения.НекоммерческаяОрганизация) = Тип("Булево") Тогда
			
			НекоммерческаяОрганизация = ?(ОргСведения.НекоммерческаяОрганизация, "Х", "");
	...
	Если ОргСведения.Свойство("НаименованиеРегистрирующегоОргана") Тогда
		...
		Если ЗначениеЗаполнено(ОргСведения.НаименованиеРегистрирующегоОргана) Тогда
			
			МестоРегистрации = СокрЛП(ОргСведения.НаименованиеРегистрирующегоОргана);
			
			ДатаМестоНомерРегистрации = ?(ЗначениеЗаполнено(ДатаМестоНомерРегистрации),
			ДатаМестоНомерРегистрации + ", " + МестоРегистрации, МестоРегистрации);
			
		КонецЕсли;
		
	КонецЕсли;
	
	РазделыОтчета.Приложение1.Данные.Вставить("НекоммерческаяОрганизация", НекоммерческаяОрганизация);
	...
	РазделыОтчета.Приложение2.Данные.Вставить("МестоРегистрации", МестоРегистрации);
...
Процедура ЗаполнитьСведенияОбОрганизации(ИмяФормы,
	...
	Если НЕ ВПрограммеИзмененаДатаПодписи Тогда
		...
		Если НЕ ЗначениеЗаполнено(РегНомПФР) Тогда
			...
			РегНомПФР = СокрЛП(СтрЗаменить(РегНомПФР, "-", ""));
		КонецЕсли;
	...
	Если ТипЗнч(СтруктураДанныхПриложение) = Тип("ТабличныйДокумент") Тогда
		...
		Если НЕ СтруктураДанныхПриложение.Области.Найти("РегистрационныйНомерФСС") = Неопределено Тогда 
			СтруктураДанныхПриложение.Области.РегистрационныйНомерФСС.Значение = РегНомПФР;
		КонецЕсли;
	...
	ИначеЕсли ТипЗнч(СтруктураДанныхПриложение) = Тип("Структура") Тогда
		...
		Если СтруктураДанныхПриложение.Свойство("РегистрационныйНомерФСС") Тогда
			СтруктураДанныхПриложение.РегистрационныйНомерФСС = РегНомПФР;
		КонецЕсли;
...
Функция ВыгрузкаВозможна(ИмяФормы,
	...
	Если НЕ ЗначениеЗаполнено(ПараметрыВыгрузки.РегНомер) Тогда
		...
		СтатусПроверки = Ложь;
	ИначеЕсли (СтрДлина(ПараметрыВыгрузки.РегНомер) <> 12 И СтрДлина(ПараметрыВыгрузки.РегНомер) <> 10)
		ИЛИ НЕ СтроковыеФункцииКлиентсервер.ТолькоЦифрыВСтроке(ПараметрыВыгрузки.РегНомер) Тогда
		РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(РезультатВыгрузки, НСтр(
		"ru='Задан регистрационный номер страхователя, не состоящий из 12 (рег. номер ПФР) или 10 (рег. номер ФСС) цифр'"),
		"Приложение1", "РегистрационныйНомерФСС");
		СтатусПроверки = Ложь;
	КонецЕсли;
	
	Если НЕ ЗначениеЗаполнено(ПараметрыВыгрузки.КодТеррОргана) Тогда
		РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(РезультатВыгрузки, НСтр(
		"ru='Не задан код территориального органа Фонда пенсионного и социального страхования Российской Федерации'"),
		"Приложение1", "КодПодчиненности");
		СтатусПроверки = Ложь;
	ИначеЕсли СтрДлина(СокрЛП(ПараметрыВыгрузки.КодТеррОргана)) <> 3 Тогда
		РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(РезультатВыгрузки, НСтр(
		"ru='Код территориального органа Фонда пенсионного и социального страхования Российской Федерации"
		+ " должен состоять из 3 символов'"), "Приложение1", "КодПодчиненности");
		СтатусПроверки = Ложь;
	КонецЕсли;
	...
	Если ЗначениеЗаполнено(ПараметрыВыгрузки.ОГРН) Тогда
...
Функция СформироватьСтруктуруПараметров(ИмяФормы,
	...
	СтрокаСведений = "ИННЮЛ, КППЮЛ, ДатаРегистрации, ФИОРук, ФИОБух, АдрЮР";
	...
	СтруктураПараметров = Новый Структура("Организация, ДатаЗначения, СведенияОбОрганизации",
	СтруктураРеквизитовФормы.мСохраненныйДок.Организация, СтруктураРеквизитовФормы.мСохраненныйДок.ДатаПодписи,
	...
	ПараметрыВыгрузки.Вставить("РегНомер", ?(Приложение1.Свойство("РегистрационныйНомерФСС")
	И ЗначениеЗаполнено(Приложение1["РегистрационныйНомерФСС"]), СокрЛП(Приложение1["РегистрационныйНомерФСС"]), ""));
	ПараметрыВыгрузки.Вставить("КодТеррОргана", ?(Приложение1.Свойство("КодПодчиненности")
	И ЗначениеЗаполнено(Приложение1["КодПодчиненности"]), СокрЛП(Приложение1["КодПодчиненности"]), ""));
	...
	Если Приложение2.Свойство("СреднесписочнаяЧисленность") Тогда
		...
		ИначеЕсли ТипЗнЧ(Приложение2.СреднесписочнаяЧисленность) = Тип("Число") Тогда
			СреднЧислПредГод = Формат(Приложение2.СреднесписочнаяЧисленность, "ЧГ=;ЧН=");
		КонецЕсли;
