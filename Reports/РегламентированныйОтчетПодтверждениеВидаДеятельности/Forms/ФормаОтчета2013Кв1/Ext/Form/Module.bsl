...
Процедура Инициализация(БезОткрытияФормы = Ложь) Экспорт
	...
	Если СтруктураРеквизитовФормы.мСохраненныйДок = Неопределено
		...
		ТабличныйДокумент.Область("ПечататьСтроку2").Значение = РазделыРеглОтчета.Приложение1.Данные["ПечататьСтроку2"];
		
		ТабличныйДокумент.Область("НекоммерческаяОрганизация").Значение
		= РазделыРеглОтчета.Приложение1.Данные["НекоммерческаяОрганизация"];
...
Процедура СформироватьСпискиВыбора()
	...
	МакетСоставаПоказателей = ОбъектОтчета(ЭтаФорма.ИмяФормы).ПолучитьМакет("СпискиВыбора2013Кв1");
...
Процедура ВывестиМакетВТаблДокНаСервере(ИмяТекРаздела, НомерСтраницыРазделаОтчета)
	...
	Если ИмяТекРаздела = "Приложение2" Тогда
		...
		УстановитьВидимость_СекцияРегистрирующийОрган();
	КонецЕсли;
...
Процедура ОбработкаВыбораВПолеТабличногоДокумента(Элемент, Область, СтандартнаяОбработка)
	...
	ИначеЕсли СтрЧислоВхождений(Область.Имя, "УдалитьСтроку") > 0 И Область.Гиперссылка Тогда
		УдалитьСтроку("");
	ИначеЕсли Область.Имя = "РегистрирующийОрган" Тогда
		ЗаполнитьРегистрирующийОрган();
	КонецЕсли;
...
Процедура ОбработкаИзмененияСодержимогоВПолеТабличногоДокумента(Элемент, Область)
	...
	ИначеЕсли СтруктураРеквизитовФормы.НаимТекущегоРаздела = "Приложение2" Тогда
		...
		Если СтрНайти(ИмяПоказателя, "П0000200001") = 1 Тогда
			УстановитьВидДеятельностиВПриложениях(ЭтаФорма);
		ИначеЕсли ИмяПоказателя = "МестоРегистрации" Тогда
			УстановитьВидимость_СекцияРегистрирующийОрган();
		КонецЕсли;
	КонецЕсли;
...
&НаСервере
Процедура УстановитьВидимость_СекцияРегистрирующийОрган()
	
	СекцияРегистрирующийОрган = ТабличныйДокумент.Области.Найти("СекцияРегистрирующийОрган");
	
	Если СекцияРегистрирующийОрган <> Неопределено Тогда
		
		ВидимостьСекции = Ложь;
		
		Если НЕ ЗначениеЗаполнено(ТабличныйДокумент.Области.Найти("МестоРегистрации").Значение) Тогда
			...
			"Сервис_УказываетсяНаименованиеРегистрирующегоОргана,НаименованиеРегистрирующегоОргана");
			
			Если ОргСведения.Свойство("Сервис_УказываетсяНаименованиеРегистрирующегоОргана")
				И ТипЗнч(ОргСведения.Сервис_УказываетсяНаименованиеРегистрирующегоОргана) = Тип("Булево")
				И ОргСведения.Сервис_УказываетсяНаименованиеРегистрирующегоОргана
				И ОргСведения.Свойство("НаименованиеРегистрирующегоОргана")
				И НЕ ЗначениеЗаполнено(ОргСведения.НаименованиеРегистрирующегоОргана) Тогда
				
				ВидимостьСекции = Истина;
				
			КонецЕсли;
			
		КонецЕсли;
		
		СекцияРегистрирующийОрган.Видимость = ВидимостьСекции;
...
&НаКлиенте
Процедура ЗаполнитьРегистрирующийОрган()
	
	Оп = Новый ОписаниеОповещения("ЗаполнитьРегистрирующийОрганЗавершение", ЭтотОбъект);
	РегламентированнаяОтчетностьКлиент.ОткрытьФормуДляЗаполненияРегистрирующегоОргана(
	СтруктураРеквизитовФормы.Организация, Оп);
	
КонецПроцедуры

&НаКлиенте
Процедура ЗаполнитьРегистрирующийОрганЗавершение(РезультатЗаполнения, ДопПараметры) Экспорт
	
	ТекстСообщения = "";
	ЗаполнитьРегистрирующийОрганНаСервере(ТекстСообщения);
...
&НаСервере
Процедура ЗаполнитьРегистрирующийОрганНаСервере(ТекстСообщения = "")
	
	ОргСведения = РегламентированнаяОтчетностьВызовСервера.ПолучитьСведенияОбОрганизации(
	СтруктураРеквизитовФормы.Организация, ДатаПодписи, "НаименованиеРегистрирующегоОргана");
	
	МестоРегистрации = "";
	Если ОргСведения.Свойство("НаименованиеРегистрирующегоОргана") Тогда
		МестоРегистрации = СокрЛП(ОргСведения.НаименованиеРегистрирующегоОргана);
	КонецЕсли;
	
	Если СтруктураРеквизитовФормы.НаимТекущегоРаздела = "Приложение2" Тогда
		ТабличныйДокумент.Области["МестоРегистрации"].Значение = МестоРегистрации;
		УстановитьВидимость_СекцияРегистрирующийОрган();
	Иначе
		СтруктураДанныхПриложение2.Вставить("МестоРегистрации", МестоРегистрации);
	КонецЕсли;
	
	Если МестоРегистрации = "" Тогда
		ТекстСообщения = НСтр("ru='В реквизитах организации не найдены сведения о регистрирующем органе'");
	КонецЕсли;
