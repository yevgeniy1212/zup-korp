...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Параметры.Свойство("ПараметрыЗаполнения") Тогда 
		ЗагрузитьПараметрыЗаполнения(Параметры.ПараметрыЗаполнения);
	КонецЕсли;
...
Процедура ЗагрузитьДанные()
	...
	НоваяСтрока.КОД_НО = КодНалоговогоОргана();
...
Процедура ЗаполнитьТитульный(НовыйЛист)
	...
	Объект.ДатаПодписи = ?(ЗначениеЗаполнено(Объект.ДатаПодписи), Объект.ДатаПодписи, ТекущаяДатаСеанса());
	...
	НовыйЛист.ДАТА_ПОДПИСИ = Объект.ДатаПодписи;
...
Процедура СформироватьДерево()
	...
	Стр2.Наименование = "Сведения о"+Символы.ПС+"виде деятельности";
	...
	Листы3.Наименование = "Сведения о"+Символы.ПС+"транспортных средствах";
...
Процедура СформироватьМакетНаСервере()
	Документы.УведомлениеОСпецрежимахНалогообложения.СформироватьМакетОтчетаНаСервере(ЭтотОбъект, Объект.ИмяОтчета, "Форма2016_1", ПолучитьИмяОбласти(ТекущийТипСтраницы), ПолучитьИмяТаблицы(ТекущийТипСтраницы));
...
Процедура ОбработкаКодаНОЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	Если Результат <> Неопределено Тогда 
		...
		КОД_НО = КодНалоговогоОргана();
		ПредставлениеУведомления.Области[Инфо.Имя].Значение = КОД_НО;
		ТитульнаяСтраница[0].КОД_НО = КОД_НО;
		Модифицированность = Истина;
	КонецЕсли;
...
&НаСервере
Процедура ЗагрузитьПараметрыЗаполнения(ПараметрыЗаполнения)
	Если ТипЗнч(ПараметрыЗаполнения) = Тип("Структура") 
		И ПараметрыЗаполнения.Свойство("ПризнакПодписанта")  Тогда 
		
		ТитСтр = ТитульнаяСтраница[0];
		ПодписантПризнак = Неопределено;
		ПараметрыЗаполнения.Свойство("ПризнакПодписанта", ПодписантПризнак);
		ПодписантПризнак = Строка(ПодписантПризнак);
		ТитСтр.ПРИЗНАК_НП_ПОДВАЛ = ПодписантПризнак;
		Объект.ПодписантПризнак = ПодписантПризнак;
		ПараметрыЗаполнения.Свойство("НомерТелефонаПодписанта", ТитСтр.ТЕЛЕФОН);
		ПараметрыЗаполнения.Свойство("НомерТелефонаПодписанта", Объект.ПодписантТелефон);
		ПараметрыЗаполнения.Свойство("ФамилияПодписанта", Объект.ПодписантФамилия);
		ПараметрыЗаполнения.Свойство("ИмяПодписанта", Объект.ПодписантИмя);
		ПараметрыЗаполнения.Свойство("ОтчествоПодписанта", Объект.ПодписантОтчество);
		ТитСтр.ФИО_РУКОВОДИТЕЛЯ_ПРЕДСТАВИТЕЛЯ = СокрЛП(Объект.ПодписантФамилия + " " + Объект.ПодписантИмя + " " + Объект.ПодписантОтчество);
		Если ПараметрыЗаполнения.Свойство("ДатаПодписи") Тогда 
			Объект.ДатаПодписи = ПараметрыЗаполнения.ДатаПодписи;
			ТитСтр.ДАТА_ПОДПИСИ = ПараметрыЗаполнения.ДатаПодписи;
		КонецЕсли;
...
Процедура УстановитьДоступностьКнопок()
	Если Элементы.РазделыЗаявления.ТекущиеДанные = Неопределено Тогда 
		Возврат;
	КонецЕсли;
...
Процедура УдалитьСтраницу() Экспорт
	...
	УстановитьТекущуюСтрокуВДеревеРазделов(UID_новый);
	Модифицированность = Истина;
КонецПроцедуры

&НаКлиенте
Процедура УстановитьТекущуюСтрокуВДеревеРазделов(НовыйUID) Экспорт
	Если НовыйUID <> Неопределено Тогда
		НомСтр = РегламентированнаяОтчетностьКлиентСервер.НайтиИДВДереве(Разделы.ПолучитьЭлементы(), НовыйUID, UID_Пустой);
		Если НомСтр <> Неопределено Тогда
			Элементы.РазделыЗаявления.ТекущаяСтрока = НомСтр;
		КонецЕсли;
	КонецЕсли;
КонецПроцедуры
...
Функция СформироватьВыгрузкуИПолучитьДанные() Экспорт 
	...
	СтруктураВыгрузки.Вставить("Данные", УведомлениеОСпецрежимахНалогообложения.ПолучитьМакетДвоичныхДанных(Объект.ИмяОтчета, "TIFF_2016_1"));
