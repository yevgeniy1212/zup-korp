	...
	Если Элемент.ТекущиеДанные <> Неопределено Тогда
		...
		Иначе
			...
			Если НаборЗаписей.Количество() > 1 Тогда
				...
				ПоследняяЗапись = НаборЗаписей[НаборЗаписей.Количество() - 2];
				Если Не ПоследняяЗапись.Прерван
					 И ПоследняяЗапись.ДатаОтсчета > ЗарплатаКадрыКлиентСервер.ДатаОтсчетаПериодическихСведений() Тогда
					
					ДанныеСтажа = ЗарплатаКадрыРасширенныйКлиентСервер.СведенияОСтаже();
					
					ДанныеСтажа.ДатаОтсчета = ПоследняяЗапись.ДатаОтсчета;
					ДанныеСтажа.ИсчисляетсяСДатыПриема = ПоследняяЗапись.ИсчисляетсяСДатыПриема;
					ДанныеСтажа.Лет = ПоследняяЗапись.Лет;
					ДанныеСтажа.Месяцев = ПоследняяЗапись.РазмерМесяцев;
					ДанныеСтажа.Дней = ПоследняяЗапись.РазмерДней;
					
					ПродолжительностьСтажа = ЗарплатаКадрыРасширенныйКлиентСервер.ПродолжительностьСтажа(ДанныеСтажа, Запись.ДатаОтсчета);
					
					Запись.Лет = ПродолжительностьСтажа.Лет;
					Запись.РазмерМесяцев = ПродолжительностьСтажа.Месяцев;
					Запись.РазмерДней = ПродолжительностьСтажа.Дней;
					
				Иначе
					
					Запись.Лет = ПоследняяЗапись.Лет;
					Запись.РазмерМесяцев = ПоследняяЗапись.РазмерМесяцев;
					Запись.РазмерДней = ПоследняяЗапись.РазмерДней;
	...
	Если ТекущиеДанные <> Неопределено Тогда
		...
		Если ТекущийНаборЗаписей.Количество() > 1 Тогда
			...
			Если Не ПредыдущиеСведения.Прерван
				...
				ДанныеСтажа.ДатаОтсчета = ПредыдущиеСведения.ДатаОтсчета;
				ДанныеСтажа.ИсчисляетсяСДатыПриема = ПредыдущиеСведения.ИсчисляетсяСДатыПриема;
				ДанныеСтажа.Лет = ПредыдущиеСведения.Лет;
				ДанныеСтажа.Месяцев = ПредыдущиеСведения.РазмерМесяцев;
				ДанныеСтажа.Дней = ПредыдущиеСведения.РазмерДней;
				
				ПродолжительностьСтажа = ЗарплатаКадрыРасширенныйКлиентСервер.ПродолжительностьСтажа(ДанныеСтажа, ТекущиеДанные.ДатаОтсчета);
				
				ТекущиеДанные.Лет = ПродолжительностьСтажа.Лет;
				ТекущиеДанные.РазмерМесяцев = ПродолжительностьСтажа.Месяцев;
				ТекущиеДанные.РазмерДней = ПродолжительностьСтажа.Дней;
...
Процедура ОК(Команда)
	...
	Если РезультатФормы <> Неопределено И ОписаниеОповещенияОЗакрытии = Неопределено Тогда
		Оповестить("ИзмененСтажФизическогоЛица", РезультатФормы, ЭтотОбъект);
	КонецЕсли;
...
Процедура ДополнитьФорму()
	...
	Для Каждого ВидСтажа Из ВидыСтажа Цикл 
		...
		ПолеСпособИсчисленияСтажа.СписокВыбора.Добавить(0, НСтр("ru = 'по трудовой книжке по'"));
...
Процедура УстановитьПодсказкуСтажаПоНомеруСтажа(Форма, НомерСтажа)
	...
	ИначеЕсли ТекущаяЗапись.ДатаОтсчета < ЗарплатаКадрыКлиентСервер.ДатаОтсчетаПериодическихСведений() Тогда
		
		Подсказка = НСтр("ru = 'Слишком маленькая дата отсчета'");
	...
	Иначе
		...
		Иначе
			...
			ДатаСведенийСтажа = ?(Форма.ДатаСведений < ТекущаяЗапись.ДатаОтсчета, ТекущаяЗапись.ДатаОтсчета, Форма.ДатаСведений);
			ПродолжительностьСтажа = ЗарплатаКадрыРасширенныйКлиентСервер.ПродолжительностьСтажа(ДанныеСтажа, ДатаСведенийСтажа);
			Подсказка = ПредставлениеПодсказкиСтажа(ДанныеСтажа, ДатаСведенийСтажа, ПродолжительностьСтажа, ИсчисляетсяСДатыПриема(Форма, НомерСтажа));
...
Функция ПредставлениеПодсказкиСтажа(ДанныеСтажа, ДатаСведений, ПродолжительностьСтажа, ИсчисляетсяСДатыПриема)
	...
	Иначе
		...
		ТекстПодсказки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			ТекстПодсказки,
			Формат(ДатаСведений, "ДЛФ=Д"),
			ПредставлениеСтажа,
			Формат(КонецДня(ДанныеСтажа.ДатаОтсчета) + 1, "ДЛФ=Д"));
	...
	Для Каждого ВидСтажа Из ВидыСтажа Цикл
		...
		Если Не РасширенныйВводСтажа И ЭтаФорма["Набор" + Сч].Количество() < 2 Тогда
			...
			Если ИсчисляетсяСДатыПриема(ЭтотОбъект, Сч) И Не ЗначениеЗаполнено(ЭтотОбъект["ДатаПриема" + Сч]) Тогда 
				...
				ОбщегоНазначенияКлиент.СообщитьПользователю(ТекстСообщения, , "ДатаПриема" + Сч, , Отказ);
			КонецЕсли;
			
			Если Не ИсчисляетсяСДатыПриема(ЭтотОбъект, Сч) И Не ЗначениеЗаполнено(ЭтотОбъект["ДатаОтсчета" + Сч]) Тогда 
				ТекстСообщения = НСтр("ru = 'Не указана дата сведений о стаже.'");
				ОбщегоНазначенияКлиент.СообщитьПользователю(ТекстСообщения, , "ДатаОтсчета" + Сч, , Отказ);
			КонецЕсли;
			
			Если Не ИсчисляетсяСДатыПриема(ЭтотОбъект, Сч) И ЭтотОбъект["Лет" + Сч] = 0 И ЭтотОбъект["Месяцев" + Сч] = 0 И ЭтотОбъект["Дней" + Сч] = 0 Тогда
				ТекстСообщения = НСтр("ru = 'Не задан стаж.'");
				ОбщегоНазначенияКлиент.СообщитьПользователю(ТекстСообщения, , "Дней" + Сч, , Отказ);
			КонецЕсли;
			
			Если ЗначениеЗаполнено(ЭтотОбъект["ДатаОтсчета" + Сч])
				И ЭтотОбъект["ДатаОтсчета" + Сч] < ЗарплатаКадрыКлиентСервер.ДатаОтсчетаПериодическихСведенийСПериодомМесяц() Тогда
				
				ТекстСообщения = НСтр("ru = 'Слишком маленькая дата отсчета.'");
				ОбщегоНазначенияКлиент.СообщитьПользователю(ТекстСообщения, , "ДатаОтсчета" + Сч, , Отказ);
			КонецЕсли;
		...
		Иначе
			...
			Для каждого Запись Из ЭтаФорма["Набор" + Сч] Цикл
				...
				Если Не ЗначениеЗаполнено(Запись.ДатаОтсчета) Тогда
					...
					ТекстСообщения = НСтр("ru = 'Не указана дата сведений о стаже.'");
					ОбщегоНазначенияКлиент.СообщитьПользователю(ТекстСообщения, , "Набор" + Сч +"[" + НомерЗаписи + "].ДатаОтсчета", , Отказ);
					
				ИначеЕсли Запись.ДатаОтсчета < ЗарплатаКадрыКлиентСервер.ДатаОтсчетаПериодическихСведенийСПериодомМесяц() Тогда
				...
				ИначеЕсли Не Запись.ИсчисляетсяСДатыПриема И Запись.Лет = 0 И Запись.РазмерМесяцев = 0 И Запись.РазмерДней = 0 Тогда
					
					ОбщегоНазначенияКлиент.СообщитьПользователю(
						НСтр("ru='Не задан стаж'"),
						, "Набор" + Сч +"[" + НомерЗаписи + "].Период", , Отказ);
...
Функция РезультатФормы()
	Если Модифицированность Тогда
		...
		Возврат СохранитьДанныеНаСервере();
	Иначе
...
Функция СохранитьДанныеНаСервере()
	...
	РезультатФормы.Вставить("Сотрудник", Сотрудник);
	РезультатФормы.Вставить("ФизическоеЛицо", ФизическоеЛицо);
	РезультатФормы.Вставить("ДанныеСтажей", СтажиФизическихЛиц);
