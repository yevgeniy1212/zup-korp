...
&НаКлиенте
Процедура Подключаемый_ИННОкончаниеВводаТекста(Элемент, Текст, ДанныеВыбора, Параметры, СтандартнаяОбработка)
...
Процедура НаименованиеПриИзменении(Элемент)
	...
	Если ПустаяСтрока(РегистрацияВНалоговомОргане.НаименованиеИФНС) Тогда
		...
		// ЗУПК
		НаименованиеИФНС = СтрЗаменить(НаименованиеИФНС,	НСтр("ru='КГД'"),	НСтр("ru='Комитет государственных доходов'"));
		НаименованиеИФНС = СтрЗаменить(НаименованиеИФНС,	НСтр("ru='ДГД'"),	НСтр("ru='Департамент государственных доходов'"));
		НаименованиеИФНС = СтрЗаменить(НаименованиеИФНС,	НСтр("ru='УГД'"),	НСтр("ru='Управление государственных доходов'"));
		НаименованиеИФНС = СтрЗаменить(НаименованиеИФНС,	НСтр("ru='НК'"),	НСтр("ru='Налоговый комитет'"));
		// ЗУПК
...
Процедура ЗаписатьРегистрациюВНалоговомОрганеЗавершение(Ответ, ДополнительныеПараметры) Экспорт 
	...
	Если Не Отказ Тогда
		...
		// в ЗУПК для корректного заполнения формы 200 для филиала можем указывать свою регистрацию в налоговом органе, отличную от головы, 
		// в этой указанной ситуации при записи регистрации должен перезаполниться реквизит РегистрацияВНалоговомОргане в карточке организации
		ПараметрОповещения = Новый Структура("Ссылка, Владелец", РегистрацияВНалоговомОргане.Ссылка, СтруктурнаяЕдиница);
		// ЗУПК
...
Процедура УстановитьДоступностьЭлементовФормы(Форма)
	...
	//ЗУПК
		И (ЗначениеЗаполнено(Форма.РегистрацияВНалоговомОргане.ИНН) ИЛИ Форма.ЭтоФизическоеЛицо);
	//ЗУПК		
