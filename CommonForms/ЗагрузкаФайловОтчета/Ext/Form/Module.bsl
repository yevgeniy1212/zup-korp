...
Функция ИмяТекущегоТипаПолучателя(ТипПолучателя)
	...
	Если ЗначениеЗаполнено(ТипПолучателя) Тогда
		...
		ИначеЕсли ТипПолучателя = ПредопределенноеЗначение("Перечисление.ТипыКонтролирующихОрганов.ФТС") Тогда
			Возврат "ФТС";
		ИначеЕсли ТипПолучателя = ПредопределенноеЗначение("Перечисление.ТипыКонтролирующихОрганов.УдалитьБанкРоссии") Тогда
			Возврат "УдалитьБанкРоссии";
	...
	Если ТипЗнч(ВладелецФормы) = Тип("ФормаКлиентскогоПриложения") Тогда
		Если РегламентированнаяОтчетностьЗагрузкаКлиент.ЭтоФормаРегламентированногоОтчета(ВладелецФормы) Тогда
			...
			Если МаксИндекс > 0 Тогда
				...
				Если СвойстваВидаОтчета <> Неопределено И СвойстваВидаОтчета.Свойство("ВерсияФормата")
				 И НЕ РегламентированнаяОтчетностьВызовСервера.ФормаРеглОтчетаСоответствуетФормату(
						ИсточникФормыОтчета, ДатаОкончания, СвойстваВидаОтчета.ВерсияФормата, ИмяФормыОтчета) Тогда
					
					Сообщение = Новый СообщениеПользователю;
					ШаблонСообщения = НСтр("ru ='Формат ""%1"" загружаемого электронного представления не соответствует формату выбранной формы отчета ""%2"".'");
					Сообщение.Текст = СтрШаблон(ШаблонСообщения, СвойстваВидаОтчета.ВерсияФормата, ВладелецФормы.Заголовок);
					Сообщение.Сообщить();
