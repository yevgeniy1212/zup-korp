...
Процедура ПеречитатьДанныеУчета(Команда)
	...
	ОценкаПроизводительностиКлиент.ЗамерВремени("ПересчетДанныхВФормеВводДанныхДляРасчетаСреднегоЗаработкаФСС");
...
Функция ДанныеДляРасчетаСреднего()
	...
	ДанныеДляРасчета.Вставить("СреднийЗаработокБольничныйЗаСчетРаботодателя", СреднийЗаработокБольничныйЗаСчетРаботодателя);
...
Функция РезультатРедактированияНачислений(Форма, ВыбранныеГоды = Неопределено, БольничныйЗаСчетРаботодателя = Ложь)
	...
	Для Каждого СтрокаРасшифровки Из Форма.РасшифровкаСреднегоЗаработка Цикл
		Если Форма.МесяцыРасчета.Получить(СтрокаРасшифровки.Период) = Неопределено Тогда
			...
			Продолжить;
		КонецЕсли;
		Если БольничныйЗаСчетРаботодателя И СтрокаРасшифровки.СпособУчета = ПредопределенноеЗначение("Перечисление.УчетНачисленийВСреднемЗаработкеФСС.НеУчитыватьПриОплатеБольничногоЗаСчетРаботодателя") Тогда 
			Продолжить;
		КонецЕсли;
		ОписаниеСтроки = УчетПособийСоциальногоСтрахованияРасширенныйКлиентСервер.ОписаниеСтрокиДанныхОНачисленияхСреднегоЗаработкаФСС();
...
Процедура РассчитатьСреднийЗаработок(Форма)
	...
	Если Форма.РассчитыватьСреднийЗаработокДляБольничногоЗаСчетРаботодателя Тогда 
		ПараметрыРасчета.ДанныеНачислений = РезультатРедактированияНачислений(Форма, , Истина);
		Форма.СреднийЗаработокБольничныйЗаСчетРаботодателя = УчетПособийСоциальногоСтрахованияКлиентСервер.СреднийЗаработокФСС(ПараметрыРасчета);
	КонецЕсли;
...
Процедура ИнициализироватьРеквизитыФормыПриСозданииНаСервере(Отказ)
	...
	РассчитыватьСреднийЗаработокДляБольничногоЗаСчетРаботодателя = ПолучитьФункциональнуюОпцию("РассчитыватьОтдельноСреднийЗаработокФССДляБольничногоЗаСчетРаботодателя");
	...
	УчитыватьДниПриостановленияТД = Параметры.УчитыватьДниПриостановленияТД;
	ДнейПриостановленияТДЗаГод1 = Параметры.ДнейПриостановленияТДЗаГод1;
	ДнейПриостановленияТДЗаГод2 = Параметры.ДнейПриостановленияТДЗаГод2;
	Элементы.ГруппаДнейПриостановленияТД.Видимость = УчитыватьДниПриостановленияТД
		И (ЗначениеЗаполнено(ДнейПриостановленияТДЗаГод1)
			Или ЗначениеЗаполнено(ДнейПриостановленияТДЗаГод2)
			Или ПервыйГод >= 2021
			Или ВторойГод >= 2021);
...
Процедура УстановитьЗаголовокФормы()
	...
	Элементы.ДанныеУчетаСреднегоЗаработкаГруппа.Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru = 'Данные %1'"), ЗарплатаКадры.ГоловнаяОрганизация(Организация));
...
Процедура ОткрытьФормуРасшифровки(ИмяЯчейки, ДанныеСтроки)
	...
	ПараметрыРасшифровки.Вставить("ОтображатьСпособУчета", Истина);
...
Процедура ЗаполнитьДанныеРасшифровки(ДанныеРасшифровки, Месяц)
	...
	Для Каждого СтрокаРасшифровки Из ДанныеРасшифровки Цикл 
		Если СтрокаРасшифровки.Сумма > 0 Тогда 
			ЗаполнитьЗначенияСвойств(РасшифровкаСреднегоЗаработка.Добавить(), СтрокаРасшифровки);
		КонецЕсли;
	КонецЦикла;
...
Процедура ПриИзмененииПериодаРасчетаНаСервере()
	...
	Если ПорядокРасчета = Перечисления.ПорядокРасчетаСреднегоЗаработкаФСС.Постановление2011 Тогда
		...
		ОтключаемыеГоды = ОбщегоНазначенияКлиентСервер.РазностьМассивов(ГодыВТаблице, ГодыРасчета);
		ВключаемыеГоды = ОбщегоНазначенияКлиентСервер.РазностьМассивов(ГодыРасчета, ГодыВТаблице);
		...
		Для Каждого Год Из ОтключаемыеГоды Цикл
			...
			СтрокиРасшифровки = Новый Массив;
			Для Каждого СтрокаРасшифровки Из РасшифровкаСреднегоЗаработка Цикл 
				Если Год(СтрокаРасшифровки.Период) = Год Тогда
					СтрокиРасшифровки.Добавить(СтрокаРасшифровки);
				КонецЕсли;
			КонецЦикла;
			Для Каждого СтрокаРасшифровки Из СтрокиРасшифровки Цикл 
				РасшифровкаСреднегоЗаработка.Удалить(СтрокаРасшифровки);
