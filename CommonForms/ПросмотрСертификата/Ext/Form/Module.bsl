...
Функция РазобратьСтрокуСубъекта(СтрокаДляРазбора)
	...
	Если ТипЗнч(СтрокаДляРазбора) = Тип("Структура") ИЛИ ТипЗнч(СтрокаДляРазбора) = Тип("ФиксированнаяСтруктура") Тогда
		ПодстрокаДляРазбора = КриптографияЭДКОСлужебныйВызовСервера.ПреобразоватьВСтроку(СтрокаДляРазбора);
	Иначе
