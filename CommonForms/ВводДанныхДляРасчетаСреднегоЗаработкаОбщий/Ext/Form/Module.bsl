...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
    ...
    // ЗУПК
	Если НЕ Параметры.Свойство("ПорядокУчетаРабочегоВремени") ИЛИ Параметры.ПорядокУчетаРабочегоВремени = Неопределено Тогда
		ЭтаФорма["ПорядокУчетаРабочегоВремени"] = УчетСреднегоЗаработкаРК.ПорядокУчетаРабочегоВремени(Сотрудник, ДатаНачалаСобытия, Организация);
	Иначе
		ЭтаФорма["ПорядокУчетаРабочегоВремени"] = Параметры.ПорядокУчетаРабочегоВремени;
	КонецЕсли;
    // ЗУПК
...
Функция РезультатРедактированияНачислений(Форма)
	...
	Для Каждого СтрокаРасшифровки Из РасшифровкаСреднегоЗаработка Цикл
        // ЗУПК
		Если СтрокаРасшифровки.Период < Форма.НачалоПериодаРасчета 
				ИЛИ СтрокаРасшифровки.Период > Форма.ОкончаниеПериодаРасчета Тогда
        // ЗУПК
			// Скрытые данные
			Продолжить;
		КонецЕсли;
...
Процедура ЗаполнитьФорму(ДанныеОНачислениях, ДанныеОВремени, ИндексацияСреднегоЗаработка = Неопределено)
	...
	РассчитатьСреднийЗаработок(ЭтаФорма);
    // ЗУПК
	ЗаполнитьИнформациюСреднийЗаработок(ЭтаФорма);
    // ЗУПК
...
Процедура РассчитатьСреднийЗаработок(Форма)
	...
	// ЗУПК
	ДополнительныеПараметры.ПорядокУчетаРабочегоВремени	= Форма.ПорядокУчетаРабочегоВремени;
	// ЗУПК
...
Процедура ДополнитьПредположительно()
	...
	Запрос.Текст =
		...
		// ЗУПК
		|	ДанныеУчетаВремени.ПроизводственныйКалендарьШестидневкаЧасы КАК ОтработаноЧасовШестидневка,
		// ЗУПК
...
Процедура ЗаполнитьОписаниеСтрок()
    ...
    // ЗУПК
	// Отработано часов по шестидневной неделе.
	НоваяСтрока = ОписаниеСтрок.Добавить();
	НоваяСтрока.Идентификатор = "ОтработаноЧасовШестидневка";
	НоваяСтрока.ЗаголовокСтроки = НСтр("ru = 'Отработано часов по шестидневной неделе'");
	НоваяСтрока.ДобавлятьПоУмолчанию = НастройкиРасчета.ИспользоватьОтработаноПоШестидневке И НастройкиРасчета.ИспользоватьОтработаноЧасов;
	НоваяСтрока.Точность = 2;
    // ЗУПК
...
Процедура ЗаполнитьИнформациюОбИндексации()
	...
	Пока ИндексСтроки >= 0 Цикл
		...
		Если СтрокаИндексации.КоэффициентИндексации <> ПредыдущийКоэффициент Тогда
			...
			Иначе 
				...
				ПериодДействияКоэффициента = КонецМесяца(СтрокаИндексации.Период) + 1;
				
				Если ПериодДействияКоэффициента <= ДатаНачалаСобытия
					И НЕ ИндексСтроки = ДанныеОбИндексации.Количество() - 1 Тогда
					ТекстИндексации = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
									НСтр("ru = '%1 года c коэффициентом %2'"), 
									Формат(СтрокаИндексации.Период, "ДФ='д ММММ гггг'"), 
									Формат(ПредыдущийКоэффициент, "ЧДЦ=8"));
				Иначе 
					ТекстИндексации = "";
				КонецЕсли;
	...
	Элементы.СведенияОбИндексацииГруппа.Заголовок = ?(ЗначениеЗаполнено(ИнфоНадписьКоэффициентыИндексации), 
	                                                    // ЗУПК
														НСтр("ru = 'Индексация заработка сотрудника:'"), 
	                                                    // ЗУПК
...
&НаКлиентеНаСервереБезКонтекста
Процедура ЗаполнитьИнформациюСреднийЗаработок(Форма)
	
	ИнфоНадписьСреднегоЗаработка = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Порядок учета рабочего времени: %1 %2'"),  
								  ?(ЗначениеЗаполнено(Форма.ПорядокУчетаРабочегоВремени), НРег(Строка(Форма.ПорядокУчетаРабочегоВремени)), НРег(Строка(ПредопределенноеЗначение("Перечисление.УчетРабочегоВремениПриРасчетеСреднегоЗаработка.ПоФактическомуГрафику")))), 
								  ?(Форма.ЧасовойСреднийЗаработок, НСтр("ru = ' по часам'"), НСтр("ru = 'по дням'")));
	
	Форма.Элементы.СреднийЗаработокИнформационныйТекст.Заголовок = ИнфоНадписьСреднегоЗаработка;
...
Функция ИдентификаторыСтрокДанныхВремени()
	...
	// ЗУПК
	Соответствие.Вставить("ОтработаноЧасовШестидневка", "ОтработаноЧасовШестидневка");
	// ЗУПК
...
Функция ОписаниеСтрокиДанныхОВремени()
    ...
    // ЗУПК
	ПоляОписания.Вставить("ОтработаноЧасовШестидневка", Новый ОписаниеТипов("Число"));
    // ЗУПК
...
Процедура ОбработкаИзмененияОтработанныхДней()
    ...
    // ЗУПК
	ПересчитываемыеСтроки.Добавить("ОтработаноЧасовШестидневка");
    // ЗУПК
	...
	Для Каждого СтрокаТаблицы Из СреднийЗаработок Цикл
		...
		Если СтрокаТаблицы.Идентификатор = "ОтработаноЧасов"
			// ЗУПК
			Или СтрокаТаблицы.Идентификатор = "ОтработаноЧасовШестидневка"
			// ЗУПК
...
Функция ДанныеРасшифровкиГодовойПремии(СоставнаяЧасть, Год)
	...
	// ЗУПК использует дату выплаты, в связи с чем данные по году могут отличаться
...
Процедура ЗаполнитьДанныеРасшифровкиГодовыхПремий(ДанныеРасшифровки, СоставнаяЧасть, Год)
	...
	// ЗУПК использует дату выплаты, в связи с чем данные по году могут отличаться
	ОтборСтрок = Новый Структура("СоставнаяЧасть", СоставнаяЧасть);
	// ЗУПК
...
Процедура ЗаполнитьПериодРасчетаПоУмолчанию()
    ...
    // ЗУПК
    НачислениеОценочныхОбязательств = Ложь;
    Если ТипЗнч(ДокументСсылка) = Тип("ДокументСсылка.НачислениеОценочныхОбязательствПоОтпускам") 
       ИЛИ ТипЗнч(ДокументСсылка) = Тип("ДокументСсылка.РезервыПоОплатеТруда") Тогда
        НачислениеОценочныхОбязательств = Истина;
    КонецЕсли;
    ПериодРасчета = УчетСреднегоЗаработка.ПериодРасчетаОбщегоСреднегоЗаработкаСотрудника(ДатаНачалаСобытия, Сотрудник, Начисление, НачислениеОценочныхОбязательств);
    // ЗУПК
