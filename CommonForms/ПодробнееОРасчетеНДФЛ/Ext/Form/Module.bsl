...
Процедура ЗаполнитьФормуНаСервере(АдресСведенийОбНДФЛ)
	...
	РегистрацияВНалоговомОрганеПодразделений = Новый Соответствие;
	Для Каждого СтрокаНДФЛ Из НДФЛ Цикл 
		Если ЗначениеЗаполнено(СтрокаНДФЛ.Подразделение) Тогда 
			РегистрацияВНалоговомОрганеПодразделения = ЗарплатаКадры.РегистрацияВНалоговомОргане(
			СтрокаНДФЛ.Подразделение, СтрокаНДФЛ.МесяцНалоговогоПериода);
			РегистрацияВНалоговомОрганеПодразделений.Вставить(СтрокаНДФЛ.Подразделение, РегистрацияВНалоговомОрганеПодразделения);
		КонецЕсли;
	КонецЦикла;
	...
	НалогСПревышения = НДФЛ.Итог("НалогСПревышения");
	...
	ТекущийРасчетИтоги.НалогСПревышения = НалогСПревышения;
	...
	Для Каждого СтрокаНДФЛ Из ДанныеПрошлыхПериодов.Налоги Цикл 
		...
		Если СтрокаНДФЛ.ДатаПолученияДохода >= НачалоРасчетаНДФЛПоПрогрессивнойШкале Тогда
			Если СтрокаНДФЛ.Начислено = 0 И СтрокаНДФЛ.НачисленоСПревышения = 0 Тогда 
				Продолжить;
			КонецЕсли;
		Иначе
		...
		Если НоваяСтрока.МесяцНалоговогоПериода >= НачалоРасчетаНДФЛПоПрогрессивнойШкале Тогда
			НоваяСтрока.НалогСПревышения = СтрокаНДФЛ.НачисленоСПревышения;
		КонецЕсли;
		НоваяСтрока.Подробно = Истина;
	...
	НДФЛПрошлыеПериоды.Свернуть("МесяцНалоговогоПериода,РегистрацияВНалоговомОргане,Подразделение,Ставка,СтрокаИтогов,ОтличаетсяПериод,Подробно,ПредставлениеПериода,Регистратор,ПредыдущиеРасчеты,Комментарий", "Налог,НалогСПревышения,Доход,Вычеты");
	...
	Для Сч = 1 По КоличествоСтрок Цикл 
		...
		Если СтрокаНДФЛ.СтрокаИтогов Тогда 
			...
			СтрокаНДФЛ.НалогСПревышения = НалогСПревышенияЗаМесяц;
		...
		НалогСПревышенияЗаМесяц = НалогСПревышенияЗаМесяц + СтрокаНДФЛ.НалогСПревышения;
...
Функция ДанныеВычетов(ИдентификаторСтроки)
	...
	НалоговыеВычеты = ВычетыКДоходам.Выгрузить(, "ИдентификаторСтроки,КодВычета,СуммаВычета,Начисление,МесяцПериодаПредоставленияВычета");
...
Процедура ЗаполнитьИтогиНДФЛ()
	...
	Для Каждого СтрокаНДФЛ Из НДФЛ Цикл 
		...
		НалогСПревышения = НалогСПревышения + СтрокаНДФЛ.НалогСПревышения;
