...
Процедура ОбработкаЗаполнения(ДанныеЗаполнения, ТекстЗаполнения, СтандартнаяОбработка)
	...
	Если ДанныеЗаполнения.Свойство("ФизическоеЛицо") Тогда
		Наименование = СтрШаблон(НСтр("ru = 'Заявка на удержание добровольных страховых взносов %1'"), Строка(ДанныеЗаполнения.ФизическоеЛицо));
	КонецЕсли;
КонецПроцедуры
...
Процедура ПередЗаписью(Отказ)
	...
	ЗаполнитьПравилаОбработкиЗаявки();
...
Функция ДоступноВыполнение() Экспорт
	Возврат БизнесПроцессыЗаявокСотрудников.ДоступноВыполнение(ЭтотОбъект, УдержаниеДСВ);	
КонецФункции
...
Процедура ЗаполнитьПравилаОбработкиЗаявки()
	
	Если Исполнитель <> Неопределено Тогда
		Возврат;
	КонецЕсли;
	
	Исполнитель = Справочники.РолиИсполнителей.ОтветственныйЗаУдержаниеДСВ;
