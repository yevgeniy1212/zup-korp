...
Процедура ОбработкаЗаполнения(ДанныеЗаполнения, ТекстЗаполнения, СтандартнаяОбработка)
	...
	Если ДанныеЗаполнения.Свойство("ФизическоеЛицо") Тогда
		Если КоличествоДнейКомпенсации <> 0 Тогда
			Наименование = СтрШаблон(НСтр("ru = 'Заявление на компенсацию дней отпуска %1'"), Строка(ДанныеЗаполнения.ФизическоеЛицо));
		Иначе
			Если ЗаСвойСчет Тогда
				Наименование = СтрШаблон(НСтр("ru = 'Заявление на отпуск без сохранения оплаты %1'"), Строка(ДанныеЗаполнения.ФизическоеЛицо));
			ИначеЕсли УчебныйОтпуск Тогда
				Наименование = СтрШаблон(НСтр("ru = 'Заявление на учебный отпуск %1'"), Строка(ДанныеЗаполнения.ФизическоеЛицо));
			Иначе
				Наименование = СтрШаблон(НСтр("ru = 'Заявление на оплачиваемый отпуск %1'"), Строка(ДанныеЗаполнения.ФизическоеЛицо));
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
...
Функция ДоступноВыполнение() Экспорт
	Возврат БизнесПроцессыЗаявокСотрудников.ДоступноВыполнение(ЭтотОбъект, Отпуска.Выгрузить());	
КонецФункции
