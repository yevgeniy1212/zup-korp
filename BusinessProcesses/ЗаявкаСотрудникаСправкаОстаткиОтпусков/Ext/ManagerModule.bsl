...
Функция ФормаВыполненияЗадачи(ЗадачаСсылка, ТочкаМаршрутаБизнесПроцесса) Экспорт
	...
	Результат.Вставить("ИмяФормы", "БизнесПроцесс.ЗаявкаСотрудникаСправкаОстаткиОтпусков.Форма.Действие" + ТочкаМаршрутаБизнесПроцесса.Имя);
	Возврат Результат;
КонецФункции
...
Процедура ОбновлениеДляИспользованияКЭДО(ПараметрыОбновления = Неопределено) Экспорт
	...
	Для Каждого СтрокаТЗ Из ТаблицаЗаявок Цикл
		...
		Попытка
			...
			ПеренестиИСоздатьДокументыКЭДОФайловОтветаЗаявки(СтрокаТЗ, Организация, ОписаниеОшибки);
			СоздатьИЗаписатьДокументКЭДООбновление(СтрокаТЗ, Организация);
...
Функция РезультатЗапросаЗаявокОбновлениеДляИспользованияКЭДО()
	...
	Запрос.Текст = "ВЫБРАТЬ
	               ...
	               |	И ДокументКадровогоЭДО.Ссылка ЕСТЬ NULL
	               |
	               |УПОРЯДОЧИТЬ ПО                          
	               |	Дата УБЫВ";
...
Процедура ПеренестиИСоздатьДокументыКЭДОФайловОтветаЗаявки(Заявка, Организация, ОписаниеОшибки)
	...
	ДокументКЭДО.Организация 			= Организация;
...
Процедура СоздатьИЗаписатьДокументКЭДООбновление(ЗаявкаСотрудника, Организация) Экспорт 
	ДокументКЭДО = Документы.ДокументКадровогоЭДО.СоздатьДокумент();
