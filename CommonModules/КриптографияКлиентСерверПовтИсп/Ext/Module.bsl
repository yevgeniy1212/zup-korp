...
Функция НовыйКриптопровайдерWSSocket(УникальныйИдентификаторФормыКлиента = Неопределено, РежимТишины = Ложь, ТолькоПодключение = Ложь) Экспорт
	...
	УдалосьПодключить = ПодключитьВнешнююКомпоненту(СсылкаНаМодуль, КриптографияКлиентСервер.ИмяКомпонентыКриптографииНЦА(), ТипВнешнейКомпоненты.Native, ТипПодключенияВнешнейКомпоненты.НеИзолированно);
	...
	Если НЕ ТолькоПодключение И КриптографияКлиентСервер.ВерсияКомпонентыНЦАВБазе() > СтрЗаменить(Криптопровайдер.Версия, ",", ".") Тогда 
