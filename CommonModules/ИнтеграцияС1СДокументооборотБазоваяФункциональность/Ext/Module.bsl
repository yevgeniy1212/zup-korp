...
//     * ИнтеграцияС1СДокументооборотПарольИзвестен - Булево
//
Функция ПараметрыДлительнойОперации(ИспользоватьКеш = Истина) Экспорт
...
Процедура ПодключитьсяИПолучитьВерсиюСервисаВФоне(Параметры, АдресХранилища) Экспорт
	...
	Если Не ИнтеграцияС1СДокументооборотБазоваяФункциональностьПовтИсп.ЭтоФоновоеЗадание() Тогда
		ИнтеграцияС1СДокументооборотБазоваяФункциональностьВызовСервера.СброситьКешПодключенияКДО();
	КонецЕсли;
...
Процедура ПриОпределенииНастроекРегламентныхЗаданий(Настройки) Экспорт
	...
	Настройка = Настройки.Добавить();
	Настройка.РегламентноеЗадание = Метаданные.РегламентныеЗадания.ИнтеграцияС1СДокументооборотВыполнитьОбменДанными;
	Настройка.ФункциональнаяОпция = Метаданные.ФункциональныеОпции.ИспользоватьИнтеграциюС1СДокументооборот3;
	Настройка.РаботаетСВнешнимиРесурсами = Истина;
	Настройка.ДоступноВМоделиСервиса = Ложь;
...
//
Функция ПолучитьОбъект(Прокси, Тип, ID, Колонки = Неопределено) Экспорт
...
Функция ПолучитьОбъекты(Прокси, Объекты, Колонки = Неопределено) Экспорт
	...
	Запрос = ПолучитьОбъектыЗапрос(Прокси, Объекты, Колонки);
	Ответ = ВыполнитьЗапрос(Прокси, Запрос);
	ПроверитьВозвратВебСервиса(Прокси, Ответ);
...
//
Функция ПолучитьОбъектыЗапрос(Прокси, Объекты, Колонки = Неопределено) Экспорт
...
Функция ПолучитьПрокси(ВызыватьИсключение = Истина, ИмяПользователя = Неопределено, Пароль = Неопределено,
		...
		ИспользуетсяАутентификацияJWT = Ложь, ТекстСообщенияОбОшибке = "") Экспорт
...
Функция ПечатныеФормыКСозданию(Источник, ПрисоединяемыеПечатныеФормы, ТипФайловСохраненияПечатныхФорм,
		ТолькоОбновление = Ложь, ИдентификаторФормы = Неопределено) Экспорт
...
Функция ПрисоединенныеФайлыВИС(ВладелецФайлов) Экспорт
	...
	ИмяСправочникаХранилищаФайлов = МодульРаботаСФайламиСлужебный.ИмяСправочникаХраненияФайлов(ВладелецФайлов);
...
Функция ДанныеВебСервисаДокументооборота(АдресВебСервисаДокументооборота)
...
Функция ПолучитьИОбновитьКешWSОпределения(ДанныеСервиса, МестоположениеWSDL, ИмяПользователя, Пароль,
	...
	Попытка
		...
		Иначе
			ТекстСообщенияОбОшибке = СокрЛП(СтрШаблон(
				НСтр("ru = 'Не удалось получить WSDL файл по адресу %2, т.к. сервер вернул HTTP код: %1. %3'"),
...
Функция ПроверкаПодключенияПоЛогинуПаролю(АдресВебСервисаДокументооборота, ДанныеСервиса, ИмяПользователя, Пароль,
	...
	Если ЗначениеЗаполнено(ИмяПользователя) Тогда
		Попытка
			...
			Определения = ПолучитьИОбновитьКешWSОпределения(
				ДанныеСервиса,
				МестоположениеWSDL,
				ИмяПользователя,
				Пароль);
