...
Процедура ПрочитатьСекциюReport(КонтекстФормы, ФайлОбмена, ПараметрыReport)
	...
	Если КонтекстФормы.СтруктураРеквизитовФормы.мКодШаблона <> ПараметрыReport["code"] Тогда
		Если Сред(КонтекстФормы.СтруктураРеквизитовФормы.мКодОтчета, 2) = Лев(ПараметрыReport["code"], 6) Тогда 
			ОбщегоНазначения.СообщитьПользователю(НСтр("ru='Не соответствует код шаблона. Возможно, файл от другого отчета'"));
		Иначе
		...
		Для Каждого Элемент Из Данные Цикл
			Если СокрЛП(Элемент.КодКолонки) = ЗначениеПоУмолчанию Тогда 
				...
				Если Обл <> Неопределено И Обл.СодержитЗначение = Истина И Обл.Защита = Ложь Тогда 
					...
					Если Элемент.ЗначениеКолонки = "0"
						И КонтекстФормы.СтруктураОписанийЯчеек.Свойство(Ключ) Тогда 
						
						КонтекстФормы.СтруктураДанныхОтчета.Вставить("ЧН" + Ключ, "ЧН=0");
						ОблТабДок = ТабДок.Области.Найти(Ключ);
						Если ТипЗнч(ОблТабДок) = Тип("ОбластьЯчеекТабличногоДокумента") Тогда 
							ОблТабДок.Формат = "ЧН=0";
						КонецЕсли;
					КонецЕсли;
				КонецЕсли;
			КонецЕсли;
...
Функция ЗагрузитьОтчет(КонтекстФормы, ВХ, Ошибка, ПараметрСтрокаXML = Ложь) Экспорт 
	...
	Попытка
		...
		Если ФайлОткрыт Тогда 
			...
			Исключение
				ОбщегоНазначения.СообщитьПользователю("Не удалось удалить файл:" + ИмяФайла);
			КонецПопытки;
		КонецЕсли;
...
Процедура ПодготовитьДанныеОтчетаПоШаблону(КонтекстФормы, ДанныеШаблонаОтчета, Отказ = Ложь) Экспорт 
	...
	Исключение
		ОбщегоНазначения.СообщитьПользователю(
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Не удалось получить текст XML-шаблона:%1'"), Символы.ПС + ОписаниеОшибки()));
		Отказ = Истина;
		Возврат;
	КонецПопытки;
...
Процедура ДобавитьКонтрольноеСоотношениеВТаблицу(КонтекстФормы, УзелКонтрольноеСоотношение)
	...
	ОбязательностьКонтроля = ПолучитьАттрибутОбязательностьКонтроля(УзелКонтрольноеСоотношение);
...
Функция ПолучитьАттрибутОбязательностьКонтроля(УзелКонтрольноеСоотношение) Экспорт 
	...
	НРегОбязательностьКонтроля = НРег(ОбязательностьКонтроля);
	Если НРегОбязательностьКонтроля = "false" Тогда 
		Возврат 0;
	ИначеЕсли НРегОбязательностьКонтроля = "true" Тогда 
		Возврат 1;
	Иначе
		Возврат ?(ЗначениеЗаполнено(ОбязательностьКонтроля), ОТЧ.ПривестиЗначение(ОбязательностьКонтроля), 1)
	КонецЕсли;
КонецФункции
...
Функция ПроверитьКонтрольныеСоотношенияВОтчете(ПараметрыДляПроверки) Экспорт 
	...
	Если ПараметрыДляПроверки.Свойство("ДанныеШаблонаОтчета") Тогда
		...
		Если Ошибка = Истина Тогда 
			ОбщегоНазначения.СообщитьПользователю("Не удалось загрузить макет");
			Возврат Ложь;
		КонецЕсли;
	КонецЕсли;
		...
		Если ЗначениеЗаполнено(Док) Тогда 
			...
			Если ЗначениеЗаполнено(ВыгрузкаПредидущегоПериода) Тогда 
				...
				Если Не ЗагрузитьОтчет(ПараметрыДляПроверки, ВыгрузкаПредидущегоПериода, Ошибка, Истина) Тогда 
					ОбщегоНазначения.СообщитьПользователю("Не удалось получить выгрузку предидущего периода:"+Ошибка);
				Иначе
	...
	Если ПараметрыДляПроверки.Свойство("НеобходимоПолучениеВыгрузкиТекущегоПериода") Тогда 
		...
		Если Не ЗагрузитьОтчет(ПараметрыДляПроверки, АдресВоВремХранилище, Ошибка, Истина) Тогда 
			ОбщегоНазначения.СообщитьПользователю("Не удалось загрузить отчет:"+Ошибка);
			Возврат Ложь;
		КонецЕсли;
	...
	Для каждого КонтрольноеСоотношение Из ПараметрыДляПроверки.ТаблицаКонтрольныхСоотношений Цикл
		...
		Описание = СтрЗаменить(Описание, "»", """");
		Расшифровка = СтрЗаменить(Расшифровка, "«", """");
		Расшифровка = СтрЗаменить(Расшифровка, "»", """");
		...
		Если Не ВектРезУсловие = Неопределено Тогда 
			...
			Если ПараметрыДляПроверки.КвоОшибокКС = 0 Тогда
				...
				ИначеЕсли ВектРезФормула <> Неопределено
					...
					Возврат Результат;
				ИначеЕсли ВектРезФормула = Неопределено Тогда
		...
		Для Инд2 = 0 По МатрицаРезультата[0].ВГраница() Цикл
			...
			Если ПроверятьПоУсловию Тогда
				Если МатрицаУсловий[Инд1][Инд2] Тогда
					МатрицаРезультата[Инд1][Инд2] = ОбщегоНазначения.ВычислитьВБезопасномРежиме(ТекстовоеВыражение);
				Иначе
...
Функция ВекторныйРезультатВычисленияЛогическогоПодвыражения(ПараметрыДляПроверки, ЭтоМежПериоднаяПроверка, МатрицаРезультата, ШаблонВыражения, ТаблицаЭлементов, ИндексПодвыражения = 0, СчетчикОшибок = 0)
	...
	Если ЭлементыВыражения.Количество() = 0 Тогда
		Возврат ОбщегоНазначения.ВычислитьВБезопасномРежиме(ШаблонВыражения);
	КонецЕсли;
...
Функция РезультатВычисленияВыраженияИзСтроки(ПараметрыДляПроверки, ТекстовоеПредставлениеВыражения, РазрядностьОкругления = 2, ДопустимоеОтклонение = 0) Экспорт 
	...
	Возврат ОбщегоНазначения.ВычислитьВБезопасномРежиме(ТекстовоеВыражение);
...
Функция РезультатУсловияНаПериод(ПараметрыДляПроверки, ЛогОператор, КодыПериодов)
	...
	Иначе
		Попытка
			Возврат ОбщегоНазначения.ВычислитьВБезопасномРежиме(Формат(КодНомераПериода, "ЧН=;ЧГ=") + ЛогОператор + Формат(НомерПериодаИзСправочника(ПараметрыДляПроверки, КодыПериодов), "ЧН=;ЧГ="));
		Исключение
			Возврат Ложь;
		КонецПопытки
	КонецЕсли;
...
Функция СоответствиеФункцийРеализациям() Экспорт 
	...
	Соответствие.Вставить("abs",      "УниверсальныйОтчетСтатистики.АбсЗначение");
	Соответствие.Вставить("coalesce", "УниверсальныйОтчетСтатистики.ПервоеЗаполненное");
	Соответствие.Вставить("floor",    "УниверсальныйОтчетСтатистики.НаибольшееЦелое");
	Соответствие.Вставить("isnull",   "УниверсальныйОтчетСтатистики.ЕслиПусто");
	Соответствие.Вставить("nullif",   "УниверсальныйОтчетСтатистики.НеопределеноЕсли");
	Соответствие.Вставить("round",    "УниверсальныйОтчетСтатистики.ОкрЗначение");
...
Функция РезультатВычисленияЭлементарногоВыражения(ВычисляемоеВыражение, СчетчикОшибок) Экспорт 
	...
	Попытка
		РезультатВычисления = ОбщегоНазначения.ВычислитьВБезопасномРежиме(ВычисляемоеВыражение);
	Исключение
...
Функция ПолучитьПараметрыИнициализацииУниверсальногоОтчета(Форма, ИмяМакетаАтрибутов, ИмяМакетаШаблона, ИмяСхемы, ФормироватьТекстВыгрузки = Истина, ЗагружатьШаблон = Истина) Экспорт 
	Попытка
		...
		Если СтрНачинаетсяС(Форма.ИмяФормы, "Внешний") Тогда 
			ДанныеШаблонаИзРегистра = Неопределено;
		Иначе
			ДанныеШаблонаИзРегистра = РегламентированнаяОтчетность.ПолучитьДанныеШаблона(Форма, ИмяМакетаАтрибутов, ИмяМакетаШаблона, СведШаблон, СведШаблон.Версия);
		КонецЕсли;
		Если ТипЗнч(ДанныеШаблонаИзРегистра) = Тип("ХранилищеЗначения") Тогда 
			Результат.Вставить("ПроизведенаЗаменаФормата", Истина);
			Результат.Вставить("ПроизведенаЗаменаФормата_Версия", СведШаблон.Версия);
			Результат["Версия"] = СведШаблон.Версия;
			Результат.Вставить("ИмяФайлаШаблона", Строка(Новый УникальныйИдентификатор) + ".tmp");
			Результат.Вставить("Шаблон", ДанныеШаблонаИзРегистра);
			Результат["НеобходимоЗагрузитьШаблон"] = Ложь;
		ИначеЕсли ЗагружатьШаблон Тогда 
...
Функция ПолучитьПостроительПоПараметру(Шаблон)
	Если ТипЗнч(Шаблон) = Тип("Строка") Тогда 
		...
		ЧтениеXML.Закрыть();
		Возврат ДокументDOM;
	ИначеЕсли ТипЗнч(Шаблон) = Тип("ХранилищеЗначения") Тогда
		КаталогРазархивирования = ПолучитьИмяВременногоФайла();
		СоздатьКаталог(КаталогРазархивирования);
		КаталогРазархивирования = КаталогРазархивирования + ПолучитьРазделительПути();
		
		ВФАрх = КаталогРазархивирования + Строка(Новый УникальныйИдентификатор);
		Шаблон.Получить().Записать(ВФАрх);
		
		ЧтениеXML = Новый ЧтениеXML;
		ЧтениеXML.ОткрытьФайл(ВФАрх);
		ПостроительDOM = Новый ПостроительDOM;
		ДокументDOM = ПостроительDOM.Прочитать(ЧтениеXML);
		ЧтениеXML.Закрыть();
		...
		УдалитьФайлы(КаталогРазархивирования);
		Возврат ДокументDOM;
	ИначеЕсли ТипЗнч(Шаблон) = Тип("ДвоичныеДанные") Тогда
		КаталогРазархивирования = ПолучитьИмяВременногоФайла();
		СоздатьКаталог(КаталогРазархивирования);
		КаталогРазархивирования = КаталогРазархивирования + ПолучитьРазделительПути();
		
		ВФАрх = КаталогРазархивирования + Строка(Новый УникальныйИдентификатор);
		Шаблон.Записать(ВФАрх);
		...
		ЧтениеXML = Новый ЧтениеXML;
		ЧтениеXML.ОткрытьФайл(ИтоговыйФайл);
		ПостроительDOM = Новый ПостроительDOM;
		ДокументDOM = ПостроительDOM.Прочитать(ЧтениеXML);
		ЧтениеXML.Закрыть();
		ЧтениеZip.Закрыть();
		...
		УдалитьФайлы(КаталогРазархивирования);
		Возврат ДокументDOM;
	ИначеЕсли ТипЗнч(Шаблон) = Тип("ЧтениеXML") Тогда
		ДокументDOM = ПостроительDOM.Прочитать(ЧтениеXML);
		Возврат ДокументDOM;
	КонецЕсли;
КонецФункции

Функция ДокументСоответствуетПравиламРосстата(Документ)
	Попытка
		Если Не ((Документ.ДочерниеУзлы.Количество() = 1) И (НРег(Документ.ПервыйДочерний.ИмяУзла) = "metaform")) Тогда 
			Возврат Ложь;
		КонецЕсли;
		
		ЕстьТитул = Ложь; ЕстьСекции = Ложь;
		Для Каждого Узел Из Документ.ПервыйДочерний.ДочерниеУзлы Цикл 
			Если Нрег(Узел.ИмяУзла) = "title" Тогда 
				ЕстьТитул = Истина;
			КонецЕсли;
			Если Нрег(Узел.ИмяУзла) = "sections" Тогда 
				ЕстьСекции = Истина;
				Для Каждого УзелПодч Из Узел.ДочерниеУзлы Цикл 
					Если Нрег(УзелПодч.ИмяУзла) <> "section" Тогда 
						Возврат Ложь;
					КонецЕсли;
				КонецЦикла;
			КонецЕсли;
		...
		Если (Не ЕстьТитул) Или (Не ЕстьСекции) Тогда 
			Возврат Ложь;
		КонецЕсли;
...
Функция ПолучитьУзелЗаголовок(Документ)
	Для Каждого Узел Из Документ.ПервыйДочерний.ДочерниеУзлы Цикл 
		Если Нрег(Узел.ИмяУзла) = "title" Тогда 
			Возврат Узел;
		КонецЕсли;
...
Функция ЗаголовкиДокументовСовпадают(Документ1, Документ2)
	Заголовок1 = ПолучитьУзелЗаголовок(Документ1);
	Заголовок2 = ПолучитьУзелЗаголовок(Документ2);
	Если Заголовок1 = Неопределено Или Заголовок2 = Неопределено Или Заголовок1.ДочерниеУзлы.Количество() <> Заголовок2.ДочерниеУзлы.Количество() Тогда 
		Возврат Ложь;
	КонецЕсли;
	
	ЭлементыЗаголовка = Новый Массив;
	Для Каждого Узел Из Заголовок2.ДочерниеУзлы Цикл 
		ЭлтЗаголовка = Узел.Атрибуты.ПолучитьИменованныйЭлемент("field");
		ЭлементыЗаголовка.Добавить(ЭлтЗаголовка.ЗначениеУзла);
	КонецЦикла;
	Для Каждого Узел Из Заголовок1.ДочерниеУзлы Цикл 
		ЭлтЗаголовка = Узел.Атрибуты.ПолучитьИменованныйЭлемент("field");
		Если ЭлементыЗаголовка.Найти(ЭлтЗаголовка.ЗначениеУзла) = Неопределено Тогда 
			Возврат Ложь;
		КонецЕсли;
	КонецЦикла;
	Возврат Истина;
...
Функция ПолучитьСекции(Документ)
	Для Каждого Узел Из Документ.ПервыйДочерний.ДочерниеУзлы Цикл 
		Если Нрег(Узел.ИмяУзла) = "sections" Тогда 
			Возврат Узел.ДочерниеУзлы;
		КонецЕсли;
	КонецЦикла;
КонецФункции

Функция ПолучитьСтрокиУзла(Узел)
	Для Каждого УзелПодч Из Узел.ДочерниеУзлы Цикл 
		Если Нрег(УзелПодч.ИмяУзла) = "rows" Тогда 
			Возврат УзелПодч;
		КонецЕсли;
	КонецЦикла;
КонецФункции

Функция ПолучитьКолонкиУзла(Узел)
	Для Каждого УзелПодч Из Узел.ДочерниеУзлы Цикл 
		Если Нрег(УзелПодч.ИмяУзла) = "columns" Тогда 
			Возврат УзелПодч;
		КонецЕсли;
	КонецЦикла;
КонецФункции

Функция СформироватьСвойстваКолонок(Колонки)
	Свойства = Новый Соответствие;
	Для Каждого Кол Из Колонки.ДочерниеУзлы Цикл 
		Описание = Новый Структура;
		Для Каждого Подч Из Кол.ДочерниеУзлы Цикл 
			Если Нрег(Подч.ИмяУзла) = "default-cell" Тогда
				Для Каждого Аттр Из Подч.Атрибуты Цикл 
					Описание.Вставить(Аттр.ИмяУзла, Аттр.ЗначениеУзла);
				КонецЦикла;
				Прервать;
			КонецЕсли;
		КонецЦикла;
		Свойства.Вставить(Кол.Атрибуты.ПолучитьИменованныйЭлемент("code").ЗначениеУзла, Описание);
	КонецЦикла;
	Возврат Свойства;
КонецФункции

Функция СравнитьКолонки(Колонки1, Колонки2, Свойства1, Свойства2, Поля)
	Если Колонки1 = Неопределено Или Колонки2 = Неопределено Или Колонки1.ДочерниеУзлы.Количество() <> Колонки2.ДочерниеУзлы.Количество() Тогда 
		Возврат Ложь;
	КонецЕсли;
	Свойства1 = СформироватьСвойстваКолонок(Колонки1);
	Свойства2 = СформироватьСвойстваКолонок(Колонки2);
	Если Свойства1.Количество() <> Свойства2.Количество() Тогда 
		Возврат Ложь;
	КонецЕсли;
	
	Для Каждого КЗ Из Свойства1 Цикл
		Если Свойства2[КЗ.Ключ] = Неопределено Тогда
			Возврат Ложь;
		КонецЕсли;
	КонецЦикла;
...
Функция СформироватьСвойстваСтрок(Строки, СвойстваКолонок)
	Свойства = Новый Соответствие;
	Для Каждого Стр Из Строки.ДочерниеУзлы Цикл 
		Если Не ЗначениеЗаполнено(Стр.Атрибуты.ПолучитьИменованныйЭлемент("code").ЗначениеУзла) Тогда 
			Продолжить;
		КонецЕсли;
		
		СвойстваКолонокКопия = ОбщегоНазначения.СкопироватьРекурсивно(СвойстваКолонок);
		Для Каждого Кол Из Стр.ДочерниеУзлы Цикл
			КолонкаИндекс = Кол.Атрибуты.ПолучитьИменованныйЭлемент("column").ЗначениеУзла;
			ЗначениеКолонкиИсходное = СвойстваКолонокКопия[КолонкаИндекс];
			Для Каждого Аттр Из Кол.Атрибуты Цикл 
				Если ЗначениеКолонкиИсходное.Свойство(Аттр.ИмяУзла) Тогда 
					ЗначениеКолонкиИсходное[Аттр.ИмяУзла] = Аттр.ЗначениеУзла;
				ИначеЕсли НРег(Аттр.ИмяУзла) = "format" Или НРег(Аттр.ИмяУзла) = "inputtype" Или НРег(Аттр.ИмяУзла) = "column" Тогда 
					ЗначениеКолонкиИсходное.Вставить(Аттр.ИмяУзла, Аттр.ЗначениеУзла);
				КонецЕсли;
			КонецЦикла;
		КонецЦикла;
		Описание = Новый Структура;
		Описание.Вставить("__type__", Стр.Атрибуты.ПолучитьИменованныйЭлемент("type").ЗначениеУзла);
		Описание.Вставить("СвойстваКолонок", СвойстваКолонокКопия);
		Свойства.Вставить(Стр.Атрибуты.ПолучитьИменованныйЭлемент("code").ЗначениеУзла, Описание);
	КонецЦикла;
	Возврат Свойства;
КонецФункции

Функция СравнитьСтроку(Стр1, Стр2, Поля)
	Если Стр1 = Неопределено Или Стр2 = Неопределено Или Стр1.__type__ <> Стр2.__type__ Тогда 
		Возврат Ложь;
	КонецЕсли;
	
	Если Стр1.СвойстваКолонок.Количество() <> Стр2.СвойстваКолонок.Количество() Тогда 
		Возврат Ложь;
	КонецЕсли;
	
	Для Каждого КЗ Из Стр1.СвойстваКолонок Цикл 
		Для Каждого КЗ2 Из КЗ.Значение Цикл 
			Если Поля.Найти(КЗ2.Ключ) = Неопределено Тогда 
				Продолжить;
			КонецЕсли;
			
			Если КЗ2.Значение <> Стр2.СвойстваКолонок[КЗ.Ключ][КЗ2.Ключ] Тогда 
				Возврат Ложь;
			КонецЕсли;
		КонецЦикла;
	КонецЦикла;
...
Функция СравнитьСтроки(Строки1, Строки2, СвойстваКолонок1, СвойстваКолонок2, Поля)
	Если Строки1 = Неопределено Или Строки2 = Неопределено Или Строки1.ДочерниеУзлы.Количество() <> Строки2.ДочерниеУзлы.Количество() Тогда 
		Возврат Ложь;
	КонецЕсли;
	
	Свойства1 = СформироватьСвойстваСтрок(Строки1, СвойстваКолонок1);
	Свойства2 = СформироватьСвойстваСтрок(Строки2, СвойстваКолонок2);
	Если Свойства1.Количество() <> Свойства2.Количество() Тогда 
		Возврат Ложь;
	КонецЕсли;
	
	Для Каждого КЗ Из Свойства1 Цикл
		Если Не СравнитьСтроку(КЗ.Значение, Свойства2[КЗ.Ключ], Поля) Тогда 
			Возврат Ложь;
		КонецЕсли;
	КонецЦикла;
...
Функция МетаданныеСекцииСовпадают(Секция1, Секция2)
	Если Секция1 = Неопределено Или Секция2 = Неопределено Тогда 
		Возврат Ложь;
	КонецЕсли;
	
	Поля = Новый Массив;
	Поля.Добавить("column");
	Поля.Добавить("format");
	
	СвойстваКолонок1 = Неопределено; СвойстваКолонок2 = Неопределено;
	Если Не СравнитьКолонки(ПолучитьКолонкиУзла(Секция1), ПолучитьКолонкиУзла(Секция2), СвойстваКолонок1, СвойстваКолонок2, Поля) Тогда 
		Возврат Ложь;
	КонецЕсли;
	Если Не СравнитьСтроки(ПолучитьСтрокиУзла(Секция1), ПолучитьСтрокиУзла(Секция2), СвойстваКолонок1, СвойстваКолонок2, Поля) Тогда 
		Возврат Ложь;
	КонецЕсли;
...
Функция МетаданныеСодержательнойЧастиДокументовСовпадают(Документ1, Документ2)
	Секции1 = ПолучитьСекции(Документ1);
	Секции2 = ПолучитьСекции(Документ2);
	Если Секции1 = Неопределено Или Секции2 = Неопределено Или Секции1.Количество() <> Секции2.Количество() Тогда 
		Возврат Ложь;
	КонецЕсли;
	
	С1Соотв = Новый Соответствие;
	Для Каждого Узел Из Секции1 Цикл
		С1Соотв.Вставить(Узел.Атрибуты.ПолучитьИменованныйЭлемент("code").ЗначениеУзла, Узел);
	КонецЦикла;
	С2Соотв = Новый Соответствие;
	Для Каждого Узел Из Секции2 Цикл
		С2Соотв.Вставить(Узел.Атрибуты.ПолучитьИменованныйЭлемент("code").ЗначениеУзла, Узел);
	КонецЦикла;
	
	Для Каждого КЗ Из С1Соотв Цикл 
		Если Не МетаданныеСекцииСовпадают(КЗ.Значение, С2Соотв[Кз.Ключ]) Тогда 
			Возврат Ложь;
		КонецЕсли;
	КонецЦикла;
...
Функция ПолучитьСправочникПериодов(Документ)
	Для Каждого Узел Из Документ.ПервыйДочерний.ДочерниеУзлы Цикл 
		Если Нрег(Узел.ИмяУзла) = "dics" Тогда 
			Для Каждого УзДоч Из Узел.ДочерниеУзлы Цикл 
				Если УзДоч.Атрибуты.ПолучитьИменованныйЭлемент("id").ЗначениеУзла = "s_mes" Тогда 
					Результат = Новый Соответствие;
					Для Каждого Элем Из УзДоч.ДочерниеУзлы Цикл 
						Результат.Вставить(Элем.Атрибуты.ПолучитьИменованныйЭлемент("id").ЗначениеУзла, Истина);
					КонецЦикла;
					Возврат Результат;
				КонецЕсли;
			КонецЦикла;
...
Функция СправочникиПериодовСовпадают(Документ1, Документ2)
	СпрПериодов1 = ПолучитьСправочникПериодов(Документ1);
	СпрПериодов2 = ПолучитьСправочникПериодов(Документ1);
	Если СпрПериодов1 = Неопределено Или СпрПериодов2 = Неопределено Или СпрПериодов1.Количество() <> СпрПериодов2.Количество() Тогда 
		Возврат Ложь;
	КонецЕсли;
	Для Каждого КЗ Из СпрПериодов1 Цикл 
		Если СпрПериодов2.Получить(КЗ.Ключ) = Неопределено Тогда 
			Возврат Ложь;
		КонецЕсли;
	КонецЦикла;
	Возврат Истина;
...
Функция СравнитьСтруктурыШаблонов(Шаблон1, Шаблон2) Экспорт 
	Попытка
		Документ1 = ПолучитьПостроительПоПараметру(Шаблон1);
		Документ2 = ПолучитьПостроительПоПараметру(Шаблон2);
		
		Если (Не ДокументСоответствуетПравиламРосстата(Документ1)) Или (Не ДокументСоответствуетПравиламРосстата(Документ2)) Тогда 
			Возврат Ложь;
		КонецЕсли;
		
		Если Не ЗаголовкиДокументовСовпадают(Документ1, Документ2) Тогда 
			Возврат Ложь;
		КонецЕсли;
		
		Если Не СправочникиПериодовСовпадают(Документ1, Документ2) Тогда 
			Возврат Ложь;
		КонецЕсли;
		
		Если Не МетаданныеСодержательнойЧастиДокументовСовпадают(Документ1, Документ2) Тогда 
			Возврат Ложь;
		КонецЕсли;
