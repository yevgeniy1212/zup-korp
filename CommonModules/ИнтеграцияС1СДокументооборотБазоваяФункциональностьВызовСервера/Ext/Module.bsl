...
//   УникальныйИдентификатор - УникальныйИдентификатор - уникальный идентификатор формы, из которой выполняется запрос.
//   ИспользоватьКеш - Булево - при проверке возможности авторизации без пароля не требуется использовать кеш.
//
// Возвращаемое значение:
//   Структура - см. ДлительныеОперации.ВыполнитьВФоне
//
Функция ПодключитьсяИПолучитьВерсиюСервисаАсинхронно(ИмяПользователя, Пароль,
		УникальныйИдентификатор = Неопределено, ИспользоватьКеш = Истина) Экспорт
...
Функция ДанныеЗаполненияИнтегрируемогоОбъекта(УИДПравила, ТипОбъектаДО, ИдентификаторОбъектаДО) Экспорт
	...
	Если Настройки.ИспользоватьИнтеграциюДО2 Тогда
		Правило = Справочники["ПравилаИнтеграцииС1СДокументооборотом"].ПолучитьСсылку(УИДПравила);
	ИначеЕсли Настройки.ИспользоватьИнтеграциюДО3 Тогда
		Правило = Справочники["ПравилаИнтеграцииС1СДокументооборотом3"].ПолучитьСсылку(УИДПравила);
	Иначе
	...
	Если Настройки.ИспользоватьИнтеграциюДО2 Тогда
		Справочники["ПравилаИнтеграцииС1СДокументооборотом"].ЗаполнитьОбъектПоОбъектуXDTO(
			Прокси,
			Объект,
			ОбъектXDTO,
			Правило);
	ИначеЕсли Настройки.ИспользоватьИнтеграциюДО3 Тогда
		Справочники["ПравилаИнтеграцииС1СДокументооборотом3"].ЗаполнитьОбъектИСПоОбъектуXDTO(
			Объект,
			ОбъектXDTO,
			Правило);
	КонецЕсли;
...
Функция ПроверитьПодключение(Знач ИмяПользователя, Знач Пароль, Знач АдресСервиса = Неопределено,
		...
		ТекстСообщенияОбОшибке = "") Экспорт
...
//
Процедура СброситьКешПодключенияКДО() Экспорт
...
Функция ФайлXDTOИзПараметровСоздания(Прокси, ПараметрыСоздания) Экспорт
	...
	Если ЗначениеЗаполнено(ПараметрыСоздания.ВнешнийОбъект.ID) Тогда
		ВнешнийОбъектXDTO = ИнтеграцияС1СДокументооборотБазоваяФункциональность.СоздатьОбъект(
			Прокси,
			"ExternalObject");
		ВнешнийОбъектXDTO.ID = ПараметрыСоздания.ВнешнийОбъект.ID;
		ВнешнийОбъектXDTO.type = ПараметрыСоздания.ВнешнийОбъект.type;
		ВнешнийОбъектXDTO.name = ПараметрыСоздания.ВнешнийОбъект.name;
		ФайлXDTO.externalObject = ВнешнийОбъектXDTO;
	КонецЕсли;
	...
	ИначеЕсли ЗначениеЗаполнено(ПараметрыСоздания.ШаблонID) Тогда
		...
		ФайлШаблон.objectID = ИнтеграцияС1СДокументооборотБазоваяФункциональность.СоздатьObjectID(Прокси,
...
Процедура ПроверитьПодписи(ТаблицаПодписей, УникальныйИдентификатор,
	...
	ИнтеграцияС1СДокументооборотБазоваяФункциональностьКлиентСервер.ПроверитьПодписи(
...
Функция ОперацияПодключенияИПолученияВерсииСервиса(ИспользоватьКеш, ИмяПользователя, Пароль,
