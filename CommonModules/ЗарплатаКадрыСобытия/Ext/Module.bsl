...
Процедура ПодразделенияОрганизацийПередЗаписью(Источник, Отказ) Экспорт
	...
	Если Источник.ЭтоНовый() Или Не Источник.ОбособленноеПодразделение Тогда
		...
		Если Не Источник.ОбособленноеПодразделение Или Источник.РайонныйКоэффициент < 1 Тогда
            ...
            // ЗУПК обход РФ ошибки: ПрименяетсяРайонныйКоэффициент может быть в значении Неопределено 
			Если ПрименяетсяРайонныйКоэффициент = Истина Тогда
            // ЗУПК обход РФ ошибки: ПрименяетсяРайонныйКоэффициент может быть в значении Неопределено 
...
Процедура УстановитьНастройкиЗарплатаКадрыПередЗаписью(Источник, Отказ) Экспорт
	...
	|	НастройкиЗарплатаКадры.ПрименятьСевернуюНадбавкуВПодразделениях КАК ПрименятьСевернуюНадбавкуВПодразделениях,
	|	НастройкиЗарплатаКадры.ПрименятьРайонныйКоэффициентВПодразделениях КАК ПрименятьРайонныйКоэффициентВПодразделениях,
	|	НастройкиЗарплатаКадры.ДатаВыплатыАвансаНеПозжеЧем КАК ДатаВыплатыАвансаНеПозжеЧем,
	|	НастройкиЗарплатаКадры.ДатаВыплатыЗарплатыНеПозжеЧем КАК ДатаВыплатыЗарплатыНеПозжеЧем,
	|	НастройкиЗарплатаКадры.ВыплачиватьЗарплатуВПоследнийДеньМесяца КАК ВыплачиватьЗарплатуВПоследнийДеньМесяца
	|ИЗ
...
Процедура ОбработкаДокументовПечатныхФорм() Экспорт
	...
	ОбщегоНазначения.ПриНачалеВыполненияРегламентногоЗадания(Метаданные.РегламентныеЗадания.ОбработкаДокументовПечатныхФорм);
	ЗарплатаКадры.ОбработкаДокументовПечатныхФорм();
