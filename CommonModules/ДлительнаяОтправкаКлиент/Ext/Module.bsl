...
Процедура ПоказатьОшибки(ПутьКОбъекту, ВходящийКонтекст) Экспорт
	...
	Если ЕстьОшибкаРазмераПисьмаФНС(ПараметрыФормы) Тогда
		...
		ТекстОшибки = ОшибкаРазмераПисьмаФНС(ПараметрыФормы.Ошибки);
		
		ПоказатьОшибкуРазмераПисьмаФНС(
...
Процедура ПоказатьОшибкуРазмераПисьмаФНС(Письмо, ПутьКОбъекту, ТекстОшибки, ОписаниеЗавершения = Неопределено) Экспорт
	
	ДополнительныеПараметры = Новый Структура();
	ДополнительныеПараметры.Вставить("ОписаниеОшибки", ТекстОшибки);
