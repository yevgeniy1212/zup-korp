...
Функция ЕстьСформированнаяОтчетностьЗаПериодОбновляемогоНабора(МенеджерВременныхТаблиц, ПериодыССуществующейОтчетностью)
...
Функция РесурсыУчетаСтажаПФР() Экспорт
	...
	Ресурсы.Добавить("ТерриториальныеУсловияДополнительные");
...
Процедура ДополнитьЗапросВТИсходныеДанныеУчетаСтажа(Запрос) Экспорт
	...
	ТекстСоединения = "
	|	ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.КатегорииДолжностейПолучателейСтимулирующихВыплатФСС КАК КатегорииПолучателейВыплатФСС
	|	ПО ПериодыСтажаПФР.Должность = КатегорииПолучателейВыплатФСС.Должность";
	...
	Если ИспользоватьШтатноеРасписание Тогда
		ТекстСоединения = СтрЗаменить(ТекстСоединения, "ПериодыСтажаПФР.Должность", "ПериодыСтажаПФР.ДолжностьПоШтатномуРасписанию");
	КонецЕсли;
...
Процедура ДополнитьЗапросПараметрыПериодовСтажаПФР(Запрос, ПриЗаписи) Экспорт
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("ЗарплатаКадрыКорпоративнаяПодсистемы.ОхранаТруда") Тогда
		...
		МодульОхранаТруда.ДополнитьЗапросПараметрыПериодовСтажаПФР(Запрос, ПриЗаписи);
	КонецЕсли;
...
Функция ТекстДополнительныхПолейЗапросаВТДанныеУчетаСтажаПФР() Экспорт
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("ЗарплатаКадрыКорпоративнаяПодсистемы.ОхранаТруда") Тогда
		МодульОхранаТруда = ОбщегоНазначения.ОбщийМодуль("ОхранаТруда");
		МодульОхранаТруда.ДополнитьТекстДополнительныхПолейЗапросаВТДанныеУчетаСтажаПФР(ТекстыПолейЗапроса);
	КонецЕсли;
	
	Если ТекстыПолейЗапроса.Получить("ОснованиеДосрочногоНазначенияПенсии") = Неопределено Тогда
		ТекстПоля = "ВЫБОР
	...
	Если ТекстыПолейЗапроса.Получить("КодПозицииСписка") = Неопределено Тогда
		ТекстПоля = "ВЫБОР
		|		КОГДА ПериодыСтажаПФР.УсловияТруда <> ЗНАЧЕНИЕ(Справочник.УсловияТруда.ПустаяСсылка)
	...
	ТекстПоля = "ПериодыСтажаПФР.ТерриториальныеУсловияДополнительные";
	ТекстыПолейЗапроса.Вставить("ТерриториальныеУсловияДополнительные", ТекстПоля);
	
	Если ТекстыПолейЗапроса.Получить("ОсобыеУсловияТруда") = Неопределено Тогда
		ТекстПоля = "ВЫБОР
	...
	ТекстПоля = "ПериодыСтажаПФР.ЗарегистрированоЗначениеТерриториальныеУсловияДополнительные";
	...
	"ВЫБОР
	|	КОГДА ПериодыСтажаПФР.ДолжностьПоШтатномуРасписанию <> ЗНАЧЕНИЕ(Справочник.ШтатноеРасписание.ПустаяСсылка)
	|		ТОГДА ПериодыСтажаПФР.ДолжностьПоШтатномуРасписанию
	|	ИНАЧЕ ПериодыСтажаПФР.Подразделение
	|КОНЕЦ";
	ТекстыПолейЗапроса.Вставить("ВладелецДопТерриторий", ТекстПоля);
	
	ТекстПоля = "ВЫБОР
	|		КОГДА ПериодыСтажаПФР.ДолжностьПоШтатномуРасписанию <> ЗНАЧЕНИЕ(Справочник.ШтатноеРасписание.ПустаяСсылка)
	|			ТОГДА ПериодыСтажаПФР.ДолжностьПоШтатномуРасписанию.РаботаСКоронавирусом
	|		ИНАЧЕ ПериодыСтажаПФР.Должность.РаботаСКоронавирусом
	|	КОНЕЦ";
	ТекстыПолейЗапроса.Вставить("РаботаСКоронавирусом", ТекстПоля);
	
	ТекстПоля = "КатегорииПолучателейВыплатФСС.КатегорияПолучателяСтимулирующихВыплатФСС.ОказываетМедицинскуюПомощьCOVID";
	ТекстыПолейЗапроса.Вставить("ОказываетМедицинскуюПомощьCOVID", ТекстПоля);
	
	ТекстПоля = "ПериодыСтажаПФР.РабочееМесто";
	ТекстыПолейЗапроса.Вставить("РабочееМесто", ТекстПоля);
...
Процедура СоздатьВТСоответствиеВидовСтажаПараметрамИсчисления2014(МенеджерВременныхТаблиц) Экспорт
	...
	|
	|ВЫБРАТЬ
	|	ЗНАЧЕНИЕ(Перечисление.ВидыСтажаПФР2014.ВОЕНСЛ),
	|	ЗНАЧЕНИЕ(Справочник.ПараметрыИсчисляемогоСтраховогоСтажа.ВОЕНСЛ)";
...
Процедура СоздатьВТПериодыРаботыСКоронавирусом(МенеджерВременныхТаблиц) Экспорт
	...
	Запрос.Текст = 
		...
		|	ПериодыРаботыСКоронавирусом.ДатаНачала КАК Период
		|ПОМЕСТИТЬ ВТСотрудникиРаботающиеСКоронавирусом
		|ИЗ
		...
		|		ПО ПериодыРаботыСКоронавирусом.ФизическоеЛицо = ВТФизическиеЛица.ФизическоеЛицо
		|			И ПериодыРаботыСКоронавирусом.Организация.ГоловнаяОрганизация = ВТФизическиеЛица.ГоловнаяОрганизация
		|			И ПериодыРаботыСКоронавирусом.ДатаНачала <= ВТФизическиеЛица.ДатаОкончания
		|			И (ВЫБОР
		|				КОГДА ПериодыРаботыСКоронавирусом.ДатаОкончания = ДАТАВРЕМЯ(1, 1, 1)
		|					ТОГДА ИСТИНА
		|				ИНАЧЕ ПериодыРаботыСКоронавирусом.ДатаОкончания >= ВТФизическиеЛица.ДатаНачала
		|			КОНЕЦ)";
	
	Запрос.Выполнить();
	
	ОписаниеФильтра = ЗарплатаКадрыОбщиеНаборыДанных.ОписаниеФильтраДляСоздатьВТИмяРегистра("ВТСотрудникиРаботающиеСКоронавирусом", "Сотрудник");
	ЗарплатаКадрыОбщиеНаборыДанных.СоздатьВТИмяРегистраСрезПоследних("ВидыЗанятостиСотрудников", МенеджерВременныхТаблиц, Истина, ОписаниеФильтра);
	
	Запрос.Текст = 
		"ВЫБРАТЬ РАЗЛИЧНЫЕ
		|	ПериодыРаботыСКоронавирусом.ФизическоеЛицо КАК ФизическоеЛицо,
		|	ПериодыРаботыСКоронавирусом.Организация.ГоловнаяОрганизация КАК ГоловнаяОрганизация,
		|	ВЫБОР
		|		КОГДА ПериодыРаботыСКоронавирусом.ДатаНачала < ВТФизическиеЛица.ДатаНачала
		|			ТОГДА ВТФизическиеЛица.ДатаНачала
		|		ИНАЧЕ ПериодыРаботыСКоронавирусом.ДатаНачала
		|	КОНЕЦ КАК ДатаНачала,
		|	ВЫБОР
		|		КОГДА ПериодыРаботыСКоронавирусом.ДатаОкончания = ДАТАВРЕМЯ(1, 1, 1)
		|			ТОГДА ВТФизическиеЛица.ДатаОкончания
		|		КОГДА ПериодыРаботыСКоронавирусом.ДатаОкончания > ВТФизическиеЛица.ДатаОкончания
		|			ТОГДА ВТФизическиеЛица.ДатаОкончания
		...
		|		ПО (ВидыЗанятостиСотрудников.Сотрудник = ПериодыРаботыСКоронавирусом.Сотрудник)
		|			И (ВидыЗанятостиСотрудников.Период = ПериодыРаботыСКоронавирусом.ДатаНачала)
		|			И (ВидыЗанятостиСотрудников.ВидЗанятости = ЗНАЧЕНИЕ(Перечисление.ВидыЗанятости.ОсновноеМестоРаботы))
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|УНИЧТОЖИТЬ ВТСотрудникиРаботающиеСКоронавирусом
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|УНИЧТОЖИТЬ ВТВидыЗанятостиСотрудниковСрезПоследних";
...
Функция ВыборкаПоИзменившимсяДаннымСтажа(МенеджерВременныхТаблиц)
	...
	Запрос.Текст = 
	"ВЫБРАТЬ
	|	ИзменившиесяДанныеСтажа.ГоловнаяОрганизация КАК Организация,
	|	ИзменившиесяДанныеСтажа.Начало КАК ОтчетныйПериод,
	|	ИзменившиесяДанныеСтажа.ФизическоеЛицо КАК ФизическоеЛицо,
	|	ИзменившиесяДанныеСтажа.ТипДоговора
	|ИЗ
	|	ВТКлючиИзменившихсяДанных КАК ИзменившиесяДанныеСтажа
	|
	|УПОРЯДОЧИТЬ ПО
	|	Организация,
	|	ОтчетныйПериод,
...
Процедура ЗарегистрироватьКорректировкиСтажаПФР(МенеджерВременныхТаблиц)	
	...
	Если Не ЕстьСформированнаяОтчетностьЗаПериодОбновляемогоНабора(МенеджерВременныхТаблиц, ПериодыССуществующейОтчетностью) Тогда
		Возврат;
	КонецЕсли;
	...
	Пока ВыборкаПоИзменившимсяДаннымСтажа.СледующийПоЗначениюПоля("Организация") Цикл
		Пока ВыборкаПоИзменившимсяДаннымСтажа.СледующийПоЗначениюПоля("ОтчетныйПериод") Цикл 
			...
			СтруктураПоиска.ОтчетныйПериод = ПерсонифицированныйУчетКлиентСервер.ПолучитьНачалоОтчетногоПериода(ВыборкаПоИзменившимсяДаннымСтажа.ОтчетныйПериод);
...
Процедура ПроверитьДанныеКадровогоПеревода(ДанныеОбъекта, ОписаниеПолейОбъекта, Отказ) Экспорт
	...
	Если СохранятьЛьготныйСтаж Тогда
		Если Не ЗначениеЗаполнено(ДатаОкончания) Тогда
			...
			ОбщегоНазначения.СообщитьПользователю(ТекстСообщения, ДанныеОбъекта.Ссылка, ОписаниеПолейОбъекта.ИмяПоляДатаОкончания, "Объект", Отказ); 
		КонецЕсли;	
		...
		Если Не ЗначениеЗаполнено(ВидСтажаПФР) Тогда
			...
			ОбщегоНазначения.СообщитьПользователю(ТекстСообщения, ДанныеОбъекта.Ссылка, ОписаниеПолейОбъекта.ИмяПоляВидСтажаПФР, "Объект", Отказ);
		КонецЕсли;	
	КонецЕсли;		
КонецПроцедуры	
...
Процедура ЗаполнитьДатуНачалаДвиженийПФР(ПараметрыОбновления = Неопределено) Экспорт
	
	Запрос = Новый Запрос;
	Запрос.МенеджерВременныхТаблиц = Новый МенеджерВременныхТаблиц;
	Запрос.Текст =  
		"ВЫБРАТЬ
		|	СтажПФРОтпускаПоУходу.Регистратор КАК Регистратор,
		|	МИНИМУМ(СтажПФРНачальнойШтатнойРасстановки.Начало) КАК ДатаНачалаПФР
		|ПОМЕСТИТЬ ВТРегистраторы
		|ИЗ
		|	РегистрСведений.ПараметрыПериодовСтажаПФР КАК СтажПФРОтпускаПоУходу
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.ПараметрыПериодовСтажаПФР КАК СтажПФРНачальнойШтатнойРасстановки
		|		ПО СтажПФРОтпускаПоУходу.ГоловнаяОрганизация = СтажПФРНачальнойШтатнойРасстановки.ГоловнаяОрганизация
		|			И СтажПФРОтпускаПоУходу.ФизическоеЛицо = СтажПФРНачальнойШтатнойРасстановки.ФизическоеЛицо
		|			И СтажПФРОтпускаПоУходу.Сотрудник = СтажПФРНачальнойШтатнойРасстановки.Сотрудник
		|			И СтажПФРОтпускаПоУходу.ТипДоговора = СтажПФРНачальнойШтатнойРасстановки.ТипДоговора
		|			И СтажПФРОтпускаПоУходу.ДатаНачалаСобытия < СтажПФРНачальнойШтатнойРасстановки.Начало
		|			И (СтажПФРОтпускаПоУходу.Регистратор ССЫЛКА Документ.ОтпускПоУходуЗаРебенком)
		|			И (СтажПФРНачальнойШтатнойРасстановки.Регистратор ССЫЛКА Документ.НачальнаяШтатнаяРасстановка)
		|
		|СГРУППИРОВАТЬ ПО
		|	СтажПФРОтпускаПоУходу.Регистратор";
	...
	Пока Выборка.Следующий() Цикл
		...
		ДокументОбъект.ДатаНачалаПФР = Выборка.ДатаНачалаПФР;
		ОбновлениеИнформационнойБазы.ЗаписатьДанные(ДокументОбъект);
		МассивСсылок.Добавить(Выборка.Регистратор);
	КонецЦикла;
	
	ДанныеДляРегистрацииВУчетаСтажаПФР = Документы.ОтпускПоУходуЗаРебенком.ДанныеДляРегистрацииВУчетаСтажаПФР(МассивСсылок);
	
	Для Каждого КлючИЗначение Из ДанныеДляРегистрацииВУчетаСтажаПФР Цикл
		...
		УчетСтажаПФР.ЗарегистрироватьПериодыВУчетеСтажаПФР(Движения, КлючИЗначение.Значение);
		ОбновлениеИнформационнойБазы.ЗаписатьНаборЗаписей(Движения.ПараметрыПериодовСтажаПФР);
	...
	Запрос.Текст = 
		...
		|		ПО ПараметрыПериодовСтажаПФР.Регистратор = Регистраторы.Регистратор";
