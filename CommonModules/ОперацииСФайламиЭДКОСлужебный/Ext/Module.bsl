...
Функция ПолучитьРазмерФайла(Знач Соединение, Знач ПутьНаСервере)
	...
	Исключение
		...
		ЗаписьЖурналаРегистрации(НСтр("ru = 'Электронный документооборот с контролирующими органами. Скачивание файла'", ОбщегоНазначения.КодОсновногоЯзыка()),
...
Процедура ИзменитьПрогрессЗагрузки(Загружено, ВсегоКЗагрузке)
	...
	Если ЭтоФоновоеЗадание() Тогда
		...
		ОбщегоНазначения.СообщитьПользователю("#JSON#:" + ЗаписьJSON.Закрыть());
	КонецЕсли;
