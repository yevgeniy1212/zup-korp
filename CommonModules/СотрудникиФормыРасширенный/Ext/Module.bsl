...
Процедура ДобавитьРеквизитыВФормеСотрудника(Форма)
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("ЗарплатаКадрыКорпоративнаяПодсистемы.ОхранаТруда") Тогда
		МодульОхранаТрудаФормы = ОбщегоНазначения.ОбщийМодуль("ОхранаТрудаФормы");
		МодульОхранаТрудаФормы.ДобавитьВМассивСоздаваемыеРеквизитыФормыСотрудника(МассивДобавляемыхРеквизитов);
	КонецЕсли; 
...
Процедура СотрудникиОбновитьЭлементыФормы(Форма) Экспорт
	...
	Если Пользователи.РолиДоступны("ДобавлениеИзменениеДанныхДляНачисленияЗарплаты,ЧтениеДанныхДляНачисленияЗарплаты") Тогда
		...
		Если Форма.ПолучитьФункциональнуюОпциюФормы("ПрименятьСевернуюНадбавку") Тогда
			...
			ТерриториальныеУсловияПФР = ЗарплатаКадрыРасширенный.ТерриториальныеУсловияПФРСтруктурнойЕдиницы(
				Форма.ТекущееПодразделение, ОбщегоНазначения.ТекущаяДатаПользователя());
			
			ПрименятьСевернуюНадбавку =
				ЗарплатаКадрыРасширенныйКлиентСервер.ПрименятьНадбавкуЗаОсобыеКлиматическиеУсловияПоТерриториальнымУсловиям(ТерриториальныеУсловияПФР);
...
Функция РезультатыПроверкиУникальностиФизическогоЛица(ФизическоеЛицоСсылка, ИНН, СтраховойНомерПФР, ДокументВид = "", ДокументСерия = "", ДокументНомер = "") Экспорт
	...
	Если ТаблицаСовпадений.Количество() > 0 Тогда
        ...
        // ЗУПК
					НСтр("ru = 'Найдена запись о человеке, имеющем такой же ИИН (%1)'"),
        // ЗУПК
					ИНН);
...
Функция КлючиСтруктурыТекущихКадровыхДанныхСотрудника() Экспорт
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("ЗарплатаКадрыКорпоративнаяПодсистемы.ОхранаТруда") Тогда 
		МодульОхранаТрудаФормы = ОбщегоНазначения.ОбщийМодуль("ОхранаТрудаФормы");
		МодульОхранаТрудаФормы.ДобавитьКлючиСтруктурыТекущихКадровыхДанныхСотрудника(
			КлючиСтруктурыТекущихКадровыхДанныхСотрудника);
	КонецЕсли;
...
Процедура СохранитьДанныеДополнительнойФормы(Форма, ИмяФормы, Отказ, ТекущийОбъект) Экспорт
    ...
    // ЗУПК
	ИначеЕсли ИмяФормы = "Справочник.ФизическиеЛица.Форма.ПрочиеСведения" Тогда
		...
		РедактированиеПериодическихСведений.ЗаписатьЗаписьПослеРедактированияВФорме(Форма, "СведенияОФизическихЛицахУчастникахВОВ", Форма.ФизическоеЛицоСсылка);
		РедактированиеПериодическихСведений.ЗаписатьЗаписьПослеРедактированияВФорме(Форма, "СведенияОПенсионномОбеспеченииФизическихЛиц", Форма.ФизическоеЛицоСсылка);
				
	ИначеЕсли ИмяФормы = "Справочник.Сотрудники.Форма.УчетСреднегоЗаработка" Тогда
		
		РедактированиеПериодическихСведений.ЗаписатьЗаписьПослеРедактированияВФорме(Форма, "РасчетСреднегоЗаработкаСотрудников", Форма.СотрудникСсылка);
    // ЗУПК
...
Функция ОписаниеМенюВводаНаОсновании(ПараметрыПостроения) Экспорт
	...
	Если Не ПолучитьФункциональнуюОпцию("ОтключенныйФункционалЗарплатаКадры") Тогда 
		ИсключаемыеПунктыМеню.Добавить("Документы.СправкаНДФЛ");
	КонецЕсли;
	...
	Если ПараметрыПостроения = Неопределено
		...
		Если Не ПолучитьФункциональнуюОпцию("ОтключенныйФункционалЗарплатаКадры") Тогда
			...
			ИсключаемыеПунктыМеню = Новый Массив;
			ИсключаемыеПунктыМеню.Добавить("Документы.ОплатаДнейУходаЗаДетьмиИнвалидами");
			ИсключаемыеПунктыМеню.Добавить("Документы.ПризПодарок");
		
		КонецЕсли;
		ОписаниеМеню.Пункты.Добавить(
			СотрудникиФормы.РазделМеню(НСтр("ru='Начисления, удержания'"), ПунктыМеню, ИсключаемыеПунктыМеню));
	...
	Если Не ПолучитьФункциональнуюОпцию("ОтключенныйФункционалЗарплатаКадры") Тогда
		...
		ИсключаемыеПунктыМеню.Добавить("Документы.ЗаявлениеНаПредоставлениеСтандартныхВычетовПоНДФЛ");
		ИсключаемыеПунктыМеню.Добавить("Документы.УведомлениеОПравеНаИмущественныйВычетДляНДФЛ");
		ИсключаемыеПунктыМеню.Добавить("Документы.ПрекращениеСтандартныхВычетовНДФЛ");
		ИсключаемыеПунктыМеню.Добавить("Документы.ВозвратНДФЛ");
		ИсключаемыеПунктыМеню.Добавить("Документы.ОперацияНалоговогоУчетаПоНДФЛ");
		
	КонецЕсли;	
	
	ОписаниеМеню.Пункты.Добавить(
		СотрудникиФормы.РазделМеню(НСтр("ru='НДФЛ'"), ПунктыМеню,
			?(Не ПолучитьФункциональнуюОпцию("ОтключенныйФункционалЗарплатаКадры"),ИсключаемыеПунктыМеню, Неопределено)));
	...
	Если Не ПолучитьФункциональнуюОпцию("ОтключенныйФункционалЗарплатаКадры") Тогда
		...
		ИсключаемыеПунктыМеню.Добавить("Документы.ОперацияУчетаПоСтраховымВзносам");
		ИсключаемыеПунктыМеню.Добавить("Документы.ЗаявлениеСотрудникаНаВыплатуПособия");
		ИсключаемыеПунктыМеню.Добавить("Документы.ЕдиновременноеПособиеЗаСчетФСС");
		
		Если ПараметрыПостроения = Неопределено
			ИЛИ ПараметрыПостроения.ОформленПоТрудовомуДоговору И НЕ ЗначениеЗаполнено(ПараметрыПостроения.ДатаУвольнения) Тогда
			
			ИсключаемыеПунктыМеню.Добавить("Документы.ИсходящаяСправкаОЗаработкеДляРасчетаПособий");
			ИсключаемыеПунктыМеню.Добавить("Документы.ВходящаяСправкаОЗаработкеДляРасчетаПособий");
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("ЗарплатаКадрыКорпоративнаяПодсистемы.ОхранаТруда") Тогда
		...
		МодульОхранаТруда.ДобавитьПодменюПоНазначениюРабочихМестСотрудникам(ОписаниеМеню);
	КонецЕсли;
	...
	Если ПараметрыПостроения = Неопределено
		...
		Если Не ПолучитьФункциональнуюОпцию("ОтключенныйФункционалЗарплатаКадры") Тогда
			...
			ИсключаемыеПунктыМеню.Добавить("Документы.АвансовыйПлатежИностранцаПоНДФЛ");
...
Процедура ЗаполнитьИсториюВзаимоотношений(История, ФизическиеЛица) Экспорт
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("ЗарплатаКадрыКорпоративнаяПодсистемы.ПодборПерсонала") Тогда
		МодульПодборПерсонала = ОбщегоНазначения.ОбщийМодуль("ПодборПерсонала");
		МодульПодборПерсонала.ДополнитьИсториюВзаимоотношенийКандидатамиПоФизическимЛицам(История, ФизическиеЛица);
		МодульПодборПерсонала.ДополнитьИсториюВзаимоотношенийКандидатамиПоЛичнымДанным(История, ФизическиеЛица);
	КонецЕсли;
