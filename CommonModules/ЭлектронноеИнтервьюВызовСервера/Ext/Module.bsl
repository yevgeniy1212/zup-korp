...
Функция АнкетаКандидата(ПараметрыФормы, Кандидат, ЭтапРаботы, ОтвеченныеВопросы) Экспорт
	...
	ЗаписьАнкетыКандидата.ЛичныеДанныеКандидата =
		ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Кандидат, "ЛичныеДанные");
...
//
// Параметры:
//  Респондент - СправочникСсылка.ЛичныеДанныеКандидатов - личные данные кандидата, для которого ищутся
	...
	Иначе
		...
		Если СтрокиЭтапа.Количество() > 0 Тогда
			Если Не ЗначениеЗаполнено(СтрокиЭтапа[0].ШаблонАнкеты) Тогда
				Возврат Ложь;
			Иначе
				Если ТипЗнч(СтрокиЭтапа[0].ШаблонАнкеты) = Тип("СправочникСсылка.ШаблоныАнкет") Тогда
					Возврат Истина;
				Иначе
					Возврат Ложь;
				КонецЕсли;
