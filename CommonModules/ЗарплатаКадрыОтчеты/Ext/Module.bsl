	...
	Для Каждого ОписаниеНастраиваемойФормы Из ВариантыОтчетовПечатныхФорм Цикл
		...
		Если СтрНайти(ПутьКВариантуОтчета, ИмяОтчета) = 1 Тогда
			...
			Если УправлениеПечатью.НужноПечататьМакет(КоллекцияПечатныхФорм, ИмяМакетаПечатнойФормы) Тогда
				...
				Иначе
					МакетКомпоновкиДанных = МакетКомпоновкиДанныхОтчетаПоКлючуВарианта(ОбъектОтчета, КлючВарианта);
				КонецЕсли;
...
Функция НомерНаПечать(Знач Номер, Знач НомерПриказа = Неопределено) Экспорт
	...
	Если НастройкиПечатныхФорм.УдалятьПрефиксыОрганизацииИИБИзНомеровКадровыхПриказов Тогда
		НовыйНомер = ПрефиксацияОбъектовКлиентСервер.НомерНаПечать(Номер, Истина, Истина);
		Если Не ПустаяСтрока(НовыйНомер) Тогда
			Номер = НовыйНомер;
		КонецЕсли;
	КонецЕсли;
...
Функция ДатаНаПечать(Знач Дата, Знач ДатаПриказа = Неопределено) Экспорт
...
Функция ТарифнаяСтавкаНаПечать(Знач ТарифнаяСтавка, Группировка = Ложь) Экспорт
	...
	Если ЗначениеЗаполнено(ТарифнаяСтавка) Тогда
		...
		// ЗУПК
		Возврат Формат(Цел(ТарифнаяСтавка), ФорматнаяСтрока) + " " + НСтр("ru='тг'") + ". " + Формат((ТарифнаяСтавка - Цел(ТарифнаяСтавка)), "ЧЦ=2; ЧДЦ=0; ЧС=-2; ЧН=; ЧВН=") + " " + НСтр("ru='тиын'");
		// ЗУПК
		
	КонецЕсли;
	
	// ЗУПК
	Возврат "_______________ " + НСтр("ru='тг'") + ". ____ " + НСтр("ru='тиын'");
	// ЗУПК
...
Процедура ПередНачаломКомпоновкиРезультата(ОбъектОтчета) Экспорт
	...
	ЗарплатаКадрыОбщиеНаборыДанных.ЗаменитьПредставленияЗапросов(ОбъектОтчета.СхемаКомпоновкиДанных.НаборыДанных,
		ОбъектОтчета.СхемаКомпоновкиДанных);
...
Процедура УстановитьПараметрыСтраницыТабличногоДокументаПоОбразцу(ДокументРезультат, ДокументОбразец) Экспорт
	
	ЗаполнитьЗначенияСвойств(ДокументРезультат, ПараметрыСтраницыТабличногоДокумента(ДокументОбразец));
	
КонецПроцедуры

Процедура ВывестиБланкВТабличныйДокумент(ДокументРезультат, ВыводимыйДокумент, ДобавлятьРазделительСтраниц = Истина) Экспорт
	
	Если ДокументРезультат.ВысотаТаблицы = 0 Тогда
		УстановитьПараметрыСтраницыТабличногоДокументаПоОбразцу(ДокументРезультат, ВыводимыйДокумент);
	Иначе
		Если ДобавлятьРазделительСтраниц Тогда
			ДокументРезультат.ВывестиГоризонтальныйРазделительСтраниц();
		КонецЕсли;
	КонецЕсли;
	
	ДокументРезультат.Вывести(ВыводимыйДокумент);
...
// Возвращает таблицу значений с данными об организациях.
// 
// Параметры:
//  Организации - Массив из СправочникСсылка.Организации, Неопределено -
//  Период - Дата, Неопределено -
// 
// Возвращаемое значение:
//  ТаблицаЗначений - Данные организаций:
...
// * ЮрАдрес - Строка -
// * ФактАдрес - Строка -
// * ГородФактическогоАдреса - Строка -
...
// * ДолжностьГлавногоБухгалтера - Строка -
Функция ДанныеОрганизаций(Организации = Неопределено, Период = Неопределено) Экспорт
	
	ТаблицаДанныхОрганизаций = НоваяТаблицаДанныхОрганизаций();
	...
	Если Организации = Неопределено Тогда
		Организации = ОбщегоНазначения.ВыгрузитьКолонку(РезультатЗапроса.Выгрузить(), "Организация", Истина);
	КонецЕсли;
	
	АдресаОрганизаций = УправлениеКонтактнойИнформациейЗарплатаКадры.АдресаОрганизаций(Организации, Период);
	ЗапрашиваемыеСведения = ЗапрашиваемыеСведенияОбОрганизации();
	...
	Пока Выборка.Следующий() Цикл
		...
		НоваяСтрокаСведенияОбОрганизациях.Организация = Организация;
		НоваяСтрокаСведенияОбОрганизациях.Период = Период;
		
		СведенияОбОрганизации = РегламентированнаяОтчетностьВызовСервера.ПолучитьСведенияОбОрганизации(Организация,
			Период, ЗапрашиваемыеСведения);
		
		Если СведенияОбОрганизации.Свойство("Организация") Тогда
			НоваяСтрокаСведенияОбОрганизациях.Организация = Организация;
		КонецЕсли;
		
		Если СведенияОбОрганизации.Свойство("НаимЮЛПол") Тогда
			НоваяСтрокаСведенияОбОрганизациях.НаименованиеПолное = СведенияОбОрганизации.НаимЮЛПол;
		КонецЕсли;
		
		Если СведенияОбОрганизации.Свойство("ИННЮЛ") Тогда
			НоваяСтрокаСведенияОбОрганизациях.ИНН = СведенияОбОрганизации.ИННЮЛ;
		КонецЕсли;
		
		Если СведенияОбОрганизации.Свойство("КППЮЛ") Тогда
			НоваяСтрокаСведенияОбОрганизациях.КПП = СведенияОбОрганизации.КППЮЛ;
		КонецЕсли;
		
		Если СведенияОбОрганизации.Свойство("ТелОрганизации") Тогда
			НоваяСтрокаСведенияОбОрганизациях.Телефон = СведенияОбОрганизации.ТелОрганизации;
		КонецЕсли;
		
		Если СведенияОбОрганизации.Свойство("ФаксОрганизации") Тогда
			НоваяСтрокаСведенияОбОрганизациях.Факс = СведенияОбОрганизации.ФаксОрганизации;
		КонецЕсли;
		
		Если СведенияОбОрганизации.Свойство("БанкСчетНомер") Тогда
			НоваяСтрокаСведенияОбОрганизациях.РасчетныйСчет = СведенияОбОрганизации.БанкСчетНомер;
		КонецЕсли;
		
		Если СведенияОбОрганизации.Свойство("БанкСчетНаимБанка") Тогда
			НоваяСтрокаСведенияОбОрганизациях.Банк = СведенияОбОрганизации.БанкСчетНаимБанка;
		КонецЕсли;
		
		Если СведенияОбОрганизации.Свойство("БанкСчетКоррСчетБанка") Тогда
			НоваяСтрокаСведенияОбОрганизациях.КорСчет = СведенияОбОрганизации.БанкСчетКоррСчетБанка;
		КонецЕсли;
		
		Если СведенияОбОрганизации.Свойство("БанкСчетБИКБанка") Тогда
			НоваяСтрокаСведенияОбОрганизациях.БИК = СведенияОбОрганизации.БанкСчетБИКБанка;
		КонецЕсли;
		...
		ОписаниеФактическогоАдреса = УправлениеКонтактнойИнформациейЗарплатаКадры.АдресОрганизации(АдресаОрганизаций,
			Организация, Справочники.ВидыКонтактнойИнформации.ФактАдресОрганизации);
		...
		ЗапрашиваемыеЗначения = ЗапрашиваемыеЗначенияДляЗаполненияПодписей();
...
Функция ОтчетРасчетныйЛисток(ВариантОтчета = Неопределено) Экспорт
	
	ОтчетОбъект = Отчеты.АнализНачисленийИУдержаний.Создать();
	КлючПредопределенногоВарианта = "РасчетныйЛисток";
	Если ВариантОтчета = Неопределено Тогда
		ВариантОтчета = ВариантыОтчетов.ВариантОтчета(
			ОбщегоНазначения.ИдентификаторОбъектаМетаданных(ОтчетОбъект.Метаданные()), КлючПредопределенногоВарианта);
	КонецЕсли;
	НастроитьРасчетныйЛисток(ОтчетОбъект, ВариантОтчета, КлючПредопределенногоВарианта);
	
	Возврат ОтчетОбъект;
	
КонецФункции

Функция ОтчетРасчетныйЛистокЗаПервуюПоловинуМесяца(ВариантОтчета = Неопределено) Экспорт
	
	ОтчетОбъект = Отчеты.АнализНачисленийИУдержанийАвансом.Создать();
	КлючПредопределенногоВарианта = "РасчетныйЛистокПерваяПоловинаМесяца";
	Если ВариантОтчета = Неопределено Тогда
		ВариантОтчета = ВариантыОтчетов.ВариантОтчета(
			ОбщегоНазначения.ИдентификаторОбъектаМетаданных(ОтчетОбъект.Метаданные()), КлючПредопределенногоВарианта);
	КонецЕсли;
...
Процедура НастроитьРасчетныйЛисток(ОтчетОбъект, ВариантОтчета, КлючПредопределенногоВарианта)
	
	ОтчетОбъект.КомпоновщикНастроек.ПользовательскиеНастройки.ДополнительныеСвойства.Вставить(
		"УстанавливаетсяМакетКомпоновкиДанных", Истина);
	СвойстваВарианта = СвойстваВариантаОтчета(ОтчетОбъект, ВариантОтчета, КлючПредопределенногоВарианта);
	
	НастройкиОтчета = ХранилищаНастроек.ХранилищеВариантовОтчетов.Загрузить(
		ОтчетОбъект.Метаданные().ПолноеИмя(), СвойстваВарианта.КлючВарианта);
	
	НастройкиОтчетаКД = Неопределено;
	ПользовательскиеНастройкиКД = Неопределено;
	Если НастройкиОтчета = Неопределено Тогда
		НастройкиОтчетаКД = ОтчетОбъект.СхемаКомпоновкиДанных.ВариантыНастроек[СвойстваВарианта.КлючПредопределенногоВарианта].Настройки;
	ИначеЕсли ТипЗнч(НастройкиОтчета) = Тип("Структура") Тогда
		НастройкиОтчетаКД = НастройкиОтчета.Настройки;
		ПользовательскиеНастройкиКД = НастройкиОтчета.ПользовательскиеНастройкиКД;
	Иначе
		ОтчетОбъект.КомпоновщикНастроек.ЗагрузитьНастройки(ОтчетОбъект.СхемаКомпоновкиДанных.ВариантыНастроек[СвойстваВарианта.КлючПредопределенногоВарианта].Настройки);
		ПрименитьНастрокиКОтчету(ОтчетОбъект, СвойстваВарианта.КлючВарианта, НастройкиОтчета, Неопределено)
	КонецЕсли;
	...
	ОтчетОбъект.ИнициализироватьОтчет(СвойстваВарианта.КлючВарианта);
	
	МакетКомпоновкиДанных = МакетКомпоновкиДанныхОтчета(
		ОтчетОбъект, СвойстваВарианта, , ОтчетОбъект.КомпоновщикНастроек.ПолучитьНастройки());
	
	Если НастройкиОтчетаКД <> Неопределено Тогда
		Для Каждого ПараметрДанных Из НастройкиОтчетаКД.ПараметрыДанных.Элементы Цикл
			Если ПараметрДанных.Использование Тогда
				ПараметрМакета = МакетКомпоновкиДанных.ЗначенияПараметров.Найти(Строка(ПараметрДанных.Параметр));
				Если ПараметрМакета <> Неопределено Тогда
					ПараметрМакета.Значение = ПараметрДанных.Значение;
				КонецЕсли;
			КонецЕсли;
		КонецЦикла;
	КонецЕсли;
...
Функция ДанныеРасчетныхЛистков(ФизическиеЛица, Организация, ДатаНачала, ДатаОкончания, ОтчетРасчетныйЛисток = Неопределено, ПараметрыСтраницы = Неопределено) Экспорт
	...
	ОтчетРасчетныйЛисток.КомпоновщикНастроек.ПользовательскиеНастройки.
		ДополнительныеСвойства.Вставить("ГотовитьДанныеДляКабинетаСотрудников", Истина);
	
	// Формирование расчетного листка
	ОтчетРасчетныйЛисток.СкомпоноватьРезультат(ДокументРезультат);
	Результат = Новый Структура("ДокументРезультат", ДокументРезультат);
	...
	Результат.Вставить("ДанныеРасчетныхЛистков", ДанныеРасчетныхЛистков);
	
	ОтчетРасчетныйЛисток.КомпоновщикНастроек.ПользовательскиеНастройки.ДополнительныеСвойства.Удалить("ГотовитьДанныеДляКабинетаСотрудников");
	ОтчетРасчетныйЛисток.КомпоновщикНастроек.ПользовательскиеНастройки.ДополнительныеСвойства.Удалить("ДанныеРасчетныхЛистков");
...
Функция СвойстваВариантаОтчета(ОтчетОбъект, ВариантОтчета, КлючПредопределенногоВарианта)
	Если ЗначениеЗаполнено(ВариантОтчета) И ОбщегоНазначения.СсылкаСуществует(ВариантОтчета) Тогда
		СвойстваВарианта = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(ВариантОтчета, "Ссылка,КлючВарианта,Родитель,ХешНастроек");
		Если ЗначениеЗаполнено(СвойстваВарианта.Родитель) Тогда
			СвойстваВарианта.Вставить("КлючПредопределенногоВарианта",
				ОбщегоНазначения.ЗначениеРеквизитаОбъекта(СвойстваВарианта.Родитель, "КлючВарианта"));
		Иначе
			СвойстваВарианта.Вставить("КлючПредопределенногоВарианта", СвойстваВарианта.КлючВарианта);
		КонецЕсли;
		Возврат СвойстваВарианта
	Иначе
		Возврат Новый Структура("Ссылка,КлючВарианта,Родитель,ХешНастроек,КлючПредопределенногоВарианта", Неопределено, КлючПредопределенногоВарианта, Неопределено, "", КлючПредопределенногоВарианта);
	КонецЕсли;
	МетаданныеОтчета = ОтчетОбъект.Метаданные();
	Возврат СвойстваВариантаОтчета(
		ОтчетОбъект,
		ВариантыОтчетов.ВариантОтчета(ОбщегоНазначения.ИдентификаторОбъектаМетаданных(МетаданныеОтчета), КлючПредопределенногоВарианта),
		"");
КонецФункции

Функция МакетКомпоновкиДанныхОтчета(ОтчетОбъект, СвойстваВарианта, ПодготовитьМакетЕслиНеНайден = Истина, НастройкиДобавляемогоВарианта = Неопределено) Экспорт
	...
	МакетКомпоновки = СохраненныйМакетКомпоновкиДанныхОтчета(Отчет, СвойстваВарианта.КлючВарианта, СвойстваВарианта.ХешНастроек);
	
	Если МакетКомпоновки = Неопределено И ПодготовитьМакетЕслиНеНайден Тогда
		ДобавитьМакетКомпоновкиДанныхОтчета(Отчет, МетаданныеОтчета.Имя, СвойстваВарианта, ОтчетОбъект, НастройкиДобавляемогоВарианта);
		МакетКомпоновки = СохраненныйМакетКомпоновкиДанныхОтчета(Отчет, СвойстваВарианта.КлючВарианта, СвойстваВарианта.ХешНастроек);
	КонецЕсли;
	ЗагрузитьНастройкиПечатнойФормы(ОтчетОбъект.КомпоновщикНастроек, НастройкиПечатнойФормы(Отчет, СвойстваВарианта.КлючВарианта));
...
Функция МакетКомпоновкиДанныхОтчетаПоКлючуВарианта(ОтчетОбъект, Знач КлючВарианта, ПодготовитьМакетЕслиНеНайден = Истина) Экспорт
	МетаданныеОтчета = ОтчетОбъект.Метаданные();
	Отчет = ОбщегоНазначения.ИдентификаторОбъектаМетаданных(МетаданныеОтчета);
	ВариантОтчета = ВариантыОтчетов.ВариантОтчета(Отчет, КлючВарианта);
	СвойстваВарианта = СвойстваВариантаОтчета(ОтчетОбъект, ВариантОтчета, КлючВарианта);
	Возврат МакетКомпоновкиДанныхОтчета(ОтчетОбъект, СвойстваВарианта, ПодготовитьМакетЕслиНеНайден);
КонецФункции

Функция СохраненныйМакетКомпоновкиДанныхОтчета(Отчет, КлючВарианта, ХешНастроек = "")
	...
	Запрос.Текст =
		"ВЫБРАТЬ
		|	МакетыКомпоновкиДанных.МакетКомпоновкиДанных КАК МакетКомпоновкиДанных,
		|	МакетыКомпоновкиДанных.ВерсияКонфигурации КАК ВерсияКонфигурации
		|ИЗ
		...
		|	И МакетыКомпоновкиДанных.ХешНастроек = &ХешНастроек";
...
Функция ДобавитьМакетКомпоновкиДанныхОтчета(Отчет, ИмяОтчета, СвойстваВарианта, ОтчетОбъект = Неопределено, НастройкиДобавляемогоВарианта = Неопределено)
	...
	НастройкиПечатнойФормы = НастройкиПечатнойФормы(Отчет, СвойстваВарианта.КлючВарианта);
	Если НастройкиПечатнойФормы <> Неопределено Тогда
		...
		ИнициализироватьНастройкиОтчета(ОтчетОбъект, "", СвойстваВарианта.КлючВарианта, НастройкиОтчетаКД, ПользовательскиеНастройкиКД);
		
		Если НастройкиОтчетаКД = Неопределено Тогда
			НастройкиОтчетаКД = ОтчетОбъект.СхемаКомпоновкиДанных.ВариантыНастроек[СвойстваВарианта.КлючВарианта].Настройки;
		КонецЕсли;
	...
	Если НастройкиДобавляемогоВарианта <> Неопределено Тогда
		ОтчетОбъект.КомпоновщикНастроек.ЗагрузитьНастройки(НастройкиДобавляемогоВарианта);
	КонецЕсли;
	...
	ЗапомнитьМакетКомпоновкиДанных(Отчет, СвойстваВарианта.КлючВарианта, МакетКомпоновкиДанных, СвойстваВарианта.ХешНастроек);
...
Процедура ЗапомнитьМакетКомпоновкиДанных(Отчет, КлючВарианта, МакетКомпоновкиДанных, ХешНастроек = "") Экспорт
	...
	Попытка
		...
		Запись.ХешНастроек = ХешНастроек;
...
Процедура ПриКомпоновкеРезультатаРасчетныйЛистокПоФизическимЛицам(ОбъектОтчета, ДокументРезультат, ДанныеРасшифровки, СтандартнаяОбработка, НаАванс = Ложь)
	...
	Попытка
		...
		Если ПредварительныйПросмотр И ПолучитьФункциональнуюОпцию("ОтключенныйФункционалЗарплатаКадры") Тогда
		...
		Если НастройкиВывода.ВыводитьОсобенностиРасчетаНДФЛ
            // ЗУПК
			Или (НЕ ПолучитьФункциональнуюОпцию("ОтключенныйФункционалЗарплатаКадры")
				И НЕ УправленческийРасчетныйЛисток)
            // ЗУПК
	...
	Если НастройкиВывода.ВыводитьОсобенностиРасчетаНДФЛ И СтрокаДанныхПоНДФЛ <> Неопределено
		...
		Если НастройкиВывода.ВыводитьИнформациюОНачисленныхВзносахВПФР Тогда
			...
			Если ВсегоВзносов <> 0 Тогда
				...
				Если СтрокаДанныхПоНДФЛ.МесяцНачисления < УчетСтраховыхВзносов.ДатаОбъединенияВзносов() Тогда
					ТекстСекции = ТекстСекции
						+ СтрокаСНепереносимымиПробелами(НСтр("ru='Страховые взносы в ПФР'") + ": " + Формат(ВсегоВзносов, "ЧДЦ=2; ЧН="));
				Иначе
					ТекстСекции = ТекстСекции
						+ СтрокаСНепереносимымиПробелами(НСтр("ru='Взносы по единому тарифу'") + ": " + Формат(ВсегоВзносов, "ЧДЦ=2; ЧН="));
				КонецЕсли;
...
Функция ВывестиТелоРасчетногоЛистка(ДокументРезультат, Макет, СтрокиГоловныхСотрудников, СоответствиеПользовательскихПолей, ДанныеПечатнойФормы, ДополнительныеПараметры)
	...
	Для каждого СтрокаГоловногоСотрудника Из СтрокиГоловныхСотрудников Цикл
		...
		Для каждого СтрокаСотрудника Из СтрокаГоловногоСотрудника.Строки Цикл
					...
					Если НастройкиВывода.ВыводитьПериодыНачислений Тогда
						...
						Если СтрокаНачислений.ПериодДействия = СтрокаНачислений.МесяцНачисления Тогда
							...
							Иначе
								...
								Если СтрокаНачислений.Сумма <> 0
									Или НастройкиВывода.ВыводитьОтработанноеОплаченноеВремя
										И (СтрокаНачислений.ОтработаноДней <> 0 
											Или СтрокаНачислений.ОтработаноЧасов <> 0
											Или СтрокаНачислений.ОплаченоДней <> 0 
											Или СтрокаНачислений.ОплаченоЧасов <> 0) Тогда
			...
			Если НастройкиВывода.ВыводитьИнформациюОЛьготах Тогда
				...
				// ЗУПК
				ДанныеПоНДФЛИВзносам = ДополнительныеПараметры.ДанныеПоНДФЛИВзносам.Получить(СтрокаСотрудника.МесяцНачисления);
				Если НЕ ДанныеПоНДФЛИВзносам = Неопределено Тогда
	                Если СтрокаСотрудника.ДатаУвольнения = NULL ИЛИ СтрокаСотрудника.ДатаУвольнения = '00010101' 
	                    ИЛИ СтрокаСотрудника.ДатаУвольнения >= СтрокаСотрудника.МесяцНачисления Тогда
						
                        Если СтрокаСотрудника.МесяцНачисления < Дата("20180101") Тогда
							КорректировкаИПН = ДанныеПоНДФЛИВзносам.ИПН.НайтиСтроки(Новый Структура("ФизическоеЛицо", СтрокаСотрудника.ФизическоеЛицо));
						Иначе	
							КорректировкаИПН = ДанныеПоНДФЛИВзносам.ИПН.НайтиСтроки(Новый Структура("ФизическоеЛицо, Подразделение", СтрокаСотрудника.ФизическоеЛицо, СтрокаСотрудника.ПодразделениеГоловногоСотрудникаНаКонецПериода));
                        КонецЕсли;
							
	                    Для каждого СтрокаКорректировки Из КорректировкаИПН Цикл
	                        
	                        ВидРасчета = ?(СтрокаСотрудника.МесяцНачисления >= '20210101', НСтр("ru = 'Сумма уменьшения облагаемого дохода'"), НСтр("ru = 'Примененная корректировка'"));
	                        
	                        Если НЕ ЗначениеЗаполнено(СтрокаКорректировки.СуммаУменьшенияОблагаемогоДохода) Тогда
	                            Продолжить;	
	                        КонецЕсли;
	                        
	                        СтрокаКоллекции = ИзвестныеСтрокиЛьготы.Получить(ВидРасчета);
	                        
	                        Если СтрокаКоллекции = Неопределено Тогда
	                            
	                            СтрокаКоллекции = Новый Структура("ВидРасчета,Сумма");
	                            СтрокаКоллекции.ВидРасчета = ?(СтрокаСотрудника.МесяцНачисления >= '20210101', НСтр("ru = 'Сумма уменьшения облагаемого дохода'"), НСтр("ru = 'Примененная корректировка'"));
	                            СтрокаКоллекции.Сумма = СтрокаКорректировки.СуммаУменьшенияОблагаемогоДохода;
	                            КоллекцииРазделов.Льготы.Добавить(СтрокаКоллекции);
	                            ИзвестныеСтрокиЛьготы.Вставить(ВидРасчета, СтрокаКоллекции);
	                        Иначе
	                            СтрокаКоллекции.Сумма = СтрокаКоллекции.Сумма + СтрокаКорректировки.СуммаУменьшенияОблагаемогоДохода;
	                        КонецЕсли;
	                        
	                    КонецЦикла;
	                    
	                    ДанныеВычеты = ДанныеПоНДФЛИВзносам.Вычеты.НайтиСтроки(Новый Структура("ФизическоеЛицо, Подразделение", СтрокаСотрудника.ФизическоеЛицо, СтрокаСотрудника.ПодразделениеГоловногоСотрудникаНаКонецПериода));
	                    
	                    Для каждого СтрокаВычет Из ДанныеВычеты Цикл
	                        
	                        СтрокаКоллекции = ИзвестныеСтрокиЛьготы.Получить(СтрокаВычет.ВидВычета);
	                        Если СтрокаКоллекции = Неопределено Тогда
	                            СтрокаКоллекции = Новый Структура("ВидРасчета,Сумма");
	                            СтрокаКоллекции.ВидРасчета = СтрокаВычет.ВидВычета;
	                            СтрокаКоллекции.Сумма = СтрокаВычет.СуммаВычета;
	                            КоллекцииРазделов.Льготы.Добавить(СтрокаКоллекции);
	                            ИзвестныеСтрокиЛьготы.Вставить(СтрокаВычет.ВидВычета, СтрокаКоллекции);
							ИначеЕсли СтрокаКоллекции.ВидРасчета = СтрокаВычет.ВидВычета И
								СтрокаКоллекции.Сумма = СтрокаВычет.СуммаВычета Тогда
								Продолжить;
	                        Иначе
	                            СтрокаКоллекции.Сумма = СтрокаКоллекции.Сумма + СтрокаВычет.СуммаВычета;
	                        КонецЕсли;
	...
	Для Каждого СтрокаОтчетаНачислено Из СтрокиОтчетаНачислено Цикл
		...
		ДобавитьСтрокуДанныхРасчетногоЛистка(ДанныеРасчетныхЛистков, СтрокаОтчетаНачислено);
		
	КонецЦикла;
	
	Для Каждого СтрокаВидаРасчета Из СтрокиОтчетаУдержано Цикл
		ДобавитьСтрокуДанныхРасчетногоЛистка(ДанныеРасчетныхЛистков, СтрокаВидаРасчета, СотрудникОтчета);
	КонецЦикла;
...
Процедура ДобавитьСтрокуДанныхРасчетногоЛистка(ДанныеРасчетныхЛистков, СтрокаОтчета, СотрудникОтчета = Неопределено)
	
	Если ДанныеРасчетныхЛистков.Количество() > 0 Тогда
		
		Если Не ЗначениеЗаполнено(СтрокаОтчета.Сумма)
			И ЗначениеЗаполнено(СтрокаОтчета.ВидРасчета)
			И СтрокаОтчета.ВидРасчета = ДанныеРасчетныхЛистков[ДанныеРасчетныхЛистков.Количество() - 1].ВидРасчета Тогда
	...
	КонецЕсли;
	СтрокаДанныхРасчетногоЛистка = ДанныеРасчетныхЛистков.Добавить();
	ЗаполнитьЗначенияСвойств(СтрокаДанныхРасчетногоЛистка, СтрокаОтчета);
	...
	Если СотрудникОтчета <> Неопределено Тогда
		СтрокаДанныхРасчетногоЛистка.Сотрудник = СотрудникОтчета;
	КонецЕсли;
...
Функция ПредставлениеВыплаты(ВидВзаиморасчетов, СвойстваРегистратора)
	...
	Иначе
		Представление = ПредставлениеВидаВзаиморасчетов(ВидВзаиморасчетов);
	КонецЕсли;
...
Функция ПредставлениеЗначения(Значение, ПредставлениеПоУмолчанию) Экспорт
...
Функция ПредставлениеВидаВзаиморасчетов(ВидВзаиморасчетов)

	Если Перечисления.ВидыВзаиморасчетовССотрудниками.ЭтоВозврат(ВидВзаиморасчетов) Тогда
		Возврат НСтр("ru='Отказ банка в зачислении'");
	Иначе
	...
	Для Каждого СтрокаДанных Из СтрокиДанных Цикл
		...
		Если СтрокаВидаРасчета Тогда
			...
			Если НастройкиВывода.ВыводитьОснованияНачисленийИУдержаний
				...
				Если НастройкиВывода.ВыводитьРасшифровкиОднойСтрокой Тогда
					СтрокаДанныхРасшифровки = СтрокаДанныхИзСоответствияСтрок(
						СоответствиеСтрокРазделаСтрокамДанных,
						СтрокаДанных,
						ИменаИзмерений);
					Если ЗначениеЗаполнено(СтрокаДанныхРасшифровки) Тогда
						РасшифровкиВидаРасчета.Добавить(
							РасшифровкаПоСтроке(
								СтрокаДанныхРасшифровки,
								СоответствиеПользовательскихПолей,
								Макет,
...
Функция НастройкиВыводаРасчетногоЛистка(НастройкиОтчета, КлючВарианта, ПредварительныйПросмотр = Ложь, НаАванс = Ложь)
	...
	Иначе
		НастройкиВывода.Вставить(ИмяПараметра, Ложь);
	КонецЕсли;
...
Процедура ПриКомпоновкеРезультатаРасчетныйЛистокПоРабочимМестам(ОбъектОтчета, ДокументРезультат, ДанныеРасшифровки, СтандартнаяОбработка, НаАванс = Ложь)
	...
	Попытка
		...
		Для каждого СтрокаМесяца Из ДанныеОтчета.Строки Цикл
			...
			// ЗУПК
			ДанныеПоНДФЛИВзносам = ДанныеПоНДФЛИВзносамПоРабочимМестам(СтрокаМесяца.Строки, СтрокаМесяца.МесяцНачисления, Ложь, Ложь);
			
			//Вывод нового варианта вычетов
			НовыйВариантОтчета = Ложь;
			Если СтрокаМесяца.МесяцНачисления >= '2018-01-01' Тогда
				Макеты.Вставить("Шапка1_2018_Сотрудник",Макет.ПолучитьОбласть("Шапка1_2018_Сотрудник | Сотрудник_2018"));
				Макеты.Вставить("Шапка1_2018_ПустаяСтрока",Макет.ПолучитьОбласть("Шапка1_2018_ПустаяСтрока | Сотрудник_2018"));
				Макеты.Вставить("Шапка1_2018_Период",Макет.ПолучитьОбласть("Шапка1_2018_Период"));
				Макеты.Вставить("Шапка1_2018_ВычетыШапка",Макет.ПолучитьОбласть("Шапка1_2018_ВычетыШапка | Вычеты_2018"));
				Макеты.Вставить("Шапка1_2018_ВычетыСтрока",Макет.ПолучитьОбласть("Шапка1_2018_ВычетыСтрока | Вычеты_2018"));
				Если СтрокаМесяца.МесяцНачисления >= '2021-01-01' Тогда
					Макеты.Вставить("Шапка1_2021_СуммаУменьшенияОблагаемогоДохода",Макет.ПолучитьОбласть("Шапка1_2021_СуммаУменьшенияОблагаемогоДохода | Вычеты_2018"));
				ИначеЕсли СтрокаМесяца.МесяцНачисления >= '2019-01-01' Тогда
					Макеты.Вставить("Шапка1_2019_КорректировкаДохода",Макет.ПолучитьОбласть("Шапка1_2019_КорректировкаДохода | Вычеты_2018"));
				КонецЕсли;
				НовыйВариантОтчета = Истина;
			КонецЕсли;
			// ЗУПК
			...
			Для каждого СтрокаКоллекции Из КоллекцияДанных Цикл
				...
				ВывестиСотрудникаРасчетныйЛистокПоРабочимМестам(ВыводимаяКоллекция, ДокументРезультат, Макеты, СоответствиеПользовательскихПолей, ДанныеПоНДФЛИВзносам, НастройкиВывода.ГруппироватьПоИсточникамФинансирования, НовыйВариантОтчета);
...
Процедура ВывестиСотрудникаРасчетныйЛистокПоРабочимМестам(СтрокиГоловныхСотрудников, ДокументРезультатПромежуточный, Макеты, СоответствиеПользовательскихПолей, ДанныеПоНДФЛИВзносам, ГруппироватьПоИсточникамФинансирования, НовыйВариант = Ложь)
	...
	Если НовыйВариант Тогда
		КоличествоСтрокОграничения = 4;
		//Период
		Макеты.Шапка1_2018_Период.Параметры.ПериодПредставление = Формат(СтрокаШапки.МесяцНачисления, "ДФ='ММММ гггг'");
		ДокументРезультат.Вывести(Макеты.Шапка1_2018_Период);
		
		//Шапка
		Макеты.Шапка1_2018_Сотрудник.Параметры.Заполнить(СтрокаШапки);
		ЗарплатаКадрыОтчеты.ЗаполнитьПараметрыПользовательскихПолей(Макеты.Шапка1_2018_Сотрудник, СтрокаШапки, СоответствиеПользовательскихПолей);
		
		Если СтрокаПервогоГоловногоСотрудника.Владелец().Колонки.Найти("РазрядКатегорияГоловногоСотрудникаНаКонецПериода") <> Неопределено
			И ЗначениеЗаполнено(СтрокаПервогоГоловногоСотрудника.РазрядКатегорияГоловногоСотрудникаНаКонецПериода) Тогда
			Макеты.Шапка1_2018_Сотрудник.Параметры.РазрядКатегорияСотрудника = ", " + СтрокаПервогоГоловногоСотрудника.РазрядКатегорияГоловногоСотрудникаНаКонецПериода;
		КонецЕсли; 
		
		// Заполнение параметров макета
		Макеты.Шапка1_2018_Сотрудник.Параметры.ФизическоеЛицоФИО = СтрокаШапки.ФизическоеЛицоФИО;
		Макеты.Шапка1_2018_Сотрудник.Параметры.СотрудникКод      = СтрокаШапки.СотрудникКод;
		
		//ВычетыШапка
		ЗарплатаКадрыОтчеты.ЗаполнитьПараметрыПользовательскихПолей(Макеты.Шапка1_2018_ВычетыШапка, СтрокаПервогоГоловногоСотрудника, СоответствиеПользовательскихПолей, "КВыплате");
		Если НЕ ЗначениеЗаполнено(Макеты.Шапка1_2018_ВычетыШапка.Параметры.КВыплате) ИЛИ Макеты.Шапка1_2018_ВычетыШапка.Параметры.КВыплате < 0 Тогда
			Макеты.Шапка1_2018_ВычетыШапка.Параметры.КВыплате = 0;
		КонецЕсли;
		
		//Вычеты
		ВремТабДокВычеты = Новый ТабличныйДокумент;
		
		ДанныеВычеты = ДанныеПоНДФЛИВзносам.Вычеты.НайтиСтроки(Новый Структура("ФизическоеЛицо, Подразделение",СтрокаШапки.ФизическоеЛицо, СтрокаШапки.ПодразделениеГоловногоСотрудникаНаКонецПериода));
		
		Макеты.Шапка1_2018_ВычетыШапка.Параметры.ВычетПредоставляется = Формат(ДанныеВычеты.Количество() > 0,"Л=ru_RU; БЛ='Вычеты не предоставляются'; БИ=''");
		
		ВремТабДокВычеты.Вывести(Макеты.Шапка1_2018_ВычетыШапка);
		
		КоличествоСтрок = 0;
		Для каждого СтрокаВычет Из ДанныеВычеты Цикл
			
			Макеты.Шапка1_2018_ВычетыСтрока.Параметры.Заполнить(СтрокаВычет);
			ВремТабДокВычеты.Вывести(Макеты.Шапка1_2018_ВычетыСтрока);
			КоличествоСтрок = КоличествоСтрок + 1;
			Если КоличествоСтрок = КоличествоСтрокОграничения Тогда
				Прервать;
			КонецЕсли;
		КонецЦикла;
		
		СтрокаДанныхПоИПН = ДанныеПоНДФЛИВзносам.ИПН.НайтиСтроки(Новый Структура("ФизическоеЛицо, Подразделение",СтрокаШапки.ФизическоеЛицо, СтрокаШапки.ПодразделениеГоловногоСотрудникаНаКонецПериода));
		Если СтрокаДанныхПоИПН.Количество() > 0 И Макеты.Свойство("Шапка1_2021_СуммаУменьшенияОблагаемогоДохода") Тогда
			Если ЗначениеЗаполнено(СтрокаДанныхПоИПН[0].СуммаУменьшенияОблагаемогоДохода) Тогда
				Макеты.Шапка1_2021_СуммаУменьшенияОблагаемогоДохода.Параметры.Заполнить(СтрокаДанныхПоИПН[0]);
				ВремТабДокВычеты.Вывести(Макеты.Шапка1_2021_СуммаУменьшенияОблагаемогоДохода);
			КонецЕсли;
		ИначеЕсли СтрокаДанныхПоИПН.Количество() > 0 И Макеты.Свойство("Шапка1_2019_КорректировкаДохода") Тогда
			Если ЗначениеЗаполнено(СтрокаДанныхПоИПН[0].ПримененнаяКорректировка) Тогда
				Макеты.Шапка1_2019_КорректировкаДохода.Параметры.Заполнить(СтрокаДанныхПоИПН[0]);
				ВремТабДокВычеты.Вывести(Макеты.Шапка1_2019_КорректировкаДохода);
			КонецЕсли;
		КонецЕсли; 

		ДокументРезультат.Вывести(Макеты.Шапка1_2018_Сотрудник);
		ДокументРезультат.Присоединить(ВремТабДокВычеты);
		
		//Выводим оставшиеся вычеты
		Если ДанныеВычеты.Количество() > КоличествоСтрокОграничения Тогда
			
			Для Индекс = КоличествоСтрокОграничения + 1  По ДанныеВычеты.Количество() Цикл
				
				ДокументРезультат.Вывести(Макеты.Шапка1_2018_ПустаяСтрока);
				Макеты.Шапка1_2018_ВычетыСтрока.Параметры.Заполнить(ДанныеВычеты[Индекс - 1]);
				ДокументРезультат.Присоединить(Макеты.Шапка1_2018_ВычетыСтрока);
	...
	Иначе
		...
		КонецЕсли;
		// Заполнение параметров макета
		Макеты.Шапка1.Параметры.ФизическоеЛицоФИО = СтрокаШапки.ФизическоеЛицоФИО;
		Макеты.Шапка1.Параметры.СотрудникКод      = СтрокаШапки.СотрудникКод;
		
		СтрокаДанныхПоИПН = ДанныеПоНДФЛИВзносам.ИПН.Найти(СтрокаШапки.ФизическоеЛицо, "ФизическоеЛицо");
		Если СтрокаДанныхПоИПН <> Неопределено Тогда
			Макеты.Шапка1.Параметры.Заполнить(СтрокаДанныхПоИПН);
		КонецЕсли; 
...
Функция ДанныеПоНДФЛИВзносамПоРабочимМестам(СтрокиСотрудников, МесяцНачисления, ВыводитьОсобенностиРасчетаНДФЛ, ВыводитьИнформациюОНачисленныхВзносахВПФР)
	...
	Сотрудники.Колонки.Добавить("Подразделение", Новый ОписаниеТипов("СправочникСсылка.ПодразделенияОрганизаций"));
	Сотрудники.Колонки.Добавить("ФизическоеЛицо", Новый ОписаниеТипов("СправочникСсылка.ФизическиеЛица"));
	Сотрудники.Колонки.Добавить("ГоловнаяОрганизация", Новый ОписаниеТипов("СправочникСсылка.Организации"));
	Сотрудники.Колонки.Добавить("Организация", Новый ОписаниеТипов("СправочникСсылка.Организации"));
	
	Для Каждого СтрокаДерева Из СтрокиСотрудников Цикл
		НоваяСтрока = Сотрудники.Добавить();
		НоваяСтрока.Сотрудник = СтрокаДерева.ГоловнойСотрудник;
		НоваяСтрока.ГоловнаяОрганизация = ЗарплатаКадры.ГоловнаяОрганизация(СтрокаДерева.Организация);
		НоваяСтрока.Организация = СтрокаДерева.Организация;
		НоваяСтрока.Подразделение = СтрокаДерева.ПодразделениеГоловногоСотрудникаНаКонецПериода;
		НоваяСтрока.ФизическоеЛицо = СтрокаДерева.ФизическоеЛицо;
	КонецЦикла;
	
	Запрос.УстановитьПараметр("СотрудникиСПодразделениями", Сотрудники);
	
	Запрос.Текст =
		"ВЫБРАТЬ РАЗЛИЧНЫЕ
		|	ВТ_СотрудникиСПодразделениями.Сотрудник КАК Сотрудник,
		|	ВТ_СотрудникиСПодразделениями.ГоловнаяОрганизация КАК ГоловнаяОрганизация,
		|	ВТ_СотрудникиСПодразделениями.ФизическоеЛицо КАК ФизическоеЛицо,
		|	ВТ_СотрудникиСПодразделениями.Организация КАК Организация,
		|	ВТ_СотрудникиСПодразделениями.Подразделение КАК Подразделение
		|ПОМЕСТИТЬ ВТ_СотрудникиСПодразделениями
		|ИЗ
		|	&СотрудникиСПодразделениями КАК ВТ_СотрудникиСПодразделениями
		|
		|ИНДЕКСИРОВАТЬ ПО
		|	Сотрудник,
		|	ФизическоеЛицо,
		...
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ РАЗЛИЧНЫЕ
		|	СотрудникиСПодразделениями.Сотрудник КАК Сотрудник,
		|	СотрудникиСПодразделениями.ФизическоеЛицо КАК ФизическоеЛицо,
		|	СотрудникиСПодразделениями.ГоловнаяОрганизация КАК ГоловнаяОрганизация
		|ПОМЕСТИТЬ ВТ_СотрудникиСовместители
		|ИЗ
		|	ВТ_СотрудникиСПодразделениями КАК СотрудникиСПодразделениями
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВТ_СотрудникиСПодразделениями КАК СотрудникиСПодразделениямиСовместители
		|		ПО СотрудникиСПодразделениями.ФизическоеЛицо = СотрудникиСПодразделениямиСовместители.ФизическоеЛицо
		|			И СотрудникиСПодразделениями.Сотрудник <> СотрудникиСПодразделениямиСовместители.Сотрудник
		|			И СотрудникиСПодразделениями.Подразделение <> СотрудникиСПодразделениямиСовместители.Подразделение";
...
Функция ДанныеПоНДФЛИВзносамПоФизическимЛицам(СтрокиСотрудников, МесяцНачисления, ВыводитьОсобенностиРасчетаНДФЛ, ВыводитьИнформациюОНачисленныхВзносахВПФР)
	...
	// ЗУПК
	ФизическиеЛица.Колонки.Добавить("Подразделение", Новый ОписаниеТипов("СправочникСсылка.ПодразделенияОрганизаций"));
	ФизическиеЛица.Колонки.Добавить("Организация", Новый ОписаниеТипов("СправочникСсылка.Организации"));
	// ЗУПК
	...
	Для Каждого СтрокаСотрудника Из СтрокиСотрудниковМесяца Цикл
		...
		// ЗУПК
		НоваяСтрокаФизическиеЛица.Подразделение = СтрокаСотрудника.ПодразделениеСортировки;
		НоваяСтрокаФизическиеЛица.Организация = СтрокаСотрудника.ОрганизацияСортировки;
		// ЗУПК
	...
	Запрос.УстановитьПараметр("СотрудникиСПодразделениями", ФизическиеЛица);
...
Функция ДанныеПоНДФЛИВзносам(МенеджерВременныхТаблиц, МесяцНачисления, ВыводитьОсобенностиРасчетаНДФЛ, ВыводитьИнформациюОНачисленныхВзносахВПФР)
	...
	Если ВыводитьОсобенностиРасчетаНДФЛ Тогда
		...
		Запрос.Текст = 
			...
			|	СУММА(СведенияОДоходахНДФЛОбороты.СуммаДохода - ВЫБОР
			|			КОГДА СведенияОДоходахНДФЛОбороты.КодВычета = ЗНАЧЕНИЕ(Справочник.ВидыВычетовНДФЛ.ПустаяСсылка)
			|				ТОГДА 0
			|			ИНАЧЕ СведенияОДоходахНДФЛОбороты.СуммаВычета
			...
			|ГДЕ
			|	СведенияОДоходахНДФЛОбороты.Период <= &КонецПериода
			|	И СведенияОДоходахНДФЛОбороты.МесяцНалоговогоПериода МЕЖДУ &НачалоГода И &КонецПериода
			|	И &УсловиеОблагаемостиУНалоговогоАгента
			|	И СведенияОДоходахНДФЛОбороты.ФизическоеЛицо В
			|			(ВЫБРАТЬ
			|				ФизическиеЛица.ФизическоеЛицо
			|			ИЗ
		...
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "&УсловиеОблагаемостиУНалоговогоАгента", УчетНДФЛ.УсловиеОблагаемостиУНалоговогоАгента("СведенияОДоходахНДФЛОбороты"));
		Запрос.Выполнить();
	...
	Если ВыводитьИнформациюОНачисленныхВзносахВПФР Тогда
		...
		Запрос.Текст = 
			...
			|	ИсчисленныеСтраховыеВзносыОбороты.ГоловнаяОрганизация КАК ГоловнаяОрганизация,
			|	ИсчисленныеСтраховыеВзносыОбороты.ФизическоеЛицо КАК ФизическоеЛицо,
			|	СУММА(&ПоляВзносов) КАК ПоляВзносов
			|ИЗ
			|	РегистрНакопления.ИсчисленныеСтраховыеВзносы КАК ИсчисленныеСтраховыеВзносыОбороты
			|ГДЕ
			|	ИсчисленныеСтраховыеВзносыОбороты.Период МЕЖДУ &НачалоПериода И &КонецПериода
			|	И ИсчисленныеСтраховыеВзносыОбороты.ФизическоеЛицо В
			|			(ВЫБРАТЬ
			|				ФизическиеЛица.ФизическоеЛицо
			|			ИЗ
			|				ВТФизическиеЛица КАК ФизическиеЛица)
		...
		Для каждого ПолеВзносов Из  ПоляВзносов Цикл
			...
			ТекстЗапросаСуммПолей = ТекстЗапросаСуммПолей + "СУММА(ИсчисленныеСтраховыеВзносыОбороты." + ПолеВзносов + ") КАК " + ПолеВзносов;
	...
	Запрос.Текст =
		...
		|	СотрудникиСПодразделениями.ФизическоеЛицо КАК ФизическоеЛицо,
		|	СотрудникиСПодразделениями.ГоловнаяОрганизация КАК ГоловнаяОрганизация,
		|	СотрудникиСПодразделениями.Организация КАК Организация,
		|	СотрудникиСПодразделениями.Подразделение КАК Подразделение,
		|	ВЫБОР КОГДА СотрудникиСовместители.Сотрудник ЕСТЬ NULL ТОГДА ЛОЖЬ ИНАЧЕ ИСТИНА КОНЕЦ КАК Совместитель
		|ПОМЕСТИТЬ ВТСотрудникиИПН
		|ИЗ
		|	ВТ_СотрудникиСПодразделениями КАК СотрудникиСПодразделениями
		|		ЛЕВОЕ СОЕДИНЕНИЕ ВТ_СотрудникиСовместители КАК СотрудникиСовместители
		|		ПО СотрудникиСПодразделениями.Сотрудник = СотрудникиСовместители.Сотрудник
		|
		|ИНДЕКСИРОВАТЬ ПО
		|	МесяцНачисления,
		|	ГоловнаяОрганизация,
		...
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ РАЗРЕШЕННЫЕ
		|	СведенияОбИсчисленииНалоговВзносовОтчисленийОбороты.Период КАК МесяцНачисления,
		|	СведенияОбИсчисленииНалоговВзносовОтчисленийОбороты.Организация КАК ГоловнаяОрганизация,
		|	СведенияОбИсчисленииНалоговВзносовОтчисленийОбороты.ФизическоеЛицо КАК ФизическоеЛицо,
		|	СведенияОбИсчисленииНалоговВзносовОтчисленийОбороты.Подразделение КАК Подразделение,
		|	СведенияОбИсчисленииНалоговВзносовОтчисленийОбороты.ПримененныйВычетОборот КАК ВычетИПН,
		|	ВЫБОР
		|		КОГДА СведенияОбИсчисленииНалоговВзносовОтчисленийОбороты.Период < ДАТАВРЕМЯ (2019, 1, 1)
		|			ТОГДА СведенияОбИсчисленииНалоговВзносовОтчисленийОбороты.ПримененнаяЛьготаОборот
		|		ИНАЧЕ 0
		|	КОНЕЦ КАК ЛьготаИПН,
		|	ВЫБОР
		|		КОГДА СведенияОбИсчисленииНалоговВзносовОтчисленийОбороты.Период >= ДАТАВРЕМЯ (2019, 1, 1)
		...
		|	ВЫБОР
		|		КОГДА СведенияОбИсчисленииНалоговВзносовОтчисленийОбороты.Период >= ДАТАВРЕМЯ (2021, 1, 1)
		|			ТОГДА СведенияОбИсчисленииНалоговВзносовОтчисленийОбороты.ПримененнаяЛьготаОборот
		|		ИНАЧЕ 0
		|	КОНЕЦ КАК СуммаУменьшенияОблагаемогоДохода,
		|	СведенияОбИсчисленииНалоговВзносовОтчисленийОбороты.ПрочиеВычетыОборот КАК ПрочиеВычеты
		|ПОМЕСТИТЬ ВТВычетыИПН
		|{ВЫБРАТЬ
		|	МесяцНачисления,
		|	ГоловнаяОрганизация.*,
		|	ФизическоеЛицо.*,
		...
		|	ВычетИПН,
		|	ЛьготаИПН,
		|	ПримененнаяКорректировка,
		|	СуммаУменьшенияОблагаемогоДохода,
		|	ПрочиеВычеты}
		|ИЗ
		|	РегистрНакопления.СведенияОбИсчисленииНалоговВзносовОтчислений.Обороты(&НачалоПериода, &КонецПериода, Месяц, 
		|			ВидНалогаВзносаОтчисления = ЗНАЧЕНИЕ(Перечисление.ВидыНалоговВзносовОтчислений.ИПН)
		|				И (Организация, ФизическоеЛицо) В
		|				(ВЫБРАТЬ
		|					ВТСотрудникиИПН.ГоловнаяОрганизация,
		|					ВТСотрудникиИПН.ФизическоеЛицо
		|				ИЗ
		|					ВТСотрудникиИПН)) КАК СведенияОбИсчисленииНалоговВзносовОтчисленийОбороты
		|{ГДЕ
		|	СведенияОбИсчисленииНалоговВзносовОтчисленийОбороты.Организация.*,
		...
		|	Сотрудники.ФизическоеЛицо," + ?(МесяцНачисления >= '2018-01-01',"
		|	Сотрудники.Подразделение,","")+ "
		|	СУММА(Вычеты.ВычетИПН) КАК ВычетИПН,
		|	СУММА(Вычеты.ЛьготаИПН) КАК ЛьготаИПН,
		|	СУММА(Вычеты.ПримененнаяКорректировка) КАК ПримененнаяКорректировка,
		|	СУММА(Вычеты.СуммаУменьшенияОблагаемогоДохода) КАК СуммаУменьшенияОблагаемогоДохода,
		|	СУММА(Вычеты.ПрочиеВычеты) КАК ПрочиеВычеты
		|ИЗ
		|	ВТСотрудникиИПН КАК Сотрудники
		|		ЛЕВОЕ СОЕДИНЕНИЕ ВТВычетыИПН КАК Вычеты
		|		ПО Сотрудники.МесяцНачисления = Вычеты.МесяцНачисления
		|			И Сотрудники.ГоловнаяОрганизация = Вычеты.ГоловнаяОрганизация
		|			И Сотрудники.ФизическоеЛицо = Вычеты.ФизическоеЛицо" + ?(МесяцНачисления >= '2018-01-01',"
		|			И ВЫБОР КОГДА Сотрудники.Совместитель ТОГДА Сотрудники.Подразделение = Вычеты.Подразделение ИНАЧЕ ИСТИНА КОНЕЦ","")+ "
		|
		|СГРУППИРОВАТЬ ПО
		|	Сотрудники.МесяцНачисления,
		|	Сотрудники.ФизическоеЛицо" + ?(МесяцНачисления >= '2018-01-01',",
	...
	ВозвращаемыеДанные.Вставить("ИПН", Запрос.Выполнить().Выгрузить());
	...
	|	СведенияОбИсчисленииНалоговВзносовОтчисленийОбороты.Организация КАК ГоловнаяОрганизация,
	|	СведенияОбИсчисленииНалоговВзносовОтчисленийОбороты.ФизическоеЛицо КАК ФизическоеЛицо,
	|	СведенияОбИсчисленииНалоговВзносовОтчисленийОбороты.НалогВзносОборот КАК ОППВ
	|ПОМЕСТИТЬ ВТВычетыОППВ
	|{ВЫБРАТЬ
	|	МесяцНачисления,
	|	ГоловнаяОрганизация.*,
	|	ФизическоеЛицо.*}
	|ИЗ
	|	РегистрНакопления.СведенияОбИсчисленииНалоговВзносовОтчислений.Обороты(&НачалоПериода, &КонецПериода, Месяц, 
	|			ВидНалогаВзносаОтчисления = ЗНАЧЕНИЕ(Перечисление.ВидыНалоговВзносовОтчислений.ОППВ)
	|				И (Организация, ФизическоеЛицо) В
	|				(ВЫБРАТЬ
	|					ВТСотрудникиОППВ.ГоловнаяОрганизация,
	|					ВТСотрудникиОППВ.ФизическоеЛицо
	|				ИЗ
	|					ВТСотрудникиОППВ)) КАК СведенияОбИсчисленииНалоговВзносовОтчисленийОбороты
	|{ГДЕ
	|	СведенияОбИсчисленииНалоговВзносовОтчисленийОбороты.Организация.*,
	|	СведенияОбИсчисленииНалоговВзносовОтчисленийОбороты.ФизическоеЛицо.*,
	|	СведенияОбИсчисленииНалоговВзносовОтчисленийОбороты.Период КАК МесяцНачисления}
	...
	|	Вычеты.ОППВ
	|ИЗ
	|	ВТСотрудникиОППВ КАК Сотрудники
	|		ЛЕВОЕ СОЕДИНЕНИЕ ВТВычетыОППВ КАК Вычеты
	|		ПО Сотрудники.МесяцНачисления = Вычеты.МесяцНачисления
	|			И Сотрудники.ГоловнаяОрганизация = Вычеты.ГоловнаяОрганизация
	|			И Сотрудники.ФизическоеЛицо = Вычеты.ФизическоеЛицо";
	
	ВозвращаемыеДанные.Вставить("ОППВ", Запрос.Выполнить().Выгрузить());
	...
	Запрос.Текст = "ВЫБРАТЬ РАЗРЕШЕННЫЕ РАЗЛИЧНЫЕ
		...
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ РАЗРЕШЕННЫЕ
		|	СведенияОбИсчисленииВычетовИПН.Период КАК МесяцНачисления,
		|	СведенияОбИсчисленииВычетовИПН.Организация КАК Организация,
		|	СведенияОбИсчисленииВычетовИПН.ФизическоеЛицо КАК ФизическоеЛицо,
		|	СведенияОбИсчисленииВычетовИПН.Подразделение КАК Подразделение,
		|	СведенияОбИсчисленииВычетовИПН.ВидВычета КАК ВидВычета,
		|	СведенияОбИсчисленииВычетовИПН.ПримененныйВычетОборот КАК СуммаВычета,
		|	СведенияОбИсчисленииВычетовИПН.ВидВычета.Приоритет КАК ВидВычетаПриоритет
		|ПОМЕСТИТЬ ВТВычеты
		|{ВЫБРАТЬ
		|	МесяцНачисления,
		|	Организация.*,
		|	ФизическоеЛицо.*}
		|ИЗ
		|	РегистрНакопления.СведенияОбИсчисленииВычетовИПН.Обороты(&НачалоПериода, &КонецПериода, Месяц,
		|				(Организация, ФизическоеЛицо, ВидДохода) В
		|				(ВЫБРАТЬ
		|					ВТСотрудникиИПН.Организация,
		|					ВТСотрудникиИПН.ФизическоеЛицо,
		|					ЗНАЧЕНИЕ(Справочник.ВидыДоходов.ДоходыОтРаботодателя)
		|				ИЗ
		|					ВТСотрудникиИПН)) КАК СведенияОбИсчисленииВычетовИПН
		|{ГДЕ
		...
		|ИЗ
		|	ВТСотрудникиВычеты КАК Сотрудники
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВТВычеты КАК Вычеты
		|		ПО Сотрудники.МесяцНачисления = Вычеты.МесяцНачисления
		|			И Сотрудники.Организация = Вычеты.Организация
		|			И Сотрудники.ФизическоеЛицо = Вычеты.ФизическоеЛицо
		|			И ВЫБОР КОГДА Сотрудники.Совместитель ТОГДА Сотрудники.Подразделение = Вычеты.Подразделение ИНАЧЕ ИСТИНА КОНЕЦ
		|
		|СГРУППИРОВАТЬ ПО
		...
		|	Сотрудники.Подразделение,
		|	Вычеты.ВидВычета,
		|	Вычеты.ВидВычетаПриоритет
		|
		|УПОРЯДОЧИТЬ ПО
		|	Вычеты.ВидВычетаПриоритет";
	
	ВозвращаемыеДанные.Вставить("Вычеты", Запрос.Выполнить().Выгрузить());
    // ЗУПК
...
Процедура КонтрольНастроекОтчетовПриЗаписиРассылкиОтчетов(Рассылка, Отказ) Экспорт
	РезультатПроверки = Новый Структура("ИмяСобытия, Метаданные, Данные, ТекстОшибки");
	РезультатПроверки.ИмяСобытия  = НСтр("ru = 'Рассылка отчетов. Проверка настроек'", ОбщегоНазначения.КодОсновногоЯзыка());
	РезультатПроверки.Данные      = Рассылка.Ссылка;
	РезультатПроверки.Метаданные  = Рассылка.Метаданные();
	РезультатПроверки.ТекстОшибки = "";
	
	Попытка
		ПроверитьНастройкиРассылки(РезультатПроверки, Рассылка);
	Исключение
		Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'В процедуре %1 не удалось проверить настройки рассылки отчетов %2:'"),
			"ЗарплатаКадрыОтчеты.КонтрольНастроекОтчетовПриЗаписиРассылкиОтчетов",
			Рассылка);
		РассылкаОтчетов.ЗаписьЖурнала(РезультатПроверки, , Текст, ИнформацияОбОшибке());
		Если Рассылка.ОбменДанными.Загрузка Тогда
			Возврат;
		Иначе
			ВызватьИсключение;
		КонецЕсли;
...
//
// Параметры:
//  Результат - Структура - Информация о результатах проверки, в т.ч. пригодная для записи в журнал регистрации.
...
//
Процедура ПроверитьНастройкиРассылки(Результат, Рассылка) Экспорт
...
Процедура ИнициализироватьНастройкиОтчета(ОтчетОбъект, КлючСхемы, КлючВарианта, НовыеНастройкиКД = Неопределено, НовыеПользовательскиеНастройкиКД = Неопределено) Экспорт
	...
	Если КлючСхемы <> КлючВарианта Тогда
		...
		КлючВариантаНастроек = КлючВарианта(ОтчетОбъект.КомпоновщикНастроек);
		Если КлючВариантаНастроек = Неопределено Тогда
			КлючВариантаНастроек = КлючВарианта;
		КонецЕсли;
		
		Если ВариантНастраиваемойПечатнойФормы(ОтчетОбъект, КлючВариантаНастроек) Тогда
			
			НастройкиУстановлены = ПрименитьНастрокиКОтчету(ОтчетОбъект, КлючВариантаНастроек, НовыеНастройкиКД, НовыеПользовательскиеНастройкиКД);
...
Функция ПрименитьНастрокиКОтчету(ОтчетОбъект, КлючВариантаНастроек, НовыеНастройкиКД, НовыеПользовательскиеНастройкиКД)
	...
	Если НовыеПользовательскиеНастройкиКД <> Неопределено
		...
		Если НовыеПользовательскиеНастройкиКД.ДополнительныеСвойства.Свойство("НастройкиПечатнойФормы") Тогда
			НовыеНастройкиКД = НовыеПользовательскиеНастройкиКД.ДополнительныеСвойства.НастройкиПечатнойФормы;
		КонецЕсли;
	...
	Если Не НастройкиУстановлены Тогда
		...
		ВариантОтчета = ВариантОтчетаПечатнойФормыПоОбъекту(ОтчетОбъект, КлючВариантаНастроек);
		Если ВариантОтчета <> Неопределено Тогда
			
			НастройкиВарианта = ВариантОтчета.Настройки.Получить();
			Если ТипЗнч(НастройкиВарианта) = Тип("Структура") Тогда
				Если НастройкиВарианта.Настройки <> Неопределено Тогда
					НовыеНастройкиКД = НастройкиВарианта.Настройки;
				КонецЕсли;
				Если НастройкиВарианта.ПользовательскиеНастройкиКД <> Неопределено Тогда
					Если НовыеПользовательскиеНастройкиКД <> Неопределено
						И НовыеПользовательскиеНастройкиКД.Элементы.Количество() > 0 Тогда
						
						Для Каждого ЭлементНастройкиВарианта Из НастройкиВарианта.ПользовательскиеНастройкиКД.Элементы Цикл
							Если ТипЗнч(ЭлементНастройкиВарианта) = Тип("ЗначениеПараметраНастроекКомпоновкиДанных") Тогда
								
								Элемент = НовыеПользовательскиеНастройкиКД.Элементы.Найти(ЭлементНастройкиВарианта.ИдентификаторПользовательскойНастройки);
								Если Элемент <> Неопределено
									И Не Элемент.Использование И Не ЭлементНастройкиВарианта.Использование Тогда
								...
								Если Элемент <> Неопределено
									И ПараметрВарианта <> Неопределено Тогда
									
									Если Элемент.Использование И ПараметрВарианта.Использование Тогда
										Если Не ЗначенияЭлементаКомпоновкиДанныхИдентичны(Элемент.Значение, ПараметрВарианта.Значение) Тогда
											ЭлементНастройкиВарианта.Значение = Элемент.Значение;
										КонецЕсли;
									ИначеЕсли Элемент.Использование И Не ПараметрВарианта.Использование И Не ЭлементНастройкиВарианта.Использование Тогда
										ЭлементНастройкиВарианта.Значение = Элемент.Значение;
										ЭлементНастройкиВарианта.Использование = Истина;
									ИначеЕсли Не Элемент.Использование И ПараметрВарианта.Использование И ЭлементНастройкиВарианта.Использование Тогда
										ЭлементНастройкиВарианта.Использование = Ложь;
									КонецЕсли;
							...
							ИначеЕсли ТипЗнч(ЭлементНастройкиВарианта) = Тип("ЭлементОтбораКомпоновкиДанных") Тогда
								...
								ЭлементВарианта =
									НайтиЭлементНастройкиПоИдентификаторПользовательскойНастройки(
								...
								Если Элемент <> Неопределено
									И ЭлементВарианта <> Неопределено Тогда
									
									Если Элемент.Использование И ЭлементВарианта.Использование Тогда
										Если Не ЗначенияЭлементаКомпоновкиДанныхИдентичны(Элемент.ПравоеЗначение, ЭлементВарианта.ПравоеЗначение)
											Или Элемент.ВидСравнения <> ЭлементВарианта.ВидСравнения Тогда
											
											ЭлементНастройкиВарианта.ПравоеЗначение = Элемент.ПравоеЗначение;
											ЭлементНастройкиВарианта.ВидСравнения = Элемент.ВидСравнения;
										КонецЕсли;
									ИначеЕсли Элемент.Использование И Не ЭлементВарианта.Использование И Не ЭлементНастройкиВарианта.Использование Тогда
										ЭлементНастройкиВарианта.ПравоеЗначение = Элемент.ПравоеЗначение;
										ЭлементНастройкиВарианта.ВидСравнения = Элемент.ВидСравнения;
										ЭлементНастройкиВарианта.Использование = Истина;
										Продолжить;
									ИначеЕсли Не Элемент.Использование И ЭлементВарианта.Использование И ЭлементНастройкиВарианта.Использование Тогда
										ЭлементНастройкиВарианта.Использование = Ложь;
									КонецЕсли;
								КонецЕсли;
...
Функция НайтиЭлементНастройкиПоИдентификаторПользовательскойНастройки(КоллекцияНастроек, ИдентификаторПользовательскойНастройки)
	Для Каждого ЭлементНастройки Из КоллекцияНастроек Цикл
...
Функция ЗначенияЭлементаКомпоновкиДанныхИдентичны(Значение1, Значение2)
	
	Если ТипЗнч(Значение1) = ТипЗнч(Значение2) Тогда
		Если ТипЗнч(Значение1) = Тип("СписокЗначений")
			Или ТипЗнч(Значение1) = Тип("Массив")
			Или ТипЗнч(Значение1) = Тип("ФиксированныйМассив") Тогда
			
			Возврат ОбщегоНазначения.КоллекцииИдентичны(Значение1, Значение2);
		КонецЕсли;
	Иначе
		Возврат Ложь
	КонецЕсли;
...
// Возвращаемое значение:
//  ТаблицаЗначений - Новая таблица данных организаций:
// * Организация - СправочникСсылка.Организации -
// * Период - Дата -
// * НаименованиеПолное - Строка -
// * ИНН - Строка -
...
// * ДолжностьРуководителя - Строка -
// * ДолжностьГлавногоБухгалтера - Строка -
Функция НоваяТаблицаДанныхОрганизаций()
	...
	ТаблицаДанныхОрганизаций.Колонки.Добавить("Период", Новый ОписаниеТипов("Дата"));
	ТаблицаДанныхОрганизаций.Колонки.Добавить("НаименованиеПолное", Новый ОписаниеТипов("Строка"));
	ТаблицаДанныхОрганизаций.Колонки.Добавить("ИНН", Новый ОписаниеТипов("Строка"));
	...
	ТаблицаДанныхОрганизаций.Колонки.Добавить("РасчетныйСчет", Новый ОписаниеТипов("Строка"));
	ТаблицаДанныхОрганизаций.Колонки.Добавить("Банк", Новый ОписаниеТипов("Строка"));
	ТаблицаДанныхОрганизаций.Колонки.Добавить("КорСчет", Новый ОписаниеТипов("Строка"));
	ТаблицаДанныхОрганизаций.Колонки.Добавить("БИК", Новый ОписаниеТипов("Строка"));
	ТаблицаДанныхОрганизаций.Колонки.Добавить("ФИОРуководителя", Новый ОписаниеТипов("Строка"));
	ТаблицаДанныхОрганизаций.Колонки.Добавить("ФИОГлавногоБухгалтера", Новый ОписаниеТипов("Строка"));
	ТаблицаДанныхОрганизаций.Колонки.Добавить("ДолжностьРуководителя", Новый ОписаниеТипов("Строка"));
	ТаблицаДанныхОрганизаций.Колонки.Добавить("ДолжностьГлавногоБухгалтера", Новый ОписаниеТипов("Строка"));
	
	Возврат ТаблицаДанныхОрганизаций;
	
КонецФункции

// Запрашиваемые значения для заполнения подписей.
// 
// Возвращаемое значение:
//  Структура - Запрашиваемые значения для заполнения подписей:
// * Организация - СправочникСсылка.Организации
// * Руководитель - СправочникСсылка.ФизическиеЛица
// * ДолжностьРуководителя - СправочникСсылка.Должности
// * ГлавныйБухгалтер - СправочникСсылка.ФизическиеЛица
// * ДолжностьГлавногоБухгалтера - СправочникСсылка.Должности
Функция ЗапрашиваемыеЗначенияДляЗаполненияПодписей()
	
	ЗапрашиваемыеЗначения = Новый Структура;
	
	ЗапрашиваемыеЗначения.Вставить("Организация", Справочники.Организации.ПустаяСсылка());
	ЗапрашиваемыеЗначения.Вставить("Руководитель", Справочники.ФизическиеЛица.ПустаяСсылка());
	ЗапрашиваемыеЗначения.Вставить("ДолжностьРуководителя", Справочники.Должности.ПустаяСсылка());
	ЗапрашиваемыеЗначения.Вставить("ГлавныйБухгалтер", Справочники.ФизическиеЛица.ПустаяСсылка());
	ЗапрашиваемыеЗначения.Вставить("ДолжностьГлавногоБухгалтера", Справочники.Должности.ПустаяСсылка());
...
Функция ЗапрашиваемыеСведенияОбОрганизации()
