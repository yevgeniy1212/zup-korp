...
Процедура ПриДобавленииОбработчиковОбновления(Обработчики) Экспорт
	...
	РегистрыСведений.ДатыВступленияВСилуНА.ПриДобавленииОбработчиковОбновления(Обработчики);
	...
	БизнесПроцессыЗаявокСотрудников.ЗарегистрироватьОбработчикиОбновления(Обработчики);
	
	ФизическиеЛицаЗарплатаКадры.ЗарегистрироватьОбработчикиОбновления(Обработчики);
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("ЗарплатаКадрыПриложения.ОценочныеОбязательстваЗарплатаКадры") Тогда
		...
		МодульРезервОтпусков.ЗарегистрироватьОбработчикиОбновления(Обработчики);
		МодульРезервыПоОплатеТруда = ОбщегоНазначения.ОбщийМодуль("РезервыПоОплатеТруда");
		МодульРезервыПоОплатеТруда.ЗарегистрироватьОбработчикиОбновления(Обработчики);
	КонецЕсли;
...
Процедура ПриДобавленииОбработчиковПереходаСДругойПрограммы(Обработчики) Экспорт
	...
	ИмяПроцедуры = "УправлениеСвойствамиБЗК.УстановитьДоступностьВсехНаборовДополнительныхРеквизитов";
	ОбщиеДанные  = Ложь;
...
// См. ОбновлениеБЗК.ОсновнойРежимВыполненияОбновления.
Функция ОсновнойРежимВыполненияОбновления() Экспорт
	Возврат ОбновлениеБЗК.ОсновнойРежимВыполненияОбновления();
КонецФункции
...
// * Ключ - Строка - Пространство блокировки -
// * Значение - Структура - Поля блокировки -
// 		* Ключ - Строка - имя поля
// 		* Значение - Произвольный - значение блокировки 
...
Процедура ЗаменитьСсылкиВРегистре(МетаданныеРегистра, Поля, СоответствиеСсылок) Экспорт
	...
	Для Каждого Поле Из Поля Цикл
		...
		СтрокаУсловия = СтрокаУсловия 
			+ СтрШаблон("(Таблица.%1 = &%1ПустаяСсылка И Таблица.Удалить%1 <> &Удалить%1ПустаяСсылка)", Поле.Имя);
	КонецЦикла;
	...
	Для Каждого Измерение Из МассивИзмерений Цикл
		...
		КонецЕсли;
		СтрокаСоединения = СтрокаСоединения + СтрШаблон("(Таблица.%1 = Измерения.%1)", Измерение);
