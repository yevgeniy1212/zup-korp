...
Процедура ДанныеОтчетаВТаблицу(ФайлВырузки, Контекст, Заголовок)
	...
	Для Каждого РеквизитОтчета Из ДокументDOM.ДочерниеУзлы[0].Атрибуты Цикл 
		Если НРег(РеквизитОтчета.ИмяУзла) = "year" тогда
			Контекст.Вставить("РеквизитОтчетаГод", РеквизитОтчета.ЗначениеУзла);
		ИначеЕсли НРег(РеквизитОтчета.ИмяУзла) = "period" тогда
			Контекст.Вставить("РеквизитОтчетаПериод", РеквизитОтчета.ЗначениеУзла);
		КонецЕсли;
	КонецЦикла;
		...
		Если Контекст.ТаблицаМножественныхСтрок.НайтиСтроки(Отбор).Количество() > 0 Тогда 
			Если Контекст.ОсновнойПериод_ДанныеОтчета.НайтиСтроки(Отбор).Количество() > 0 Тогда 
				...
				Если ОтобранныеСтроки.Количество() = 0
					Или Не ЗначениеЗаполнено(ОтобранныеСтроки[0][ИмяКол]) Тогда 
					ФлагОшибки = Истина;
				КонецЕсли;
...
Функция ПроверитьКонтрольныеСоотношенияВОтчете(ПараметрыДляПроверки)
	...
	Попытка
		...
		Если Контекст.ПроверкаОтчетаНаПустоту 
			Или Контекст.ОсновнойПериод_ДанныеОтчета.Количество() <> 0 Тогда 
...
Процедура ОтброситьРасчетЗапрещенныхДляВводаГраф(Контекст, ОтобранныеКонтрольныеСоотношения)
	...
	Для Каждого Стр Из ОтобранныеКонтрольныеСоотношения Цикл
		Попытка
			...
			Если Контекст.ЗапрещенныеДляВводаГрафы.НайтиСтроки(СтруктураПоиска).Количество() > 0 Тогда 
				СтрокиКУдалению.Добавить(Стр);
			КонецЕсли;
		Исключение
			СтрокиКУдалению.Добавить(Стр);
