...
Функция ПолучитьНастройкиОтображения() Экспорт
	...
	// Отображать события, полученные из внешних систем.
	НастройкиОтображения.Вставить(
		"ОтображатьСобытияВнешнихСистем",
		Ложь);
	
	// Адрес временного хранилища с таблицей занятости, полученной из внешних систем.
	НастройкиОтображения.Вставить(
...
Функция ПолучитьЦветТабличногоДокумента(ЦветКалендаря) Экспорт
	...
	ИначеЕсли ЦветКалендаря = ПредопределенноеЗначение("Перечисление.ЦветаРабочегоКалендаря.Белый") Тогда
		ЦветТабличногоДокумента = ЦветаСтиля.ЦветКалендаряФонПустойЯчейкиВсегоДня;
	Иначе
...
Функция ПроверитьДоступностьПользователей(
	...
	ДатаОкончания,
	ИсключенияЗанятости,
	АдресТаблицыЗанятостиИзВнешнихСистем = "") Экспорт
...
//
// Параметры:
//  МассивПользователей	 				 - Массив	 - пользователи, для которых будет получена занятость.
//  ДатаНачала			 				 - Дата		 - дата начала интервала занятости.
//  ДатаОкончания		 				 - Дата		 - дата окончания интервала занятости.
//  ИсключенияЗанятости					 - Массив	 - исключения занятости.
//	АдресТаблицыЗанятостиИзВнешнихСистем - Строка - адрес, в который будет помещена таблица занятости из внешних систем.
//
// Возвращаемое значение:
//  ТаблицаЗначений.
//
Функция ПолучитьТаблицуОбщейЗанятости(Знач МассивПользователей, ДатаНачала, ДатаОкончания, ИсключенияЗанятости = Неопределено, АдресТаблицыЗанятостиИзВнешнихСистем = "") Экспорт
...
Функция НовоеПравилоПовторения() Экспорт
	...
	ПравилоПовторения.Вставить("ПравилоОкончанияПовторения");
...
// Возвращает настройки повторения переданных записей календаря
//
// Параметры:
//  ЗаписиКалендаря	- Массив из СправочникСсылка.ЗаписиРабочегоКалендаря
//					- СправочникСсылка.ЗаписиРабочегоКалендаря
// 
// Возвращаемое значение:
//   - Соответствие - 	Ключ: 		СправочникСсылка.ЗаписиРабочегоКалендаря
//						Значение: 	Структура - см. РаботаСРабочимКалендарем.НовоеПравилоПовторения
//
Функция НастройкиПовторенияЗаписейКалендаря(ЗаписиКалендаря) Экспорт
	...
	Пока Выборка.Следующий() Цикл
		...
		Результат.Вставить(Выборка.Ссылка, ПравилоПовторения);
...
Процедура ЗагрузитьДанныеПланировщика(Планировщик, НастройкиОтображения)
	...
	Если НастройкиОтображения.ОтображатьСобытияВнешнихСистем Тогда
		
		ЗагрузитьДанныеВнешнихСистем(Планировщик, НастройкиОтображения);
		
	КонецЕсли;
	
КонецПроцедуры

// Загружает данные внешних систем.
//
// Параметры:
//  Планировщик - Планировщик - планировщик.
//  НастройкиОтображения - Структура - структура настроек отображения календаря.
//
Процедура ЗагрузитьДанныеВнешнихСистем(Планировщик, НастройкиОтображения)
	
	АдресТаблицы = НастройкиОтображения.АдресТаблицыЗанятостиИзВнешнихСистем;
	Если Не ЗначениеЗаполнено(АдресТаблицы) Тогда
		Возврат;
	КонецЕсли;
	
	ТаблицаЗанятости = ПолучитьИзВременногоХранилища(АдресТаблицы);
	Для Каждого Занятость Из ТаблицаЗанятости Цикл
		Если Занятость.Занят = Перечисления.СостоянияЗанятости.Доступен
			 Или ЕстьИдентичныйЭлемент(Планировщик.Элементы, Занятость) Тогда
			Продолжить;
		КонецЕсли;
		Элемент = Планировщик.Элементы.Добавить(Занятость.ДатаНачала, Занятость.ДатаОкончания);
		СтруктураЭлемента = ОписаниеЭлементаПланировщикаПоЗанятости(Занятость);
		СтруктураЭлемента.Текст = СинхронизацияКалендарей.ТекстЭлементаПланировщика(СтруктураЭлемента.Текст,
...
Функция ЕстьИдентичныйЭлемент(ЭлементыПланировщика, Занятость)
	...
	Для Каждого ЭлементПланировщика Из ЭлементыПланировщика Цикл
		...
		Для Каждого КлючИЗначение Из ЭлементПланировщика.ЗначенияИзмерений Цикл
			Если КлючИЗначение.Ключ = "Пользователь" И КлючИЗначение.Значение = Занятость.Пользователь Тогда
				ЕстьИзмерение = Истина;
				Прервать;
			КонецЕсли; 
		КонецЦикла;
		
		Если ЕстьИзмерение 
			 И ЭлементПланировщика.Начало = Занятость.ДатаНачала
			 И ЭлементПланировщика.Конец = Занятость.ДатаОкончания Тогда
			Результат = Истина;
		КонецЕсли;
...
Функция ПолучитьТаблицуСобытий(
	...
	Запрос.Текст = 
		...
		|		КОГДА НЕ ЗаписиРабочегоКалендаря.СвязанныйПредмет = ЗНАЧЕНИЕ(Справочник.ЭтапыРаботыСКандидатами.ПустаяСсылка)
		|				И УчастникиЗаписи.Участник ЕСТЬ NULL
		|			ТОГДА ЗНАЧЕНИЕ(Перечисление.ЦветаРабочегоКалендаря.Белый)
		...
		|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.УчастникиЭтапаРаботыСКандидатом КАК УчастникиЗаписи
		|		ПО (НЕ ЗаписиРабочегоКалендаря.СвязанныйПредмет = ЗНАЧЕНИЕ(Справочник.ЭтапыРаботыСКандидатами.ПустаяСсылка))
		|			И ЗаписиРабочегоКалендаря.Предмет = УчастникиЗаписи.Кандидат
		|			И ЗаписиРабочегоКалендаря.СвязанныйПредмет = УчастникиЗаписи.ЭтапРаботы
		|			И ЗаписиРабочегоКалендаря.Пользователь = УчастникиЗаписи.Участник
		|ГДЕ
...
Функция ПолучитьТаблицуЗанятости(
	...
	ТаблицаЗанятостиИзВнешнихСистем = 
		ТаблицаЗанятостиИзВнешнихСистем(
			МассивПользователей, АдресТаблицыЗанятостиИзВнешнихСистем, ДатаНачала, ДатаОкончания, ТолькоИтоги);
	...
	Для Каждого Пользователь Из МассивПользователей Цикл
		...
		Если Не ТолькоИтоги Тогда
			...
			// Занятое время по событиям внешних систем
			ВнестиЗанятостиВТаблицуЗанятости(ТаблицаЗанятости, ТаблицаЗанятостиИзВнешнихСистем,
				Пользователь, ДатаНачала, ДатаОкончания);
		...
		Иначе
			...
			// Занятое время по событиям внешних систем
			РасчетЗанятостиЗавершен = РассчитатьЗанятостьПользователяПоКраткомуАлгоритму(
				ТаблицаЗанятостиИзВнешнихСистем, Пользователь, НоваяСтрока.Занят);
			Если РасчетЗанятостиЗавершен Тогда
				Продолжить;
			КонецЕсли;
...
Функция ПолучитьТаблицуРекомендаций(Знач МассивПользователей,
		ДатаНачала, ДатаОкончания, Длительность, ИсключенияЗанятости = Неопределено, АдресТаблицыЗанятостиИзВнешнихСистем = "") Экспорт
...
Функция ТаблицаЗанятостиИзВнешнихСистем(МассивПользователей, АдресТаблицыЗанятости, ДатаНачала, ДатаОкончания, ТолькоИтоги)
	
	Если Не ЗначениеЗаполнено(АдресТаблицыЗанятости) Тогда
		Возврат СформироватьТаблицуЗанятости();
	КонецЕсли;
	
	ТаблицаЗанятостиИзВнешнихСистем = ПолучитьИзВременногоХранилища(АдресТаблицыЗанятости);
		
	Если ТолькоИтоги Тогда
		Доступен = Перечисления.СостоянияЗанятости.Доступен;
		
		Для Каждого Пользователь Из МассивПользователей Цикл
			ТекущаяЗанятость = Доступен;
			
			Отбор = Новый Структура("Пользователь", Пользователь); 
			
			СтрокиПользователя = ТаблицаЗанятостиИзВнешнихСистем.НайтиСтроки(Отбор);
			
			Для Каждого СтрокаПользователя Из СтрокиПользователя Цикл
				Если СтрокаПользователя.Занят <> Доступен Тогда
					Если ДатаНачала < СтрокаПользователя.ДатаОкончания
						И ДатаОкончания > СтрокаПользователя.ДатаНачала Тогда
						ТекущаяЗанятость = СтрокаПользователя.Занят;
						Прервать;
					КонецЕсли; 
				КонецЕсли; 
			КонецЦикла;
			...
			Для Каждого СтрокаПользователя Из СтрокиПользователя Цикл
				ТаблицаЗанятостиИзВнешнихСистем.Удалить(СтрокаПользователя);
			КонецЦикла;
			
			Если ТекущаяЗанятость <> Доступен Тогда
				НоваяСтрока = ТаблицаЗанятостиИзВнешнихСистем.Добавить();
				НоваяСтрока.Занят = ТекущаяЗанятость;
				НоваяСтрока.ДатаНачала = НачалоДня(ДатаНачала);
				НоваяСтрока.ДатаОкончания = КонецДня(ДатаОкончания) + 1;
			КонецЕсли; 
		КонецЦикла; 
	КонецЕсли; 
	
	СортироватьПоСостояниюЗанятости(ТаблицаЗанятостиИзВнешнихСистем);
	
	Возврат ТаблицаЗанятостиИзВнешнихСистем;
