...
Функция ГрафикПогашенияЗайма(ПараметрыПостроения) Экспорт
	...
	ОстатокДолга			= 0;
	Если ПараметрыПостроения.ТраншиЗайма.Количество() = 0 Тогда
		ОстатокДолга = ПараметрыПостроения.СуммаЗайма;
	Иначе
		Для Каждого Транш Из ПараметрыПостроения.ТраншиЗайма Цикл
			Если Транш.ДатаПредоставления < МесяцНачала Тогда
	...
	Пока ТекущийМесяц <= МесяцОкончания Цикл
		...
		Если ТекущийМесяц = НачалоМесяца(ДатаПредоставления) Тогда
			...
			// следующий день).
			ДнейВМесяцеПроценты = День(КонецМесяца(ТекущийМесяц)) - День(ДатаПредоставления);
			ДнейВМесяцеОсновнойДолг = ДнейВМесяцеПроценты + 1;
		Иначе
			ДнейВМесяцеПроценты	= День(КонецМесяца(ТекущийМесяц));
			ДнейВМесяцеОсновнойДолг = ДнейВМесяцеПроценты;
		КонецЕсли;
		НачислениеПроцентов	= Макс(ОстатокДолга, 0) * ПараметрыПостроения.ГодоваяСтавка * 0.01 * ДнейВМесяцеПроценты / ДнейВГоду;
		Если ВидыПлатежейРазмерПогашения.Найти(ВидПлатежей) <> Неопределено Тогда
			НачисленоОсновногоДолга	= РазмерПогашения * (ДнейВМесяцеОсновнойДолг) / ЗарплатаКадрыКлиентСервер.КоличествоДнейМесяца(ТекущийМесяц);
		Иначе
			НачисленоОсновногоДолга	= РазмерПлатежа * (ДнейВМесяцеОсновнойДолг) / ЗарплатаКадрыКлиентСервер.КоличествоДнейМесяца(ТекущийМесяц) - НачислениеПроцентов;
		КонецЕсли;
		...
		Если ПараметрыПостроения.СпособПогашения = ПредопределенноеЗначение("Перечисление.СпособыПогашенияЗаймаСотруднику.ВТечениеСрока") Тогда
			...
			Если ВидПлатежей = ПредопределенноеЗначение("Перечисление.ВидыПлатежейПогашенияЗаймаСотруднику.АннуитетныеПлатежи") Тогда
				...
				ПогашениеДолга		= ?(РасчетРазмераПлатежа, НачисленоОсновногоДолга, Мин(НачисленоОсновногоДолга, ОстатокДолга));
...
Функция РазмерАннуитетногоПлатежа(ДанныеЗайма) Экспорт
	...
	Если Не ЗначениеЗаполнено(ПредыдущийОстатокДолга)
		Или ДанныеЗайма.ТраншиЗайма.Количество() = 0 Тогда
...
Функция СрокПогашенияЗайма(ДанныеЗайма) Экспорт
	...
	ПредыдущийОстатокДолга = СальдоВзаиморасчетовПоЗавершению(ДанныеЗайма.РазмерПлатежа, 
		ДатаОкончанияПоСроку(ДанныеЗайма.МесяцПредоставления, ЦелоеВБольшуюСторону(ПредыдущийСрок)), ДанныеЗайма, КорректировкаСрока);
	
	// Делаем второе предположение.
	ТекущийСрок = ПредыдущийСрок * 0.8;
	ТекущийОстатокДолга = СальдоВзаиморасчетовПоЗавершению(ДанныеЗайма.РазмерПлатежа, 
		ДатаОкончанияПоСроку(ДанныеЗайма.МесяцПредоставления, ЦелоеВБольшуюСторону(ТекущийСрок)), ДанныеЗайма, КорректировкаСрока);
...
Функция СальдоВзаиморасчетовПоЗавершению(РазмерПлатежа, МесяцОкончания, ДанныеЗайма, КорректировкаСрока = Неопределено)
	...
	Индекс = ГрафикПогашения.ВГраница();
	Пока Индекс >= 0 Цикл
...
Процедура ЗаполнитьСрокПоДатеОкончания(Срок, ДатаОкончания, ДатаНачала) Экспорт
	...
	Срок = СрокПоДатеОкончания(ДатаОкончания, ДатаНачала);
...
Процедура УстановитьЭлементыВводаЧастичногоПогашенияЗайма(Форма) Экспорт 
	...
	Если Форма.Объект.ЗаемЧастичноПогашен Тогда
		...
		Если Форма.Объект.СпособПредоставления <> ЕдиновременнаяВыдача Тогда
			...
			УстановитьЭлементыВводаТраншей(Форма, Форма.Объект.СпособПредоставления);
        КонецЕсли;
	...
	УстановитьЭлементыВыдачиЗаймаДокументомДоговора(Форма);
...
Процедура УстановитьОтметкуНезаполненногоПогашеннойСуммыЗайма(Форма) Экспорт
	
	Объект = Форма.Объект;
	АвтоОтметкаНезаполненного = Объект.ЗаемЧастичноПогашен И Не (ЗначениеЗаполнено(Объект.ДатаНачалаПогашения) И Объект.ДатаПогашения <= КонецМесяца(Объект.ДатаНачалаПогашения));
	ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Форма.Элементы, "ПогашеннаяСуммаЗайма", "АвтоОтметкаНезаполненного", АвтоОтметкаНезаполненного); 
	ОтметкаНезаполненного = АвтоОтметкаНезаполненного И Не ЗначениеЗаполнено(Объект.ПогашеннаяСуммаЗайма); 
	ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Форма.Элементы, "ПогашеннаяСуммаЗайма", "ОтметкаНезаполненного", ОтметкаНезаполненного); 
...
Процедура ЗаполнитьРазмерПлатежа(РазмерПлатежа, Срок, ВидПлатежей, ДанныеЗайма) Экспорт
	...
	РазмерПлатежа = РазмерАннуитетногоПлатежа(ДанныеЗайма);
...
Процедура ЗаполнитьДатуОкончанияПоСроку(РедактируемыйОбъект, ПутьКДатеНачала, ПутьКДатеОкончания, ПутьКПредставлениюДатыОкончания, Срок) Экспорт
	...
	ДатаОкончания = ДатаОкончанияПоСроку(ДатаНачала, Срок);
...
Процедура ЗаполнитьСрокПолногоПогашенияЗайма(Форма, СпособПогашения, ВидПлатежей, Срок, ДанныеЗайма) Экспорт
	...
	Если РасчетСрокаВозможен Тогда
		Срок = СрокПогашенияЗайма(ДанныеЗайма);
	КонецЕсли;
