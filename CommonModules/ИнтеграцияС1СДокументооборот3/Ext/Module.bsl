...
Функция HTMLПредпросмотрОбъекта(Прокси, ID, Тип) Экспорт
	...
	Запрос = ЗапросНаПредпросмотрОбъекта(Прокси, ID, Тип);
...
//
Функция ЗапросНаПредпросмотрОбъекта(Прокси, ID, Тип) Экспорт
...
//   РезультатДействияТекстом - Строка - результат действия задачи.
//   ВерсияДанныхДействияЗадачи - Строка - версия данных действия задачи.
//
// Возвращаемое значение:
//   ОбъектXDTO - объект XDTO типа DMExecuteTaskActionRequest.
//
Функция ВыполнитьДействиеЗадачиЗапрос(Прокси, ДействиеЗадачи, РезультатДействия, РезультатДействияТекстом,
		ВерсияДанныхДействияЗадачи) Экспорт
	...
	Запрос.taskAction = ИнтеграцияС1СДокументооборотБазоваяФункциональность.СоздатьОбъект(
		Прокси,
		ДействиеЗадачи.Тип);
	Запрос.taskAction.name = ДействиеЗадачи.Наименование;
	...
	Запрос.resultOfTaskAction = ИнтеграцияС1СДокументооборотБазоваяФункциональность.СоздатьОбъект(
		Прокси,
		РезультатДействия.Тип);
	Запрос.resultOfTaskAction.name = РезультатДействия.Наименование;
	Запрос.resultOfTaskAction.objectID = ИнтеграцияС1СДокументооборотБазоваяФункциональность.СоздатьObjectID(
		Прокси,
		РезультатДействия.ID,
		РезультатДействия.Тип);
	
	Запрос.actionResultString = РезультатДействияТекстом;
	Запрос.dataVersion = ВерсияДанныхДействияЗадачи;
...
// Возвращаемое значение:
//   Структура:
//     * ОбластьДействия - см. ИнтеграцияС1СДокументооборот3.ОбластьДействияЗадачи
...
Функция ДополнительныеДанныеПоЗадаче(ЗадачаXDTO, ТекущийПользовательИСотрудники = Неопределено) Экспорт
	...
	ДополнительныеДанные.Вставить("ОбластьДействия", ОбластьДействияЗадачи(ЗадачаXDTO.actionArea));
...
//
Функция ЗапросНаПолучениеСпискаЗадач(Прокси, ТекущаяСтраница, РазмерСтраницыДинамическогоСписка,
		ПолеДляСортировки = Неопределено, НаправлениеСортировкиПоля = Ложь, Фильтры = Неопределено,
		ОтображениеКарточкиЗадачи = Неопределено) Экспорт
	...
	Если ПолеДляСортировки <> Неопределено Тогда
...
// Возвращаемое значение:
//   Структура:
//     * ВыполнитьДействиеЗадачиВариант1 - см. ИнтеграцияС1СДокументооборот3.СтруктураКнопкиФормы
//     * ВыполнитьДействиеЗадачиВариант2 - см. ИнтеграцияС1СДокументооборот3.СтруктураКнопкиФормы
//     * ВыполнитьДействиеЗадачиВариант3 - см. ИнтеграцияС1СДокументооборот3.СтруктураКнопкиФормы
//     * ВыполнитьДействиеЗадачиПоШаблону1 - см. ИнтеграцияС1СДокументооборот3.СтруктураКнопкиФормы
//     * ВыполнитьДействиеЗадачиПоШаблону2 - см. ИнтеграцияС1СДокументооборот3.СтруктураКнопкиФормы
//     * ВыполнитьДействиеЗадачиПоШаблону3 - см. ИнтеграцияС1СДокументооборот3.СтруктураКнопкиФормы
//     * ВыбратьШаблонДляВыполненияЗадачи - см. ИнтеграцияС1СДокументооборот3.СтруктураКнопкиФормы
//     * ВыполнитьПодключаемоеДействиеЗадачи1 - см. ИнтеграцияС1СДокументооборот3.СтруктураКнопкиФормы
//     * ОтменитьВыполнение - см. ИнтеграцияС1СДокументооборот3.СтруктураКнопкиФормы
//     * ВерсияДанныхДействияЗадачи - Строка
//     * РезультатДействияТекстом - Строка
//     * РезультатДействияПолеВвода - см. ИнтеграцияС1СДокументооборот3.СтруктураПоляВводаФормы
//     * РезультатВарианта1 - Строка
//     * РезультатВарианта1ID - Строка
//     * РезультатВарианта1Тип - Строка
//     * РезультатВарианта2 - Строка
...
//     * РезультатВарианта3Тип - Строка
//
Функция ОбластьДействияЗадачи(ОбластьДействияXDTO) Экспорт
