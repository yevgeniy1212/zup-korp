...
Функция ЕстьОшибкаВДоверенности(РезультатПроверкиПоМЧД)
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("ЭлектронноеВзаимодействие.ОбменСКонтрагентами.МашиночитаемыеДоверенности") Тогда
		...
		Возврат РезультатПроверкиПоМЧД <> Неопределено
			...
			И Не Модуль.ДоверенностьПроверенаУспешно(РезультатПроверкиПоМЧД.ПротоколПроверки.ПроверкаМЧД);
	Иначе
		Возврат Ложь;
	КонецЕсли;
