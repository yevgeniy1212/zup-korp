...
Процедура ПрочитатьДанныеПозицииВФорму(Форма, Объект, АдресДанныхПозиции) Экспорт
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("ЗарплатаКадрыКорпоративнаяПодсистемы.ХарактеристикиПерсонала") Тогда
		МодульХарактеристикиПерсоналаФормы = ОбщегоНазначения.ОбщийМодуль("ХарактеристикиПерсоналаФормы");
		МодульХарактеристикиПерсоналаФормы.ПрочитатьДанныеПозицииВФорму(Форма, Объект, НовыеДанныеПозиции);
	КонецЕсли;
...
Процедура ОбработатьЗаполнениеДокументаВФорме(Форма) Экспорт
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("ЗарплатаКадрыКорпоративнаяПодсистемы.ХарактеристикиПерсонала") Тогда
		МодульХарактеристикиПерсоналаФормы = ОбщегоНазначения.ОбщийМодуль("ХарактеристикиПерсоналаФормы");
		МодульХарактеристикиПерсоналаФормы.ДополнитьФормуДокументовИзмененияШтатногоРасписания(Форма);
	КонецЕсли;
...
Процедура ДобавитьДополнительныеСведенияПозиций(Форма, ТекущийОбъект)
	...
	Если Форма.Модифицированность Тогда
		...
		Для каждого СтрокаПозиции Из Форма.Объект.Позиции Цикл
			...
			СведенияОНаименованиях.Вставить(СтрокаПозиции.ИдентификаторСтрокиПозиции, Новый Структура("Наименование, НаименованиеПолное", СтрокаПозиции.Наименование, СтрокаПозиции.НаименованиеПолное));
			
		КонецЦикла;
		
		Если ПозицииДокумента.Количество() > 0 Тогда
			
			ТекущиеНаименования = ОбщегоНазначения.ЗначенияРеквизитовОбъектов(ПозицииДокумента, "Наименование, НаименованиеПолное");
			Для каждого ОписаниеНаименования Из ТекущиеНаименования Цикл
				...
				Если ИдентификаторСтрокиПозиции <> Неопределено Тогда
					...
					Если НаименованиеВДокументе.Наименование = ОписаниеНаименования.Значение.Наименование
						И НаименованиеВДокументе.НаименованиеПолное = ОписаниеНаименования.Значение.НаименованиеПолное Тогда
		...
		Если ОбщегоНазначения.ПодсистемаСуществует("ЗарплатаКадрыКорпоративнаяПодсистемы.ХарактеристикиПерсонала") Тогда
			МодульХарактеристикиПерсоналаФормы = ОбщегоНазначения.ОбщийМодуль("ХарактеристикиПерсоналаФормы");
			МодульХарактеристикиПерсоналаФормы.ДобавитьДополнительныеСведенияОХарактеристикахПозиций(Форма, ТекущийОбъект);
		КонецЕсли;
...
Процедура РедактированиеСостоянияШРПрочитатьТекущиеДанные(ДокументСсылка, Период, Форма, ОписаниеТаблицыПозиций, ОписаниеТаблицыВидовРасчета)
	...
	Если Форма.ПолучитьФункциональнуюОпциюФормы("ИспользоватьИсториюИзмененияШтатногоРасписания") Тогда
		...
		Запрос.Текст =
			...
			|	ВЫРАЗИТЬ(Позиции.Позиция КАК Справочник.ШтатноеРасписание).НаименованиеПолное КАК НаименованиеПолное,
		...
		Для Каждого СтрокаПозиции Из ПозицииДанныеФормы Цикл
			...
			Если ЗначениеЗаполнено(СтрокаПозиции.Позиция) Тогда
				...
				СтрокаПозиции.НаименованиеПолное = ВыборкаПозиций.НаименованиеПолное;
			Иначе
				
				ДополнительныеПараметры = Новый Структура("РазрядКатегория", СтрокаПозиции.РазрядКатегория);
				ДополнительныеПараметры.Вставить("ПолноеНаименованиеДолжности", ОбщегоНазначения.ЗначениеРеквизитаОбъекта(СтрокаПозиции.Должность, "НаименованиеДляЗаписейОТрудовойДеятельности"));
				...
				СтрокаПозиции.Наименование = НаименованиеПозиции.Наименование;
				СтрокаПозиции.НаименованиеПолное = НаименованиеПозиции.НаименованиеПолное;
...
Процедура СкопироватьДанныеПозиции(Объект, ИдентификаторТекущейПозиции, ИдентификаторПозицииИсточника) Экспорт
	...
	Для каждого ИсходнаяСтрока Из ИсходныеСтроки Цикл
		...
		ИсходныеСтрокиПоказателей = Объект.Показатели.НайтиСтроки(Новый Структура("ИдентификаторСтрокиПозиции,ИдентификаторСтрокиВидаРасчета", СтрокаИсточник.ИдентификаторСтрокиПозиции, НоваяСтрока.ИдентификаторСтрокиВидаРасчета));
		Для каждого ИсходнаяСтрокаПоказателей Из ИсходныеСтрокиПоказателей Цикл
			
			НоваяСтрокаПоказателей = Объект.Показатели.Добавить();
			ЗаполнитьЗначенияСвойств(НоваяСтрокаПоказателей, ИсходнаяСтрокаПоказателей);
