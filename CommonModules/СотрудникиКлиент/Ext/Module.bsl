...
//
Функция ПараметрыОткрытияФормыФизическиеЛицаСПохожимиДанными(РезультатыПроверки = Неопределено) Экспорт
	
	ПараметрыОткрытия = Новый Структура();
	ПараметрыОткрытия.Вставить("ЗаголовокФормы", "");
	ПараметрыОткрытия.Вставить("ТекстИнформационнойНадписи", "");
	ПараметрыОткрытия.Вставить("Фамилия", "");
	ПараметрыОткрытия.Вставить("Имя", "");
	ПараметрыОткрытия.Вставить("Отчество", "");
	ПараметрыОткрытия.Вставить("ФизическоеЛицоУникально", Истина);
	ПараметрыОткрытия.Вставить("ДанныеФизическихЛиц", Новый Массив);
	ПараметрыОткрытия.Вставить("ДанныеФизическихЛицДоступны", Истина);
	ПараметрыОткрытия.Вставить("ВозможнаПроверкаПоИНН", Истина);
	ПараметрыОткрытия.Вставить("ВозможнаПроверкаПоСНИЛС", Истина);
	ПараметрыОткрытия.Вставить("СкрытьКомандуДругойЧеловек", Ложь);
	ПараметрыОткрытия.Вставить("УчитыватьОтрицательныйВыбор", Ложь);
	ПараметрыОткрытия.Вставить("КлючНазначенияИспользования", "БезСписка");
	...
	Если ОбщегоНазначенияКлиент.ПодсистемаСуществует("ЗарплатаКадрыКорпоративнаяПодсистемы.ПодборПерсонала") Тогда
		...
		МодульПодборПерсоналаКлиент.ДополнитьПараметрыОткрытияФормыФизическиеЛицаСПохожимиДанными(ПараметрыОткрытия);
	КонецЕсли;
	
	Если РезультатыПроверки <> Неопределено Тогда
		ЗаполнитьЗначенияСвойств(ПараметрыОткрытия, РезультатыПроверки);
	КонецЕсли;
	Если ПараметрыОткрытия.ДанныеФизическихЛиц.Количество() > 1 Тогда
		ПараметрыОткрытия.КлючНазначенияИспользования = "СоСписком";
	КонецЕсли;
...
Процедура ОбработатьВыборФизическогоЛицаОднофамильца(Форма, ФизическоеЛицоСсылка) Экспорт
	
	Форма.Модифицированность = Ложь;
	Форма.Закрыть();
	
	Если Форма.РежимВыбора Тогда
		Оповестить("СозданоФизическоеЛицо", ФизическоеЛицоСсылка, Форма.ВладелецФормы);
	Иначе
		ОткрытьФорму("Справочник.ФизическиеЛица.ФормаОбъекта", Новый Структура("Ключ", ФизическоеЛицоСсылка));
	КонецЕсли;
...
Процедура ФизическиеЛицаПриОткрытии(Форма) Экспорт
	...
	Если Не Форма.Параметры.Ключ.Пустая() Тогда
		ПриПереходеНаСтраницуИстория(Форма, Неопределено, Форма.Элементы.Страницы.ТекущаяСтраница);
	КонецЕсли;
...
Процедура ОповеститьОбИзмененииДанныхФизическогоЛица(Форма, ФизическоеЛицо)
	...
	Если СтрокиАдреса.Количество() > 0 Тогда
		ДанныеФизическогоЛица.АдресРегистрации = СтрокиАдреса[0].Значение;
	...
	Если СтрокиАдреса.Количество() > 0 Тогда
		ДанныеФизическогоЛица.АдресФактический = СтрокиАдреса[0].Значение;
	...
	Если СтрокиАдреса.Количество() > 0 Тогда
		ДанныеФизическогоЛица.АдресДляИнформирования = СтрокиАдреса[0].Значение;
...
Процедура НачатьЗапись(Форма, ЗакрытьПослеЗаписи = Ложь, ОповещениеЗавершения = Неопределено) Экспорт
	
	ДляОтвета = Новый Структура();
	ДляОтвета.Вставить("Форма", Форма);
	ДляОтвета.Вставить("ЗакрытьПослеЗаписи", ЗакрытьПослеЗаписи);
	ДляОтвета.Вставить("ПослеЗаписи", ОповещениеЗавершения);
	
	ТекстВопроса = НСтр("ru = 'Данные физического лица не записаны.
						|Записать и продолжить?'");
	
	ДляВопроса = СтандартныеПодсистемыКлиент.ПараметрыВопросаПользователю();
	ДляВопроса.Заголовок = НСтр("ru = 'Запись данных'");
	ДляВопроса.ПредлагатьБольшеНеЗадаватьЭтотВопрос = Ложь;
	
	СтандартныеПодсистемыКлиент.ПоказатьВопросПользователю(
		Новый ОписаниеОповещения("ЗавершитьЗапись", ЭтотОбъект, ДляОтвета), 
		ТекстВопроса, 
...
Процедура ЗавершитьЗапись(Результат, ДополнительныеПараметры) Экспорт
	
	Если ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(Результат, "Значение") <> КодВозвратаДиалога.Да Тогда
		Возврат;
	КонецЕсли;
	
	ДополнительныеПараметры.Форма.ЗаписатьНаКлиенте(ДополнительныеПараметры.ЗакрытьПослеЗаписи, ДополнительныеПараметры.ПослеЗаписи);
...
Процедура ПриИзмененииФИО(Форма, ФизическоеЛицо, МенеджерЗаписиФИО)
	...
	Если ЗначениеЗаполнено(МенеджерЗаписиФИО.Фамилия)
		И Не ЗначениеЗаполнено(ФизическоеЛицо.Пол) Тогда
		
		Если ЗначениеЗаполнено(МенеджерЗаписиФИО.Отчество) Тогда
			ФизическоеЛицо.Пол = СотрудникиКлиентСервер.ОпределитьПолПоОтчеству(МенеджерЗаписиФИО.Отчество);
		КонецЕсли;
		
		Если ФизическоеЛицо.Пол = ПредопределенноеЗначение("Перечисление.ПолФизическогоЛица.ПустаяСсылка") Тогда
			ФизическоеЛицо.Пол = ПредопределенноеЗначение("Перечисление.ПолФизическогоЛица.Мужской");
			ТекстСостояния = НСтр("ru = 'Не удалось установить пол сотрудника.
				|Назначен по умолчанию - %1.'");
		Иначе
			ТекстСостояния = НСтр("ru = 'Установлен пол сотрудника - %1.'");
		КонецЕсли;
		Состояние(СтрШаблон(ТекстСостояния, ФизическоеЛицо.Пол));
...
Процедура ПроверитьОднофамильцев(Форма, ИзФормыСотрудника, ОповещениеЗавершения = Неопределено)
	...
	Если (Форма.ИННУказанПравильно ИЛИ ПустаяСтрока(Форма.ФизическоеЛицо.ИНН))
		И (Форма.СНИЛСУказанПравильно
			ИЛИ НЕ КадровыйУчетКлиентСервер.СНИЛСЗаполнен(Форма.ФизическоеЛицо.СтраховойНомерПФР))
		И (НЕ ПустаяСтрока(Форма.ФизическоеЛицо.ИНН)
			ИЛИ КадровыйУчетКлиентСервер.СНИЛСЗаполнен(Форма.ФизическоеЛицо.СтраховойНомерПФР)) Тогда
		
		Если СотрудникиВызовСервера.ИННИСНИЛСУникальны(Форма.ФизическоеЛицо.ИНН,
			Форма.ФизическоеЛицо.СтраховойНомерПФР) Тогда
	...
	ПараметрыПоиска.Ссылка = Форма.ФизическоеЛицоСсылка;
	...
	РезультатПроверки = СотрудникиВызовСервера.ПодобратьСписокФизЛиц(ПараметрыПоиска);
	
	ПараметрыОткрытия = ПараметрыОткрытияФормыФизическиеЛицаСПохожимиДанными(РезультатПроверки);
	
	ДополнительныеПараметры = Новый Структура;	
	ДополнительныеПараметры.Вставить("Форма", Форма);
	ДополнительныеПараметры.Вставить("ИзФормыСотрудника", ИзФормыСотрудника);
	ДополнительныеПараметры.Вставить("ОповещениеЗавершения", ОповещениеЗавершения);
	ДополнительныеПараметры.Вставить("ФизическоеЛицоУникально", РезультатПроверки.ФизическоеЛицоУникально);
	ДополнительныеПараметры.Вставить("КоличествоНайденных", РезультатПроверки.ДанныеФизическихЛиц.Количество());
	
	Если ОбщегоНазначенияКлиент.ПодсистемаСуществует("ЗарплатаКадрыКорпоративнаяПодсистемы.ПодборПерсонала") Тогда
		МодульПодборПерсоналаКлиент = ОбщегоНазначенияКлиент.ОбщийМодуль("ПодборПерсоналаКлиент");
		МодульПодборПерсоналаКлиент.ПроверитьОднофамильцевСотрудникаСредиКандидатов(Форма, ПараметрыОткрытия,
			ДополнительныеПараметры);
	КонецЕсли;
	
	Если ДополнительныеПараметры.ФизическоеЛицоУникально Тогда
		Если ОповещениеЗавершения <> Неопределено Тогда 
			ВыполнитьОбработкуОповещения(ОповещениеЗавершения);
		КонецЕсли;
		Возврат;
	...
	Если ДополнительныеПараметры.КоличествоНайденных > 0 Тогда
		
		Оповещение = Новый ОписаниеОповещения("ПроверитьОднофамильцевДанныеФизическихЛиц", ЭтотОбъект,
			ДополнительныеПараметры);
		ОткрытьФорму(
	...
	Иначе
		...
		Если РезультатПроверки.ВозможнаПроверкаПоИНН
			ИЛИ РезультатПроверки.ВозможнаПроверкаПоСНИЛС Тогда
			
			Оповещение = Новый ОписаниеОповещения("ПроверитьОднофамильцевИНН_СНИЛС", ЭтотОбъект,
				ДополнительныеПараметры);
...
Процедура ПроверитьОднофамильцевДанныеФизическихЛиц(РезультатВыбора, ДополнительныеПараметры) Экспорт 
	...
	Если РезультатВыбора <> Неопределено Тогда
		...
		Если ИзФормыСотрудника Тогда
			...
			Если ОбщегоНазначенияКлиент.ПодсистемаСуществует("ЗарплатаКадрыКорпоративнаяПодсистемы.ПодборПерсонала") Тогда
				МодульПодборПерсоналаКлиент = ОбщегоНазначенияКлиент.ОбщийМодуль("ПодборПерсоналаКлиент");
				МодульПодборПерсоналаКлиент.ПроверитьОднофамильцевСотрудникаСредиКандидатовЗавершение(РезультатВыбора,
					ДополнительныеПараметры);
			Иначе	
		...
		Иначе
			...
			Если ОбщегоНазначенияКлиент.ПодсистемаСуществует("ЗарплатаКадрыКорпоративнаяПодсистемы.ПодборПерсонала") Тогда
				МодульПодборПерсоналаКлиент = ОбщегоНазначенияКлиент.ОбщийМодуль("ПодборПерсоналаКлиент");
				МодульПодборПерсоналаКлиент.ПроверитьОднофамильцевФизическогоЛицаСредиКандидатовЗавершение(
					РезультатВыбора, ДополнительныеПараметры);
			Иначе
				ОбработатьВыборФизическогоЛицаОднофамильца(Форма, РезультатВыбора);
			КонецЕсли;
...
Процедура ДокументыФизическихЛицВидДокументаНачалоВыбора(Форма, Элемент, СтандартнаяОбработка) Экспорт
	Если НЕ ЗначениеЗаполнено(Форма.ДокументыФизическихЛиц.ВидДокумента) Тогда
        // ЗУПК
		Форма.ДокументыФизическихЛиц.ВидДокумента = ПредопределенноеЗначение("Справочник.ВидыДокументовФизическихЛиц.УдостоверениеЛичностиРК");
        // ЗУПК
	КонецЕсли; 
КонецПроцедуры
	...
	Если ФормаИсточник.ДоступенПросмотрДанныхФизическихЛиц Тогда
		...
		ТекстВопроса = СтрШаблон(
				НСтр("ru =  'При редактировании изменены сведения о документе, удостоверяющем личность.
				            ...
				            |Если у сотрудника с %1 изменился документ, удостоверяющий личность, нажмите ""%2""'"),
				Формат(ДатаИзменения, "ДФ='д ММММ гггг ""г""'"),
				ТекстКнопкиДа);
...
Процедура СотрудникиОткрытьФормуРедактированияИстории(ИмяРегистра, Форма) Экспорт
	ФизлицоЗаблокировано = ЗаблокироватьФизическоеЛицоПриРедактировании(Форма, Истина);
	Если ФизлицоЗаблокировано Тогда
...
Процедура НачатьСозданиеНовогоСотрудника(Форма) Экспорт

	Если Не Форма.Параметры.Ключ.Пустая() Тогда
		ОткрытьМестоРаботы(Форма);
		Возврат;
	КонецЕсли;
	
	ПараметрыОповещения = Новый Структура();
	ПараметрыОповещения.Вставить("Форма", Форма);
	Форма.НачатьЗапись(, Новый ОписаниеОповещения("ЗавершитьСозданиеНовогоСотрудника", ЭтотОбъект, ПараметрыОповещения));
	
КонецПроцедуры

Процедура ЗавершитьСозданиеНовогоСотрудника(ПараметрыЗаписи, ДополнительныеПараметры) Экспорт
...
Процедура ЗаполнитьИсториюВзаимоотношений(Форма)
	...
	Если Форма.Параметры.Ключ.Пустая() Тогда
		ЗавершитьЗаполнениеИсторииВзаимоотношений(Форма);
		Возврат;
	КонецЕсли;
...
Процедура ЗавершитьЗаполнениеИсторииВзаимоотношений(Форма, Результат = Неопределено, ДополнительныеПараметры = Неопределено) Экспорт
