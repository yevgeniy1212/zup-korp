...
Процедура СоздатьВТНалогУдержанный(МенеджерВременныхТаблиц, ИсточникНДФЛ, ДатаПолученияДохода, ИмяВТ = "ВТНалогУдержанный") Экспорт
	...
	Если ДатаПолученияДохода < ДатаНачалаПереносаДатыПолученияДоходов
        // ЗУПК
		ИЛИ Истина Тогда
        // ЗУПК
		Запрос.Текст =
	...
	Запрос.УстановитьПараметр("КатегорииДоходаНДФЛСФиксированнойДатойПолученияДохода", Перечисления.КатегорииДоходовНДФЛ.СФиксированнойДатойПолученияДохода(ДатаПолученияДохода));
