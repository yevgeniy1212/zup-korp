...
Процедура УстановитьИспользованиеРегламентныхЗаданийПоФункциональнымОпциям(ВключатьЗадания = Ложь) Экспорт
	...
	Для Каждого СтрокаЗадание Из Задания Цикл
		...
		Если Использование = Неопределено
			...
			// ЗУПК - обход ошибки в сервисе с ФО ОтключенныйФункционал при вызове РегламентныеЗаданияСлужебный.УстановитьИспользованиеРегламентныхЗаданийПоФункциональнымОпциям(Истина) после создания данных области:
			// при таком варианте запуска отключенные регламентированные задания не попадают в Продолжить отключенных регл задание (ниже): Если Использование = Неопределено Или (Использование И Не ВключатьЗадания) Или (Не Использование И Не ОтключитьЗадание)  
			// и облачный сервис пытается произвести изменений задания, которое отключено с ошибкой
	        // ЗУПК
            Или (Не Использование И ВключатьЗадания)
	        // ЗУПК
