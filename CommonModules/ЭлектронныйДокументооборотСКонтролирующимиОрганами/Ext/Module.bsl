...
Процедура ОпределитьТаблицуОписанияОбъектовРегламентированнойОтчетности(ТаблицаОписания) Экспорт
	...
	НаименованиеМЧДФСС = НСтр("ru = 'Машиночитаемая доверенность (ФСС)'");
	НаименованиеМЧДФСС = ДокументооборотСКОКлиентСервер.ЗаменитьПФРиФССнаСФР(НаименованиеМЧДФСС, Истина, Истина);
	
	НоваяСтрока = ТаблицаОписания.Добавить();
	НоваяСтрока.Наименование                       = НаименованиеМЧДФСС;
	...
	НоваяСтрока = ТаблицаОписания.Добавить();
	НоваяСтрока.Наименование                       = НСтр("ru = 'Машиночитаемая доверенность (ФНС, распределенный реестр)'");
...
Процедура СоздатьЗаписьВРегистреЖурналОтправокВКонтролирующиеОрганы(ЗначенияЗаполнения) Экспорт
	...
	Попытка
		...
		Если ТипЗнч(Ссылка) = Тип("СправочникСсылка.МашиночитаемыеДоверенностиФНС") Тогда
			...
			Если НаборЗаписей.Количество() <> 0 И (НаборЗаписей[0].СтатусОтправки <> СтатусОтправки
				ИЛИ НаборЗаписей[0].СостояниеСдачиОтчетности <> СостояниеСдачиОтчетности) Тогда
...
Процедура УчестьУведомленияВСоставеОтветовНаТребования(Объект) Экспорт
	...
	Если Объект.ВидУведомления = Перечисления.ВидыУведомленийОСпецрежимахНалогообложения.РеестрДокументовПодтверждающихЛьготы Тогда
...
Функция ОбъектОтправляетсяВКонтролирующиеОрганы(Ссылка) Экспорт
	...
	Если ТипЗнч(Ссылка) = Тип("ДокументСсылка.УведомлениеОСпецрежимахНалогообложения") Тогда
		Если ПравоДоступа("Чтение", Метаданные.Справочники.ВидыОтправляемыхДокументов) Тогда
			...
			Если ЭтоУведомлениеОРегистрацииЮЛ(Ссылка.ВидУведомления) Тогда
				ОбъектОтправляется = Истина;	
			ИначеЕсли ВидыОтправляемыхУведомлений.Найти(Ссылка.ВидУведомления) = Неопределено Тогда
...
Процедура ЗаполнитьПараметрыДляФормыПодключенияКСервисуЭлектронныхДокументов(Организация, СтруктураПараметров) Экспорт
	...
	Запрос.Текст = 
		...
		|	УчетныеЗаписиДокументооборота.МодельХраненияЗакрытогоКлюча,
	...
	Если Выборка.Следующий() Тогда
		...
		Если СтруктураПараметров.Свойство("МестоХраненияКлюча") Тогда
			Если КриптографияЭДКОКлиентСервер.ЭтоОблачнаяПодпись(Выборка.МодельХраненияЗакрытогоКлюча)
				И ДоступнаСерверОблачнойПодписи Тогда
					МодульСервисКриптографииDSS = ОбщегоНазначения.ОбщийМодуль("СервисКриптографииDSS");
					НашлиСертификат = МодульСервисКриптографииDSS.НайтиСертификат(Новый Структура("Отпечаток", Выборка.СертификатРуководителя));
				УчеткаОблачнойПодписи = ?(НЕ НашлиСертификат.Выполнено, Неопределено, НашлиСертификат.УчетнаяЗапись);
				МестоХраненияКлюча = КриптографияЭДКОКлиентСервер.ОпределитьМестоХраненияКлюча(Выборка.МодельХраненияЗакрытогоКлюча, УчеткаОблачнойПодписи);
			Иначе
				МестоХраненияКлюча = КриптографияЭДКОКлиентСервер.ОпределитьМестоХраненияКлюча(Выборка.МодельХраненияЗакрытогоКлюча);
			КонецЕсли;	
	...
	Если (ТипЗнч(Свойства.Ссылка) = Тип("СправочникСсылка.ЗаявленияОбОтзывеМЧДФНС") ИЛИ
		...
		Если ЗначениеЗаполнено(ОтборСсылка) Тогда
			...
			Для каждого Строка из НаборЗаписей Цикл
				...
				СтатусОтправкиИзменился = (Строка.СтатусОтправки <> "Отозвано");
				Строка.СтатусОтправки = "Отозвано";
...
Функция ПолучитьГруппуНовогоСобытияДокументооборотаСКО(Запись)
	...
	ЗавершенныеСостояния = СтроковыеФункцииКлиентСервер.РазложитьСтрокуВМассивПодстрок(
		"Не сдано;Не принято в обработку;Сдано, требует уточнения;Сдано;Сдано, получен РНПТ;Сдано, ошибки НСП;"
		+ "Не принято;Принято;Доставлено;Сдан частично;Получен комментарий;Зарегистрировано;Не зарегистрировано;"
		+ "Отозвано;Подписано;Зарегистрировано, срок действия не наступил;Истек срок действия;"
		+ "Зарегистрировано, ошибка отзыва;Зарегистрировано, отказ в отзыве в ФНС", ";");
	...
	НезавершенныеСостояния = СтроковыеФункцииКлиентСервер.РазложитьСтрокуВМассивПодстрок(
		...
		+ "Отправлено заявление на отзыв в ФНС", ";");
