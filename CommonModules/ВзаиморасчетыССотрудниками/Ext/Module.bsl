...
Функция ЕстьОплатаПоВедомости(Ведомость) Экспорт
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("ЗарплатаКадрыПриложения.ВнешниеХозяйственныеОперации.ВзаиморасчетыССотрудникамиВХО") Тогда
		...
		Модуль.ЕстьОплатаПоВедомостиТекст(Ведомость, Запрос.Текст);
	КонецЕсли;
	// ЗУПК
...
Функция ТекстЗапросаДанныеДляОплатыБезналично(
	...
	ТекстыПодзапросов[1] = ВедомостьНаВыплатуЗарплаты.ТекстЗапросаДанныеДляОплаты(ИмяТипа, ИмяПараметраВедомости, ИмяПараметраФизическиеЛица);
...
Процедура ЗарегистрироватьОбработчикиОбновления(Обработчики) Экспорт
	...
	Обработчик = Обработчики.Добавить();
	Обработчик.Версия = "3.1.15.93";
	Обработчик.Процедура = "Документы.ПодтверждениеЗачисленияЗарплаты.УстановитьВозвратныеВидыВзаиморасчетов";
	Обработчик.РежимВыполнения = ОбновлениеИнформационнойБазыЗарплатаКадрыБазовый.ОсновнойРежимВыполненияОбновления();
	Обработчик.Идентификатор = Новый УникальныйИдентификатор("4f9340c7-0872-11eb-80cc-1831bf5241f7"); 
	Обработчик.Комментарий = НСтр("ru = 'Установка вида взаиморасчетов незачисленной зарплаты при обмене с банками.'");
	
КонецПроцедуры

Процедура УстановитьВозвратныеВидыВзаиморасчетовДокумента(МетаданныеРегистратора, ПараметрыОбновления) Экспорт
	...
	|	МАКСИМУМ(ВзаиморасчетыССотрудниками.Период) КАК Период
	|ИЗ
	|	РегистрНакопления.ВзаиморасчетыССотрудниками КАК ВзаиморасчетыССотрудниками
	|ГДЕ
	|	ТИПЗНАЧЕНИЯ(ВзаиморасчетыССотрудниками.Регистратор) = &ТипРегистратора
	...
	|
	|УПОРЯДОЧИТЬ ПО
	|	МАКСИМУМ(ВзаиморасчетыССотрудниками.Период) УБЫВ";
		
	Запрос.Текст = СтрЗаменить(
		Запрос.Текст, 
		"&ТипРегистратора",  
		СтрШаблон("ТИП(%1)", МетаданныеРегистратора.ПолноеИмя())); 
	...
	Пока ВыборкаОбновляемыхДанных.Следующий() Цикл
		...
		Для Каждого МетаданныеРегистра Из МетаданныеРегистров Цикл
			...
			Для Каждого Запись Из НаборЗаписей Цикл
				Если Перечисления.ВидыВзаиморасчетовССотрудниками.ЭтоВыплата(Запись.ВидВзаиморасчетов) Тогда
					Запись.ВидВзаиморасчетов = Перечисления.ВидыВзаиморасчетовССотрудниками.Невыплата(Запись.ВидВзаиморасчетов);
				КонецЕсли;	
			КонецЦикла;
	...
	Для Каждого МетаданныеРегистра Из МетаданныеРегистров Цикл
		РегистрыНакопления[МетаданныеРегистра.Имя].УстановитьИспользованиеИтогов(ИспользованиеИтогов[МетаданныеРегистра]);
	КонецЦикла;	
...
Функция ЗапросДанныеВедомостейДляОплатыДокументами(Документы, Ведомости, ФизическиеЛица = Неопределено, Оплаченные = Ложь) Экспорт
	...
	// УПОРЯДОЧИТЬ: получаемый порядок записей сам по себе не важен, 
	// но при этом он должен быть одинаковым вне зависимости от применяемой СУБД. 
...
// а вид взаиморасчетов заменяется на "невыплату".
//
// Параметры:
// 	Движения       - КоллекцияДвижений - коллекция наборов записей движений ведомости.
// 	Отказ          - Булево            - признак отказа выполнения операции.
...
Процедура ЗарегистрироватьНевыплатуПоВедомости(Движения, Отказ, Ведомость, ФизическиеЛица) Экспорт
	...
	Пока ВыборкаВзаиморасчетов.Следующий() Цикл
		...
		НоваяЗапись.ВидВзаиморасчетов   = Перечисления.ВидыВзаиморасчетовССотрудниками.Невыплата(НоваяЗапись.ВидВзаиморасчетов);
...
Процедура СоздатьВТЗарплатаКВыплатеОграниченнаяСальдоФизлиц(МенеджерВременныхТаблиц, ТолькоРазрешенные, Параметры) Экспорт
	...
	Иначе
		ВыплачиватьПлановыйАванс = Истина;	
	КонецЕсли;
