...
//	Респондент - СправочникСсылка.ЛичныеДанныеКандидатов - личные данные кандидата, для которого создается анкета.
...
Процедура ОткрытьСравнениеПоХарактеристикам(ФормаВладелец, Вакансия, Кандидаты = Неопределено) Экспорт
	
	Если Не ЗначениеЗаполнено(Вакансия) Тогда
		ПоказатьПредупреждение(Неопределено, 
			НСтр("ru = 'Не выбрана вакансия. Пожалуйста, выберите вакансию и повторите попытку.'"), ,
			НСтр("ru = 'Не удалось сравнить кандидатов'"));
		Возврат;
	КонецЕсли;
	...
	Если Кандидаты <> Неопределено Тогда
		ПараметрыФормы.Вставить("Кандидаты", Кандидаты);
	КонецЕсли;

	ОткрытьФорму("Обработка.СравнениеХарактеристикКандидатов.Форма.Форма", ПараметрыФормы, ФормаВладелец, Вакансия);
	
	// АПК: 65-выкл Получение формы нужно для того, чтобы в случае, 
	// если она уже открыта, не открывать повторно, а оповестить для обработки изменений.
	ФормаСравнения = ПолучитьФорму("Обработка.СравнениеХарактеристикКандидатов.Форма.Форма",
		ПараметрыФормы, ФормаВладелец, Вакансия);
	// АПК: 65-вкл
	
	Если Не ФормаСравнения.Открыта() Тогда
		ФормаСравнения.Открыть();
		Возврат;
	КонецЕсли;
	
	Оповестить(ИмяСобытияСравнениеХарактеристикОбновитьПараметры(), ПараметрыФормы, ФормаВладелец);
	
КонецПроцедуры

Функция ИмяСобытияСравнениеХарактеристикОбновитьПараметры() Экспорт
	Возврат "Обработка_СравнениеХарактеристикКандидатов_ОбновитьПараметры";
КонецФункции
