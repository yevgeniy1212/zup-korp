...
Процедура СформироватьВТКадровыхДанных(Организация, ПериодРегистрации, МенеджерВременныхТаблиц, МенеджерКадровогоУчета) Экспорт
	...
	Если Не ПолучитьФункциональнуюОпцию("ИспользуютсяРаботыСДосрочнойПенсией", Новый Структура("Организация",Организация)) Тогда
		...
		УчетСтраховыхВзносов.ДополнитьПоляЗапросаРабочимМестом(Запрос, "КадровыхДанных", , Ложь);
		УчетСтраховыхВзносов.ДополнитьПоляЗапросаРабочимМестом(Запрос, "ВременнаяТаблица", , Ложь);
	...
	КадровыйУчет.ДополнитьПоляЗапросаВТКадровыхДанных(Запрос);
	Запрос.Выполнить();
...
Процедура СформироватьВТНачисленияСДаннымиУчета(Организация, ПериодРегистрации, МенеджерВременныхТаблиц, ИсключаемыйРегистратор) Экспорт
	...
	УчетСтраховыхВзносовБазовый.СформироватьВТНачисленияСДаннымиУчета(Организация, ПериодРегистрации, МенеджерВременныхТаблиц, ПолучитьФункциональнуюОпцию("ИспользоватьОсобыеУсловияТрудаВОрганизации", Новый Структура("Организация",Организация)));
...
Процедура СформироватьВТСДаннымиУчета(Организация, ПериодРегистрации, МенеджерВременныхТаблиц) Экспорт 
	...
	ДополнитьПоляЗапросаРабочимМестом(Запрос, "ДанныеОСотруднике", , Ложь, Истина);
...
Процедура СоздатьВТКлассыУсловийТруда(МенеджерВременныхТаблиц) Экспорт
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("ЗарплатаКадрыКорпоративнаяПодсистемы.ОхранаТруда") Тогда
		...
		МодульОхранаТруда.ДополнитьЗапросВТКлассыУсловийТруда(Запрос);
	КонецЕсли;
...
Процедура ДополнитьПоляЗапросаРабочимМестом(Запрос, ИмяТаблицы, ДобавитьПредставление = Ложь, ВключатьИспользованиеРесурса = Истина, ЗамещатьДолжность = Ложь) Экспорт
	
	Если ОбщегоНазначения.ПодсистемаСуществует("ЗарплатаКадрыКорпоративнаяПодсистемы.ОхранаТруда") Тогда
	    МодульОхранаТруда = ОбщегоНазначения.ОбщийМодуль("ОхранаТруда");
	    МодульОхранаТруда.ДополнитьПоляЗапросаРабочимМестом(Запрос, ИмяТаблицы, ДобавитьПредставление, ВключатьИспользованиеРесурса, ЗамещатьДолжность);
	КонецЕсли;
