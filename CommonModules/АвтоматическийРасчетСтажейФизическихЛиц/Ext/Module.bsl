...
Процедура ЗарегистрироватьСписокФизическихЛицКПересчетуСтажей(ОбновляемыеВидыСтажей, СтандартнаяОбработка) Экспорт
	...
	Запрос.Текст =
		...
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	МИНИМУМ(ПериодыФизическихЛицКПересчетуСтажей.Период) КАК Период,
		|	ПериодыФизическихЛицКПересчетуСтажей.ФизическоеЛицо КАК ФизическоеЛицо
		|ПОМЕСТИТЬ ВТПериодыФизическихЛицКПересчетуСтажей
		|ИЗ
		|	ВТПериодыФизическихЛицКПересчетуСтажейПредварительно КАК ПериодыФизическихЛицКПересчетуСтажей
		|
		|СГРУППИРОВАТЬ ПО
		|	ПериодыФизическихЛицКПересчетуСтажей.ФизическоеЛицо";
...
// Запоминает должности при изменении свойств которых требуется обновить накопленные стажи,
// работающих на них сотрудников.
//
// Параметры:
//  Источник             - РегистрСведенийНаборЗаписей
//  Отказ                - Булево
//  ИменаРесурсовСтрокой - Строка
//
...
// Регистрирует физических лиц к пересчету стажей, работающих на должностях от которых
// зависят стажи после изменения свойств должностей.
//
// Параметры:
//  Источник             - РегистрСведенийНаборЗаписей
//  Отказ                - Булево
...
Процедура ОбновитьНакопленныеСтажиФизическихЛицПоВременнойТаблице(МенеджерВременныхТаблиц)
	...
	Запрос.Текст =
		...
		|ПОМЕСТИТЬ ВТДатыНачалаСтажейФизическихЛицПредварительно
		|ИЗ
		|	ВТСтажиФизическихЛицКПересчету КАК СтажиФизическихЛицКПересчету
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.СтажиФизическихЛиц КАК СтажиФизическихЛиц
		...
		|	СтажиФизическихЛиц.ФизическоеЛицо КАК ФизическоеЛицо,
		|	СтажиФизическихЛиц.ВидСтажа КАК ВидСтажа,
		|	СтажиФизическихЛиц.ДатаНачала КАК ДатаНачала
		|ПОМЕСТИТЬ ВТДатыНачалаСтажейФизическихЛиц
		|ИЗ
		|	ВТДатыНачалаСтажейФизическихЛицПредварительно КАК СтажиФизическихЛиц
		|
		|ОБЪЕДИНИТЬ ВСЕ
		|
		|ВЫБРАТЬ
		|	СтажиФизическихЛиц.ФизическоеЛицо,
		|	СтажиФизическихЛиц.ВидСтажа,
		|	МИНИМУМ(СтажиФизическихЛиц.Период)
		|ИЗ
		|	ВТСтажиФизическихЛицКПересчету КАК СтажиФизическихЛицКПересчету
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.СтажиФизическихЛиц КАК СтажиФизическихЛиц
		|		ПО СтажиФизическихЛицКПересчету.ФизическоеЛицо = СтажиФизическихЛиц.ФизическоеЛицо
		|			И СтажиФизическихЛицКПересчету.НачалоПериода <= СтажиФизическихЛиц.Период
		|		ЛЕВОЕ СОЕДИНЕНИЕ ВТДатыНачалаСтажейФизическихЛицПредварительно КАК ДатыНачалаСтажейФизическихЛицПредварительно
		|		ПО СтажиФизическихЛицКПересчету.ФизическоеЛицо = ДатыНачалаСтажейФизическихЛицПредварительно.ФизическоеЛицо
		|			И (СтажиФизическихЛиц.ВидСтажа = ДатыНачалаСтажейФизическихЛицПредварительно.ВидСтажа)
		|ГДЕ
		|	ДатыНачалаСтажейФизическихЛицПредварительно.ВидСтажа ЕСТЬ NULL
		|
		|СГРУППИРОВАТЬ ПО
		|	СтажиФизическихЛиц.ФизическоеЛицо,
		|	СтажиФизическихЛиц.ВидСтажа
	...
	Для Каждого Должность Из Должности Цикл
		...
		Если Не РезультатЗапроса.Пустой() Тогда
			...
			Если ТипЗнч(Должность) = Тип("СправочникСсылка.ШтатноеРасписание") Тогда
				...
				УправлениеШтатнымРасписанием.ОтключитьОбновлениеНастройкиИспользованияСтраховыхВзносовПоКлассамУсловийТруда(ОбъектСЗависимымСтажем);
				УправлениеШтатнымРасписанием.ОтключитьОбновлениеНастройкиПубликации(ОбъектСЗависимымСтажем);
			КонецЕсли;
