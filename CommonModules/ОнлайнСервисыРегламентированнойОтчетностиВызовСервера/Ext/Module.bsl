...
Функция ПолучитьПараметрыИНастройкиМеханизмаОнлайнСервисовРО() Экспорт
	...
	ПараметрыИНастройки = Новый Структура;
	...
	ПараметрыИНастройки.Вставить("ЗапроситьПодтверждениеДоступаВИнтернет",
		ВерсияРелизаНаКоторомДоступВИнтернетЗапрещен <> ВерсияПрограммы());
	ПараметрыИНастройки.Вставить("НастройкиМеханизмаОнлайнСервисов",
		ПолучитьНастройкиМеханизмаОнлайнСервисовРО());
	ПараметрыИНастройки.Вставить("ЕстьПравоИзмененияРегистраРесурсов",
		ЕстьПравоИзмененияРегистраРесурсыМеханизмаОнлайнСервисовРО());
	
	Возврат ПараметрыИНастройки;
...
Процедура СохранитьОбщиеНастройкиМеханизмаОнлайнСервисовРО(ИспользоватьМеханизмОнлайнСервисовРО = Неопределено, УведомлятьОбОшибкахМеханизмаОнлайнСервисовРО = Неопределено, АвтоматическиПодключатьФормыРОЧерезМеханизмОнлайнСервисовРО = Неопределено) Экспорт
	...
	Если ОбщегоНазначения.РазделениеВключено() и ОбщегоНазначения.ДоступноИспользованиеРазделенныхДанных() Тогда
 		// настройки механизма онлайн сервисов являются общими данными
		Возврат;
	КонецЕсли;
...
Процедура ВключитьМеханизмОнлайнСервисовРО() Экспорт
	...
	ОнлайнСервисыРегламентированнойОтчетностиВызовСервера.СохранитьОбщиеНастройкиМеханизмаОнлайнСервисовРО(Истина, Ложь, Истина);	
...
Функция ЗапуститьФоновоеЗаданиеАктуализироватьСлужебнуюИнформациюМеханизмаОнлайнСервисовРО(УникальныйИдентификаторФормы, МожноЗагружатьВнешниеФормыОтчетов = Ложь) Экспорт
	...
	ПараметрыВыполнения.НаименованиеФоновогоЗадания = НСтр("ru = 'Производится обновление служебной информации из Интернет...'");
	ПараметрыВыполнения.ОжидатьЗавершение = 0;
	ПараметрыВыполнения.ЗапуститьВФоне = Истина;
	
	ПараметрыПроцедуры = Новый Структура("МожноЗагружатьВнешниеФормыОтчетов", МожноЗагружатьВнешниеФормыОтчетов);
...
Процедура АктуализироватьСлужебнуюИнформациюМеханизмаОнлайнСервисовРО(ПараметрыОтчета, АдресХранилища) Экспорт
	...
	// в режиме сервиса информация загружается через механизм поставляемых данных
	Если ОбщегоНазначения.РазделениеВключено() Тогда 
		Возврат;
	КонецЕсли;
	
#Область ИнициализацияПеременныхИКонстант
	...
	ТипРесурсаСправочникСобытий 		= Перечисления.ТипыРесурсовМеханизмаОнлайнСервисовРО.СправочникСобытий;
	...
	ИДРесурсаСправочникСобытий			= "EVE";
	
	ДатаПолученияРесурсов				= ТекущаяДатаСеанса();
	...
	ВремяОткрытияФормыПодождите = ТекущаяДатаСеанса();
...
#КонецОбласти 
	...
	РезультатРазбораКлючевогоФайла = ПолучитьРазобратьКлючевойФайл(Соединение, ИмяКлючевогоФайла, УведомлятьОбОшибках, ОписаниеОшибки);
...
#Область АктуализацияСправочникаСобытий

	// актуализация справочника событий
	ИмяФайлаСправочникаСобытий = РезультатРазбораКлючевогоФайла[ИДРесурсаСправочникСобытий];
	СвойстваХранимогоВИБСправочникаСобытий = ХранимаяВИБИнформация[ТипРесурсаСправочникСобытий];	
	ДеревоСправочникаСобытий = Неопределено;	
	АктуализироватьСправочникСобытия(Соединение, ДеревоСправочникаСобытий, ТаблицаАктуальныхРесурсов, 
									 ИмяФайлаСправочникаСобытий, СвойстваХранимогоВИБСправочникаСобытий, РасширениеФайловНаСервере, 
									 ТипРесурсаСправочникСобытий, ДатаПолученияРесурсов, УведомлятьОбОшибках);

#КонецОбласти 
									 
#Область АктуализацияСправочникФормФорматов
	...
	Если ИмяФайлаСправочникаФормаИФорматов <> Неопределено Тогда
		...
		ИмяХранимогоВИБФайла = ?(СвойстваХранимогоВИБСправочникаФормИФорматов = Неопределено, Неопределено, 
								 СвойстваХранимогоВИБСправочникаФормИФорматов.Имя);
...
#Область АктуализацияСправочникаРелизов
...
#Область АктуализацияИнформацииОТекущемРелизе
...
#КонецОбласти 
	...
	СохранитьДанныеВРесурсыМеханизмаОнлайнСервисовРО(ТаблицаАктуальныхРесурсов, УведомлятьОбОшибках, ОписаниеОшибки);
...
Функция АктуализироватьСправочникСобытия(ХранилищеРесурсов, 
										 ДеревоСправочникаСобытий, 
										 ТаблицаАктуальныхРесурсов, 
										 ИмяФайлаСправочникаСобытий, 
									     СвойстваХранимогоВИБСправочникаСобытий, 
										 РасширениеФайловНаСервере, 
										 ТипРесурсаСправочникСобытий, 
										 ДатаПолученияРесурсов, 
										 УведомлятьОбОшибках)						
	
	ОписаниеОшибки = "";
		
	// актуализация справочника событий
	Если ИмяФайлаСправочникаСобытий <> Неопределено Тогда
		// получаем версию хранящейся в ИБ информации
		ИмяХранимогоВИБФайла = ?(СвойстваХранимогоВИБСправочникаСобытий = Неопределено, Неопределено, СвойстваХранимогоВИБСправочникаСобытий.Имя);
		
		Если ИмяФайлаСправочникаСобытий <> ИмяХранимогоВИБФайла Тогда
		
			// получаем файл
			ИмяВременногоФайлаСправочникаСобытий = ПолучитьРесурсССервера(ХранилищеРесурсов, ИмяФайлаСправочникаСобытий + РасширениеФайловНаСервере, ОписаниеОшибки);
			Если ИмяВременногоФайлаСправочникаСобытий = Неопределено Тогда
				ОписаниеОшибки = "Ошибка получения справочника событий с сервера справочной информации" + ?(ЗначениеЗаполнено(ОписаниеОшибки), ": " + ОписаниеОшибки, ".");
				Если УведомлятьОбОшибках Тогда
					ОбщегоНазначения.СообщитьПользователю(ОписаниеОшибки);
				КонецЕсли;
			...
			// распаковываем полученный файл
			РезультатРаспаковки = РаспаковатьФайл(ИмяВременногоФайлаСправочникаСобытий, ОписаниеОшибки);
			Если НЕ РезультатРаспаковки Тогда
				ОписаниеОшибки = "Некорректный формат файла справочника событий" + ?(ЗначениеЗаполнено(ОписаниеОшибки), ": " + ОписаниеОшибки, ".");				
				Если УведомлятьОбОшибках Тогда
					ОбщегоНазначения.СообщитьПользователю(ОписаниеОшибки);
				КонецЕсли;
			...
			// считываем содержимое
			СодержимоеФайлаСправочникаСобытий = ПрочитатьТекстовыйФайл(ИмяВременногоФайлаСправочникаСобытий, ОписаниеОшибки, Истина);
			Если НЕ ЗначениеЗаполнено(СодержимоеФайлаСправочникаСобытий) Тогда
				ОписаниеОшибки = "Некорректный формат файла справочника событий" + ?(ЗначениеЗаполнено(ОписаниеОшибки), ": " + ОписаниеОшибки, ".");
				Если УведомлятьОбОшибках Тогда
					ОбщегоНазначения.СообщитьПользователю(ОписаниеОшибки);
				КонецЕсли;
			...
			// загружаем XML в дерево
			ДеревоСправочникаСобытий = ЗагрузитьСтрокуXMLВДеревоЗначений(СодержимоеФайлаСправочникаСобытий, ОписаниеОшибки);
			Если ДеревоСправочникаСобытий = Неопределено Тогда
				ОписаниеОшибки = "Некорректный формат файла справочника событий" + ?(ЗначениеЗаполнено(ОписаниеОшибки), ": " + ОписаниеОшибки, ".");
				Если УведомлятьОбОшибках Тогда
					ОбщегоНазначения.СообщитьПользователю(ОписаниеОшибки);
				КонецЕсли;
			...
			// сохраняем полученную XML-строку в таблице-результате
			ДобавитьСтрокуВТаблицуАктуальныхРесурсов(ТаблицаАктуальныхРесурсов, 
													 ТипРесурсаСправочникСобытий, 
													 ИмяФайлаСправочникаСобытий, 
													 СодержимоеФайлаСправочникаСобытий, 
													 ДатаПолученияРесурсов);
			
		Иначе
			Если СвойстваХранимогоВИБСправочникаСобытий = Неопределено Тогда
				ДобавитьСтрокуВТаблицуАктуальныхРесурсов(ТаблицаАктуальныхРесурсов, ТипРесурсаСправочникСобытий, , , ДатаПолученияРесурсов);
			Иначе
				ДобавитьСтрокуВТаблицуАктуальныхРесурсов(ТаблицаАктуальныхРесурсов, 
														 ТипРесурсаСправочникСобытий, 
														 СвойстваХранимогоВИБСправочникаСобытий.Имя, 
														 СвойстваХранимогоВИБСправочникаСобытий.Данные, 
														 СвойстваХранимогоВИБСправочникаСобытий.ДатаПолучения);
			КонецЕсли;
		КонецЕсли;
		
	ИначеЕсли СвойстваХранимогоВИБСправочникаСобытий = Неопределено Тогда
		ДобавитьСтрокуВТаблицуАктуальныхРесурсов(ТаблицаАктуальныхРесурсов, ТипРесурсаСправочникСобытий, , , ДатаПолученияРесурсов);
	Иначе
		ДобавитьСтрокуВТаблицуАктуальныхРесурсов(ТаблицаАктуальныхРесурсов, 
												 ТипРесурсаСправочникСобытий, 
												 , 
												 , 
												 ?(ЗначениеЗаполнено(СвойстваХранимогоВИБСправочникаСобытий.Имя), 
												    ДатаПолученияРесурсов, 
													СвойстваХранимогоВИБСправочникаСобытий.ДатаПолучения));		
	КонецЕсли;
...
Функция ПолучитьРазобратьКлючевойФайл(ХранилищеРесурсов, ИмяКлючевогоФайла, УведомлятьОбОшибках, ОписаниеОшибки = "")
		
	// получаем ключевой файл
	ИмяВременногоКлючевогоФайла = ПолучитьРесурсССервера(ХранилищеРесурсов, ИмяКлючевогоФайла, ОписаниеОшибки);
	...
	Возврат РезультатРазбораКлючевогоФайла;
...
Функция ПолучитьДанныеПоСобытиям(Знач Актуализировать, Знач Таймаут = 0)
	
	//не обновляем информацию в режиме сервиса, так как она обновляется через механизм поставляемых данных
	Если ОбщегоНазначения.РазделениеВключено() Тогда 
		Актуализировать = Ложь;
	КонецЕсли;		
			
	УведомлятьОбОшибках = Ложь;
	ОписаниеОшибки = "";	
	ДеревоСправочникаСобытий = Неопределено;
	...
	ДанныеПоСобытиямИБ = ХранимаяВИБИнформация[ТипРесурсаСправочникСобытий];
	Если ДанныеПоСобытиямИБ <> Неопределено Тогда		
		//берем данные по событиям из кэша в БД
		ДеревоСправочникаСобытий = ЗагрузитьСтрокуXMLВДеревоЗначений(ДанныеПоСобытиямИБ.Данные, ОписаниеОшибки);
	КонецЕсли;
		
	Если Актуализировать Тогда
		//загружаем информацию о событиях из интернет
				
		// устанавливаем соединение с сервером
		Соединение = УстановитьСоединениеССерверомМеханизмаОнлайнСервисовРО(ОписаниеОшибки, Таймаут);
		Если Соединение = Неопределено Тогда
			ОписаниеОшибки = "Ошибка установки соединения с сервером справочной информации" + ?(ЗначениеЗаполнено(ОписаниеОшибки), ": " + ОписаниеОшибки, ".");
			Если УведомлятьОбОшибках Тогда
				ОбщегоНазначения.СообщитьПользователю(ОписаниеОшибки);
			...
			Возврат ДеревоСправочникаСобытий;
		КонецЕсли;
		...
		Если РезультатРазбораКлючевогоФайла = Неопределено Тогда
			Возврат ДеревоСправочникаСобытий;
		КонецЕсли; 
		...
		АктуализироватьСправочникСобытия(Соединение, 
										 ДеревоСправочникаСобытий, 
										 ТаблицаАктуальныхРесурсов, 
										 ИмяФайлаСправочникаСобытий, 
										 СвойстваХранимогоВИБСправочникаСобытий, 
...
Функция СохранитьДанныеВРесурсыМеханизмаОнлайнСервисовРО(ТаблицаАктуальныхРесурсов, УведомлятьОбОшибках, ОписаниеОшибки)
	
	РезультатЗаписиДанных = Истина;
	...
	Если НаборЗаписей.Модифицированность() Тогда
		...
		Исключение
			...
			РезультатЗаписиДанных = Ложь;
		КонецПопытки;
	КонецЕсли;	
	
	Возврат РезультатЗаписиДанных;
	
КонецФункции

Функция ТекущаяДатаДляПроверкиНаступленияСобытия()
	
	// сверяем дату по московскому времени
	ЧасовойПоясСобытий = "Europe/Moscow";
	// наступление события проверяется по дате сервера
	УниверсальнаяДата = УниверсальноеВремя(ТекущаяДата());
	ДатаПриведенная = МестноеВремя(УниверсальнаяДата, ЧасовойПоясСобытий);
	Возврат НачалоДня(ДатаПриведенная);
...
Функция ФорматироватьИмяСобытия(Знач Имя, Знач ВерхнийРегистр=Ложь) Экспорт
	...
	Имя = ?(ВерхнийРегистр, ВРег(СокрЛП(Имя)), СокрЛП(Имя));
	Возврат Имя;
...
//	НенайденноеСчитатьНаступившим 	- Булево - считать событие ненаступившим, если оно не найдено или произошла ошибка
//											   получения данных.
// Возвращаемое значение:
...
Функция СобытиеНаступило(Знач ИмяСобытия, Знач НенайденноеСчитатьНаступившим = Ложь) Экспорт
	...
	Если ТипЗнч(ИмяСобытия) = Тип("Массив") ИЛИ ТипЗнч(ИмяСобытия) = Тип("ФиксированныйМассив") Тогда
		МассивИменСобытий = ИмяСобытия;
		Если НенайденноеСчитатьНаступившим Тогда
			Для каждого ЭлементМассива Из МассивИменСобытий Цикл
				Если НЕ СобытиеНаступило(ЭлементМассива, НенайденноеСчитатьНаступившим) Тогда
					Возврат Ложь;
				КонецЕсли;
			КонецЦикла;
			Возврат Истина;
		...
		Иначе
			Для каждого ЭлементМассива Из МассивИменСобытий Цикл
				Если СобытиеНаступило(ЭлементМассива, НенайденноеСчитатьНаступившим) Тогда
					Возврат Истина;
				КонецЕсли;
			КонецЦикла;
			Возврат Ложь;
	...
	Актуализировать = Ложь;
	СведенияОСобытии = СобытиеНаступилоРасширенная(ИмяСобытия, Актуализировать);
	Возврат СведенияОСобытии.СобытиеНаступило ИЛИ (НенайденноеСчитатьНаступившим
		И (СведенияОСобытии.СобытиеНеНайдено ИЛИ СведенияОСобытии.ОшибкаПолученияДанных));
...
Функция СобытиеНаступилоРасширенная(Знач ИмяСобытия, Знач Актуализировать = Ложь) Экспорт
	...
	Результат.Вставить("СобытиеНеНайдено", Ложь);
	Результат.Вставить("ОшибкаПолученияДанных", Ложь);
	Результат.Вставить("СобытиеНаступило", Ложь);
	
	ДатаДляПроверкиСобытия = ТекущаяДатаДляПроверкиНаступленияСобытия();
	
	ИмяМетаданныхТекущейКонфигурации = СокрЛП(Метаданные.Имя);
	ВерсияТекущейКонфигурации = ВерсияПрограммы(); 
	
	ДеревоСправочникаСобытий = Неопределено;
	// Короткий таймаут.
	Таймаут = 10;
	Попытка
		// Подгружаем данные из интернет или берем из кэша.
		ДеревоСправочникаСобытий = ПолучитьДанныеПоСобытиям(Актуализировать, Таймаут);
	Исключение
		// Произошла ошибка получения данных, возможно сервер недоступен.
		Результат.ОшибкаПолученияДанных = Истина;
		Возврат Результат;
	КонецПопытки;
	...
	Если ДеревоСправочникаСобытий = Неопределено Тогда
		...
		Результат.СобытиеНеНайдено = Истина;
		Результат.ОшибкаПолученияДанных = Истина;
		Возврат Результат;
	КонецЕсли; 
	
	ИмяСобытия = ФорматироватьИмяСобытия(ИмяСобытия, Истина);
	// Ищем событие в дереве.
	Отбор = Новый Структура("Значение", ИмяСобытия);
	Строки = ДеревоСправочникаСобытий.Строки.НайтиСтроки(Отбор, Истина);
	Если Строки.Количество() = 0 Тогда
		// События нет в дереве.
		Результат.СобытиеНеНайдено = Истина;
		Возврат Результат;
	КонецЕсли; 
	
	Если Строки.Количество() > 1 Тогда
		// Больше одного события в дереве, такого быть не должно.
		Результат.ОшибкаПолученияДанных = Истина;
		Возврат Результат;
	КонецЕсли;
	...
	ДанныеСобытия = Строки[0].Родитель;
	Для каждого УзелСобытия Из ДанныеСобытия.Строки Цикл
		
		// Проверка дат.
		Если УзелСобытия.Имя = "ДатаНачала" Тогда
			ДатаНачала = СтрДатаXMLВДата1С(УзелСобытия.Значение);
			Если ДатаНачала > ДатаДляПроверкиСобытия Тогда
				Возврат Результат;
			КонецЕсли; 
		КонецЕсли; 
		
		Если УзелСобытия.Имя = "ДатаОкончания" Тогда
			ДатаОкончания = СтрДатаXMLВДата1С(УзелСобытия.Значение);
			Если ДатаОкончания < ДатаДляПроверкиСобытия Тогда
				Возврат Результат;
			КонецЕсли; 
		КонецЕсли;
		
		// Проверяем исключения.
		Если УзелСобытия.Имя = "Исключения" Тогда
			Для каждого УзелВыпуска Из УзелСобытия.Строки Цикл
				Если УзелВыпуска.Имя = "Выпуск" Тогда
					Для каждого ДанныеВыпуска Из УзелВыпуска.Строки  Цикл
						Если ДанныеВыпуска.Имя = "ИдентификаторМетаданных" Тогда
							ИдКонфигурации = ДанныеВыпуска.Значение;
						КонецЕсли; 
						Если ДанныеВыпуска.Имя = "Версия" Тогда
							Версия = ДанныеВыпуска.Значение;
						КонецЕсли;
					КонецЦикла;
					Если СокрЛП(Врег(ИдКонфигурации)) = СокрЛП(Врег(ИмяМетаданныхТекущейКонфигурации)) 
						и СокрЛП(Версия) = СокрЛП(ВерсияТекущейКонфигурации) Тогда
						Возврат Результат;
					КонецЕсли; 
				КонецЕсли; 
			КонецЦикла;
	...
	Если ДатаНачала = Дата(1, 1, 1) Тогда
		// Если у события не определена дата начала считаем, что оно не наступило.
		Возврат Результат;
	КонецЕсли; 
	
	Результат.СобытиеНаступило = Истина;
	Возврат Результат;
...
Процедура АктуализироватьСлужебнуюИнформациюМеханизмаОнлайнСервисовРОРегламентноеЗадание() Экспорт
	...
	ОбщегоНазначения.ПриНачалеВыполненияРегламентногоЗадания(Метаданные.РегламентныеЗадания.ОбновлениеДанныхОнлайнСервисовРО);
	
	ПараметрыИНастройки = ПолучитьПараметрыИНастройкиМеханизмаОнлайнСервисовРО();
	...
	Параметры.Вставить("МожноЗагружатьВнешниеФормыОтчетов", Ложь);
	
	АктуализироватьСлужебнуюИнформациюМеханизмаОнлайнСервисовРО(Параметры, Неопределено);
...
Функция ПрочитатьТекстовыйФайл(ИмяФайла, ОписаниеОшибки, УдалятьПослеПрочтения = Ложь, Кодировка = "windows-1251")
	
	Попытка
		ОбъектЧтение = Новый ЧтениеТекста(ИмяФайла, Кодировка);
...
Процедура ДобавитьСтрокуВТаблицуАктуальныхРесурсов(ТаблицаРесурсов, Тип, Имя = "", Данные = "", ДатаПолучения = Неопределено)
	...
	НовСтр.ДатаПолучения	= ?(ДатаПолучения = Неопределено, ТекущаяДатаСеанса(), ДатаПолучения);
...
Функция УсловиеВыполняется(Условие, ИДПеременной, ЗначениеПеременной)
	...
	Попытка
		РезультатВычисления = ОбщегоНазначения.ВычислитьВБезопасномРежиме(СтрЗаменить(Условие, ИДПеременной, ЗначениеПеременной));
	Исключение
		Возврат Ложь;
	КонецПопытки;
...
Функция ПроверкаФорматаПоПериодуПримененияСервер(
			СтруктураРеквизитовФормы, ДатаКонцаПериодаОтчета, ИмяФормы, ИДОтчета, СведенияОФормате = Неопределено) Экспорт
	...
	// если заполнен параметр функции со сведениями о формате, то используем эти сведения для проверки на применимость формата в указанном периоде
	...
	// проверяем форму на применимость в указанном периоде
	ПараметрыДиалогаБлокировки = Неопределено;
	Если ФорматПрименимВУказанномПериоде(ТаблицаПериодовПримененияФорматов, КодФормы, ДатаПриказаФормы, НомерПриказаФормы, ВерсияФормата, ДатаПриказаФормат, НомерПриказаФормат, ДатаКонцаПериодаОтчета, ПараметрыДиалогаБлокировки)
		И ИмяФайлаXSDСхемыСоответствует(ТаблицаПериодовПримененияФорматов, КодФормы, ВерсияФормата, ИмяФайлаСхемы, ПараметрыДиалогаБлокировки) Тогда
		Возврат СведенияПоБлокировке;
	КонецЕсли;
		...
		Если УзелФрм.Имя = "Фрм" И УзелФрм.Тип = "Э" Тогда
			...
			Для Каждого УзелФрмт Из УзелФрм.Строки Цикл
				Если УзелФрмт.Имя = "Фрмт" И УзелФрмт.Тип = "Э" Тогда
					...
					НовСтр.ИмяФайлаСхемыВыгрузки	= ?(АтрибутыУзлаФрмт.Свойство("ИмСх") И ЗначениеЗаполнено(АтрибутыУзлаФрмт.ИмСх),	СокрЛП(АтрибутыУзлаФрмт.ИмСх),				Неопределено);
					НовСтр.ИзмененияКритичны		= (АтрибутыУзлаФрмт.Свойство("ИзмКрт") И АтрибутыУзлаФрмт.ИзмКрт = "1");
...
Функция ИмяФайлаXSDСхемыСоответствует(ТаблицаПериодовПримененияФорматов, КодФормы, ВерсияФормата, ИмяФайлаСхемы, ПараметрыДиалогаБлокировки)
	Если НЕ ЗначениеЗаполнено(КодФормы) ИЛИ НЕ ЗначениеЗаполнено(ВерсияФормата) Тогда
		Возврат Истина;
	КонецЕсли;
	РезультатПоиска = ТаблицаПериодовПримененияФорматов.НайтиСтроки(Новый Структура("КодФормы, Версия", КодФормы, ВерсияФормата));
	Если РезультатПоиска.Количество() = 0 Тогда
		Возврат Истина;
	КонецЕсли;
	СтрРезультатПоиска = РезультатПоиска[0];
	ИмяФайлаXSDСхемыИзУО = ВРег(СтрРезультатПоиска.ИмяФайлаСхемыВыгрузки);
	Если (ИмяФайлаXSDСхемыИзУО = ИмяФайлаСхемы) ИЛИ (НЕ СтрРезультатПоиска.ИзмененияКритичны) Тогда
		Возврат Истина;
	Иначе
		ИмяФайлаXSDСхемыФайлаОбмена = ?(ЗначениеЗаполнено(ИмяФайлаСхемы), СокрЛП(ИмяФайлаСхемы), " [файл не указан] ");
		ТекстЗаголовок = "XSD-схема " + ИмяФайлаXSDСхемыФайлаОбмена + " более не применяется.";
		Подробно = "Необходимо обновить регламентированный отчет.";
		ПараметрыДиалогаБлокировки = Новый Структура;
		ПараметрыДиалогаБлокировки.Вставить("Жесткая", Истина);
		ПараметрыДиалогаБлокировки.Вставить("Подробно", Подробно);
		ПараметрыДиалогаБлокировки.Вставить("Заголовок", ТекстЗаголовок);
		ПараметрыДиалогаБлокировки.Вставить("ЗаменяющийДокумент", ИмяФайлаXSDСхемыИзУО);
		Возврат Ложь;
	КонецЕсли;
КонецФункции
...
Функция СоздатьТаблицуПериодовПримененияФорматов()
	...
	Результат.Колонки.Добавить("ДатаНачалаСообщение");
	Результат.Колонки.Добавить("ДатаОкончанияСообщение");
	Результат.Колонки.Добавить("ИмяФайлаСхемыВыгрузки");
	Результат.Колонки.Добавить("ИзмененияКритичны");
	Результат.Колонки.Добавить("Обновления");
...
Процедура ЗагрузитьВнешниеОтчетыВСправочникРО(ТаблицаВнешнихФормРО)
	...
	УстановитьПривилегированныйРежим(Истина);
	Аутентификация = ОнлайнСервисыРегламентированнойОтчетности.ПараметрыАутентификацииНаСайте();
	УстановитьПривилегированныйРежим(Ложь);
	...
	Если НЕ ПустаяСтрока(СтруктураURI.ПутьНаСервере) Тогда
		...
		Если ПозицияПараметры > 0 Тогда
			...
			Если ПозицияПараметраПуть > 0 Тогда
				...
				ПараметрПутьНаСервере = СтрЗаменить(ПараметрПутьНаСервере , "%5c", "/");
				АдресСайта = "https://downloads.v8.1c.ru/ftp//" + ПараметрПутьНаСервере;
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
...
Процедура ОбработатьИЗаписатьВнешнийОтчетВСправочникРО(ПараметрыВнешнейФормы, ПутьКФайлу)
	...
	Попытка
		...
		ОбъектВнешнийОтчет = ВнешниеОтчеты.Создать(ФайлОтчета.ПолноеИмя, Истина, ОбщегоНазначения.ОписаниеЗащитыБезПредупреждений());
...
Процедура ЗаписатьВЖурналРегистрации(Знач Уровень, ТекстОшибки) Экспорт
	
	УровеньВажности = ?(ТипЗнч(Уровень) = Тип("Строка"), ПредопределенноеЗначение(Уровень), Уровень);
	ЗаписьЖурналаРегистрации(НСтр("ru = 'Загрузка внешних регламентированных отчетов'"), УровеньВажности,,, ТекстОшибки);
