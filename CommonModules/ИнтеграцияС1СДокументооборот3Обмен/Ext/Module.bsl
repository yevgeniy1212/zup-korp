...
Функция ПолучитьXDTOИзмененийИзОбъекта(Прокси, ДанныеОбъекта) Экспорт
	...
	СтруктураСозданияОбъекта.externalObject = ИнтеграцияС1СДокументооборотБазоваяФункциональность.СоздатьОбъект(
		Прокси,
		"ExternalObjectID");
	СтруктураСозданияОбъекта.externalObject.ID = Строка(ДанныеОбъекта.Объект.УникальныйИдентификатор());
	СтруктураСозданияОбъекта.externalObject.type = ДанныеОбъекта.Объект.Метаданные().ПолноеИмя();
