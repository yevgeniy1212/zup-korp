...
Процедура УстановитьВидЗанятостиНовогоСотрудника(Форма) Экспорт
	Если Не ЗначениеЗаполнено(Форма.Сотрудник.Ссылка) И Не Форма.ИспользоватьКадровыйУчет Тогда
		Форма.ТекущийВидЗанятости = СотрудникиВызовСервера.ПолучитьВидЗанятостиДляНовогоСотрудника(Форма.Сотрудник,
			Форма.ТекущаяОрганизация, Форма.ФизическоеЛицоСсылка);
	КонецЕсли;
КонецПроцедуры
...
Функция ПараметрыПоискаПохожих() Экспорт
	...
	ПараметрыПоиска.Вставить("Ссылка", ПредопределенноеЗначение("Справочник.ФизическиеЛица.ПустаяСсылка"));
	...
	ПараметрыПоиска.Вставить("ФИОПолностью", Ложь);
	ПараметрыПоиска.Вставить("ТочноеСовпадение", Ложь);
	ПараметрыПоиска.Вставить("ОбязательноЗаполненноеФИО", Истина);
...
Процедура УстановитьМаскуВводаКодаПодразделения(Форма) Экспорт
	
	ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(
		Форма.Элементы,
		"ДокументыФизическихЛицКодПодразделения",
		"Маска",
		ДокументыФизическихЛицКлиентСервер.МаскаВводаКодаПодразделения(Форма.ДокументыФизическихЛиц.ВидДокумента));
...
Процедура УстановитьИнфоНадпись(Форма, ДатаСеанса) Экспорт
	...
	ИначеЕсли ЗначениеЗаполнено(Форма.ДатаУвольнения) Тогда
		...
		Если ПрошлоДнейСМоментаУвольнения > 370 Тогда
			СотрудникИнфо = СтрШаблон(
				НСтр("ru = 'Сотрудник давно уволен. Для того, чтобы сотрудник не отображался в списках, можно установить флажок ""%1""'"),
...
Процедура ОбработатьОтображениеПоляИНН(ИНН, Элемент, Форма) Экспорт
	...
	Иначе
		...
		СообщенияПроверки = НСтр("ru = 'Не указан ИИН'");
		ЭлементЦветТекста = Форма.ЦветСтиляЦветТекстаПоля;
...
Процедура УстановитьПодсказкуКДатеРождения(Форма) Экспорт
	...
	Иначе
		ТекстПодсказки = НСтр("ru='Не указана дата рождения'");
