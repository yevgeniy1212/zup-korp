...
Процедура ЭкспортироватьСертификатВBase64(ОповещениеОЗавершении, Сертификат, ВыводитьСообщения, МенеджерКриптографии, ДвоичныеДанные) Экспорт
	...
	Если КриптографияЭДКОКлиентСервер.СертификатВЗащищенномХранилищеНаСервере(Сертификат) 
		ИЛИ КриптографияЭДКОКлиентСервер.СертификатОблачнойПодписи(Сертификат) Тогда
		ВыполнитьОбработкуОповещения(
			ОповещениеОЗавершении,
			КриптографияЭДКОКлиентСервер.ПодготовитьРезультат(
				Истина, "СтрокаBase64", 
...
Процедура ЗашифроватьPKCS7(
	...
	ВходящиеПараметры = ПараметрыВМассив(5, ИсходныйФайл, Сертификаты, ИмяФайлаИлиРасширениеИлиАдрес, МестоХраненияКлюча, Алгоритм);
	...
	Если СертификатыОблачнойПодписи
		...
		ПредварительноеОповещениеОЗавершении = Новый ОписаниеОповещения("ЗашифроватьPKCS7ШифрованиеСервисомОблачнойПодписи", ЭтотОбъект, ПараметрыЦикла);
...
Функция ПроверитьНастройкиУчетнойЗаписиОблачнойПодписи(УчетнаяЗаписьОблачнойПодписи, ОповещениеОЗавершении = Неопределено, ВидОперации = "")
	...
	ИначеЕсли ОповещениеОЗавершении <> Неопределено Тогда
		...
		Если ЗначениеЗаполнено(ВидОперации) Тогда
			ТекстОшибки = НСтр("ru = 'Для выполнения операции: '") + ВидОперации 
