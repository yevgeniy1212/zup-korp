...
Функция ДоступныеЛокальныеСертификатыПользователя() Экспорт
	...
	Запрос.Текст =
		...
		|	Организации.УчетнаяЗаписьОбмена.МодельХраненияЗакрытогоКлюча КАК МодельХраненияЗакрытогоКлюча
		|ИЗ
	...
	Пока Выборка.Следующий() Цикл
		Если НЕ КриптографияЭДКОКлиентСервер.ЭтоЛокальнаяПодпись(Выборка.МодельХраненияЗакрытогоКлюча) Тогда
