...
// Устанавливает подсказку ввода для реквизитов:
// НаименованиеПервичногоДокумента, ДатаПервичногоДокумента, НомерПервичногоДокумента
Процедура УстановитьОтображениеРеквизитовПервичногоДокумента(УправляемаяФорма) Экспорт
	
	Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(УправляемаяФорма.Объект, "НомерПриказа")
		И ЗначениеЗаполнено(УправляемаяФорма.Объект.НомерПриказа) Тогда
		ПодсказкаВвода = УправляемаяФорма.Объект.НомерПриказа;
	ИначеЕсли ЗначениеЗаполнено(УправляемаяФорма.Объект.Номер) Тогда
		ПодсказкаВвода = УправляемаяФорма.Объект.Номер;
	Иначе
		ПодсказкаВвода = "";
	КонецЕсли;
	...
	Если ЗначениеЗаполнено(УправляемаяФорма.Объект.Дата) Тогда
		ПодсказкаВвода = Формат(УправляемаяФорма.Объект.Дата,"ДФ=dd.MM.yyyy");
	Иначе
		ПодсказкаВвода = "";
	КонецЕсли;
	...
	Если ЗначениеЗаполнено(УправляемаяФорма.Объект.ДатаПервичногоДокумента) Тогда
		УправляемаяФорма.ДатаПервичногоДокументаПредставление = Формат(УправляемаяФорма.Объект.ДатаПервичногоДокумента,"ДФ=dd.MM.yyyy");
	КонецЕсли;
		
	#Если Сервер Или ТолстыйКлиентОбычноеПриложение Или ВнешнееСоединение Тогда
		ПодсказкаВвода = Строка(УправляемаяФорма.Объект.Ссылка.Метаданные().Представление());
	#Иначе
		ПодсказкаВвода = "";
	#КонецЕсли
	
	Если ЗначениеЗаполнено(ПодсказкаВвода) Тогда
		ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(
			УправляемаяФорма.Элементы,
...
Функция ИмяРеквизитаРазрешеноРедактироватьСторно(ИмяТаблицы) Экспорт
	
	Возврат ИмяТаблицы + "РазрешеноРедактироватьСторно";
...
Функция ИмяПВРПоИмениТаблицы(ИмяТаблицы) Экспорт
	
	Если ИмяТаблицы = "Удержания" Или ИмяТаблицы = "УдержанияПерерасчет" Тогда
		Возврат "Удержания";
	Иначе
		Возврат "Начисления";
	КонецЕсли;
...
Процедура УстановитьВидимостьГруппыПоказателей(Форма, ИмяТаблицы, Видимость) Экспорт 
	...
	ИмяПВР = ИмяПВРПоИмениТаблицы(ИмяТаблицы);
...
//
Функция УникальноеИмяРеквизита(Ссылка = Неопределено) Экспорт
	УникальныйИдентификатор = ?(Ссылка = Неопределено, Новый УникальныйИдентификатор, Ссылка.УникальныйИдентификатор());
	Возврат СтрЗаменить(Строка(УникальныйИдентификатор), "-", "");
КонецФункции
...
Функция РазницаВМесяцах(НачалоПериода, КонецПериода) Экспорт
	Возврат Месяц(КонецПериода) - Месяц(НачалоПериода) + 12 * (Год(КонецПериода) - Год(НачалоПериода));
КонецФункции
...
Процедура ВключитьФлагНеобходимостиВыполненияРасчета(Форма, ИмяКнопкиРассчитать = "Рассчитать") Экспорт
