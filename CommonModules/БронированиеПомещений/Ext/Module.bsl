...
Процедура ОтобразитьДанныеВнешнихСистем(Планировщик, ДоступностьПомещений, Помещения, ДатаНачала, ДатаОкончания) Экспорт
	
	Для Каждого Помещение Из Помещения Цикл
		ДоступностьПомещения = ДоступностьПомещений[Помещение];
		Если ДоступностьПомещения = Неопределено Тогда
			Продолжить;
		КонецЕсли;
		
		Если ДоступностьПомещения.ДоступностьПодробно.Количество() <> 0 Тогда
		
			Для каждого ЭлементДоступности Из ДоступностьПомещения.ДоступностьПодробно Цикл
				
				ДобавитьЭлементПланировщика(
					Планировщик, Помещение, ЭлементДоступности.ДатаНачала, ЭлементДоступности.ДатаОкончания);
		...
		Иначе
			...
			Дата1 = Неопределено;
			Дата2 = Неопределено;
			ТекВремя = НачалоЧаса(ДатаНачала);

			Для ИндексСимвола = 1 По СтрДлина(ДоступностьПомещения.ДоступностьСтрокой) Цикл
				ТекущийСимвол = Символ(КодСимвола(ДоступностьПомещения.ДоступностьСтрокой, ИндексСимвола));
				Если ТекущийСимвол = "0" Тогда
					Если Не Дата1 = Неопределено Тогда
						Дата2 = ТекВремя;
					КонецЕсли;
				ИначеЕсли Дата1 = Неопределено Тогда
					Дата1 = ТекВремя;
				КонецЕсли;
				ТекВремя = ТекВремя + ШагДаты;
		
				Если Не Дата2 = Неопределено Тогда
					ДобавитьЭлементПланировщика(Планировщик, Помещение, Дата1, Дата2);
					Дата1 = Неопределено;
					Дата2 = Неопределено;
				КонецЕсли;
...
Процедура ДобавитьЭлементПланировщика(Планировщик, Помещение, ДатаНачала, ДатаОкончания)
	
	Элемент = Планировщик.Элементы.Добавить(ДатаНачала, ДатаОкончания);
	Элемент.Текст = СинхронизацияКалендарей.ТекстЭлементаПланировщика(
		НСтр("ru = 'Занято'"), ПредопределенноеЗначение("Перечисление.ТипыСинхронизацииКалендарей.MSExchange"));
	Элемент.ЦветФона = ЦветаСтиля.ЦветКалендаряПоУмолчанию;
	УстановитьЗначениеИзмерения(Элемент, "Помещение", Помещение);
	Элемент.Значение = Новый Структура("СобытиеВнешнейСистемы, ВесьДень");
	Элемент.Значение.СобытиеВнешнейСистемы = Истина;
	Элемент.Значение.ВесьДень = Ложь;
