...
Функция ПредставлениеДокументаРеглОтч(Док) Экспорт
	...
	Если СтрНачинаетсяС(Представление, "Расчет суммы утилизационного сбора (колесные ТС)") Тогда
		Представление = "Расчет суммы утилизационного сбора (колесные ТС) от "
		+ ПредставлениеПериода(НачалоДня(Док.ДатаОкончания), КонецДня(Док.ДатаОкончания), "ФП=Истина");
		
	ИначеЕсли СтрНачинаетсяС(Представление, "Расчет суммы утилизационного сбора (самоходные машины)") Тогда
		Если НачалоДня(Док.ДатаНачала) = НачалоДня(Док.ДатаОкончания) Тогда
			Представление = "Расчет суммы утилизационного сбора (самоходные машины) от "
			+ ПредставлениеПериода(НачалоДня(Док.ДатаОкончания), КонецДня(Док.ДатаОкончания), "ФП=Истина");
		Иначе
			Представление = "Расчет суммы утилизационного сбора (самоходные машины) за "
			+ ПредставлениеПериода(НачалоДня(Док.ДатаНачала), КонецДня(Док.ДатаОкончания), "ФП=Истина");
		КонецЕсли;
		
	ИначеЕсли СтрНачинаетсяС(Представление, "Страновой отчет") Тогда
	...
	ИначеЕсли СтрНачинаетсяС(Представление, "Уведомление о ввозе товаров, подлежащих прослеживаемости") Тогда
		Представление = "Уведомление о ввозе товаров, подлежащих прослеживаемости, от "
		+ ПредставлениеПериода(НачалоДня(Док.ДатаОкончания), КонецДня(Док.ДатаОкончания), "ФП=Истина");
		
	ИначеЕсли СтрНачинаетсяС(Представление, "Уведомление об остатках товаров, подлежащих прослеживаемости") Тогда
		Представление = "Уведомление об остатках товаров, подлежащих прослеживаемости, от "
		+ ПредставлениеПериода(НачалоДня(Док.ДатаОкончания), КонецДня(Док.ДатаОкончания), "ФП=Истина");
		
	ИначеЕсли СтрНачинаетсяС(Представление, "Уведомление о перемещении товаров, подлежащих прослеживаемости") Тогда
		Представление = "Уведомление о перемещении товаров, подлежащих прослеживаемости, от "
		+ ПредставлениеПериода(НачалоДня(Док.ДатаОкончания), КонецДня(Док.ДатаОкончания), "ФП=Истина");
	...
	Если Док.Вид = 0 ИЛИ Док.Вид = "П" Тогда
		...
		// в ЗУПК используется Очередной
		Представление = Представление + " (Очередной";
        // ЗУПК
	...
	Иначе
		...
		// в ЗУПК используется Код в налоговом органе
		Представление = Представление + ". Код налогового органа: " + СокрЛП(Док.КодНалоговогоОргана) + ")";
        // ЗУПК
...
Процедура ЗаполнитьСведенияОбОрганизацииДляОтчетаСтатистики(Форма, ВПрограммеИзмененаДатаПодписи = Ложь, ИмяПоля = "ПолеТабличногоДокументаФормаОтчета", СхемаНаимПолноеКраткое = Ложь) Экспорт
	...
	Попытка
		Если (Форма.СтруктураРеквизитовФормы.мВПрограммеИзмененаОрганизация)
			...
			Иначе
				СтрАдрПрописки_JSON =
					?(ЗначениеЗаполнено(АдрПрописки_JSON), АдрПрописки_JSON, Форма.СтруктураРеквизитовФормы["Адрес_JSON"]);
				Если ЗначениеЗаполнено(СтрАдрПрописки_JSON) Тогда
					ПредставлениеАдреса =
						РегламентированнаяОтчетностьВызовСервера.ПредставлениеАдресаJSON(СтрАдрПрописки_JSON);
					Форма.СтруктураРеквизитовФормы.Адрес_JSON = СтрАдрПрописки_JSON;
					Форма[ИмяПоля].Области.ОргАдрес.Значение = ПредставлениеАдреса;
	...
	Если СвойствоОпределено(Форма, "СтруктураДанныхПодписи") Тогда
		Если ТипЗнч(Форма.СтруктураДанныхПодписи) = Тип("Структура") Тогда
	...
	Если (Форма.СтруктураРеквизитовФормы.мВПрограммеИзмененаОрганизация) Или (ПустаяСтрока(Форма.ПолеТабличногоДокументаФормаОтчета.Области.ОргАдрес.Значение)) Тогда
		...
		Если ЗначениеЗаполнено(СтрАдрПочт_JSON) Тогда
			ПредставлениеАдреса =
				РегламентированнаяОтчетностьВызовСервера.ПредставлениеАдресаJSON(СтрАдрПочт_JSON);
			Форма.СтруктураРеквизитовФормы.Адрес_JSON = СтрАдрПочт_JSON;
			Форма.ПолеТабличногоДокументаФормаОтчета.Области.ОргАдрес.Значение = ПредставлениеАдреса;
			Форма.СтруктураРеквизитовФормы.мАдресвФорматеДляВыгрузки = АдрПочт;
		Иначе
...
Функция СобратьДанныеТекущегоТаблПоля(Форма, ИмяТабличногоПоля) Экспорт 
	...
	Если ЗначениеЗаполнено(Форма.СтруктураРеквизитовФормы.мАдресвФорматеДляВыгрузки)
	   И Форма.СтруктураРеквизитовФормы.мДатаНачалаПериодаОтчета < '20200101' Тогда
...
Процедура ОтрисоватьЗначкиУдаленияСтрок(Форма, ДлинаИмениОбластейСДопСтроками = 26) Экспорт 
	...
	ТД = Новый Структура("ТабличныйДокумент, ПолеТабличногоДокументаФормаОтчета");
	ЗаполнитьЗначенияСвойств(ТД, Форма);
	Если ТД.ТабличныйДокумент <> Неопределено Тогда 
...
Процедура ПриИнициализацииФормыРегламентированногоОтчетаОбщиеДействия(Форма, КонтролирующийОрган)
	...
	// Устанавливаем видимость ЭУ регламентированных отчетов из настроек, добавляем команду разрешения редактирования.
	Если (СтрНачинаетсяС(Форма.ИмяФормы, "Отчет") ИЛИ СтрНачинаетсяС(Форма.ИмяФормы, "ВнешнийОтчет"))
		...
		#Если НаСервере Тогда
			...
			// Отчет, успешно сданнный в контролирующий орган, будет заблокирован для редактирования.
...
Функция ОперацииСРегламентированнымОтчетом(ИмяОтчета, ИмяФормы) Экспорт
	...
	РеглОтчеты.Вставить(ВРег("РегламентированныйОтчетПрибыль.ФормаОтчета2020Кв4"),
		Новый ФиксированнаяСтруктура(ОперацииСРеглОтчетом));
	РеглОтчеты.Вставить(ВРег("РегламентированныйОтчетПрибыль.ФормаОтчета2021Кв4"),
		Новый ФиксированнаяСтруктура(ОперацииСРеглОтчетом));
	РеглОтчеты.Вставить(ВРег("РегламентированныйОтчетПрибыль.ФормаОтчета2022Кв4"),
		Новый ФиксированнаяСтруктура(ОперацииСРеглОтчетом));
	
	РеглОтчеты.Вставить(ВРег("РегламентированныйОтчетНДС.ФормаОтчета2020Кв4"),
		Новый ФиксированнаяСтруктура(ОперацииСРеглОтчетом));
	РеглОтчеты.Вставить(ВРег("РегламентированныйОтчетНДС.ФормаОтчета2021Кв3"),
		Новый ФиксированнаяСтруктура(ОперацииСРеглОтчетом));
	РеглОтчеты.Вставить(ВРег("РегламентированныйОтчетНДС.ФормаОтчета2022Кв1"),
		Новый ФиксированнаяСтруктура(ОперацииСРеглОтчетом));
	РеглОтчеты.Вставить(ВРег("РегламентированныйОтчетНДС.ФормаОтчета2023Кв1"),
		Новый ФиксированнаяСтруктура(ОперацииСРеглОтчетом));
	...
	РеглОтчеты.Вставить(ВРег("РегламентированныйОтчетУСН.ФормаОтчета2015Кв1"),
		Новый ФиксированнаяСтруктура(ОперацииСРеглОтчетом));
	РеглОтчеты.Вставить(ВРег("РегламентированныйОтчетУСН.ФормаОтчета2021Кв1"),
		Новый ФиксированнаяСтруктура(ОперацииСРеглОтчетом));
	РеглОтчеты.Вставить(ВРег("РегламентированныйОтчетУСН.ФормаОтчета2023Кв1"),
		Новый ФиксированнаяСтруктура(ОперацииСРеглОтчетом));
	...
	РеглОтчеты.Вставить(ВРег("РегламентированныйОтчетРасчетПоСтраховымВзносам.ФормаОтчета2020Кв1"),
		Новый ФиксированнаяСтруктура(ОперацииСРеглОтчетом));
	РеглОтчеты.Вставить(ВРег("РегламентированныйОтчетРасчетПоСтраховымВзносам.ФормаОтчета2020Кв4"),
		Новый ФиксированнаяСтруктура(ОперацииСРеглОтчетом));
	РеглОтчеты.Вставить(ВРег("РегламентированныйОтчетРасчетПоСтраховымВзносам.ФормаОтчета2022Кв1"),
		Новый ФиксированнаяСтруктура(ОперацииСРеглОтчетом));
	РеглОтчеты.Вставить(ВРег("РегламентированныйОтчетРасчетПоСтраховымВзносам.ФормаОтчета2023Кв1"),
		Новый ФиксированнаяСтруктура(ОперацииСРеглОтчетом));
	
	РеглОтчеты.Вставить(ВРег("РегламентированныйОтчетПерсонифицированныеСведенияОФизическихЛицах.ФормаОтчета2023Кв1"),
		Новый ФиксированнаяСтруктура(ОперацииСРеглОтчетом));
	
	РеглОтчеты.Вставить(ВРег("РегламентированныйОтчет6_НДФЛ.ФормаОтчета2021Кв1"),
		Новый ФиксированнаяСтруктура(ОперацииСРеглОтчетом));
	РеглОтчеты.Вставить(ВРег("РегламентированныйОтчет6_НДФЛ.ФормаОтчета2021Кв4"),
		Новый ФиксированнаяСтруктура(ОперацииСРеглОтчетом));
	РеглОтчеты.Вставить(ВРег("РегламентированныйОтчет6_НДФЛ.ФормаОтчета2023Кв1"),
		Новый ФиксированнаяСтруктура(ОперацииСРеглОтчетом));
	...
	РеглОтчеты.Вставить(ВРег("РегламентированныйОтчет4ФСС.ФормаОтчета2022Кв2"),
		Новый ФиксированнаяСтруктура(ОперацииСРеглОтчетом));
	...
	РеглОтчеты.Вставить(ВРег("РегламентированныйОтчетЕСХН.ФормаОтчета2015Кв1"),
		Новый ФиксированнаяСтруктура(ОперацииСРеглОтчетом));
	РеглОтчеты.Вставить(ВРег("РегламентированныйОтчетЕСХН.ФормаОтчета2020Кв1"),
		Новый ФиксированнаяСтруктура(ОперацииСРеглОтчетом));
	
	РеглОтчеты.Вставить(ВРег("РегламентированныйОтчет3НДФЛ.ФормаОтчета2020кв1"),
		Новый ФиксированнаяСтруктура(ОперацииСРеглОтчетом));
	РеглОтчеты.Вставить(ВРег("РегламентированныйОтчет3НДФЛ.ФормаОтчета2021кв1"),
		Новый ФиксированнаяСтруктура(ОперацииСРеглОтчетом));
	РеглОтчеты.Вставить(ВРег("РегламентированныйОтчет3НДФЛ.ФормаОтчета2022кв1"),
		Новый ФиксированнаяСтруктура(ОперацииСРеглОтчетом));
	РеглОтчеты.Вставить(ВРег("РегламентированныйОтчет3НДФЛ.ФормаОтчета2023кв1"),
		Новый ФиксированнаяСтруктура(ОперацииСРеглОтчетом));
	
	РеглОтчеты.Вставить(ВРег("РегламентированныйОтчетИмущество.ФормаОтчета2020Кв4"),
		Новый ФиксированнаяСтруктура(ОперацииСРеглОтчетом));
	РеглОтчеты.Вставить(ВРег("РегламентированныйОтчетИмущество.ФормаОтчета2020Кв4_1"),
		Новый ФиксированнаяСтруктура(ОперацииСРеглОтчетом));
	РеглОтчеты.Вставить(ВРег("РегламентированныйОтчетИмущество.ФормаОтчета2021Кв4"),
		Новый ФиксированнаяСтруктура(ОперацииСРеглОтчетом));
	РеглОтчеты.Вставить(ВРег("РегламентированныйОтчетИмущество.ФормаОтчета2022Кв1"),
		Новый ФиксированнаяСтруктура(ОперацииСРеглОтчетом));
	РеглОтчеты.Вставить(ВРег("РегламентированныйОтчетИмущество.ФормаОтчета2022Кв4"),
		Новый ФиксированнаяСтруктура(ОперацииСРеглОтчетом));
...
Функция СтатусыОбъектовРеглОтчетностиПриРучномВводе(ОбъектРеглОтчетности = Неопределено, ВидКонтролирующегоОргана = Неопределено) Экспорт
	...
	ИначеЕсли ТипЗнч(ОбъектРеглОтчетности) = Тип("ФормаКлиентскогоПриложения")
		И (СтрНачинаетсяС(ОбъектРеглОтчетности.ИмяФормы, "Справочник.МашиночитаемыеДоверенностиРаспределенныйРеестр")
			ИЛИ СтрНачинаетсяС(ОбъектРеглОтчетности.ИмяФормы, "Справочник.МашиночитаемыеДоверенностиФНС")
			ИЛИ СтрНачинаетсяС(ОбъектРеглОтчетности.ИмяФормы, "Справочник.ЗаявленияОбОтзывеМЧДФНС")
			ИЛИ СтрНачинаетсяС(ОбъектРеглОтчетности.ИмяФормы, "Справочник.МашиночитаемыеДоверенностиФСС"))
		ИЛИ ТипЗнч(ОбъектРеглОтчетности) = Тип("СправочникСсылка.МашиночитаемыеДоверенностиРаспределенныйРеестр")
		ИЛИ ТипЗнч(ОбъектРеглОтчетности) = Тип("СправочникСсылка.МашиночитаемыеДоверенностиФНС")
		ИЛИ ТипЗнч(ОбъектРеглОтчетности) = Тип("СправочникСсылка.ЗаявленияОбОтзывеМЧДФНС")
		ИЛИ ТипЗнч(ОбъектРеглОтчетности) = Тип("СправочникСсылка.МашиночитаемыеДоверенностиФСС") Тогда
		
		Статусы = Новый Массив;
		Статусы.Добавить("В работе");
		Статусы.Добавить("Подготовлено");
		Статусы.Добавить("Зарегистрировано");
		Статусы.Добавить("Не зарегистрировано");
		Если ТипЗнч(ОбъектРеглОтчетности) = Тип("ФормаКлиентскогоПриложения")
			...
			Статусы.Добавить("Отозвано");
		КонецЕсли;
		
	ИначеЕсли ТипЗнч(ОбъектРеглОтчетности) = Тип("ФормаКлиентскогоПриложения")
		И СтрНачинаетсяС(ОбъектРеглОтчетности.ИмяФормы, "Справочник.МашиночитаемыеДоверенностиЦБ")
		ИЛИ ДокументооборотСКОКлиентСервер.ПодсистемаЦБСуществует()
		И ТипЗнч(ОбъектРеглОтчетности) = Тип("СправочникСсылка." + ИмяТипаСправочникаМашиночитаемыеДоверенностиЦБ) Тогда
		
		Статусы = Новый Массив;
		Статусы.Добавить("В работе");
		Статусы.Добавить("Подписано");
...
Функция ПредставлениеСтраницыРаздела3ФормыРСВ(СтруктураПараметров) Экспорт
	...
	Возврат ПредставлениеСтраницыРазделаРеглОтчетаСодержащегоДанныеФизическихЛиц(СтруктураПараметров);
	
КонецФункции

Функция ПредставлениеСтраницыРазделаРеглОтчетаСодержащегоДанныеФизическихЛиц(СтруктураПараметров) Экспорт
	
	Фам = СокрЛП(СтруктураПараметров.Фам);
	Имя = СокрЛП(СтруктураПараметров.Имя);
	Отч = СокрЛП(СтруктураПараметров.Отч);
	
	ДатаРожд = ?(СтруктураПараметров.Свойство("ДатаРожд"), СтруктураПараметров.ДатаРожд, Неопределено);
	СНИЛС = ?(СтруктураПараметров.Свойство("СНИЛС"), СтруктураПараметров.СНИЛС, Неопределено);
	
	ОтображатьНомерСтраницыПередФИО = ?(СтруктураПараметров.Свойство("ОтображатьНомерСтраницыПередФИО"),
	СтруктураПараметров.ОтображатьНомерСтраницыПередФИО, Неопределено);
	...
	КонецЕсли;
	Если ЗначениеЗаполнено(Предст) И ЗначениеЗаполнено(ДатаРожд) Тогда
		Предст = Предст + " " + Формат(ДатаРожд, "ДФ=дд.ММ.гггг");
	КонецЕсли;
	Если ЗначениеЗаполнено(Предст) И ЗначениеЗаполнено(СНИЛС) Тогда
		Предст = Предст + " " + СНИЛС;
	КонецЕсли;
	...
	Если СтруктураПараметров.Свойство("ПроверкаВыгрузкиСтраницы") Тогда
		...
		Иначе
			Возврат "Страница " + Формат(СтруктураПараметров.НомерСтраницы, "ЧГ=") + " (" + Предст + ")";
		КонецЕсли;
	...
	Иначе // представление страницы в форме отчета (или форме предварительного просмотра перед выводом на печать)
		...
		Иначе
			Если ОтображатьНомерСтраницыПередФИО = Истина Тогда
				Возврат Формат(СтруктураПараметров.НомерСтраницы, "ЧГ=") + ". " + Предст;
			Иначе
				Возврат Предст;
			КонецЕсли;
		КонецЕсли;
...
Процедура ИзменитьВариантыЗаполненияПоказателейТаблицы1_1Формы4ФСС(ВариантыЗаполненияТаблицы1_1,
	...
	Для Каждого ВариантЗаполненияТаблицы1_1 Из ВариантыЗаполненияТаблицы1_1 Цикл
		...
		Если ВариантЗаполненияТаблицы1_1.КодПоказателя = "П000110000106"
			ИЛИ ВариантЗаполненияТаблицы1_1.КодПоказателя = "П000110000107" Тогда
...
Процедура ИзменитьВариантыЗаполненияПоказателейТаблицы2Формы4ФСС(ВариантыЗаполненияТаблицы2,
																 СтруктураРеквизитовФормы) Экспорт
	
	Для Каждого ВариантЗаполненияТаблицы2 Из ВариантыЗаполненияТаблицы2 Цикл
		
		Если ВариантЗаполненияТаблицы2.КодПоказателя = "П000020000108"
			ИЛИ ВариантЗаполненияТаблицы2.КодПоказателя = "П000020000109" Тогда
			
			Если СтруктураРеквизитовФормы.ПериодОтчетаВходитВПервыйКвартал
				И НЕ СтруктураРеквизитовФормы.ФлажокОтклАвтоРасчет Тогда
				ВариантЗаполненияТаблицы2.ВариантЗаполнения = "4";
			Иначе
				ВариантЗаполненияТаблицы2.ВариантЗаполнения = "0";
			КонецЕсли;
...
Функция ТекущаяТаблица(Форма, ТолькоРазделыСТаблицами = Ложь) Экспорт
	
	ТекущийРаздел = ТекущийРазделФормы(Форма, ТолькоРазделыСТаблицами);
...
Функция ТекущийРазделФормы(Форма, ТолькоРазделыСТаблицами = Ложь) Экспорт
	
	Результат = Неопределено;
	
	Для каждого Раздел Из Разделы(Форма, ТолькоРазделыСТаблицами) Цикл
