...
Процедура ОткрытьФормуРедактированияСтрокиДокумента(СсылкаНаПозицию, АдресДанныхПозиции, ВладелецФормы) Экспорт
	
	ПараметрыОткрытия = Новый Структура;
	ПараметрыОткрытия.Вставить("Ключ", СсылкаНаПозицию);
	ПараметрыОткрытия.Вставить("АдресДанныхПозицииВХранилище", АдресДанныхПозиции);
	ПараметрыОткрытия.Вставить("СтрокаДокумента", Истина);
	ПараметрыОткрытия.Вставить("КлючНазначенияИспользования", "РедактированиеСтрокиДокумента");
	
	Если ОбщегоНазначенияКлиент.РежимОтладки() Тогда
		ПутьКФорме = "Справочник.ШтатноеРасписание.ФормаОбъекта";
	Иначе
		ПутьКФорме = "ОбщаяФорма.ФормаСтрокиДокументаИзмененияШтатногоРасписания";
	КонецЕсли;
	
	ОткрытьФорму(ПутьКФорме, ПараметрыОткрытия, ВладелецФормы, ВладелецФормы.УникальныйИдентификатор);
...
Процедура ПозицииПередУдалением(Форма, Отказ, РазрешеноУдаление = Ложь) Экспорт
	...
	Если ОбрабатываютсяНовыеПозиции Тогда
		Форма.УдалитьНачисленияИЕжегодныеОтпуска();
	Иначе
		Отказ = Истина;
	КонецЕсли;
...
Процедура ПеречистатьДанныеОбъектаФормы(УправляемаяФорма) Экспорт
	
	УправляемаяФорма.Прочитать();
	
КонецПроцедуры

Функция СохранитьДанныеОбъектаФормы(УправляемаяФорма, ПараметрыЗаписи = Неопределено) Экспорт
	
	Возврат УправляемаяФорма.Записать();
