...
Процедура ЗначенияПоказателей_РасчетПоНалогуНаПрибыль_2015Кв1(ПараметрыОтчета, Контейнер) Экспорт
	...
	Иначе
		...
		ПоляАдреса = "Индекс,КодРегиона,Район,Город,НаселенныйПункт,Улица,Дом,Корпус,Квартира";
		...
		Пока Справки.Следующий() Цикл
			...
			СтруктураАдреса = Новый Структура(ПоляАдреса);
			Если ЗначениеЗаполнено(Справки.Адрес) Тогда
				РегламентированнаяОтчетностьВызовСервера.СформироватьАдрес(Справки.Адрес, СтруктураАдреса);
			КонецЕсли;
			...
			Если ЗначениеЗаполнено(Справки.АдресЗарубежом) Тогда
				СтруктураАдреса = РаботаСАдресами.СведенияОбАдресе(Справки.АдресЗаРубежом);
