...
Функция СведенияОПодписяхПоУмолчаниюДляОбъектаМетаданных(ОбъектМетаданных, Организация) Экспорт 
	...
	ОписаниеПодписей = ОписаниеПодписейОбъектаПоМетаданным(ОбъектМетаданных);
	ВидДокумента = Тип(СтрЗаменить(ОбъектМетаданных.ПолноеИмя(), ".", "Ссылка."));
	Возврат СведенияОПодписяхДокументов(ОписаниеПодписей, Организация, , ВидДокумента);
...
// Возвращает сведения об ответственных лицах организации, их должностях и основаниях подписи.
//
// Параметры:
//   Организация - СправочникСсылка.Организации
//   ИменаРолей  - Строка - Роли получаемых ответственных лиц.
...
//
// Возвращаемое значение:
//   Структура - Для каждой роли возвращается 3 поля:
//       * <ИмяРоли>                 - СправочникСсылка.ФизическиеЛица, Неопределено
//       * <ИмяРоли>Должность        - СправочникСсылка.Должности, Неопределено
//       * <ИмяРоли>ОснованиеПодписи - Строка, Неопределено
//
Функция ОтветственныеЛицаОрганизации(Организация, ИменаРолей, ДатаПолучения = Неопределено) Экспорт
	...
	ТребуемыеРоли = Новый Структура(ИменаРолей);
	ТребуемыеПодписи = Новый Соответствие;
	Для Каждого КлючИЗначение Из ТребуемыеРоли Цикл
		ИмяРоли = КлючИЗначение.Ключ;
		РеквизитыПодписи = ОписаниеРеквизитовПодписанта();
		РеквизитыПодписи.ФизическоеЛицо   = ИмяРоли;
		РеквизитыПодписи.Должность        = ИмяРоли + "Должность";
		РеквизитыПодписи.ОснованиеПодписи = ИмяРоли + "ОснованиеПодписи";
		ТребуемыеПодписи.Вставить(ИмяРоли, РеквизитыПодписи);
		Результат.Вставить(РеквизитыПодписи.ФизическоеЛицо);
		Результат.Вставить(РеквизитыПодписи.Должность);
		Результат.Вставить(РеквизитыПодписи.ОснованиеПодписи);
	КонецЦикла;
	
	Сведения = СведенияОПодписяхДокументов(ТребуемыеПодписи, Организация, ДатаПолучения);
