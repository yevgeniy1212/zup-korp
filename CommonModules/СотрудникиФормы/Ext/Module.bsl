...
//						* ДоступнаРольСохранениеДанныхЗадвоенныхФизическихЛиц - Булево
//
Функция ПодобратьСписокФизЛиц(ПараметрыПоиска) Экспорт
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("ЗарплатаКадрыРасширеннаяПодсистемы.КадровыйУчетРасширенная") Тогда
		МодульКадровыйУчетФормыРасширенный = ОбщегоНазначения.ОбщийМодуль("КадровыйУчетФормыРасширенный");
		МодульКадровыйУчетФормыРасширенный.ПриПодбореСпискаФизЛиц(СтруктураВозврат);
	КонецЕсли;
	
	Если ПараметрыПоиска.ОбязательноЗаполненноеФИО Тогда
		Наименование = ПараметрыПоиска.Фамилия + " " + ПараметрыПоиска.Имя + " " + ПараметрыПоиска.Отчество;
	...
	ТаблицаПохожих = ПохожиеФизическиеЛица(ПараметрыПоиска); 
	МассивНайденных = ОбщегоНазначения.ВыгрузитьКолонку(ТаблицаПохожих, "Ссылка", Истина);
	...
	ДанныеФизическихЛиц = КадровыйУчет.КадровыеДанныеФизическихЛиц(Истина, 
		МассивДоступных, КадровыеДанныеДляФормыПохожихФизическихЛиц(), ТекущаяДатаСеанса());
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("ЗарплатаКадрыКорпоративнаяПодсистемы.ПодборПерсонала") Тогда
		...
		МодульПодборПерсоналаФормы.ДобавитьЗаполнитьКолонкуСовпадающиеДанные(ТаблицаПохожих, ДанныеФизическихЛиц,
...
Процедура ФизическиеЛицаПередЗаписьюНаСервере(Форма, Отказ, ТекущийОбъект, ПараметрыЗаписи) Экспорт
	...
	Если Форма.Параметры.Ключ.Пустая() Тогда
		...
		Если ОбщегоНазначения.ПодсистемаСуществует("ЗарплатаКадрыКорпоративнаяПодсистемы.ПодборПерсонала") Тогда
			МодульПодборПерсоналаФормы = ОбщегоНазначения.ОбщийМодуль("ПодборПерсоналаФормы");
			МодульПодборПерсоналаФормы.ЗаполнитьСвязанныеДанныеФизическогоЛица(Форма, ТекущийОбъект);
		КонецЕсли;
	КонецЕсли;
...
Процедура ОбновитьОтображениеЛичныхДанных(Форма) Экспорт
	...
	СотрудникиКлиентСервер.УстановитьМаскуВводаКодаПодразделения(Форма);
КонецПроцедуры
...
Процедура ЗаписатьФизическоеЛицоСотрудника(Форма) Экспорт
	...
	Если Форма.ФизическоеЛицоЗаблокировано ИЛИ Форма.ФизическоеЛицо.Ссылка.Пустая() Тогда
		...
		Если ОбщегоНазначения.ПодсистемаСуществует("ЗарплатаКадрыКорпоративнаяПодсистемы.ПодборПерсонала") Тогда
			МодульПодборПерсоналаФормы = ОбщегоНазначения.ОбщийМодуль("ПодборПерсоналаФормы");
			МодульПодборПерсоналаФормы.ЗаполнитьСвязанныеДанныеФизическогоЛица(Форма, ФизическоеЛицоОбъект);
		КонецЕсли;
...
// 
// Параметры:
//  ФизическоеЛицоСсылка - СправочникСсылка.ФизическиеЛица - ссылка на физлицо, которое проверяем.
//  Фамилия - Строка -
//  Имя - Строка -
//  Отчество - Строка -
//  Уточнение - Строка - проверяемые ФИО и уточнение.
// 
// Возвращаемое значение:
//  СписокЗначений
Функция ПодобратьСписокУточненийТекущегоФизическогоЛица(ФизическоеЛицоСсылка, Знач Фамилия, Знач Имя, Знач Отчество,
	Знач Уточнение = Неопределено) Экспорт
	...
	ПараметрыПоиска.Ссылка = ФизическоеЛицоСсылка;
	ПараметрыПоиска.Фамилия = Фамилия;
	ПараметрыПоиска.Имя = Имя;
	ПараметрыПоиска.Отчество = Отчество;
	ПараметрыПоиска.УточнениеНаименования = Уточнение;
	ПараметрыПоиска.ТочноеСовпадение = Истина;
	ПараметрыПоиска.ФИОПолностью = Истина;
	
	ПохожиеФизическиеЛица = ПохожиеФизическиеЛица(ПараметрыПоиска);
...
Функция КадровыеДанныеДляФормыПохожихФизическихЛиц() Экспорт
...
// Параметры:
// 	ПараметрыПоиска - см. СотрудникиКлиентСервер.ПараметрыПоискаПохожих
Функция ПохожиеФизическиеЛица(ПараметрыПоиска)
	
	Запрос = Новый Запрос;
	
	ФИО = ПараметрыПоиска.Фамилия + " " + ПараметрыПоиска.Имя + " " + ПараметрыПоиска.Отчество;
	...
	ПодсистемаПодборПерсоналаСуществует = ОбщегоНазначения.ПодсистемаСуществует(
		"ЗарплатаКадрыКорпоративнаяПодсистемы.ПодборПерсонала");
	Если ПодсистемаПодборПерсоналаСуществует Тогда
		МодульПодборПерсоналаФормы = ОбщегоНазначения.ОбщийМодуль("ПодборПерсоналаФормы");
		МодульПодборПерсоналаФормы.СформироватьЗапросПоискаПохожихФизическихЛиц(Запрос, ПараметрыПоиска);
	Иначе
		Запрос.Текст =
			"ВЫБРАТЬ РАЗРЕШЕННЫЕ
	...
	Запрос.УстановитьПараметр("ФизическоеЛицо", ПараметрыПоиска.Ссылка);
	Если ЗначениеЗаполнено(ФИО) Тогда
		
		ТекстПоискаПоФИО = "(ФизическиеЛица.НаименованиеСлужебное ПОДОБНО &Фамилия СПЕЦСИМВОЛ ""~""
		...
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "&ФИО", ТекстПоискаПоФИО);
		
		Если ПараметрыПоиска.ТочноеСовпадение Тогда
		...
		Запрос.УстановитьПараметр("ФамилияИмяОтчествоУточнение", Фамилия + ?(ПустаяСтрока(Имя), "", " " + Имя)
			...
			+ ПараметрыПоиска.УточнениеНаименования));
		
		Если ПараметрыПоиска.ФИОПолностью Тогда
			Запрос.УстановитьПараметр("Фамилия", Фамилия + ?(ПустаяСтрока(Имя), "", " " + Имя)
				+ ?(ПустаяСтрока(Отчество), "", " " + Отчество));
			Запрос.УстановитьПараметр("ФамилияИмя", Фамилия + ?(ПустаяСтрока(Имя), "", " " + Имя)
				+ ?(ПустаяСтрока(Отчество), "", " " + Отчество));
		КонецЕсли;
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("ЗарплатаКадрыКорпоративнаяПодсистемы.АрхивФизическихЛиц") Тогда
		МодульАрхивФизическихЛиц = ОбщегоНазначения.ОбщийМодуль("АрхивФизическихЛиц");
		МодульАрхивФизическихЛиц.ДополнитьТаблицуПохожихФизическихЛиц(ТаблицаПохожих, ЧастиИмени,
			ПараметрыПоиска.ТочноеСовпадение, ПараметрыПоиска.Ссылка);
	КонецЕсли;
	ОбщегоНазначенияБЗК.ДобавитьИндексКоллекции(ТаблицаПохожих, "Ссылка");
...
Функция ТекстСообщенияОКонфликтеВидаЗанятости(ВидЗанятости, Организация, КонфликтныйСотрудникОрганизация = Неопределено, КонфликтныйСотрудникВидЗанятости = Неопределено, КонфликтныйСотрудникТабельныйНомер = "", КонфликтныйСотрудникДолжность = Неопределено) Экспорт
	...
	КонецЕсли;
	Если Организация = КонфликтныйСотрудникОрганизация
		Или ВидЗанятости = Перечисления.ВидыЗанятости.ВнутреннееСовместительство
			И Не ЗначениеЗаполнено(КонфликтныйСотрудникОрганизация) Тогда
		ПредставлениеОрганизации = НСтр("ru = 'нашей организации'");
	ИначеЕсли ЗначениеЗаполнено(КонфликтныйСотрудникОрганизация) Тогда
		ПредставлениеОрганизации = Строка(КонфликтныйСотрудникОрганизация);
	Иначе
		ПредставлениеОрганизации = НСтр("ru = 'другой организации'");
	КонецЕсли;
	Если КонфликтныйСотрудникВидЗанятости = Перечисления.ВидыЗанятости.ОсновноеМестоРаботы Тогда
		ПредставлениеВидаЗанятости = НСтр("ru = 'по основному месту работы'");
	Иначе
		ПредставлениеВидаЗанятости = НСтр("ru = 'как внешний совместитель'");
	КонецЕсли;
...
Процедура УстановитьПараметрыИнфоНадписиФормыФизЛица(Форма)
	...
	Если Форма.ИмеютсяАктуальныеСотрудники  Тогда
		...
		ИнфоНадписьМестаРаботы = СтрШаблон(
...
Процедура ЗаполнитьФормуМестамиРаботы(Форма, ИмяГруппы, ИмяПоследнегоЭлементаГруппы = "", СотрудникИсключение = Неопределено, ВыводитьПодробнуюИнформацию = Ложь) Экспорт
	...
	ГруппаСотрудников = Форма.Элементы.Найти(ИмяГруппы);
	Если ГруппаСотрудников <> Неопределено Тогда
		Пока ГруппаСотрудников.ПодчиненныеЭлементы.Количество() > 0 Цикл
			Форма.Элементы.Удалить(ГруппаСотрудников.ПодчиненныеЭлементы.Получить(0));
		КонецЦикла;
	КонецЕсли;
...
Процедура ДобавитьЭлементыФормыДляМестаРаботыКратко(Форма, ДанныеСотрудника, РеквизитыОрганизации, ИмяКоманды, ИмяГруппы)
	...
	Если Форма.Элементы.Найти(ИмяКоманды + "Группа") = Неопределено Тогда
		ГруппаФормы = Форма.Элементы.Вставить(ИмяКоманды + "Группа", Тип("ГруппаФормы"), Форма.Элементы[ИмяГруппы]);
		ГруппаФормы.Вид = ВидГруппыФормы.ОбычнаяГруппа;
		ГруппаФормы.Группировка = ГруппировкаПодчиненныхЭлементовФормы.Вертикальная;
		ГруппаФормы.ОтображатьЗаголовок = Ложь;
		ГруппаФормы.Отображение = ОтображениеОбычнойГруппы.Нет;
		...
		Кнопка = Форма.Элементы.Вставить(ИмяКоманды, Тип("КнопкаФормы"), ГруппаФормы);
...
Процедура ПрочитатьДанныеИзХранилищаВФорму(Форма, ОписаниеДополнительнойФормы, АдресВХранилище) Экспорт
	...
	Для каждого ДанныеФормы Из РедактируемыеДанные.ДополнительныеДанные Цикл
		...
		Если Не ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(Форма, ДанныеФормы.Ключ) Тогда
			Продолжить;
		КонецЕсли;
...
// Возвращаемое значение:
//  ТаблицаЗначений - Таблица истории взаимоотношений:
//   * ФизическоеЛицо - СправочникСсылка.ФизическиеЛица -
...
Функция ТаблицаИсторииВзаимоотношений()
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("ЗарплатаКадрыКорпоративнаяПодсистемы.ПодборПерсонала") Тогда
		МодульПодборПерсоналаФормы = ОбщегоНазначения.ОбщийМодуль("ПодборПерсоналаФормы");
		МодульПодборПерсоналаФормы.ДополнитьКолонкиИсторииВзаимоотношений(Таблица);
	КонецЕсли;
