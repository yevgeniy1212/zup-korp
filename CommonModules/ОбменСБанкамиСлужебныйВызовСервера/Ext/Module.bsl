...
Функция ЗапускЗаданияОтправкиДокументовАсинхронныйОбмен(
	...
	ИдентификаторСессии = СтрокаИзДвоичныхДанных(ИдентификаторСессии);
...
//
// Параметры:
//  ПрисоединенныйФайл - СправочникСсылка.ПисьмоОбменСБанкамиПрисоединенныеФайлы - ссылка на присоединенный файл.
//
// Возвращаемое значение:
...
Функция ТекстСостоянияЭД(Знач СсылкаНаВладельца, Гиперссылка = Неопределено) Экспорт
	...
	Если ЗначениеЗаполнено(СсылкаНаВладельца) Тогда
		Если ОбщегоНазначенияБЭДПовтИсп.ЗначениеФункциональнойОпции("ИспользоватьОбменСБанками") Тогда
...
Функция ЗапускЗаданияПоОбработкеЭлектронныхДокументов(Параметры) Экспорт
	...
	Если Не Параметры.Свойство("МассивОтпечатковСертификатов")
		И (ОбменСБанкамиКлиентСервер.ЕстьДействие(Параметры.Действия, "Подписать")
			ИЛИ ОбменСБанкамиКлиентСервер.ЕстьДействие(Параметры.Действия, "Отправить"))
...
Процедура ДобавитьПодписиВСообщениеОбмена(СообщениеОбмена, ДанныеПодписей) Экспорт
	...
	Для Каждого СертификатИПодпись Из ДанныеПодписей Цикл
		Для Каждого КлючЗначение Из СертификатИПодпись Цикл
			...
			СвойстваПодписи.Вставить("СрокДействияПоследнейМеткиВремени", Неопределено);
		...
		Для Каждого СообщениеОбмена Из МассивСообщенийОбменаКОбновлениюСтатуса Цикл
			...
			Попытка
				...
				Если ОбменСБанкамиСлужебный.ДокументПодписываетсяПоМаршруту(СообщениеОбмена) Тогда
					ВесМаршрута = 0;
					МаршрутыПодписанияБЭД.ОбновитьМаршрутПриПодписании(СообщениеОбмена, Сертификат, ВесМаршрута);
...
Процедура ДобавитьПодпись(Знач СообщениеОбмена, Знач Подпись, Знач Сертификат, Знач ДанныеПроверкиПодписи = Неопределено) Экспорт
	...
	ДанныеПодписи = Новый Структура("Подпись, УстановившийПодпись, Комментарий, ИмяФайлаПодписи, ДатаПодписи, Отпечаток,
		|Сертификат, КомуВыданСертификат, ТипПодписи, СрокДействияПоследнейМеткиВремени");
	...
	Попытка
		...
		Если ОбщегоНазначения.ЗначениеРеквизитаОбъекта(СообщениеОбмена, "Направление") = Перечисления.НаправленияЭДО.Исходящий Тогда
			СтруктураПараметров = Новый Структура;
			Если ОбменСБанкамиСлужебный.ДокументПодписываетсяПоМаршруту(СообщениеОбмена) Тогда
				ВесМаршрута = 0;
				МаршрутыПодписанияБЭД.ОбновитьМаршрутПриПодписании(СообщениеОбмена, СертификатПодписи, ВесМаршрута);
...
Функция ЭлектронныйДокументПолностьюПодписан(СообщениеОбмена) Экспорт
	...
	Если ТребуетсяПодпись Тогда
		...
		МаршрутПодписанияЗавершен = МаршрутыПодписанияБЭД.МаршрутПодписанияЗавершен(СообщениеОбмена);
		ПрисоединенныйФайл = ОбменСБанкамиСлужебный.ПрисоединенныйФайл(СообщениеОбмена);
...
//          * ПодписьВерна - Булево - признак валидности подписи.
//
Процедура СохранитьРезультатыПроверкиПодписейПрисоединенногоФайла(Знач ПрисоединенныйФайл, Знач МассивРезультатов) Экспорт
...
//  СтатусПакета - ПеречислениеСсылка.СтатусыТранспортныхСообщенийБЭД - новый статус пакета.
//
...
//    НовыйСтатусПакетаЭД - ПеречислениеСсылка.СтатусыТранспортныхСообщенийБЭД - назначаемый статус пакета;
...
Процедура ОбновитьСтатусыДокументовПакетаЭДО(Знач ПакетОбменСБанками, Знач НовыйСтатусПакетаЭД, Знач СтруктураПараметров, СообщенияОбмена = Неопределено) Экспорт
	...
	Попытка
		...
		|	И ИСТИНА";
		Если НовыйСтатусПакетаЭД = Перечисления.СтатусыТранспортныхСообщенийБЭД.Доставлен Тогда
		...
		Пока Выборка.Следующий() Цикл
			Если НовыйСтатусПакетаЭД = Перечисления.СтатусыТранспортныхСообщенийБЭД.Отправлен Тогда
...
Функция ПодготовленныеКОтправкеПакеты(Знач НастройкаОбмена) Экспорт
	...
	|	И ПакетЭД.Статус = ЗНАЧЕНИЕ(Перечисление.СтатусыТранспортныхСообщенийБЭД.ПодготовленКОтправке)
...
Функция СоответствиеДоступныхСертификатовИПараметров(Знач МассивОтпечатковСертификатов, Знач НастройкаОбмена = Неопределено) Экспорт
	...
	УстановитьПривилегированныйРежим(Истина);	
	Сертификаты = КриптографияБЭД.НайтиСертификаты(МассивОтпечатковСертификатов);
	СоответствиеСертификатовИПаролей = КриптографияБЭД.ПаролиСертификатов(Сертификаты);
	УстановитьПривилегированныйРежим(Ложь);
...
Функция ДанныеСертификатов(Сертификаты) Экспорт
	...
	|	&ЗначениеПоляПользовательОповещенОСрокеДействия КАК ПользовательОповещенОСрокеДействия
	|ИЗ
	|	Справочник.СертификатыКлючейЭлектроннойПодписиИШифрования КАК Сертификаты
	|ГДЕ
	|	Сертификаты.Ссылка В (&Сертификаты)";
	Запрос.УстановитьПараметр("Сертификаты", Сертификаты);
	МетаданныеОбъекта = Метаданные.Справочники.СертификатыКлючейЭлектроннойПодписиИШифрования;
	Если ОбщегоНазначения.ЕстьРеквизитОбъекта("ПользовательОповещенОСрокеДействия", МетаданныеОбъекта) Тогда
		ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "&ЗначениеПоляПользовательОповещенОСрокеДействия", "Сертификаты.ПользовательОповещенОСрокеДействия");
	Иначе
		ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "&ЗначениеПоляПользовательОповещенОСрокеДействия", "ЛОЖЬ");
	КонецЕсли;
...
Функция ПаролиСертификатов(Сертификаты) Экспорт
	...
	УстановитьПривилегированныйРежим(Истина);
	ПаролиСертификатов = КриптографияБЭДВызовСервера.ПаролиСертификатов(Сертификаты);
	УстановитьПривилегированныйРежим(Ложь);
...
Функция ПоставляемыеНастройкиОбмена(Банк, ВключитьФО = Ложь, ЕстьПравоСозданияНастройкиЭДО = Ложь) Экспорт
	...
	Если ВключитьФО Тогда
		ИспользуетсяОбменСБанком = ОбщегоНазначенияБЭДПовтИсп.ЗначениеФункциональнойОпции(
...
Функция ТекущееСостояниеПисемСБанками() Экспорт
	...
	|	ГДЕ
	|		ПисьмоОбменСБанками.Направление = ЗНАЧЕНИЕ(Перечисление.НаправленияЭДО.Входящий)
...
Функция ТипыПисемСбербанк(Знач ОтветБанка, Знач УникальныйИдентификатор, Знач НастройкаОбмена) Экспорт
	...
	Если postFix = Неопределено Тогда
		...
		РаботаСФайламиБЭД.СохранитьXDTO(ЭД, ИмяФайла, Ложь, "UTF-8");
		ОбработатьОшибку(ВидОперации, ТекстОшибки, ТекстСообщения);
		Возврат Неопределено;
	КонецЕсли;
	...
	ПапкаДляРаспаковки = РаботаСФайламиБЭД.ВременныйКаталог();
	Попытка
		ЧтениеZIP = Новый ЧтениеZIPФайла(ЧтениеДанныхАрхив.ИсходныйПоток());
		ЧтениеZIP.ИзвлечьВсе(ПапкаДляРаспаковки);
	Исключение
		ПодробноеПредставлениеОшибки = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
		Если НЕ РаботаСФайламиБЭД.ВозможноИзвлечьФайлы(ЧтениеZIP, ПапкаДляРаспаковки) Тогда
...
Функция ПакетXMLСбербанка(Знач СообщениеОбмена, Знач НастройкаОбмена, Знач ФродПараметры = Неопределено, Знач Логин = Неопределено) Экспорт
	...
	Исключение
		...
		ОбщегоНазначения.СообщитьПользователю(ТекстОшибки + Символы.ПС + ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
		Возврат Неопределено;
	КонецПопытки;
...
Функция СоздатьСертификатСбербанка(Знач ДвоичныеДанныеСертификата, Знач Организация) Экспорт
	...
	НовыйСертификат = КриптографияБЭД.НайтиСоздатьСертификатЭП(
...
Процедура СформироватьЗапросЗонд(
	...
	Попытка
		...
		// ЗУПК
		ЗаполнитьСвойствоXDTO(Отправитель, "inn", РеквизитыОрганизации.ИННЮЛ, Истина, ТекстОшибки);
		ЗаполнитьСвойствоXDTO(Отправитель, "kbe", РеквизитыОрганизации.КБЕ, , ТекстОшибки);
        // ЗУПК		
		...
		// ЗУПК
		ЗаполнитьСвойствоXDTO(Получатель, "bic", СокрЛП(РеквизитыБанка.Код), Истина, ТекстОшибки);
		// ЗУПК
	...
	СтруктураРеквизитов.Вставить("НастройкаОбмена", НастройкаОбмена);
	СтруктураРеквизитов.Вставить("Направление", Перечисления.НаправленияЭДО.Исходящий);
...
Процедура ОбработатьОтветБанкаПослеОтправкиДокументовЧерезВнешнююОбработку(
	...
	Для Каждого Элемент Из ПакетыЭДО Цикл
		...
		ОбновитьСтатусыДокументовПакетаЭДО(Элемент.Ключ, Перечисления.СтатусыТранспортныхСообщенийБЭД.Отправлен, Параметры);
	КонецЦикла;
...
Функция СоздатьВходящееПисьмоСбербанк(НастройкаОбмена, ЭД, ДляВалютногоКонтроля)
	...
	ПисьмоОбъект.Направление = Перечисления.НаправленияЭДО.Входящий;
	...
	Если ЗначениеЗаполнено(ТипПисьмаСтрокой) Тогда
		...
		Запрос.Текст = "ВЫБРАТЬ ПЕРВЫЕ 1
		               ...
		               |	И ТипыПисемОбменСБанками.Направление = ЗНАЧЕНИЕ(Перечисление.НаправленияЭДО.Входящий)
		...
		Иначе
			...
			НовыйТипПисьма.Направление = Перечисления.НаправленияЭДО.Входящий;
...
Функция ПоследняяВходящаяДатаСбербанк(НастройкаОбмена)
	...
	Если Выборка.Следующий() Тогда
		Возврат Выборка.ПоследняяДатаПолученияЭД;
	КонецЕсли;
...
// Сохраняет настройку пользователя в ХранилищеОбщихНастроек для настроек по 1С:ДиректБанк
//
// Параметры:
//  ИдентификаторКлиента  - УникальныйИдентификатор - Идентификатор полученный из функции СистемнаяИнформация.ИдентификаторКлиента
//  КлючНастроек  - Строка - раздел настроек (блок информации), информацию по которому необходимо сохранить в настройках
//  ИмяПараметра - Строка - имя параметра, информацию по которому необходимо сохранить в настройках в разделе КлючНастроек
//  ЗначениеПараметра  - Любое значение - значение параметра, информацию по которому необходимо сохранить в настройках в разделе КлючНастроек
//
Процедура СохранитьНастройкуПользователяДиректБанк(ИдентификаторКлиента, КлючНастроек, ИмяПараметра, ЗначениеПараметра) Экспорт
	
	КлючОбъекта = "НастройкиДиректБанка";
	Если КлючНастроек = "VPNKeyTLS" Тогда
		
		НастройкиДиректБанкаVPNKeyTLSСохраненные = ОбщегоНазначения.ХранилищеОбщихНастроекЗагрузить(КлючОбъекта, КлючНастроек);
		НастройкиДиректБанкаVPNKeyTLSТекущие = НовыеНастройкиДиректБанкаVPNKeyTLS();
		Если ТипЗнч(НастройкиДиректБанкаVPNKeyTLSСохраненные) = Тип("Структура") Тогда
			ЗаполнитьЗначенияСвойств(НастройкиДиректБанкаVPNKeyTLSТекущие, НастройкиДиректБанкаVPNKeyTLSСохраненные);
		КонецЕсли;
		
		Если ИмяПараметра = "ИспользоватьАвтоматическоеОпределениеWindows7" Тогда
			НастройкиДиректБанкаVPNKeyTLSТекущие.ИспользоватьАвтоматическоеОпределениеWindows7 = ЗначениеПараметра;
		ИначеЕсли ИмяПараметра = "ВерсииWindowsКлиентскихПриложений" Тогда
			НастройкиДиректБанкаVPNKeyTLSТекущие.ВерсииWindowsКлиентскихПриложений.Вставить(ИдентификаторКлиента, ЗначениеПараметра);
		ИначеЕсли ИмяПараметра = "ОчиститьВерсииWindowsКлиентскихПриложений" Тогда
			НастройкиДиректБанкаVPNKeyTLSТекущие.ВерсииWindowsКлиентскихПриложений.Очистить();
		КонецЕсли;
		
		ОбщегоНазначения.ХранилищеОбщихНастроекСохранить(КлючОбъекта, КлючНастроек, НастройкиДиректБанкаVPNKeyTLSТекущие);
	КонецЕсли;
	
КонецПроцедуры

// Возвращает настройку пользователя из ХранилищеОбщихНастроек для настроек по 1С:ДиректБанк
//
// Параметры:
//  ИдентификаторКлиента  - УникальныйИдентификатор - Идентификатор полученный из функции СистемнаяИнформация.ИдентификаторКлиента
//  КлючНастроек  - Строка - раздел настроек (блок информации), информация по которому содержится в настройках
//  ИмяПараметра - Строка - имя параметра, информация по которому содержится в настройках в разделе КлючНастроек
//
// Возвращаемое значение:
//  ЗначениеПараметра  - Любое значение - значение параметра, информация по которому содержится в настройках в разделе КлючНастроек,
//   если такие КлючНастроек или ИмяПараметра не найдены, то возвращается Неопределено
//
Функция ПолучитьНастройкуПользователяДиректБанк(ИдентификаторКлиента, КлючНастроек, ИмяПараметра) Экспорт
	...
	Если КлючНастроек = "VPNKeyTLS" Тогда
		...
		Если ИмяПараметра = "ИспользоватьАвтоматическоеОпределениеWindows7" Тогда
			ЗначениеПараметра = НастройкиДиректБанкаVPNKeyTLSТекущие.ИспользоватьАвтоматическоеОпределениеWindows7;
		ИначеЕсли ИмяПараметра = "ВерсииWindowsКлиентскихПриложений" Тогда
			ЗначениеПараметра = НастройкиДиректБанкаVPNKeyTLSТекущие.ВерсииWindowsКлиентскихПриложений.Получить(ИдентификаторКлиента);
		КонецЕсли;
...
Функция НовыеНастройкиДиректБанкаVPNKeyTLS()
	
	СтруктураНастройкиДиректБанкаVPNKeyTLS = Новый Структура;
	СтруктураНастройкиДиректБанкаVPNKeyTLS.Вставить("ИспользоватьАвтоматическоеОпределениеWindows7", Истина);
	ВерсииWindowsКлиентскихПриложений = Новый Соответствие();
	СтруктураНастройкиДиректБанкаVPNKeyTLS.Вставить("ВерсииWindowsКлиентскихПриложений", ВерсииWindowsКлиентскихПриложений);
	Возврат СтруктураНастройкиДиректБанкаVPNKeyTLS;
...
Функция ЗапускЗаданияОтправкиЗапросЗондаНаСервере(Знач ПараметрыОтправки, Знач УникальныйИдентификатор) Экспорт
	...
	ПараметрыОтправки.ИдентификаторСессии = СтрокаИзДвоичныхДанных(
...
Функция ЗапускЗаданияПолученияИзвещенияОСостоянииЭДНаСервере(
	...
	ПараметрыПроцедуры.ИдентификаторСессии = СтрокаИзДвоичныхДанных(
