...
Процедура ПриНажатииНаКнопкуОтправкиВКонтролирующийОрган(
	...
	ИначеЕсли КонтролирующийОрган = "РЭЦ" Тогда
		...
		ОписаниеОповещения = Новый ОписаниеОповещения("ПриНажатииНаКнопкуОтправкиВКонтролирующийОрганРЭЦЗавершение", ЭтотОбъект, ДополнительныеПараметры);
		ДокументооборотСКОКлиент.ПолучитьКонтекстЭДО(ОписаниеОповещения);
	...
	ИначеЕсли КонтролирующийОрган = "РегистрацияЮЛФНС" Тогда
		Если ОбщегоНазначенияКлиент.ПодсистемаСуществует("РегламентированнаяОтчетность.ЭлектронныйДокументооборотСКонтролирующимиОрганами.ГосударственнаяРегистрацияВФНС") Тогда
			...
			МодульДокументооборотРегистрацияВФНССлужебныйКлиент.ПриНажатииНаКнопкуОтправкиВКонтролирующийОрганРегистрацияЮЛ(Истина, ДополнительныеПараметры);
		КонецЕсли;
		
	ИначеЕсли КонтролирующийОрган = "РР" ИЛИ ТипЗнч(СсылкаНаОтчет) = Тип("СправочникСсылка.МашиночитаемыеДоверенностиРаспределенныйРеестр") Тогда
		
		ДокументооборотСКОКлиент.ЗарегистрироватьМЧДРР(Неопределено, СсылкаНаОтчет);
...
Процедура ПоказатьФормуПредложениеОформитьЗаявлениеНаПодключение(Организация = Неопределено, ПоказыватьКнопки = Неопределено) Экспорт
	...
	ЭтоИнтеграцияСБанком = ОбработкаЗаявленийАбонентаВызовСервера.ЭтоИнтеграцияСБанком();
	Если ЭтоИнтеграцияСБанком Тогда
		ОткрытьФорму("Обработка.ДокументооборотСКонтролирующимиОрганами.Форма.ПредложениеПодключитьсяПСБ", ПарметрыОткрытия,,,,,ОписаниеОповещения,РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
	Иначе
...
Процедура ОбновитьОтправкуИзПанелиОтправкиЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	ИначеЕсли КонтролирующийОрган = "УдалитьБанкРоссии" Тогда
		Если ДокументооборотСКОКлиентСервер.ПодсистемаУдалитьЦБСуществует() Тогда
			...
			Ссылка 		= МодульУдалитьДокументооборотСБанкомРоссииКлиентСервер.СсылкаНаОтчетПоФорме(Форма);
			Организация	= МодульУдалитьДокументооборотСБанкомРоссииКлиентСервер.ПолучитьОрганизациюПоФорме(Форма);	
		КонецЕсли;
	ИначеЕсли КонтролирующийОрган = "ЦБ" И ДокументооборотСКОКлиентСервер.ПодсистемаЦБСуществует() Тогда
		МодульДокументооборотСБанкомРоссииКлиентСервер =
			ОбщегоНазначенияКлиент.ОбщийМодуль("ДокументооборотСБанкомРоссииКлиентСервер");
	...
	ИначеЕсли КонтролирующийОрган = "РегистрацияЮЛФНС" Тогда
		Если ОбщегоНазначенияКлиент.ПодсистемаСуществует("РегламентированнаяОтчетность.ЭлектронныйДокументооборотСКонтролирующимиОрганами.ГосударственнаяРегистрацияВФНС") Тогда
			...
			Ссылка 		= МодульДокументооборотРегистрацияВФНСКлиентСервер.ПолучитьСсылкуДанных(Форма);
			Организация	= МодульДокументооборотРегистрацияВФНСКлиентСервер.ПолучитьОрганизациюПоФорме(Форма);
		Иначе
			Ссылка 		= Неопределено;
			Организация = Неопределено;
		КонецЕсли;
	...
	ПараметрыДлительногоОбмена.Вставить("НаименованиеКонтролирующегоОргана", КонтролирующийОрган);
...
Процедура ОткрытьПротоколПоСсылкеИзПанелиОтправки(Форма, КонтролирующийОрган = "ФНС", НавигационнаяСсылка, СтандартнаяОбработка = Ложь) Экспорт
	...
	Если ЗначениеЗаполнено(Ссылка) Тогда
		...
		ОписаниеОповещения = Новый ОписаниеОповещения("ОткрытьПротоколПоСсылкеИзПанелиОтправкиЗавершение", ЭтотОбъект, ДополнительныеПараметры);
...
Процедура ОткрытьПротоколПоСсылкеИзПанелиОтправкиЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	Форма = ДополнительныеПараметры.Форма;
	КонтролирующийОрган = ДополнительныеПараметры.КонтролирующийОрган;
	НавигационнаяСсылка = ДополнительныеПараметры.НавигационнаяСсылка;
	Ссылка = ДополнительныеПараметры.Ссылка;
	...
	КонтекстЭДОКлиент.ОткрытьПротоколПоССылке(Ссылка, КонтролирующийОрган, НавигационнаяСсылка);
...
Процедура ОткрытьСостояниеОтправкиИзПанелиОтправки(Форма, КонтролирующийОрган = "ФНС") Экспорт
	...
	ИначеЕсли КонтролирующийОрган = "РЭЦ" Тогда
		...
		Ссылка = МодульДокументооборотСРЭЦКлиентСервер.СсылкаНаОтчетПоФорме(Форма);
	ИначеЕсли КонтролирующийОрган = "РегистрацияЮЛФНС" Тогда
		Если ОбщегоНазначенияКлиент.ПодсистемаСуществует("РегламентированнаяОтчетность.ЭлектронныйДокументооборотСКонтролирующимиОрганами.ГосударственнаяРегистрацияВФНС") Тогда
			МодульДокументооборотРегистрацияВФНСКлиентСервер = ОбщегоНазначенияКлиент.ОбщийМодуль("ДокументооборотРегистрацияВФНСКлиентСервер");
			Ссылка = МодульДокументооборотРегистрацияВФНСКлиентСервер.ПолучитьСсылкуДанных(Форма);
...
Процедура ПриНажатииНаКнопкуОтправкиВКонтролирующийОрганРЭЦЗавершение(Результат, ДополнительныеПараметры) Экспорт
	
	КонтекстЭДОКлиент = Результат.КонтекстЭДО;
	КонтекстЭДОКлиент.ПриНажатииНаКнопкуОтправкиВКонтролирующийОрганДляРЭЦ(Результат, ДополнительныеПараметры);
...
Процедура ПолучитьВходящиеСообщенияСЭДОФССПослеПолученияКонтекста(Результат, ДополнительныеПараметры) Экспорт
	...
	ПоказыватьДлительнуюОперацию 	= ДополнительныеПараметры.ПоказыватьДлительнуюОперацию;
	ТипВзаимодействия 				= ДополнительныеПараметры.ТипВзаимодействия;
...
Процедура ПолучитьСообщенияСЭДОФССПослеПолученияКонтекста(Результат, ДополнительныеПараметры) Экспорт
	...
	ОткрыватьФормуДлительнойОперации 	= ДополнительныеПараметры.ОткрыватьФормуДлительнойОперации;
	ЗадаватьВопросОТестовомСервере 		= ДополнительныеПараметры.ЗадаватьВопросОТестовомСервере;
	ТипыВзаимодействий 					= ДополнительныеПараметры.ТипыВзаимодействий;
	...
	КонтекстЭДОКлиент.ПолучитьСообщенияСЭДОФСС(
		ОповещениеОбратногоВызова,
		Организация,
		Идентификаторы,
		ОткрыватьФормуДлительнойОперации,
		ЗадаватьВопросОТестовомСервере,
		ТипыВзаимодействий);
...
Процедура ОтправитьСообщениеСЭДОФССПослеПолученияКонтекста(Результат, ДополнительныеПараметры) Экспорт
	...
	Если КонтекстЭДОКлиент = Неопределено Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстОшибкиИнициализацииКонтекстаЭДО);
		ВыполнитьОбработкуОповещения(ПараметрыСообщения.ОповещениеОбратногоВызова, Неопределено);
		Возврат;
	КонецЕсли;
	...
	ЗаполнитьЗначенияСвойств(ПараметрыОтправкаСообщения, ПараметрыСообщения);
	Если ЭтоАдресВременногоХранилища(ПараметрыСообщения.СодержимоеИлиАдресСообщения) Тогда
		ДанныеСообщения = ПолучитьИзВременногоХранилища(ПараметрыСообщения.СодержимоеИлиАдресСообщения);
		Если ТипЗнч(ДанныеСообщения) = Тип("ДвоичныеДанные") Тогда
			ПараметрыОтправкаСообщения.СодержимоеСообщения = ПолучитьСтрокуИзДвоичныхДанных(ДанныеСообщения);
		Иначе
			ПараметрыОтправкаСообщения.СодержимоеСообщения = ДанныеСообщения;
		КонецЕсли;
	Иначе
		ПараметрыОтправкаСообщения.СодержимоеСообщения = ПараметрыСообщения.СодержимоеИлиАдресСообщения;
	КонецЕсли;
	ПараметрыОтправкаСообщения.ОповещениеОбратногоВызова = ОповещениеОбратногоВызова;
...
Процедура ПерейтиНаПортал() Экспорт
	...
	ИнтернетПоддержкаПользователейКлиент.ОткрытьСтраницуИнтегрированногоСайта(URLСтраницыПорталаПоддержки);
...
Функция ТребованияКИзображениямВМакетахПенсионныхДел(ДопустимыеТипыФайлов, МаксимальныйРазмерФайла, ИгнорироватьОшибки = Ложь) Экспорт
	...
	ПараметрыМетода.Вставить("Пояснение", 					НСтр("ru = 'Письма, содержащие изображения, не соответствующие указанным требованиям, могут быть не приняты СФР (бывш. ПФР).'"));
	ПараметрыМетода.Вставить("ИгнорироватьОшибки",          ИгнорироватьОшибки);
...
Функция РасширениеПоУмолчаниюИзДопустимыхТипов(ДопустимыеТипыФайлов)
	...
	Иначе
		РасширениеПоУмолчанию      = ОперацииСФайламиЭДКОКлиентСервер.РасширениеПоУмолчанию();
	КонецЕсли;
...
Функция ПолучитьДанныеПрисоединенногоФайла(ОбработкаОповещения, ПрисоединенныйФайл) Экспорт
	
	Результат = ЭлектронныйДокументооборотСКонтролирующимиОрганамиВызовСервера.КонтекстПолученияДанныхФайлаИзвещенияФСС(
		ПрисоединенныйФайл);
