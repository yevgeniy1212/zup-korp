...
Процедура РасчетЗарплатыРасчетныеДокументыУдержанияПередЗаписью(Источник, Отказ, РежимЗаписи, РежимПроведения) Экспорт
	...
	Если МетаданныеДокумента.ТабличныеЧасти.Найти("НДФЛ") <> Неопределено Тогда
		СуммаУдержано = СуммаУдержано + Источник.НДФЛ.Итог("Налог") + Источник.НДФЛ.Итог("НалогСПревышения");
	КонецЕсли; 
...
Процедура ЗапомнитьИзмененияСтажейФизическихЛицПередЗаписью(Источник, Отказ, Замещение) Экспорт
	...
	Запрос.Текст =
		...
		|ГДЕ
		|	(ЕСТЬNULL(ЗаписываемыйНабор.ДатаОтсчета, 0) <> ЕСТЬNULL(ТекущийНабор.ДатаОтсчета, 0)
		|			ИЛИ ЕСТЬNULL(ЗаписываемыйНабор.РазмерМесяцев, 0) <> ЕСТЬNULL(ТекущийНабор.РазмерМесяцев, 0)
		|			ИЛИ ЕСТЬNULL(ЗаписываемыйНабор.РазмерДней, 0) <> ЕСТЬNULL(ТекущийНабор.РазмерДней, 0)
		|			ИЛИ ЕСТЬNULL(ЗаписываемыйНабор.Прерван, НЕОПРЕДЕЛЕНО) <> ЕСТЬNULL(ТекущийНабор.Прерван, ЛОЖЬ))
		|
		|СГРУППИРОВАТЬ ПО
