...
Функция ИнициализироватьСоединениеССерверомСдачиОтчетности(ОписаниеОшибки)
	...
	Попытка
		Возврат Новый HTTPСоединение(АдресСервера,,,,, 30);
	Исключение
...
Функция АктуализироватьСостояниеОтправкиОтчетности(СтрНезавершеннаяОтправка, Соединение, ОписаниеОшибки = "")
	...
	Попытка
		Соединение.ОтправитьДляОбработки(ФайлInfo, АдресРесурсаНаСервереОнлайнЦентраПоСдачеОтчетности());
...
Функция ОтправитьГотовыйКОтправкеПакет(ОтчетностьКОтправке, Соединение, ОписаниеОшибки = "")
	...
	ФайлРезультата = ПолучитьИмяВременногоФайла();
	Попытка
		Соединение.ОтправитьДляОбработки(ПолноеИмяФайлаПакета, АдресРесурсаНаСервереОнлайнЦентраПоСдачеОтчетности());
...
Функция ПроверитьПередРегистрациейЗаявкиНаОтправкуОтчетности(Отчет, КонтролирующийОрган = "ФНС", ТекстВопроса = "") Экспорт
	...
	Если НЕ НаправлениеСдачиОтчетностиОткрыто(Организация, ТипНаправления, КодНаправления) Тогда
		ОбщегоНазначения.СообщитьПользователю(НСтр("ru = 'Направление сдачи отчетности по организации закрыто. Отправка невозможна.'"));
		Возврат Ложь;
	КонецЕсли;
...
Функция ЗарегистрироватьОтправкуОтчетностиВСправочнике(ДанныеПакета, ИмяФайлаПакета, Отчет) Экспорт
	...
	Если ТипНаправления = Перечисления.ТипыНаправленийСдачиОтчетности.ФНС Тогда
		...
		ИЛИ НЕ СтроковыеФункцииКлиентСервер.ТолькоЦифрыВСтроке(КодНаправления) Тогда
			ОбщегоНазначения.СообщитьПользователю(НСтр("ru='Код инспекции-получателя отчета должен состоять строго из четырех цифр.'"));
			Возврат Ложь;
		КонецЕсли;
	ИначеЕсли ТипНаправления = Перечисления.ТипыНаправленийСдачиОтчетности.ФСГС Тогда
		...
		ИЛИ Сред(КодНаправления, 3, 1) <> "-" Тогда
			ОбщегоНазначения.СообщитьПользователю(НСтр("ru = 'Код органа ФСГС, в который представляется отчетность организации (задается в справочнике ""Организации""), должен удовлетворять шаблону XX-XXZZZZZ, где X - любая цифра.'"));
			Возврат Ложь;
		КонецЕсли;
	Иначе
		...
		ИЛИ Сред(КодНаправления, 4, 1) <> "-" Тогда
			ОбщегоНазначения.СообщитьПользователю(НСтр("ru = 'Код органа ПФР, в который представляется отчетность организации (задается в справочнике ""Организации""), должен удовлетворять шаблону XXX-XXX, где X - любая цифра.'"));
			Возврат Ложь;
		КонецЕсли;
	КонецЕсли;
	...
	Если МетаданныеОтчета = Неопределено Тогда
		ОбщегоНазначения.СообщитьПользователю(НСтр("ru = 'Неизвестный вид отчета.'"));
		Возврат Ложь;
	КонецЕсли;
	...
	Если НЕ ЗначениеЗаполнено(ВидОтчета) Тогда
		ОбщегоНазначения.СообщитьПользователю(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'В справочнике ""Регламентированные отчеты"" не найдено определение отчета вида %1.'"), ИсточникОтчета));
		Возврат Ложь;
	КонецЕсли;
	...
	Исключение
		ОтменитьТранзакцию();
		ОбщегоНазначения.СообщитьПользователю(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Не удалось запланировать отправку отчета:
				|
...
Функция ОпеределениеReporting(ВебАдрес, ОписаниеОшибки = "") Экспорт
	...
	Попытка
		...
		Возврат Новый WSОпределения(ВебАдрес + "/ws/Reporting?wsdl", ИмяИПароль.Имя, ИмяИПароль.Пароль,, 30);
	Исключение
		ОписаниеОшибки = ИнформацияОбОшибке().Описание;
		Возврат Неопределено;
	КонецПопытки;
...
Функция ИнициализироватьВебСервисReporting(ОпределениеСервиса, ОписаниеОшибки = "")
	...
	Попытка
		Прокси = Новый WSПрокси(ОпределениеСервиса, "http://www.1c.ru/SaaS/1.0/WS", "Reporting", "ReportingSoap",, 30);
		ИмяИПароль = ИмяПользователяИПарольУдаленногоДоступаКВебСервисамМС();
...
Функция СформироватьФайлПакета(
	...
	Для Инд = 0 По КороткиеИменаФайловВыгрузки.Количество() - 1 Цикл
		...
		Иначе
			Если НЕ ЗаписатьТекстВФайл(ЭлТекстВыгрузки, ПолноеИмяФайлаВыгрузки, ЭлКодировкаТекстаВыгрузки) Тогда
				ОбщегоНазначения.СообщитьПользователю(НСтр("ru = 'Не удалось запланировать отправку отчета: не удалось записать временный файл с отчетностью.'"));
				Возврат Ложь;
			КонецЕсли;
		КонецЕсли;
	...
	Если КонтролирующийОрган = "ФНС" Тогда
		...
		Иначе
			ОбщегоНазначения.СообщитьПользователю(НСтр("ru = 'Не удалось запланировать отправку отчета: отсутствуют сведения о доверенности.'"));
			Возврат Ложь;
		КонецЕсли;
	КонецЕсли;
	...
	Исключение
		ОбщегоНазначения.СообщитьПользователю(НСтр("ru='Не удалось запланировать отправку отчета:
...
Функция ИзменитьФайлВыгрузкиПриОтправкеЧерезУполномоченногоПредставителя(ТекстВыгрузки, ИмяФайлаВыгрузки, Доверенность, КодНО, ДатаПериода = '00010101')
	...
	Если нрег(РасширениеФайла) = ".xml" Тогда // формат xml - меняем ИдФайл и имя файла
		...
		Если КомпонентыИмени.Количество() < 5 Тогда
			ОбщегоНазначения.СообщитьПользователю(НСтр("ru = 'Невозможно изменить реквизиты отправителя в имени файла отчета: некорректное исходное имя файла выгрузки.'"));
			Возврат Ложь;
		КонецЕсли;
...
Функция СформироватьИдФайлДоверенности(РегистрацияВИФНС, СведенияДоверенности, Префикс = "", Организация = Неопределено)
	...
	НОПосредник = ?(ЭтоКрупнейший, СокрЛП(КодНОКрупнейшего), "");
...
Функция ПроверитьВозможностьВыгрузкиДоверенности(СведенияДоверенности)
	...
	Если ОшибкиПроверки.Количество() > 0 Тогда
		Для Каждого СтрОшибкаПроверки Из ОшибкиПроверки Цикл
			ОбщегоНазначения.СообщитьПользователю(НСтр(СтрОшибкаПроверки.ТекстОшибки), СтрОшибкаПроверки.КлючДанных, СтрОшибкаПроверки.Поле, СтрОшибкаПроверки.ПутьКДанным);
		КонецЦикла;
	КонецЕсли;
...
Функция ПолучитьСоответствиеРеквизитовКодамПолномочийДоверенности()
	...
	СоответствиеРеквизитовКодамПолномочий.Вставить("Признак23", "23");
	СоответствиеРеквизитовКодамПолномочий.Вставить("Признак24", "24");
	СоответствиеРеквизитовКодамПолномочий.Вставить("Признак25", "25");
	СоответствиеРеквизитовКодамПолномочий.Вставить("Признак26", "26");
	СоответствиеРеквизитовКодамПолномочий.Вставить("Признак27", "27");
	СоответствиеРеквизитовКодамПолномочий.Вставить("Признак28", "28");
	СоответствиеРеквизитовКодамПолномочий.Вставить("Признак29", "29");
	СоответствиеРеквизитовКодамПолномочий.Вставить("Признак30", "30");
	СоответствиеРеквизитовКодамПолномочий.Вставить("Признак31", "31");
	СоответствиеРеквизитовКодамПолномочий.Вставить("Признак32", "32");
	СоответствиеРеквизитовКодамПолномочий.Вставить("Признак33", "33");
	СоответствиеРеквизитовКодамПолномочий.Вставить("Признак34", "34");
	СоответствиеРеквизитовКодамПолномочий.Вставить("Признак35", "35");
	СоответствиеРеквизитовКодамПолномочий.Вставить("Признак36", "36");
	СоответствиеРеквизитовКодамПолномочий.Вставить("Признак37", "37");
	СоответствиеРеквизитовКодамПолномочий.Вставить("Признак38", "38");
	СоответствиеРеквизитовКодамПолномочий.Вставить("Признак39", "39");
...
Функция ЭтоОтчетСтатистики(Отчет)
	...
	Если  СтрНайти(ВРЕГ(ВидОтчета.Родитель.Наименование), ВРЕГ("Статистика")) > 0
		И СтрНайти(ВРЕГ(ВидОтчета.Родитель.Наименование), ВРЕГ("СтатистикаБанкРоссии")) = 0 Тогда
		Возврат Истина;
	КонецЕсли; 
