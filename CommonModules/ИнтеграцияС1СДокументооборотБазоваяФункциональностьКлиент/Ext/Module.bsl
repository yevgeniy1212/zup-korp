...
Процедура СоздатьИнтегрированныйОбъектИзДанныхЗаполнения(ДанныеЗаполнения, Форма = Неопределено) Экспорт
	...
	ПараметрыФормы.Вставить("Основание", ДанныеЗаполнения.ЗначенияРеквизитов);
...
Процедура СохранитьИзмененияРедактируемогоФайла(ID, Наименование, Расширение,
	...
	ОписаниеОповещения = Новый ОписаниеОповещения(
		...
		Параметры);
	Попытка
		НачатьПомещениеФайла(ОписаниеОповещения,,СведенияОРедактировании.ПолныйПуть, Ложь, УникальныйИдентификаторФормы);
	Исключение
		ТекстПредупреждения = СтрШаблон(
			НСтр("ru = 'Не удалось сохранить изменения по причине:
				|%1'"),
...
//
Функция ДанныеПодписей(ДеревоПодписей) Экспорт
...
Процедура НачатьДобавлениеЭПИзФайлаЗавершение(Результат, ПараметрыОповещения) Экспорт
	...
	Для Каждого Подпись Из Подписи Цикл
		...
		ДанныеПодписи = ИнтеграцияС1СДокументооборотБазоваяФункциональностьКлиентСервер.ДанныеПодписи(
			...
			СвойстваПодписи);
		МассивПодписей.Добавить(ДанныеПодписи);
	КонецЦикла;
