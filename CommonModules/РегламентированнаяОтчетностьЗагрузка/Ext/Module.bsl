...
Функция ЗагрузкаОтчетаИзФайла(ФормаОтчета, ПараметрыОтчета, ДанныеДляЗагрузки, ДеревоДляЗагрузки) Экспорт
	...
	ИначеЕсли ДанныеДляЗагрузки.ТипДокумента = Перечисления.ТипыОтправляемыхДокументов.ПрочаяОтчетностьФНС
		 ...
		 ИЛИ СтрНайти(ОсновнойФайл.ИмяФайла, "ON_OTCHOPTOV") = 1 // файл отчета "Отчет об операциях с товарами, подлежащими прослеживаемости"
	...
	ИначеЕсли ДанныеДляЗагрузки.ТипДокумента = Перечисления.ТипыОтправляемыхДокументов.ПрочаяОтчетностьПФР Тогда
		
		СвойстваОсновногоФайла = СвойстваФайлаЗагрузки(ТекстОсновногоФайла, 10, Истина, Истина, Истина);
		ПараметрыОтчета.Вставить("ИмяФайлаВыгрузки", ОсновнойФайл.ИмяФайла);
	...
	ПараметрыВыполнения.Вставить("КопированиеОтчета",             ДанныеДляЗагрузки.Свойство("КопированиеОтчета"));
	ПараметрыВыполнения.Вставить("ОбнулятьОкругления",            ДанныеДляЗагрузки.Свойство("ОбнулятьОкругления") И ДанныеДляЗагрузки.ОбнулятьОкругления);
...
Процедура ЗагрузкаОтчетаИзФайлаДлительнаяОперация(ПараметрыПроцедуры, АдресРезультата) Экспорт
	...
	КопированиеОтчета             = ПараметрыПроцедуры.КопированиеОтчета;
	ОбнулятьОкругления            = ПараметрыПроцедуры.ОбнулятьОкругления;
	...
	Попытка
		...
		Если КопированиеОтчета Тогда 
			ДеревоДляЗагрузки.Колонки.Добавить("_КопированиеОтчета");
		КонецЕсли;
		...
		Если ОбнулятьОкругления Тогда 
			ОбнулениеЧисловыхПараметровПриНеправильномОкруглении(ДеревоДляЗагрузки.Строки, Новый ОписаниеТипов("Число"));
		КонецЕсли;
	Исключение
...
Функция ЗаполнитьУзелДерева(ОбъектЧтениеXML, УзелДерева, ЭлементыРодительскогоУзлаДерева, ДопПараметры = Неопределено, ЭлементаНетВДереве = Ложь);
	...
	Пока ОбъектЧтениеXML.Прочитать() Цикл
		...
		Если ОбъектЧтениеXML.ТипУзла = ТипУзлаXML.НачалоЭлемента Тогда
			...
			ЭлементНайден = Ложь;
			ДеревоЗагрузки = ?(ТипЗнч(УзелДерева) = Тип("ДеревоЗначений"), УзелДерева, УзелДерева.Владелец());
			ЭтоКопирование = (ДеревоЗагрузки.Колонки.Найти("_КопированиеОтчета") <> Неопределено)
								И (ТипЗнч(УзелДерева) = Тип("СтрокаДереваЗначений"));
			...
			Для НомПопытки = 1 По 2 Цикл
				...
				Для Инд = 0 По МаксИндекс Цикл
					...
					// Дополнено:
					// при копировании на нижнем уровне дерева не требуем полного совпадения реквизитов
					// т.е. если была многострочка ДатаДок,НомДок,ТипДок а стало ДатаДок,НомДок,ВидДок то
					// ДатаДок,НомДок скопируются из старого отчета
					Если НЕ УстановитьВсеАтрибуты(АтрибутыУзлаДерева, АтрибутыУзлаXML, ЭтоКопирование) Тогда
						Продолжить;
					КонецЕсли;
...
Функция УстановитьВсеАтрибуты(АтрибутыУзлаПриемника, АтрибутыУзлаИсточника, ЭтоКопирование)
...
Функция ДатаИзСтрокиЛюбогоФормата(СтрДата) Экспорт 
	...
	Если ДлинаСтроки = 7
		И СтрНайти(СтрДата, ".") = 3 Тогда 
		
		Месяц = XMLЗначение(Тип("Число"), Лев(СтрДата, 2));
		Год = XMLЗначение(Тип("Число"), Прав(СтрДата, 4));
		
		Если Месяц * Год > 0 И Месяц < 12 Тогда 
			Возврат Дата(Год, Месяц, 1, 0, 0, 0);
		КонецЕсли;
	КонецЕсли;
	...
	Если СтрДлина(МассивПолей[0]) = 8 ИЛИ СтрДлина(МассивПолей[0]) = 14 Тогда
		...
		Возврат ОТД.ПривестиЗначение(МассивПолей[0]);
	...
	Для Инд = 0 По КоличествоСтрок - 1 Цикл
		...
		Если МногоуровневыйУзел(ДочернийУзел) Тогда
			...
			ИначеЕсли МножественныйУзел(ДочернийУзел) И МногострочностьУзла(ДочернийУзел) Тогда
				...
				КлючУзла = "Строки;" + РазделУзла(ДочернийУзел) + ";" + ДочернийУзел.Код;
		...
		Для Каждого НомСтраницыЭлемент Из НомСтраницы Цикл
			...
			Если ВетвьШаблона.Данные = Неопределено Тогда // это группа - перешагнем ее
				...
				Если НомСтраницыЭлемент = НомерСтрокиШаблона Тогда
					Если ПараметрыОтчета.Свойство("НомСтраницыПриложения", НомСтраницыЭлемент) Тогда // Для отчета НДДУ и др.
						Если НЕ (ТипЗнч(НомСтраницыЭлемент) = Тип("Число") И НомСтраницыЭлемент > 0) Тогда
...
Процедура ДополнитьСтруктуруРеквизитовПараметрамиКопирования(СтруктураРеквизитовФормы) Экспорт 
	СтруктураРеквизитовФормы.Вставить("мСкопированнаяФормаДругойРедакции");
	СтруктураРеквизитовФормы.Вставить("ТекущийФормат");
	СтруктураРеквизитовФормы.Вставить("ФорматКопируемойФормы");
	СтруктураРеквизитовФормы.Вставить("НеобходимоПреобразованиеИзПрошлойРедакции", Ложь);
	СтруктураРеквизитовФормы.Вставить("ДатаНачалаСкопированногоОтчета");
	СтруктураРеквизитовФормы.Вставить("ДатаКонцаСкопированногоОтчета");
	СтруктураРеквизитовФормы.Вставить("ДокументИсточник");
КонецПроцедуры

Процедура ЗаполнитьПарметрыКопирования(Форма) Экспорт 
	ДеревоФормИФорматов = ДеревоФормИФорматов(Форма);
	
	СтруктураРеквизитовФормы = Форма.СтруктураРеквизитовФормы;
	Форма.СтруктураРеквизитовФормы.мСкопированнаяФормаДругойРедакции = Форма.СтруктураРеквизитовФормы.мСохраненныйДок;
	Форма.Параметры.Свойство("ДокументИсточник", Форма.СтруктураРеквизитовФормы.ДокументИсточник);
	
	ОтбСтроки = ДеревоФормИФорматов.Строки.НайтиСтроки(Новый Структура("ИмяОбъекта", СтруктураРеквизитовФормы.мВыбраннаяФорма));
	Если ОтбСтроки.Количество() = 1
		И ОтбСтроки[0].Строки.Количество() > 0 Тогда 
		
		СтруктураРеквизитовФормы.ТекущийФормат = ОтбСтроки[0].Строки[0].Код;
	КонецЕсли;
	
	Если ЗначениеЗаполнено(СтруктураРеквизитовФормы.мСкопированаФорма) Тогда 
		ОтбСтроки = ДеревоФормИФорматов.Строки.НайтиСтроки(Новый Структура("ИмяОбъекта", СтруктураРеквизитовФормы.мСкопированаФорма));
		Если ОтбСтроки.Количество() = 1
			И ОтбСтроки[0].Строки.Количество() > 0 Тогда 
			
			СтруктураРеквизитовФормы.ФорматКопируемойФормы = ОтбСтроки[0].Строки[0].Код;
			СтруктураРеквизитовФормы.НеобходимоПреобразованиеИзПрошлойРедакции =
				(СтруктураРеквизитовФормы.ФорматКопируемойФормы <> СтруктураРеквизитовФормы.ТекущийФормат);
		КонецЕсли;
			
		ПериодКопируемогоОтчета = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(СтруктураРеквизитовФормы.мСохраненныйДок, "ДатаНачала, ДатаОкончания");
		СтруктураРеквизитовФормы.Вставить("ДатаНачалаСкопированногоОтчета", ПериодКопируемогоОтчета.ДатаНачала);
		СтруктураРеквизитовФормы.Вставить("ДатаКонцаСкопированногоОтчета", ПериодКопируемогоОтчета.ДатаОкончания);
	КонецЕсли;
КонецПроцедуры

Функция ДеревоФормИФорматов(Форма)
	Если СтрНачинаетсяС(Форма.ИмяФормы, "ВнешнийОтчет") Тогда 
		Возврат Форма.ОбъектОтчета(Форма.ИмяФормы).ДеревоФормИФорматов();
	Иначе
		Возврат Отчеты[СтрРазделить(Форма.ИмяФормы, ".")[1]].ДеревоФормИФорматов();
	КонецЕсли;
КонецФункции

Процедура ОбнулениеЧисловыхПараметровПриНеправильномОкруглении(ДеревоДляЗагрузкиСтроки, ОТЧ0)
	Для Каждого Стр Из ДеревоДляЗагрузкиСтроки Цикл
		Если (Стр.Тип = "А" Или Стр.Тип = "A")
			И Стр.Формат = "N" Тогда
			
			МаксРазмерность = Стр.МаксРазмерность;
			МинРазмерность = ?(Стр.МинРазмерность > 100, 0, Стр.МинРазмерность);
			ОТЧ = Новый ОписаниеТипов("Число", Новый КвалификаторыЧисла(МаксРазмерность, МинРазмерность));
			
			СтрокаФормата = "ЧРД=.;ЧН=0;ЧГ=;";
			Если Стр.МаксРазмерность <> 0 И Стр.МаксРазмерность <> 99999 Тогда
				СтрокаФормата = СтрокаФормата + "ЧЦ=" + Формат(Стр.МаксРазмерность, "ЧГ=") + ";";
			КонецЕсли;
			Если Стр.МинРазмерность <> 99999 Тогда
				СтрокаФормата = СтрокаФормата + "ЧДЦ=" + Формат(Стр.МинРазмерность, "ЧГ=") + ";";
			КонецЕсли;
			ПриведенноеЗначение = СокрЛП(Формат(ОТЧ.ПривестиЗначение(Стр.Значение), СтрокаФормата));
			Если (ПриведенноеЗначение <> Стр.Значение) 
				И ОТЧ0.ПривестиЗначение(ПриведенноеЗначение) <> ОТЧ0.ПривестиЗначение(Стр.Значение) Тогда 
				
				Стр.Значение = "0";
			КонецЕсли;
		КонецЕсли;
		
		ОбнулениеЧисловыхПараметровПриНеправильномОкруглении(Стр.Строки, ОТЧ0);
	КонецЦикла;
КонецПроцедуры
