...
Процедура ОбработатьДанныеОнлайнСервисов(Дескриптор, ПутьКФайлу)
	...
	Попытка
		...
		КаталогРаспаковки = КаталогРаспаковки + ПолучитьРазделительПути();
...
Процедура СохранитьДанныеОнлайнСервисовРО(ТипДанных, ПутьКФайлу, ИмяФайла, ДатаПолучения)
	...
	Если СодержимоеФайла = Неопределено Тогда
		Шаблон = НСтр("ru = 'Ошибка чтения файла с данными. Описание ошибки: %1.'");
	...
	Если НаборЗаписей.Модифицированность() Тогда
		...
		Исключение
			Шаблон = НСтр("ru = 'Ошибка сохранения в информационной базе информации: %1.'");
			ТекстОшибки = СтрШаблон(Шаблон, ИнформацияОбОшибке().Описание);
			ЗаписатьВЖурналРегистрации(УровеньЖурналаРегистрации.Ошибка, ТекстОшибки);
		КонецПопытки;
	КонецЕсли;
