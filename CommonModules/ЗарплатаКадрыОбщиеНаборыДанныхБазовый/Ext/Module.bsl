...
Функция ПолучитьТекстОбщегоЗапроса(ИсточникДанных, ТолькоРазрешенные) Экспорт
	...
	Если ВРег(ИсточникДанных) = ВРег("НачисленияУдержания") Тогда
		...
		// ЗУПК
		|	ВЫБОР
		|		КОГДА БухгалтерскиеВзаиморасчетыССотрудниками.СтатьяРасходов = ЗНАЧЕНИЕ(Справочник.СтатьиРасходовЗарплата.ПустаяСсылка)
		|			ТОГДА ЕСТЬNULL(СтатьиРасходовЗарплата.Ссылка, ЗНАЧЕНИЕ(Справочник.СтатьиРасходовЗарплата.ПустаяСсылка))
		|		ИНАЧЕ БухгалтерскиеВзаиморасчетыССотрудниками.СтатьяРасходов
		...
		// ЗУПК
		|	СУММА(БухгалтерскиеВзаиморасчетыССотрудниками.СуммаВзаиморасчетовНачальныйОстаток) КАК НачальныйОстаток,
		|	СУММА(БухгалтерскиеВзаиморасчетыССотрудниками.СуммаВзаиморасчетовКонечныйОстаток) КАК КонечныйОстаток,
		// ЗУПК
		...
		|		ПО БухгалтерскиеВзаиморасчетыССотрудниками.СтатьяРасходов = ЗНАЧЕНИЕ(Справочник.СтатьиРасходовЗарплата.ПустаяСсылка)
		|			И СтатьиРасходовЗарплата.Код = ""ОТ""
		|
        |СГРУППИРОВАТЬ ПО
		...
		|	БухгалтерскиеВзаиморасчетыССотрудниками.Период
		// ЗУПК
		|{ГДЕ
		|	БухгалтерскиеВзаиморасчетыССотрудниками.Организация.*,
		|	БухгалтерскиеВзаиморасчетыССотрудниками.Сотрудник.*,
		...
		// ЗУПК
		|	ВЫБОР
		|		КОГДА ВзаиморасчетыССотрудниками.СтатьяРасходов = ЗНАЧЕНИЕ(Справочник.СтатьиРасходовЗарплата.ПустаяСсылка)
		|			ТОГДА ЕСТЬNULL(СтатьиРасходовЗарплата.Ссылка, ЗНАЧЕНИЕ(Справочник.СтатьиРасходовЗарплата.ПустаяСсылка))
		|		ИНАЧЕ ВзаиморасчетыССотрудниками.СтатьяРасходов
		|	КОНЕЦ КАК СтатьяРасходов,
		// ЗУПК
		|	ВзаиморасчетыССотрудниками.Организация КАК Организация,
		|	ВзаиморасчетыССотрудниками.Подразделение КАК Подразделение,
		// ЗУПК
		|	СУММА(ВзаиморасчетыССотрудниками.СуммаВзаиморасчетовНачальныйОстаток) КАК НачальныйОстаток,
		|	СУММА(ВзаиморасчетыССотрудниками.СуммаВзаиморасчетовКонечныйОстаток) КАК КонечныйОстаток,
		// ЗУПК
		...
		// ЗУПК
		|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.СтатьиРасходовЗарплата КАК СтатьиРасходовЗарплата
		|		ПО ВзаиморасчетыССотрудниками.СтатьяРасходов = ЗНАЧЕНИЕ(Справочник.СтатьиРасходовЗарплата.ПустаяСсылка)
		|			И СтатьиРасходовЗарплата.Код = ""ОТ""
		|
        |СГРУППИРОВАТЬ ПО
		...
		// ЗУПК
		|	ВЫБОР
		|		КОГДА НачисленияИУдержания.СтатьяРасходов = ЗНАЧЕНИЕ(Справочник.СтатьиРасходовЗарплата.ПустаяСсылка)
		|			ТОГДА ЕСТЬNULL(СтатьиРасходовЗарплата.Ссылка, ЗНАЧЕНИЕ(Справочник.СтатьиРасходовЗарплата.ПустаяСсылка))
		|		ИНАЧЕ НачисленияИУдержания.СтатьяРасходов
		|	КОНЕЦ КАК СтатьяРасходов,
		// ЗУПК
		|	НачисленияИУдержания.Подразделение КАК Подразделение,
		|	НачисленияИУдержания.ГруппаНачисленияУдержанияВыплаты КАК Группа,
		|	ВЫБОР
		|		КОГДА НачисленияИУдержания.НачислениеУдержание = ЗНАЧЕНИЕ(Перечисление.ВидыОсобыхНачисленийИУдержаний.ЕП)
		|			ТОГДА ""Единый платеж (за счет работника)""
		...
		// ЗУПК
		|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.СтатьиРасходовЗарплата КАК СтатьиРасходовЗарплата
		|		ПО (НачисленияИУдержания.СтатьяРасходов = ЗНАЧЕНИЕ(Справочник.СтатьиРасходовЗарплата.ПустаяСсылка))
		|			И (СтатьиРасходовЗарплата.Код = ""ОТ"")
		// ЗУПК
		|ГДЕ
		...
		// ЗУПК
		|	И НЕ НачисленияИУдержания.НачислениеУдержание В (&УдержанияЗУПК)
		// ЗУПК
		|{ГДЕ
		...
		|
		|ВЫБРАТЬ
		|	НАЧАЛОПЕРИОДА(НачисленияИУдержания.ПериодДействия, МЕСЯЦ) КАК МесяцНачисления,
		|	НачисленияИУдержания.ПериодДействия КАК Период,
		|	НачисленияИУдержания.ФизическоеЛицо КАК ФизическоеЛицо,
		|	НачисленияИУдержания.СтатьяФинансирования КАК СтатьяФинансирования,
		|	ВЫБОР
		|		КОГДА НачисленияИУдержания.СтатьяРасходов = ЗНАЧЕНИЕ(Справочник.СтатьиРасходовЗарплата.ПустаяСсылка)
		...
		|	НачисленияИУдержания.ПериодДействия МЕЖДУ &НачалоПериода И &КонецПериода
		|	И НЕ НачисленияИУдержания.Регистратор В (&ИсключаемыеСсылки)
		|	И НачисленияИУдержания.НачислениеУдержание В (&УдержанияЗУПК)
		|{ГДЕ
		|	НачисленияИУдержания.Сотрудник.*,
		|	НачисленияИУдержания.ФизическоеЛицо.*,
		|	НачисленияИУдержания.Организация.*,
		...
		|	ВТСотрудникиМесяцы.СтатьяФинансирования,
		|	ВТСотрудникиМесяцы.СтатьяРасходов,
		|	ВТСотрудникиМесяцы.Подразделение,
		|	СУММА(ЕПКомпоненты.ООСМС + ЕПКомпоненты.СО + ЕПКомпоненты.ОПВР) КАК ЕПЗаСчетработодателя, 
		|	СУММА(ЕПКомпоненты.ООСМС) КАК ООСМС,
		|	СУММА(ЕПКомпоненты.СО) КАК СО,
		|	СУММА(ЕПКомпоненты.ОПВР) КАК ОПВР,
		|	СУММА(ЕПКомпоненты.ОПВ) КАК ОПВ,
		|	СУММА(ЕПКомпоненты.ИПН) КАК ИПН,
		|	СУММА(ЕПКомпоненты.ВОСМС) КАК ВОСМС,
		|	ЕПКомпоненты.Организация,
		|	ВТСотрудникиМесяцы.Сотрудник,
		|	ЕПКомпоненты.Период,
		|	ЕПКомпоненты.Регистратор,
		|	ЕПКомпоненты.Начисление
		|ПОМЕСТИТЬ ВТЕПКомпоненты
		|ИЗ
		|	ВТСотрудникиМесяцы КАК ВТСотрудникиМесяцы
		|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрНакопления.ЕПКомпоненты КАК ЕПКомпоненты
		|		ПО (КОНЕЦПЕРИОДА(ВТСотрудникиМесяцы.Месяц, МЕСЯЦ) = КОНЕЦПЕРИОДА(ЕПКомпоненты.Период, МЕСЯЦ))
//		|			И ВТСотрудникиМесяцы.Сотрудник = ЕПКомпоненты.Сотрудник
		|			И ВТСотрудникиМесяцы.ФизическоеЛицо = ЕПКомпоненты.ФизическоеЛицо
		|			И ВТСотрудникиМесяцы.Организация = ЕПКомпоненты.Организация
		|			И ВТСотрудникиМесяцы.Подразделение = ЕПКомпоненты.Подразделение
		|ГДЕ
		|	ЕПКомпоненты.Период МЕЖДУ &НачалоПериода И &КонецПериода
		|
		|СГРУППИРОВАТЬ ПО
		|	ЕПКомпоненты.Период,
		|	ЕПКомпоненты.ФизическоеЛицо,
		...
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ РАЗРЕШЕННЫЕ
		|	НалогиВзносыОтчисленияПоСотрудникам.Период КАК МесяцНачисления,
		|	НалогиВзносыОтчисленияПоСотрудникам.ФизическоеЛицо КАК ФизическоеЛицо,
		|	ВТСотрудникиМесяцы.СтатьяФинансирования КАК СтатьяФинансирования,
		|	ВТСотрудникиМесяцы.СтатьяРасходов КАК СтатьяРасходов,
		|	ВТСотрудникиМесяцы.Подразделение КАК Подразделение,
		|	ЗНАЧЕНИЕ(Перечисление.ГруппыНачисленияУдержанияВыплаты.Справочно) КАК Группа,
		|	ЗНАЧЕНИЕ(Перечисление.ВидыНалоговВзносовОтчислений.СО) КАК ВидРасчета,
		|	НалогиВзносыОтчисленияПоСотрудникам.СО КАК Сумма,
		|	НалогиВзносыОтчисленияПоСотрудникам.Организация КАК Организация,
		|	ВТСотрудникиМесяцы.Сотрудник КАК Сотрудник,
		|	НалогиВзносыОтчисленияПоСотрудникам.Период КАК ПериодДействия,
		...
		|ПОМЕСТИТЬ ВТДанныеОтчислений
		|ИЗ
		|	ВТСотрудникиМесяцы КАК ВТСотрудникиМесяцы
		|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрНакопления.НалогиВзносыОтчисленияПоСотрудникам КАК НалогиВзносыОтчисленияПоСотрудникам
		|		ПО (КОНЕЦПЕРИОДА(ВТСотрудникиМесяцы.Месяц, МЕСЯЦ) = КОНЕЦПЕРИОДА(НалогиВзносыОтчисленияПоСотрудникам.Период, МЕСЯЦ))
		|			И ВТСотрудникиМесяцы.Сотрудник = НалогиВзносыОтчисленияПоСотрудникам.Сотрудник
		|			И ВТСотрудникиМесяцы.Организация = НалогиВзносыОтчисленияПоСотрудникам.Организация
		|			И ВТСотрудникиМесяцы.Подразделение = НалогиВзносыОтчисленияПоСотрудникам.Подразделение
		|ГДЕ
		|	НалогиВзносыОтчисленияПоСотрудникам.Период МЕЖДУ &НачалоПериода И &КонецПериода
		|	И НалогиВзносыОтчисленияПоСотрудникам.ВидДохода = ЗНАЧЕНИЕ(Справочник.ВидыДоходов.ДоходыОтРаботодателя)
		|	И НалогиВзносыОтчисленияПоСотрудникам.СО <> 0
		|
		|ОБЪЕДИНИТЬ ВСЕ
		|
		|ВЫБРАТЬ
		|	НалогиВзносыОтчисленияПоСотрудникам.Период,
		|	НалогиВзносыОтчисленияПоСотрудникам.ФизическоеЛицо,
		|	ВТСотрудникиМесяцы.СтатьяФинансирования,
		|	ВТСотрудникиМесяцы.СтатьяРасходов,
		|	ВТСотрудникиМесяцы.Подразделение,
		|	ЗНАЧЕНИЕ(Перечисление.ГруппыНачисленияУдержанияВыплаты.Справочно),
		|	ЗНАЧЕНИЕ(Перечисление.ВидыНалоговВзносовОтчислений.СН),
		|	НалогиВзносыОтчисленияПоСотрудникам.СН,
		|	НалогиВзносыОтчисленияПоСотрудникам.Организация,
		|	ВТСотрудникиМесяцы.Сотрудник,
		|	НалогиВзносыОтчисленияПоСотрудникам.Период,
		|	НалогиВзносыОтчисленияПоСотрудникам.Регистратор,
		|	НалогиВзносыОтчисленияПоСотрудникам.Начисление
		|ИЗ
		|	ВТСотрудникиМесяцы КАК ВТСотрудникиМесяцы
		|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрНакопления.НалогиВзносыОтчисленияПоСотрудникам КАК НалогиВзносыОтчисленияПоСотрудникам
		|		ПО (КОНЕЦПЕРИОДА(ВТСотрудникиМесяцы.Месяц, МЕСЯЦ) = КОНЕЦПЕРИОДА(НалогиВзносыОтчисленияПоСотрудникам.Период, МЕСЯЦ))
		...
		|	НалогиВзносыОтчисленияПоСотрудникам.Период МЕЖДУ &НачалоПериода И &КонецПериода
		|	И НалогиВзносыОтчисленияПоСотрудникам.ВидДохода = ЗНАЧЕНИЕ(Справочник.ВидыДоходов.ДоходыОтРаботодателя)
		|	И НалогиВзносыОтчисленияПоСотрудникам.СН <> 0
		|
		|ОБЪЕДИНИТЬ ВСЕ
		|
		|ВЫБРАТЬ
		...
		|	ВТСотрудникиМесяцы.Подразделение,
		|	ЗНАЧЕНИЕ(Перечисление.ГруппыНачисленияУдержанияВыплаты.Справочно),
		|	ЗНАЧЕНИЕ(Перечисление.ВидыНалоговВзносовОтчислений.ОППВ),
		|	НалогиВзносыОтчисленияПоСотрудникам.ОППВ,
		|	НалогиВзносыОтчисленияПоСотрудникам.Организация,
		|	ВТСотрудникиМесяцы.Сотрудник,
		|	НалогиВзносыОтчисленияПоСотрудникам.Период,
		|	НалогиВзносыОтчисленияПоСотрудникам.Регистратор,
		...
		|	НалогиВзносыОтчисленияПоСотрудникам.Период МЕЖДУ &НачалоПериода И &КонецПериода
		|	И НалогиВзносыОтчисленияПоСотрудникам.ВидДохода = ЗНАЧЕНИЕ(Справочник.ВидыДоходов.ДоходыОтРаботодателя)
		|	И НалогиВзносыОтчисленияПоСотрудникам.ОППВ <> 0
		|
		|ОБЪЕДИНИТЬ ВСЕ
		|
		|ВЫБРАТЬ
		...
		|	НалогиВзносыОтчисленияПоСотрудникам.ООСМС,
		|	НалогиВзносыОтчисленияПоСотрудникам.Организация,
		|	ВТСотрудникиМесяцы.Сотрудник,
		|	НалогиВзносыОтчисленияПоСотрудникам.Период,
		|	НалогиВзносыОтчисленияПоСотрудникам.Регистратор,
		...
		|	НалогиВзносыОтчисленияПоСотрудникам.Период МЕЖДУ &НачалоПериода И &КонецПериода
		|	И НалогиВзносыОтчисленияПоСотрудникам.ВидДохода = ЗНАЧЕНИЕ(Справочник.ВидыДоходов.ДоходыОтРаботодателя)
		|	И НалогиВзносыОтчисленияПоСотрудникам.ООСМС <> 0
		|
		|ОБЪЕДИНИТЬ ВСЕ
		|
		|ВЫБРАТЬ
		...
		|	ВТСотрудникиМесяцы.Подразделение,
		|	ЗНАЧЕНИЕ(Перечисление.ГруппыНачисленияУдержанияВыплаты.Справочно),
		|	ЗНАЧЕНИЕ(Перечисление.ВидыНалоговВзносовОтчислений.ОПВР),
		|	НалогиВзносыОтчисленияПоСотрудникам.ОПВР,
		|	НалогиВзносыОтчисленияПоСотрудникам.Организация,
		|	ВТСотрудникиМесяцы.Сотрудник,
		|	НалогиВзносыОтчисленияПоСотрудникам.Период,
		|	НалогиВзносыОтчисленияПоСотрудникам.Регистратор,
		...
		|	НалогиВзносыОтчисленияПоСотрудникам.Период МЕЖДУ &НачалоПериода И &КонецПериода
		|	И НалогиВзносыОтчисленияПоСотрудникам.ВидДохода = ЗНАЧЕНИЕ(Справочник.ВидыДоходов.ДоходыОтРаботодателя)
		|	И НЕ НалогиВзносыОтчисленияПоСотрудникам.ОПВР = 0
		|
		|ОБЪЕДИНИТЬ ВСЕ
		|
		|ВЫБРАТЬ
		|	ЕПКомпоненты.Период,
		|	ЕПКомпоненты.ФизическоеЛицо,
		|	ЕПКомпоненты.СтатьяФинансирования,
		|	ЕПКомпоненты.СтатьяРасходов,
		|	ЕПКомпоненты.Подразделение,
		|	ЗНАЧЕНИЕ(Перечисление.ГруппыНачисленияУдержанияВыплаты.Справочно),
		|	""Единый платеж (за счет работодателя)"",
		|	ЕПКомпоненты.ЕПЗаСчетРаботодателя КАК ЕПЗаСчетРаботодателя,
		|	ЕПКомпоненты.Организация,
		...
		|	ЕПКомпоненты.Начисление
		|ИЗ
		|	ВТЕПКомпоненты КАК ЕПКомпоненты
		|ГДЕ
		|	ЕПКомпоненты.ЕПЗаСчетРаботодателя <> 0
		|
		|ОБЪЕДИНИТЬ ВСЕ
		|
		|ВЫБРАТЬ
		...
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ РАЗРЕШЕННЫЕ
		|	НАЧАЛОПЕРИОДА(ДанныеОтчислений.МесяцНачисления, МЕСЯЦ) КАК МесяцНачисления,
		|	ДанныеОтчислений.ФизическоеЛицо КАК ФизическоеЛицо,
		|	ДанныеОтчислений.СтатьяФинансирования КАК СтатьяФинансирования,
		|	ДанныеОтчислений.СтатьяРасходов КАК СтатьяРасходов,
		|	ДанныеОтчислений.Подразделение КАК Подразделение,
		|	ДанныеОтчислений.Группа КАК Группа,
		|	ДанныеОтчислений.ВидРасчета КАК ВидРасчета,
		|	СУММА(ДанныеОтчислений.Сумма) КАК Сумма,
		|	ДанныеОтчислений.Организация КАК Организация,
		|	ДанныеОтчислений.Сотрудник КАК Сотрудник,
		|	ДанныеОтчислений.ПериодДействия КАК ПериодДействия,
		|	ДанныеОтчислений.Регистратор КАК Регистратор,
		|	ДанныеОтчислений.Начисление КАК Начисление
		|ПОМЕСТИТЬ ВТДанныеОтчисленийСгруппированные
		|ИЗ
		|	ВТДанныеОтчислений КАК ДанныеОтчислений
		|{ГДЕ
		|	ДанныеОтчислений.Сотрудник.*,
		|	ДанныеОтчислений.ФизическоеЛицо.*,
		|	ДанныеОтчислений.Организация.*,
		|	ДанныеОтчислений.Сотрудник.ГоловнойСотрудник.* КАК ГоловнойСотрудник}
		|
		|СГРУППИРОВАТЬ ПО
		|	НАЧАЛОПЕРИОДА(ДанныеОтчислений.МесяцНачисления, МЕСЯЦ),
		|	ДанныеОтчислений.ФизическоеЛицо,
		|	ДанныеОтчислений.СтатьяФинансирования,
		|	ДанныеОтчислений.СтатьяРасходов,
		|	ДанныеОтчислений.Подразделение,
		|	ДанныеОтчислений.Группа,
		|	ДанныеОтчислений.ВидРасчета,
		|	ДанныеОтчислений.Организация,
		|	ДанныеОтчислений.Сотрудник,
		|	ДанныеОтчислений.ПериодДействия,
		|	ДанныеОтчислений.Регистратор,
		|	ДанныеОтчислений.Начисление
		|
		|ИНДЕКСИРОВАТЬ ПО
		|	Сотрудник,
		|	МесяцНачисления
		...
		|ПОМЕСТИТЬ ВТОбъединениеНачисленияИУдержанияСКадровымиДанными
		|ИЗ
		|	ВТНачисленияИУдержанияБезКадровыхДанных КАК НачисленияИУдержания
		...
		|	ДанныеОтчислений.МесяцНачисления,
		|	ДанныеОтчислений.ФизическоеЛицо,
		|	ДанныеОтчислений.СтатьяФинансирования,
		|	ДанныеОтчислений.СтатьяРасходов,
		|	ДанныеОтчислений.Подразделение,
		|	ДанныеОтчислений.Группа,
		|	ДанныеОтчислений.ВидРасчета,
		|	ДанныеОтчислений.Сумма,
		|	ДанныеОтчислений.Организация,
		|	ДанныеОтчислений.Сотрудник,
		|	ДанныеОтчислений.МесяцНачисления,
		|	КОНЕЦПЕРИОДА(ДанныеОтчислений.МесяцНачисления, МЕСЯЦ),
		|	ВЫРАЗИТЬ(ДанныеОтчислений.Сотрудник КАК Справочник.Сотрудники).ГоловнойСотрудник,
		|	КадровыеДанныеГоловныхСотрудников.ФИОПолные,
		|	КадровыеДанныеГоловныхСотрудников.ФамилияИО,
		...
		|	КадровыеДанныеГоловныхСотрудников.Организация,
		...
		|	NULL,
		|	ИСТИНА,
		|	КадровыеДанныеГоловныхСотрудников.ДатаПриема,
		|	КадровыеДанныеГоловныхСотрудников.ДатаУвольнения,
		|	ЗНАЧЕНИЕ(Документ.РазовоеНачисление.ПустаяСсылка),
		|	0,
		|	ЛОЖЬ,
		...
		//|	КОНЕЦ КАК ПриоритетВидаРасчета
        // ЗУПК
		|	0 КАК ПриоритетВидаРасчета
		|ИЗ
		|	ВТДанныеОтчисленийСгруппированные КАК ДанныеОтчислений
		|		{ЛЕВОЕ СОЕДИНЕНИЕ Представления_КадровыеДанныеСотрудниковАнализНачисленийИУдержаний КАК КадровыеДанныеСотрудниковНаКонецПериода
		|		ПО (КОНЕЦПЕРИОДА(ДанныеОтчислений.МесяцНачисления, МЕСЯЦ) = КадровыеДанныеСотрудниковНаКонецПериода.Период)
		|			И ДанныеОтчислений.Сотрудник = КадровыеДанныеСотрудниковНаКонецПериода.Сотрудник}
		|		{ЛЕВОЕ СОЕДИНЕНИЕ Представления_КадровыеДанныеСотрудниковАнализНачисленийИУдержаний КАК КадровыеДанныеГоловныхСотрудников
		|		ПО (ВЫРАЗИТЬ(ДанныеОтчислений.Сотрудник КАК Справочник.Сотрудники).ГоловнойСотрудник = КадровыеДанныеГоловныхСотрудников.Сотрудник)
		|			И (КОНЕЦПЕРИОДА(ДанныеОтчислений.МесяцНачисления, МЕСЯЦ) = КадровыеДанныеГоловныхСотрудников.Период)}
		|		{ЛЕВОЕ СОЕДИНЕНИЕ Представления_КадровыеДанныеСотрудниковАнализНачисленийИУдержаний КАК КадровыеДанныеСотрудников
		|		ПО КОНЕЦПЕРИОДА(ДанныеОтчислений.МесяцНачисления, МЕСЯЦ) = КадровыеДанныеСотрудников.Период
		|			И ДанныеОтчислений.Сотрудник = КадровыеДанныеСотрудников.Сотрудник}
		|		{ЛЕВОЕ СОЕДИНЕНИЕ Представления_ПодразделенияСортировкиСотрудников КАК ПодразделенияСортировкиСотрудников
		|		ПО ДанныеОтчислений.ФизическоеЛицо = ПодразделенияСортировкиСотрудников.ФизическоеЛицо
		|			И (КОНЕЦПЕРИОДА(ДанныеОтчислений.МесяцНачисления, МЕСЯЦ) = ПодразделенияСортировкиСотрудников.Период)
		|			И ДанныеОтчислений.Организация.ГоловнаяОрганизация = ПодразделенияСортировкиСотрудников.ГоловнаяОрганизация}
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ РАЗРЕШЕННЫЕ
		|	НачисленияИУдержания.ОрганизацияСортировки КАК ОрганизацияСортировки,
		|	НачисленияИУдержания.ПодразделениеСортировки КАК ПодразделениеСортировки,
		|	НачисленияИУдержания.ПриоритетРабочегоМеста КАК ПриоритетРабочегоМеста,
		|	НачисленияИУдержания.МесяцНачисления КАК МесяцНачисления,
		|	НачисленияИУдержания.ФизическоеЛицо КАК ФизическоеЛицо,
		|	НачисленияИУдержания.СтатьяФинансирования КАК СтатьяФинансирования,
		|	НачисленияИУдержания.СтатьяРасходов КАК СтатьяРасходов,
		...
		|	НачисленияИУдержания.ДатаОкончания КАК ДатаОкончания,
		|	НачисленияИУдержания.ГоловнойСотрудник КАК ГоловнойСотрудник,
		|	НачисленияИУдержания.ФИОПолные КАК ФИОПолные,
		|	НачисленияИУдержания.ФамилияИО КАК ФамилияИО,
		|	НачисленияИУдержания.Фамилия КАК Фамилия,
		|	НачисленияИУдержания.Имя КАК Имя,
		|	НачисленияИУдержания.Отчество КАК Отчество,
		|	НачисленияИУдержания.ДолжностьГоловногоСотрудникаНаКонецПериода КАК ДолжностьГоловногоСотрудникаНаКонецПериода,
		|	НачисленияИУдержания.ОрганизацияГоловногоСотрудникаНаКонецПериода КАК ОрганизацияГоловногоСотрудникаНаКонецПериода,
		|	НачисленияИУдержания.ПодразделениеГоловногоСотрудникаНаКонецПериода КАК ПодразделениеГоловногоСотрудникаНаКонецПериода,
		|	НачисленияИУдержания.ВидЗанятостиГоловногоСотрудникаНаКонецПериода КАК ВидЗанятостиГоловногоСотрудникаНаКонецПериода,
		|	НачисленияИУдержания.ПериодДействия КАК ПериодДействия,
		|	НачисленияИУдержания.Регистратор КАК Регистратор,
		|	НачисленияИУдержания.ПодразделениеНаКонецПериода КАК ПодразделениеНаКонецПериода,
		|	НачисленияИУдержания.ВидЗанятостиНаКонецПериода КАК ВидЗанятостиНаКонецПериода,
		|	НачисленияИУдержания.ДолжностьНаКонецПериода КАК ДолжностьНаКонецПериода,
		|	НачисленияИУдержания.Должность КАК Должность,
		|	НачисленияИУдержания.ТарифнаяСтавкаНаКонецПериода КАК ТарифнаяСтавкаНаКонецПериода,
		|	НачисленияИУдержания.ТарифнаяСтавка КАК ТарифнаяСтавка,
		|	НачисленияИУдержания.ВидЗанятости КАК ВидЗанятости,
		|	NULL КАК Показатель,
		|	0 КАК Значение,
		|	NULL КАК ПорядокПоказателей,
		|	ИСТИНА КАК ПервыйПоказатель,
		|	НачисленияИУдержания.ДатаПриема КАК ДатаПриема,
		|	НачисленияИУдержания.ДатаУвольнения КАК ДатаУвольнения,
		|	НачисленияИУдержания.ДокументОснование КАК ДокументОснование,
		|	НачисленияИУдержания.ИдентификаторСтроки КАК ИдентификаторСтроки,
		|	НачисленияИУдержания.Сторно КАК Сторно,
		|	НачисленияИУдержания.ТерриторияНачисления КАК ТерриторияНачисления,
		|	НачисленияИУдержания.НачислениеТекущегоПериода КАК НачислениеТекущегоПериода,
		|	НачисленияИУдержания.ПриоритетВидаРасчета КАК ПриоритетВидаРасчета
		|ПОМЕСТИТЬ ВТНачисленияИУдержанияСКадровымиДанными
		|ИЗ
		|	ВТОбъединениеНачисленияИУдержанияСКадровымиДанными КАК НачисленияИУдержания
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ РАЗРЕШЕННЫЕ
		...
		// ЗУПК
		ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "&УдержанияЗУПК", "ЗНАЧЕНИЕ(Перечисление.ВидыОсобыхНачисленийИУдержаний.ИПН),
																   |ЗНАЧЕНИЕ(Перечисление.ВидыОсобыхНачисленийИУдержаний.ОПВ),
																   |ЗНАЧЕНИЕ(Перечисление.ВидыОсобыхНачисленийИУдержаний.ВОСМС),
																   |ЗНАЧЕНИЕ(Перечисление.ВидыОсобыхНачисленийИУдержаний.ЕП)");
		// ЗУПК
