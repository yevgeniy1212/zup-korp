...
Функция ФизическоеЛицоПоСНИЛСИлиФИО(СНИЛС, Фамилия, Имя, Отчество) Экспорт
	Результат = Новый Структура("ФизическоеЛицо, ФИО, ТекстОшибки", Неопределено, "", "");
...
Функция ИмяСлужебное(Знач Имя)
	...
	Если ЗначениеЗаполнено(Имя)
		И (Имя = "НАТАЛЬЯ" Или Имя = "НАТАЛИЯ") Тогда
...
Процедура ЗаполнитьДанныеДляЭлектронногоТестированияКандидата(ДанныеДляЗаполнения, Учащийся) Экспорт
	...
	ФизическиеЛицаЗарплатаКадрыВнутренний.ЗаполнитьДанныеДляЭлектронногоТестированияКандидата(ДанныеДляЗаполнения,
		Учащийся);
...
Процедура ЗаписатьСвязанныеОбъекты(ФизическоеЛицо) Экспорт
	...
	ФизическиеЛицаЗарплатаКадрыВнутренний.ЗаписатьСвязанныеОбъекты(ФизическоеЛицо);
...
Процедура ЗаполнитьФизическоеЛицоПоКандидату(Форма) Экспорт
	...
	ФизическиеЛицаЗарплатаКадрыВнутренний.ЗаполнитьФизическоеЛицоПоКандидату(Форма);
...
Процедура ЗарегистрироватьОбработчикиОбновления(Обработчики) Экспорт
	...
	Обработчик.Идентификатор   = Новый УникальныйИдентификатор("6cf8c69e-9d0d-11eb-80f1-4cedfb43b11a");
	Обработчик.Процедура       = "ФизическиеЛицаЗарплатаКадры.ЗаполнитьКонтактнуюИнформациюJSON";
	Обработчик.Комментарий     = НСтр("ru = 'Заполнение контактной информации физических лиц в формате JSON из формата XML.'");
	
КонецПроцедуры

// Заполнение контактной информации физических лиц в формате JSON из других форматов.
Процедура ЗаполнитьКонтактнуюИнформациюJSON(ПараметрыОбновления = Неопределено) Экспорт
	ОбработкаЗавершена   = Истина;
	ОбработчикОтложенный = (ПараметрыОбновления <> Неопределено);
	ОшибкиКратко         = Новый Массив;
	ОшибкиПодробно       = Новый Массив;
	КоличествоОшибок     = 0;
	
	МетаданныеСправочника = Метаданные.Справочники.ФизическиеЛица;
	ПолеБлокировки        = "Ссылка";
	ТаблицаБлокировки     = МетаданныеСправочника.ПолноеИмя();
	
	РазмерПорции = ?(ОбработчикОтложенный, 1000, Неопределено);
	МассивСсылок = УправлениеКонтактнойИнформацией.ОбъектыТребующиеОбновленияКонтактнойИнформации(МетаданныеСправочника, РазмерПорции);
	...
	Для Каждого Ссылка Из МассивСсылок Цикл
		...
		Попытка
			ОбъектИзменен = УправлениеКонтактнойИнформацией.ОбновитьКонтактнуюИнформациюОбъекта(СправочникОбъект);
		Исключение
			ОбработкаЗавершена = Ложь;
			ОбъектИзменен = Ложь;
			...
			ДанныеXML = ОбщегоНазначения.ЗначениеВСтрокуXML(Ссылка.КонтактнаяИнформация.Выгрузить());
			
			ЗаголовокОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Ошибка при вызове функции %1 для физического лица %2: %3.'"),
				"УправлениеКонтактнойИнформацией.ОбновитьКонтактнуюИнформациюОбъекта",
				Строка(Ссылка),
				Кратко);
			...
			ЗаписьЖурналаРегистрации(
				НСтр("ru = 'Физические лица'", ОбщегоНазначения.КодОсновногоЯзыка()),
				УровеньЖурналаРегистрации.Ошибка,
				Ссылка.Метаданные(),
				Ссылка,
				ПолныйТекстОшибки);
			
			Если ОшибкиКратко.Найти(Кратко) = Неопределено Тогда
				ОшибкиКратко.Добавить(Кратко);
				ОшибкиПодробно.Добавить(ЗаголовокОшибки);
			КонецЕсли;
		КонецПопытки;
	...
	Если КоличествоОшибок > 0 Тогда
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'При обновлении контактной информации физических лиц (%1) возникли ошибки: %2.'"),
			Формат(КоличествоОшибок, "ЧГ="),
			Символы.ПС + Символы.Таб + СтрСоединить(ОшибкиКратко, ";" + Символы.ПС + Символы.Таб));
...
// 	ТолькоДействующиеПользователи 	- Булево
// 	
// Возвращаемое значение:
// 	СправочникСсылка.Пользователи, Неопределено - пользователь.
//
Функция ПользовательФизическогоЛица(ФизическоеЛицо, ТолькоДействующиеПользователи = Ложь) Экспорт
	...
	Запрос.Текст = 
		...
		|	И &ОтборДействующихПользователей
		|
		|УПОРЯДОЧИТЬ ПО
		|	Приоритет";
	
	Если ТолькоДействующиеПользователи Тогда
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "&ОтборДействующихПользователей",
			"НЕ Пользователи.ПометкаУдаления
			|И НЕ Пользователи.Недействителен
			|И НЕ Пользователи.Служебный");
	Иначе
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "&ОтборДействующихПользователей", "(ИСТИНА)");
	КонецЕсли;
