...
Процедура Печать(МассивОбъектов, ПараметрыПечати, КоллекцияПечатныхФорм, ОбъектыПечати, ПараметрыВывода) Экспорт
	...
	// ЗУПК
	ИначеЕсли УправлениеПечатью.НужноПечататьМакет(КоллекцияПечатныхФорм, "ПФ_MXL_Т1_ПоПравиламВеденияДО") Тогда
		ПечатьТ1(МассивОбъектов, ПараметрыПечати, КоллекцияПечатныхФорм, ОбъектыПечати, ПараметрыВывода);
	ИначеЕсли УправлениеПечатью.НужноПечататьМакет(КоллекцияПечатныхФорм, "ПФ_MXL_Т8_ПоПравиламВеденияДО") Тогда
		ПечатьТ8(МассивОбъектов, ПараметрыПечати, КоллекцияПечатныхФорм, ОбъектыПечати, ПараметрыВывода);
	// ЗУПК
	КонецЕсли;	
...
Функция ДанныеТрудовыхДоговоров(МассивОбъектов, ДистанционныеРаботы, ДоговорСНадомником)
	...
	Если Не РезультатЗапроса.Пустой() Тогда
		...
		ТекстИсключения = СтрШаблон(НСтр("ru='Сотрудник %1 не принят на работу'"), Выборка.Сотрудник);
	...
	Запрос.Текст =
		...
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	ДанныеПриказовОПриеме.Ссылка КАК СсылкаНаОбъект,
	...
	Если Не ПолучитьФункциональнуюОпцию("ИспользоватьКадровыйУчет") Тогда
		Запрос.Текст = СтрЗаменить(Запрос.Текст,
			"ДанныеПриказовОПриеме.Ссылка КАК СсылкаНаОбъект,",
			"КадровыеДанныеСотрудников.ФизическоеЛицо КАК СсылкаНаОбъект,");
	КонецЕсли;
	...
	Пока Выборка.Следующий() Цикл
		...
		Если ЗначениеЗаполнено(Выборка.РаботаТрудовойДоговорРуководительФамилияИмяОтчествоВРодительномПадеже) Тогда
			...
			Если ФизическиеЛицаЗарплатаКадры.Просклонять(Выборка.РаботаТрудовойДоговорРуководительФамилияИмяОтчествоВРодительномПадеже, 2,
				РезультатСклонения, Выборка.РаботаТрудовойДоговорРуководительПол) Тогда
...
// ЗУПК
	...
	Запрос.Текст =
		...
		// ЗУПК
		|	ПриемНаРаботу.РежимДистанционнойРаботы,
		// ЗУПК
...
Процедура УстановитьНомерПодпунктаВПараметрахНумерации(ПараметрыНумерации, Знач НомерПодпункта)
	...
	ПараметрыНумерации.Вставить("НомерПодпунктаБуквой", Сред("абвгдежзиклмнопрстуфхцчшщ", НомерПодпункта, 1)); // АПК:1036
...
Процедура ПечатьТ1ПоСправочнику(МассивОбъектов, ПараметрыПечати, КоллекцияПечатныхФорм, ОбъектыПечати, ПараметрыВывода)
	...
	Сведения = Новый СписокЗначений;
	// АПК:1036-выкл Орфографию для показателей сведений об организации БРО проверять не нужно
	...
	Запрос.Текст =
		...
		|	КадровыеДанныеСотрудников.Организация.НаименованиеСокращенное КАК РаботаОрганизацияНаименованиеСокращенное,
		|	КадровыеДанныеСотрудников.Организация.КодПоОКПО КАК РаботаОрганизацияКодПоОКПО,
...
Процедура ПечатьТ5ПоСправочнику(МассивОбъектов, ПараметрыПечати, КоллекцияПечатныхФорм, ОбъектыПечати, ПараметрыВывода) Экспорт
	...
	Запрос.Текст =
		...
		|	КадровыеДанныеСотрудников.ФизическоеЛицо КАК СсылкаНаОбъект,
		|	КадровыеДанныеСотрудников.Сотрудник КАК РаботаСотрудник,
		|	КадровыеДанныеСотрудников.Организация.НаименованиеСокращенное КАК РаботаОрганизацияПрежняяНаименованиеСокращенное,
		|	КадровыеДанныеСотрудников.Организация.КодПоОКПО КАК РаботаОрганизацияПрежняяКодПоОКПО,
		|	КадровыеДанныеСотрудников.ДатаПриема КАК РаботаДатаПриема,
		|	КадровыеДанныеСотрудников.ФИОПолные КАК ЛичныеДанныеФИОПолные,
		|	КадровыеДанныеСотрудников.ТабельныйНомер КАК РаботаСотрудникКод,
		|	КадровыеДанныеСотрудников.Подразделение КАК РаботаПодразделение,
...
Процедура ПечатьТ8ПоСправочнику(МассивОбъектов, ПараметрыПечати, КоллекцияПечатныхФорм, ОбъектыПечати, ПараметрыВывода) Экспорт
	...
	ДокументРезультат = Новый ТабличныйДокумент;
	Если ДанныеСсылок.Строки.Количество() > 0 Тогда
		КадровыйУчет.ВывестиНаПечатьТ8(ПараметрыПечати, ДокументРезультат, ДанныеСсылок.Строки, УправлениеПечатью.МакетПечатнойФормы("ОбщийМакет.ПФ_MXL_Т8"), , ОбъектыПечати);
	КонецЕсли;
