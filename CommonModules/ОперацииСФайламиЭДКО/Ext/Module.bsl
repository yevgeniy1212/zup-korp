...
// Структуру параметра Параметры см в ОперацииСФайламиЭДКОКлиентСервер.ТребованияКСканам()
...
Функция НужноПреобразоватьФормат(ДопустимыеРасширенияФайлов, Расширение) Экспорт
...
Процедура ПроверитьФормат(ОписаниеФайла, ДопустимыеРасширенияФайлов, ПараметрыПреобразования)
	...
	Если НужноПреобразоватьФормат(ДопустимыеРасширенияФайлов, Расширение) И НЕ ПараметрыПреобразования.РазрешатьДругиеТипы Тогда
		...
		ТекстОшибки = НСтр("ru = 'Файл формата %1 не удалось преобразовать к формату, определенному требованиями (%2)'");
