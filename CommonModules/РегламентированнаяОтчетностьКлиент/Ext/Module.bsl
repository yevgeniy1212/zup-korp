...
Процедура ВыбратьСтатусОтправкиИзСписка(ТекущиеДанные, ТекСтатус, СсылкаНаОбъект, ЭтоОтчет, Форма) Экспорт
	
	ДопПараметры = Новый Структура;
	ДопПараметры.Вставить("Организация",		 ТекущиеДанные.Организация);
	ДопПараметры.Вставить("КонтролирующийОрган", ТекущиеДанные.ВидКонтролирующегоОргана);
	ДопПараметры.Вставить("СсылкаНаОбъект",		 СсылкаНаОбъект);
	ДопПараметры.Вставить("ЭтоОтчет",		 	 ЭтоОтчет);
...
Процедура ОтправитьОтчетИзФормыСписка(Форма, ИмяКоманды, КонтекстЭДОКлиент, ТекущиеДанные) Экспорт
	...
	Если ЗначениеЗаполнено(Форма.ИмяТипаДокументаРеестрДанныхЭЛН)
	...
	Если ТекущиеДанныеНаименованиеОтчета = "Уведомление о розничных ценах на табачные изделия"
		...
		Если ОписаниеРегОтчета.ВыбраннаяФорма = "ФормаОтчета2022Кв1"
			И НЕ ОнлайнСервисыРегламентированнойОтчетностиВызовСервера.СобытиеНаступило("Отпр.розн.цен табак 22к1") Тогда
			ПоказатьИВернутьОшибку(Форма, НСтр(
	...
	Если ТекущиеДанныеНаименованиеОтчета =
		"Реестр для налоговых вычетов по акцизам на этиловый спирт (денатурированный этиловый спирт)"
	...
	Если ТекущиеДанныеНаименованиеОтчета = "Расчет доли доходов от пассивной деятельности"
		И НЕ ОнлайнСервисыРегламентированнойОтчетностиВызовСервера.СобытиеНаступило("Отпр.дох.от пассивн.деят.") Тогда
		ПоказатьИВернутьОшибку(Форма,
			НСтр("ru = 'Отправка в электронном виде формы отчета не предусмотрена
					   |законодательством. Форма сдается в бумажном виде'"));
		Возврат;
...
Процедура ПоказатьПечатныйБланкРегламентированногоОтчетаСЭЦП(Форма, ПараметрыФормы)
	...
	ОписаниеОповещения = Новый ОписаниеОповещения(
		"ПоказатьПечатныйБланкРегламентированногоОтчетаСЭЦППродолжение", ЭтотОбъект, ДополнительныеПараметры);
	ДокументооборотСКОКлиент.ПолучитьКонтекстЭДО(ОписаниеОповещения);
	
КонецПроцедуры

Процедура ПоказатьПечатныйБланкРегламентированногоОтчетаСЭЦППродолжение(Результат, ДополнительныеПараметры) Экспорт
	...
	ЦиклОбмена = Форма.СтруктураРеквизитовФормы.ПоследнийЦиклОбмена;
	...
	ДополнительныеПараметры.Вставить("ЦиклОбмена", ЦиклОбмена);
	ДополнительныеПараметры.Вставить("НастройкиПечати", НастройкиПечати);
	ДополнительныеПараметры.Вставить("КонтекстЭДОКлиент", КонтекстЭДОКлиент);
	
	ОписаниеОповещения = Новый ОписаниеОповещения(
		"ПоказатьПечатныйБланкРегламентированногоОтчетаСЭЦППроверкаРезультата", ЭтотОбъект, ДополнительныеПараметры);
	
	КонтекстЭДОКлиент.СформироватьПечатныеДокументы(ЦиклОбмена, НастройкиПечати, ОписаниеОповещения);
	
КонецПроцедуры

Процедура ПоказатьПечатныйБланкРегламентированногоОтчетаСЭЦППроверкаРезультата(Результат, ДополнительныеПараметры) Экспорт
	
	Форма = ДополнительныеПараметры.Форма;
	ПараметрыФормы = ДополнительныеПараметры.ПараметрыФормы;
	НастройкиПечати = ДополнительныеПараметры.НастройкиПечати;
	...
	Если Результат.Количество() = 0 Тогда
		...
		ОписаниеОповещения = Новый ОписаниеОповещения(
			"ПоказатьПечатныйБланкРегламентированногоОтчетаСЭЦПЗавершение", ЭтотОбъект, ДополнительныеПараметры);
		
		ЦиклОбмена = ДополнительныеПараметры.ЦиклОбмена;
		КонтекстЭДОКлиент.СформироватьПечатныеДокументы(ЦиклОбмена, НастройкиПечати, ОписаниеОповещения);
		
		Возврат;
	КонецЕсли;
	
	ПоказатьПечатныйБланкРегламентированногоОтчетаСЭЦПЗавершение(Результат, ДополнительныеПараметры);
	
КонецПроцедуры

Процедура ПоказатьПечатныйБланкРегламентированногоОтчетаСЭЦПЗавершение(Результат, ДополнительныеПараметры) Экспорт
...
Процедура ПредставлениеЭталонаФорматаОбласти(ДополнительныеПараметры) Экспорт
	...
	Если Область.СодержитЗначение И Область.ТипЗначения.СодержитТип(Тип("Число")) Тогда
		Если Область.Значение <> 0 Тогда
			...
			Если СтрНайти(ИмяМакета, "_") = 0 Тогда
				НаимТекущегоРаздела = Неопределено;
				Форма.СтруктураРеквизитовФормы.Свойство("НаимТекущегоРаздела", НаимТекущегоРаздела);
			...
			ФорматЭталон =
				РегламентированнаяОтчетностьВызовСервера.ПолучитьЭталонФорматаОбласти(Форма.ИмяФормы, ИмяМакета, Область.Имя);
			
			Если ФорматЭталон <> Неопределено И (Область.Формат <> ФорматЭталон) Тогда
				Область.Формат = ФорматЭталон;
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
КонецПроцедуры
...
Процедура ПроверитьКонтрольныеСоотношения_СКП_Завершение(Результат, ДополнительныеПараметры) Экспорт
	...
	ИначеЕсли РезультатПроверки.ЧислоОшибок = 0 Тогда
		...
		ПоказатьПредупреждение(, НСтр("ru='При проверке контрольных соотношений ошибки не обнаружены'"), 5);
		Возврат;
...
Процедура РазрешитьРедактированиеРеквизитовОтчетаПослеПодтверждения(Ответ, Параметры) Экспорт
	...
	Если ОбщегоНазначенияКлиент.ПодсистемаСуществует("СтандартныеПодсистемы.ЗапретРедактированияРеквизитовОбъектов") Тогда
		...
		Если ЭлементНаименованиеЭтапа <> Неопределено И ЭлементНаименованиеЭтапа.Заголовок = НСтр("ru = 'Сдано'") Тогда
			ЭлементНаименованиеЭтапа.Заголовок = НСтр("ru = 'Сдан, модифицирован'"); //статус будет сохранен при записи отчета.
		КонецЕсли;
		...
		ПоказатьОповещениеПользователя(НСтр("ru = 'Установка разрешения редактирования отчета'"), ,ТекстСообщения);
