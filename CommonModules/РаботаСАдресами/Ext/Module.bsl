...
Функция РегионАдресаКонтактнойИнформации(Знач Адрес) Экспорт
	...
	ИначеЕсли ТипЗнч(Адрес) <> Тип("Структура") Тогда
        ...
        // ЗУПК
		ВызватьИсключение НСтр("ru = 'Невозможно определить субъект, ожидается адрес.'");
        // ЗУПК
...
Процедура ПриДобавленииКлассификаторов(Классификаторы) Экспорт
	...
	// ЗУПК
	Описание.Наименование               = НСтр("ru = 'Классификатор стран мира'");
	// ЗУПК
...
Процедура ПриНачальномЗаполненииЭлементов(КодыЯзыков, Элементы, ТабличныеЧасти) Экспорт
    ...
    // ЗУПК
	Элемент = Элементы.Добавить();
	Элемент.ИмяПредопределенныхДанных = "Казахстан";
	Элемент.Код                       = "398";
	Элемент.Наименование              = НСтр("ru = 'КАЗАХСТАН'", ОбщегоНазначения.КодОсновногоЯзыка());
	Элемент.НаименованиеПолное        = НСтр("ru = 'Республика Казахстан'", ОбщегоНазначения.КодОсновногоЯзыка());
	Элемент.КодАльфа2                 = "KZ";
	Элемент.КодАльфа3                 = "KAZ";
	Элемент.УчастникЕАЭС              = Истина;
	Элемент.МеждународноеНаименование = НСтр("ru = 'Republic of Kazakhstan'", ОбщегоНазначения.КодОсновногоЯзыка());
    // ЗУПК
...
Функция НазванияГородовФедеральногоЗначения()
    ...
    // ЗУПК
	СписокГородов.Добавить("АСТАНА");
	СписокГородов.Добавить("АЛМАТЫ");
	СписокГородов.Добавить("ШЫМКЕНТ");
    // ЗУПК
...
Функция ТаблицаКлассификатора() Экспорт
	...
	Иначе
		...
		Иначе
			...
			ТаблицаСтран.Колонки.Добавить("Код", Новый ОписаниеТипов("Строка", ,Новый КвалификаторыСтроки(3)));
			ТаблицаСтран.Колонки.Добавить("Наименование", Новый ОписаниеТипов("Строка", ,Новый КвалификаторыСтроки(60)));
			ТаблицаСтран.Колонки.Добавить("НаименованиеПолное", Новый ОписаниеТипов("Строка", ,Новый КвалификаторыСтроки(100)));
			ТаблицаСтран.Колонки.Добавить("КодАльфа2", Новый ОписаниеТипов("Строка", ,Новый КвалификаторыСтроки(2)));
			ТаблицаСтран.Колонки.Добавить("КодАльфа3", Новый ОписаниеТипов("Строка", ,Новый КвалификаторыСтроки(3)));
			ТаблицаСтран.Колонки.Добавить("УчастникЕАЭС", Новый ОписаниеТипов("Булево"));
			ТаблицаСтран.Колонки.Добавить("МеждународноеНаименование", Новый ОписаниеТипов("Строка", ,Новый КвалификаторыСтроки(100)));
			ТаблицаСтран.Колонки.Добавить("Неактуальный", Новый ОписаниеТипов("Булево"));
			
			Для Каждого СтрокаСтрана Из КлассификаторJSON Цикл
				
				СтрокаТаблицаСтран = ТаблицаСтран.Добавить();
				
				СтрокаТаблицаСтран.Код						 = ?(СтрокаСтрана.Свойство("countryCode"),
																 СтрокаСтрана.countryCode, "");
				СтрокаТаблицаСтран.Наименование				 = ?(СтрокаСтрана.Свойство("countryName"),
																 СтрокаСтрана.countryName, "");
				СтрокаТаблицаСтран.НаименованиеПолное		 = ?(СтрокаСтрана.Свойство("countryFullName"),
																 СтрокаСтрана.countryFullName, "");
				СтрокаТаблицаСтран.КодАльфа2				 = ?(СтрокаСтрана.Свойство("alphaCode02"),
																 СтрокаСтрана.alphaCode02, "");
				СтрокаТаблицаСтран.КодАльфа3				 = ?(СтрокаСтрана.Свойство("alphaCode03"),
																 СтрокаСтрана.alphaCode03, "");
				СтрокаТаблицаСтран.УчастникЕАЭС				 = ?(СтрокаСтрана.Свойство("memberEAEU"),
																СтрокаСтрана.memberEAEU, Ложь);
				СтрокаТаблицаСтран.МеждународноеНаименование = "";
				СтрокаТаблицаСтран.Неактуальный = Ложь;
			КонецЦикла;
