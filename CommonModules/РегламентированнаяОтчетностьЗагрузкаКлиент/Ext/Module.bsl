...
Процедура ЗагрузитьИзФайлаОтчет(Форма) Экспорт
	...
	КнопкаЗагрузить = Форма.Элементы.Найти(
		?(ЭтоФормаРегламентированногоОтчета(Форма), "Загрузить", "ОтчетыЗагрузитьИзФайлаВФормуОтчета"));
	Если КнопкаЗагрузить <> Неопределено И КнопкаЗагрузить.Доступность <> Ложь Тогда
		КнопкаЗагрузить.Доступность = Ложь; // доступность будет включена после закрытия диалога выбора файлов
	Иначе
		Возврат; // для исключения повторных вызовов при множественных нажатиях на кнопку
	КонецЕсли;
	
	ДополнительныеПараметрыОповещения = Новый Структура;
	ДополнительныеПараметрыОповещения.Вставить("Форма", Форма);
	ДополнительныеПараметрыОповещения.Вставить("КнопкаЗагрузить", КнопкаЗагрузить);
...
Процедура ЗагрузитьИзФайлаОтчетПродолжение(Результат, ДополнительныеПараметры) Экспорт
	...
	Если ДополнительныеПараметры.КнопкаЗагрузить <> Неопределено Тогда
		ДополнительныеПараметры.КнопкаЗагрузить.Доступность = Истина;
	КонецЕсли;
...
Процедура ПолучениеФайловДляЗагрузкиПослеВыбораФайлов(ВыбранныеФайлы, ДополнительныеПараметры) Экспорт
	...
	Иначе
		...
		Если ДополнительныеПараметры.Свойство("ОповещениеОЗакрытииФормы", ОповещениеОЗакрытииФормы) Тогда
			ДополнительныеПараметрыОповещения = ОповещениеОЗакрытииФормы.ДополнительныеПараметры;
			Если ДополнительныеПараметрыОповещения.КнопкаЗагрузить <> Неопределено Тогда
				ДополнительныеПараметрыОповещения.КнопкаЗагрузить.Доступность = Истина;
			КонецЕсли;
		КонецЕсли;
...
Процедура ПолучениеФайловДляЗагрузкиЗавершение(ФайлПолучен, ВходящийКонтекст) Экспорт
	...
	Если ФайлПолучен Тогда
		...
		Иначе
			...
			ОткрытьФорму("ОбщаяФорма.ЗагрузкаФайловОтчета", ПараметрыФормы, ВладелецФормы, Ложь, , 
...
#Область КопированиеОтчетовРазличающихсяРедакций

Процедура ВыполнитьПреобразованиеИзПрошлойРедакции(ФормаОтчета, РезультатВопроса) Экспорт
	Перем ИмяФормыОтчета;
	Перем СохраненныйДок;
	Перем ПредставлениеДокументаРеглОтч;
	
	Если РезультатВопроса <> КодВозвратаДиалога.Да Тогда
		ОбщегоНазначенияКлиент.СообщитьПользователю("Данные отчета не были скопированы, создан новый пустой отчет");
		ФормаОтчета.Обновить(Неопределено);
		Возврат;
	КонецЕсли;
	
	СтруктураРеквизитовФормы = ФормаОтчета.СтруктураРеквизитовФормы;
	ИмяФормыДляВыгрузки = СтрЗаменить(ФормаОтчета.ИмяФормы, 
		СтруктураРеквизитовФормы.мВыбраннаяФорма, 
		СтруктураРеквизитовФормы.мСкопированаФорма);
	ПараметрыФормыДляВыгрузки = Новый Структура;
	ПараметрыФормыДляВыгрузки.Вставить("БезОткрытияФормы", Истина);
	ПараметрыФормыДляВыгрузки.Вставить("мСохраненныйДок", СтруктураРеквизитовФормы.мСкопированнаяФормаДругойРедакции);
	ПараметрыФормыДляВыгрузки.Вставить("мДатаНачалаПериодаОтчета", СтруктураРеквизитовФормы.ДатаНачалаСкопированногоОтчета);
	ПараметрыФормыДляВыгрузки.Вставить("мДатаКонцаПериодаОтчета", СтруктураРеквизитовФормы.ДатаКонцаСкопированногоОтчета);
	ФормаДляВыгрузки = ПолучитьФорму(ИмяФормыДляВыгрузки, ПараметрыФормыДляВыгрузки, ФормаОтчета);
	
	Если ФормаДляВыгрузки.СтруктураРеквизитовФормы.Свойство("мПроверятьСоотношенияПриПечатиИВыгрузки") Тогда 
		ФормаДляВыгрузки.СтруктураРеквизитовФормы.мПроверятьСоотношенияПриПечатиИВыгрузки = Ложь;
	КонецЕсли;
	
	ТекстВыгрузки = Неопределено;
	РезультатВыгрузки = ФормаДляВыгрузки.УниверсальнаяВыгрузка(ТекстВыгрузки, Ложь, Ложь);
	Если РезультатВыгрузки <> Истина Тогда 
		ОбщегоНазначенияКлиент.СообщитьПользователю("Не удалось сформировать текст выгрузки отчета прошлой редакции, копирование невозможно");
		Возврат;
	КонецЕсли;
	
	Если ТипЗнч(ТекстВыгрузки) <> Тип("Строка")  
			И ТипЗнч(ТекстВыгрузки) <> Тип("Массив") Тогда 
		ОбщегоНазначенияКлиент.СообщитьПользователю("Не удалось распознать текст выгрузки отчета прошлой редакции, копирование невозможно");
		Возврат;
	КонецЕсли;
	
	ДанныеДляЗагрузки = Новый Структура;
	ДанныеДляЗагрузки.Вставить("СвойстваОсновногоФайла", Новый Структура);
	ДанныеДляЗагрузки.Вставить("Организация", СтруктураРеквизитовФормы.Организация);
	ДанныеДляЗагрузки.Вставить("КопированиеОтчета", Истина);
	ДанныеДляЗагрузки.Вставить("ОбнулятьОкругления", Истина);
	СтруктураСерверныхПараметров = Новый Структура;
	СтруктураСерверныхПараметров.Вставить("ТекстВыгрузки", ТекстВыгрузки);
	СтруктураСерверныхПараметров.Вставить("ИмяФормы", ФормаОтчета.ИмяФормы);
	СтруктураСерверныхПараметров.Вставить("СтруктураРеквизитовФормы", СтруктураРеквизитовФормы);
	РегламентированнаяОтчетностьВызовСервера.ДополнитьСерверныеПараметры(ДанныеДляЗагрузки, СтруктураСерверныхПараметров);
	...
	СохраненныйДок = СтруктураРеквизитовФормы.мСкопированнаяФормаДругойРедакции;
	ПараметрыФормыИзСохраненногоОтчета = РегламентированнаяОтчетностьВызовСервера.ПолучитьПараметрыФормыИзСохраненногоОтчета(СохраненныйДок, ПредставлениеДокументаРеглОтч);
	ЗаполнитьЗначенияСвойств(ПараметрыФормыИзСохраненногоОтчета, СтруктураРеквизитовФормы);
	ЗаполнитьЗначенияСвойств(ПараметрыФормыОтчета, ПараметрыФормыИзСохраненногоОтчета);
	ПараметрыФормыОтчета.Вставить("мФормаОтчета", ПараметрыФормыИзСохраненногоОтчета.мВыбраннаяФорма);
	
	ФормаОтчета.Комментарий = НСтр("ru = 'Данные отчета сконвертированы из другой редакции.'");
	Если СтруктураРеквизитовФормы.Свойство("ФлажокОтклАвтоРасчет") Тогда
		СтруктураРеквизитовФормы.ФлажокОтклАвтоРасчет = Истина;
		Если ДанныеДляЗагрузки.ВидОтчета = ПредопределенноеЗначение("Справочник.ВидыОтправляемыхДокументов.НДС") Тогда
			РегламентированнаяОтчетностьКлиентСервер.ИзменитьТаблицуВариантовЗаполнения(ФормаОтчета, Ложь);
		КонецЕсли;
	КонецЕсли;
	...
	Если ТипЗнч(СвойстваФормыОтчета.НомерКорректировки) = Тип("УникальныйИдентификатор") Тогда
		ФормаОтчета.СтруктураРеквизитовФормы.мВариант = 0;
		ФормаОтчета.СохранитьНаКлиенте(Истина);
	Иначе
		ФормаОтчета.НомерКорректировки = 999;
		СтруктураРеквизитовФормы.ВидДокумента = -1;
		СтруктураРеквизитовФормы.мВариант = ФормаОтчета.СтруктураРеквизитовФормы.ВидДокумента * ФормаОтчета.НомерКорректировки;
		ФормаОтчета.СохранитьНаКлиенте(Истина);
	...
	РезультатЗагрузки = ФормаОтчета.ЗагрузкаОтчетаИзФайла(ДанныеДляЗагрузки);
	ДополнительныеПараметры = Новый Структура;
	ДополнительныеПараметры.Вставить("ФормаОтчета",          ФормаОтчета);
	ДополнительныеПараметры.Вставить("ДокументИсточник",     СохраненныйДок);
	ДополнительныеПараметры.Вставить("НовыйСохраненныйДок",  ФормаОтчета.СтруктураРеквизитовФормы.мСохраненныйДок);
	ДополнительныеПараметры.Вставить("ЭтоФормаРеглОтчета",   Ложь);
	ДополнительныеПараметры.Вставить("ПредставлениеОтчета",  ПредставлениеДокументаРеглОтч);
	ДополнительныеПараметры.Вставить("ПараметрыФормыОтчета", ПараметрыФормыОтчета);
	ДополнительныеПараметры.Вставить("ДанныеДляЗагрузки",    ДанныеДляЗагрузки);
	
	ОповещениеОЗавершении = Новый ОписаниеОповещения("ВыполнитьПреобразованиеИзПрошлойРедакцииЗавершение", ЭтотОбъект, ДополнительныеПараметры);
	РезультатДлительнойОперации = Неопределено;
	Если РезультатЗагрузки И ДанныеДляЗагрузки.Свойство("РезультатДлительнойОперации", РезультатДлительнойОперации) Тогда
		ПараметрыОжидания = ДлительныеОперацииКлиент.ПараметрыОжидания(ФормаОтчета);
		ПараметрыОжидания.ТекстСообщения = НСтр("ru = 'Выполняется конвертация отчета из другой редакции'");
		ПараметрыОжидания.ВыводитьПрогрессВыполнения = Истина;
...
Процедура ВыполнитьПреобразованиеИзПрошлойРедакцииЗавершение(РезультатЗагрузки, ДополнительныеПараметры) Экспорт
	...
	Если РезультатЗагрузки = Неопределено
		...
		ФормаОтчета.Модифицированность = Ложь;
		ФормаОтчета.Обновить(Неопределено);
		ФормаОтчета.СтруктураРеквизитовФормы.мСохраненныйДок = Неопределено;
		РегламентированнаяОтчетностьВызовСервера.УдалитьСохраненныйДокумент(ДополнительныеПараметры.НовыйСохраненныйДок);
		ОповеститьОбИзменении(Тип("РегистрСведенийКлючЗаписи.ЖурналОтчетовСтатусы"));
		ФормаОтчета.Модифицированность = Ложь;
		Возврат;
	КонецЕсли;
	...
	ПараметрыФормыОтчета = ДополнительныеПараметры.ПараметрыФормыОтчета;
	ДанныеДляЗагрузки    = ДополнительныеПараметры.ДанныеДляЗагрузки;
	ДокументИсточник     = ДополнительныеПараметры.ДокументИсточник;
	СтатусЗагрузки       = ПолучитьИзВременногоХранилища(РезультатЗагрузки.АдресРезультата);
	...
	Если СтатусЗагрузки Тогда
		...
		ПараметрыФормыОтчета.Вставить("БезОткрытияФормы", Ложь);
		ПараметрыФормыОтчета.Вставить("ПредставлениеВидаОтчета", ПредставлениеОтчета);
		ПараметрыФормыОтчета.Вставить("ДокументИсточник",        ДокументИсточник);
		...
		ФормаОтчетаПослеЗагрузки.Модифицированность = Истина;
		ФормаОтчетаПослеЗагрузки.Открыть();
		ФормаОтчетаПослеЗагрузки.Обновить(Неопределено);
		ОбщегоНазначенияКлиент.СообщитьПользователю("Выполнено преобразование. Необходимо проверить корректность переноса данных");
	Иначе
		Если НЕ ЭтоОтчетФСРАР Тогда
			ФормаОтчетаПослеЗагрузки.СтруктураРеквизитовФормы.мСохраненныйДок = Неопределено;
		КонецЕсли;
