...
Процедура ЗаполнитьНазначаемыеМероприятия(Форма, ПараметрыЗаполнения, ОчиститьПередЗаполнением = Истина) Экспорт
	...
	Если Не ПолучитьФункциональнуюОпцию("ИспользоватьАдаптациюУвольнение") Тогда
		Возврат;
	КонецЕсли;
	...
	АдаптацияУвольнение.ЗаполнитьЗаданияВДанныхМероприятий(ОбъектСсылка, ДанныеМероприятий);
...
Процедура УстановитьВидимостьПредупрежденияПоОплатеТруда(Форма, ФОТ) Экспорт
	
	Если Не ПравоДоступа("Чтение", Метаданные.РегистрыСведений.ДанныеОбОплатеТрудаДляПодбораСотрудников) Тогда
		Возврат;
	КонецЕсли;
	...
	СогласованИнойДоход = Ложь;
	ЗначенияРеквизитов = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(Решение, "ВариантОплатыТруда, СогласованныйДоход");
	Если ЗначенияРеквизитов.ВариантОплатыТруда = Перечисления.ВариантыОплатыТруда.СогласованныйДоход
		И ЗначенияРеквизитов.СогласованныйДоход <> ФОТ
		И Элементы.ГруппаНачисленияИтогРазмер.Видимость Тогда
		СогласованИнойДоход = Истина;
	КонецЕсли;
	
	ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(
		Элементы, "ГруппаПредупреждениеПоОплатеТрудаГлавное", "Видимость", СогласованИнойДоход);
	
	ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(
		Элементы, "ГруппаПредупреждениеПоОплатеТруда", "Видимость", СогласованИнойДоход);
...
// Форма документа при получении данных.
// 
// Параметры:
...
Процедура РешениеУстановитьУсловноеОформлениеРезультатаВыполнения(УсловноеОформление) Экспорт
	
	УстановитьУсловноеОформлениеРезультатаВыполнения(УсловноеОформление, "НеНазначено", ЦветаСтиля.НедоступныеДанныеЦвет);
	УстановитьУсловноеОформлениеРезультатаВыполнения(УсловноеОформление, "Выполнено", ЦветаСтиля.ЦветАкцента);
	УстановитьУсловноеОформлениеРезультатаВыполнения(УсловноеОформление, "Просрочено", ЦветаСтиля.ПросроченныеДанныеЦвет);
	УстановитьУсловноеОформлениеРезультатаВыполнения(УсловноеОформление, "ОжидаетВыполнения", ЦветаСтиля.ЦветКалендаряОранжевый);
...
// Устарела. Следует использовать КадровыеРешенияФормы.ОбработкаПроверкиЗаполненияНаСервере
Процедура ОбработкаПроверкиЗаполненияНаСервере(Форма, Отказ, ПроверяемыеРеквизиты) Экспорт
	КадровыеРешенияФормы.ОбработкаПроверкиЗаполненияНаСервере(Форма, Отказ, ПроверяемыеРеквизиты);
КонецПроцедуры
...
Процедура ДополнитьФормуОбъектаНазначения(Форма, ЭтоФормаПозицииШР)
	...
	ДобавляемыеРеквизиты.Добавить(
		Новый РеквизитФормы("МероприятияАдаптации",
							Новый ОписаниеТипов("ТаблицаЗначений"),,
							НСтр("ru = 'Мероприятия адаптации'")));
	ДобавляемыеРеквизиты.Добавить(
		Новый РеквизитФормы("Мероприятие",
							Новый ОписаниеТипов("СправочникСсылка.МероприятияАдаптацииУвольнения"),
							"МероприятияАдаптации",
							НСтр("ru = 'Мероприятие'")));
	ДобавляемыеРеквизиты.Добавить(
		Новый РеквизитФормы("События",
							Новый ОписаниеТипов("СписокЗначений"),
							"МероприятияАдаптации",
							НСтр("ru = 'События'")));
	ДобавляемыеРеквизиты.Добавить(
		Новый РеквизитФормы("ПредставлениеСобытий",
							ОбщегоНазначения.ОписаниеТипаСтрока(100),
							"МероприятияАдаптации",
							НСтр("ru = 'Представление событий'")));
	
	ДобавляемыеРеквизиты.Добавить(
		Новый РеквизитФормы("МероприятияУвольнения",
							Новый ОписаниеТипов("ТаблицаЗначений"),,
							НСтр("ru = 'Мероприятия увольнения'")));
	ДобавляемыеРеквизиты.Добавить(
		Новый РеквизитФормы("Мероприятие",
							Новый ОписаниеТипов("СправочникСсылка.МероприятияАдаптацииУвольнения"),
							"МероприятияУвольнения",
							НСтр("ru = 'Мероприятие'")));
	ДобавляемыеРеквизиты.Добавить(
		Новый РеквизитФормы("События",
							Новый ОписаниеТипов("СписокЗначений"),
	...
	ДобавляемыеРеквизиты.Добавить(
		Новый РеквизитФормы("МероприятияПрежние",
							Новый ОписаниеТипов(),,
							НСтр("ru = 'Мероприятия прежние'")));
	ДобавляемыеРеквизиты.Добавить(
		Новый РеквизитФормы("МероприятиеПрежнее",
							Новый ОписаниеТипов("СправочникСсылка.МероприятияАдаптацииУвольнения"),,
							НСтр("ru = 'Мероприятие прежнее'")));
	
	Если ЭтоФормаПозицииШР Тогда
		ДобавляемыеРеквизиты.Добавить(
			Новый РеквизитФормы("Назначение",
...
Процедура УстановитьУсловноеОФормлениеРезультатаВыполнения(УсловноеОформление, КодРезультата, ЦветТекстаПоля)
	
	Элемент = УсловноеОформление.Элементы.Добавить();

	ПолеЭлемента = Элемент.Поля.Элементы.Добавить();
	ПолеЭлемента.Поле = Новый ПолеКомпоновкиДанных("НазначаемыеМероприятияРезультатВыполнения");
	
	ОтборЭлемента = Элемент.Отбор.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
	ОтборЭлемента.ЛевоеЗначение = Новый ПолеКомпоновкиДанных("Объект.МероприятияАдаптацииУвольнения.РезультатВыполненияКод");
	ОтборЭлемента.ВидСравнения = ВидСравненияКомпоновкиДанных.Равно;
	ОтборЭлемента.ПравоеЗначение = КодРезультата;
