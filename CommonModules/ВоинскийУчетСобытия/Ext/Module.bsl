...
Процедура ЗарегистрироватьИзменениеАдресаФизическогоЛицаПередЗаписью(Источник, Отказ) Экспорт
	...
	АдресМестаПроживания = ОбщегоНазначения.ПредопределенныйЭлемент("Справочник.ВидыКонтактнойИнформации.АдресМестаПроживанияФизическиеЛица");
	АдресПоПрописке = ОбщегоНазначения.ПредопределенныйЭлемент("Справочник.ВидыКонтактнойИнформации.АдресПоПропискеФизическиеЛица");
	
	ВидыАдресов = Новый Массив;
	ВидыАдресов.Добавить(АдресМестаПроживания);
	ВидыАдресов.Добавить(АдресПоПрописке);
	
	Запрос = Новый Запрос;
	
	Запрос.УстановитьПараметр("ФизическоеЛицо", ФизическоеЛицо);
	Запрос.УстановитьПараметр("Тип", Перечисления.ТипыКонтактнойИнформации.Адрес);
	Запрос.УстановитьПараметр("Вид", ВидыАдресов);
	...
	Для Каждого ТекущаяЗапись Из ТаблицаЗаписейОбъекта Цикл 
		Если ВидыАдресов.Найти(ТекущаяЗапись.Вид) = Неопределено Тогда 
			СтрокиКУдалению.Добавить(ТекущаяЗапись);
		КонецЕсли;
	КонецЦикла;
	
	Для Каждого СтрокаКУдалению Из СтрокиКУдалению Цикл 
		ТаблицаЗаписейОбъекта.Удалить(СтрокаКУдалению);
	КонецЦикла;
...
Процедура ЗарегистрироватьИзменениеОбразованияФизическогоЛицаПередЗаписью(Источник, Отказ, Замещение) Экспорт
	...
	Если СтрокаСсылки = Неопределено Тогда
		СтрокаСсылки = НоваяТаблицаЗаписей.Добавить();
		СтрокаСсылки.Ссылка = Источник.Ссылка;
		СтрокаСсылки.ФизическоеЛицо = ФизическоеЛицо;
	КонецЕсли;
