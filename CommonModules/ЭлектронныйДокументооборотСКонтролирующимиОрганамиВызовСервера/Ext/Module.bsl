...
Функция ЗаписатьСобытиеДокументооборотаСКОСЭДОФСС(Ссылка, НовоеЗначение)
	...
	Если МенеджерЗаписи.Выбран() Тогда
		...
		МенеджерЗаписи.Просмотрено	= НЕ НовоеЗначение;
...
Процедура ИзменитьЗначениеВФорме1СОтчетность(Ссылка, ИмяПоля, Знач НовоеЗначение) Экспорт
	...
	Попытка
		...
		ИначеЕсли ТипЗнч(Ссылка) = Тип("ДокументСсылка.ВходящийДокументСЭДОФСС") Тогда
			Регистр = "ЖурналНовыхСобытий";
		Иначе
		...
		Если Регистр = "ЖурналНовыхСобытий" Тогда
			Если ИмяПоля = "Скрыт" Тогда
				Для каждого ЗаписьНабора Из НаборЗаписей Цикл
					ЗаписьНабора[ИмяПоля] = НовоеЗначение;
				КонецЦикла;
			ИначеЕсли ИмяПоля = "Непрочтенное" Тогда
				Для каждого ЗаписьНабора Из НаборЗаписей Цикл
					ЗаписьНабора["Просмотрено"] = НЕ НовоеЗначение;
				КонецЦикла;
			КонецЕсли;
		Иначе
			Для каждого ЗаписьНабора Из НаборЗаписей Цикл
		...
		Если ТипЗнч(Ссылка) = Тип("ДокументСсылка.ВходящийДокументСЭДОФСС")
			...
			Если ИмяПоля = "Просмотрено" Тогда
				ЗаписатьСобытиеДокументооборотаСКОСЭДОФСС(Ссылка, НовоеЗначение);
			КонецЕсли;
		КонецЕсли;
...
Функция ЗначенияРеквизитовОбмена(ОрганизацияИлиУчетнаяЗапись, НастройкиПолучения = Неопределено) Экспорт
	...
	Если НастройкиПолученияРеквизитов.ПолучатьРеквизитыУчетнойЗаписи Тогда
		РеквизитыУчетнойЗаписи = ЭлектроннаяПодписьВМоделиСервисаБРОВызовСервера.РеквизитыУчетнойЗаписи(Результат.УчетнаяЗапись);
		Результат.ЭтоЭлектроннаяПодписьВМоделиСервиса = РеквизитыУчетнойЗаписи.ЭлектроннаяПодписьВМоделиСервиса;
		МестоХраненияКлюча = КриптографияЭДКОКлиентСервер.ОпределитьМестоХраненияКлюча(РеквизитыУчетнойЗаписи.МодельХраненияЗакрытогоКлюча, РеквизитыУчетнойЗаписи.УчетнаяЗаписьОблачнойПодписи);
