	...
	Пока ДанныеСотрудников.Следующий() Цикл
		...
		Если ЗначениеЗаполнено(ДанныеСотрудников.ДатаПриема)
			...
			Если ОбщегоНазначения.ПодсистемаСуществует("ЗарплатаКадрыПриложения.ЗарплатаКадрыДляНебольшихОрганизаций.РасчетЗарплаты") Тогда
				...
				МодульРасчетЗарплатыДляНебольшихОрганизаций.ДополнитьОтборНачислений(Отбор, ДокументПриемНаРаботу.Организация, ДокументПриемНаРаботу.ДатаПриема);
			КонецЕсли;
...
Процедура ДобавитьВКоллекциюКадровыеДанныеДляСведенийОКлассахУсловийТруда(ИсточникиДанных, ИмяПоля) Экспорт
...
Функция ИмяИсточникаДанныхКадровойИсторииСотрудников(ИсточникиДанных) Экспорт
	...
	Возврат ИсточникиДанных.Получить("ИмяВТСведенияКадровойИсторииСотрудников");
...
Функция СвязанныеОбъектыДанныхФизическогоЛица(ДанныеФизическогоЛица) Экспорт
	...
	Если ДанныеФизическогоЛица.Свойство("Гражданство") Тогда
		СвязанныеОбъекты.Вставить("Гражданство", ДанныеФизическогоЛица.Гражданство);
	КонецЕсли;
...
Процедура ЗаполнитьНастройкиПечатныхФормПоУмолчанию(ОписанияНастроек) Экспорт
	...
	КадровыйЭДО.ДобавитьНастройкуПечатнойФормы(
		ОписанияНастроек,
		Метаданные.ОбщиеМакеты.ПФ_MXL_ТрудовойДоговорМикропредприятий.Имя,
	...
	КадровыйЭДО.ДобавитьНастройкуПечатнойФормы(
		ОписанияНастроек,
		Метаданные.ОбщиеМакеты.ПФ_MXL_Т1.Имя,
		Перечисления.СодержимоеДокументов.СодержитТарифныеСтавки);
	
	КадровыйЭДО.ДобавитьНастройкуПечатнойФормы(
		ОписанияНастроек,
		Метаданные.ОбщиеМакеты.ПФ_MXL_Т5.Имя,
		Перечисления.СодержимоеДокументов.СодержитТарифныеСтавки);
...
Функция КритерииПоискаСотрудниковПоКоллекцииОтборов(Отборы) Экспорт
	...
	Для каждого УсловиеПоиска Из Отборы Цикл
		...
		ИначеЕсли ДобавитьКритерийПоискаСправочникаФизическиеЛица(КритерииПоиска, УсловиеПоиска)
			...
			Или ОбменСБанкамиПоЗарплатнымПроектам.ДобавитьКритерийПоискаПоСведениямОЗарплатныхПроектах(КритерииПоиска, УсловиеПоиска)
			Или ДобавитьКритерийПоискаПоСведениямОВидахЗанятостиСотрудников(КритерииПоиска, УсловиеПоиска)
			Или ЕстьПодсистемаБухучет
				И МодульОтражениеЗарплатыВБухучете.ДобавитьКритерийПоискаПоСведениямОбУчетеЗатрат(КритерииПоиска, УсловиеПоиска)Тогда
...
Процедура ЗаполнитьПоляПредставленийКадровыхДанныхФизическихЛиц(ДополнительныеПоляПредставлений, ПутьКПолямЛичныхДанных) Экспорт
	...
	ОписаниеПоля = ДополнительныеПоляПредставлений.Добавить();
	ОписаниеПоля.ИмяПоля = "АдресДляИнформированияСтрана";
	ОписаниеПоля.ПустоеЗначениеНаЯзыкеЗапросов = """""";
	ОписаниеПоля.ПутьПоляСКД = ПутьКПолямЛичныхДанных + ".КонтактнаяИнформация.АдресДляИнформирования.Страна";
	ОписаниеПоля.ДляПоиска = Истина;
	
	ОписаниеПоля = ДополнительныеПоляПредставлений.Добавить();
	ОписаниеПоля.ИмяПоля = "АдресДляИнформированияРегион";
	ОписаниеПоля.ПустоеЗначениеНаЯзыкеЗапросов = """""";
	ОписаниеПоля.ПутьПоляСКД = ПутьКПолямЛичныхДанных + ".КонтактнаяИнформация.АдресДляИнформирования.Регион";
	ОписаниеПоля.ДляПоиска = Истина;
	
	ОписаниеПоля = ДополнительныеПоляПредставлений.Добавить();
	ОписаниеПоля.ИмяПоля = "АдресДляИнформированияГород";
	ОписаниеПоля.ПустоеЗначениеНаЯзыкеЗапросов = """""";
	ОписаниеПоля.ПутьПоляСКД = ПутьКПолямЛичныхДанных + ".КонтактнаяИнформация.АдресДляИнформирования.Город";
	ОписаниеПоля.ДляПоиска = Истина;
	...
	ОписаниеПоля = ДополнительныеПоляПредставлений.Добавить();
	ОписаниеПоля.ИмяПоля = "АдресЗаПределамиРФСтрана";
	ОписаниеПоля.ПустоеЗначениеНаЯзыкеЗапросов = """""";
	ОписаниеПоля.ПутьПоляСКД = ПутьКПолямЛичныхДанных + ".КонтактнаяИнформация.АдресЗаПределамиРФ.Страна";
	ОписаниеПоля.ДляПоиска = Истина;
	...
	ОписаниеПоля = ДополнительныеПоляПредставлений.Добавить();
	ОписаниеПоля.ИмяПоля = "АдресМестаПроживанияСтрана";
	ОписаниеПоля.ПустоеЗначениеНаЯзыкеЗапросов = """""";
	ОписаниеПоля.ПутьПоляСКД = ПутьКПолямЛичныхДанных + ".КонтактнаяИнформация.АдресМестаПроживания.Страна";
	ОписаниеПоля.ДляПоиска = Истина;
	
	ОписаниеПоля = ДополнительныеПоляПредставлений.Добавить();
	ОписаниеПоля.ИмяПоля = "АдресМестаПроживанияРегион";
	ОписаниеПоля.ПустоеЗначениеНаЯзыкеЗапросов = """""";
	ОписаниеПоля.ПутьПоляСКД = ПутьКПолямЛичныхДанных + ".КонтактнаяИнформация.АдресМестаПроживания.Регион";
	ОписаниеПоля.ДляПоиска = Истина;
	
	ОписаниеПоля = ДополнительныеПоляПредставлений.Добавить();
	ОписаниеПоля.ИмяПоля = "АдресМестаПроживанияГород";
	ОписаниеПоля.ПустоеЗначениеНаЯзыкеЗапросов = """""";
	ОписаниеПоля.ПутьПоляСКД = ПутьКПолямЛичныхДанных + ".КонтактнаяИнформация.АдресМестаПроживания.Город";
	ОписаниеПоля.ДляПоиска = Истина;
	...
	ОписаниеПоля = ДополнительныеПоляПредставлений.Добавить();
	ОписаниеПоля.ИмяПоля = "АдресПоПропискеСтрана";
	ОписаниеПоля.ПустоеЗначениеНаЯзыкеЗапросов = """""";
	ОписаниеПоля.ПутьПоляСКД = ПутьКПолямЛичныхДанных + ".КонтактнаяИнформация.АдресПоПрописке.Страна";
	ОписаниеПоля.ДляПоиска = Истина;
	
	ОписаниеПоля = ДополнительныеПоляПредставлений.Добавить();
	ОписаниеПоля.ИмяПоля = "АдресПоПропискеРегион";
	ОписаниеПоля.ПустоеЗначениеНаЯзыкеЗапросов = """""";
	ОписаниеПоля.ПутьПоляСКД = ПутьКПолямЛичныхДанных + ".КонтактнаяИнформация.АдресПоПрописке.Регион";
	ОписаниеПоля.ДляПоиска = Истина;
	
	ОписаниеПоля = ДополнительныеПоляПредставлений.Добавить();
	ОписаниеПоля.ИмяПоля = "АдресПоПропискеГород";
	ОписаниеПоля.ПустоеЗначениеНаЯзыкеЗапросов = """""";
	ОписаниеПоля.ПутьПоляСКД = ПутьКПолямЛичныхДанных + ".КонтактнаяИнформация.АдресПоПрописке.Город";
	ОписаниеПоля.ДляПоиска = Истина;
...
Функция ЗапросВТКонтактнаяИнформация(ТолькоРазрешенные, ИмяВТКонтактнаяИнформация, ПолноеИмяОбъектаМетаданных, ОписаниеВременнойТаблицыОтборов) Экспорт
	...
	Запрос.Текст =
		...
		|			ТОГДА КонтактнаяИнформация.ЗначенияПолей
		|		ИНАЧЕ КонтактнаяИнформация.Значение
		|	КОНЕЦ КАК ЗначенияПолей
		|ПОМЕСТИТЬ ВТВсяКонтактнаяИнформация
		|{ВЫБРАТЬ
		|	Страна КАК СтранаКонтактнойИнформации}
		|ИЗ
		...
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	ВсяКонтактнаяИнформация.Объект КАК Объект,
...
Процедура НастроитьВариантыОтчетаПоСотрудникам(Настройки, НастройкиОтчета) Экспорт
	...
	НастройкиВарианта.Описание =
		НСтр("ru = 'Паспортные данные, ИИН, дата рождения и прочие личные данные'");
		...
		Пока Выборка.СледующийПоЗначениюПоля("Период") Цикл
			...
			Пока Выборка.Следующий() Цикл
				...
				Если Выборка.НачислениеТарифнойСтавки Тогда
					...
					ФорматОклада = "ЧДЦ=0";
					Если ОписаниеСостава.ОкладТариф > Цел(ОписаниеСостава.ОкладТариф) Тогда
						ФорматОклада = "ЧДЦ=2";
					КонецЕсли; 
					ОписаниеСостава.ОписаниеОклада = ОписаниеСостава.ПоказательОкладТариф
						+ ?(ОписаниеСостава.ОкладТариф = 0, "", ": " + Формат(ОписаниеСостава.ОкладТариф, ФорматОклада));
...
Процедура ОбновитьТекущиеТарифныеСтавки(МассивСотрудниковДляОбновления) Экспорт
	...
	Запрос.Текст =
		...
		|	НовыеКадровыеДанные.ТекущаяОрганизация КАК ТекущаяОрганизация,
		|	НовыеКадровыеДанные.ТекущаяТарифнаяСтавка КАК ТекущаяТарифнаяСтавка,
		|	НовыеКадровыеДанные.ТекущийСпособРасчетаАванса КАК ТекущийСпособРасчетаАванса,
		|	НовыеКадровыеДанные.ТекущийАванс КАК ТекущийАванс
		|ИЗ
...
Функция ЗапросВТТарифныеСтавкиСотрудников(ТолькоРазрешенные, ИмяВТТарифныеСтавкиСотрудников, ИмяВременнойТаблицыОтборовСотрудников, ПоляОтбораСотрудников, ПоляОтбораПериодическихДанных, ФормироватьСПериодичностьДень) Экспорт
	...
	ПараметрыПостроения.СоответствиеПсевдонимовПолей.Вставить("ДатаОкончания", "ДатаОкончанияИнтервала");
...
Функция НеобходимыЗначенияРеквизитовСправочникаФизическиеЛица(ИмяПоля) Экспорт
	...
	Возврат Метаданные.Справочники.ФизическиеЛица.Реквизиты.Найти(ИмяПоля) <> Неопределено
		...
		Или ИмяПоля = ВРег("ЛьготаПриНачисленииПособий")
		Или ИмяПоля = ВРег("ОтношениеКОсобойЗоне")
		Или ИмяПоля = ВРег("ВидОсобойЗоны");
...
Функция ПутьКДаннымЗначенийРеквизитовСправочникаФизическиеЛица(Знач ИмяПоля)
	...
	ИначеЕсли (ИмяПоляВВерхнемРегистре = ВРег("ЛьготаПриНачисленииПособий")
			Или ИмяПоляВВерхнемРегистре = ВРег("ОтношениеКОсобойЗоне")
			Или ИмяПоляВВерхнемРегистре = ВРег("ВидОсобойЗоны"))
		И Метаданные.Справочники.ФизическиеЛица.Реквизиты.Найти(ИмяПоля) = Неопределено Тогда
...
Функция ДобавитьПолеКонтактнойИнформацииФизическихЛиц(ИмяПоля, ТекстыОписанияПолей, ИсточникиДанных)
	...
	Если НеобходимаКонтактнаяИнформацияФизическогоЛица(ИмяПоля) Тогда
		...
		Если СтрЗаканчиваетсяНа(ИмяПоляВРег, ВРег("Представление")) Тогда
			ИмяТаблицы = Лев(ИмяПоля, СтрДлина(ИмяПоля) - СтрДлина("Представление"));
		ИначеЕсли СтрЗаканчиваетсяНа(ИмяПоляВРег, ВРег("Страна")) Тогда
			ИмяТаблицы = Лев(ИмяПоля, СтрДлина(ИмяПоля) - СтрДлина("Страна"));
		ИначеЕсли СтрЗаканчиваетсяНа(ИмяПоляВРег, ВРег("Регион")) Тогда
			ИмяТаблицы = Лев(ИмяПоля, СтрДлина(ИмяПоля) - СтрДлина("Регион"));
		ИначеЕсли СтрЗаканчиваетсяНа(ИмяПоляВРег, ВРег("Город")) Тогда
			ИмяТаблицы = Лев(ИмяПоля, СтрДлина(ИмяПоля) - СтрДлина("Город"));
		Иначе
			ИмяТаблицы = ИмяПоля;
		КонецЕсли; 
...
Функция НеобходимаКонтактнаяИнформацияФизическогоЛица(ИмяПоля)
	...
	Возврат ИмяПоля = ВРег("АдресДляИнформирования")
		...
		Или ИмяПоля = ВРег("АдресДляИнформированияСтрана")
		Или ИмяПоля = ВРег("АдресДляИнформированияРегион")
		Или ИмяПоля = ВРег("АдресДляИнформированияГород")
		Или ИмяПоля = ВРег("АдресЗаПределамиРФ")
		Или ИмяПоля = ВРег("АдресЗаПределамиРФПредставление")
		Или ИмяПоля = ВРег("АдресЗаПределамиРФСтрана")
		Или ИмяПоля = ВРег("АдресМестаПроживания")
		Или ИмяПоля = ВРег("АдресМестаПроживанияПредставление")
		Или ИмяПоля = ВРег("АдресМестаПроживанияСтрана")
		Или ИмяПоля = ВРег("АдресМестаПроживанияРегион")
		Или ИмяПоля = ВРег("АдресМестаПроживанияГород")
		Или ИмяПоля = ВРег("АдресПоПрописке")
		Или ИмяПоля = ВРег("АдресПоПропискеПредставление")
		Или ИмяПоля = ВРег("АдресПоПропискеСтрана")
		Или ИмяПоля = ВРег("АдресПоПропискеРегион")
		Или ИмяПоля = ВРег("АдресПоПропискеГород")
...
Функция ПутьКДаннымКонтактнойИнформацииФизическихЛиц(Знач ИмяПоля, ИмяТаблицы)
	...
	ИмяПоля = ВРег(ИмяПоля);
	Если СтрЗаканчиваетсяНа(ИмяПоля, ВРег("Представление")) Тогда
		ПутьКДанным = Символы.Таб + ИмяТаблицы + "ФизическиеЛица.Представление";
	ИначеЕсли СтрЗаканчиваетсяНа(ИмяПоля, ВРег("Страна")) Тогда
		ПутьКДанным = Символы.Таб + "ЕСТЬNULL(" + ИмяТаблицы + "ФизическиеЛица.Страна, """")";
	ИначеЕсли СтрЗаканчиваетсяНа(ИмяПоля, ВРег("Регион")) Тогда
		ПутьКДанным = Символы.Таб + "ЕСТЬNULL(" + ИмяТаблицы + "ФизическиеЛица.Регион, """")";
	ИначеЕсли СтрЗаканчиваетсяНа(ИмяПоля, ВРег("Город")) Тогда
		ПутьКДанным = Символы.Таб + "ЕСТЬNULL(" + ИмяТаблицы + "ФизическиеЛица.Город, """")";
	Иначе
...
Функция ДобавитьКритерийПоискаПоДокументамУдостоверяющимЛичность(КритерииПоиска, УсловиеПоиска)
	...
	КритерийДобавлен = Ложь;
	Если НеобходимыСведенияДокументовУдостоверяющихЛичность(УсловиеПоиска.ЛевоеЗначение)
...
Функция НеобходимыСведенияОбИнвалидности(Знач ИмяПоля)
	...
	Возврат ИмяПоля = ВРег("ИнвалидностьПериодРегистрации")
        ...
        // ЗУПК
		Или ИмяПоля = ВРег("ИнвалидностьОтношениеКИнвалидности")
        // ЗУПК
...
Функция ДобавитьКритерийПоискаПоСведениямОбИнвалидности(КритерииПоиска, УсловиеПоиска)
	...
	КритерийДобавлен = Ложь;
	Если НеобходимыСведенияОбИнвалидности(УсловиеПоиска.ЛевоеЗначение)
		Или ИмяПоля = ВРег("Период")
		Или ИмяПоля = ВРег("ДатаНачала")
		Или ИмяПоля = ВРег("ДатаОкончания") Тогда
		...
		ИначеЕсли ИмяПоля <> ВРег("Период") Тогда
            ...
            // ЗУПК
			ИначеЕсли ИмяПоля = ВРег("ИнвалидностьОтношениеКИнвалидности") Тогда
				УсловиеПоиска.ЛевоеЗначение = "ОтношениеКИнвалидности";
            // ЗУПК
...
Функция ПутьКДаннымСведенийОбИнвалидности(Знач ИмяПоля)
	...
	ИначеЕсли ИмяПоляВВерхнемРегистре = ВРег("ИнвалидностьОтношениеКИнвалидности") Тогда
		ПутьКДанным = "	СведенияОбИнвалидности.ОтношениеКИнвалидности";
	Иначе
...
Функция ДобавитьКритерийПоискаПоСведениямОГражданстве(КритерииПоиска, УсловиеПоиска)
	...
	КритерийДобавлен = Ложь;
	Если НеобходимыСведенияОГражданстве(УсловиеПоиска.ЛевоеЗначение)
		Или ИмяПоля = ВРег("Период")
		Или ИмяПоля = ВРег("ДатаНачала")
		Или ИмяПоля = ВРег("ДатаОкончания") Тогда
...
Функция ПутьКДаннымСведенийОГражданстве(Знач ИмяПоля)
	...
	ИначеЕсли ИмяПоляВВерхнемРегистре = ВРег("Страна") Тогда
		ПутьКДанным = "	ЕСТЬNULL(ГражданствоФизическихЛиц.Страна, ЗНАЧЕНИЕ(Справочник.СтраныМира.Казахстан))";
...
Функция ДобавитьКритерийПоискаПоСведениямОФИОФизическихЛиц(КритерииПоиска, УсловиеПоиска)
	...
	КритерийДобавлен = Ложь;
	Если НеобходимыСведенияОФИОФизическихЛиц(УсловиеПоиска.ЛевоеЗначение)
		Или ИмяПоля = ВРег("Период")
		Или ИмяПоля = ВРег("ДатаНачала")
		Или ИмяПоля = ВРег("ДатаОкончания") Тогда
...
Функция ЗапросВТКадровыеДанныеСотрудников(ТолькоРазрешенные, ОписательВременнойТаблицыОтборов, КадровыеДанные, ПоляОтбораПериодическихДанных, ФормироватьСПериодичностьДень, ИсточникиДанных, ИмяВТКадровыеДанныеСотрудников, ТаблицаОтборовСодержитПолеФизическоеЛицо, УничтожитьВспомогательныеТаблицы) Экспорт
	...
	Для каждого ИмяЗапрашиваемыхДанных Из ИменаКадровыхДанных Цикл
		...
		ИначеЕсли ДобавитьПолеСведенийКадровойИсторииСотрудников(ИмяКадровыхДанных, ТекстыОписанияПолей, ИсточникиДанных)
			...
			Или ЕстьПодсистемаБухучет
				И МодульОтражениеЗарплатыВБухучете.ДобавитьПолеСведенийОбУчетеЗатрат(ИмяКадровыхДанных, ТекстыОписанияПолей, ИсточникиДанных) Тогда
	...
	Иначе
		...
		Если ЕстьПодсистемаБухучет Тогда
			МодульОтражениеЗарплатыВБухучете.ДобавитьТекстЗапросаВТСведенияОбУчетеЗатрат(Запрос, ТолькоРазрешенные, ОписательВТОтборов, ПоляОтбораПериодическихДанных, ИсточникиДанных);
		КонецЕсли;
		...
		Если ЗначениеЗаполнено(ОписательВТОтборов.ПутьКПолюОрганизацияФункциональныхОпций) Тогда
			ПутьКПолюОрганизация = "ЕСТЬNULL(" + ПутьКПолюОрганизация + ", " + ОписательВТОтборов.ПутьКПолюОрганизацияФункциональныхОпций + ")";
		КонецЕсли;
...
Функция ПутьКДаннымСведенийТекущихКадровыхДанныхСотрудников(ИмяПоля) Экспорт
	...
	ИначеЕсли Не ФОИспользоватьКадровыйУчет И ИмяПоляВВерхнемРегистре = ВРег("РабочееМестоРегистратор") Тогда
		ПутьКДанным = "	ВЫРАЗИТЬ(NULL КАК Документ.ПриемНаРаботу)";
...
Процедура ДобавитьТекстЗапросаДанныхПриказовОПриемеОбУвольнении(Запрос, ТолькоРазрешенные, ОписательВременнойТаблицыОтборов, ИсточникиДанных)
	...
	Если НеобходимыеСведения.НеобходимыСведенияПриказаОбУвольнении Тогда
		...
		ЧастиЗапроса.Добавить(
			...
			|			И НЕ ДатыУвольнений.ЭтоВозвратноеСобытие
...
Функция ПутьКДаннымСведенийЗависящихОтФункциональныхОпцийОрганизаций(Знач ИмяПоля) Экспорт
	...
	ИначеЕсли ИмяПоляВВерхнемРегистре = ВРег("ЯвляетсяПрокурором")
		...
		Или ИмяПоляВВерхнемРегистре = ВРег("РасчетОтпускаПоРабочимДням")
		...
		Или ИмяПоляВВерхнемРегистре = ВРег("ЯвляетсяМедицинскойДолжностью")
...
Функция ДобавитьПолеСведенийОКлассахУсловийТруда(ИмяПоля, ТекстыОписанияПолей, ИсточникиДанных)
	...
	Если НеобходимыСведенияОКлассахУсловийТруда(ИмяПоля) Тогда
		...
		КадровыйУчет.ДобавитьВКоллекциюКадровыеДанныеДляСведенийОКлассахУсловийТруда(ИсточникиДанных, ИмяПоляДолжность);
...
Процедура ДобавитьТекстЗапросаВТСведенияОКлассахУсловийТруда(Запрос, ТолькоРазрешенные, ОписательВременнойТаблицыОтборов, ПоляОтбораПериодическихДанных, ИсточникиДанных)
	...
	ЗапросВТ = ЗапросВТСведенияОКлассахУсловийТруда(ТолькоРазрешенные, ОписательВременнойТаблицыОтборов,
		КадровыйУчет.ИмяИсточникаДанныхКадровойИсторииСотрудников(ИсточникиДанных), ИмяПоляДолжность, ИсточникиДанных);
	...
	КадровыйУчет.ДобавитьСоединениеСКадровымиДанными(ОписательВременнойТаблицыОтборов, ЧастиЗапроса, ИмяПоляДолжность);
...
Функция ДобавитьПолеСведенийОбУсловияхДоговоровОпеки(ИмяПоля, ТекстыОписанияПолей, ИсточникиДанных)
	
	ДобавленоПолеСведений = Ложь;
	Если НеобходимыСведенияОбУсловияхДоговоровОпеки(ИмяПоля) Тогда
		
		ДобавленоПолеСведений = Истина;
		ИсточникиДанных.Вставить("СведенияОбУсловияхДоговоровОпеки", Истина);
		
		ПутьКДанным = КадровыйУчет.ПутьКДаннымСведенийОбУсловияхДоговоровОпеки(ИмяПоля);
		ТекстыОписанияПолей.Добавить(ПутьКДанным + " КАК " + ИмяПоля);
...
Функция НеобходимыСведенияОбУсловияхДоговоровОпеки(Знач ИмяПоля)
	
	ИмяПоля = ВРег(ИмяПоля);
	
	Возврат ИмяПоля = ВРег("ЯвляетсяОпекуномПенсионером");
...
Функция ДобавитьПолеСведенийМестРаботыСотрудников(ИмяПоля, ТекстыОписанияПолей, ИсточникиДанных) Экспорт
	...
	ДобавленоПолеСведений = Ложь;
	Если НеобходимыСведенияМестРаботыСотрудников(ИмяПоля) Тогда
		
		ДобавленоПолеСведений = Истина;
		ИсточникиДанных.Вставить("СведенияМестРаботыСотрудников", Истина);
		
		Если ПолучитьФункциональнуюОпцию("ИспользоватьКадровыйУчет") Тогда
			КадровыйУчет.ДобавитьВКоллекциюИмяКадровыхДанных(ИсточникиДанных, "КадроваяИсторияСотрудников", "Организация");
			КадровыйУчет.ДобавитьВКоллекциюИмяКадровыхДанных(ИсточникиДанных, "КадроваяИсторияСотрудников", "Подразделение");
		Иначе
			КадровыйУчет.ДобавитьВКоллекциюИмяКадровыхДанных(ИсточникиДанных, "ПостоянныеКадровыеДанныеСотрудников", "Организация");
			КадровыйУчет.ДобавитьВКоллекциюИмяКадровыхДанных(ИсточникиДанных, "ПостоянныеКадровыеДанныеСотрудников", "Подразделение");
		КонецЕсли;
		
		ПутьКДанным = КадровыйУчет.ПутьКДаннымСведенийМестРаботыСотрудников(ИмяПоля);
		ТекстыОписанияПолей.Добавить(ПутьКДанным + " КАК " + ИмяПоля);
...
Функция НеобходимыСведенияМестРаботыСотрудников(Знач ИмяПоля)
	
	ИмяПоляВРег = ВРег(ИмяПоля);
	
	Возврат ИмяПоляВРег = ВРег("РайонныйКоэффициент")
		Или ИмяПоляВРег = ВРег("РайонныйКоэффициентРФ");
...
Функция ПутьКДаннымСведенийМестРаботыСотрудников(Знач ИмяПоля) Экспорт
	...
	Если ПолучитьФункциональнуюОпцию("ИспользоватьКадровыйУчет") Тогда
		...
		Если ИмяПоляВВерхнемРегистре = ВРег("РайонныйКоэффициент") Тогда
			ПутьКДанным =
				"	ВЫБОР КОГДА
				|		ЕСТЬNULL(КадроваяИсторияСотрудников.Подразделение, ЗНАЧЕНИЕ(Справочник.ПодразделенияОрганизаций.ПустаяСсылка)) <> ЗНАЧЕНИЕ(Справочник.ПодразделенияОрганизаций.ПустаяСсылка) ТОГДА
				|			КадроваяИсторияСотрудников.Подразделение.РайонныйКоэффициент
				|		ИНАЧЕ КадроваяИсторияСотрудников.Организация.РайонныйКоэффициент
				|	КОНЕЦ";
		ИначеЕсли ИмяПоляВВерхнемРегистре = ВРег("РайонныйКоэффициентРФ") Тогда
			ПутьКДанным =
				"	ВЫБОР КОГДА
				|		ЕСТЬNULL(КадроваяИсторияСотрудников.Подразделение, ЗНАЧЕНИЕ(Справочник.ПодразделенияОрганизаций.ПустаяСсылка)) <> ЗНАЧЕНИЕ(Справочник.ПодразделенияОрганизаций.ПустаяСсылка) ТОГДА
				|			КадроваяИсторияСотрудников.Подразделение.РайонныйКоэффициентРФ
				|		ИНАЧЕ КадроваяИсторияСотрудников.Организация.РайонныйКоэффициентРФ
				|	КОНЕЦ";
		КонецЕсли;
	...
	Иначе
		...
		Если ИмяПоляВВерхнемРегистре = ВРег("РайонныйКоэффициент") Тогда
			ПутьКДанным =
				"	ВЫБОР КОГДА
				|		ЕСТЬNULL(ТекущиеКадровыеДанныеСотрудников.ТекущееПодразделение, ЗНАЧЕНИЕ(Справочник.ПодразделенияОрганизаций.ПустаяСсылка)) <> ЗНАЧЕНИЕ(Справочник.ПодразделенияОрганизаций.ПустаяСсылка) ТОГДА
				|			ТекущиеКадровыеДанныеСотрудников.ТекущееПодразделение.РайонныйКоэффициент
				|		ИНАЧЕ ТекущиеКадровыеДанныеСотрудников.ТекущаяОрганизация.РайонныйКоэффициент
				|	КОНЕЦ";
		ИначеЕсли ИмяПоляВВерхнемРегистре = ВРег("РайонныйКоэффициентРФ") Тогда
			ПутьКДанным =
				"	ВЫБОР КОГДА
				|		ЕСТЬNULL(ТекущиеКадровыеДанныеСотрудников.ТекущееПодразделение, ЗНАЧЕНИЕ(Справочник.ПодразделенияОрганизаций.ПустаяСсылка)) <> ЗНАЧЕНИЕ(Справочник.ПодразделенияОрганизаций.ПустаяСсылка) ТОГДА
				|			ТекущиеКадровыеДанныеСотрудников.ТекущееПодразделение.РайонныйКоэффициентРФ
				|		ИНАЧЕ ТекущиеКадровыеДанныеСотрудников.ТекущаяОрганизация.РайонныйКоэффициентРФ
				|	КОНЕЦ";
		КонецЕсли;
...
Процедура ОпределитьПроверкиВеденияУчета(ГруппыПроверок, Проверки) Экспорт
	...
	Проверка = Проверки.Добавить();
	Проверка.ИдентификаторГруппы			= ГруппаПроверок.Идентификатор;
	Проверка.Наименование					= НСтр("ru = 'ИИН сотрудников, учет которых начался с начала прошлого месяца'");
	Проверка.Причины						= НСтр("ru = 'Не введен (введен некорректный) ИИН'");
	Проверка.Рекомендация					= НСтр("ru = 'Ввести (исправить) ИИН'");
	...
	Иначе
		...
		Проверка = Проверки.Добавить();
		Проверка.ИдентификаторГруппы			= ГруппаПроверок.Идентификатор;
		Проверка.Наименование					= НСтр("ru = 'Статус физического лица как налогоплательщика'");
		// ЗУПК Выводим данные за весь период учета, если необходимо выводить за последний месяц пользователь очищает дату начала в настройках проверки, если сузить период, пользователь меняет дату начала в настройках проверка
		Проверка.ДатаНачалаПроверки             = ЗарплатаКадрыКлиентСервер.ДатаОтсчетаПериодическихСведений();
		Проверка.Причины						= НСтр("ru = 'Не указан (указан некорректно) статус налогоплательщика'");
		Проверка.Рекомендация					= НСтр("ru = 'Ввести (исправить) статус налогоплательщика'");
		Проверка.Идентификатор					= ГруппаПроверок.Идентификатор + ".ПроверкаНаличияСтатусаФизическихЛицКакНалогоплательщиков";
		Проверка.ОбработчикПроверки				= "КонтрольВеденияУчетаПереопределяемый.ПроверкаНаличияСтатусаФизическихЛицКакНалогоплательщиков";
		Проверка.КонтекстПроверокВеденияУчета	= ГруппаПроверок.КонтекстПроверокВеденияУчета;
	...
	Проверка = Проверки.Добавить();
	Проверка.ИдентификаторГруппы			= ГруппаПроверок.Идентификатор;
	Проверка.Наименование					= НСтр("ru='Проверка заполненности пола сотрудников'");
	Проверка.Причины						= НСтр("ru='Не указан пол сотрудника'");
	Проверка.Рекомендация					= НСтр("ru='Указать пол сотруднику'");
	Проверка.Идентификатор					= ГруппаПроверок.Идентификатор + ".ПроверкаЗаполненностиПола";
	Проверка.ОбработчикПроверки				= "Обработки.КонтрольВеденияКадровогоУчета.ПроверкаЗаполненностиПола";
	Проверка.КонтекстПроверокВеденияУчета	= ГруппаПроверок.КонтекстПроверокВеденияУчета;
...
Функция ПриостановленияТрудовыхДоговоров(ТолькоРазрешенные, ТаблицаОтбора, Отбор) Экспорт
	...
	Возврат КадровыйУчет.ПустаяТаблицаПриостановленийТрудовыхДоговоров();
