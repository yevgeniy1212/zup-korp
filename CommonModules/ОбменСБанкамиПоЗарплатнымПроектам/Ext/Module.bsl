...
Процедура УстановитьДоступностьСпособовЗачисленияПоЗарплатномуПроекту(СпособыЗачисления, ЗарплатныйПроект) Экспорт
	...
	СоответствиеФорматов = СоответствиеФорматовФайла();
	...
	СпособыЗачисления[Перечисления.СпособыЗачисленияВыплат.ПоЛицевомуСчету] = Истина;
	
	Если ФорматФайла > 3.6 Тогда
		
		СпособыЗачисления[Перечисления.СпособыЗачисленияВыплат.ПоНомеруКарты] = Истина;
		СпособыЗачисления[Перечисления.СпособыЗачисленияВыплат.ПоНомеруТелефона] = Истина;
		СпособыЗачисления[Перечисления.СпособыЗачисленияВыплат.ПоСНИЛС] = Истина;
		
	КонецЕсли;
	
	ОбменСБанкамиПоЗарплатнымПроектамВнутренний.УстановитьДоступностьСпособовЗачисленияПоЗарплатномуПроекту(СпособыЗачисления, ЗарплатныйПроект);
...
Функция НовоеСоответствиеСпособовЗачисления() Экспорт
	...
	СоответствиеСпособовЗачисления = Новый Соответствие;
	
	Для Каждого СпособЗачисления Из Перечисления.СпособыЗачисленияВыплат Цикл
		СоответствиеСпособовЗачисления.Вставить(СпособЗачисления, Ложь);
	КонецЦикла;
	
	Возврат СоответствиеСпособовЗачисления;
...
Процедура ПриОпределенииВидовПодключаемыхКоманд(ВидыПодключаемыхКоманд) Экспорт
	...
	Вид.Имя = "ВыгрузкаФайловОбменаСБанком";
	Вид.ИмяПодменю = "ПодменюВыгрузкаФайловОбменаСБанком";
	Вид.Картинка = БиблиотекаКартинок.ВыгрузитьФайлСертификата;
	Вид.Отображение = ОтображениеКнопки.КартинкаИТекст;
	
	Вид = ВидыПодключаемыхКоманд.Добавить();
	Вид.Имя = "ЗагрузкаФайловОбменаСБанками";
	Вид.ИмяПодменю = "ПодменюЗагрузкаФайловОбменаСБанком";
	Вид.Картинка = БиблиотекаКартинок.ЗагрузитьФайлСертификата;
	Вид.Отображение = ОтображениеКнопки.КартинкаИТекст;
...
Процедура ПриОпределенииКомандПодключенныхКОбъекту(НастройкиФормы, Источники, ПодключенныеОтчетыИОбработки, Команды) Экспорт
	...
	Если Не ПравоДоступа("Чтение", Метаданные.Справочники.ЗарплатныеПроекты) 
		Или Не ИсточникиПоЗарплатнымПроектам(Источники) Тогда
		Возврат;
	КонецЕсли;

	ЗарплатныеПроектыИспользующиеВыгрузкуФайла = Справочники.ЗарплатныеПроекты.ИспользующиеВыгрузкуФайла();
	Если ЗарплатныеПроектыИспользующиеВыгрузкуФайла.Количество() > 0 Тогда
		Команда = Команды.Добавить();
		Команда.Вид = "ВыгрузкаФайловОбменаСБанком";
		Команда.Идентификатор = "ВыгрузитьФайлОбменаСБанком";
		Команда.Представление = НСтр("ru = 'Выгрузить файл'");
		Команда.Важность = "Обычное";
		Команда.ОтображениеКнопки = ОтображениеКнопки.КартинкаИТекст;
		Команда.Картинка = БиблиотекаКартинок.ВыгрузитьФайлСертификата;
		Команда.ТипПараметра = Новый ОписаниеТипов(ТипыДокументовВыгружаемыхВБанк());
		Команда.ФункциональныеОпции = "ИспользоватьЭлектронныйОбменСБанкамиПоЗарплатнымПроектам";
		Команда.ИзменяетВыбранныеОбъекты = Ложь;
		Команда.РежимЗаписи = "Записывать";
		Команда.Обработчик = "ОбменСБанкамиПоЗарплатнымПроектамКлиент.ВыгрузитьФайлОбменаСБанком";
		Команда.МножественныйВыбор = Истина;
		Команда.ЕстьУсловияВидимости = Истина;
		ПодключаемыеКоманды.ДобавитьУсловиеВидимостиКоманды(
			Команда, 
			"ЗарплатныйПроект", 
			ЗарплатныеПроектыИспользующиеВыгрузкуФайла,
			ВидСравненияКомпоновкиДанных.ВСписке);
		Команда = Команды.Добавить();
		Команда.Вид = "ЗагрузкаФайловОбменаСБанками";
		Команда.Идентификатор = "ЗагрузитьФайлОбменаСБанком";
		Команда.Представление = НСтр("ru = 'Загрузить подтверждения банка'");
		Команда.Важность = "Обычное";
		Команда.ОтображениеКнопки = ОтображениеКнопки.КартинкаИТекст;
		Команда.Картинка = БиблиотекаКартинок.ЗагрузитьФайлСертификата;
		Команда.ТипПараметра = Новый ОписаниеТипов(ТипыДокументовЗагружаемыхИзБанка());
		Команда.ФункциональныеОпции = "ИспользоватьЭлектронныйОбменСБанкамиПоЗарплатнымПроектам";
		Команда.ИзменяетВыбранныеОбъекты = Ложь;
		Команда.РежимЗаписи = "Записывать";
		Команда.Обработчик = "ОбменСБанкамиПоЗарплатнымПроектамКлиент.ЗагрузитьФайлОбменаСБанком";
		Команда.МножественныйВыбор = Истина;
...
// Возвращает признак использования электронного обмена информацией с банком по зарплатному проекту.
//
// Параметры:
//  ЗарплатныйПроект - СправочникСсылка.ЗарплатныеПроекты - зарплатный проект.
//
// Возвращаемое значение:
//  Булево -  Истина, если по зарплатному проекту ведется обмен электронными документами с банком 
//
...
// Возвращает первый зарплатный проект по организации.
//
// Параметры:
//  Организация - СправочникСсылка.Организации
//  ПолучитьЕслиЕдинственный - Булево - проверять, чтобы зарплатный проект был единственным в организации.
//                                      Если зарплатный проект не единственный, будет возвращена пустая ссылка.
//
// Возвращаемое значение:
//  СправочникСсылка.ЗарплатныеПроекты - зарплатный проект организации. 
//                                       Если для организации нет зарплатного проекта, то возвращается пустая ссылка.
//
...
// Устанавливает константу использования электронного обмена с банками по зарплатным проектам.
// Использование электронного обмена с банками включается, если используется расчет зарплаты 
// и есть хотя бы один зарплатный проект, для которого используется ЭОИ.
//
// Параметры:
...
//  ИмяВременнойТаблицыОтборов - Строка - имя временной таблицы в менеджере временных таблиц.
//  ИмяПоляФизическоеЛицо      - Строка - имя поля временной таблицы, содержащего ссылку на физическое лицо.
//  ИмяПоляПериод              - Строка - имя поля временной таблицу, содержащее период, 
//                                        на который предполагается получить лицевые счета.
//  ДатаПолученияДанных        - Дата, Неопределено
// Возвращаемое значение:
//  Структура:
...
Функция ЗапросВТЛицевыеСчетаСотрудниковПоВременнойТаблице(ТолькоРазрешенные, ОписаниеВременнойТаблицы, ПоляОтбораПериодическихДанных, ИмяВТЛицевыеСчетаСотрудниковПоЗарплатнымПроектам = "ВТЛицевыеСчетаСотрудников") Экспорт
	...
	ТекстЗапросаУникальныхЗаписей = 
		...
		|{ВЫБРАТЬ
		|	Период КАК ПериодПоЗарплатнымПроектам,
		|	ДатаОткрытияЛицевогоСчета КАК ДатаОткрытияЛицевогоСчетаПоЗарплатнымПроектам,
		|	Организация КАК ОрганизацияПоЗарплатнымПроектам,
		|	ФизическоеЛицо КАК ФизическоеЛицоПоЗарплатнымПроектам,
		|	НомерЛицевогоСчета КАК НомерЛицевогоСчетаПоЗарплатнымПроектам,
		|	ЗарплатныйПроект КАК ЗарплатныйПроектПоЗарплатнымПроектам,
		|	Банк.* КАК БанкПоЗарплатнымПроектам,
		|	ДокументОснование КАК ДокументОснованиеПоЗарплатнымПроектам}
		|ИЗ
...
// Создает в МенеджерВременныхТаблиц временную таблицу с именем ВТЛицевыеСчетаСотрудников и полями:
//  МесяцОткрытия
//  Сотрудник
//  ФизическоеЛицо
//  ЗарплатныйПроект
...
//  Организация - СправочникСсылка.Организации - организация, по которой нужно получить лицевые счета.
//  ЗарплатныйПроект - СправочникСсылка.ЗарплатныеПроекты - зарплатный проект
//                   - Неопределено                       - по всем зарплатным проектам
//
//
...
//
// Параметры:
//  СписокСотрудников   - Массив из СправочникСсылка.Сотрудники - сотрудники, по которым нужно получить лицевые счета.
//  ТолькоРазрешенные   - Булево
//  Организация         - СправочникСсылка.Организации - организация, по которой нужно получить лицевые счета.
//  ДатаПолученияДанных - Дата         - дата на которую необходимо получить данные о номерах лицевых счетов
//                      - Неопределено - на текущую дату
//  ЗарплатныйПроект    - СправочникСсылка.ЗарплатныеПроекты - зарплатный проект
//                      - Неопределено                       - по всем зарплатным проектам
//
// Возвращаемое значение:
...
// Возвращает таблицу значений с сотрудниками и их способами выплат.
//
// Параметры:
//  СписокСотрудников   - Массив из СправочникСсылка.Сотрудники - сотрудники, по которым нужно получить способы выплат.
//  ТолькоРазрешенные   - Булево
//  Организация         - СправочникСсылка.Организации - организация, по которой нужно получить способы выплат.
//  ЗарплатныйПроект    - СправочникСсылка.ЗарплатныеПроекты - зарплатный проект
//                      - Неопределено                       - по всем зарплатным проектам
//
// Возвращаемое значение:
...
//     * СпособВыплаты      - ПеречислениеСсылка.СпособыЗачисленияВыплат
//     * БанковскийСчет     - ОпределяемыйТип.БанковскиеСчетаФизическихЛиц
//     * БанковскаяКарта    - СправочникСсылка.БанковскиеКартыКонтрагентов
//     * Телефон            - СправочникСсылка.ВидыКонтактнойИнформации
//
Функция СпособыВыплатыСотрудников(СписокСотрудников, ТолькоРазрешенные, Организация, ЗарплатныйПроект = Неопределено) Экспорт
	
	Запрос = Новый Запрос;
	Запрос.МенеджерВременныхТаблиц = Новый МенеджерВременныхТаблиц;
	
	СоздатьВТСпособыВыплатСотрудников(Запрос.МенеджерВременныхТаблиц, ТолькоРазрешенные, СписокСотрудников, Организация, ЗарплатныйПроект);
	
	Запрос.Текст =
		"ВЫБРАТЬ
		|	*
		|ИЗ
		|	ВТСпособыВыплатСотрудников КАК СпособыВыплатСотрудниковПоЗарплатнымПроектам";
	
	Возврат Запрос.Выполнить().Выгрузить();
	
КонецФункции

// Возвращает таблицу значений с физическими лицами и их способами выплат в указанной организации.
//
// Параметры:
//  СписокФизическихЛиц - Массив из СправочникСсылка.ФизическиеЛица
//  ТолькоРазрешенные   - Булево
//  Организация         - СправочникСсылка.Организации - организация, по которой нужно получить лицевые счета.
//  ДатаПолученияДанных     - Дата - дата на которую необходимо получить данные о номерах лицевых счетов
//  ЗарплатныйПроект        - СправочникСсылка.ЗарплатныеПроекты - зарплатный проект
...
//     * Телефон            - СправочникСсылка.ВидыКонтактнойИнформации
//
Функция СпособыВыплатыФизическихЛиц(СписокФизическихЛиц, ТолькоРазрешенные, Организация, ДатаПолученияДанных, ЗарплатныйПроект) Экспорт
	...
	Запрос.Текст = 
		...
		|	ЛицевыеСчета.СпособЗачисления КАК СпособЗачисления,
		|	ЛицевыеСчета.БанковскийСчет КАК БанковскийСчет,
		|	ЛицевыеСчета.БанковскаяКарта КАК БанковскаяКарта,
...
Процедура СохранитьСведенияОЛицевомСчетеСотрудника(Форма, ДополнительныеСвойства) Экспорт
	...
	Если ЗначениеЗаполнено(Форма.ЛицевыеСчетаСотрудниковПоЗарплатнымПроектам.НомерЛицевогоСчета)
		Или ЗначениеЗаполнено(Форма.ЛицевыеСчетаСотрудниковПоЗарплатнымПроектам.СпособЗачисления) Тогда
...
//
Процедура ВыгрузитьФайлыОбменаСБанкомПоВедомости(СтруктураПараметровДляФормированияФайла) Экспорт
	...
	|		ПО ВедомостьВБанк.Ссылка = СостоянияДокументовЗачисленияЗарплаты.ДокументЗачисленияЗарплаты
	...
	Пока Выборка.Следующий() Цикл
		...
		Если ЗначениеЗаполнено(Выборка.Состояние) Тогда
			ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'По документу %1 уже получено подтверждение о зачислении зарплаты'"),
				Выборка.Ведомость);
...
Процедура ЗаполнитьСписокСпособовЗачисления(СписокСпособовЗачисления, ЗарплатныйПроект) Экспорт
	
	СпособыЗачисления = Новый Массив;
	
	СпособыЗачисленияСоответствие = НовоеСоответствиеСпособовЗачисления();
	
	УстановитьДоступностьСпособовЗачисленияПоЗарплатномуПроекту(СпособыЗачисленияСоответствие, ЗарплатныйПроект);
	
	Для Каждого СпособЗачисления Из СпособыЗачисленияСоответствие Цикл
		
		Если СпособЗачисления.Значение Тогда
			СпособыЗачисления.Добавить(СпособЗачисления.Ключ);
		КонецЕсли;
		
	КонецЦикла;
	
	СписокСпособовЗачисления.СписокВыбора.Очистить();
	СписокСпособовЗачисления.СписокВыбора.ЗагрузитьЗначения(СпособыЗачисления);
...
//
// Параметры:
//  ЗарплатныйПроект - СправочникСсылка.ЗарплатныеПроекты - ссылка на зарплатный проект.
//
// Возвращаемое значение:
//  Булево - Истина, если по зарплатному проекту ведется прямой обмен электронными документами с банком, 
//
...
Процедура ЗаполнитьПараметрыЭДПоИсточнику(Источник, ПараметрыЭД) Экспорт
	...
	Если ЗначениеЗаполнено(ВидЭД) Тогда
		...
		РеквизитыИсточника = Новый Структура("Организация, ЗарплатныйПроект");
		Если ОбщегоНазначения.ЭтоСсылка(ТипИсточника) Тогда
...
Процедура ПриФормированииXMLФайла(ОбъектДляВыгрузки, ИмяФайла, АдресФайла) Экспорт
	...
	ОбменСБанкамиПоЗарплатнымПроектамВызовСервера.ВыгрузитьФайлыОбменаСБанком(СтруктураПараметров, АдресХранилища);
...
Процедура ПриПолученииXMLФайла(АдресДанныхФайла, ИмяФайла, ОбъектВладелец, ДанныеОповещения) Экспорт
	...
	СозданныеДокументы = ОбменСБанкамиПоЗарплатнымПроектамВызовСервера.ЗагрузитьПодтвержденияИзБанка(ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(АдресДанныхФайла));
...
//
// Параметры:
//  ПлатежныйДокумент - ДокументСсылка - Ссылка на платежный документ.
//
...
Функция СведенияОПлатежномДокументе(МассивВедомостей, ЗначенияДляЗаполнения = Неопределено) Экспорт
    ...
    |   ВедомостьНаВыплатуЗарплатыПеречислением.СпособВыплаты.Наименование,
	...
	//
	// ВедомостейВедомостьПрочихДоходовВБанк в процедуре ОбменСБанкамиПоЗарплатнымПроектамВнутренний.ДополнитьТаблицу ниже
	|
    |ОБЪЕДИНИТЬ ВСЕ
    |
    |ВЫБРАТЬ РАЗЛИЧНЫЕ
    |   ВедомостьПрочихДоходовПеречислением.Организация,
    |   ЗНАЧЕНИЕ(Справочник.ЗарплатныеПроекты.ПустаяСсылка),
    |   ВедомостьПрочихДоходовПеречислением.СпособВыплаты,
    |   ВедомостьПрочихДоходовПеречислением.СпособВыплаты,
    |   ВедомостьПрочихДоходовПеречислением.ПериодРегистрации
    |ИЗ
    |   Документ.ВедомостьПрочихДоходовПеречислением КАК ВедомостьПрочихДоходовПеречислением
    |ГДЕ
    |   ВедомостьПрочихДоходовПеречислением.Ссылка В(&МассивВедомостей)
	|
	|ОБЪЕДИНИТЬ ВСЕ
    |
    |ВЫБРАТЬ РАЗЛИЧНЫЕ
    |   ВедомостьНаПеречислениеПрочихВыплат.Организация,
    |   ЗНАЧЕНИЕ(Справочник.ЗарплатныеПроекты.ПустаяСсылка),
    |   """",
    |   ЗНАЧЕНИЕ(Справочник.СпособыВыплатыЗарплаты.ПустаяСсылка),
    ...
    |
    |ВЫБРАТЬ
    |   ЕСТЬNULL(СУММА(ВедомостьНаВыплатуЗарплатыПеречислениемЗарплата.КВыплате + ВедомостьНаВыплатуЗарплатыПеречислениемЗарплата.КомпенсацияЗаЗадержкуЗарплаты), 0)
    |ИЗ
	...
	//
	// ВедомостейВедомостьПрочихДоходовВБанк в процедуре ОбменСБанкамиПоЗарплатнымПроектамВнутренний.ДополнитьСуммуПоДокументу ниже
    |
    |ОБЪЕДИНИТЬ ВСЕ
    |
    |ВЫБРАТЬ
    |   ЕСТЬNULL(СУММА(ВедомостьПрочихДоходовПеречислением.КВыплате), 0)
    |ИЗ
    ...
    |
    |ВЫБРАТЬ
    |   ЕСТЬNULL(СУММА(ВедомостьНаПеречислениеПрочихВыплатПрочиеВыплаты.Сумма), 0)
    |ИЗ
    |   Документ.ВедомостьНаПеречислениеПрочихВыплат.ПрочиеВыплаты КАК ВедомостьНаПеречислениеПрочихВыплатПрочиеВыплаты
    |ГДЕ
    |   ВедомостьНаПеречислениеПрочихВыплатПрочиеВыплаты.Ссылка В(&МассивВедомостей)";
	// ЗУПК
	Выборка = Запрос.Выполнить().Выбрать();
    ...
    // ЗУПК
	Пока Выборка.Следующий() Цикл
		СуммаПоДокументу = Выборка.СуммаПоДокументу + СуммаПоДокументу;
	КонецЦикла;
    // ЗУПК
...
Процедура ЗаполнитьРеквизитыНаФормеПлатежногоДокумента(Форма, Организация, ЗначенияДляЗаполнения, МассивВедомостей) Экспорт
	...
	Если ЗначенияДляЗаполнения.Свойство("ИНН") Или ЗначенияДляЗаполнения.Свойство("КПП") Тогда
		СведенияОбОрганизации = ЗарплатаКадры.СведенияОбОрганизации(Организация, "ИННЮЛ, КППЮЛ, ТипНП");
...
Функция ВедомостиПлатежногоДокументаПеречисленияЗарплаты(ПлатежныйДокумент) Экспорт
	...
	ОбменСБанкамиПоЗарплатнымПроектамПереопределяемый.ВедомостиПлатежногоДокументаПеречисленияЗарплаты(
		ПлатежныйДокумент, Ведомости, СтандартнаяОбработка);
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("ЗарплатаКадрыПриложения.ВнешниеХозяйственныеОперации.ВзаиморасчетыССотрудникамиВХО") Тогда
		Модуль = ОбщегоНазначения.ОбщийМодуль("ВзаиморасчетыССотрудникамиВХО");
		Возврат Модуль.ВедомостиПлатежногоДокументаПеречисленияЗарплаты(ПлатежныйДокумент);
	КонецЕсли;
...
Функция ПечатьСпискаПеречисленийПоДокументам(МассивОбъектов, ОбъектыПечати, ПлатежныйДокумент = Неопределено) Экспорт
	...
	Если МассивОбъектов.Количество() = 0 Тогда
        // ЗУПК
		Если ТипЗнч(МассивОбъектов[0]) = Тип("ДокументСсылка.ВедомостьНаВыплатуЗарплатыПеречислением") Тогда
			ДокументМенеджер = Документы.ВедомостьНаВыплатуЗарплатыПеречислением;
        ИначеЕсли ТипЗнч(МассивОбъектов[0]) = Тип("ДокументСсылка.ВедомостьПрочихДоходовВБанк") Тогда 
            ДокументМенеджер = Документы.ВедомостьПрочихДоходовВБанк;
        ИначеЕсли ТипЗнч(МассивОбъектов[0]) = Тип("ДокументСсылка.ВедомостьПрочихДоходовПеречислением") Тогда 
            ДокументМенеджер = Документы.ВедомостьПрочихДоходовПеречислением;
        ИначеЕсли НЕ ТипЗнч(МассивОбъектов[0]) = Тип("ДокументСсылка.ВедомостьНаПеречислениеПрочихВыплат") Тогда 
...
Функция ПечатьСпискаПеречисленийПоЗарплатномуПроектуПоДокументам(МассивОбъектов, ОбъектыПечати, ПлатежныйДокумент = Неопределено) Экспорт
	...
	Иначе	
		ДокументМенеджер = ОбщегоНазначения.МенеджерОбъектаПоСсылке(МассивОбъектов[0]);
	КонецЕсли;
	Возврат ДокументМенеджер.ПечатьСпискаПеречисленийПоЗарплатномуПроектуПоДокументам(МассивОбъектов, ОбъектыПечати, ПлатежныйДокумент);
...
//
// Параметры:
//  ПлатежныйДокумент - ДокументОбъект - ссылка на платежный документ.
//  МассивВедомостей - Массив из ОпределяемыйТип.ВедомостьВБанкЗарплатаКадры - массив ссылок ведомостей в банк.
//  МассивОшибок - Массив из Структура:
//     * ТекстСообщения - Строка - Текст ошибки.
//     * Ведомость - ДокументСсылка - ссылка на ведомость, к которой относится ошибка,
...
Процедура ПроверитьЗаполнениеПлатежногоДокумента(ПлатежныйДокумент, МассивВедомостей, МассивОшибок) Экспорт
    ...
    // ЗУПК
  	ВедомостиТаблица.Колонки.Добавить("Банк", Новый ОписаниеТипов("СправочникСсылка.КлассификаторБанков"));
    // ЗУПК
	...
	Для Каждого КлючИЗначение Из ВедомостиПоТипамДокументов Цикл
        ...
        // ЗУПК
		Если ТипЗнч(ВедомостиПоТипу[0]) = Тип("ДокументСсылка.ВедомостьНаВыплатуЗарплатыПеречислением")
			ИЛИ ТипЗнч(ВедомостиПоТипу[0]) = Тип("ДокументСсылка.ВедомостьПрочихДоходовПеречислением") Тогда
            ЗначенияРеквизитов = ОбщегоНазначения.ЗначенияРеквизитовОбъектов(ВедомостиПоТипу, "Организация,Банк,Проведен"); 	
		ИначеЕсли ТипЗнч(ВедомостиПоТипу[0]) = Тип("ДокументСсылка.ВедомостьНаПеречислениеПрочихВыплат") Тогда
			ЗначенияРеквизитов = ОбщегоНазначения.ЗначенияРеквизитовОбъектов(ВедомостиПоТипу, "Организация,Проведен");
        Иначе
	...
	|	ВедомостьНаВыплатуЗарплатыВБанк.ЗарплатныйПроект КАК ЗарплатныйПроект,
    // ЗУПК
  	|	ВедомостьНаВыплатуЗарплатыВБанк.Банк КАК Банк,
    // ЗУПК
	|	ВедомостьНаВыплатуЗарплатыВБанк.Проведен КАК Проведен
    ...
    // ЗУПК
  	|	ВедомостиПлатежногоДокумента.Банк КАК Банк,
    // ЗУПК
	...
	Если Результат.Количество() > 1 Тогда
        // ЗУПК
		ТекстОшибки = СтрШаблон(
			НСтр("ru = 'В документе %1 должны присутствовать ведомости только из одной организации и одного банка/зарплатного проекта, в табличной части должны быть выбраны ведомости одного вида.'"), 
			ПлатежныйДокумент);
        // ЗУПК
		МассивОшибок.Добавить(Новый Структура("ТекстСообщения, Ведомость", ТекстОшибки, Неопределено));
	КонецЕсли;
	...
	Выборка = Запрос.Выполнить().Выбрать();
	Пока Выборка.Следующий() Цикл
		ТекстОшибки = СтрШаблон(НСтр("ru = 'Ведомость %1 не проведена.'"), Выборка.Ведомость);
		МассивОшибок.Добавить(Новый Структура("ТекстСообщения, Ведомость", ТекстОшибки, Выборка.Ведомость));
	КонецЦикла;
...
Функция ДобавитьПолеСведенийОЗарплатныхПроектах(ИмяПоля, ТекстыОписанияПолей, ИсточникиДанных) Экспорт
...
Функция ДобавитьКритерийПоискаПоСведениямОЗарплатныхПроектах(КритерииПоиска, УсловиеПоиска) Экспорт
...
Процедура ДобавитьТекстЗапросаВТСведенияОЗарплатныхПроектах(Запрос, ТолькоРазрешенные, ОписательВременнойТаблицыОтборов, ПоляОтбораПериодическихДанных, ИсточникиДанных) Экспорт
	
	Если ИсточникиДанных.Получить("СведенияОЗарплатныхПроектах") = Неопределено Тогда
		Возврат;
	КонецЕсли;
	
	ОписательВременнойТаблицыОтборовЛицевыхСчетов = ОписаниеВременнойТаблицыДляСоздатьВТЛицевыеСчетаСотрудников(Неопределено,
	...
	ЗапросВТ = ЗапросВТЛицевыеСчетаСотрудниковПоВременнойТаблице(
...
Функция СоответствиеФорматовФайла() Экспорт
	...
	СоответствиеФорматаФайла.Вставить(Перечисления.ФорматыФайловОбменаПоЗарплатномуПроекту.Версия37, 3.7);
...
Функция НомерТелефонаДляОбменаСБанками(НомерТелефонаПредставление) Экспорт
	...
	НомерТелефона = "";
	Для Шаг = 1 По СтрДлина(НомерТелефонаПредставление) Цикл
		НомерТелефона = НомерТелефона + ?(КодСимвола(НомерТелефонаПредставление, Шаг) >= КодСимвола("0") 
				И КодСимвола(НомерТелефонаПредставление, Шаг) <= КодСимвола("9"),
				Сред(НомерТелефонаПредставление, Шаг, 1),
				"");
	КонецЦикла;
...
// Формирует временную таблицу сотрудников с номерами лицевых счетов по переданной организации и списку сотрудников.
// Создает в МенеджерВременныхТаблиц временную таблицу с именем ВТЛицевыеСчетаСотрудников.
// Описание колонок см. СоздатьПоВременнойТаблицеВТЛицевыеСчетаСотрудников.
//
// Параметры:
//  МенеджерВременныхТаблиц - МенеджерВременныхТаблиц
//  ТолькоРазрешенные       - Булево
...
// Формирует временную таблицу сотрудников со способами выплат по переданной организации и списку сотрудников.
// Создает в МенеджерВременныхТаблиц временную таблицу с именем ВТСпособыВыплатСотрудников.
// Описание колонок см. СоздатьПоВременнойТаблицеВТЛицевыеСчетаСотрудников.
//
// Параметры:
//  МенеджерВременныхТаблиц - МенеджерВременныхТаблиц
...
//                          - Неопределено                       - по всем зарплатным проектам.
//
Процедура СоздатьВТСпособыВыплатСотрудников(МенеджерВременныхТаблиц, ТолькоРазрешенные, СписокСотрудников, Организация, ЗарплатныйПроект = Неопределено)
	...
	|	Сотрудники.Ссылка КАК Сотрудник,
	|	Сотрудники.ФизическоеЛицо КАК ФизическоеЛицо
	|ПОМЕСТИТЬ ВТСписокСотрудниковДляПолученияСпособовВыплатСотрудников
	|ИЗ
	|	Справочник.Сотрудники КАК Сотрудники
	|ГДЕ
	|	Сотрудники.Ссылка В(&СписокСотрудников)
	...
	|	ВТСписокСотрудниковДляПолученияСпособовВыплатСотрудников.Сотрудник КАК Сотрудник
	|ПОМЕСТИТЬ ВТСпособыВыплатСотрудников
	|ИЗ
	|	ВТСписокСотрудниковДляПолученияСпособовВыплатСотрудников КАК ВТСписокСотрудниковДляПолученияСпособовВыплатСотрудников
	...
	|		ПО ВТСписокСотрудниковДляПолученияСпособовВыплатСотрудников.ФизическоеЛицо = ЛицевыеСчетаСотрудниковПоЗарплатнымПроектам.ФизическоеЛицо
	|ГДЕ
	|	ЛицевыеСчетаСотрудниковПоЗарплатнымПроектам.ЗарплатныйПроект = &ЗарплатныйПроект
	|	И ЛицевыеСчетаСотрудниковПоЗарплатнымПроектам.Организация = &Организация
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|УНИЧТОЖИТЬ ВТСписокСотрудниковДляПолученияСпособовВыплатСотрудников";
	...
	Если ЗарплатныйПроект = Неопределено Тогда
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "ЛицевыеСчетаСотрудниковПоЗарплатнымПроектам.ЗарплатныйПроект = &ЗарплатныйПроект", "ИСТИНА");
	КонецЕсли;
...
//
// Возвращаемое значение:
//  Строка - Если система расчетов по банковским картам не найдена, то возвращается Неопределено.
//
...
//
// Параметры:
//  ПервичныйДокумент     - ДокументСсылка.ЗаявкаНаОткрытиеЛицевыхСчетовСотрудников
//  ДанныеДокументов      - ТаблицаЗначений - данные документов, удостоверяющих личность:
//     * ДокументВид      - Строка - наименование вида документа.
//     * КодВидаДокумента - Строка - код МВД документа.
//     * ДокументСерия    - Строка - серия документа.
//     * ДокументНомер    - Строка - номер документа.
//
// Возвращаемое значение:
//  ТаблицаЗначений:
//     * ДокументВид      - Строка - наименование вида документа.
...
//     * ДокументСерия    - Строка - серия документа.
//     * ДокументНомер    - Строка - номер документа.
//     * ФизическоеЛицо   - СправочникСсылка.ФизическиеЛица - физическое лицо, которому принадлежит документ.
//
...
//  ИмяЭлемента      - Строка - имя элемента формы, в котором заполняется список выбора.
//
...
Процедура СброситьКешУсловийВидимостиКомандОбменаСБанками() Экспорт

	ОбновитьПовторноИспользуемыеЗначения();

КонецПроцедуры

#Область ДокументыОбменаСБанком
...
Функция ТипыДокументовВыгружаемыхВБанк()
	
	Типы = Новый Массив;
	
	ОбщегоНазначенияКлиентСервер.ДополнитьМассив(Типы, Метаданные.ОпределяемыеТипы.ДокументЗачисленияЗарплаты.Тип.Типы());
	Типы.Добавить(Тип("ДокументСсылка.ЗаявкаНаОткрытиеЛицевыхСчетовСотрудников"));
	Типы.Добавить(Тип("ДокументСсылка.ЗаявкаНаЗакрытиеЛицевыхСчетовСотрудников"));
	
	Возврат Типы

КонецФункции	

Функция ТипыДокументовЗагружаемыхИзБанка()
	
	Типы = ТипыДокументовВыгружаемыхВБанк();

	Типы.Добавить(Тип("ДокументСсылка.ПодтверждениеЗачисленияЗарплаты"));
	Типы.Добавить(Тип("ДокументСсылка.ПодтверждениеОткрытияЛицевыхСчетовСотрудников"));
	Типы.Добавить(Тип("ТаблицаФормы"));
...
Процедура ВыгрузитьФайлыОбменаСБанком(СтруктураПараметров, АдресХранилища) Экспорт
	ОбменСБанкамиПоЗарплатнымПроектамВнутренний.ВыгрузитьФайлыОбменаСБанком(СтруктураПараметров, АдресХранилища);
КонецПроцедуры
...
Процедура ЗаполнитьПоляФормата(ОбъектXDTO, ТекущиеДанные, ДанныеСтрокДокумента, СоответствиеПолей, ОбязательныеПоля, СоответствиеПреобразованияЗначений, Ошибки)
	...
	Для Каждого СвойствоОбъектаXDTO Из ОбъектXDTO.Свойства() Цикл
		...
		Если СвойствоОбъектаXDTO.Тип.Имя = "string" ИЛИ СвойствоОбъектаXDTO.Тип.БазовыйТип.Имя = "string" Тогда
			...
			Если СвойствоОбъектаXDTO.Тип.Имя = "СНИЛС" Тогда
			
				ЗначениеПоля = СокрЛП(СтрЗаменить(СтрЗаменить(ЗначениеПоля, "-", ""), " ", ""));
...
Функция КорневыеСвойства(ВидОперации) Экспорт
	...
	ИначеЕсли ВидОперации = "ПодтверждениеЗачисленияЗарплаты" Тогда
		...
		СчетаПК.Добавить("КомиссияРеестра");
...
Функция СоответствиеПолейФайла(ВидОперации, Знач ФорматФайла = Неопределено)
	...
	Если ВидОперации = "ВедомостьНаВыплатуЗарплатыВБанк" Тогда
		...
		КонецЕсли;
		Если СоответствиеФорматаФайла.Получить(ФорматФайла) >= 3.7 Тогда
			СоответствиеЗачислениеЗарплаты.Вставить("ИдентификаторПлатежа", "ИдентификаторПлатежа");
			СоответствиеЗачислениеЗарплаты.Вставить("НомерКарты", "БанковскаяКарта");
			СоответствиеЗачислениеЗарплаты.Вставить("СНИЛС", "СНИЛС");
			СоответствиеЗачислениеЗарплаты.Вставить("БИК", "БИКБанкаСчета");
			СоответствиеЗачислениеЗарплаты.Вставить("НомерТелефона", "НомерТелефона");
		КонецЕсли;
	...
	ИначеЕсли ВидОперации = "ПодтверждениеЗачисленияЗарплаты" Тогда
		...
		Если СоответствиеФорматаФайла.Получить(ФорматФайла) >= 3.7 Тогда
			СоответствиеПолей.Вставить("ИдентификаторПлатежа", "ИдентификаторПлатежа");
			СоответствиеПолей.Вставить("Комиссия", "Комиссия");
			СоответствиеПолей.Вставить("БИК", "БИКБанкаСчета");
			СоответствиеПолей.Вставить("ЛицевойСчетУдовлетворяетТребованиям", "ЛицевойСчетУдовлетворяетТребованиям");
			СоответствиеПолей.Вставить("НомерСчетаУдовлетворяющегоТребованиям", "НомерСчетаУдовлетворяющегоТребованиям");
			СоответствиеПолей.Вставить("БИКСчетаУдовлетворяющегоТребованиям", "БИКСчетаУдовлетворяющегоТребованиям");
		КонецЕсли;
...
Функция СоответствиеПреобразованияЗначений(ВидОперации) Экспорт
	...
	Если ВидОперации = "ВедомостьНаВыплатуЗарплатыВБанк" Тогда
		...
		СоответствиеПреобразованияЗначений.Вставить(Перечисления.ВидыДоходовИсполнительногоПроизводства.КомпенсацииНеоблагаемыеПериодические, "2");
		СоответствиеПреобразованияЗначений.Вставить(Перечисления.ВидыДоходовИсполнительногоПроизводства.КомпенсацииОблагаемыеПериодические,   "3");
		СоответствиеПреобразованияЗначений.Вставить(Перечисления.ВидыДоходовИсполнительногоПроизводства.КомпенсацииНеоблагаемыеЕдиновременные,"4");
		СоответствиеПреобразованияЗначений.Вставить(Перечисления.ВидыДоходовИсполнительногоПроизводства.КомпенсацииОблагаемыеЕдиновременные,  "5");
...
Функция СоответствиеТипов(ВидОперации) Экспорт
	...
	Если ВидОперации = "ПодтверждениеЗачисленияЗарплаты" Тогда
		...
		СоответствиеТипов.Вставить("ИдентификаторПлатежа", Новый ОписаниеТипов("УникальныйИдентификатор"));
		СоответствиеТипов.Вставить("БИК", Новый ОписаниеТипов("Строка", , Новый КвалификаторыСтроки(9)));
		СоответствиеТипов.Вставить("ЛицевойСчетУдовлетворяетТребованиям", Новый ОписаниеТипов("Булево"));
		СоответствиеТипов.Вставить("НомерСчетаУдовлетворяющегоТребованиям", Новый ОписаниеТипов("Строка", , Новый КвалификаторыСтроки(20)));
		СоответствиеТипов.Вставить("БИКСчетаУдовлетворяющегоТребованиям", Новый ОписаниеТипов("Строка", , Новый КвалификаторыСтроки(9)));
		СоответствиеТипов.Вставить("Комиссия", Новый ОписаниеТипов("Строка", Новый КвалификаторыЧисла(15, 2)));
...
//
Функция РезультатВыгрузкиФайловОбменаСБанком(Форма, МассивДокументов) Экспорт
	
	НаименованиеЗадания = НСтр("ru = 'Выгрузка файлов для обмена с банком'");
	ИмяЭкспортнойПроцедуры = "ОбменСБанкамиПоЗарплатнымПроектам.ВыгрузитьФайлыОбменаСБанком";
	...
	Для Каждого КорневоеСвойствоXDTO Из Пакет.КорневыеСвойства Цикл
		...
		Для Каждого ЗагружаемоеСвойство Из ОбъектXDTO.Свойства() Цикл
			...
			Если ЗагружаемоеСвойство.Имя = "ИдПервичногоДокумента" И ЗначениеЗаполнено(ПервичныйДокумент) Тогда
				...
				КонецЕсли;
			ИначеЕсли ЗагружаемоеСвойство.Имя = "КомиссияРеестра" Тогда
				СтруктураДанныхДляЗаполненияДокумента.Вставить("КомиссияРеестра", ОбъектXDTO.КомиссияРеестра);
			КонецЕсли;
			
			Если ЗначениеЗаполнено(СтруктураДанныхДляЗаполненияДокумента.ЗарплатныйПроект) Тогда
				ФорматФайла = СтруктураДанныхДляЗаполненияДокумента.ЗарплатныйПроект.ФорматФайла;
			Иначе
				ФорматФайла = ПредопределенноеЗначение("Перечисление.ФорматыФайловОбменаПоЗарплатномуПроекту.Версия36");
			КонецЕсли;
			
			СоответствиеПолей = СоответствиеПолейФайла(ВидПодтверждения, ФорматФайла);
...
Процедура ЗаполнитьКоллекциюИзСвойствXDTO(Коллекция, ОбъектXDTO, СвойстваXDTO, СоответствиеПолей, СоответствиеПреобразованияЗначений, ОбязательныеПоля)
	...
	Для Каждого СвойствоXDTO Из СвойстваXDTO Цикл
		...
		Если ТипЗнч(Коллекция[НаименованиеКолонки]) = Тип("УникальныйИдентификатор") Тогда
			Коллекция[НаименованиеКолонки] = Новый УникальныйИдентификатор(ПреобразованноеЗначениеПоСоответствию(ОбъектXDTO[СвойствоXDTO.Имя], СоответствиеПреобразованияЗначений));
		Иначе
...
Функция ДанныеСотрудниковДляОткрытияЛицевыхСчетов(МенеджерВременныхТаблиц, ДатаПолученияДанных, Организация, ЗарплатныйПроект, СписокФизическихЛиц = Неопределено, ЗаявкаНаОткрытиеЛицевыхСчетов = Неопределено, ПолучитьТолькоЛицевойСчетНеОткрыт = Ложь) Экспорт
	...
	Если ТаблицаКИ.Количество() = 0 Тогда
		АдресМестаРаботы = Новый Структура("Значение, Представление", "", "");
	Иначе
		АдресМестаРаботы = ТаблицаКИ[0];
	КонецЕсли;
...
Функция ПредставлениеУдостоверенияЛичностиИзЭДXML(УдостоверениеЛичности)
	...
	Представление.Добавить(УдостоверениеЛичности.ВидДокумента);
	Если ЗначениеЗаполнено(УдостоверениеЛичности.Серия) Тогда
		Представление.Добавить(СтрШаблон(НСтр("ru = 'серия: %1'"), УдостоверениеЛичности.Серия))
	КонецЕсли;
	Если ЗначениеЗаполнено(УдостоверениеЛичности.Номер) Тогда
		Представление.Добавить(СтрШаблон(НСтр("ru = '№ %1'"), УдостоверениеЛичности.Номер))
	КонецЕсли;
	Если ЗначениеЗаполнено(УдостоверениеЛичности.ДатаВыдачи) Тогда
		Представление.Добавить(СтрШаблон(НСтр("ru = 'выдан: %1 года'"), Формат(УдостоверениеЛичности.ДатаВыдачи, "ДЛФ=ДД")))
	КонецЕсли;
	Если ЗначениеЗаполнено(УдостоверениеЛичности.КемВыдан) Тогда
		Представление.Добавить(УдостоверениеЛичности.КемВыдан)
	КонецЕсли;	
	Если ЗначениеЗаполнено(УдостоверениеЛичности.КодПодразделения) 
		И УдостоверениеЛичности.КодВидаДокумента = Справочники.ВидыДокументовФизическихЛиц.ПаспортРФ Тогда
		Представление.Добавить(СтрШаблон(НСтр("ru = '№ подр. %1'"), УдостоверениеЛичности.КодПодразделения))
	КонецЕсли;	
	
	Возврат СтрСоединить(Представление, ", ")
...
Функция ИсточникиПоЗарплатнымПроектам(Источники)
	
	ТипыДокументовОбменаСБанками = Метаданные.ОпределяемыеТипы.ВладелецОбъектОбменСБанкамиБЗК.Тип.Типы();
	
	Для Каждого ТипИсточника Из ТипыДокументовОбменаСБанками Цикл
		ДокументОбъект = Новый (ТипИсточника);
		Если Источники.Строки.Найти(ДокументОбъект.Метаданные(), "Метаданные") <> Неопределено Тогда
			Возврат Истина;
		КонецЕсли;
	КонецЦикла;
	
	ЖурналДокументов = Метаданные.ЖурналыДокументов.ДокументыОбменаСБанкамиПоЗарплатнымПроектам;
	
	Если Источники.Строки.Найти(ЖурналДокументов, "Метаданные") <> Неопределено Тогда
		Возврат Истина;
	КонецЕсли;
