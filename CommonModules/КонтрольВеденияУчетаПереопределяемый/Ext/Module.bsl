...
Процедура ПроверкаНастроекНалогообложения(Проверка, ПараметрыПроверки) Экспорт
	...
	Результат = РезультатПроверкиНастроекНалогообложения(ПараметрыПроверки.ДатаНачалаПроверки);
	Если Не Результат.Пустой() Тогда
		...
		Пока Выборка.Следующий() Цикл
			...
			УточнениеПроблемы = СтрШаблон(НСтр("ru='На дату (%1) не установлены настройки налогообложения.'"), Формат(Выборка.Период, "ДФ=dd.MM.yyyy"));
			
			Проблема = МодульКонтрольВеденияУчета.ОписаниеПроблемы(Выборка.ВидРасчета, ПараметрыПроверки);
			Проблема.УточнениеПроблемы = УточнениеПроблемы;
			КонтрольВеденияУчета.ЗаписатьПроблему(Проблема, ПараметрыПроверки);
...
Функция РезультатПроверкиНастроекНалогообложения(ДатаНачала = Неопределено)
	...
	Запрос.Текст =
		...
		|ГДЕ
		|	НЕ Начисления.КатегорияНачисленияИлиНеоплаченногоВремени = ЗНАЧЕНИЕ(Перечисление.КатегорииНачисленийИНеоплаченногоВремени.УправленческийУчет)
		|	И НалогообложениеНачисленийСрезПоследних.Начисление ЕСТЬ NULL
		|
		|УПОРЯДОЧИТЬ ПО
