...
Процедура ПриУстановкеЗаголовкаКлиентскогоПриложения(ЗаголовокПриложения, ПриЗапуске) Экспорт
	...
	//для вывода заголовка, без локализатора 	
	...
	Если ПараметрыКлиента.ДоступноИспользованиеРазделенныхДанных Тогда
		...
		Если ПараметрыКлиента.Свойство("СинонимКонфигурации") Тогда
	...
	Иначе
		ШаблонЗаголовка = "%1 / %2";
		ЗаголовокПриложения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонЗаголовка, НСтр("ru = 'Не установлены разделители'"),
			?(ПараметрыКлиента.Свойство("СинонимКонфигурации"),ПараметрыКлиента.СинонимКонфигурации, ПараметрыКлиента.ПодробнаяИнформация));
	КонецЕсли;
