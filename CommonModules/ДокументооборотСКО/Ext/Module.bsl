...
Процедура ОбработатьИсключение(ИнформацияОбОшибке, ВыполняемоеДействие = "") Экспорт
	...
	Подсистема = НСтр("ru = 'Электронный документооборот с контролирующими органами. '", КодОсновногоЯзыка);
...
Функция ДваждыРазобратьОтветНаPostЗапрос(Ответ) Экспорт
	...
	Содержимое = ЭлектронныйДокументооборотСКонтролирующимиОрганами.ПолучитьЗначениеУзлаXML(ДОМ, "string");
	Содержимое = "<?xml version=""1.0"" encoding=""windows-1251""?>" + Символы.ПС + Содержимое;
...
Функция ОрганПодключенХотяБыУОднойОрганизации(Орган) Экспорт
...
Функция СоздатьТекстИнформацииБРО(ПараметрыОтображения, Тариф) Экспорт
	...
	Если Создать Тогда
		...
		Если Тариф = Перечисления.ТарифыОператораЭДО.КадровыеРешения Тогда
			Если ЭтоСЭДО Тогда
				Текст = НСтр("ru = 'Подключите обмен с СФР (бывш. ФСС и ПФР) бесплатно на 3 месяца! Попробуйте Социальный документооборот и другие возможности.'");
			Иначе
				Текст = НСтр("ru = 'Подключите обмен с СФР (бывш. ФСС и ПФР) бесплатно на 3 месяца!'");
			КонецЕсли;
