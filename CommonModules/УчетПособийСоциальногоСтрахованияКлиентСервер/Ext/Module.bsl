...
Функция СреднийЗаработокФСС2011(ПараметрыРасчета)
	...
	ИначеЕсли ПараметрыРасчета.ПрименяетсяФЗ_20_2023 И ПрименятьПредельнуюВеличину Тогда
		МаксимальныйСреднедневнойЗаработок = МаксимальныйСреднедневнойЗаработокДляФЗ_20_2023(ПараметрыРасчета.ДатаНачалаСобытия);
		СреднедневнойЗаработок = Мин(МаксимальныйСреднедневнойЗаработок, СреднедневнойЗаработок);
...
Процедура ДобавитьДанныеПредыдущихСтрахователей(ДанныеРасчета, ПараметрыРасчета)
	...
	Для каждого ПредыдущийСтрахователь Из ПараметрыРасчета.ДанныеСтрахователей Цикл
		...
		Отбор.Очистить();
		Отбор.Вставить("РасчетныйГод", ПредыдущийСтрахователь.Год);
		СтрокаПоГоду = ЭлементДанныхДляРасчетаСреднегоЗаработкаПоОтбору(ДанныеРасчета, Отбор);
		...
		Если СтрокаПоГоду <> Неопределено Тогда			
			...
			Отбор.Очистить();
			Отбор.Вставить("Страхователь", ПредыдущийСтрахователь.Страхователь);
...
Функция НеВключатьПериодВДанныеДляРасчета(Период, ПараметрыРасчета) 
	...
	Возврат (ПараметрыРасчета.НеполныйРасчетныйПериод И Период < '20140301'
		ИЛИ ПараметрыРасчета.ПрименяетсяФЗ_20_2023 И Период < '20220901');
...
Функция УчитываемыйЗаработокФСС(ПараметрыРасчета, ДанныеРасчетаСреднегоЗаработка) Экспорт
	...
	Если ПараметрыРасчета.ПрименятьПредельнуюВеличину 
		И (Не ПараметрыРасчета.НеполныйРасчетныйПериод)
		И (Не ПараметрыРасчета.ПрименяетсяФЗ_20_2023) Тогда
