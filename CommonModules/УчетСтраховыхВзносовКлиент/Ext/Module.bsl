...
Функция ФормаРасшифровкиРегламентированногоОтчета(ИДОтчета, ИДРедакцииОтчета, ИДИменПоказателей, ПараметрыОтчета) Экспорт
	...
	Если ИДОтчета = "РегламентированныйОтчетРасчетПоСтраховымВзносам" Тогда
		...
		ИначеЕсли Год = 2022 Тогда	
			ИмяРасчета = "РасчетПоказателей_РСВ_2022Кв1";
			ДополнитьПараметрыОтчетаРСВза2022год(ПараметрыОтчета, ИмяПоказателя);
		ИначеЕсли Год = 2023 Тогда	
			ИмяРасчета = "РасчетПоказателей_РСВ_2023Кв1";
			ДополнитьПараметрыОтчетаРСВза2023год(ПараметрыОтчета, ИмяПоказателя);
		КонецЕсли;
	...
	ИначеЕсли ИДОтчета = "РегламентированныйОтчетПерсонифицированныеСведенияОФизическихЛицах" Тогда
		ИмяОтчетаРасшифровки = "РасшифровкаПерсонифицированныеСведенияОФизическихЛицах";
		ИмяРасчета = "РасчетПоказателей_ПерсонифицированныеСведенияОФизическихЛицах_2023Кв1";
		ДополнитьПараметрыОтчетаПерсонифицированныеСведенияОФизическихЛицахЗа2023год(ПараметрыОтчета, ИмяПоказателя);
		ИмяНабораДанных = ПараметрыОтчета.ИмяСКД;
	ИначеЕсли ИДОтчета = "РегламентированныйОтчетЕФС1ОСС" Тогда
		ИмяОтчетаРасшифровки = "Расшифровка4ФСС";
		ДополнитьПараметрыОтчетаЕФС1ОСС2023Год(ПараметрыОтчета, ИмяПоказателя);
		ИмяНабораДанных = ПараметрыОтчета.ИмяСКД;
	ИначеЕсли ИДОтчета = "РегламентированныйОтчет4ФСС" Тогда
		...
		ИначеЕсли Год = 2017 Тогда	
			...
			КонецЕсли;
		ИначеЕсли Год = 2022 Тогда	
			ИмяРасчета = "РасчетПоказателей_4ФСС_2022";
			ДополнитьПараметрыОтчета4ФССЗа2022Год(ПараметрыОтчета, ИмяПоказателя);
		КонецЕсли;
		ИмяНабораДанных = ПараметрыОтчета.ИмяСКД;
...
Процедура ДополнитьПараметрыОтчетаЕФС1ОСС2023Год(ПараметрыОтчета, ИмяПоказателя)
	...
	Если Раздел = "21" И НомерКолонки > "03" Тогда	
		Если НомерСтроки = "001" Или НомерСтроки = "002" Или НомерСтроки = "004" Тогда
			ИмяНабораДанных = "Доходы2023";
			ЗаголовокРасшифровки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Подраздел 2.1, строка %1, графа %2'"),
			Формат(Число(НомерСтроки), "ЧГ="),
			Формат(Число(НомерКолонки), "ЧГ="));
		КонецЕсли;
		Если НомерСтроки = "009" Тогда
			ИмяНабораДанных = "Взносы2023";
			ЗаголовокРасшифровки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Подраздел 2.1, строка %1, графа %2'"),
			Формат(Число(НомерСтроки), "ЧГ="),
			Формат(Число(НомерКолонки), "ЧГ="));
		КонецЕсли;
	ИначеЕсли Раздел = "11" И НомерКолонки > "08" Тогда	
		НомерСтроки = Сред(ИмяПоказателя, 7, 2);
		Если НомерСтроки = "М1" Тогда
			ИмяНабораДанных = "ОсновнойОКВЭД";
			ЗаголовокРасшифровки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Подраздел 2.1.1, строка основного ВЭД, графа %1'"),
			Формат(Число(НомерКолонки), "ЧГ="));
		ИначеЕсли НомерСтроки = "М2" Тогда
			ИмяНабораДанных = "СКЕ2023";
			ЗаголовокРасшифровки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Подраздел 2.1.1, строка СКЕ, графа %1'"),
			Формат(Число(НомерКолонки), "ЧГ="));
		КонецЕсли;
	КонецЕсли;
...
Процедура ДополнитьПараметрыОтчетаПерсонифицированныеСведенияОФизическихЛицахЗа2023год(ПараметрыОтчета, ИмяПоказателя)
	...
	ОписаниеРаздела = "";
	Если ИмяПоказателя = "П000010007001" Тогда
		ИмяНабораДанных = "Доходы"
	КонецЕсли;
	
	Если ЗначениеЗаполнено(ИмяНабораДанных) Тогда
			
		ПараметрыОтчета.Вставить("ИмяСКД", ИмяНабораДанных);
		ПараметрыОтчета.Вставить("ЗаголовокРасшифровки","");
...
Процедура ДополнитьПараметрыОтчетаРСВза2023год(ПараметрыОтчета, ИмяПоказателя)
	...
	ОписаниеРаздела = "";
	Если Раздел = "0011" Тогда
		ОписаниеРаздела = "Подраздел 1 Раздела 1";
		Если НомерСтроки = "061" Или НомерСтроки = "062" Тогда
			ИмяНабораДанных = "ВзносыВРазрезеТарифов2023"
		ИначеЕсли НомерСтроки = "030" Или НомерСтроки = "040" Или НомерСтроки = "045" Или НомерСтроки = "051" Или НомерСтроки = "052" Тогда
			ИмяНабораДанных = "ДоходыВРазрезеТарифов2023"
		ИначеЕсли НомерСтроки = "010" Тогда
			ИмяНабораДанных = "ЧисленностьЗастрахованныхВРазрезеТарифов2023"
		КонецЕсли;
	ИначеЕсли Раздел = "0012" Тогда	
		ОписаниеРаздела = "Подраздел 2 Раздела 1";
		Если НомерСтроки = "050" Тогда
			ИмяНабораДанных = "Подраздел2Взносы2023"
		ИначеЕсли НомерСтроки = "020" Или НомерСтроки = "030" Тогда
			ИмяНабораДанных = "Подраздел2Доходы2023"
		КонецЕсли;
	ИначеЕсли Раздел = "0013" Тогда	
		ОписаниеРаздела = "Подраздел 3 Раздела 1";
		Если НомерСтроки = "050" Тогда
			ИмяНабораДанных = "Подраздел3Взносы2023"
		ИначеЕсли НомерСтроки = "020" Или НомерСтроки = "030" Тогда
			ИмяНабораДанных = "Подраздел3Доходы2023"
		КонецЕсли;
	ИначеЕсли Раздел = "0041" Тогда	
		ОписаниеРаздела = "Подраздел 1 Раздела 4";
		Если НомерСтроки = "070" Или НомерСтроки = "090" Тогда
			ИмяНабораДанных = "Раздел4Взносы2023"
		ИначеЕсли НомерСтроки = "030" Или НомерСтроки = "040" Или НомерСтроки = "050" Тогда
			ИмяНабораДанных = "Раздел4Доходы2023"
		КонецЕсли;
	КонецЕсли;
	...
	Если ЗначениеЗаполнено(ИмяНабораДанных) Тогда
		...
		Если ИмяНабораДанных = "ЧисленностьЗастрахованныхВРазрезеТарифов2023" Тогда
...
Процедура ДополнитьПараметрыОтчетаРСВза2022год(ПараметрыОтчета, ИмяПоказателя)
	...
	Если Раздел = "1111" Тогда
		...
		Если НомерСтроки = "061" Или НомерСтроки = "062" Тогда
			ИмяНабораДанных = "ВзносыВРазрезеТарифов2022"
		ИначеЕсли НомерСтроки = "030" Или НомерСтроки = "040" Или НомерСтроки = "045" Или НомерСтроки = "051" Тогда
			ИмяНабораДанных = "ДоходыВРазрезеТарифов2022"
		ИначеЕсли НомерСтроки = "010" Тогда
			ИмяНабораДанных = "ЧисленностьЗастрахованныхВРазрезеТарифов2022"
		КонецЕсли;
	...
	ИначеЕсли Раздел = "1131" Тогда	
		...
		Если НомерСтроки = "050" Тогда
			ИмяНабораДанных = "Подраздел31Взносы2022"
		ИначеЕсли НомерСтроки = "020" Или НомерСтроки = "030" Тогда
			ИмяНабораДанных = "Подраздел31Доходы2022"
		КонецЕсли;
	ИначеЕсли Раздел = "1132" Тогда	
		ОписаниеРаздела = "Подраздел 1.3.2";
		Если НомерСтроки = "050" Тогда
			ИмяНабораДанных = "Подраздел32Взносы2022"
		ИначеЕсли НомерСтроки = "020" Или НомерСтроки = "030" Тогда
			ИмяНабораДанных = "Подраздел32Доходы2022"
		КонецЕсли;
	ИначеЕсли Раздел = "0111" Тогда	
		ОписаниеРаздела = "Приложение 1.1";
		Если НомерСтроки = "050" Тогда
			ИмяНабораДанных = "Приложение11Взносы2022"
		ИначеЕсли НомерСтроки = "020" Или НомерСтроки = "030" Тогда
			ИмяНабораДанных = "Приложение11Доходы2022"
		КонецЕсли;
	ИначеЕсли Раздел = "0012" Тогда	
	...
	Если ЗначениеЗаполнено(ИмяНабораДанных) Тогда
		...
		Если ИмяНабораДанных = "ЧисленностьЗастрахованныхВРазрезеТарифов2022" Тогда
			ЗаголовокПоля = НСтр("ru='Застрахован (1-да, 0-нет)'");
		КонецЕсли;
...
Процедура ДополнитьПараметрыОтчета4ФССЗа2022Год(ПараметрыОтчета, ИмяПоказателя)
	...
	ИначеЕсли Раздел = "11" Тогда	
		Если НомерСтроки = "ВЭД" Тогда
			Если НомерКолонки > "08" Тогда
				ИмяНабораДанных = "ОсновнойОКВЭД";
				ЗаголовокРасшифровки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Табл. 1.1, строка основного ВЭД, графа %1'"),
				Формат(Число(НомерКолонки), "ЧГ="));
			КонецЕсли;
		ИначеЕсли НомерСтроки = "ЕГО" Тогда
			
		ИначеЕсли НомерКолонки > "08" Тогда
			ИмяНабораДанных = "СКЕ2022";
			ЗаголовокРасшифровки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Табл. 1.1, строка СКЕ, графа %1'"),
				Формат(Число(НомерКолонки), "ЧГ="));
		КонецЕсли;
	КонецЕсли;
