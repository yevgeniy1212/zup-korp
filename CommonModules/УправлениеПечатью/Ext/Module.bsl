...
Функция ИсточникиКомандПечати() Экспорт
	...
	Для Каждого ОбъектМенеджер Из ОбъектыСКомандамиПечати Цикл
        // ЗУПК
		СоставФО = Метаданные.ФункциональныеОпции.ОтключенныйФункционалЗарплатаКадры.Состав;
		Если НЕ СоставФО.Найти(Метаданные.НайтиПоТипу(ТипЗнч(ОбъектМенеджер))) = Неопределено Тогда
			Продолжить;
		КонецЕсли;	
		// ЗУПК
...
Процедура ВставитьКартинкиВHTML(ИмяФайлаHTML) Экспорт
	...
	Для Каждого ФайлКартинки Из ФайлыКартинок Цикл
		...
		ТекстHTML = СтрЗаменить(ТекстHTML, ИмяКаталогаКартинок + ПолучитьРазделительПути() + ФайлКартинки.Имя, КартинкаТекстом);
	КонецЦикла;
...
Функция МакетыПечатныхФорм() Экспорт
    ...
    // ЗУПК
	СоставФО = Метаданные.ФункциональныеОпции.ОтключенныйФункционалЗарплатаКадры.Состав;
	МассивМакетовИсключений = МодификацияКонфигурацииЗУПК.ПолучитьМассивМакетовИсключений();
	// ЗУПК
	...
	Для Каждого КоллекцияОбъектовМетаданных Из КоллекцииОбъектовМетаданных Цикл
		Для Каждого ОбъектМетаданныхКоллекции Из КоллекцияОбъектовМетаданных Цикл
    		...
    		// ЗУПК
			Если НЕ СоставФО.Найти(ОбъектМетаданных) = Неопределено Тогда
				Продолжить;
			КонецЕсли;	
			// ЗУПК
			Для Каждого Макет Из ОбъектМетаданных.Макеты Цикл
	    		// ЗУПК
				Если НЕ МассивМакетовИсключений.Найти(Макет.Имя) = Неопределено Тогда
					Продолжить;
				КонецЕсли;	
				// ЗУПК
