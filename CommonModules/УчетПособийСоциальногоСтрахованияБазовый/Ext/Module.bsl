...
Функция ПриИзмененииСтатусаОтправкиДокумента(Ссылка, СтатусОтправки) Экспорт
	...
	Если ТипЗнч(Ссылка) = Тип("ДокументСсылка.РеестрСведенийНеобходимыхДляНазначенияИВыплатыПособий") Тогда
		...
		Документы.РеестрСведенийНеобходимыхДляНазначенияИВыплатыПособий.ПриИзмененииСтатусаОтправкиДокумента(Ссылка, СтатусОтправки);
...
Процедура ЗаполнитьНастройкиПечатныхФормПоУмолчанию(ОписанияНастроек) Экспорт
	...
	КадровыйЭДО.ДобавитьНастройкуПечатнойФормы(
		ОписанияНастроек,
		Метаданные.Документы.ИсходящаяСправкаОЗаработкеДляРасчетаПособий.Макеты.ПФ_MXL_СправкаРасширенная.Имя,
		Перечисления.СодержимоеДокументов.СодержитЗарплату);
	
	КадровыйЭДО.ДобавитьНастройкуПечатнойФормы(
		ОписанияНастроек,
		Метаданные.Документы.ИсходящаяСправкаОЗаработкеДляРасчетаПособий.Макеты.ПФ_MXL_Справка_2022.Имя,
		Перечисления.СодержимоеДокументов.СодержитЗарплату);
...
Процедура СоздатьВТДанныеДокументовДляРасчетаСреднегоЗаработкаФСС(МенеджерВременныхТаблиц, МассивСсылок) Экспорт
	...
	|				И БольничныйЛист.ДатаНачалаСобытия >= &ДатаОтдельногоРасчетаПособийПоМатеринству
	|			ТОГДА ИСТИНА
	|		ИНАЧЕ ЛОЖЬ
	...
	|	ДанныеДокументаРасчетаСреднего.УчитыватьДниПриостановленияТД КАК УчитыватьДниПриостановленияТД,
	|	ДанныеДокументаРасчетаСреднего.ДнейПриостановленияТДЗаГод1 КАК ДнейПриостановленияТДЗаГод1,
	|	ДанныеДокументаРасчетаСреднего.ДнейПриостановленияТДЗаГод2 КАК ДнейПриостановленияТДЗаГод2,
...
Функция ТаблицыДанныхСреднегоЗаработкаФСС(ИмяДокумента, МассивСсылок) Экспорт
	...
	Если ЕстьПриостановленияТрудовыхДоговоров Тогда
		ДанныеДляРасчета.Вставить("ПриостановленияТрудовыхДоговоров", Результат[2].Выгрузить());
	Иначе
