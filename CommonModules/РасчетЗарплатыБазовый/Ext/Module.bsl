...
Функция РезультатНачисленияРасчетаЗарплаты(Организация, МесяцНачисления, Документ, Подразделение, Сотрудники, ПерваяПоловинаМесяца) Экспорт
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("ЗарплатаКадрыПриложения.ЗарплатаКадрыДляНебольшихОрганизаций.РасчетЗарплаты") Тогда
		...
		Если ПерваяПоловинаМесяца Тогда
			Запрос.УстановитьПараметр("ПланируемаяДатаВыплаты", ДатыВыплаты.Аванс);
		Иначе
			Запрос.УстановитьПараметр("ПланируемаяДатаВыплаты", ДатыВыплаты.Зарплата);
		КонецЕсли;
	КонецЕсли;
	
	КадровыеДанныеДокумента = "ГоловнаяОрганизация,Организация,Подразделение,ДоляНеполногоРабочегоВремени,ТарифнаяСтавка,ПроцентСевернойНадбавки,ДатаРождения,ДатаЗавершенияРаботы";
...
Функция ТаблицаЗаполненияНачислений(МенеджерВременныхТаблиц, Организация, МесяцНачисления) Экспорт
	...
	Запрос.УстановитьПараметр("КатегорияДоходаНадбавок", Перечисления.КатегорииДоходовНДФЛ.ОплатаТруда);
	Если ОбщегоНазначения.ПодсистемаСуществует("ЗарплатаКадрыПриложения.ЗарплатаКадрыДляНебольшихОрганизаций.РасчетЗарплаты") Тогда
		МодульРасчетЗарплатыДляНебольшихОрганизаций = ОбщегоНазначения.ОбщийМодуль("РасчетЗарплатыДляНебольшихОрганизаций");
		Запрос.УстановитьПараметр("КатегорияДоходаНадбавок",
			МодульРасчетЗарплатыДляНебольшихОрганизаций.КатегорияДоходаНадбавок(Организация, МесяцНачисления));
	КонецЕсли;
	...
	Запрос.Текст =
		...
		|			И (Начисления.КатегорияДохода = &КатегорияДоходаНадбавок)
		|		ЛЕВОЕ СОЕДИНЕНИЕ ВТСотрудникиОрганизации КАК КадровыеДанныеСотрудников
...
Процедура ЗаполнитьОтработанноеВремя(СтрокиНачислений, МесяцНачисления, Организация, ПерваяПоловинаМесяца) Экспорт
	...
	Если КалендарьОрганизации <> Неопределено Тогда
		...
		Если ПерваяПоловинаМесяца И МодульРасчетЗарплатыДляНебольшихОрганизацийПовтИсп <> Неопределено Тогда
			ДатыВыплаты = МодульРасчетЗарплатыДляНебольшихОрганизацийПовтИсп.ДатыВыплатыОрганизации(
				Организация, МесяцНачисления);
			КонецПериодаНачисления = ДатыВыплаты.Аванс;
		КонецЕсли;
...
Процедура СоздатьВТДляРасчетаНДФЛиВзносов(МенеджерВременныхТаблиц, Объект, Сотрудники = Неопределено) Экспорт
	...
	СоздатьВТНачисленияСтраховыеВзносы(МенеджерВременныхТаблиц, Объект.Организация, Объект.МесяцНачисления, "ВТНачисленияСтраховыеВзносы");
...
Функция РезультатРасчетаНДФЛ(МенеджерВременныхТаблиц, Объект, ДатаОперации, ПланируемаяДатаВыплатыВТаблицеНачислений = Ложь) Экспорт
	...
	// Расчет НДФЛ
	РезультатРасчетаНДФЛ = УчетНДФЛ.РезультатРасчетаНДФЛ(МенеджерВременныхТаблиц, Объект.Ссылка, Объект.Организация, Объект.МесяцНачисления, ОкончательныйРасчет, ПланируемаяДатаВыплаты, НаборыЗаписей);
...
Функция РезультатРасчетаКорректировкиВыплаты(Объект, МесяцНачисленияИмя = "МесяцНачисления") Экспорт
	...
	КЗачетуВозврату.Свернуть("ФизическоеЛицо", "КорректировкаВыплаты");
