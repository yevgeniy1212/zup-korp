...
Процедура ОбработатьЭлектронныеДокументы(МассивСсылокНаОбъект, Действия, СообщениеОбмена = Неопределено) Экспорт
	...
	Если ЗначениеЗаполнено(Фрод) Тогда
		ДополнительныеПараметры.Вставить("Фрод", Фрод);
	Иначе
		...
		Если Устройства <> Неопределено И Устройства.Количество() > 0 Тогда
			...
			Для Каждого ДанныеСертификата Из ДанныеСертификатов Цикл
				...
				Если ПарольУстановлен Тогда
					Соответствие = Новый Соответствие;
					СтруктураСертификата = СвойстваСертификата(ДанныеСертификата.СертификатСсылка);
			...
			Если НЕ ПарольУстановлен Тогда
				...
				Для Каждого Сертификат Из СписокВыбораСертификата Цикл
					СтруктураСертификата = СвойстваСертификата(Сертификат);
...
Процедура ПроверитьНеобходимостьУстановкиПинКода(ВнешнийПодключаемыйМодуль, Параметры) Экспорт
	...
	Если ВнешнийПодключаемыйМодуль <> Неопределено Тогда
		...
		РеквизитыСертификата = СвойстваСертификата(СертификатСсылка);
...
Процедура СформироватьЭД(ПараметрКоманды, ПараметрыВыполненияКоманды) Экспорт
	...
	ОбщегоНазначенияБЭДКлиент.ВыполнитьПроверкуПроведенияДокументов(
...
Процедура СформироватьПодписатьОтправитьЭД(ПараметрКоманды, ПараметрыВыполненияКоманды) Экспорт
	...
	Обработчик = Новый ОписаниеОповещения(
		"СформироватьПодписатьОтправитьЭДПослеПроверкиПроведения", ЭтотОбъект, ПараметрыВыполненияКоманды);
	ОбщегоНазначенияБЭДКлиент.ВыполнитьПроверкуПроведенияДокументов(
		ПараметрКоманды, Обработчик, ПараметрыВыполненияКоманды.Источник);
	...
	Если ТипЗнч(СоотвСертификатовИИхСтруктур) = Тип("Соответствие") Тогда
		...
		Для Каждого КлючИЗначение Из СоотвСертификатовИИхСтруктур Цикл
			...
			Если НЕ ЗначениеЗаполнено(ПараметрыСертификата) Тогда
				ПараметрыСертификата = СвойстваСертификата(Сертификат);
			КонецЕсли;
...
Процедура ПодключитьКомпоненту(Оповещение, Идентификатор, ТекстПояснения = "") Экспорт
	...
	Если Идентификатор = ОбменСБанкамиКлиентСервер.ИдентификаторВнешнейКомпонентыСбербанк() Тогда
		ПодключитьКомпонентуСбербанк(Оповещение, Идентификатор, , ПараметрыПодключения);
	Иначе
...
// Выбирает какую компоненту для Сбербанка подключить в зависимости от вида и версии ОС и подключает эту компоненту
// Т.к. для Windows7 нужна особая версия компоненты, для остальных ОС используется штатный механизм подключения компоненты.
//
// Параметры:
//  Оповещение - ОписаниеОповещения - описание оповещения о подключении со следующими параметрами:
//      * Результат - Структура - результат подключения компоненты:
...
//
Процедура ПодключитьКомпонентуСбербанк(Оповещение, Идентификатор, Версия = Неопределено,
	ПараметрыПодключения = Неопределено) Экспорт
	
	Если ОбщегоНазначенияКлиент.ЭтоWindowsКлиент() Тогда
		ЭтоWindows7 = ЭтоWindows7();
		Если ЭтоWindows7 = Неопределено Тогда
			ДополнительныеПараметры = Новый Структура;
			ДополнительныеПараметры.Вставить("Оповещение", Оповещение);
			...
			ОповещениеПослеПодтвержденияВерсииWindows = Новый ОписаниеОповещения(
				"ПослеПодтвержденияВерсииWindows", ЭтотОбъект, ДополнительныеПараметры);
			СписокКнопок = Новый СписокЗначений;
			СписокКнопок.Добавить("Windows7", НСтр("ru = 'Windows 7'"));
			СписокКнопок.Добавить("ДругаяВерсияWindows", НСтр("ru = 'Другая версия Windows'"));
			ПоказатьВопрос(
				ОповещениеПослеПодтвержденияВерсииWindows,
				НСтр("ru = 'Укажите версию Windows на компьютере, с которого подключаетесь к Сбербанку'"),
				СписокКнопок,
				,
				"ДругаяВерсияWindows",
				НСтр("ru = 'Подключение 1С:ДиректБанк'"));
			Возврат;
		КонецЕсли;
		НачатьПодключениеКомпонентыСбербанк(ЭтоWindows7, Оповещение, Идентификатор, ПараметрыПодключения);
	Иначе
		ВнешниеКомпонентыКлиент.ПодключитьКомпоненту(
			Оповещение,
			Идентификатор,
...
Процедура ПослеПодтвержденияВерсииWindows(Результат, ДополнительныеПараметры) Экспорт
	
	Если Результат = "Windows7" Тогда
		ОбменСБанкамиСлужебныйКлиент.СохранитьНастройкуПользователяДиректБанк(
			"VPNKeyTLS",
			"ВерсииWindowsКлиентскихПриложений",
			"Windows7");
		НачатьПодключениеКомпонентыСбербанк(
			Истина, 
			ДополнительныеПараметры.Оповещение, 
			ДополнительныеПараметры.Идентификатор, 
			ДополнительныеПараметры.ПараметрыПодключения);
	Иначе
		ОбменСБанкамиСлужебныйКлиент.СохранитьНастройкуПользователяДиректБанк(
			"VPNKeyTLS",
			"ВерсииWindowsКлиентскихПриложений",
...
Процедура НачатьПодключениеКомпонентыСбербанк(ЭтоВерсияОСWindows7, Оповещение, Идентификатор, ПараметрыПодключения)
	
	Если ЭтоВерсияОСWindows7 Тогда
		
		ПодключаемыйМодуль = ЗначениеИзКешаСбербанк("VPNKeyTLS_1_2_9_6");
		Если (ПодключаемыйМодуль <> Неопределено) И (ПодключаемыйМодуль <> NULL) Тогда
			СтруктураВозврата = Новый Структура;
			СтруктураВозврата.Вставить("Подключено", Истина);
			СтруктураВозврата.Вставить("ПодключаемыйМодуль", ПодключаемыйМодуль);
			ВыполнитьОбработкуОповещения(Оповещение, СтруктураВозврата);
			Возврат;
		КонецЕсли;
		
		ДополнительныеПараметры = Новый Структура;
		ДополнительныеПараметры.Вставить("ОповещениеПослеПодключения", Оповещение);
		ДополнительныеПараметры.Вставить("ПараметрыПодключения", ПараметрыПодключения);
		ДополнительныеПараметры.Вставить("ПервичноеПодключение", Истина);
		ДополнительныеПараметры.Вставить("Идентификатор", Идентификатор);
		ОповещениеПослеПопыткиПодключения = Новый ОписаниеОповещения(
			"ПослеПопыткиПодключенияКомпонентыVPNKeyTLS_1_2_9_6", 
			ЭтотОбъект,
			ДополнительныеПараметры,
			"ПослеПопыткиПодключенияКомпонентыVPNKeyTLS_1_2_9_6ПриОбработкеОшибки",
			ЭтотОбъект);
		НачатьПодключениеВнешнейКомпоненты(ОповещениеПослеПопыткиПодключения, "Обработка.ОбменСБанками.Макет.VPNKeyTLS_1_2_9_6", Идентификатор);
	Иначе
		ВнешниеКомпонентыКлиент.ПодключитьКомпоненту(
			Оповещение, 
			Идентификатор, 
...
Процедура ПослеПопыткиПодключенияКомпонентыVPNKeyTLS_1_2_9_6(Подключено, ДополнительныеПараметры) Экспорт
	
	ИнициализацияУспешна = Ложь;
	СтруктураВозврата = РезультатПодключенияКомпоненты();
	Если Подключено Тогда
		ПодключаемыйМодуль = Новый("AddIn." + ОбменСБанкамиКлиентСервер.ИдентификаторВнешнейКомпонентыСбербанк() + "."
									+ ОбменСБанкамиКлиентСервер.ИдентификаторВнешнейКомпонентыСбербанк());
									
		ЗакешироватьПараметрСбербанка("VPNKeyTLS_1_2_9_6", ПодключаемыйМодуль);
		СтруктураВозврата.Вставить("Подключено", Истина);
		СтруктураВозврата.Вставить("ПодключаемыйМодуль", ПодключаемыйМодуль);
		ИнициализацияУспешна = Истина;
		ВыполнитьОбработкуОповещения(ДополнительныеПараметры.ОповещениеПослеПодключения, СтруктураВозврата);
	КонецЕсли;
	...
	Если ДополнительныеПараметры.ПервичноеПодключение Тогда
		Оповещение = Новый ОписаниеОповещения(
			"ПослеПопыткиУстановкиКомпонентыVPNKeyTLS_1_2_9_6", 
			ЭтотОбъект, 
			ДополнительныеПараметры,
			"ПослеПопыткиУстановкиКомпонентыVPNKeyTLS_1_2_9_6ПриОбработкеОшибки",
			ЭтотОбъект); 
		НачатьУстановкуВнешнейКомпоненты(Оповещение, "Обработка.ОбменСБанками.Макет.VPNKeyTLS_1_2_9_6");
	Иначе
		СтруктураВозврата.ОписаниеОшибки = НСтр("ru = 'Не удалось подключить внешнюю компоненту'");
		ВыполнитьОбработкуОповещения(ДополнительныеПараметры.ОповещениеПослеПодключения, СтруктураВозврата);
	КонецЕсли;
...
Процедура ПослеПопыткиПодключенияКомпонентыVPNKeyTLS_1_2_9_6ПриОбработкеОшибки(ИнформацияОбОшибке, СтандартнаяОбработка, ДополнительныеПараметры) Экспорт
	
	СтандартнаяОбработка = Ложь;
	
	ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru = 'Не удалось подключить внешнюю компоненту ""%1"" на клиенте по причине:
		           |%2'"),
		ДополнительныеПараметры.Идентификатор,
	...
	Оповещение = ДополнительныеПараметры.ОповещениеПослеПодключения;
...
Процедура ПослеПопыткиУстановкиКомпонентыVPNKeyTLS_1_2_9_6(ДополнительныеПараметры) Экспорт
	
	ДополнительныеПараметры.ПервичноеПодключение = Ложь;
	ОповещениеПослеПопыткиПодключения = Новый ОписаниеОповещения(
		"ПослеПопыткиПодключенияКомпонентыVPNKeyTLS_1_2_9_6", ЭтотОбъект, ДополнительныеПараметры);
	Идентификатор = ОбменСБанкамиКлиентСервер.ИдентификаторВнешнейКомпонентыСбербанк();
	НачатьПодключениеВнешнейКомпоненты(ОповещениеПослеПопыткиПодключения, "Обработка.ОбменСБанками.Макет.VPNKeyTLS_1_2_9_6", Идентификатор);
		
КонецПроцедуры

Процедура ПослеПопыткиУстановкиКомпонентыVPNKeyTLS_1_2_9_6ПриОбработкеОшибки(ИнформацияОбОшибке, СтандартнаяОбработка, ДополнительныеПараметры) Экспорт
	
	СтандартнаяОбработка = Ложь;
	
	ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru = 'Не удалось установить внешнюю компоненту ""%1"" на клиенте по причине:
		           |%2'"),
		ДополнительныеПараметры.Идентификатор,
		КраткоеПредставлениеОшибки(ИнформацияОбОшибке));
	...
	Если Не ПустаяСтрока(ТекстОшибки) Тогда
		ЖурналРегистрацииКлиент.ДобавитьСообщениеДляЖурналаРегистрации(
			НСтр("ru = 'Установка внешней компоненты на клиенте'", ОбщегоНазначенияКлиент.КодОсновногоЯзыка()),
			"Ошибка", ТекстОшибки,, Истина);
	КонецЕсли;
...
//
Функция ПредставлениеОшибкиПодписания(КлючОшибки)
...
Процедура ОпределитьСтатусОчереднойПодписиПрисоединенногоФайла(ДополнительныеПараметры)
	
	Если ДополнительныеПараметры.Подписи.Количество() = 0 Тогда
		ОбменСБанкамиСлужебныйВызовСервера.СохранитьРезультатыПроверкиПодписейПрисоединенногоФайла(
			ДополнительныеПараметры.ПрисоединенныйФайл, ДополнительныеПараметры.РезультатПроверки);
	...
	Оповещение = Новый ОписаниеОповещения(
		"ПослеОпределенияСтатусаОчереднойПодписиПрисоединенногоФайла", ЭтотОбъект, ДополнительныеПараметры);
...
Процедура ПослеОпределенияСтатусаОчереднойПодписиПрисоединенногоФайла(Результат, ДополнительныеПараметры) Экспорт
	...
	ИначеЕсли ТипЗнч(Результат) = Тип("Строка") Тогда
		...
		ТекстСообщения = НСтр("ru = 'При проверке подписи присоединенного файла произошла ошибка.
									|Присоединенный файл: %1,
									|Текст ошибки: %2'");
		
		ТекстСообщения = СтрШаблон(ТекстСообщения, ДополнительныеПараметры.ПрисоединенныйФайл, Результат);
		ОбработатьОшибку(ВидОперации, ТекстСообщения, ТекстСообщения, ДополнительныеПараметры.ПрисоединенныйФайл);
		СтруктураЗаписи.Вставить("ПодписьВерна", Ложь);
		СтруктураЗаписи.Вставить("ДатаПроверкиПодписи", ОбщегоНазначенияКлиент.ДатаСеанса());
		ДополнительныеПараметры.РезультатПроверки.Добавить(СтруктураЗаписи);
	КонецЕсли;

	ОпределитьСтатусОчереднойПодписиПрисоединенногоФайла(ДополнительныеПараметры)
...
Функция ПарольКСертификатуПолучен(СертификатЭП, ПарольСертификата, НаВремяСеанса = Ложь) Экспорт
	...
	Если ПарольСертификата = Неопределено Тогда
		...
		ПарольСертификата = ОбменСБанкамиСлужебныйВызовСервера.ПаролиСертификатов(
			ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(СертификатЭП))[СертификатЭП];
...
Процедура ВыполнитьДействияПослеОтправки(Параметры)
	...
	Если ОбменСБанкамиКлиентСервер.ЕстьДействие(Параметры.Действия, "Показать") Тогда
		Для Каждого ТекЭл Из Параметры.Результат.МассивНовыхСообщенийОбмена Цикл
			ОткрытьЭДДляПросмотра(ТекЭл);
		КонецЦикла;
	ИначеЕсли ОбменСБанкамиКлиентСервер.ЕстьДействие(Параметры.Действия, "Подписать") Тогда
		Результат = Параметры.Результат;
		...
		Если Параметры.Свойство("МассивСсылокНаОбъект", МассивСсылокНаОбъект)
			...
			Для Каждого КлючИЗначение Из СоотвСообщенийОбменаИОшибокПодписи Цикл
				СообщениеОбОшибке = ПредставлениеОшибкиПодписания(КлючИЗначение.Значение);
...
Процедура ДобавитьПодписиВЭДПослеПолученияСертификатов(МассивСертификатовИПодписей, ДополнительныеПараметры) Экспорт
	...
	ИначеЕсли ТипЗнч(ДополнительныеПараметры.СообщениеОбмена) = Тип("СправочникСсылка.ПисьмоОбменСБанкамиПрисоединенныеФайлы") Тогда
		ОпределитьСтатусыПодписейПрисоединенногоФайла(
			ДополнительныеПараметры.ОповещениеПослеДобавленияИПроверкиПодписей,
			ДополнительныеПараметры.СообщениеОбмена);
	Иначе
...
Процедура СформироватьПодписатьОтправитьЭДПослеПроверкиПроведения(ПараметрКоманды, ПараметрыВыполненияКоманды) Экспорт
	...
	ОбработатьЭД(МассивСсылок, "СформироватьПодписатьОтправить", , ПараметрыВыполненияКоманды);
...
//
Процедура ОбработатьЭД(
	Знач МассивСсылокНаОбъект,
	Действия,
	Знач СообщениеОбмена = Неопределено,
	...
	Если ОбменСБанкамиКлиентСервер.ЕстьДействие(Действия, "Сформировать") Тогда
		СтандартнаяОбработка = Истина;
		ОбработчикОповещения = Новый ОписаниеОповещения(
		...
		ОбменСБанкамиКлиентПереопределяемый.ПриФормированииЭлектронныхДокументов(
			ОбработчикОповещения, МассивСсылокНаОбъект, ПараметрыВыполненияКоманды, СтандартнаяОбработка);
...
Процедура ПродолжитьОбработкуДокументовПослеПереопределения(МассивСсылокНаОбъект, ДополнительныеПараметры) Экспорт
	...
	Если Не ЗначениеЗаполнено(Фрод) И МожноПолучитьMAC() Тогда
		...
		#Если МобильныйКлиент Тогда
			ОтправитьДокументыПослеПолученияФрода(Истина, ДополнительныеПараметры);
		#Иначе
			...
			Если РезультатВыполнения.Свойство("ВыбранныйСертификат") Тогда
				...
				Если МассивСообщенийОбмена = Неопределено Тогда
					...
					СоответствиеСертификатовПодписаннымЭд.Вставить(
						РезультатВыполнения.ВыбранныйСертификат.Ссылка, МассивСообщенийОбмена);
				КонецЕсли;
			КонецЕсли;
...
//
Функция КоличествоПодписанныхЭД(РезультатВыполнения)
...
Функция АктуаленКэшПароляСертификатаЧерезДополнительнуюОбработку(ДанныеСертификата, ВнешнийПодключаемыйМодуль)
	...
	РеквизитыСертификата = СвойстваСертификата(ДанныеСертификата.СертификатПодписи);
...
Процедура ПослеПолученияИнформацииОСетевыхАдаптерах(ИнформацияОСетевыхАдаптерах, ДополнительныеПараметры) Экспорт
	...
	Для Каждого Элемент Из ИнформацияОСетевыхАдаптерах Цикл
		...
		МассивIP.Добавить(Элемент.IPАдрес);
	КонецЦикла;
	
	ДобавитьЗначениеВКеш("Фрод", Новый Структура("MAC, IP", МассивMAC, МассивIP));
	
	ВыполнитьОбработкуОповещения(ДополнительныеПараметры.ОповещениеПослеПолученияФрода, Истина);
	
КонецПроцедуры

Процедура ОпределитьСтатусыПодписейПрисоединенногоФайла(Оповещение, ПрисоединенныйФайл)

	ДанныеПрисоединенногоФайла = ОбменСБанкамиСлужебныйВызовСервера.ДанныеПрисоединенногоФайла(ПрисоединенныйФайл);
	...
	ДополнительныеПараметры = Новый Структура;
	ДополнительныеПараметры.Вставить("ДвоичныеДанныеЭД", ДанныеПрисоединенногоФайла.ДанныеЭД);
	ДополнительныеПараметры.Вставить("Подписи", ДанныеПрисоединенногоФайла.Подписи);
...
Процедура ПослеОтправкиПакетаЧерезВК(РезультатВызова, ПараметрыВызова, ДополнительныеПараметры) Экспорт
	...
	СтатусПакетаОтправлен = ПредопределенноеЗначение("Перечисление.СтатусыТранспортныхСообщенийБЭД.Отправлен");
	...
	Иначе // выполнено
		...
		Иначе
			...
			Если ЗначениеЗаполнено(РезультатЗадания.Сообщения) Тогда
				ВывестиСообщения(РезультатЗадания.Сообщения, ИдентификаторНазначения);
			КонецЕсли;
			ВыполнитьОбработкуОповещения(ДополнительныеПараметры.ОповещениеПослеПолученияВыписки, СтруктураВозврата);
		КонецЕсли;
	КонецЕсли;
...
Процедура ОбработатьОшибку(ВидОперации, Знач ПодробныйТекстОшибки, Знач ТекстСообщения = "", СсылкаНаОбъект = Неопределено)
...
Функция СвойстваСертификата(Сертификат)
	...
	СвойстваСертификатов = КриптографияБЭДВызовСервера.СвойстваСертификатов(Сертификаты);
	Для Каждого Свойство Из СвойстваСертификатов Цикл
		Свойство.Значение.Вставить("ДвоичныеДанныеСертификата", Свойство.Значение.ДанныеСертификата);
	КонецЦикла;
...
Процедура ВывестиСообщения(СообщенияПользователю, ИдентификаторНазначения)
...
// Определяет, работает ли пользователь в ОС Windows7 или нет
//
// Возвращаемое значение:
//  ЭтоWindows7 - Булево, Неопределено - принимает следующие значения:
//		Истина - это ОС Windows7
//		Ложь - это не ОС Windows7
//		Неопределено - не удалось определить версию ОС Windows
//
Функция ЭтоWindows7() Экспорт

	ЭтоWindows7 = Неопределено;
	ИспользоватьАвтоматическоеОпределениеWindows7 = ПолучитьНастройкуПользователяДиректБанк(
		"VPNKeyTLS",
	...
	#Если ВебКлиент Тогда
		// Если 1С открыта в веб-браузере, то ИнформацияПрограммыПросмотра для Windows7 
		//  обычно определяется как Windows NT 6.1
		// Для избежания появления лишних символов и регистра символов букв проводим
		//  упрощение представления этой строки
		ИнформацияПрограммыПросмотра = НРег(СокрЛП(СисИнфо.ИнформацияПрограммыПросмотра));
		ИнформацияПрограммыПросмотра = СтрЗаменить(ИнформацияПрограммыПросмотра, " ", "");
		ИнформацияПрограммыПросмотра = СтрЗаменить(ИнформацияПрограммыПросмотра, Символы.НПП, "");
		Если СтрНайти(ИнформацияПрограммыПросмотра, "windowsnt6.1") > 0 Тогда
			ЭтоWindows7 = Истина;
		ИначеЕсли СтрНайти(ИнформацияПрограммыПросмотра, "windows") > 0 Тогда
			ЭтоWindows7 = Ложь;
		КонецЕсли;
	#Иначе
		// Если 1С открыта НЕ в веб-браузере, то ВерсияОС для Windows7 
		//  обычно определяется как Windows 7
		// Для избежания появления лишних символов и регистра символов букв проводим
		//  упрощение представления этой строки
		ВерсияОС = НРег(СокрЛП(СисИнфо.ВерсияОС));
		ВерсияОС = СтрЗаменить(ВерсияОС, " ", "");
		ВерсияОС = СтрЗаменить(ВерсияОС, Символы.НПП, "");
		Если СтрНайти(ВерсияОС, "windows7") > 0 Тогда
			ЭтоWindows7 = Истина;
		ИначеЕсли СтрНайти(ВерсияОС, "windows") > 0 Тогда
			ЭтоWindows7 = Ложь;
		КонецЕсли;
	#КонецЕсли
	
	Если ЭтоWindows7 = Неопределено Тогда
		ВерсииWindowsКлиентскихПриложений = ПолучитьНастройкуПользователяДиректБанк(
			"VPNKeyTLS",
			"ВерсииWindowsКлиентскихПриложений");
		Если ВерсииWindowsКлиентскихПриложений = "Windows7" Тогда
			ЭтоWindows7 = Истина;
		ИначеЕсли ВерсииWindowsКлиентскихПриложений = "ДругаяВерсияWindows" Тогда
			ЭтоWindows7 = Ложь;
		КонецЕсли;
	КонецЕсли;
...
#Область СохраняемыеНастройкиПользователяДиректБанк

Процедура СохранитьНастройкуПользователяДиректБанк(КлючНастроек, ИмяПараметра, ЗначениеПараметра) Экспорт
	
	СисИнфо = Новый СистемнаяИнформация;
	ИдентификаторКлиента = СисИнфо.ИдентификаторКлиента;
	ОбменСБанкамиСлужебныйВызовСервера.СохранитьНастройкуПользователяДиректБанк(ИдентификаторКлиента, КлючНастроек, ИмяПараметра, ЗначениеПараметра)
			
КонецПроцедуры

Функция ПолучитьНастройкуПользователяДиректБанк(КлючНастроек, ИмяПараметра) Экспорт
	
	СисИнфо = Новый СистемнаяИнформация;
	ИдентификаторКлиента = СисИнфо.ИдентификаторКлиента;
	Возврат ОбменСБанкамиСлужебныйВызовСервера.ПолучитьНастройкуПользователяДиректБанк(ИдентификаторКлиента, КлючНастроек, ИмяПараметра);
