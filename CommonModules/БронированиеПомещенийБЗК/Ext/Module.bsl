...
Процедура ПередЗаписьюБрони(ЭтотОбъект, Отказ) Экспорт
	...
	Если Не Результат.Пустой()
		...
		ПараметрыПланируемогоЭтапа = Справочники.ЭтапыРаботыСКандидатами.ПараметрыПланируемогоЭтапа(Кандидаты, ЭтапРаботы);
		
		ОписаниеСобытия = Справочники.ЭтапыРаботыСКандидатами.ОписаниеПланируемогоЭтапа(ПараметрыПланируемогоЭтапа);
	...
	Пока ВыборкаБроньДляКандидатов.СледующийПоЗначениюПоля("Бронь") Цикл
		...
		Если БроньПересекается 
			...
			ПараметрыПланируемогоЭтапа =
				Справочники.ЭтапыРаботыСКандидатами.ПараметрыПланируемогоЭтапа(КандидатыБрони, ЭтапРаботыБрони);
			ПараметрыПланируемогоЭтапа.Развернуто = Истина;
...
Процедура ЗарегистрироватьОбработчикиОбновления(Обработчики) Экспорт
	...
	Обработчик.Версия = "3.1.19.3";
	Обработчик.НачальноеЗаполнение = Истина;
	Обработчик.Идентификатор = Новый УникальныйИдентификатор("730c19d2-2f1c-4858-a02b-d9bab7db9b0b");
	Обработчик.Процедура = "БронированиеПомещенийБЗК.ЗаполнитьВидыКонтактнойИнформацииТерриторииИПомещения";
	
КонецПроцедуры

// Инициализирует виды контактной информации помещений.
Процедура ЗаполнитьВидыКонтактнойИнформацииТерриторииИПомещения() Экспорт

	// АдресТерриторииИПомещения
	...
	СвойстваВидаКИ.Вид = Справочники.ВидыКонтактнойИнформации.АдресТерриторииИПомещения;
	СвойстваВидаКИ.Наименование = НСтр("ru = 'Адрес'");
	...
	СвойстваВидаКИ.Вид = Справочники.ВидыКонтактнойИнформации.ТелефонТерриторииИПомещения;
	...
	СвойстваВидаКИ.Вид = Справочники.ВидыКонтактнойИнформации.EMailТерриторииИПомещения;
...
Процедура ЗадатьБроньПоЭтапу(МенеджерВременныхТаблиц, Кандидаты, ЭтапРаботы, Отказ) Экспорт
	...
	Пока ВыборкаБроньДляКандидатов.СледующийПоЗначениюПоля("Бронь") Цикл
		...
		Если БроньПересекается И ЭтапРаботыБрони <> ЭтапРаботы.Этап Тогда
			ВызватьИсключение ТекстИсключения;
		КонецЕсли;
		...
		Иначе
			...
			Если ЭтапРаботы.Состояние = Перечисления.СостоянияЭтаповРаботыСКандидатами.Запланирован Тогда 
				...
				Иначе
					...
					ПараметрыПланируемогоЭтапа =
						Справочники.ЭтапыРаботыСКандидатами.ПараметрыПланируемогоЭтапа(НеУчитываемыеКандидаты, ЭтапРаботы.Этап);
					ПараметрыПланируемогоЭтапа.Развернуто = Истина;
					
					Комментарий = Справочники.ЭтапыРаботыСКандидатами.ОписаниеПланируемогоЭтапа(ПараметрыПланируемогоЭтапа);
					...
					ПараметрыПланируемогоЭтапа =
						Справочники.ЭтапыРаботыСКандидатами.ПараметрыПланируемогоЭтапа(Кандидаты, ЭтапРаботы.Этап);
					ПараметрыПланируемогоЭтапа.Развернуто = Истина;
