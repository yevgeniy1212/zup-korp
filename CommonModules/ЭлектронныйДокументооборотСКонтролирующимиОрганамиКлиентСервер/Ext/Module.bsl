...
Процедура ПриИнициализацииФормыРегламентированногоОтчета(Форма, КонтролирующийОрган = "ФНС") Экспорт
	...
	ИначеЕсли КонтролирующийОрган = "УдалитьБанкРоссии" Тогда
		...
		#Если Клиент Тогда
			МодульУдалитьДокументооборотСБанкомРоссииКлиентСервер =
				ОбщегоНазначенияКлиент.ОбщийМодуль("УдалитьДокументооборотСБанкомРоссииКлиентСервер");
		#Иначе
			МодульУдалитьДокументооборотСБанкомРоссииКлиентСервер =
				ОбщегоНазначения.ОбщийМодуль("УдалитьДокументооборотСБанкомРоссииКлиентСервер");
		#КонецЕсли
		МодульУдалитьДокументооборотСБанкомРоссииКлиентСервер.ПриИнициализацииФормыРегламентированногоОтчета(Форма,
			ПараметрыПрорисовки);
	...
	ИначеЕсли КонтролирующийОрган = "РегистрацияЮЛФНС" Тогда
		
		ПараметрыПрорисовки = Неопределено;
		МодульДокументооборотРегистрацияВФНСКлиентСервер = Неопределено;
		#Если Клиент Тогда
			Если ОбщегоНазначенияКлиент.ПодсистемаСуществует("РегламентированнаяОтчетность.ЭлектронныйДокументооборотСКонтролирующимиОрганами.ГосударственнаяРегистрацияВФНС") Тогда
		...
		#Иначе
			Если ОбщегоНазначения.ПодсистемаСуществует("РегламентированнаяОтчетность.ЭлектронныйДокументооборотСКонтролирующимиОрганами.ГосударственнаяРегистрацияВФНС") Тогда
				МодульДокументооборотРегистрацияВФНСКлиентСервер = ОбщегоНазначения.ОбщийМодуль("ДокументооборотРегистрацияВФНСКлиентСервер");
			КонецЕсли;
		#КонецЕсли
		
		Если МодульДокументооборотРегистрацияВФНСКлиентСервер <> Неопределено Тогда
			МодульДокументооборотРегистрацияВФНСКлиентСервер.ПриИнициализацииФормыРегламентированногоОтчета(Форма, ПараметрыПрорисовки);
		Иначе
...
Процедура ОбновитьПанельСостоянияОтправкиВРегламентированномОтчете(Форма, КонтролирующийОрган) Экспорт
	...
	ИначеЕсли КонтролирующийОрган = "УдалитьБанкРоссии" Тогда
		...
		#Если Клиент Тогда
			МодульУдалитьДокументооборотСБанкомРоссииКлиентСервер = ОбщегоНазначенияКлиент.ОбщийМодуль("УдалитьДокументооборотСБанкомРоссииКлиентСервер");
		#Иначе
			МодульУдалитьДокументооборотСБанкомРоссииКлиентСервер = ОбщегоНазначения.ОбщийМодуль("УдалитьДокументооборотСБанкомРоссииКлиентСервер");
		#КонецЕсли
		ПараметрыПрорисовки = МодульУдалитьДокументооборотСБанкомРоссииКлиентСервер.ПолучитьПараметрыПрорисовкиПанелиОтправки(Форма);
	...
	ИначеЕсли КонтролирующийОрган = "РЭЦ" Тогда
			
		#Если Клиент Тогда
			МодульДокументооборотСРЭЦКлиентСервер = ОбщегоНазначенияКлиент.ОбщийМодуль("ДокументооборотСРЭЦКлиентСервер");
		#Иначе
			МодульДокументооборотСРЭЦКлиентСервер = ОбщегоНазначения.ОбщийМодуль("ДокументооборотСРЭЦКлиентСервер");
		#КонецЕсли
		ПараметрыПрорисовки = МодульДокументооборотСРЭЦКлиентСервер.ПолучитьПараметрыПрорисовкиПанелиОтправки(Форма);
	...
	ИначеЕсли КонтролирующийОрган = "РегистрацияЮЛФНС" Тогда
		...
		Если МодульДокументооборотРегистрацияВФНСКлиентСервер <> Неопределено Тогда
			ПараметрыПрорисовки = МодульДокументооборотРегистрацияВФНСКлиентСервер.ПолучитьПараметрыПрорисовкиПанелиОтправки(Форма);
		Иначе
			ПараметрыПрорисовки = Неопределено;
		КонецЕсли;
...
Процедура ПрименитьПараметрыПрорисовкиПанелиОтправки(Форма, ПараметрыПрорисовки) Экспорт
	...
	Если ПараметрыПрорисовки.КонтролирующийОрган = "РЭЦ" Или ПараметрыПрорисовки.КонтролирующийОрган = "ФНС" Тогда
		СостояниеОтправкиРЭЦ = Неопределено;
		Если ПараметрыПрорисовки.КонтролирующийОрган = "ФНС" Тогда
			Если ПараметрыПрорисовки.Свойство("ТекущийЭтапОтправкиРЭЦ") Тогда	
				СостояниеОтправкиРЭЦ = ПараметрыПрорисовки.ТекущийЭтапОтправкиРЭЦ;
			КонецЕсли;
		Иначе
			СостояниеОтправкиРЭЦ = ПараметрыПрорисовки.ТекущийЭтапОтправки;	
		КонецЕсли;
		БлокСостоянияОтправкиРЭЦ = Форма.Элементы.Найти("БлокСостоянияОтправкиРЭЦ");
		Если БлокСостоянияОтправкиРЭЦ <> Неопределено Тогда  
			// Определяем цвет фона.
			БлокСостоянияОтправкиРЭЦ.ЦветФона = ДокументооборотСКОВызовСервера.ЦветФонаПанелиОтправкиПоСтатусу(СостояниеОтправкиРЭЦ.СостояниеСдачиОтчетности);
			Если СостояниеОтправкиРЭЦ.СостояниеСдачиОтчетности = ПредопределенноеЗначение("Перечисление.СостояниеСдачиОтчетности.ДокументооборотНеНачат") Тогда
				БлокСостоянияОтправкиРЭЦ.Видимость = Ложь;
			Иначе
				БлокСостоянияОтправкиРЭЦ.Видимость = Истина;
				// Наименование протокола
				ЭлементФормыГиперссылкаПротоколРЭЦ = Форма.Элементы.Найти("ГиперссылкаПротоколРЭЦ");
				Если ЭлементФормыГиперссылкаПротоколРЭЦ <> Неопределено Тогда
					Если ЗначениеЗаполнено(СостояниеОтправкиРЭЦ.НаименованиеПротокола) Тогда
						ЭлементФормыГиперссылкаПротоколРЭЦ.Видимость = Истина;
						ЭлементФормыГиперссылкаПротоколРЭЦ.Заголовок = СостояниеОтправкиРЭЦ.НаименованиеПротокола;
					Иначе
						ЭлементФормыГиперссылкаПротоколРЭЦ.Видимость = Ложь;
					КонецЕсли;
				КонецЕсли;
				// Наименование текущего этапа
				ЭлементФормыНаименованиеЭтапаРЭЦ = Форма.Элементы.Найти("НаименованиеЭтапаРЭЦ");
				Если ЭлементФормыНаименованиеЭтапаРЭЦ <> Неопределено Тогда
					НаименованиеЭтапаЗаголовокРЭЦ = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = '%1'"), СостояниеОтправкиРЭЦ.ТекстНадписи);
					ЭлементФормыНаименованиеЭтапаРЭЦ.Гиперссылка = Ложь;
					ЭлементФормыНаименованиеЭтапаРЭЦ.Заголовок = НаименованиеЭтапаЗаголовокРЭЦ;
				КонецЕсли;
				ОтступПередКнопкойОбновитьОтправкуРЭЦ = Форма.Элементы.Найти("ОтступПередКнопкойОбновитьОтправкуРЭЦ");
				Если ОтступПередКнопкойОбновитьОтправкуРЭЦ <> Неопределено Тогда 
					ОтступПередКнопкойОбновитьОтправкуРЭЦ.Видимость = Истина;
				КонецЕсли;
				ОбновитьОтправкуРЭЦ = Форма.Элементы.Найти("ОбновитьОтправкуРЭЦ");
				Если ОбновитьОтправкуРЭЦ <> Неопределено Тогда 
					Если СостояниеОтправкиРЭЦ.СостояниеСдачиОтчетности = ПредопределенноеЗначение("Перечисление.СостояниеСдачиОтчетности.ДокументооборотНачат") Тогда
						ОбновитьОтправкуРЭЦ.Видимость = Истина;
					Иначе
						ОбновитьОтправкуРЭЦ.Видимость = Ложь;
					КонецЕсли;
				КонецЕсли;						
			КонецЕсли;
		КонецЕсли; 
	...
	Если ПараметрыПрорисовки.КонтролирующийОрган <> "РЭЦ" Тогда
		...
		Иначе
			...
			Если ЭлементФормыГиперссылкаПротокол <> Неопределено Тогда
				Если ЗначениеЗаполнено(СостояниеОтправки.НаименованиеПротокола) Тогда
					...
					Если СостояниеОтправки.Протокол = СостояниеОтправки.НаименованиеПротокола Тогда
						#Если Сервер Или ТолстыйКлиентОбычноеПриложение Или ВнешнееСоединение Тогда
							ЭлементФормыГиперссылкаПротокол.Заголовок = СтроковыеФункции.ФорматированнаяСтрока(СостояниеОтправки.НаименованиеПротокола);
						#Иначе
							ЭлементФормыГиперссылкаПротокол.Заголовок = СтроковыеФункцииКлиент.ФорматированнаяСтрока(СостояниеОтправки.НаименованиеПротокола);
						#КонецЕсли
			 		 	ЭлементФормыГиперссылкаПротокол.Гиперссылка = Ложь;
					Иначе
						ЭлементФормыГиперссылкаПротокол.Заголовок = СостояниеОтправки.НаименованиеПротокола;
			 		 	ЭлементФормыГиперссылкаПротокол.Гиперссылка = Истина;
					КонецЕсли;
				Иначе
			...
			Если ЭлементФормыНаименованиеЭтапа <> Неопределено Тогда
				...
				Если НЕ (СтрНачинаетсяС(Форма.ИмяФормы, "Справочник.ЭлектронныеПредставленияРегламентированныхОтчетов")
					...
					ИЛИ СтрНачинаетсяС(Форма.ИмяФормы, "Документ.ЗаявленияПоЭлДокументооборотуСПФР")
					ИЛИ СтрНачинаетсяС(Форма.ИмяФормы, "Документ.УведомлениеОПредоставленииПолномочийПредставителю")
					ИЛИ СтрНачинаетсяС(Форма.ИмяФормы, "Документ.УведомлениеОПрекращенииПолномочийПредставителя")) Тогда
				...
				Если ПараметрыПрорисовки.КонтролирующийОрган <> "Минобороны" Тогда
					ЭлементФормыЭтапыОтправки.Видимость =
						СостояниеСдачиОтчетности <> ПредопределенноеЗначение("Перечисление.СостояниеСдачиОтчетности.ДокументооборотНеНачат")
							И (ПараметрыПрорисовки.КонтролирующийОрган <> "ЦБ" ИЛИ Лев(СостояниеОтправки.ТекстСтатуса, 9) <> "Подписано");
				Иначе
					ЭлементФормыЭтапыОтправки.Видимость =
			...
			// Квитанции о присвоении РНПТ
			ЭлементФормыЗаполнитьРНПТ = Форма.Элементы.Найти("ЗаполнитьРНПТ");
			Если ЭлементФормыЗаполнитьРНПТ <> Неопределено
				И ПараметрыПрорисовки.Свойство("ДокументыРеализацииПолномочийНО") Тогда
				
				РНПТ = "";
				Для каждого ДокументРеализацииПолномочийНО Из ПараметрыПрорисовки.ДокументыРеализацииПолномочийНО Цикл
					Если ЗначениеЗаполнено(ДокументРеализацииПолномочийНО.РНПТ) Тогда
						РНПТ = СокрЛП(ДокументРеализацииПолномочийНО.РНПТ);
						Прервать;
					КонецЕсли;
				КонецЦикла;
				
				ОбъектРНПТ = "";
				Если РегламентированнаяОтчетностьКлиентСервер.СвойствоОпределено(Форма, "Объект") И Форма.Объект <> Неопределено
					И РегламентированнаяОтчетностьКлиентСервер.СвойствоОпределено(Форма.Объект, "РНПТ") Тогда
					
					ОбъектРНПТ = Форма.Объект.РНПТ;
				КонецЕсли;
				
				ЭлементФормыЗаполнитьРНПТ.Видимость = ЗначениеЗаполнено(РНПТ) И НЕ ЗначениеЗаполнено(ОбъектРНПТ);
				
				Если РегламентированнаяОтчетностьКлиентСервер.СвойствоОпределено(Форма, "ДокументыРеализацииПолномочийНО") Тогда
					Форма.ДокументыРеализацииПолномочийНО.Очистить();
					Для каждого ДокументРеализацииПолномочийНО Из ПараметрыПрорисовки.ДокументыРеализацииПолномочийНО Цикл
						СтрокаТаблицы = Форма.ДокументыРеализацииПолномочийНО.Добавить();
						ЗаполнитьЗначенияСвойств(СтрокаТаблицы, ДокументРеализацииПолномочийНО);
					КонецЦикла;
				КонецЕсли;
			КонецЕсли;
	...
	Если Используется1СОтчетность И СостояниеСдачиОтчетности =
		ПредопределенноеЗначение("Перечисление.СостояниеСдачиОтчетности.ПоложительныйРезультатДокументооборота")
		И СтрНачинаетсяС(Форма.ИмяФормы, "Справочник.МашиночитаемыеДоверенностиФСС") Тогда
		
		// Показ кнопки "Отозвать".
		ЭлементФормыОтозвать = Форма.Элементы.Найти("Отозвать");
		Если ЭлементФормыОтозвать <> Неопределено Тогда
			ЭлементФормыОтозвать.Видимость = Истина;
		КонецЕсли;
	КонецЕсли;
...
Функция ВидимостьКнопкиОтправкаПоСостоянию(
	...
	Возврат ((СостояниеСдачиОтчетности <> ПредопределенноеЗначение("Перечисление.СостояниеСдачиОтчетности.ПоложительныйРезультатДокументооборота")
			...
			И (ТипОтправляемогоДокумента <> "РеестрСтимулирующихВыплатМедицинскимИСоциальнымРаботникам"
				ИЛИ СостояниеСдачиОтчетности = ПредопределенноеЗначение("Перечисление.СостояниеСдачиОтчетности.ДокументооборотНачат"))
...
Функция СтрокаОС() Экспорт
	...
	ИначеЕсли СисИнфо.ТипПлатформы = ТипПлатформы.Linux_x86_64 Тогда
		Возврат "Linux64";
	ИначеЕсли СисИнфо.ТипПлатформы = ТипПлатформы.MacOS_x86 Тогда
		Возврат "MacOS32";
	ИначеЕсли СисИнфо.ТипПлатформы = ТипПлатформы.MacOS_x86_64 Тогда
		Возврат "MacOS64";
...
Функция ЭтоЛинуксИлиМакОС() Экспорт
	...
	Возврат (СисИнфо.ТипПлатформы = ТипПлатформы.Linux_x86 ИЛИ СисИнфо.ТипПлатформы = ТипПлатформы.Linux_x86_64
		ИЛИ СисИнфо.ТипПлатформы = ТипПлатформы.MacOS_x86 ИЛИ СисИнфо.ТипПлатформы = ТипПлатформы.MacOS_x86_64);
...
//
Функция МассивВидовПрочихУведомленийПоддерживающихДокументооборот(
		МассивВидовОтправляемыхДокументов,
		ДобавитьПоСоответствиюИмен = Истина) Экспорт
	
	МассивВидовПрочихУведомленийПоддерживающихДокументооборот = Новый Массив;
	
	Если МассивВидовОтправляемыхДокументов.Найти(ПредопределенноеЗначение("Справочник.ВидыОтправляемыхДокументов.ДорожнаяКартаНМ")) <> Неопределено Тогда
		МассивВидовПрочихУведомленийПоддерживающихДокументооборот.Добавить(ПредопределенноеЗначение("Перечисление.ВидыУведомленийОСпецрежимахНалогообложения.ДорожнаяКартаНМ"));
	КонецЕсли;
	Если МассивВидовОтправляемыхДокументов.Найти(ПредопределенноеЗначение("Справочник.ВидыОтправляемыхДокументов.ЗапросМотивированногоМнения")) <> Неопределено Тогда
		МассивВидовПрочихУведомленийПоддерживающихДокументооборот.Добавить(ПредопределенноеЗначение("Перечисление.ВидыУведомленийОСпецрежимахНалогообложения.ЗапросМотивированногоМнения"));
	КонецЕсли;
	Если МассивВидовОтправляемыхДокументов.Найти(ПредопределенноеЗначение("Справочник.ВидыОтправляемыхДокументов.СогласиеСМотивированнымМнением")) <> Неопределено Тогда
		МассивВидовПрочихУведомленийПоддерживающихДокументооборот.Добавить(ПредопределенноеЗначение("Перечисление.ВидыУведомленийОСпецрежимахНалогообложения.СогласиеСМотивированнымМнением"));
	КонецЕсли;
	Если МассивВидовОтправляемыхДокументов.Найти(ПредопределенноеЗначение("Справочник.ВидыОтправляемыхДокументов.РазногласиеСМотивированнымМнением")) <> Неопределено Тогда
		МассивВидовПрочихУведомленийПоддерживающихДокументооборот.Добавить(ПредопределенноеЗначение("Перечисление.ВидыУведомленийОСпецрежимахНалогообложения.РазногласиеСМотивированнымМнением"));
	КонецЕсли;
	Если МассивВидовОтправляемыхДокументов.Найти(ПредопределенноеЗначение("Справочник.ВидыОтправляемыхДокументов.ВзаимосогласительнаяПроцедура")) <> Неопределено Тогда
		МассивВидовПрочихУведомленийПоддерживающихДокументооборот.Добавить(ПредопределенноеЗначение("Перечисление.ВидыУведомленийОСпецрежимахНалогообложения.ВзаимосогласительнаяПроцедура"));
	КонецЕсли;
	Если МассивВидовОтправляемыхДокументов.Найти(ПредопределенноеЗначение("Справочник.ВидыОтправляемыхДокументов.ЗаявительныйПорядокВозмещенияНДСАкциз")) <> Неопределено Тогда
		МассивВидовПрочихУведомленийПоддерживающихДокументооборот.Добавить(ПредопределенноеЗначение("Перечисление.ВидыУведомленийОСпецрежимахНалогообложения.ЗаявительныйПорядокВозмещенияНДСАкциз"));
	КонецЕсли;
	...
	КонецЕсли;
	Если МассивВидовОтправляемыхДокументов.Найти(ПредопределенноеЗначение("Справочник.ВидыОтправляемыхДокументов.ОтзывЗапросаМотивированногоМнения")) <> Неопределено Тогда
		МассивВидовПрочихУведомленийПоддерживающихДокументооборот.Добавить(ПредопределенноеЗначение("Перечисление.ВидыУведомленийОСпецрежимахНалогообложения.ОтзывЗапросаМотивированногоМнения"));
	КонецЕсли;
	Если МассивВидовОтправляемыхДокументов.Найти(ПредопределенноеЗначение("Справочник.ВидыОтправляемыхДокументов.ИнформацияОРазницахБУиНУ")) <> Неопределено Тогда
		МассивВидовПрочихУведомленийПоддерживающихДокументооборот.Добавить(ПредопределенноеЗначение("Перечисление.ВидыУведомленийОСпецрежимахНалогообложения.ИнформацияОРазницахБУиНУ"));
	КонецЕсли;
	Если МассивВидовОтправляемыхДокументов.Найти(ПредопределенноеЗначение("Справочник.ВидыОтправляемыхДокументов.ИнформацияУдельныйВес")) <> Неопределено Тогда
		МассивВидовПрочихУведомленийПоддерживающихДокументооборот.Добавить(ПредопределенноеЗначение("Перечисление.ВидыУведомленийОСпецрежимахНалогообложения.ИнформацияУдельныйВес"));
	КонецЕсли;
	Если МассивВидовОтправляемыхДокументов.Найти(ПредопределенноеЗначение("Справочник.ВидыОтправляемыхДокументов.РасшифровкаПоОКВЭД")) <> Неопределено Тогда
		МассивВидовПрочихУведомленийПоддерживающихДокументооборот.Добавить(ПредопределенноеЗначение("Перечисление.ВидыУведомленийОСпецрежимахНалогообложения.РасшифровкаПоОКВЭД"));
	КонецЕсли;
	Если МассивВидовОтправляемыхДокументов.Найти(ПредопределенноеЗначение("Справочник.ВидыОтправляемыхДокументов.ИнформацияОРискахПоОтдельнымСделкам")) <> Неопределено Тогда
		МассивВидовПрочихУведомленийПоддерживающихДокументооборот.Добавить(ПредопределенноеЗначение("Перечисление.ВидыУведомленийОСпецрежимахНалогообложения.ИнформацияОРискахПоОтдельнымСделкам"));
	КонецЕсли;
	Если МассивВидовОтправляемыхДокументов.Найти(ПредопределенноеЗначение("Справочник.ВидыОтправляемыхДокументов.СтруктураВзаимосвязейВнутриГруппыКомпаний")) <> Неопределено Тогда
		МассивВидовПрочихУведомленийПоддерживающихДокументооборот.Добавить(ПредопределенноеЗначение("Перечисление.ВидыУведомленийОСпецрежимахНалогообложения.СтруктураВзаимосвязейВнутриГруппыКомпаний"));
	КонецЕсли;
	...
	КонецЕсли;
	Если МассивВидовОтправляемыхДокументов.Найти(ПредопределенноеЗначение("Справочник.ВидыОтправляемыхДокументов.КонтрольныеПроцедуры")) <> Неопределено Тогда
		МассивВидовПрочихУведомленийПоддерживающихДокументооборот.Добавить(ПредопределенноеЗначение("Перечисление.ВидыУведомленийОСпецрежимахНалогообложения.КонтрольныеПроцедуры"));
	КонецЕсли;
	Если МассивВидовОтправляемыхДокументов.Найти(ПредопределенноеЗначение("Справочник.ВидыОтправляемыхДокументов.МатрицаРисковКП")) <> Неопределено Тогда
		МассивВидовПрочихУведомленийПоддерживающихДокументооборот.Добавить(ПредопределенноеЗначение("Перечисление.ВидыУведомленийОСпецрежимахНалогообложения.МатрицаРисковКП"));
	КонецЕсли;
	Если МассивВидовОтправляемыхДокументов.Найти(ПредопределенноеЗначение("Справочник.ВидыОтправляемыхДокументов.МероприятияПоСовершенствованию")) <> Неопределено Тогда
		МассивВидовПрочихУведомленийПоддерживающихДокументооборот.Добавить(ПредопределенноеЗначение("Перечисление.ВидыУведомленийОСпецрежимахНалогообложения.МероприятияПоСовершенствованию"));
	КонецЕсли;
	Если МассивВидовОтправляемыхДокументов.Найти(ПредопределенноеЗначение("Справочник.ВидыОтправляемыхДокументов.ОценкаСистемыВнутреннегоКонтроля")) <> Неопределено Тогда
		МассивВидовПрочихУведомленийПоддерживающихДокументооборот.Добавить(ПредопределенноеЗначение("Перечисление.ВидыУведомленийОСпецрежимахНалогообложения.ОценкаСистемыВнутреннегоКонтроля"));
	КонецЕсли;
	Если МассивВидовОтправляемыхДокументов.Найти(ПредопределенноеЗначение("Справочник.ВидыОтправляемыхДокументов.ГрафикПереходаНалоговыйМониторинг")) <> Неопределено Тогда
		МассивВидовПрочихУведомленийПоддерживающихДокументооборот.Добавить(ПредопределенноеЗначение("Перечисление.ВидыУведомленийОСпецрежимахНалогообложения.ГрафикПереходаНалоговыйМониторинг"));
	КонецЕсли;
	...
	КонецЕсли;
	Если МассивВидовОтправляемыхДокументов.Найти(ПредопределенноеЗначение("Справочник.ВидыОтправляемыхДокументов.РезультатыВыполнения")) <> Неопределено Тогда
		МассивВидовПрочихУведомленийПоддерживающихДокументооборот.Добавить(ПредопределенноеЗначение("Перечисление.ВидыУведомленийОСпецрежимахНалогообложения.РезультатыВыполнения"));
	КонецЕсли;
	Если МассивВидовОтправляемыхДокументов.Найти(ПредопределенноеЗначение("Справочник.ВидыОтправляемыхДокументов.РискиОрганизацииНалоговыйМониторинг")) <> Неопределено Тогда
		МассивВидовПрочихУведомленийПоддерживающихДокументооборот.Добавить(ПредопределенноеЗначение("Перечисление.ВидыУведомленийОСпецрежимахНалогообложения.РискиОрганизацииНалоговыйМониторинг"));
	КонецЕсли;
	...
	Если МассивВидовОтправляемыхДокументов.Найти(ПредопределенноеЗначение("Справочник.ВидыОтправляемыхДокументов.ВыборНалоговогоОрганаДляПредставленияДеклараций")) <> Неопределено Тогда
		МассивВидовПрочихУведомленийПоддерживающихДокументооборот.Добавить(ПредопределенноеЗначение("Перечисление.ВидыУведомленийОСпецрежимахНалогообложения.ВыборНалоговогоОрганаДляПредставленияДеклараций"));
	КонецЕсли;
	Если МассивВидовОтправляемыхДокументов.Найти(ПредопределенноеЗначение("Справочник.ВидыОтправляемыхДокументов.ОсвобождениеОтНДСПриЕСХН")) <> Неопределено Тогда
		МассивВидовПрочихУведомленийПоддерживающихДокументооборот.Добавить(ПредопределенноеЗначение("Перечисление.ВидыУведомленийОСпецрежимахНалогообложения.ОсвобождениеОтНДСПриЕСХН"));
	КонецЕсли;
	Если МассивВидовОтправляемыхДокументов.Найти(ПредопределенноеЗначение("Справочник.ВидыОтправляемыхДокументов.ОсвобождениеОтУплатыНДС")) <> Неопределено Тогда
		МассивВидовПрочихУведомленийПоддерживающихДокументооборот.Добавить(ПредопределенноеЗначение("Перечисление.ВидыУведомленийОСпецрежимахНалогообложения.ОсвобождениеОтУплатыНДС"));
	КонецЕсли;
	...
	Если МассивВидовОтправляемыхДокументов.Найти(ПредопределенноеЗначение("Справочник.ВидыОтправляемыхДокументов.ПриобретениеСтатусаМХК")) <> Неопределено Тогда
		МассивВидовПрочихУведомленийПоддерживающихДокументооборот.Добавить(ПредопределенноеЗначение("Перечисление.ВидыУведомленийОСпецрежимахНалогообложения.ПриобретениеСтатусаМХК"));
	КонецЕсли;
	...
	Если МассивВидовОтправляемыхДокументов.Найти(ПредопределенноеЗначение("Справочник.ВидыОтправляемыхДокументов.ВозвратГоспошлины")) <> Неопределено Тогда
		МассивВидовПрочихУведомленийПоддерживающихДокументооборот.Добавить(ПредопределенноеЗначение("Перечисление.ВидыУведомленийОСпецрежимахНалогообложения.ВозвратГоспошлины"));
	КонецЕсли;
	...
	КонецЕсли;
	Если МассивВидовОтправляемыхДокументов.Найти(ПредопределенноеЗначение("Справочник.ВидыОтправляемыхДокументов.ВозвратНДФЛНПДБиоресурсы")) <> Неопределено Тогда
		МассивВидовПрочихУведомленийПоддерживающихДокументооборот.Добавить(ПредопределенноеЗначение("Перечисление.ВидыУведомленийОСпецрежимахНалогообложения.ВозвратНДФЛНПДБиоресурсы"));
	КонецЕсли;
	Если МассивВидовОтправляемыхДокументов.Найти(ПредопределенноеЗначение("Справочник.ВидыОтправляемыхДокументов.ЗачетГоспошлины")) <> Неопределено Тогда
		МассивВидовПрочихУведомленийПоддерживающихДокументооборот.Добавить(ПредопределенноеЗначение("Перечисление.ВидыУведомленийОСпецрежимахНалогообложения.ЗачетГоспошлины"));
	КонецЕсли;
	...
	КонецЕсли;
	Если МассивВидовОтправляемыхДокументов.Найти(ПредопределенноеЗначение("Справочник.ВидыОтправляемыхДокументов.ДоступГИРБО")) <> Неопределено Тогда
		МассивВидовПрочихУведомленийПоддерживающихДокументооборот.Добавить(ПредопределенноеЗначение("Перечисление.ВидыУведомленийОСпецрежимахНалогообложения.ДоступГИРБО"));
	КонецЕсли;
	Если МассивВидовОтправляемыхДокументов.Найти(ПредопределенноеЗначение("Справочник.ВидыОтправляемыхДокументов.ОтменаЗачетаНалога")) <> Неопределено Тогда
		МассивВидовПрочихУведомленийПоддерживающихДокументооборот.Добавить(ПредопределенноеЗначение("Перечисление.ВидыУведомленийОСпецрежимахНалогообложения.ОтменаЗачетаНалога"));
	КонецЕсли;
	...
	КонецЕсли;
	Если МассивВидовОтправляемыхДокументов.Найти(ПредопределенноеЗначение("Справочник.ВидыОтправляемыхДокументов.ЗаявлениеОГибелиИмущества")) <> Неопределено Тогда
		МассивВидовПрочихУведомленийПоддерживающихДокументооборот.Добавить(ПредопределенноеЗначение("Перечисление.ВидыУведомленийОСпецрежимахНалогообложения.ЗаявлениеОГибелиИмущества"));
	КонецЕсли;
	Если МассивВидовОтправляемыхДокументов.Найти(ПредопределенноеЗначение("Справочник.ВидыОтправляемыхДокументов.ЛьготаПоНалогуНаИмущество")) <> Неопределено Тогда
		МассивВидовПрочихУведомленийПоддерживающихДокументооборот.Добавить(ПредопределенноеЗначение("Перечисление.ВидыУведомленийОСпецрежимахНалогообложения.ЛьготаПоНалогуНаИмущество"));
	КонецЕсли;
	Если МассивВидовОтправляемыхДокументов.Найти(ПредопределенноеЗначение("Справочник.ВидыОтправляемыхДокументов.ПередачаСообщенийОСуммахНалогов")) <> Неопределено Тогда
		МассивВидовПрочихУведомленийПоддерживающихДокументооборот.Добавить(ПредопределенноеЗначение("Перечисление.ВидыУведомленийОСпецрежимахНалогообложения.ПередачаСообщенийОСуммахНалогов"));
	КонецЕсли;
	Если МассивВидовОтправляемыхДокументов.Найти(ПредопределенноеЗначение("Справочник.ВидыОтправляемыхДокументов.ИзъятиеТС")) <> Неопределено Тогда
		МассивВидовПрочихУведомленийПоддерживающихДокументооборот.Добавить(ПредопределенноеЗначение("Перечисление.ВидыУведомленийОСпецрежимахНалогообложения.ИзъятиеТС"));
	КонецЕсли;
	Если МассивВидовОтправляемыхДокументов.Найти(ПредопределенноеЗначение("Справочник.ВидыОтправляемыхДокументов.РеестрЛьготИмущество")) <> Неопределено Тогда
		МассивВидовПрочихУведомленийПоддерживающихДокументооборот.Добавить(ПредопределенноеЗначение("Перечисление.ВидыУведомленийОСпецрежимахНалогообложения.РеестрЛьготИмущество"));
	КонецЕсли;
	Если МассивВидовОтправляемыхДокументов.Найти(ПредопределенноеЗначение("Справочник.ВидыОтправляемыхДокументов.ЗапретПредставленияУполномоченнымиЛицами")) <> Неопределено Тогда
		МассивВидовПрочихУведомленийПоддерживающихДокументооборот.Добавить(ПредопределенноеЗначение("Перечисление.ВидыУведомленийОСпецрежимахНалогообложения.ЗапретПредставленияУполномоченнымиЛицами"));
	КонецЕсли;
	...
	КонецЕсли;
	Если МассивВидовОтправляемыхДокументов.Найти(ПредопределенноеЗначение("Справочник.ВидыОтправляемыхДокументов.ПоясненияПоСуммамНалогов")) <> Неопределено Тогда
		МассивВидовПрочихУведомленийПоддерживающихДокументооборот.Добавить(ПредопределенноеЗначение("Перечисление.ВидыУведомленийОСпецрежимахНалогообложения.ПоясненияПоСуммамНалогов"));
	КонецЕсли;
	...
	Если МассивВидовОтправляемыхДокументов.Найти(ПредопределенноеЗначение("Справочник.ВидыОтправляемыхДокументов.ИсправлениеОшибок")) <> Неопределено Тогда
		МассивВидовПрочихУведомленийПоддерживающихДокументооборот.Добавить(ПредопределенноеЗначение("Перечисление.ВидыУведомленийОСпецрежимахНалогообложения.ИсправлениеОшибок"));
	КонецЕсли;
	Если МассивВидовОтправляемыхДокументов.Найти(ПредопределенноеЗначение("Справочник.ВидыОтправляемыхДокументов.ДубликатСвидетельстваПереработкаНефти")) <> Неопределено Тогда
		МассивВидовПрочихУведомленийПоддерживающихДокументооборот.Добавить(ПредопределенноеЗначение("Перечисление.ВидыУведомленийОСпецрежимахНалогообложения.ДубликатСвидетельстваПереработкаНефти"));
	КонецЕсли;
	Если МассивВидовОтправляемыхДокументов.Найти(ПредопределенноеЗначение("Справочник.ВидыОтправляемыхДокументов.СвидетельствоПереработкаНефти")) <> Неопределено Тогда
		МассивВидовПрочихУведомленийПоддерживающихДокументооборот.Добавить(ПредопределенноеЗначение("Перечисление.ВидыУведомленийОСпецрежимахНалогообложения.СвидетельствоПереработкаНефти"));
	КонецЕсли;
	...
	КонецЕсли;
	Если МассивВидовОтправляемыхДокументов.Найти(ПредопределенноеЗначение("Справочник.ВидыОтправляемыхДокументов.СоблюдениеУсловийПониженныхСтавок")) <> Неопределено Тогда
		МассивВидовПрочихУведомленийПоддерживающихДокументооборот.Добавить(ПредопределенноеЗначение("Перечисление.ВидыУведомленийОСпецрежимахНалогообложения.СоблюдениеУсловийПониженныхСтавок"));
	КонецЕсли;
	...
	КонецЕсли;
	Если МассивВидовОтправляемыхДокументов.Найти(ПредопределенноеЗначение("Справочник.ВидыОтправляемыхДокументов.ПредставлениеАктаСверкиЕНП")) <> Неопределено Тогда
		МассивВидовПрочихУведомленийПоддерживающихДокументооборот.Добавить(ПредопределенноеЗначение("Перечисление.ВидыУведомленийОСпецрежимахНалогообложения.ПредставлениеАктаСверкиЕНП"));
	КонецЕсли;
	...
	КонецЕсли;
	Если МассивВидовОтправляемыхДокументов.Найти(ПредопределенноеЗначение("Справочник.ВидыОтправляемыхДокументов.НалогНаПрибыльКурилы")) <> Неопределено Тогда
		МассивВидовПрочихУведомленийПоддерживающихДокументооборот.Добавить(ПредопределенноеЗначение("Перечисление.ВидыУведомленийОСпецрежимахНалогообложения.НалогНаПрибыльКурилы"));
	КонецЕсли;
	...
	КонецЕсли;
	Если МассивВидовОтправляемыхДокументов.Найти(ПредопределенноеЗначение("Справочник.ВидыОтправляемыхДокументов.ЗапросСостояниеРасчетов")) <> Неопределено Тогда
		МассивВидовПрочихУведомленийПоддерживающихДокументооборот.Добавить(ПредопределенноеЗначение("Перечисление.ВидыУведомленийОСпецрежимахНалогообложения.ЗапросСостояниеРасчетов"));
	КонецЕсли;
	Если МассивВидовОтправляемыхДокументов.Найти(ПредопределенноеЗначение("Справочник.ВидыОтправляемыхДокументов.ЗапросСуммЕНП")) <> Неопределено Тогда
		МассивВидовПрочихУведомленийПоддерживающихДокументооборот.Добавить(ПредопределенноеЗначение("Перечисление.ВидыУведомленийОСпецрежимахНалогообложения.ЗапросСуммЕНП"));
	КонецЕсли;
	...
	КонецЕсли;
	Если МассивВидовОтправляемыхДокументов.Найти(ПредопределенноеЗначение("Справочник.ВидыОтправляемыхДокументов.СубсидияПострадавшимМСП")) <> Неопределено Тогда
		МассивВидовПрочихУведомленийПоддерживающихДокументооборот.Добавить(ПредопределенноеЗначение("Перечисление.ВидыУведомленийОСпецрежимахНалогообложения.СубсидияПострадавшимМСП"));
	КонецЕсли;
	Если МассивВидовОтправляемыхДокументов.Найти(ПредопределенноеЗначение("Справочник.ВидыОтправляемыхДокументов.ЭлементДляВыпускаНовыхПатчей")) <> Неопределено Тогда
		МассивВидовПрочихУведомленийПоддерживающихДокументооборот.Добавить(ПредопределенноеЗначение("Перечисление.ВидыУведомленийОСпецрежимахНалогообложения.ЭлементДляВыпускаНовыхПатчей"));
	КонецЕсли;
	Если МассивВидовОтправляемыхДокументов.Найти(ПредопределенноеЗначение("Справочник.ВидыОтправляемыхДокументов.ЗаявлениеОПоручительстве")) <> Неопределено Тогда
		МассивВидовПрочихУведомленийПоддерживающихДокументооборот.Добавить(ПредопределенноеЗначение("Перечисление.ВидыУведомленийОСпецрежимахНалогообложения.ЗаявлениеОПоручительстве"));
	КонецЕсли;
	Если МассивВидовОтправляемыхДокументов.Найти(ПредопределенноеЗначение("Справочник.ВидыОтправляемыхДокументов.ДосрочноеИсполнениеОтсрочки")) <> Неопределено Тогда
		МассивВидовПрочихУведомленийПоддерживающихДокументооборот.Добавить(ПредопределенноеЗначение("Перечисление.ВидыУведомленийОСпецрежимахНалогообложения.ДосрочноеИсполнениеОтсрочки"));
	КонецЕсли;
	Если МассивВидовОтправляемыхДокументов.Найти(ПредопределенноеЗначение("Справочник.ВидыОтправляемыхДокументов.ПредоставлениеОтсрочки")) <> Неопределено Тогда
		МассивВидовПрочихУведомленийПоддерживающихДокументооборот.Добавить(ПредопределенноеЗначение("Перечисление.ВидыУведомленийОСпецрежимахНалогообложения.ПредоставлениеОтсрочки"));
	КонецЕсли;
	Если МассивВидовОтправляемыхДокументов.Найти(ПредопределенноеЗначение("Справочник.ВидыОтправляемыхДокументов.СубсидияПоПостановлению1513")) <> Неопределено Тогда
		МассивВидовПрочихУведомленийПоддерживающихДокументооборот.Добавить(ПредопределенноеЗначение("Перечисление.ВидыУведомленийОСпецрежимахНалогообложения.СубсидияПоПостановлению1513"));
	КонецЕсли;
	Если МассивВидовОтправляемыхДокументов.Найти(ПредопределенноеЗначение("Справочник.ВидыОтправляемыхДокументов.ИсчисленныеСуммыНалогов")) <> Неопределено Тогда
		МассивВидовПрочихУведомленийПоддерживающихДокументооборот.Добавить(ПредопределенноеЗначение("Перечисление.ВидыУведомленийОСпецрежимахНалогообложения.ИсчисленныеСуммыНалогов"));
	КонецЕсли;
	Если МассивВидовОтправляемыхДокументов.Найти(ПредопределенноеЗначение("Справочник.ВидыОтправляемыхДокументов.ОсобыйПорядокУплатыНалогов")) <> Неопределено Тогда
		МассивВидовПрочихУведомленийПоддерживающихДокументооборот.Добавить(ПредопределенноеЗначение("Перечисление.ВидыУведомленийОСпецрежимахНалогообложения.ОсобыйПорядокУплатыНалогов"));
	КонецЕсли;
	Если МассивВидовОтправляемыхДокументов.Найти(ПредопределенноеЗначение("Справочник.ВидыОтправляемыхДокументов.ВозвратЕдиногоНалоговогоПлатежа")) <> Неопределено Тогда
		МассивВидовПрочихУведомленийПоддерживающихДокументооборот.Добавить(ПредопределенноеЗначение("Перечисление.ВидыУведомленийОСпецрежимахНалогообложения.ВозвратЕдиногоНалоговогоПлатежа"));
	КонецЕсли;
	Если МассивВидовОтправляемыхДокументов.Найти(ПредопределенноеЗначение("Справочник.ВидыОтправляемыхДокументов.СведенияОКадастровыхНомерах")) <> Неопределено Тогда
		МассивВидовПрочихУведомленийПоддерживающихДокументооборот.Добавить(ПредопределенноеЗначение("Перечисление.ВидыУведомленийОСпецрежимахНалогообложения.СведенияОКадастровыхНомерах"));
	КонецЕсли;
	Если МассивВидовОтправляемыхДокументов.Найти(ПредопределенноеЗначение("Справочник.ВидыОтправляемыхДокументов.ИнформацияОПараметрахДоступаИС")) <> Неопределено Тогда
		МассивВидовПрочихУведомленийПоддерживающихДокументооборот.Добавить(ПредопределенноеЗначение("Перечисление.ВидыУведомленийОСпецрежимахНалогообложения.ИнформацияОПараметрахДоступаИС"));
	КонецЕсли;
	Если МассивВидовОтправляемыхДокументов.Найти(ПредопределенноеЗначение("Справочник.ВидыОтправляемыхДокументов.ОперацииОтчужденияЦФА")) <> Неопределено Тогда
		МассивВидовПрочихУведомленийПоддерживающихДокументооборот.Добавить(ПредопределенноеЗначение("Перечисление.ВидыУведомленийОСпецрежимахНалогообложения.ОперацииОтчужденияЦФА"));
	КонецЕсли;
	Если МассивВидовОтправляемыхДокументов.Найти(ПредопределенноеЗначение("Справочник.ВидыОтправляемыхДокументов.РезидентДФОПониженныеТарифы")) <> Неопределено Тогда
		МассивВидовПрочихУведомленийПоддерживающихДокументооборот.Добавить(ПредопределенноеЗначение("Перечисление.ВидыУведомленийОСпецрежимахНалогообложения.РезидентДФОПониженныеТарифы"));
	КонецЕсли;
	
	Если ДобавитьПоСоответствиюИмен Тогда
...
Функция НаименованияОтчетаБезПрефикса(НаименованиеОтчета, МаксимальнаяДлина = 25) Экспорт
	...
	Результат.Добавить(НаименованиеОтчета);
	
	МассивПрефиксов = Новый Массив;
	МассивПрефиксов.Добавить("Отчет о ");
	МассивПрефиксов.Добавить("Отчет об ");
	МассивПрефиксов.Добавить("Отчет ");
	МассивПрефиксов.Добавить("Декларация о ");
	МассивПрефиксов.Добавить("Декларация об ");
	МассивПрефиксов.Добавить("Декларация по ");
	МассивПрефиксов.Добавить("Декларация в ");
	МассивПрефиксов.Добавить("Декларация ");
	МассивПрефиксов.Добавить("Реестр по ");
	МассивПрефиксов.Добавить("Реестр для ");
	МассивПрефиксов.Добавить("Реестр ");
	МассивПрефиксов.Добавить("Акцизы на ");
	МассивПрефиксов.Добавить("Акцизы ");
	МассивПрефиксов.Добавить("Извещение об ");
	МассивПрефиксов.Добавить("Извещение ");
	МассивПрефиксов.Добавить("Налог на ");
	МассивПрефиксов.Добавить("Налог ");
	МассивПрефиксов.Добавить("Платежи за ");
	МассивПрефиксов.Добавить("Платежи ");
	МассивПрефиксов.Добавить("Сбор за ");
	МассивПрефиксов.Добавить("Сбор ");
	МассивПрефиксов.Добавить("Расчет по ");
	МассивПрефиксов.Добавить("Расчет ");
	МассивПрефиксов.Добавить("Плата за ");
	МассивПрефиксов.Добавить("Плата ");
	МассивПрефиксов.Добавить("Авансы по ");
	МассивПрефиксов.Добавить("Авансы ");
	МассивПрефиксов.Добавить("Сообщение о ");
	МассивПрефиксов.Добавить("Сообщение об ");
	МассивПрефиксов.Добавить("Сообщение ");
	МассивПрефиксов.Добавить("Уведомление о ");
	МассивПрефиксов.Добавить("Уведомление об ");
	МассивПрефиксов.Добавить("Уведомление ");
	МассивПрефиксов.Добавить("Заявление о ");
	МассивПрефиксов.Добавить("Заявление об ");
	МассивПрефиксов.Добавить("Заявление ");
	МассивПрефиксов.Добавить("Сведения о ");
	МассивПрефиксов.Добавить("Сведения об ");
	МассивПрефиксов.Добавить("Сведения ");
	МассивПрефиксов.Добавить("Запрос сведений, ");
	МассивПрефиксов.Добавить("Запрос сведений ");
	МассивПрефиксов.Добавить("Заявления по ");
	МассивПрефиксов.Добавить("Заявление на ");
	МассивПрефиксов.Добавить("Заявления ");
	МассивПрефиксов.Добавить("Информация о ");
	МассивПрефиксов.Добавить("Информация об ");
	МассивПрефиксов.Добавить("Информация ");
	
	Для каждого ТекстПрефикса Из МассивПрефиксов Цикл
		ДлинаПрефикса = СтрДлина(ТекстПрефикса);
		Если Лев(НаименованиеОтчета, ДлинаПрефикса) = ТекстПрефикса Тогда
			Результат[0] = Сред(Результат[0], ДлинаПрефикса + 1);
			Результат[0] = СокрЛП(Результат[0]);
			Прервать;
		КонецЕсли;
	КонецЦикла;
	
	Если МаксимальнаяДлина <> 0 Тогда
		КонецНаименованияБезПрефикса = Прав(Результат[0], МаксимальнаяДлина);
		КонецНаименованияБезПрефикса = СокрЛП(КонецНаименованияБезПрефикса);
		Результат[0] = Лев(Результат[0], МаксимальнаяДлина);
		Результат[0] = СокрЛП(Результат[0]);
		Если Результат[0] <> КонецНаименованияБезПрефикса Тогда
			Результат.Добавить(КонецНаименованияБезПрефикса);
		КонецЕсли;
	КонецЕсли;
...
Функция ПутьМодуляКриптографии() Экспорт
	...
	Если ПерсональныеНастройки.Свойство("ПутиКПрограммамЭлектроннойПодписиИШифрования")
		...
		ПоддерживаемыеКриптопровайдерыCryptoPro = КриптографияЭДКОКлиентСервер.КриптопровайдерCryptoPro("",, 0, "", Истина);
...
Функция ЭтоФайлВыгрузкиРеестраАкцизов(КороткоеИмяФайла) Экспорт
	...
	Префиксы.Добавить("KO_RRAKZ.201.11"); // 	РегламентированныйОтчетРеестрАкцизыВычетыДенатурированныйЭтиловыйСпирт
	Префиксы.Добавить("KO_RRAKZ.201.19"); // 	РегламентированныйОтчетРеестрАкцизыВычетыВиноматериалыВиногрИФруктСусло
	Префиксы.Добавить("KO_RRAKZ.201.29"); // 	РегламентированныйОтчетРеестрАкцизыВычетыВиноград
...
Функция МаксимальнойКоличествоФайловВПисьме() Экспорт
	...
	// КНД="1166102"
	// Приказ ФНС России от 29 декабря 2022 г. № ЕД-7-19/1295@
...
Функция ДопустимыеТипыФайловТребования() Экспорт
...
Функция ПредставительУказанКорректно(Форма, МастерДалее = Истина, ВыводитьСообщения) Экспорт
	...
	ПроверитьНаПустое(РезультатПроверкиРеквизитов, Форма, "ПредставительНомер", 	НСтр("ru = 'Номер'"));
	...
	ПроверитьНаПустое(РезультатПроверкиРеквизитов, Форма, "ПредставительПолномочияНомер", 		НСтр("ru = 'Номер'"));
...
Функция ЭтоСоциальнаяПенсия(Форма) Экспорт
	...
	ВидыПенсии.Добавить(ПредопределенноеЗначение("Перечисление.ВидПенсии.СоциальнаяПенсияДетямРодителиКоторыхНеизвестны"));
...
Функция НачислениеПенсииУказаноКорректно(Форма, МастерДалее = Истина, ВыводитьСообщения) Экспорт
	...
	ПроверитьНаПустое(РезультатПроверкиРеквизитов, Форма, "ОрганПФРПоМестуНазначенияПенсии", НСтр("ru = 'СФР (бывш. ПФР) по месту назначения пенсии'"));
...
// Перевод в формат вида: 112-233-445 95
Функция СНИЛССРазделителями(СНИЛСТолькоЦифры) Экспорт
	
	СНИЛС = СНИЛСБезРазделителей(СНИЛСТолькоЦифры);
	СНИЛС = Лев(СНИЛС, 3) + "-" + Сред(СНИЛС, 4, 3) + "-" + Сред(СНИЛС, 7, 3) + " " + Сред(СНИЛС, 10);
...
// Вызывает метод "Обновить" у таблицы формы, проверяет также видимость таблицы на форме
//
// Параметры:
//	ЭлементФормы - ТаблицаФормы
//
Процедура ТаблицаФормыОбновить(ЭлементФормы) Экспорт
	
	Если ЕстьВидимостьЭлемента(ЭлементФормы) Тогда
		ЭлементФормы.Обновить();
	КонецЕсли;
	
КонецПроцедуры

// Проверяет видимость элемента формы с учетом видимости родительских элементов
//
// Параметры:
//	ЭлементФормы - ГруппаФормы, ТаблицаФормы, ПолеФормы, КнопкаФормы - элемент формы 
//
// Возвращаемое значение:
//	Булево
//
Функция ЕстьВидимостьЭлемента(ЭлементФормы) Экспорт
	
	Результат = Истина;
	СледующийЭлемент = ЭлементФормы;
	
	Пока СледующийЭлемент <> Неопределено И Результат Цикл
		Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(СледующийЭлемент, "Видимость") Тогда
			Результат = Результат И СледующийЭлемент.Видимость;
		КонецЕсли;
		
		Если Результат И ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(СледующийЭлемент, "Родитель") Тогда
