...
Функция ОтборСотрудниковДляВыплаты() Экспорт
	...
	Параметры.Вставить("СпособВыплаты",  Справочники.СпособыВыплатыЗарплаты.ПустаяСсылка());
...
Функция ЗарплатаКВыплате(
	...
	ОграничитьЗарплатыСотрудниковПоСпособуМестуВыплат(Зарплата, Регистратор, ОтборСотрудников.СпособВыплаты, ОтборСотрудников.МестоВыплаты.Вид, ОтборСотрудников.МестоВыплаты.Значение, ПараметрыРасчета.Округление);
...
Функция ЗарплатаКВыплатеФизическихЛиц(
	...
	|ГДЕ
	|	(Сотрудники.ВидСобытия ЕСТЬ NULL
	|			ИЛИ НЕ(Сотрудники.ВидСобытия = ЗНАЧЕНИЕ(Перечисление.ВидыКадровыхСобытий.Увольнение)
	|					И ЗарплатаКВыплате.КВыплате = 0))
	|
	|УПОРЯДОЧИТЬ ПО
...
Функция НалогиКУдержанию(
	...
	СоздатьВТСписокСотрудниковПоТаблицеЗарплат(
		...
		ОписаниеОперации.ПериодРегистрации, 
		Регистратор,
		ОписаниеОперации.СпособПолучения <> Перечисления.СпособыПолученияЗарплатыКВыплате.Аванс);
	...
	ПараметрыРаспределения.ПериодРегистрации      = ОписаниеОперации.ПериодРегистрации;
...
Функция ВзысканияПоИсполнительнымДокументамМежрасчет(ЗарплатаКВыплате)
	...
	УчестьВзысканноеВАванс(Взыскания, УчитываемыеВыплаты);
...
Процедура УчестьВзысканноеВАванс(Взыскания, УчитываемыеВыплаты)
...
Процедура ОграничитьЗарплатыСотрудниковПоСпособуМестуВыплат(Зарплата, Регистратор, СпособВыплаты, ВидМестаВыплаты, МестоВыплаты, Округление)
	ВедомостьНаВыплатуЗарплатыВнутренний.ОграничитьЗарплатыСотрудниковПоСпособуМестуВыплат(
...
Процедура ЗарегистрироватьНДФЛКПеречислению(Ведомость, Отказ, НаборыЗаписейНДФЛ) Экспорт
	...
	УчетФактическиПолученныхДоходов.ЗарегистрироватьНовуюДатуПолученияДохода(Ведомость.Ссылка, НаборыЗаписейНДФЛ, МенеджерВременныхТаблиц, ДатаВыплаты, ДатаОперации, Отказ, Ложь);
	УчетНДФЛ.ВписатьСуммыВыплаченногоДоходаВУдержанныеНалоги(МенеджерВременныхТаблиц, Ведомость.Ссылка, ДатаВыплаты, ДатаОперации);
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("ЗарплатаКадрыПриложения.РасчетыСБюджетомПоНДФЛ") Тогда
		...
		Модуль.РасчетыСБюджетомПоНДФЛЗарегистрироватьНДФЛКПеречислению(Ведомость.Движения, Отказ);
	КонецЕсли;
