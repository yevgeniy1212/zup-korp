...
Функция ЕстьДоговорыГПХ(ФизическоеЛицо, Организация = Неопределено, НачалоПериода = Неопределено) Экспорт
	...
	ПараметрыЗапроса = ПараметрыПолученияСотрудниковОрганизацийПоСпискуФизическихЛиц();
	...
	ПараметрыЗапроса.РаботникиПоТрудовымДоговорам = Ложь;
	...
	СоздатьВТСотрудникиОрганизации(Запрос.МенеджерВременныхТаблиц, Истина, ПараметрыЗапроса);
	ЕстьДоговорыГПХ = (Запрос.Выполнить().Выгрузить().Количество() > 0);
	Возврат ЕстьДоговорыГПХ;
...
Функция ДокументыФизическихЛиц(СписокФизическихЛиц, ТолькоРазрешенные, ВидДокумента = Неопределено, ДатаПолученияДанных = Неопределено) Экспорт
	...
	|	ЕСТЬNULL(ДокументыФизическихЛиц.КатегорииТранспортныхСредств, """") КАК КатегорииТранспортныхСредств,
...
Функция НовыеДанныеФизическогоЛица() Экспорт
	...
	ДанныеФизическогоЛица.Вставить("ДатаРождения", Дата(1, 1, 1));
	ДанныеФизическогоЛица.Вставить("МестоРождения", "");
	ДанныеФизическогоЛица.Вставить("ИНН", "");
	ДанныеФизическогоЛица.Вставить("СтраховойНомерПФР", "");
	ДанныеФизическогоЛица.Вставить("Пол", Перечисления.ПолФизическогоЛица.Мужской);
	ДанныеФизическогоЛица.Вставить("ИмеетНаучныеТруды", Ложь);
	ДанныеФизическогоЛица.Вставить("ИмеетИзобретения", Ложь);
	ДанныеФизическогоЛица.Вставить("ДатаРегистрации", Дата(1, 1, 1));
	ДанныеФизическогоЛица.Вставить("ЛьготаПриНачисленииПособий",
		Перечисления.ОснованияФинансированияПособияФедеральнымБюджетом.ПустаяСсылка());
	ДанныеФизическогоЛица.Вставить("ОтношениеКОсобойЗоне", Перечисления.ОтношениеКОсобойЗоне.ПустаяСсылка());
	ДанныеФизическогоЛица.Вставить("ВидОсобойЗоны", Перечисления.ВидыОсобойЗоныПроживанияРаботы.ПустаяСсылка());
	ДанныеФизическогоЛица.Вставить("ПостоянноПроживалВКрыму18Марта2014Года", Ложь);
	ДанныеФизическогоЛица.Вставить("ПрименяетсяФЗ_20_2023", Ложь);
	ДанныеФизическогоЛица.Вставить("АдресФотографии", "");
	ДанныеФизическогоЛица.Вставить("ГруппаДоступа", Справочники.ГруппыДоступаФизическихЛиц.ПустаяСсылка());
	ДанныеФизическогоЛица.Вставить("Образование", Новый ТаблицаЗначений);
	ДанныеФизическогоЛица.Вставить("СостояниеВБраке", Новый Массив);
	ДанныеФизическогоЛица.Вставить("Гражданство", Новый Массив);
	ДанныеФизическогоЛица.Вставить("ТрудоваяДеятельность", Новый Массив);
	ДанныеФизическогоЛица.Вставить("ЗнаниеЯзыков", Новый Массив);
	ДанныеФизическогоЛица.Вставить("УченыеСтепени", Новый Массив);
	ДанныеФизическогоЛица.Вставить("КонтактнаяИнформация", Новый Массив);
...
Функция НоваяКонтактнаяИнформация() Экспорт
	...
	КонтактнаяИнформация.Вставить("ЗначенияПолей", "");
	КонтактнаяИнформация.Вставить("ВидИнформации", Справочники.ВидыКонтактнойИнформации.ПустаяСсылка());
	КонтактнаяИнформация.Вставить("ТипИнформации", Перечисления.ТипыКонтактнойИнформации.ПустаяСсылка());
...
//
Функция НовоеФизическоеЛицо(Знач ФИО, Знач ДанныеФизическогоЛица = Неопределено, Знач ПериодДанных = Неопределено,
	Знач Записывать = Истина) Экспорт
	...
	ФизическоеЛицоОбъект.ФИО = ФамилияИмяОтчество.Фамилия + " " + ФамилияИмяОтчество.Имя + " "
		+ ФамилияИмяОтчество.Отчество;
	...
	Если ДанныеФизическогоЛица <> Неопределено Тогда
		...
		Если ЗначениеЗаполнено(ФизическоеЛицоОбъект.Фамилия) Тогда
			ИсключаемыеСвойства = "Фамилия";
		КонецЕсли;
		
		Если ЗначениеЗаполнено(ФизическоеЛицоОбъект.Имя) Тогда
			ИсключаемыеСвойства = ?(ПустаяСтрока(ИсключаемыеСвойства), "", ИсключаемыеСвойства + ",") + "Имя";
		КонецЕсли;
		
		Если ЗначениеЗаполнено(ФизическоеЛицоОбъект.Отчество) Тогда
			ИсключаемыеСвойства = ?(ПустаяСтрока(ИсключаемыеСвойства), "", ИсключаемыеСвойства + ",") + "Отчество";
		КонецЕсли;
		
		Если ДанныеФизическогоЛица.Свойство("Наименование") Тогда
			ИсключаемыеСвойства = ?(ПустаяСтрока(ИсключаемыеСвойства), "", ИсключаемыеСвойства + ",") + "Наименование";
		КонецЕсли;
		...
		Если ДанныеФизическогоЛица.Свойство("УточнениеНаименования")Тогда
			ИсключаемыеСвойства = ?(ПустаяСтрока(ИсключаемыеСвойства), "", ИсключаемыеСвойства + ",")
				+ "УточнениеНаименования";
		КонецЕсли;
		...
		СвязанныеОбъекты = СвязанныеОбъектыДанныхФизическогоЛица(ДанныеФизическогоЛица);
	...
	Если Не ПустаяСтрока(Наименование) Тогда
		...
		ДанныеФИО.Вставить("Период", ПериодДанных);
...
Функция СвязанныеОбъектыДанныхФизическогоЛица(ДанныеФизическогоЛица) Экспорт
	Возврат КадровыйУчетВнутренний.СвязанныеОбъектыДанныхФизическогоЛица(ДанныеФизическогоЛица);
КонецФункции
...
Функция СреднесписочнаяЧисленностьРаботающих(Организация, НачалоПериода, КонецПериода, ПоГоловнойОрганизации = Ложь, Точность =0) Экспорт 
	Возврат СтатистикаПерсонала.СреднесписочнаяЧисленностьРаботающих(Организация, НачалоПериода, КонецПериода, ПоГоловнойОрганизации, Точность);
КонецФункции
...
//    а также имена реквизитов справочников Сотрудники и ФизическиеЛица.
...
//   ФизическоеЛицо    - СправочникСсылка.ФизическиеЛица
//   КадровыеДанные    - Строка - Имена полей перечисленные через запятую.
//     См. комментарий к КадровыйУчетВнутренний.ЗапросВТКадровыеДанныеФизическихЛиц,
//     а также имена реквизитов справочника ФизическиеЛица.
//   ДатаПолученияДанных - Дата - Дата на которую необходимо получить данные сотрудников.
//
// Возвращаемое значение:
//   Неопределено - Кадровые данные не найдены.
//   СтрокаТаблицыЗначений - Запрошенные данные.
//
Функция КадровыеДанныеФизическогоЛица(ТолькоРазрешенные, ФизическоеЛицо, КадровыеДанные, ДатаПолученияДанных) Экспорт
	Если Не ЗначениеЗаполнено(ФизическоеЛицо) Тогда
		Возврат Неопределено;
	КонецЕсли;
	СписокФизическихЛиц = Новый Массив;
	СписокФизическихЛиц.Добавить(ФизическоеЛицо);
	ТаблицаЗначений = КадровыеДанныеФизическихЛиц(ТолькоРазрешенные, СписокФизическихЛиц, КадровыеДанные, ДатаПолученияДанных);
	Если ТаблицаЗначений.Количество() = 0 Тогда
		Возврат Неопределено;
	КонецЕсли;
	Возврат ТаблицаЗначений[0];
...
// 
// Возвращаемое значение:
//  СправочникСсылка.Сотрудники - Новый сотрудник физического лица
...
// Поместить сотрудника в архив.
// 
// Параметры:
//  Сотрудник - СправочникСсылка.Сотрудники
//  ПомечатьНаУдаление - Булево - Помечать на удаление
...
Функция КадровыеДанныеОсновногоСотрудникаФизическогоЛица(Организация, ФизическоеЛицо, ИменаПолей, ДатаПолучения = '00010101', ТолькоРазрешенные = Истина, ТекстОшибки = "") Экспорт
	...
	ПараметрыПолучения.НачалоПериода       = ДатаПолучения;
	ПараметрыПолучения.ОкончаниеПериода    = ДатаПолучения;
	ПараметрыПолучения.КадровыеДанные      = ИменаПолей;
	
	Если СтрНайти(ПараметрыПолучения.КадровыеДанные, "ВидЗанятости") = 0 Тогда
		ПараметрыПолучения.КадровыеДанные = ПараметрыПолучения.КадровыеДанные + ", ВидЗанятости";
	КонецЕсли;
	
	КадровыйУчет.СоздатьВТСотрудникиОрганизации(МенеджерВременныхТаблиц, ТолькоРазрешенные, ПараметрыПолучения);
	
	Запрос = Новый Запрос("ВЫБРАТЬ * ИЗ ВТСотрудникиОрганизации");
	Запрос.МенеджерВременныхТаблиц = МенеджерВременныхТаблиц;
	Таблица = Запрос.Выполнить().Выгрузить();
	
	Найденные = Таблица.НайтиСтроки(Новый Структура("ВидЗанятости", Перечисления.ВидыЗанятости.ОсновноеМестоРаботы));
	Если Найденные.Количество() = 0 Тогда
		Найденные = Таблица.НайтиСтроки(Новый Структура("ВидЗанятости", Перечисления.ВидыЗанятости.Совместительство));
	КонецЕсли;
	Если Найденные.Количество() = 0 Тогда // Сотрудник уволен.
		...
		Если ОсновнойСотрудник <> Неопределено Тогда
			Найденные = КадровыеДанныеСотрудников(Ложь, ОсновнойСотрудник, ПараметрыПолучения.КадровыеДанные, ДатаПолучения);
		КонецЕсли;
	КонецЕсли;
	Если Найденные.Количество() > 0 Тогда
		Возврат Найденные[0];
	Иначе
		ТекстОшибки = НСтр("ru = 'Не удалось получить кадровые данные основного сотрудника ""%1"".'");
		ТекстОшибки = СтрШаблон(ТекстОшибки, ФизическоеЛицо);
		Возврат Неопределено;
	КонецЕсли;
КонецФункции
...
Функция ДоговорыФизическихЛиц(ТаблицаФизическихЛиц) Экспорт
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("ЗарплатаКадрыРасширеннаяПодсистемы.КадровыйУчетРасширенная") Тогда
		ИменаКадровыхДанных = "ТрудовойДоговорНомер,ТрудовойДоговорДата,ДатаПриема,ДатаУвольнения";
	Иначе
	...
	Запрос.Текст =
		...
		|		КОГДА ПериодыДействияДоговоровГражданскоПравовогоХарактера.ДокументОснование ЕСТЬ NULL
		|			ТОГДА ВЫБОР
		|					КОГДА НАЧАЛОПЕРИОДА(КадровыеДанныеСотрудников.ДатаУвольнения, ДЕНЬ) МЕЖДУ СотрудникиПериоды.НачалоПериода И СотрудникиПериоды.ОкончаниеПериода
		|						ТОГДА НАЧАЛОПЕРИОДА(КадровыеДанныеСотрудников.ДатаУвольнения, ДЕНЬ)
...
Функция ЗапросВТСведенияОбИнвалидности(ТолькоРазрешенные, ОписательВременнойТаблицыОтборов, ПоляОтбораПериодическихДанных, ИмяВТСведенияОбИнвалидности = "ВТСведенияОбИнвалидности") Экспорт
	...
	// ЗУПК
	ПереопределитьПризнакИнвалидностиФизическихЛиц(ЗапросВТИмяРегистраСрез.Текст);
	// ЗУПК
...
Функция ОписаниеВременнойТаблицыОтборовСотрудников(ИмяВременнойТаблицыОтборовСотрудников, ИмяПоляСотрудник = "Сотрудник", ИмяПоляПериод = "Период") Экспорт
	...
	ОписательВременнойТаблицыОтборов.Вставить("ПутьКПолюОрганизацияФункциональныхОпций", "");
...
// ЗУПК
Процедура ПереопределитьПризнакИнвалидностиФизическихЛиц(ТекстЗапроса)
	
	ПризнакИнвалидности = "ОтношениеКИнвалидности = ЗНАЧЕНИЕ(Перечисление.ОтношениеКИнвалидности.Инвалид)";
	ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "ТОГДА Инвалидность", "ТОГДА " + ПризнакИнвалидности);
...
Процедура ОбновитьТекущиеКадровыеДанныеСпискаСотрудников(Сотрудники, СотрудникиСОтмененнымСобытиемПрием) Экспорт
	...
	Запрос.Текст =
		...
		"ВЫБРАТЬ
		|	КадроваяИсторияСотрудников.Сотрудник КАК Сотрудник,
		|	КадроваяИсторияСотрудников.Период КАК ДатаУвольнения,
		|	ЕСТЬNULL(Увольнение.ПериодРегистрации, ЕСТЬNULL(УвольнениеСписком.ПериодРегистрации, КадроваяИсторияСотрудников.Период)) КАК ПериодРегистрации
		|ПОМЕСТИТЬ ВТ_УчетнаяПолитикаЗарплатаКадрыПериодРегистрации
		|ИЗ
		|	ВТКадроваяИсторияСотрудников КАК КадроваяИсторияСотрудников
		...
		|		ПО КадроваяИсторияСотрудников.Регистратор = УвольнениеСписком.Ссылка
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	МАКСИМУМ(УчетнаяПолитикаЗарплатаКадры.Период) КАК Период,
		|	УчетнаяПолитикаЗарплатаКадры.Организация КАК Организация,
		|	КадроваяИсторияСотрудников.Сотрудник КАК Сотрудник,
		|	КадроваяИсторияСотрудников.Период КАК ДатаУвольнения
		|ПОМЕСТИТЬ ВТ_МаксимумыУчетнаяПолитикаЗарплатаКадры
		|ИЗ
		|	ВТКадроваяИсторияСотрудников КАК КадроваяИсторияСотрудников
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВТ_УчетнаяПолитикаЗарплатаКадрыПериодРегистрации КАК КадроваяИсторияСотрудниковПериодРегистрации
		|		ПО КадроваяИсторияСотрудников.Сотрудник = КадроваяИсторияСотрудниковПериодРегистрации.Сотрудник
		|			И КадроваяИсторияСотрудников.Период = КадроваяИсторияСотрудниковПериодРегистрации.ДатаУвольнения
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.УчетнаяПолитикаЗарплатаКадры КАК УчетнаяПолитикаЗарплатаКадры
		|		ПО (УчетнаяПолитикаЗарплатаКадры.Организация = КадроваяИсторияСотрудников.ГоловнаяОрганизация)
		|			И КадроваяИсторияСотрудниковПериодРегистрации.ПериодРегистрации >= УчетнаяПолитикаЗарплатаКадры.Период
		|			И КадроваяИсторияСотрудников.ВидСобытия = ЗНАЧЕНИЕ(Перечисление.ВидыКадровыхСобытий.Увольнение)
		|
		|СГРУППИРОВАТЬ ПО
		|	УчетнаяПолитикаЗарплатаКадры.Организация,
		|	КадроваяИсторияСотрудников.Сотрудник,
		...
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	ВТ_МаксимумыУчетнаяПолитикаЗарплатаКадры.Сотрудник КАК Сотрудник,
		|	ВТ_МаксимумыУчетнаяПолитикаЗарплатаКадры.ДатаУвольнения КАК ДатаУвольнения,
		|	ЕСТЬNULL(УчетнаяПолитикаЗарплатаКадры.ДеньУвольненияЯвляетсяРабочимДнем, ИСТИНА) КАК ДеньУвольненияЯвляетсяРабочимДнем
		|ПОМЕСТИТЬ ВТ_УчетнаяПолитикаЗарплатаКадрыПоСотрудникам
		|ИЗ
		|	ВТ_МаксимумыУчетнаяПолитикаЗарплатаКадры КАК ВТ_МаксимумыУчетнаяПолитикаЗарплатаКадры
		|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.УчетнаяПолитикаЗарплатаКадры КАК УчетнаяПолитикаЗарплатаКадры
		|		ПО ВТ_МаксимумыУчетнаяПолитикаЗарплатаКадры.Организация = УчетнаяПолитикаЗарплатаКадры.Организация
		|			И ВТ_МаксимумыУчетнаяПолитикаЗарплатаКадры.Период = УчетнаяПолитикаЗарплатаКадры.Период
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|УНИЧТОЖИТЬ ВТ_УчетнаяПолитикаЗарплатаКадрыПериодРегистрации
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|УНИЧТОЖИТЬ ВТ_МаксимумыУчетнаяПолитикаЗарплатаКадры
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		// ЗУПК
		...
		|	ВЫБОР
		|		КОГДА КадроваяИсторияСотрудниковУвольнение.Период ЕСТЬ NULL
		|			ТОГДА ДАТАВРЕМЯ(1, 1, 1)
		|		КОГДА УчетнаяПолитикаЗарплатаКадрыПоСотрудникам.ДеньУвольненияЯвляетсяРабочимДнем ЕСТЬ NULL
		|			ИЛИ НЕ УчетнаяПолитикаЗарплатаКадрыПоСотрудникам.ДеньУвольненияЯвляетсяРабочимДнем
		|			ТОГДА КадроваяИсторияСотрудниковУвольнение.Период
		|		ИНАЧЕ ДОБАВИТЬКДАТЕ(КадроваяИсторияСотрудниковУвольнение.Период, ДЕНЬ, -1)
		...
		// ЗУПК
		|		ЛЕВОЕ СОЕДИНЕНИЕ ВТ_УчетнаяПолитикаЗарплатаКадрыПоСотрудникам КАК УчетнаяПолитикаЗарплатаКадрыПоСотрудникам
		|		ПО КадроваяИсторияСотрудниковУвольнение.Сотрудник = УчетнаяПолитикаЗарплатаКадрыПоСотрудникам.Сотрудник
		|			И КадроваяИсторияСотрудниковУвольнение.Период >= УчетнаяПолитикаЗарплатаКадрыПоСотрудникам.ДатаУвольнения
		// ЗУПК
		|ГДЕ
		...
		|	МИНИМУМ(НовыеКадровыеДанные.ДатаПриема) КАК ДатаПриема,
		|	МИНИМУМ(НовыеКадровыеДанные.ДатаУвольнения) КАК ДатаУвольнения,
		...
		|	НовыеКадровыеДанные.ОформленПоТрудовомуДоговору";
...
Процедура ИсключитьПовторениеЗаписейТекущихДанныхСотрудников(ИмяРегистраТекущихСведений, ОтключитьБизнесЛогику = Ложь) Экспорт
	...
	Если НЕ РезультатЗапроса.Пустой() Тогда
		...
		Пока Выборка.Следующий() Цикл
			...
			Набор.ДополнительныеСвойства.Вставить("ПолноеПереформирование", Истина);
			Набор.Записать();
...
Процедура ДобавитьВТекстЗапросаОписаниеПолей(ТекстЗапроса, ТекстыОписанияПолей, СтрокаПоместить) Экспорт
	...
	Если ТекстыОписанияПолей.Количество() > 0 Тогда
		...
		Если СтрНайти(ЧастиТекстаЗапроса[ЧастиТекстаЗапроса.Количество() - 1], "КадроваяИсторияСотрудников.Организация КАК Организация") > 0 Тогда
			ЧастиТекстаЗапроса.Добавить("{ВЫБРАТЬ
				|	Организация.* КАК ОрганизацияКадровыхДанных}");
		КонецЕсли;
...
Функция НеобходимыСведенияЗависящиеОтФункциональныхОпцийОрганизации(Знач ИмяПоля) Экспорт
	...
	Возврат ИмяПоля = ВРег("ЯвляетсяЧленомЛетногоЭкипажа")
		...
		Или ИмяПоля = ВРег("РасчетОтпускаПоРабочимДням")
...
Процедура ДополнитьИменаКадровыхДанных(КадровыеДанные) Экспорт
	...
	КадровыйУчетВнутренний.ДополнитьИменаКадровыхДанных(КадровыеДанные);
	
КонецПроцедуры

Процедура ДополнитьПоляЗапросаВТКадровыхДанных(Запрос) Экспорт
	
	КадровыйУчетВнутренний.ДополнитьПоляЗапросаВТКадровыхДанных(Запрос);
...
Процедура ДобавитьВКоллекциюКадровыеДанныеДляСведенийОКлассахУсловийТруда(ИсточникиДанных, Знач ИмяПоля) Экспорт
	КадровыйУчетВнутренний.ДобавитьВКоллекциюКадровыеДанныеДляСведенийОКлассахУсловийТруда(ИсточникиДанных, ИмяПоля);
КонецПроцедуры

Функция ИмяИсточникаДанныхКадровойИсторииСотрудников(Знач ИсточникиДанных) Экспорт
	Возврат КадровыйУчетВнутренний.ИмяИсточникаДанныхКадровойИсторииСотрудников(ИсточникиДанных);
КонецФункции

Процедура ДобавитьСоединениеСКадровымиДанными(ОписательВременнойТаблицыОтборов, ЧастиЗапроса, Знач ИмяПоляДолжность) Экспорт
	КадровыйУчетВнутренний.ДобавитьСоединениеСКадровымиДанными(ОписательВременнойТаблицыОтборов, ЧастиЗапроса, ИмяПоляДолжность);
КонецПроцедуры
...
Функция ПутьКДаннымСведенийОбУсловияхДоговоровОпеки(ИмяПоля) Экспорт
	...
	Возврат КадровыйУчетВнутренний.ПутьКДаннымСведенийОбУсловияхДоговоровОпеки(ИмяПоля);
	
КонецФункции

Процедура ДобавитьТекстЗапросаВТСведенияОбУсловияхДоговоровОпеки(Запрос, ТолькоРазрешенные, ОписательВременнойТаблицыОтборов, ПоляОтбораПериодическихДанных, ФормироватьСПериодичностьДень, ИсточникиДанных) Экспорт
	
	КадровыйУчетВнутренний.ДобавитьТекстЗапросаВТСведенияОбУсловияхДоговоровОпеки(Запрос, ТолькоРазрешенные, ОписательВременнойТаблицыОтборов, ПоляОтбораПериодическихДанных, ФормироватьСПериодичностьДень, ИсточникиДанных);
...
Функция ДобавитьПолеСведенийМестРаботыСотрудников(ИмяПоля, ТекстыОписанияПолей, ИсточникиДанных) Экспорт
	...
	Возврат КадровыйУчетВнутренний.ДобавитьПолеСведенийМестРаботыСотрудников(ИмяПоля, ТекстыОписанияПолей, ИсточникиДанных);
	
КонецФункции

Функция ПутьКДаннымСведенийМестРаботыСотрудников(Знач ИмяПоля) Экспорт
	
	Возврат КадровыйУчетВнутренний.ПутьКДаннымСведенийМестРаботыСотрудников(ИмяПоля)
...
Функция ПараметрыКадровогоПриказа() Экспорт
	...
	СтруктураПараметровКадровогоПриказа.Вставить("Сотрудник");
...
Функция ПредставлениеСтатьиУвольненияДляТ8(СтатьяУвольнения) Экспорт
	...
	Если Не ЗначениеЗаполнено(СвойстваОснованияУвольнения.ОснованиеУвольненияНаименованиеОснования) Тогда
		// ЗУПК
		Возврат КадровыйУчетРК.ПредставлениеПричиныУвольнения(СвойстваОснованияУвольнения, Истина);
		// ЗУПК
	КонецЕсли;
		...
		Иначе
			...
			Если РаботникиПоДоговорамГПХ = Истина
				...
				ТекстЗапросаСотрудникиОрганизацииДляКадровойИстории =
					...
					|	РабочиеМестаСотрудников.ФизическоеЛицо КАК ФизическоеЛицо,
					|	РабочиеМестаСотрудников.Организация КАК ОрганизацияФункциональныхОпций,
					|	ВЫБОР
			...
			ИначеЕсли РаботникиПоДоговорамГПХ = Истина
				...
				ТекстЗапросаСотрудникиОрганизацииДляКадровойИстории =
					...
					|
					|ИМЕЮЩИЕ
					|	МАКСИМУМ(РабочиеМестаСотрудников.РольСотрудника = ЗНАЧЕНИЕ(Перечисление.РолиСотрудников.Работник)) = ЛОЖЬ И
					|	МАКСИМУМ(РабочиеМестаСотрудников.РольСотрудника = ЗНАЧЕНИЕ(Перечисление.РолиСотрудников.Договорник)) = ИСТИНА";
...
Процедура ОбновитьОсновныхСотрудниковФизическихЛицПоТрудовымДоговорам(Сотрудники) Экспорт
	...
	Запрос.Текст =
		...
		|	И ВидыЗанятостиСотрудников.ФизическоеЛицо = ВидыЗанятостиСотрудников.Сотрудник.ФизическоеЛицо
		|
		|СГРУППИРОВАТЬ ПО
...
Процедура ВывестиНаПечатьТ1(ПараметрыПечати, ДокументРезультат, ДанныеСсылок, МакетПечатнойФормы, ИдентификаторыМакета = Неопределено, ОбъектыПечати = Неопределено) Экспорт
	...
	Для Каждого ДанныеНаПечать Из ДанныеСсылок Цикл
		...
		// ЗУПК
		ФормированиеПечатныхФормПоПравиламВеденияДО.ЗаполнениеРеквизитовШапкиБезОрганизации(МакетПечатнойФормы, ДанныеНаПечать);
		// ЗУПК
		...
		Для Каждого ДанныеДетальныхЗаписей Из ДанныеНаПечать.Строки Цикл
			...
			КадровыйЭДО.ЗадатьДетальнуюОбластьПечати(ПараметрыПечати, ДокументРезультат, "ПФ_MXL_Т1", НомерСтрокиНачалаОбласти,
...
Процедура ВывестиНаПечатьТ5(ПараметрыПечати, ДокументРезультат, ДанныеСсылок, МакетПечатнойФормы, ИдентификаторыМакета = Неопределено, ОбъектыПечати = Неопределено) Экспорт
	...
	Для Каждого ДанныеНаПечать Из ДанныеСсылок Цикл
		...
		Для Каждого ДанныеДетальныхЗаписей Из ДанныеНаПечать.Строки Цикл
			...
			КадровыйЭДО.ЗадатьДетальнуюОбластьПечати(ПараметрыПечати, ДокументРезультат, "ПФ_MXL_Т5", НомерСтрокиНачалаОбласти,
				ДанныеДетальныхЗаписей, ДанныеНаПечать.СсылкаНаОбъект);
...
Процедура ВывестиНаПечатьТ8(ПараметрыПечати, ДокументРезультат, ДанныеСсылок, МакетПечатнойФормы, ИдентификаторыМакета = Неопределено, ОбъектыПечати = Неопределено) Экспорт
	
	ДокументРезультат.КлючПараметровПечати = "ПараметрыПечати_ПриказОбУвольнении";
	ДанныеДатыНаПечать = Новый Структура();
	
	Для Каждого ДанныеНаПечать Из ДанныеСсылок Цикл
		
		Если ДанныеНаПечать.Владелец().Колонки.Найти("СсылкаНаОбъектДатаНаПечать") <> Неопределено Тогда
			ДанныеДатыНаПечать.Вставить("СсылкаНаОбъектДата", ДанныеНаПечать.СсылкаНаОбъектДатаНаПечать);
		КонецЕсли;
		ПерваяСтрокаПечатнойФормы = ДокументРезультат.ВысотаТаблицы + 1;
		...
		Для Каждого ДанныеДетальныхЗаписей Из ДанныеНаПечать.Строки Цикл
			...
			КадровыйЭДО.ЗадатьДетальнуюОбластьПечати(ПараметрыПечати, ДокументРезультат, "ПФ_MXL_Т8", НомерСтрокиНачалаОбласти,
				ДанныеДетальныхЗаписей, ДанныеНаПечать.СсылкаНаОбъект);
...
Процедура ВывестиНаПечатьТ6(ПараметрыПечати, ДокументРезультат, ДанныеСсылок, МакетПечатнойФормы, ИдентификаторыМакета = Неопределено, ОбъектыПечати = Неопределено) Экспорт
	
	ДокументРезультат.КлючПараметровПечати = "ПараметрыПечати_ПриказОПредоставленииОтпуска";
	
	Для Каждого ДанныеСотрудников Из ДанныеСсылок Цикл
		...
		Для Каждого ДанныеНаПечать Из ДанныеСотрудников.Строки Цикл
			...
			// ЗУПК
			ФормированиеПечатныхФормПоПравиламВеденияДО.ЗаполнениеРеквизитовШапки(МакетПечатнойФормы, ДанныеНаПечать,"СсылкаНаОбъектДатаНаПечать", "РаботаОрганизация", "РаботаОрганизацияНаименованиеПолное");
			// ЗУПК
			...
			СсылкаНаОбъектДанныеОтпусковОснование = "";
			ОснованиеОтпусков = Новый Структура("СсылкаНаОбъектДанныеОтпусковОснование");
			ЗаполнитьЗначенияСвойств(ОснованиеОтпусков, ДанныеНаПечать); 
			Если ЗначениеЗаполнено(ОснованиеОтпусков["СсылкаНаОбъектДанныеОтпусковОснование"]) Тогда 
				СсылкаНаОбъектДанныеОтпусковОснование = ОснованиеОтпусков["СсылкаНаОбъектДанныеОтпусковОснование"];
			КонецЕсли;
			...
			Иначе
				...
				Для Каждого ДанныеДетальныхЗаписей Из ДанныеНаПечать.Строки Цикл
					...
					Если Не ЗначениеЗаполнено(ДанныеНаПечать.СсылкаНаОбъектПродолжительностьОсновногоОтпуска) Тогда
						...
						Если ЗначениеЗаполнено(СсылкаНаОбъектДанныеОтпусковОснование) Тогда
							ДанныеВычисляемыеНаХоду.Вставить("СсылкаНаОбъектОснование", СсылкаНаОбъектДанныеОтпусковОснование);
						КонецЕсли;
				...
				ДанныеВычисляемыеНаХоду.Вставить("СсылкаНаОбъектДатаНачалаОсновногоОтпускаНаПечать", ДанныеНаПечать.СсылкаНаОбъектДатаНачалаОсновногоОтпускаНаПечать);
				ДанныеВычисляемыеНаХоду.Вставить("СсылкаНаОбъектДатаОкончанияОсновногоОтпускаНаПечать", ДанныеНаПечать.СсылкаНаОбъектДатаОкончанияОсновногоОтпускаНаПечать);
				
				Если ЗначениеЗаполнено(ДанныеНаПечать.СсылкаНаОбъектПродолжительностьОсновногоОтпуска) Тогда
					ДанныеВычисляемыеНаХоду.Вставить("СсылкаНаОбъектНачалоПериодаЗаКоторыйПредоставляетсяОтпускНаПечать", ДанныеНаПечать.СсылкаНаОбъектНачалоПериодаЗаКоторыйПредоставляетсяОтпускНаПечать);
				...
				Иначе
					...
					Если ЗначениеЗаполнено(ДанныеНаПечать.СсылкаНаОбъектПродолжительностьДополнительныхОтпусков) Тогда
						ДанныеВычисляемыеНаХоду.Вставить("СсылкаНаОбъектДанныеОтпусковПродолжительность", ДанныеНаПечать.СсылкаНаОбъектПродолжительностьДополнительныхОтпусков);
					КонецЕсли;
			...
			КадровыйЭДО.ЗадатьДетальнуюОбластьПечати(ПараметрыПечати, ДокументРезультат, "ПФ_MXL_Т6", НомерСтрокиНачалаОбласти,
...
Процедура ЗарегистрироватьОбработчикиОбновления(Обработчики) Экспорт
	...
	Обработчик.Процедура = "КадровыйУчет.ЗаполнитьВидыКонтактнойИнформацииФизическогоЛица";
	...
	Обработчик = Обработчики.Добавить();
	Обработчик.Версия = "3.1.18.120";
	Обработчик.РежимВыполнения = "Отложенно";
	Обработчик.НачальноеЗаполнение = Истина;
	Обработчик.Идентификатор = Новый УникальныйИдентификатор("87b8cf17-86b3-4a75-b4f2-714eb18cb9ad");
	Обработчик.Комментарий = НСтр("ru = 'Устанавливает признак используемости оснований увольнения'");
	Обработчик.Процедура = "Справочники.ОснованияУвольнения.ОбновитьОснованияВАрхиве";
	
	Если ЗарплатаКадры.АвтономнаяРаботаРазрешеноИзменениеДанных(Метаданные.Справочники.ОснованияУвольнения) Тогда
		Обработчик = Обработчики.Добавить();
		Обработчик.Версия = "3.1.24.306";
	...
	КадровыйЭДО.ЗарегистрироватьОбработчикиОбновления(Обработчики);
...
Процедура ЗаполнитьДокументыФизическихЛицПоКлассификаторуМВД() Экспорт
	...
	// ЗУПК
	ОписатьДокумент(Справочники.ВидыДокументовФизическихЛиц.Загранпаспорт, "",									"01","");
	ОписатьДокумент(Справочники.ВидыДокументовФизическихЛиц.ПаспортМоряка,			"",							"03","");	// ББ 0999999
	ОписатьДокумент(Справочники.ВидыДокументовФизическихЛиц.ВидНаЖительство, "",								"04","");	// -
	ОписатьДокумент(, НСтр("ru='Удостоверение беженца'"),														"","");
	ОписатьДокумент(, НСтр("ru='Служебный паспорт РК'"),										"","");
	ОписатьДокумент(, НСтр("ru='Свидетельство на возвращение'"),												"","");
	ОписатьДокумент(, НСтр("ru='Иной документ, выдаваемый уполномоченными государственными органами'"),			"","");
	ОписатьДокумент(, НСтр("ru='Удостоверение лица без гражданства'"),											"","");
...
// Инициализирует виды контактной информации физических лиц.
Процедура ЗаполнитьВидыКонтактнойИнформацииФизическогоЛица() Экспорт
	
	ЗаполнитьВидыКонтактнойИнформацииФизическогоЛицаПоПорядку(1);
...
// Не подходит для инициализации новых свойств существующих видов контактной информации,
// поскольку передает в функцию ПараметрыВидаКонтактнойИнформации значение перечисления.
Процедура ЗаполнитьВидыКонтактнойИнформацииФизическогоЛицаПоПорядку(Порядок = Неопределено)
	...
	СвойстваВидаКИ.Наименование = НСтр("ru = 'Адрес за пределами РК'");
	СвойстваВидаКИ.МожноИзменятьСпособРедактирования = Истина;
	СвойстваВидаКИ.ВидРедактирования = "Диалог";
	СвойстваВидаКИ.ОбязательноеЗаполнение = Ложь;
...
Процедура ПриДобавленииИсключенийПоискаСсылок(ИсключенияПоискаСсылок) Экспорт
	РегистрыСведений.ОсновныеСотрудникиФизическихЛиц.ПриДобавленииИсключенийПоискаСсылок(ИсключенияПоискаСсылок);
КонецПроцедуры
...
Функция КадровыеДанныеСотрудниковПоТаблицеЗначений(ТолькоРазрешенные, ТаблицаСотрудников, КадровыеДанные, ПоляОтбораПериодическихДанных = Неопределено) Экспорт
	...
	ЗапросКадровыхДанных = ЗапросВТКадровыеДанныеСотрудников(
		ТолькоРазрешенные, ОписательВременнойТаблицыОтборов, КадровыеДанные, ПоляОтбораПериодическихДанных, , , "");
...
Процедура ЗаполнитьКлассификаторОснованийУвольнения(ИмяСправочника, Макет, НормативныйДокумент = Неопределено) Экспорт
	...
	ПричиныУвольненияПФР = Запрос.Выполнить().Выгрузить();
	...
	Запрос.Текст =
		...
		|	ОснованияУвольнения.СрокДействия КАК СрокДействия
		|ИЗ
	...
	ИспользуютсяАльтернативныеНаименования = ДанныеМакетаКлассификатора.Колонки.Найти("TitleAlternative") <> Неопределено;
	Для Каждого СтрокаМакета Из ДанныеМакетаКлассификатора Цикл
		...
		// ЗУПК
		Если ПолучитьФункциональнуюОпцию("ОтключенныйФункционалЗарплатаКадры") И ЗначениеЗаполнено(СтрокаМакета.ExpirationDate) Тогда
		// ЗУПК
			СрокДействия = СтроковыеФункцииКлиентСервер.СтрокаВДату(
				СтрокаМакета.ExpirationDate, ЧастиДаты.Дата);
		Иначе
			СрокДействия = '00010101';
		КонецЕсли;
		КрайнийСрокДействия = ДобавитьМесяц(ТекущаяДатаСеанса(), -12);
		
		СтрокаКлассификатора = ДанныеКлассификатора.Найти(ВРег(СтрокаМакета.Title), "Наименование");
		Если СтрокаКлассификатора = Неопределено
			И ИспользуютсяАльтернативныеНаименования
			И ЗначениеЗаполнено(СтрокаМакета.TitleAlternative) Тогда
			
			СтрокаКлассификатора = ДанныеКлассификатора.Найти(ВРег(СтрокаМакета.TitleAlternative), "Наименование");
		КонецЕсли;
		...
		Если ИспользоватьПредопределенныеЭлементы И ЗначениеЗаполнено(СтрокаМакета.ID) Тогда
			...
			Исключение
				...
				Если СтрокаКлассификатора = Неопределено Тогда
					Если ЗначениеЗаполнено(СрокДействия)
						И СрокДействия < КрайнийСрокДействия Тогда
		...
		ПричинаУвольненияПФР = Неопределено;
		// ЗУПК
		Если ПолучитьФункциональнуюОпцию("ОтключенныйФункционалЗарплатаКадры") И ЗначениеЗаполнено(СтрокаМакета.CodePFR) Тогда
		// ЗУПК
			СтрокаПФР = ПричиныУвольненияПФР.Найти(СтрокаМакета.CodePFR, "Код");
			Если СтрокаПФР <> Неопределено Тогда
				ПричинаУвольненияПФР = СтрокаПФР.Ссылка;
			КонецЕсли;
		КонецЕсли;
		...
		// ЗУПК
		ОбъектСправочника.ПричинаУвольненияИзОрганизации = Перечисления.ПричиныУвольненияИзОрганизации[СтрокаМакета.Base];
		// ЗУПК
		...
		ОбъектСправочника.Абзац                = СтрокаМакета.Passage;
		ОбъектСправочника.ДокументОснование    = ДокументОснование;
		ОбъектСправочника.СрокДействия         = СрокДействия;
		ОбъектСправочника.ПричинаУвольненияПФР = ПричинаУвольненияПФР;
...
Процедура ОбновитьОснованияУвольненияВАрхиве(ИмяСправочника) Экспорт
	...
	Запрос.Текст =
		...
		|		ПО ОснованияУвольнения.Ссылка = ОснованияУвольненияВАрхиве.ОснованиеУвольнения
		|ГДЕ
		|	ОснованияУвольнения.СрокДействия > ДАТАВРЕМЯ(1, 1, 1)
		|	И ОснованияУвольненияВАрхиве.ОснованиеУвольнения ЕСТЬ NULL";
	
	Запрос.Текст = СтрЗаменить(Запрос.Текст, "Справочник.ОснованияУвольнения", "Справочник." + ИмяСправочника);
	
	Выборка = Запрос.Выполнить().Выбрать();
	Пока Выборка.Следующий() Цикл
		РегистрыСведений.ОснованияУвольненияВАрхиве.ПоместитьОснованиеУвольненияВАрхив(Выборка.Ссылка, Истина);
	КонецЦикла;
	
КонецПроцедуры

Процедура ОснованияУвольненияПриСозданииНаСервереФормыСписка(УправляемаяФорма) Экспорт
	
	УстановитьОтборДействующих = Истина;
	Если ЗначениеЗаполнено(УправляемаяФорма.Параметры.ТекущаяСтрока) Тогда
		
		Если ТипЗнч(УправляемаяФорма.Параметры.ТекущаяСтрока)
			<> Тип("СправочникСсылка.ОснованияУвольненияПользовательские") Тогда
			
			Если ЗначениеЗаполнено(ОбщегоНазначения.ЗначениеРеквизитаОбъекта(
				УправляемаяФорма.Параметры.ТекущаяСтрока, "СрокДействия")) Тогда
				
				УстановитьОтборДействующих = Ложь;
			КонецЕсли;
		КонецЕсли;
		
		Если УстановитьОтборДействующих Тогда
			Если РегистрыСведений.ОснованияУвольненияВАрхиве.ОснованиеВАрхиве(
				УправляемаяФорма.Параметры.ТекущаяСтрока) Тогда
	...
	ЭлементУсловногоОформления.Оформление.УстановитьЗначениеПараметра("Шрифт",
		Новый Шрифт(УправляемаяФорма.Элементы.Список.Шрифт,,,,,, Истина));
	
	ОформляемоеПоле = ЭлементУсловногоОформления.Поля.Элементы.Добавить(); 
	ОформляемоеПоле.Поле = Новый ПолеКомпоновкиДанных("Список");
	
	ЭлементОтбора = ЭлементУсловногоОформления.Отбор.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
	ЭлементОтбора.ЛевоеЗначение = Новый ПолеКомпоновкиДанных("Список.ВАрхиве");
	ЭлементОтбора.ВидСравнения = ВидСравненияКомпоновкиДанных.Равно;
	ЭлементОтбора.ПравоеЗначение = Истина;
	ЭлементОтбора.Использование = Истина;
	
	Если УстановитьОтборДействующих Тогда
		КадровыйУчетКлиентСервер.ОснованияУвольненияУстановитьОтборПоказатьНеИспользуемые(УправляемаяФорма);
	КонецЕсли;
	
	ЕстьРеквизитРабочаяДата = ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(
		УправляемаяФорма, "РабочаяДата");
	
	Если ЕстьРеквизитРабочаяДата Тогда
		
		УправляемаяФорма.РабочаяДата = ОбщегоНазначения.ТекущаяДатаПользователя();
		...
		ЭлементОтбора.ЛевоеЗначение = Новый ПолеКомпоновкиДанных("Список.СрокДействия");
		...
		ЭлементОтбора.ПравоеЗначение = УправляемаяФорма.РабочаяДата;
		ЭлементОтбора.Использование = Истина;
		
		Если УстановитьОтборДействующих Тогда
			КадровыйУчетКлиентСервер.ОснованияУвольненияУстановитьОтборТолькоДействующих(УправляемаяФорма);
		КонецЕсли;
...
Процедура ОбработкаПолученияДанныхВыбораОснованийУвольнения(ДанныеВыбора, Параметры, СтандартнаяОбработка) Экспорт
	...
	Запрос.Текст =
		...
		|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ОснованияУвольненияВАрхиве КАК ОснованияУвольненияВАрхиве
		|		ПО Источник.Ссылка = ОснованияУвольненияВАрхиве.ОснованиеУвольнения
		|ГДЕ
		|	ВЫБОР
		|			КОГДА Представления.Использовать
		|				ТОГДА Представления.Наименование
		...
		|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ОснованияУвольненияВАрхиве КАК ОснованияУвольненияВАрхиве
		|		ПО ОснованияУвольненияПользовательские.Ссылка = ОснованияУвольненияВАрхиве.ОснованиеУвольнения
		|ГДЕ
...
// Возвращает соответствие с ключи которого физические лица и значения соответствующие им основные сотрудники
...
// Возвращает признак того, что ранее по физическому лицу не регистрировались кадровые события.
// Т.е. производится первый прием на работу.
//
// Параметры:
//  ФизическоеЛицо          - СправочникСсылка.ФизическиеЛица
//  ДатаСобытия             - Дата
//  ИсключаемыеРегистраторы - ДокументСсылка, могущий выступать в качестве регистратора и движения
//                             которого необходимо исключить
//                            Массив ссылок на исключаемые регистраторы
//                            Неопределено, если ссылок на регистраторы нет
//
// Возвращаемое значение:
//  Булево - Истина, более ранние кадровые события не зарегистрированы
//
Функция ЭтоНачалоТрудовойДеятельности(Знач ФизическоеЛицо, Знач ДатаСобытия, Знач ИсключаемыеРегистраторы) Экспорт
	...
	Запрос.Текст =
		...
		|	И КадроваяИсторияСотрудников.Период <= &ДатаСобытия
		|	И НЕ КадроваяИсторияСотрудников.Регистратор В (&ИсключаемыеРегистраторы)";
...
Процедура ЗаполнитьНастройкиПечатныхФормПоУмолчанию(ОписанияНастроек) Экспорт
	...
	КадровыйУчетВнутренний.ЗаполнитьНастройкиПечатныхФормПоУмолчанию(ОписанияНастроек);
	
КонецПроцедуры

#КонецОбласти

// Возвращает информацию о действующих приостановлениях трудовых договров.
//
// Параметры:
//	ТолькоРазрешенные - Булево
//	ТаблицаОтбора	  - ТаблицаЗначений, с колонками
//						* ГоловнаяОрганизация - СправочникСсылка.Организации
//						* ФизическоеЛицо      - СправочникСсылка.ФизическиеЛица
//						* НачалоПериода       - Дата
//						* ОкончаниеПериода    - Дата
//	Отбор			  - Структура, поддерживает отбор по ключам:
//						* ОснованиеДокументОснование - Массив, ПеречислениеСсылка.НормативныеДокументыОснованийКадровыхПриказов
//
// Возвращаемое значение:
//	ТаблицаЗначений - см. ПустаяТаблицаПриостановленийТрудовыхДоговоров
//
Функция ПриостановленияТрудовыхДоговоров(ТолькоРазрешенные, ТаблицаОтбора, Отбор) Экспорт
	
	Возврат КадровыйУчетВнутренний.ПриостановленияТрудовыхДоговоров(ТолькоРазрешенные, ТаблицаОтбора, Отбор);
	
КонецФункции

// Возвращает пустую таблицу значений с периодами приостановления тьрудовых договоров.
//
// Возвращаемое значение:
//	ТаблицаЗначений:
//		* ГоловнаяОрганизация - СправочникСсылка.Организации, (как в ТаблицаОтбора)
//		* ФизическоеЛицо      - СправочникСсылка.ФизическиеЛица, (как в ТаблицаОтбора)
//		* НачалоПериода       - Дата, (как в ТаблицаОтбора)
//		* ОкончаниеПериода    - Дата, (как в ТаблицаОтбора)
//		* Сотрудник           - СправочникСсылка.Сотрудники
//		* ВидЗанятости        - ПеречислениеСсылка.ВидыЗанятости
//		* ДатаНачала          - Дата
//		* ДатаОкончания       - Дата
//		* Основание           - СправочникСсылка.ОснованияПриостановленияТрудовыхДоговоровКонтрактов
//		* ОснованиеИмяПредопределенныхДанных - Строка
//		* ОснованиеДокументОснование         - ПеречислениеСсылка.НормативныеДокументыОснованийКадровыхПриказов
//
Функция ПустаяТаблицаПриостановленийТрудовыхДоговоров() Экспорт
	
	ТаблицаПриостановлений = Новый ТаблицаЗначений;
	ТаблицаПриостановлений.Колонки.Добавить("ГоловнаяОрганизация", Новый ОписаниеТипов("СправочникСсылка.Организации"));
	ТаблицаПриостановлений.Колонки.Добавить("ФизическоеЛицо", Новый ОписаниеТипов("СправочникСсылка.ФизическиеЛица"));
	ТаблицаПриостановлений.Колонки.Добавить("НачалоПериода", Новый ОписаниеТипов("Дата"));
	ТаблицаПриостановлений.Колонки.Добавить("ОкончаниеПериода", Новый ОписаниеТипов("Дата"));
	ТаблицаПриостановлений.Колонки.Добавить("Сотрудник", Новый ОписаниеТипов("СправочникСсылка.Сотрудники"));
	ТаблицаПриостановлений.Колонки.Добавить("ВидЗанятости", Новый ОписаниеТипов("ПеречислениеСсылка.ВидыЗанятости"));
	ТаблицаПриостановлений.Колонки.Добавить("ДатаНачала", Новый ОписаниеТипов("Дата"));
	ТаблицаПриостановлений.Колонки.Добавить("ДатаОкончания", Новый ОписаниеТипов("Дата"));
	ТаблицаПриостановлений.Колонки.Добавить("Основание", Новый ОписаниеТипов("Строка"));
	ТаблицаПриостановлений.Колонки.Добавить("ОснованиеИмяПредопределенныхДанных", Новый ОписаниеТипов("Строка"));
	ТаблицаПриостановлений.Колонки.Добавить("ОснованиеДокументОснование", Новый ОписаниеТипов("ПеречислениеСсылка.НормативныеДокументыОснованийКадровыхПриказов"));
...
Функция ЗапросВТРабочиеМестаСотрудниковПоВременнойТаблицеБезКадровогоУчета(ТолькоРазрешенные, ИмяВТРабочиеМестаПоТрудовымДоговорам, Параметры, МенеджерВременныхТаблиц)
	...
	Запрос.Текст =
		...
		|			И (СотрудникиПериоды.ОкончаниеПериода = ДАТАВРЕМЯ(1, 1, 1)
		|				ИЛИ СотрудникиПериоды.ОкончаниеПериода >= ТекущиеКадровыеДанныеСотрудников.ДатаПриема)
		...
		|			И (СотрудникиПериоды.ОкончаниеПериода = ДАТАВРЕМЯ(1, 1, 1)
		|				ИЛИ СотрудникиПериоды.ОкончаниеПериода >= ТекущиеКадровыеДанныеСотрудников.ДатаУвольнения)
		|			И (ТекущиеКадровыеДанныеСотрудников.ТекущаяОрганизация <> ЗНАЧЕНИЕ(Справочник.Организации.ПустаяСсылка))
		|			И (ТекущиеКадровыеДанныеСотрудников.ОформленПоТрудовомуДоговору)
		|ГДЕ
		|	&УсловияОтбора";
...
Функция ЗапросВТСотрудникиОрганизацииПоРабочимМестамБезКадровогоУчета(ТолькоРазрешенные, ИмяВТСотрудникиОрганизации, ИмяВТСотрудникиПериоды, Параметры, МенеджерВременныхТаблиц)
	...
	ТекстыЗапросаПолейВТКадровыеДанныеСотрудников.Добавить(
		...
		|	КадровыеДанныеСотрудников.Организация КАК ОрганизацияФункциональныхОпций,
...
Процедура ДополнитьЗапросВТСотрудникиОрганизацииКадровымиДанными(Запрос, ТолькоРазрешенные, ИмяВТСотрудникиОрганизации, ИмяВТСотрудникиОрганизацииПромежуточная, КадровыеДанные, ИменаТаблицСКадровойИсториейСотрудников, МенеджерВременныхТаблиц, ИсключаемыйРегистратор = Неопределено)
	...
	ОписательВременнойТаблицыОтборов.ПутьКПолюОрганизацияФункциональныхОпций =
		"ТаблицаОтборов.ОрганизацияФункциональныхОпций";
	...
	ПоляОтборовПериодическихКадровыхДанных = Неопределено;
	Если ЗначениеЗаполнено(ИсключаемыйРегистратор) Тогда
		
		ПоляОтборовПериодическихКадровыхДанных = Новый Структура;
		
		Отборы = Новый Массив;
		ЗарплатаКадрыОбщиеНаборыДанных.ДобавитьВКоллекциюОтбор(
			Отборы, "Регистратор", "НЕ В", ИсключаемыйРегистратор);
		
		ПоляОтборовПериодическихКадровыхДанных.Вставить("КадроваяИсторияСотрудников", Отборы);
		ПоляОтборовПериодическихКадровыхДанных.Вставить("ВидыЗанятостиСотрудников", Отборы);
		ПоляОтборовПериодическихКадровыхДанных.Вставить("ПлановыеАвансы", Отборы);
		ПоляОтборовПериодическихКадровыхДанных.Вставить("ПлановыеНачисления", Отборы);
	...
	ЗапросВТКадровыеДанныеСотрудников = ЗапросВТКадровыеДанныеСотрудников(
		...
		ОписательВременнойТаблицыОтборов,
		МассивНеобходимыхКадровыхДанных,
		ПоляОтборовПериодическихКадровыхДанных,
		,
		,
		ИмяВТСотрудникиОрганизации,
		Истина);
