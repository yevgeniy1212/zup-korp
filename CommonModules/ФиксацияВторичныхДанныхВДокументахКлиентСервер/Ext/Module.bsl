...
Процедура ЗафиксироватьИзменениеРеквизита(Форма, ИмяРеквизита, ТекущаяСтрока = 0) Экспорт
	...
	Если НЕ ОписаниеФиксацииРеквизита = Неопределено Тогда
		...
		Иначе
			УстановитьФиксациюРеквизита(Форма, ОписаниеФиксацииРеквизита, ТекущаяСтрока, Истина);
		КонецЕсли;
	КонецЕсли;
КонецПроцедуры
...
//    ПараметрыФиксацииВторичныхДанных - Структура - См. ПараметрыФиксацииВторичныхДанных.
//    ИмяРеквизита - Строка - Имя элемента описания формы для механизма фиксации изменений.
//
Процедура ОтменитьФиксациюРеквизита(Форма, ИмяРеквизита, ТекущаяСтрока = 0) Экспорт
	ПараметрыФиксацииВторичныхДанных = Форма.ПараметрыФиксацииВторичныхДанных;
	ОписаниеФиксацииРеквизита = НайтиЭлементСоответствияПоКлючу(ПараметрыФиксацииВторичныхДанных.ОписаниеФиксацииРеквизитов, ИмяРеквизита);
	Если НЕ ОписаниеФиксацииРеквизита = Неопределено Тогда
		Если ОписаниеФиксацииРеквизита.Значение.ФиксацияГруппы Тогда
			УстановитьФиксациюИзмененийГруппыРеквизитов(Форма, ОписаниеФиксацииРеквизита.Значение.ИмяГруппы, ТекущаяСтрока, Ложь);
		Иначе
			УстановитьФиксациюРеквизита(Форма, ОписаниеФиксацииРеквизита, ТекущаяСтрока, Ложь);
		КонецЕсли;
	КонецЕсли;
КонецПроцедуры
...
//
Процедура УстановитьФиксациюИзмененийГруппыРеквизитов(Форма, ИмяГруппы, ТекущаяСтрока = 0, Значение = Истина) Экспорт
	...
	Для каждого ОписаниеФиксацииРеквизита Из ПараметрыФиксацииВторичныхДанных.ОписаниеФиксацииРеквизитов Цикл
		Если ОписаниеФиксацииРеквизита.Значение.ИмяГруппы = ИмяГруппы Тогда
			УстановитьФиксациюРеквизита(Форма, ОписаниеФиксацииРеквизита, ТекущаяСтрока, Значение);
		КонецЕсли;
	КонецЦикла;
КонецПроцедуры
...
Процедура СброситьФиксациюИзмененийРеквизитовПоОснованиюЗаполнения(Форма, ОснованиеЗаполнения, ТекущаяСтрока = 0) Экспорт
	Для Каждого ОписаниеФиксацииРеквизита Из Форма.ПараметрыФиксацииВторичныхДанных.ОписаниеФиксацииРеквизитов Цикл
		Если ОснованиеЗаполнения = "*" Или ОписаниеФиксацииРеквизита.Значение.ОснованиеЗаполнения = ОснованиеЗаполнения Тогда
...
// Очищает фиксацию реквизитов шапки документа.
Процедура СброситьФиксациюРеквизитовШапки(Форма) Экспорт
	ПараметрыФиксацииВторичныхДанных = Форма.ПараметрыФиксацииВторичныхДанных;
	Для Каждого ОписаниеФиксацииРеквизита Из ПараметрыФиксацииВторичныхДанных.ОписаниеФиксацииРеквизитов Цикл
		Если ОписаниеФиксацииРеквизита.Значение.РеквизитСтроки
			И ПолучитьПризнакФиксацииРеквизитаТЧ(Форма, ОписаниеФиксацииРеквизита, 0) Тогда
			УстановитьПризнакФиксацииРеквизитаТЧ(Форма, ОписаниеФиксацииРеквизита, Ложь, 0);
			ОбновитьОтображениеПредупреждения(Форма, ОписаниеФиксацииРеквизита, 0);
		КонецЕсли;
	КонецЦикла;
КонецПроцедуры
...
// Возвращает структуру - имя основания заполнения, массив имен заполняемых реквизитов.
//
// Параметры:
//    ФормаИлиПараметрыФиксации - ФормаКлиентскогоПриложения, Структура
//    ОснованияЗаполнения - Строка - Имена полей оснований заполнения перечисленные через запятую.
//
// Возвращаемое значение:
//   Структура
//       * Ключ     - Строка       - Имя поля, заполняемого на основании 2го параметра.
//       * Значение - Неопределено
//
Функция СтруктураПолейПоОснованиюЗаполнения(ФормаИлиПараметрыФиксации, ОснованияЗаполнения) Экспорт
	ИменаПолейПоОснованиямЗаполнения = ИменаПолейПоОснованиямЗаполнения(ФормаИлиПараметрыФиксации);
	ИменаПолей = "";
	ИскомыеОснования = Новый Структура(ОснованияЗаполнения);
	Для Каждого КлючИЗначение Из ИскомыеОснования Цикл
		Ключи = СтрСоединить(ИменаПолейПоОснованиямЗаполнения[КлючИЗначение.Ключ], ", ");
		Если ИменаПолей = "" Тогда
			ИменаПолей = Ключи;
		Иначе
			ИменаПолей = ИменаПолей + ", " + Ключи;
		КонецЕсли;
	КонецЦикла;
...
// Возвращаемое значение:
//   Структура
//       * Ключ     - Строка           - Основание заполнения.
//       * Значение - Массив из Строка - Имена заполняемых реквизитов.
//
Функция ИменаПолейПоОснованиямЗаполнения(ФормаИлиПараметрыФиксации) Экспорт
	Если ТипЗнч(ФормаИлиПараметрыФиксации) = Тип("ФормаКлиентскогоПриложения") Тогда
		ПараметрыФиксации = ФормаИлиПараметрыФиксации.ПараметрыФиксацииВторичныхДанных;
	Иначе
		ПараметрыФиксации = ФормаИлиПараметрыФиксации;
	КонецЕсли;
	Ключ      = "ИменаПолейПоОснованиямЗаполнения";
	Результат = Неопределено;
	Если ПараметрыФиксации.Свойство(Ключ, Результат) Тогда
		Возврат Результат;
	КонецЕсли;
	Результат = Новый Структура;
	ПараметрыФиксации.Вставить(Ключ, Результат);
	ИменаРеквизитов = Неопределено;
	Для Каждого КлючИЗначение Из ПараметрыФиксации.ОписаниеФиксацииРеквизитов Цикл
		ФиксируемыйРеквизит = КлючИЗначение.Значение;
		Если Не Результат.Свойство(ФиксируемыйРеквизит.ОснованиеЗаполнения, ИменаРеквизитов) Тогда
			ИменаРеквизитов = Новый Массив;
			Результат.Вставить(ФиксируемыйРеквизит.ОснованиеЗаполнения, ИменаРеквизитов);
		КонецЕсли;
		ИменаРеквизитов.Добавить(ФиксируемыйРеквизит.ИмяРеквизита);
	КонецЦикла;
	Возврат Результат;
КонецФункции
...
// 	Параметры:
//    	Форма - управляемая форма.
//    	КлючИЗначение - КлючИЗначение - см. описание СтруктураПараметровОписанияФиксацииРеквизитов().
//
Процедура ОбновитьОтображениеПредупреждения(Форма, КлючИЗначение, ИдентификаторСтроки = 0, ОтключитьПредупреждения = Ложь)
	ОписаниеРеквизита = КлючИЗначение.Значение;
	
	Если ОписаниеРеквизита.РеквизитСтроки Тогда
		Зафиксирован = ПолучитьПризнакФиксацииРеквизитаТЧ(Форма, КлючИЗначение, ИдентификаторСтроки);
	Иначе
		Зафиксирован = ПолучитьПризнакФиксацииРеквизита(Форма, КлючИЗначение);
	КонецЕсли;
	
	Если ОтключитьПредупреждения Тогда
		ОтображениеПредупреждения = ОтображениеПредупрежденияПриРедактировании.НеОтображать;
	ИначеЕсли Не ОписаниеРеквизита.ОтображатьПредупреждение Тогда
		ОтображениеПредупреждения = ОтображениеПредупрежденияПриРедактировании.НеОтображать;
	ИначеЕсли Зафиксирован Или НЕ ОписаниеРеквизита.Используется Тогда
		ОтображениеПредупреждения = ОтображениеПредупрежденияПриРедактировании.НеОтображать;
	ИначеЕсли ЗначениеЗаполнено(ОписаниеРеквизита.ОснованиеЗаполнения)
		И РеквизитСуществуетИНеЗаполнен(Форма, ОписаниеРеквизита.ОснованиеЗаполнения) Тогда
		ОтображениеПредупреждения = ОтображениеПредупрежденияПриРедактировании.НеОтображать;
	Иначе
		ОтображениеПредупреждения = ОтображениеПредупрежденияПриРедактировании.Отображать;
	КонецЕсли;
	
	МассивЭлементов = МассивЭлементовФормыПоОписанию(Форма, КлючИЗначение);
...
Функция РеквизитСуществуетИНеЗаполнен(Форма, ИмяРеквизита)
	// Зачастую, реквизиты документов-оснований не фиксируются,
	// поэтому для них нет описаний фиксации, из которого можно было бы извлечь путь к данным.
	ПолноеИмя = "Объект." + ИмяРеквизита;
	МассивИмен = СтрРазделить(ПолноеИмя, ".", Ложь);
	Объект = Форма;
	Для Каждого Ключ Из МассивИмен Цикл
		Структура = Новый Структура(Ключ, Null);
		ЗаполнитьЗначенияСвойств(Структура, Объект);
		Объект = Структура[Ключ];
		Если Объект = Null Тогда
			Возврат Ложь;
		КонецЕсли;
	КонецЦикла;
	Возврат Не ЗначениеЗаполнено(Объект);
КонецФункции

// Процедура устанавливает фиксацию реквизита.
//
Процедура УстановитьФиксациюРеквизита(Форма, ОписаниеФиксацииРеквизита, ИдентификаторСтроки = 0, Значение)
	Если НЕ ОписаниеФиксацииРеквизита.Значение.РеквизитСтроки И НЕ ПолучитьПризнакФиксацииРеквизита(Форма, ОписаниеФиксацииРеквизита) Тогда
		УстановитьПризнакФиксацииРеквизита(Форма, ОписаниеФиксацииРеквизита, Значение);
		ОбновитьОтображениеПредупреждения(Форма, ОписаниеФиксацииРеквизита);
	ИначеЕсли ОписаниеФиксацииРеквизита.Значение.РеквизитСтроки И НЕ ПолучитьПризнакФиксацииРеквизитаТЧ(Форма, ОписаниеФиксацииРеквизита, ИдентификаторСтроки) Тогда
		УстановитьПризнакФиксацииРеквизитаТЧ(Форма, ОписаниеФиксацииРеквизита, Значение, ИдентификаторСтроки);
