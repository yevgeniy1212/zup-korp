...
Функция ПолучитьПоКодамСведенияОПредставителе(Организация, КодНО, КПП = Неопределено, ОКТМО = Неопределено) Экспорт
	...
	Если ОКТМО <> Неопределено Тогда
		ТекстЗапроса = ТекстЗапроса + " И РегистрацииВНалоговомОргане.КодПоОКТМО = &ОКТМО";
...
Функция ФормаРеглОтчетаСоответствуетФормату(ИсточникОтчета, ДатаОкончания, ВерсияФормата, ИмяФормыОтчета) Экспорт
	...
	ИмяФормыОтчетаПоФормату = Неопределено;
	
	Попытка
		
		ДеревоФормИФорматов = РегламентированнаяОтчетность.РеглОтчеты(ИсточникОтчета).ДеревоФормИФорматов();
		
		СтрокиФорматов = ДеревоФормИФорматов.Строки.НайтиСтроки(Новый Структура("Код", ВерсияФормата), Истина);
		...
		Для Каждого СтрокаФормата Из СтрокиФорматов Цикл
			Если СтрокаФормата.Уровень() = 1 Тогда
				...
				Если ФормаРеглОтчетаДействуетВОтчетномПериоде(ИсточникОтчета, ДатаОкончания, ТекИмяФормыОтчета) Тогда
	...
	Если ИмяФормыОтчетаПоФормату = ИмяФормыОтчета Тогда
		Возврат Истина;
	КонецЕсли;
...
Функция ПолучитьЭталонФорматаОбласти(ИмяФормы, ИмяМакета, ИмяОбласти) Экспорт
	...
	ФорматЭталон = Неопределено;
	Если СтрНайти(ИмяОбласти, "_") > 0 Тогда
		ОбластьЭталон = Макет.Области.Найти(Лев(ИмяОбласти, СтрНайти(ИмяОбласти, "_")) + "1");
		Если ОбластьЭталон <> Неопределено Тогда
			ФорматЭталон = ОбластьЭталон.Формат;
		КонецЕсли;
	Иначе
		ФорматЭталон = Макет.Области[ИмяОбласти].Формат;
	КонецЕсли;
	Возврат ФорматЭталон;
КонецФункции
...
Функция ПолучитьПараметрыОтчетаИзРезультатаАнализаФайловВыгрузки(РезультатАнализаФайловВыгрузки, ПолноеИмяФормыОтчета) Экспорт
	...
	Если ПараметрыФормыОтчета.Организация = Неопределено ИЛИ ПараметрыФормыОтчета.Организация.Пустая() Тогда
		...
		Если НЕ ИспользуетсяОднаОрганизация() Тогда
			...
			Пока Выборка.Следующий() Цикл
				Если НЕ Выборка.ЭтоГруппа И ЭтоЮридическоеЛицо(Выборка.Ссылка) Тогда
...
Функция ПредставлениеКонтролирующегоОргана(ЗначенияЗаполнения, ДобавлятьБывш = Ложь) Экспорт
	...
	Вид = ЗначенияЗаполнения.ВидКонтролирующегоОргана;
	Код = ЗначенияЗаполнения.КодКонтролирующегоОргана;
	
	Представление = "";
	Если ЗначениеЗаполнено(Вид) Тогда
		КодКонтролирующегоОргана = ?(ЗначениеЗаполнено(Код), " " + Строка(Код), "");
		
		Если Вид = Перечисления.ТипыКонтролирующихОрганов.ФСРАР Тогда
			Представление = "ФСРАР" + КодКонтролирующегоОргана;
		ИначеЕсли Вид = Перечисления.ТипыКонтролирующихОрганов.РегистрацияЮЛФНС Тогда
			Представление = "Регистрация ЮЛ в ФНС" + КодКонтролирующегоОргана;
		Иначе
			Представление = Строка(Вид) + КодКонтролирующегоОргана;
		КонецЕсли;
	...
	СсылкаНаОбъект = ?(ТипЗнч(ЗначенияЗаполнения) = Тип("Структура"),
		?(ЗначенияЗаполнения.Свойство("Ссылка"), ЗначенияЗаполнения.Ссылка,
		?(ЗначенияЗаполнения.Свойство("ДокСсылка"), ЗначенияЗаполнения.ДокСсылка, Неопределено)),
		?(ТипЗнч(ЗначенияЗаполнения) = Тип("РегистрСведенийЗапись.ЖурналОтчетовСтатусы"),
		ЗначенияЗаполнения.Ссылка, Неопределено));
	ДатаОтправки = ?(ТипЗнч(ЗначенияЗаполнения) = Тип("Структура"),
		?(ЗначенияЗаполнения.Свойство("ДатаОтправки"), ЗначенияЗаполнения.ДатаОтправки,
		?(ЗначенияЗаполнения.Свойство("ДатаСоздания"), ЗначенияЗаполнения.ДатаСоздания, Неопределено)),
		?(ТипЗнч(ЗначенияЗаполнения) = Тип("РегистрСведенийЗапись.ЖурналОтчетовСтатусы"),
		ЗначенияЗаполнения.ДатаОтправки, Неопределено));
	ЭтоОтправкаВСФР = ДокументооборотСКОКлиентСервер.ЭтоОтправкаВСФР(
		СсылкаНаОбъект,,
		ДатаОтправки,
		Вид);
	Представление = ДокументооборотСКОКлиентСервер.ЗаменитьПФРиФССнаСФР(
		Представление, ДобавлятьБывш,, НЕ ЭтоОтправкаВСФР);
...
Функция КоличествоОбъектовНеЗаписанныхВРегистрЖурналОтчетовСтатусы() Экспорт
	...
	Пока ВыборкаОбъектовНеЗаписанныхВРегистр.Следующий() Цикл
		Ссылка = ВыборкаОбъектовНеЗаписанныхВРегистр.Объект;
		Если ТипЗнч(Ссылка) = Тип("ДокументСсылка.РегламентированныйОтчет") Тогда
			ПараметрыОтчета = РегламентированнаяОтчетность.СвойстваРегламентированногоОтчета(Ссылка);
			Если ПараметрыОтчета.Свойство("ОтражаетсяВЖурналеОтчетов") И НЕ ПараметрыОтчета.ОтражаетсяВЖурналеОтчетов Тогда
				Продолжить;
			КонецЕсли;
		КонецЕсли;
...
Функция НеотправляемыйРеглОтчетИмеетВозможностьИзмененияСтатусаОтправкиИзФормыОтчетаВручную(ИмяОбъекта)
	...
	МассивИмен.Добавить("РегламентированныйОтчетСоответствиеУслТрудаМикропредприятия");
...
Процедура УведомлениеОСпецрежимахНалогообложения_ОбработкаПолученияФормы(ВидФормы, Параметры, ВыбраннаяФорма, ДополнительнаяИнформация, СтандартнаяОбработка) Экспорт 
	...
	Если Параметры.Свойство("Ключ") И ЗначениеЗаполнено(Параметры.Ключ) Тогда
		...
		УведомлениеОСпецрежимахНалогообложения.ЗаменитьИмяОтчетаПередОткрытием(ВидСообщения, ИмяФормы, ИмяОтчета);
	Иначе
		...
		Если ЗначениеЗаполнено(ИмяОтчета) Тогда 
			...
			Если ТФ.Количество() > 1 Тогда 
				...
				Если ОтбСтроки.Количество() = 1 Тогда
					Если ЗначениеЗаполнено(ОтбСтроки[0].ПутьФормы) Тогда 
						ВыбраннаяФорма = ОтбСтроки[0].ПутьФормы;
					Иначе
						ВыбраннаяФорма = "Отчет." + ИмяОтчета + ".Форма." + ОтбСтроки[0].ИмяФормы;
					КонецЕсли;
				Иначе 
				...
				КонецЕсли;
			ИначеЕсли ТФ.Количество() = 1 Тогда 
				...
				Если Не ЗначениеЗаполнено(ВыбраннаяФорма) Тогда 
					ВыбраннаяФорма = Отчеты[ИмяОтчета].ПолучитьФормуПоУмолчанию();
				КонецЕсли;
				Если Не ЗначениеЗаполнено(ВыбраннаяФорма) Тогда 
...
Функция ДвоичныеДанныеФайлаВТекст(ДвоичныеДанные)
...
Процедура ДополнитьСвойстваОсновногоФайлаТекстомВыгрузки(СвойстваОсновногоФайла, ТекстФайлаВыгрузки, ВерсияДо, ВерсияПосле)
	ТекстФайлаВыгрузки = СтрЗаменить(ТекстФайлаВыгрузки, ВерсияДо, ВерсияПосле);
	СвойстваОсновногоФайла.Вставить("АдресТекста", ПоместитьВоВременноеХранилище(Новый ХранилищеЗначения(ТекстФайлаВыгрузки), Новый УникальныйИдентификатор));
КонецПроцедуры

Процедура ДополнитьСерверныеПараметры(ДанныеДляЗагрузки, СтруктураПараметров) Экспорт
	ИмяФормы = СтруктураПараметров.ИмяФормы;
	ТекстВыгрузки = СтруктураПараметров.ТекстВыгрузки;
	СтруктураРеквизитовФормы = СтруктураПараметров.СтруктураРеквизитовФормы;
	
	ВерсияДо = "ВерсФорм=""" + СтруктураРеквизитовФормы.ФорматКопируемойФормы + """";
	ВерсияПосле = "ВерсФорм=""" + СтруктураРеквизитовФормы.ТекущийФормат + """";

	ДанныеДляЗагрузки.Вставить("ВидОтчета", Справочники.ВидыОтправляемыхДокументов.НайтиПоРеквизиту("Источник", СтрРазделить(ИмяФормы, ".")[1]));
	ДанныеДляЗагрузки.Вставить("ТипДокумента", ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ДанныеДляЗагрузки.ВидОтчета, "ТипДокумента"));
	ДанныеДляЗагрузки.Вставить("Версия", 0);
	
	Если ТипЗнч(ТекстВыгрузки) = Тип("Строка") Тогда 
		ДополнитьСвойстваОсновногоФайлаТекстомВыгрузки(ДанныеДляЗагрузки.СвойстваОсновногоФайла,
			ТекстВыгрузки, ВерсияДо, ВерсияПосле);
		ДанныеДляЗагрузки.СвойстваОсновногоФайла.Вставить("РазмерТекста", СтрДлина(ТекстВыгрузки));
		...
		ДанныеДляЗагрузки.СвойстваОсновногоФайла.Вставить("ИмяФайла", ?(ТипЗнч(УзелИдФайл) = Тип("СтрокаДереваЗначений"), УзелИдФайл.ЗначениеЭлемента, ""));
	ИначеЕсли ТипЗнч(ТекстВыгрузки) = Тип("Массив") Тогда 
		Для Каждого Элт Из ТекстВыгрузки Цикл 
			Если ТипЗнч(Элт) = Тип("Структура")
				И Элт.Свойство("АдресФайлаВыгрузки") И Элт.Свойство("ИмяФайлаВыгрузки")
				И ЭтоАдресВременногоХранилища(Элт.АдресФайлаВыгрузки)
				И СтрЗаканчиваетсяНа(НРег(Элт.ИмяФайлаВыгрузки), ".xml")
				И ЭтоАдресВременногоХранилища(Элт.АдресФайлаВыгрузки) Тогда 
				
				ДанныеФайлаВыгрузки = ПолучитьИзВременногоХранилища(Элт.АдресФайлаВыгрузки);
				Если ТипЗнч(ДанныеФайлаВыгрузки) = Тип("ДвоичныеДанные") Тогда 
					ТекстФайлаВыгрузки = ДвоичныеДанныеФайлаВТекст(ДанныеФайлаВыгрузки);
					ДанныеДляЗагрузки.СвойстваОсновногоФайла.Вставить("АдресДанных", Элт.АдресФайлаВыгрузки);
					ДополнитьСвойстваОсновногоФайлаТекстомВыгрузки(ДанныеДляЗагрузки.СвойстваОсновногоФайла, ТекстФайлаВыгрузки, ВерсияДо, ВерсияПосле);
					ДанныеДляЗагрузки.СвойстваОсновногоФайла.Вставить("РазмерТекста", СтрДлина(ТекстФайлаВыгрузки));
				ИначеЕсли ТипЗнч(ДанныеФайлаВыгрузки) = Тип("Строка") Тогда 
					ДополнитьСвойстваОсновногоФайлаТекстомВыгрузки(ДанныеДляЗагрузки.СвойстваОсновногоФайла, ДанныеФайлаВыгрузки, ВерсияДо, ВерсияПосле);
					ДанныеДляЗагрузки.СвойстваОсновногоФайла.Вставить("РазмерТекста", СтрДлина(ДанныеФайлаВыгрузки));
				КонецЕсли;
				ДанныеДляЗагрузки.СвойстваОсновногоФайла.Вставить("ИмяФайла", Элт.ИмяФайлаВыгрузки);
				Прервать;
			КонецЕсли;
		КонецЦикла;
	КонецЕсли;
