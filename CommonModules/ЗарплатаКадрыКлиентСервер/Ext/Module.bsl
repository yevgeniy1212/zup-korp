...
Функция ГражданствоПоУмолчанию(ИнформацияОГражданстве) Экспорт
    ...
    // ЗУПК
	Возврат ИнформацияОГражданстве.Страна = ПредопределенноеЗначение("Справочник.СтраныМира.Казахстан");
    // ЗУПК
...
Функция СведенияОбИнвалидностиПоУмолчанию(СведенияОбИнвалидности) Экспорт
	Возврат (НЕ СведенияОбИнвалидности.Инвалидность)
        ...
        // ЗУПК
		И (НЕ ЗначениеЗаполнено(СведенияОбИнвалидности.ОтношениеКИнвалидности))
		И (НЕ ЗначениеЗаполнено(СведенияОбИнвалидности.ГруппаИнвалидности))
        // ЗУПК
...
Функция Округлить(Число, Точность = 0, Правило = Неопределено) Экспорт
	...
	Иначе
		...
		Если Правило = ПредопределенноеЗначение("Перечисление.ПравилаОкругленияПриРасчетеЗарплаты.ВБольшуюСторону") Тогда
			// при передаче сторно ЗарплатаКадрыКлиентСервер.Округлить(-1224,28, 0, ВБольшуюСторону) возвращает не корректно -1223 (разница 1.28; правильно -1225)
			Результат = Точность * (ЦелыйМножитель + ?(Множитель > 0, 1, -1));
