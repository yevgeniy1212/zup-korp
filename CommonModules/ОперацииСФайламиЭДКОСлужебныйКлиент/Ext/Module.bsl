...
Процедура КаталогВременныхФайловНаКлиенте(ОповещениеОЗавершении, ВыводитьСообщения, ДвоичныеДанные) Экспорт
	...
	Если ЭтоВеб ИЛИ ЭтоLinuxИлиMacOS Тогда
		ПредварительноСоздатьДвоичныеДанные(
			ОповещениеОЗавершении, "КаталогВременныхФайловНаКлиентеПослеСозданияДвоичныхДанных",
			,
			ДвоичныеДанные,
			ВыводитьСообщения);
	Иначе
...
Процедура СоздатьКаталогНаКлиентеПриОшибкеВызоваСоздатьКаталог(ИнформацияОбОшибке, СтандартнаяОбработка, ВходящийКонтекст) Экспорт
	...
	СтандартнаяОбработка = Ложь;
	ЭтоLinuxИлиMacOS = ОбщегоНазначенияКлиент.ЭтоLinuxКлиент() ИЛИ ОбщегоНазначенияКлиент.ЭтоMacOSКлиент();
	Если ЭтоLinuxИлиMacOS Тогда
		// обход - игнорирование ошибки
		СоздатьКаталогНаКлиентеПослеСозданияКаталога(ВходящийКонтекст.ИмяКаталога, ВходящийКонтекст);
		Возврат;
	КонецЕсли;
...
Процедура УдалитьФайлыНаКлиенте(ОповещениеОЗавершении, Путь, ВыводитьСообщения) Экспорт
	...
	Если ЗначениеЗаполнено(Путь) Тогда		
		...
		Если ЭтоВеб ИЛИ ЭтоLinuxИлиMacOS Тогда
			// FIX: Пока ничего не делаем, иначе окна с вопросом об удалении мешают
			УдалитьФайлыНаКлиентеПослеУдаления(Контекст); //При необходимости оповестим вызывающий код о завершении
		Иначе
			Оповещение = Новый ОписаниеОповещения(
...
Процедура ФайлВBase64(ОповещениеОЗавершении, ИмяФайла, ВыводитьСообщения, ДвоичныеДанные, ПроверятьСуществование) Экспорт
	...
	ВходящиеПараметры = ПараметрыВМассив(2, ИмяФайла, ПроверятьСуществование);
...
Процедура ФайлВBase64ПослеСозданияДвоичныхДанных(Результат, ВходящийКонтекст) Экспорт
	...
	Если Результат.Выполнено Тогда
		...
		ПроверятьСуществование = ВходящийКонтекст.ВходящиеПараметры[1];
		...
		Иначе
			...
			ДополнительныеПараметры.Вставить("СвойстваФайла", Новый Структура("Существует", Истина));
			ДополнительныеПараметры.Вставить("Выполнено", 	  Истина);
			
			ФайлВBase64ПослеПолученияСвойствФайла(ДополнительныеПараметры, ВходящийКонтекст);
...
Процедура ТекстВФайл(ОповещениеОЗавершении, Текст, ИмяФайлаИлиРасширение, ВыводитьСообщения, ДвоичныеДанные) Экспорт
	...
	Если ОбщегоНазначенияКлиент.ЭтоMacOSКлиент() Тогда
		ДанныеТекста = ПолучитьДвоичныеДанныеИзСтроки(Текст, "windows-1251");
		СтрокаBase64 = Base64Строка(ДанныеТекста);
...
Процедура ОткрытьФайл(ПолноеИмяФайлаИлиАдрес, ИмяФайла, ВыводитьСообщения) Экспорт
	...
	ПоказатьВопросОбУстановкеРасширенияРаботыСФайлами(
		"ОткрытьФайлПослеПодключенияРасширенияРаботыСФайлами", Контекст, ВозможноПродолжениеБезУстановки);
...
Процедура ОткрытьФайлПослеПолученияКаталогаВременныхФайлов(ИмяКаталогаВременныхФайлов, ВходящийКонтекст) Экспорт
	...
	Если ЗначениеЗаполнено(ВходящийКонтекст.ИмяФайла) Тогда
		
		ИмяПревышает260 = СтрДлина(ИмяКаталогаВременныхФайлов + ВходящийКонтекст.ИмяФайла) > 260;
		Если ИмяПревышает260 Тогда
			РасширениеБезТочки = ОбщегоНазначенияКлиентСервер.ПолучитьРасширениеИмениФайла(ВходящийКонтекст.ИмяФайла);
			ОписаниеПередаваемогоФайла.Имя = GUID + "." + РасширениеБезТочки;
		Иначе
			ОписаниеПередаваемогоФайла.Имя = ВходящийКонтекст.ИмяФайла;
		КонецЕсли;
	Иначе
...
Процедура ОткрытьФайлПослеПолученияФайлов(ПолученныеФайлы, ВходящийКонтекст) Экспорт
	...
	Если ЗначениеЗаполнено(ПолученныеФайлы) Тогда
		ОткрытьФайлЗапуститьПриложение(ПолученныеФайлы[0].ПолноеИмя, ВходящийКонтекст);
	Иначе
...
Процедура ЗапуститьПриложениеНаКлиенте(
		ОповещениеОЗавершении,
		СтрокаКоманды,
		ТекущийКаталог,
		ДождатьсяЗавершения,
		ВыводитьСообщения,
		СсылкаДляПереходаПриОшибке = "") Экспорт
	...
	Контекст.Вставить("ВыводитьСообщения", 			ВыводитьСообщения);
	Контекст.Вставить("СсылкаДляПереходаПриОшибке", СсылкаДляПереходаПриОшибке);
...
Процедура ЗапуститьПриложениеНаКлиентеПослеПодключенияРасширенияРаботыСФайлами(Подключено, ВходящийКонтекст) Экспорт
	...
	Если Подключено Тогда
		...
		НачатьЗапускПриложения(
			...
			ВходящийКонтекст.ТекущийКаталог,
			ВходящийКонтекст.ДождатьсяЗавершения);
	Иначе
...
Процедура ЗапуститьПриложениеНаКлиентеПриОшибкеЗапускаПриложения(ИнформацияОбОшибке, СтандартнаяОбработка, ВходящийКонтекст) Экспорт
	...
	Если ЗначениеЗаполнено(ВходящийКонтекст.СсылкаДляПереходаПриОшибке) Тогда
		ПерейтиПоНавигационнойСсылке(ВходящийКонтекст.СсылкаДляПереходаПриОшибке);
	Иначе
		ДлительнаяОтправкаКлиентСервер.ВывестиОшибку(ОписаниеОшибки);
	КонецЕсли;
...
Процедура СохранитьФайлы(Знач СохраняемыеФайлы, КаталогСохранения, ВыводитьСообщения) Экспорт
	...
	Если ТипЗнч(СохраняемыеФайлы) = Тип("Массив") И СохраняемыеФайлы.Количество() = 0 Тогда
		...
		ДлительнаяОтправкаКлиентСервер.ВывестиОшибку(НСтр("ru = 'Отсутствуют файлы для сохранения'"));
		Возврат;
	КонецЕсли;
...
Процедура СохранитьФайлыПослеПодключенияРасширенияРаботыСФайлами(Подключено, ВходящийКонтекст) Экспорт
	...
	Если Подключено Тогда
		...
		Для каждого СохраняемыйФайл Из СохраняемыеФайлы Цикл
			...
			Если (НЕ ЗначениеЗаполнено(СвойстваФайла.Путь) ИЛИ СвойстваФайла.Путь = "\" ИЛИ СвойстваФайла.Путь = "/")
				И НЕ ЗначениеЗаполнено(КаталогСохранения) Тогда
...
Процедура СохранитьФайлыПослеВыбораКаталога(ВыбранныеФайлы, ВходящийКонтекст) Экспорт
	...
	Если ВыбранныеФайлы <> Неопределено ИЛИ НЕ ТребуетсяПоказатьДиалогВыбораКаталога Тогда
		...
		Для каждого СохраняемыйФайл Из СохраняемыеФайлы Цикл
			...
			Если ЗначениеЗаполнено(СвойстваФайла.Путь) И СвойстваФайла.Путь <> "\" И СвойстваФайла.Путь <> "/" Тогда
...
Процедура СохранитьФайлыПроверитьДлинуПутиИСуществование(Результат, ВходящийКонтекст) Экспорт
	...
	Если НомерТекущегоФайла <= ЧислоФайлов - 1 Тогда
		...
		Если НЕ ОбщегоНазначенияКлиент.ЭтоLinuxКлиент() И НЕ ОбщегоНазначенияКлиент.ЭтоMacOSКлиент() Тогда
			Если СтрДлина(Имя) > 260 Тогда
				ПоказатьПредупреждение(, НСтр("ru = 'Не удается сохранить файлы, потому что длина пути превышает 260 символов.
                                               |Сохраните файлы в другую папку, например в корень диска С.'"));
				Возврат;
			КонецЕсли;
		КонецЕсли;
...
Процедура СохранитьФайлыПослеПолученияФайлов(ПолученныеФайлы, ВходящийКонтекст) Экспорт
	...
	Если ВходящийКонтекст.ВыводитьСообщения Тогда
		Если ПолученныеФайлы = Неопределено Тогда
			ПоказатьПредупреждение(, НСтр("ru = 'Не удалось выгрузить файлы'"));
			Возврат;
		КонецЕсли;
...
Процедура ДобавитьФайлы(ОповещениеОЗавершении, ИдентификаторФормы, Заголовок, Параметры, ВозможноПродолжениеБезУстановкиРасширения) Экспорт
	...
	Контекст.Вставить("ОповещениеОЗавершении", ОповещениеОЗавершении);
	Контекст.Вставить("ВозможноПродолжениеБезУстановкиРасширения", ВозможноПродолжениеБезУстановкиРасширения);

	ПоказатьВопросОбУстановкеРасширенияРаботыСФайлами(
		"ДобавитьФайлыПослеПодключенияРасширенияРаботыСФайлами", Контекст, ВозможноПродолжениеБезУстановкиРасширения);
...
Процедура ДобавитьФайлыПослеПодключенияРасширенияРаботыСФайлами(Подключено, ВходящийКонтекст) Экспорт
	...
	Если Подключено Тогда
		...
		Если ЗначениеЗаполнено(ВходящийКонтекст.Параметры) И ВходящийКонтекст.Параметры.Свойство("МножественныйВыбор") Тогда
			ДиалогВыбораФайлов.МножественныйВыбор = ВходящийКонтекст.Параметры.МножественныйВыбор;
		Иначе
		...
		Если ЗначениеЗаполнено(ВходящийКонтекст.Параметры) И ВходящийКонтекст.Параметры.Свойство("Фильтр") Тогда
			ДиалогВыбораФайлов.Фильтр = ВходящийКонтекст.Параметры.Фильтр;
		КонецЕсли;
	...
	ИначеЕсли ВходящийКонтекст.ВозможноПродолжениеБезУстановкиРасширения Тогда
	...
	Иначе
		...
		РезультатВыполнения.Вставить("ОписанияФайлов", Новый Массив);
		РезультатВыполнения.Вставить("ОписаниеОшибки", НСтр("ru = 'Не установлено расширение для работы с файлами'"));
...
Процедура ДобавитьФайлыПослеПомещенияФайла(Результат, Адрес, ВыбранноеИмяФайла, ВходящийКонтекст) Экспорт
	...
	Если Результат Тогда
		ОписанияФайлов.Добавить(ПолучитьОписаниеФайла(ВыбранноеИмяФайла, Адрес));
	КонецЕсли;
...
Процедура ДобавитьФайлыПослеПомещенияФайлов(ПомещенныеФайлы, ВходящийКонтекст) Экспорт
	...
	Если ЗначениеЗаполнено(ПомещенныеФайлы) Тогда
		Для Каждого ПомещенныйФайл Из ПомещенныеФайлы Цикл
			ОписанияФайлов.Добавить(ПолучитьОписаниеФайла(ПомещенныйФайл.ПолноеИмя, ПомещенныйФайл.Хранение));
		КонецЦикла;	
	КонецЕсли;
...
Процедура ДобавитьФайлыПослеВыполнения(ОписанияФайлов, ВходящийКонтекст)
	...
	Если ВходящийКонтекст <> Неопределено  
		И ВходящийКонтекст.Свойство("Параметры") 
		И ВходящийКонтекст.Параметры <> Неопределено 
		И ВходящийКонтекст.Параметры.Свойство("Требования") Тогда
		
		ОбработатьКартинки(
			ОписаниеОповещения, 
			ОписанияФайлов, 
			ВходящийКонтекст.Параметры.Требования,
			ВходящийКонтекст.ИдентификаторФормы);
...
Процедура ДобавитьФайлыПослеВыполнения_ПослеОбработки(Результат, ВходящийКонтекст) Экспорт
	...
	Если Выполнено Тогда
		// Внутри процедуры к описанию файлов добавляются дополнительные свойства. Не переносить в др. место.
		ТекстОшибки = ТекстОшибкиДобавленияФайлов(ОписанияФайлов, ВходящийКонтекст);
		...
		Если ПустаяСтрока(ТекстОшибки) Тогда
			...
			ДобавитьФайлыПослеВыполнения_ВернутьРезультат(Результат, ВходящийКонтекст);
		...
		Иначе
			...
			ДополнительныеПараметры = Новый Структура();
			ДополнительныеПараметры.Вставить("ОповещениеОЗавершении", ВходящийКонтекст.ОповещениеОЗавершении);
...
Функция ТекстОшибкиРазмера(ОписанияФайлов, Знач МаксимальныйРазмерФайла)
	
	ТекстОшибкиРазмера = "";
	
	Для Каждого ОписаниеФайла Из ОписанияФайлов Цикл
		Если ОписаниеФайла.Размер > МаксимальныйРазмерФайла Тогда
			
			ШаблонСтроки       = НСтр("ru = '- %1 (%2)
                                 |'");
			РазмерСтрокой      = ОбщегоНазначенияЭДКОКлиентСервер.ТекстовоеПредставлениеРазмераФайла(ОписаниеФайла.Размер);
			ТекстОшибкиРазмера = ТекстОшибкиРазмера + СтрШаблон(ШаблонСтроки, ОписаниеФайла.Имя, РазмерСтрокой);
			
		КонецЕсли;
	КонецЦикла;
	
	ТекстОшибкиРазмера = СокрЛП(ТекстОшибкиРазмера);
	Если НЕ ПустаяСтрока(ТекстОшибкиРазмера) Тогда
		
		ШаблонСтроки       = НСтр("ru = 'Размер следующих файлов превышает максимально допустимый размер - %1:
                                             |%2'");
		МаксРазмерСтрокой  = ОбщегоНазначенияЭДКОКлиентСервер.ТекстовоеПредставлениеРазмераФайла(МаксимальныйРазмерФайла);
		ТекстОшибкиРазмера = СтрШаблон(ШаблонСтроки, МаксРазмерСтрокой, ТекстОшибкиРазмера);
			
	КонецЕсли;
	
	Возврат ТекстОшибкиРазмера;
	
КонецФункции

Функция ТекстОшибкиТипа(ОписанияФайлов, Знач ДопустимыеТипыФайлов)
	
	ТекстОшибкиТипа = "";
	
	ДопустимыеТипыФайлов = ВРЕГ(ДопустимыеТипыФайлов);
	ДопустимыеТипыФайлов = СтроковыеФункцииКлиентСервер.РазложитьСтрокуВМассивСлов(ДопустимыеТипыФайлов);
	
	Для Каждого ОписаниеФайла Из ОписанияФайлов Цикл
		Если ДопустимыеТипыФайлов.Найти(ВРЕГ(ОписаниеФайла.Расширение)) = Неопределено Тогда
			
			ШаблонСтроки       = НСтр("ru = '- %1 (%2)
                              |'");
			ТекстОшибкиТипа =  ТекстОшибкиТипа + СтрШаблон(ШаблонСтроки, ОписаниеФайла.Имя, ОписаниеФайла.Расширение);
					
		КонецЕсли;
	КонецЦикла;
	
	ТекстОшибкиТипа = СокрЛП(ТекстОшибкиТипа);
	Если НЕ ПустаяСтрока(ТекстОшибкиТипа) Тогда
		
		ШаблонСтроки       = НСтр("ru = 'Типы следующих файлов не соответствуют требованиям (допустимые типы - ""%1""):
                                             |%2'");
		ТекстОшибкиТипа = СтрШаблон(ШаблонСтроки, СтрСоединить(ДопустимыеТипыФайлов, ", "), ТекстОшибкиТипа);
	КонецЕсли;
	
	Возврат ТекстОшибкиТипа; 
	
КонецФункции

Процедура ДобавитьФайлыПослеВыводаОшибки(ВходящийКонтекст) Экспорт

	ВыполнитьОбработкуОповещения(ВходящийКонтекст.ОповещениеОЗавершении, ВходящийКонтекст.РезультатВыполнения);
...
Функция ТекстОшибкиДобавленияНесколькихФайлов(ОписанияФайлов, ВходящийКонтекст) Экспорт
	...
	Если ВходящийКонтекст.Параметры.Свойство("МаксимальныйРазмерФайла")
		...
		ТекстОшибкиРазмера = ТекстОшибкиРазмера(ОписанияФайлов, ВходящийКонтекст.Параметры.МаксимальныйРазмерФайла);
	...
	Если ВходящийКонтекст.Параметры.Свойство("ДопустимыеТипыФайлов")
		...
		ТекстОшибкиТипа = ТекстОшибкиТипа(ОписанияФайлов, ВходящийКонтекст.Параметры.ДопустимыеТипыФайлов);
		
	КонецЕсли;
	
	ТекстОшибки = СокрЛП(ТекстОшибкиРазмера + Символы.ПС + Символы.ПС + ТекстОшибкиТипа);
...
Функция ТекстОшибкиДобавленияОдногоФайла(ФайлыДокумента, Знач МаксимальныйРазмерФайла, Знач ДопустимыеТипыФайлов) Экспорт
	...
	Если ЗначениеЗаполнено(МаксимальныйРазмерФайла) Тогда
		Для Каждого ОписаниеФайла Из ФайлыДокумента Цикл
			Если ОписаниеФайла.Размер > МаксимальныйРазмерФайла И МаксимальныйРазмерФайла <> 0 Тогда
				
				ШаблонСтроки       = НСтр("ru = 'Размер файла ""%1"" (%2) 
	                                       |превышает максимально допустимый размер - %3'");
				РазмерСтрокой      = ОбщегоНазначенияЭДКОКлиентСервер.ТекстовоеПредставлениеРазмераФайла(ОписаниеФайла.Размер);
				МаксРазмерСтрокой  = ОбщегоНазначенияЭДКОКлиентСервер.ТекстовоеПредставлениеРазмераФайла(МаксимальныйРазмерФайла);
				ТекстОшибки        = СтрШаблон(ШаблонСтроки, ОписаниеФайла.Имя, РазмерСтрокой, МаксРазмерСтрокой);
	...
	Если ЗначениеЗаполнено(ДопустимыеТипыФайлов) Тогда
		ТипыФайлов = СтроковыеФункцииКлиентСервер.РазложитьСтрокуВМассивСлов(НРег(ДопустимыеТипыФайлов));
		
		Для Каждого ОписаниеФайла Из ФайлыДокумента Цикл
			Если ТипыФайлов.Найти(НРег(ОписаниеФайла.Расширение)) = Неопределено Тогда
				
				ШаблонСтроки = НСтр("ru = 'Тип файла ""%1"" (%2) не соответствует требованиям. 
	                                 |Допустимые типы файлов - ""%3"".'");
				ТекстОшибки  = СтрШаблон(ШаблонСтроки, ОписаниеФайла.Имя, ОписаниеФайла.Расширение, СтрСоединить(ТипыФайлов, ", "));
...
Функция ТекстОшибкиДобавленияФайлов(ОписанияФайлов, ВходящийКонтекст) Экспорт
	...
	Если ЗначениеЗаполнено(ВходящийКонтекст.Параметры) Тогда
		...
		Если ОписанияФайлов.Количество() = 1 Тогда
			...
			Если ВходящийКонтекст.Параметры.Свойство("МаксимальныйРазмерФайла") Тогда
				МаксимальныйРазмерФайла = ВходящийКонтекст.Параметры.МаксимальныйРазмерФайла;
			КонецЕсли;
			
			Если ВходящийКонтекст.Параметры.Свойство("ДопустимыеТипыФайлов") Тогда
				ДопустимыеТипыФайлов = ВходящийКонтекст.Параметры.ДопустимыеТипыФайлов;
			КонецЕсли;
			
			ТекстОшибки = ТекстОшибкиДобавленияОдногоФайла(ОписанияФайлов, МаксимальныйРазмерФайла, ДопустимыеТипыФайлов);
		Иначе
			ТекстОшибки = ТекстОшибкиДобавленияНесколькихФайлов(ОписанияФайлов, ВходящийКонтекст);
		КонецЕсли;
...
Процедура ДобавитьФайлыСПредпросмотром(ОповещениеОЗавершении, ИдентификаторФормы, Заголовок, Параметры) Экспорт
	...
	Если ЗначениеЗаполнено(Параметры.Файлы) Тогда
		...
		ДобавитьФайлыСПредпросмотромОткрытьФормуПредпросмотра(Оповещение, Контекст);
	...
	Иначе
		...
		Параметры.Вставить("ДопустимыеТипыФайлов",    Параметры.Требования.ДопустимыеТипыФайлов);
		Параметры.Вставить("МаксимальныйРазмерФайла", Параметры.Требования.МаксимальныйРазмерФайла);
		Параметры.Вставить("Требования",              Параметры.Требования);

		ОперацииСФайламиЭДКОКлиент.ДобавитьФайлы(Оповещение, ИдентификаторФормы, Заголовок, Параметры);
...
Процедура ДобавитьФайлыСПредпросмотромПослеВыбораФайлов(Результат, ВходящийКонтекст) Экспорт
	...
	Если Результат.Выполнено И ЗначениеЗаполнено(Результат.ОписанияФайлов) Тогда
		...
		Для Каждого ОписаниеФайла Из Результат.ОписанияФайлов Цикл
			Файлы.Добавить(Новый Структура("Адрес,ИсходноеИмя", ОписаниеФайла.Адрес, ОписаниеФайла.Имя));
		КонецЦикла;		
		ВходящийКонтекст.Параметры.Вставить("Файлы", Файлы);
		
		Оповещение = Новый ОписаниеОповещения("ДобавитьФайлыСПредпросмотромПослеПоказаФайлов", ЭтотОбъект, ВходящийКонтекст);
		ДобавитьФайлыСПредпросмотромОткрытьФормуПредпросмотра(Оповещение, ВходящийКонтекст);
	Иначе
		ВыполнитьОбработкуОповещения(ВходящийКонтекст.ОповещениеОЗавершении, Новый Структура("Выполнено", Ложь));		
	КонецЕсли;
	
КонецПроцедуры

Процедура ДобавитьФайлыСПредпросмотромОткрытьФормуПредпросмотра(ОповещениеОЗавершении, ВходящийКонтекст)
	
	ОткрытьФорму(
		"Обработка.ДокументооборотСКонтролирующимиОрганами.Форма.ПросмотрДокументов",
		ВходящийКонтекст.Параметры, 
		ЭтотОбъект,,,, 
		ОповещениеОЗавершении, 
...
Процедура ДобавитьФайлыСПредпросмотромПослеПоказаФайлов(Результат, ВходящийКонтекст) Экспорт
	...
	Если ТипЗнч(Результат) = Тип("Массив") Тогда
		РезультатВыполнения = Новый Структура("Выполнено,Файлы", Истина, Результат);
	Иначе
...
Процедура ОбработатьКартинки(ОповещениеОЗавершении, ОписанияФайлов, Требования, ИдентификаторВладельца) Экспорт
	...
	ДополнительныеПараметры.Вставить("Параметры",              Требования);
...
Процедура ОбработатьКартинки_ПослеПолученияКонтекста(Результат, ВходящийКонтекст) Экспорт
	...
	ВходящийКонтекст.Удалить("ОповещениеОЗавершении");
	
	Если ВходящийКонтекст.ОписанияФайлов.Количество() = 0 Тогда
		ВыполнитьОбработкуОповещения(ОписаниеОповещения, Неопределено);
	Иначе
		
		// Обработка выполняется в методе ОперацииСФайламиЭДКО.ОбработатьКартинки
		ОткрытьФорму(КонтекстЭДОКлиент.ПутьКОбъекту + ".Форма.ОбработкаКартинок",
			ВходящийКонтекст,
			,
			,
			,
...
Процедура ОбработатьКартинки_Завершение(Результат, ОповещениеОЗавершении) Экспорт
	...
	Если Результат = Неопределено Тогда
		...
		ДополнительныеПараметры.Вставить("Отменено", 		Истина);
		ДополнительныеПараметры.Вставить("ОписанияФайлов", 	Новый Массив);
	Иначе
		ДополнительныеПараметры = Новый Структура();
		ДополнительныеПараметры.Вставить("Выполнено", 		Результат.Количество()>0);
		ДополнительныеПараметры.Вставить("Отменено", 		Ложь);
		ДополнительныеПараметры.Вставить("ОписанияФайлов", 	Результат);
	КонецЕсли;
	
	ВыполнитьОбработкуОповещения(ОповещениеОЗавершении, ДополнительныеПараметры)
...
Процедура СтандартнаяОбработкаОшибки(СтандартнаяОбработка, ВходящийКонтекст, ИмяМетода, ОписаниеОшибки = Неопределено, ИнформацияОбОшибке = Неопределено)
	...
	Если ВходящийКонтекст.Свойство("СтандартнаяОбработкаИсключения") Тогда
		Если ТипЗнч(ВходящийКонтекст.ОповещениеОЗавершении) = Тип("ОписаниеОповещения") 
			И (ТипЗнч(ВходящийКонтекст.ОповещениеОЗавершении.ДополнительныеПараметры) = Тип("Структура")
			ИЛИ ТипЗнч(ВходящийКонтекст.ОповещениеОЗавершении.ДополнительныеПараметры) = Тип("ФиксированнаяСтруктура"))
...
Процедура ВывестиСообщениеПриНеобходимости(ВыводитьСообщения, Сообщение)
	...
	Если ВыводитьСообщения Тогда
		ДлительнаяОтправкаКлиентСервер.ВывестиОшибку(Сообщение);
	КонецЕсли;
...
Процедура ПоказатьВопросОбУстановкеРасширенияРаботыСФайлами(ИмяМетода, Контекст, ВозможноПродолжениеБезУстановки)
	...
	ФайловаяСистемаКлиент.ПодключитьРасширениеДляРаботыСФайлами(Оповещение, ТекстСообщения, ВозможноПродолжениеБезУстановки);
	
КонецПроцедуры
	
Функция ПолучитьОписаниеФайла(ПолноеИмя, Адрес)
