...
Процедура ДобавитьКомандыПечати(КомандыПечати) Экспорт
	Если ПолучитьФункциональнуюОпцию("ИспользоватьИндексациюЗаработка") Тогда
		...
		КомандаПечати.Обработчик = "УправлениеПечатьюБЗККлиент.ВыполнитьКомандуПечати";
		КомандаПечати.МенеджерПечати = "Обработка.ПечатьКадровыхПриказовРасширенная";
		КомандаПечати.Идентификатор = "ПФ_MXL_ИндексацияЗаработка_ПоПравиламВеденияДО";
		КомандаПечати.Представление = НСтр("ru = 'Приказ об индексации заработка (По утвержденным правилам ДО)'");
		// ЗУПК
	КонецЕсли;
КонецПроцедуры
...
Функция ПараметрыПолученияНачисленийПоказателейСотрудников() Экспорт 
	...
	ПараметрыПолучения.Вставить("УчитыватьКакИндексацию", Ложь);
...
Функция ИзмененныеПоказателиСотрудниковНаПозиции(Запрос)
	...
	|	НачисленияПозицииИСотрудников.Сотрудник КАК Сотрудник,
	|	НачисленияПозицииИСотрудников.ИдентификаторСтрокиСотрудника КАК ИдентификаторСтрокиСотрудника,
	|	НачисленияПозицииИСотрудников.Позиция КАК Позиция,
	...
	|	ПоказателиПоСотрудникам.ИдентификаторСтрокиСотрудника КАК ИдентификаторСтрокиСотрудника,
	|	ПоказателиПоСотрудникам.Показатель КАК Показатель,
	|	ПоказателиПоСотрудникам.Показатель.Точность КАК Точность,
	...
	Для Каждого СтрокаПоказателей Из Показатели Цикл
		СтрокаПоказателей.Значение = Окр(СтрокаПоказателей.Значение, СтрокаПоказателей.Точность);
	КонецЦикла;
...
Процедура СоздатьВТПлановыеНачисленияСотрудников(Запрос, ОписаниеФильтра, ПараметрыПолучения)
	...
	ИсключаемыеКатегории.Добавить(Перечисления.КатегорииНачисленийИНеоплаченногоВремени.ПособиеПоУходуЗаРебенкомДоПолутораЛетВоеннослужащим);
	ИсключаемыеКатегории.Добавить(Перечисления.КатегорииНачисленийИНеоплаченногоВремени.ПособиеПоУходуЗаРебенкомДоТрехЛет);
	
	Запрос.Текст = СтрЗаменить(Запрос.Текст, "ВТФильтрСотрудников", ОписаниеФильтра.ЗначениеФильтра.ИмяВременнойТаблицы);
...
Процедура ПодготовитьВременныеТаблицыПоПозиции(Запрос, ПараметрыПолучения)
	...
	СоздатьВТИзмененияНачисленийПозиций(Запрос, ДанныеПозиций, ПредыдущиеДанныеПозиций, ПараметрыПолучения);
	СоздатьВТТекущиеПоказателиПозиций(Запрос, ДанныеПозиций, ПредыдущиеДанныеПозиций, ПараметрыПолучения);
	
	УстановитьПривилегированныйРежим(Ложь);
	
КонецПроцедуры

Процедура СоздатьВТИзмененияНачисленийПозиций(Запрос, ДанныеПозиций, ПредыдущиеДанныеПозиций, ПараметрыПолучения)
	
	ПредыдущиеНачисления = ПредыдущиеНачисленияПозиций(ПредыдущиеДанныеПозиций);
	...
	ПостоянныеНачисления = Новый Массив;
	ПостоянныеНачисления.Добавить(ПланыВидовРасчета.Начисления.НачислениеРайонныйКоэффициент());
	ПостоянныеНачисления.Добавить(ПланыВидовРасчета.Начисления.НачислениеСевернаяНадбавка());
	ПостоянныеНачисления.Добавить(ПланыВидовРасчета.Начисления.НачислениеНадбавкаЗаВредность());
	ОбщегоНазначенияКлиентСервер.УдалитьВсеВхожденияЗначенияИзМассива(ПостоянныеНачисления, Неопределено);
	...
	Для Каждого КлючИЗначение Из ДанныеПозиций Цикл
		...
		Для Каждого Начисление Из ДанныеПозиции.Начисления Цикл
			...
			Иначе
				Если ПостоянныеНачисления.Найти(Начисление.Начисление) <> Неопределено Тогда
					НоваяСтрокаНачислений.Действие = Перечисления.ДействияСНачислениямиИУдержаниями.Утвердить;
				КонецЕсли;
...
Функция ПредыдущиеНачисленияПозиций(ПредыдущиеДанныеПозиций)
...
Процедура СоздатьВТТекущиеПоказателиПозиций(Запрос, ДанныеПозиций, ПредыдущиеДанныеПозиций, ПараметрыПолучения)
	...
	ТаблицаПредыдущихПоказателей = ТаблицаПоказателей.СкопироватьКолонки();
	...
	Если ПараметрыПолучения.УчитыватьКакИндексацию Тогда
		Для Каждого КлючИЗначение Из ПредыдущиеДанныеПозиций Цикл
			ДанныеПозиции = КлючИЗначение.Значение;
			Для Каждого Начисление Из ДанныеПозиции.Начисления Цикл
				Для каждого ПоказательНачисления Из Начисление.Показатели Цикл
					НоваяСтрока = ТаблицаПредыдущихПоказателей.Добавить();
					ЗаполнитьЗначенияСвойств(НоваяСтрока, ПоказательНачисления);
					НоваяСтрока.Позиция = КлючИЗначение.Ключ;
				КонецЦикла;
			КонецЦикла;
	...
	ЗарплатаКадры.СоздатьВТПоТаблицеЗначений(Запрос.МенеджерВременныхТаблиц, ТаблицаПоказателей, "ВТПоказателиПозиции");
	ЗарплатаКадры.СоздатьВТПоТаблицеЗначений(Запрос.МенеджерВременныхТаблиц, ТаблицаПредыдущихПоказателей, "ВТПредыдущиеПоказателиПозиции");
	
	Запрос.Текст = 
	"ВЫБРАТЬ
	|	ТаблицаПоказателей.Позиция КАК Позиция,
	|	ТаблицаПоказателей.Показатель КАК Показатель,
	|	ТаблицаПоказателей.ДокументОснование КАК ДокументОснование,
	|	ТаблицаПоказателей.ЗначениеМин КАК ЗначениеПоказателяМин,
	|	ТаблицаПоказателей.ЗначениеМакс КАК ЗначениеПоказателяМакс,
	|	ЕСТЬNULL(ТаблицаПредыдущиеПоказателей.ЗначениеМин, 0) КАК ПредыдущееЗначениеПоказателяМин,
	|	ЕСТЬNULL(ТаблицаПредыдущиеПоказателей.ЗначениеМакс, 0) КАК ПредыдущееЗначениеПоказателяМакс
	|ПОМЕСТИТЬ ВТТекущиеПоказателиПозиции
	|ИЗ
	|	ВТПоказателиПозиции КАК ТаблицаПоказателей
	|		ЛЕВОЕ СОЕДИНЕНИЕ ВТПредыдущиеПоказателиПозиции КАК ТаблицаПредыдущиеПоказателей
	|		ПО ТаблицаПоказателей.Позиция = ТаблицаПредыдущиеПоказателей.Позиция
	|			И ТаблицаПоказателей.Показатель = ТаблицаПредыдущиеПоказателей.Показатель
	|			И ТаблицаПоказателей.ДокументОснование = ТаблицаПредыдущиеПоказателей.ДокументОснование";
...
Процедура СоздатьВТНачисленияПозицииИСотрудников(Запрос)
	...
	|	СотрудникиПоПозиции.ИдентификаторСтрокиСотрудника КАК ИдентификаторСтрокиСотрудника,
	|	СотрудникиПоПозиции.ДолжностьПоШтатномуРасписанию КАК Позиция,
	|	НачисленияПоПозиции.Начисление КАК Начисление,
	...
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВТИзмененияНачисленийПозиции КАК НачисленияПоПозиции
	...
	|	ЕСТЬNULL(НачисленияСотрудниковПозиции.Позиция, СотрудникиПоПозиции.ДолжностьПоШтатномуРасписанию) КАК Позиция,
	...
	|				И НачисленияСотрудниковПозиции.Действие = ЗНАЧЕНИЕ(Перечисление.ДействияСНачислениямиИУдержаниями.ПустаяСсылка)
	|			ТОГДА ЗНАЧЕНИЕ(Перечисление.ДействияСНачислениямиИУдержаниями.Отменить)
	|		ИНАЧЕ ЕСТЬNULL(НачисленияСотрудниковПозиции.Действие, ЗНАЧЕНИЕ(Перечисление.ДействияСНачислениямиИУдержаниями.ПустаяСсылка))
	|	КОНЕЦ КАК Действие
	...
	|		ЛЕВОЕ СОЕДИНЕНИЕ ВТСотрудникиПериоды КАК СотрудникиПоПозиции
	|		ПО (СотрудникиПоПозиции.Сотрудник = ПлановыеНачисления.Сотрудник)
	|			И (СотрудникиПоПозиции.Период = ПлановыеНачисления.Период)";
...
Функция НачисленияПоказателиСотрудниковПоОбъекту(ДокументОбъект, ФильтрСотрудников = Неопределено, ИспользоватьФильтрПоНачислениям = Истина, ДолжностиПоШтатномуРасписанию = Неопределено) Экспорт
	...
	ПараметрыПолучения.Вставить("УчитыватьКакИндексацию", ДокументОбъект.УчитыватьКакИндексациюЗаработка);
