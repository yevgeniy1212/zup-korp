...
Функция ПолучитьДанныеДляПроведения()
	...
	Запрос.Текст = 
		...
		|			ИзменениеПлановыхНачисленийСотрудники.ДатаСобытия КАК ДатаИндексации,
		|			ИзменениеПлановыхНачисленийСотрудники.Сотрудник КАК Сотрудник,
		|			ИзменениеПлановыхНачисленийСотрудники.КоэффициентИндексации КАК КоэффициентИндексации
		|		ИЗ
		|			ВТСотрудникиДокумента КАК ИзменениеПлановыхНачисленийСотрудники
		|		
		|		ОБЪЕДИНИТЬ ВСЕ
		...
		|				ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВТСотрудникиДокумента КАК ИзменениеПлановыхНачисленийСотрудники
		|				ПО (ИзменениеПлановыхНачисленийСотрудники.Сотрудник = КоэффициентыИндексацииЗаработка.Сотрудник)
		|		ГДЕ
		|			КоэффициентыИндексацииЗаработка.Регистратор <> &Ссылка) КАК КоэффициентыИндексацииЗаработка
		|		ПО ИзменениеПлановыхНачисленийСотрудники.Сотрудник = КоэффициентыИндексацииЗаработка.Сотрудник
		|			И (ВЫБОР
		|				КОГДА ИзменениеПлановыхНачисленийСотрудники.КоэффициентИндексации > 1
		|					ТОГДА ИзменениеПлановыхНачисленийСотрудники.ДатаСобытия >= КоэффициентыИндексацииЗаработка.ДатаИндексации
		|				ИНАЧЕ ИзменениеПлановыхНачисленийСотрудники.ДатаСобытия = КоэффициентыИндексацииЗаработка.ДатаИндексации
		|			КОНЕЦ)
		|			И (ГОД(ИзменениеПлановыхНачисленийСотрудники.ДатаСобытия) + МЕСЯЦ(ИзменениеПлановыхНачисленийСотрудники.ДатаСобытия) - ГОД(ИзменениеПлановыхНачисленийСотрудники.ДатаСобытия) - МЕСЯЦ(ИзменениеПлановыхНачисленийСотрудники.ДатаСобытия) + 1 <= 12)
		|
		|УПОРЯДОЧИТЬ ПО
		|	Сотрудник,
		|	Период УБЫВ,
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("ЗарплатаКадрыПриложения.ГосударственнаяСлужба.РасчетДенежногоСодержания") Тогда
		Модуль = ОбщегоНазначения.ОбщийМодуль("РасчетДенежногоСодержания");
		КоэффициентыИндексацииДенежногоСодержания = Модуль.ТаблицаКоэффициентовИндексацииДенежногоСодержанияСотрудников(ДанныеДляПроведения.КоэффициентыИндексации);
...
Процедура ЗаполнитьДокументПоТарифнойСетке(ТарифнаяСетка) Экспорт
	...
	Если Не РезультатЗапроса.Пустой() Тогда
		...
		Если СотрудникиДляРасчетаФОТ.Количество() > 0 Тогда
			...
			// Расчет ФОТ
			РассчитанныеДанныеНачислений = ПлановыеНачисленияСотрудников.РассчитатьВторичныеДанныеПлановыхНачислений(ТаблицаНачислений, ТаблицаПоказателей, КадровыеДанные);
			...
			Для каждого СтрокаРассчитанногоНачисления Из РассчитанныеДанныеНачислений.ПлановыйФОТ Цикл
			...
			Для Каждого СтрокаСотрудника Из СтрокиСотрудников Цикл
				...
				ДанныеСотрудников = РассчитанныеДанныеНачислений.ТарифныеСтавки.НайтиСтроки(Новый Структура("Период,Сотрудник", ВремяРегистрацииСотрудников.Получить(СтрокаСотрудника.Сотрудник), СтрокаСотрудника.Сотрудник));
