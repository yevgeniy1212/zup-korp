...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Параметры.Ключ.Пустая() Тогда
		...
		// ЗУПК
		Иначе
			Объект.ВыплачиватьПособиеДоПолутораЛет = Ложь;
		КонецЕсли;
		// ЗУПК
...
Процедура Подключаемый_НалогиВзносыОтчисленияВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	УчетНалоговВзносовОтчисленийКлиентРасширенный.ОткрытьФормуРасчетаНалогаВзносаОтчисления(
	   Объект, ЭтаФорма, ДополнительныеПараметрыНВО, ОписаниеДокумента(ПараметрыОписанияДокумента(ЭтотОбъект)), Ложь);
...
Процедура ДополнитьФорму(ОтложенноеИзменение = Неопределено)
	...
	Иначе	
		...
		// ЗУПК
		РасчетЗарплатыРасширенныйФормы.ДокументыВыполненияНачисленийДополнитьФорму(
			ЭтотОбъект, ОписаниеТаблицыНалогиВзносыОтчисления(), "", "НалогиВзносыОтчисленияАвтоКоманды" , , , ДобавлятьЭлементыФормы, ДобавлятьРеквизитыФормы, ОтложенноеИзменение, Объект.Ссылка);
		// ЗУПК
...
Функция ОписаниеТаблицыНачислений()
	...
	// ЗУПК
	ОписаниеТаблицыВидовРасчета.ИмяПоляРезультат = "Размер";
	// ЗУПК
...
Функция ОписаниеДокумента(ПараметрыОписания)
	...
	// ЗУПК
	Если НЕ ЭтоМежрасчетнаяВыплата(ПараметрыОписания.ПорядокВыплаты) Тогда
...
Процедура УстановитьВидимостьРасчетныхПолей()
	...
	// ЗУПК
	ЗарплатаКадрыРасширенный.УстановитьОтображениеГруппыФормы(Элементы, "ГруппаДанныеДляФСС", "Видимость", Ложь);
	// ЗУПК
	...
	Если Объект.РассчитатьЗарплату Тогда 
		...
		Если ЭтоМежрасчетнаяВыплата(Объект.ПорядокВыплаты) Тогда
			ИменаЭлементов.Добавить("ГруппаНалогиВзносыОтчисления")
		КонецЕсли;
		// ЗУПК
	...
	Иначе
		...
		// ЗУПК
		ЗарплатаКадрыРасширенный.УстановитьОтображениеГруппыФормы(Элементы, "ГруппаНалогиВзносыОтчисления", "Видимость", Ложь);
		// ЗУПК
		Если ОбщегоНазначения.ПодсистемаСуществует("ЗарплатаКадрыКорпоративнаяПодсистемы.ЛьготыСотрудников") Тогда
...
Процедура ДанныеФормыВДанныеМенеджераРасчета(МенеджерРасчета)
	...
	УчетНалоговВзносовОтчисленийРасширенный.НалогиВзносыОтчисленияВДанныеМенеджераРасчета(Объект.НалогиВзносыОтчисления, Объект.ВычетыИПН, Объект.ДоходыНеПодлежащиеНалогообложению, МенеджерРасчета);
    // ЗУПК
...
Процедура УстановитьУсловноеОформлениеВычеты()
	...
	ЭлементОтбора.ЛевоеЗначение = Новый ПолеКомпоновкиДанных("Объект.ПериодРегистрации");
...
Процедура ОбновитьДанныеНалогиВзносыОтчисленияНаСервере(АдресВременногоХранилища)
    ...
    Если ТипЗнч(АдресВременногоХранилища) = Тип("Строка") И ЭтоАдресВременногоХранилища(АдресВременногоХранилища) Тогда
        ...
        КонтролируемыеПоля = Новый Структура;
        ОписаниеДокумента = ОписаниеДокумента(ПараметрыОписанияДокумента(ЭтотОбъект));
        ...
        РассчитатьСотрудникаНаСервере(ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(ПараметрыДанных.ФизическоеЛицо), ОписаниеТаблицыНалогиВзносыОтчисления());
        РассчитатьИтогиВычеты(ЭтаФорма);
