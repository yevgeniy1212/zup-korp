...
Процедура ПриОткрытии(Отказ)
	...
	Для Каждого Элемент Из ТаблицаСообщений.ПолучитьЭлементы() Цикл
		Если Элемент.ПолучитьРодителя() = Неопределено Тогда
			Элемент.Описание = ?(ТолькоПредупреждения, "Предупреждений", "Ошибок") + " в отчете: "
							 + Элемент.ПолучитьЭлементы().Количество();
		КонецЕсли;
	КонецЦикла;
	...
	НадписьВсегоОшибок = "Всего " + ?(ТолькоПредупреждения, "предупреждений", "ошибок")
						+ ": "  + Формат(ВладелецТС.ТаблицаСообщений.Количество(), "ЧГ=")
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Параметры.Свойство("ТолькоПредупреждения") И Параметры.ТолькоПредупреждения = Истина Тогда
...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	Если ИмяСобытия = "ЗакрытьФормуНавигацииПоОшибкамВыгрузки" И Открыта() Тогда
