	...
	Иначе
		...
		Если НЕ ФайлВыгрузкиЗаписан Тогда
			...
			КодировкаФайлаВыгрузки = ?(Объект.Выгрузки.Получить(0).Тип = ВерсииФорматовВыгрузки.ПеречислениеВерсииФорматовВыгрузкиВерсия400
				...
				ИЛИ Объект.Выгрузки.Получить(0).Тип = ВерсииФорматовВыгрузки.ПеречислениеВерсииФорматовВыгрузкиВерсияЕФСПФР
				ИЛИ Объект.Выгрузки.Получить(0).Тип = ВерсииФорматовВыгрузки.ПеречислениеВерсииФорматовВыгрузкиВерсияБанкРоссии
				ИЛИ Объект.Выгрузки.Получить(0).Тип = ВерсииФорматовВыгрузки.ПеречислениеВерсииФорматовВыгрузкиВерсияМинобороны
				ИЛИ (Объект.Выгрузки.Получить(0).Тип = ВерсииФорматовВыгрузки.ПеречислениеВерсииФорматовВыгрузкиВерсияФСС
				И ВРег(Лев(Объект.Выгрузки.Получить(0).ИмяФайла, 4)) = "POVD"), "UTF-8", "cp866"));
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ВерсииФорматовВыгрузки.Вставить("ПеречислениеВерсииФорматовВыгрузкиВерсияЕФСПФР", Перечисления.ВерсииФорматовВыгрузки.ВерсияЕФСПФР);
	...
	ВерсииФорматовВыгрузки.Вставить("ПеречислениеВерсииФорматовВыгрузкиВерсияМинобороны", Перечисления.ВерсииФорматовВыгрузки.ВерсияМинобороны);
...
Процедура СформироватьТекстыВыгрузки(Команда)
	...
	СтруктДопПараметров = Новый Структура("УдалитьВыгрузкуПояснительнойЗапискиКДекларацииПоЕНВД, НеОбновлятьДляФСНБР");
	
	СформироватьТекстыВыгрузкиКлиент( , , Истина, , , , , СтруктДопПараметров);
...
Процедура СформироватьТекстыВыгрузкиКлиент(ПечатаемыеЛисты = Неопределено, ДляЦелейФормированияМЧБ = Ложь, ПроверкаБлокировкиФормы = Ложь, ДляОтправкиВФНС = Ложь, ПервыйДок = Неопределено, СообщенияОбОшибках = Неопределено, ВыгружатьСОшибками = Ложь, СтруктураДополнительныхПараметров = Неопределено)
	...
	ОтборЕФСПФР	 = ВерсииИОтчеты.НайтиСтроки(Новый Структура("Версия", ВерсииФорматовВыгрузки.ПеречислениеВерсииФорматовВыгрузкиВерсияЕФСПФР));
	...
	ОтборМинобороны = ВерсииИОтчеты.НайтиСтроки(Новый Структура("Версия", ВерсииФорматовВыгрузки.ПеречислениеВерсииФорматовВыгрузкиВерсияМинобороны));
	...
	Если ОтборФСС.Количество() <> 0 Тогда
		...
		Для Каждого Стр Из ОтборФСС Цикл
			...
			СообщитьОбОбработкеОтчета(Стр, НомерОтчета, ВерсииИОтчеты.Количество());
			...
			Попытка
				...
				Если ЭтаФорма.ВладелецФормы = Неопределено
				 ИЛИ НЕ ТипЗнч(ЭтаФорма.ВладелецФормы) = Тип("ФормаКлиентскогоПриложения") Тогда
			...
			Исключение
				Попытка
					Для Каждого СтрСообщения Из ТекФорма.ТаблицаСообщений Цикл
						...
						НовСтр.Отчет = Строка(РегламентированнаяОтчетностьКлиентСервер.ПредставлениеДокументаРеглОтч(Стр));
	...
	//выгрузка в формате ЕФС ПФР
	Если ОтборЕФСПФР.Количество() <> 0 И (НЕ ВыгрузкаПрервана) Тогда
		
		Текст = Новый ТекстовыйДокумент;//текст, объединяющий все выгруженные документы в формате ЕФС ПФР
		ТекТекст = Новый ТекстовыйДокумент;//текст выгрузки текущего документа
		
		//последовательно, для каждого документа из списка, инициализируем "контекст"
		//(создаем окружение) и вызываем экспортную процедуру, формирующую текст выгрузки
		Для Каждого Стр Из ОтборЕФСПФР Цикл
			НомерОтчета = НомерОтчета + 1;
			СообщитьОбОбработкеОтчета(Стр, НомерОтчета, ВерсииИОтчеты.Количество());
			...
			Попытка
				...
				Если ТипЗнч(РезультатВыполненияВыгрузки) = Тип("Структура") Тогда
					...
					Иначе
						...
						ПараметрыФормы.Вставить("НазваниеДекларации",
						РегламентированнаяОтчетностьКлиентСервер.ПредставлениеДокументаРеглОтч(Стр));
						
						Оповестить("ЗакрытьФормуСообщенийОбОшибках");
						
						ОткрытьФорму("Документ.ВыгрузкаРегламентированныхОтчетов.Форма.ФормаСообщенийОбОшибках", ПараметрыФормы);
			...
			Исключение
				...
				Если СообщенияОбОшибках = Неопределено Тогда
					...
					"ru='Не удалось выгрузить %1.'"), РегламентированнаяОтчетностьКлиентСервер.ПредставлениеДокументаРеглОтч(Стр));
					Сообщение.Сообщить();
				...
				Если ТипЗнч(ТекстВыгрузкиВXML) = Тип("Массив") Тогда
					Для Каждого ЭлементПакетаВыгрузки Из ТекстВыгрузкиВXML Цикл
						Если ТипЗнч(ЭлементПакетаВыгрузки) = Тип("Структура") Тогда
							...
							НовСтр.Тип      = ВерсииФорматовВыгрузки.ПеречислениеВерсииФорматовВыгрузкиВерсияЕФСПФР;
							НовСтр.Текст    = СтрокиПараметровИзСтруктуры(ЭлементПакетаВыгрузки);
							ТекСтр = Основания.НайтиСтроки(Новый Структура("Ссылка", Стр.Док));
							Если ТекСтр.Количество() > 0 И ТекСтр[0].Пометка
								И НЕ ЭтоПояснительнаяЗапискаКДекларацииПоЕНВД(НовСтр.ИмяФайла) Тогда
								ТекСтр[0].НомерСтрокиТекстаВыгрузки = НовСтр.НомерСтроки;
							КонецЕсли;
						Иначе
							НовСтр = Объект.Выгрузки.Добавить();
	...
	КонецЕсли;
	//конец выгрузки в формате ЕФС ПФР
		...
		Для Каждого Стр Из Отбор500 Цикл
			...
			Попытка
				...
				Если ТипЗнч(СтруктураДополнительныхПараметров) = Тип("Структура")
					И СтруктураДополнительныхПараметров.Свойство("УдалитьВыгрузкуПояснительнойЗапискиКДекларацииПоЕНВД")
					И СтрНайти(Стр.ИсточникОтчета, "РегламентированныйОтчетЕдиныйНалогНаВмененныйДоход") > 0 Тогда
					
					РезультатВыполненияВыгрузки = ТекФорма.УниверсальнаяВыгрузка(ТекстВыгрузкиВXML, Ложь);
					
				ИначеЕсли ДляОтправкиВФНС
					И ((СтрНайти(ВРег(Стр.ИсточникОтчета), ВРег("РегламентированныйОтчетЗемельныйНалог")) > 0
					И СтрНайти(ВРег(Стр.ВыбраннаяФорма), ВРег("ФормаОтчета2018Кв3")) > 0)
					ИЛИ (СтрНайти(ВРег(Стр.ИсточникОтчета), ВРег("РегламентированныйОтчетТранспортныйНалог")) > 0
					И СтрНайти(ВРег(Стр.ВыбраннаяФорма), ВРег("ФормаОтчета2019Кв1")) > 0)) Тогда
					
					РезультатВыполненияВыгрузки = ТекФорма.УниверсальнаяВыгрузка(ТекстВыгрузкиВXML, НЕ ДляЦелейФормированияМЧБ, Истина);
	...
	Если ОтборБанкРоссии.Количество() <> 0 И (НЕ ВыгрузкаПрервана) Тогда
		...
		Для Каждого Стр Из ОтборБанкРоссии Цикл
			...
			Если НЕ ВыгрузкаПрервана Тогда
				Если ТипЗнч(ТекстВыгрузкиВXML) = Тип("Массив") Тогда
					НеОбновлятьТекстыДляФСНБР = ТипЗнч(СтруктураДополнительныхПараметров) = Тип("Структура")
						И СтруктураДополнительныхПараметров.Свойство("НеОбновлятьДляФСНБР");
					Для Каждого ЭлементПакетаВыгрузки Из ТекстВыгрузкиВXML Цикл
						Если ТипЗнч(ЭлементПакетаВыгрузки) = Тип("Структура") Тогда
							НовСтр = Объект.Выгрузки.Добавить();
							НовСтр.ИмяФайла = ЭлементПакетаВыгрузки.ИмяФайлаВыгрузки;
							...
							ОбновитьТекстВыгрузки = Истина;
							ТекстВыгрузкиБанкРоссии  = ПолучитьСтрокуИзВременногоХранилища(ЭлементПакетаВыгрузки.АдресФайлаВыгрузки,
								?(ЭлементПакетаВыгрузки.Свойство("КодировкаФайлаВыгрузки"),
									ЭлементПакетаВыгрузки.КодировкаФайлаВыгрузки, "windows-1251"));
							Если Лев(ТекстВыгрузкиБанкРоссии, 9) = "Структура" Тогда
								СтруктураПараметровБанкРоссии = СтруктураИзСтрокПараметров(ТекстВыгрузкиБанкРоссии);
								Если  НеОбновлятьТекстыДляФСНБР
									И СтруктураПараметровБанкРоссии.Свойство("ТипФайлаВыгрузки")
									И СтруктураПараметровБанкРоссии["ТипФайлаВыгрузки"] = "ОтчетФСНБР"  Тогда
									ОбновитьТекстВыгрузки = Ложь;
								КонецЕсли;
							КонецЕсли;
							Если ОбновитьТекстВыгрузки Тогда
								НовСтр.Текст = ТекстВыгрузкиБанкРоссии;
							Иначе
								НовСтр.Текст = НСтр("ru='Отчет ФСН был сохранен в каталог в бинарном формате'");
							КонецЕсли;
	...
	// Выгрузка в формате Минобороны.
	Если ОтборМинобороны.Количество() <> 0 И (НЕ ВыгрузкаПрервана) Тогда
		
		Текст = Новый ТекстовыйДокумент;//текст, объединяющий все выгруженные документы в формате Минобороны
		ТекТекст = Новый ТекстовыйДокумент;//текст выгрузки текущего документа
	
		// Последовательно, для каждого документа из списка, инициализируем "контекст"
		//(создаем окружение) и вызываем экспортную процедуру, формирующую текст выгрузки.
		Для Каждого Стр Из ОтборМинобороны Цикл
			НомерОтчета = НомерОтчета + 1;
			СообщитьОбОбработкеОтчета(Стр, НомерОтчета, ВерсииИОтчеты.Количество());
				...
				Если ТипЗнч(ТекстВыгрузкиВXML) = Тип("Массив") Тогда
					Для Каждого ЭлементПакетаВыгрузки Из ТекстВыгрузкиВXML Цикл
						Если ТипЗнч(ЭлементПакетаВыгрузки) = Тип("Структура") Тогда
							НовСтр = Объект.Выгрузки.Добавить();
							НовСтр.ИмяФайла = ЭлементПакетаВыгрузки.ИмяФайлаВыгрузки;
							НовСтр.Тип      = ВерсииФорматовВыгрузки.ПеречислениеВерсииФорматовВыгрузкиВерсияМинобороны;
	...
	КонецЕсли;
	// Конец выгрузки в формате Минобороны.
...
Процедура СформироватьТекстыВыгрузкиСервер(ПервыйДок, ДляОтправкиВФНС, КПП, ОшибкаВыгрузки)
	...
	// Проверка на единый код ИФНС у всех документов-оснований.
	НалоговыеОрганы.Очистить();
	Для Каждого Стр Из Основания Цикл
		Если Стр.Пометка И СокрЛП(Стр.КодНалоговогоОргана) <> "" Тогда //исключаем из проверки отчеты в ПФР и в ФСС с пустым кодом ИФНС
	...
	// Таблица соответствия "документ для выгрузки <-> версия формата выгрузки".
	...
	Если ОтчетыВыгружаемыеТолькоИзФормы.Количество() > 0 Тогда
		...
		Для Каждого Док Из ОтчетыВыгружаемыеТолькоИзФормы Цикл
			...
			Сообщение.Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru=' - %1'"),
				РегламентированнаяОтчетностьКлиентСервер.ПредставлениеДокументаРеглОтч(Док));
			Сообщение.Сообщить();
		КонецЦикла;
	...
	ОтчетыБаланса = РегламентированнаяОтчетностьПовтИсп.ПолучитьОтчетыБаланса();
	ОтчетыБаланса.Добавить("РасшифровкиБухгалтерскойОтчетности");
	
	ОтчетыАлко = РегламентированнаяОтчетностьПовтИсп.ПолучитьОтчетыАлко();
	ОтчетыФСС = РегламентированнаяОтчетностьПовтИсп.ПолучитьОтчетыФСС();
	ОтчетыЗаявления = РегламентированнаяОтчетностьПовтИсп.ПолучитьОтчетыЗаявления();
	ОтчетыРСВ2 = РегламентированнаяОтчетностьПовтИсп.ПолучитьОтчетыРСВ2();
	ОтчетыРВ3 = РегламентированнаяОтчетностьПовтИсп.ПолучитьОтчетыРВ3();
	ОтчетыФСГС = РегламентированнаяОтчетностьПовтИсп.ПолучитьОтчетыФСГС();
	ОтчетыРПН = РегламентированнаяОтчетностьПовтИсп.ПолучитьОтчетыРПН();
	ОтчетыФСНБР = РегламентированнаяОтчетностьПовтИсп.ПолучитьОтчетыФСНБанкРоссии();
	ОтчетыМинобороны = РегламентированнаяОтчетностьПовтИсп.ПолучитьОтчетыМинобороны();
	
	ОтчетыСПолнойВнутреннейПроверкой = ОтчетыСПолнойВнутреннейПроверкой();
	...
	Для Каждого Стр Из ВерсииИОтчеты Цикл
		...
		Если ОтчетыСПолнойВнутреннейПроверкой.Найти(ИсточникОтчета) <> Неопределено
		 ИЛИ ОтчетыФСГС.НайтиПоЗначению(ИсточникОтчета) <> Неопределено
		 ИЛИ ОтчетыФСНБР.НайтиПоЗначению(ИсточникОтчета) <> Неопределено
		 ИЛИ ОтчетыРПН.НайтиПоЗначению(ИсточникОтчета) <> Неопределено
		 ИЛИ ОтчетыЗаявления.НайтиПоЗначению(ИсточникОтчета) <> Неопределено Тогда
			Продолжить;
		КонецЕсли;
		...
		ИначеЕсли ОтчетыРВ3.НайтиПоЗначению(ИсточникОтчета) <> Неопределено Тогда
			...
			КонецЕсли;
			Продолжить;
		ИначеЕсли ОтчетыМинобороны.НайтиПоЗначению(ИсточникОтчета) <> Неопределено Тогда
			Если ЕдиницаИзмерения <> ПорядкиОкругленияОтчетности.ПеречислениеПорядкиОкругленияОтчетностиОкр1
			 ИЛИ Точность <> 2 Тогда
				ОтчетыСохраненныеСНевернымОкруглением.Добавить(Стр);
			КонецЕсли;
			Продолжить;
		ИначеЕсли Стр.ИсточникОтчета = "РегламентированныйОтчетПодтверждениеВидаДеятельности" Тогда
			Продолжить;
		Иначе
	...
	Если ДляОтправкиВФНС И ОтчетыНеотправляемыеВФНС.Количество() > 0 Тогда
		...
		Для Каждого ОтчетНеотправляемыйВФНС Из ОтчетыНеотправляемыеВФНС Цикл
			Сообщение = Новый СообщениеПользователю;
			Сообщение.Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru=' - %1'"),
				РегламентированнаяОтчетностьКлиентСервер.ПредставлениеДокументаРеглОтч(ОтчетНеотправляемыйВФНС));
			Сообщение.Сообщить();
		КонецЦикла;
...
&НаКлиентеНаСервереБезКонтекста
Функция ЭтоПояснительнаяЗапискаКДекларацииПоЕНВД(Знач ИмяФайла)
...
Процедура ОтобразитьФормуНавигацииПоОшибкам()
	...
	РегламентированнаяОтчетностьКлиент.ОткрытьФормуНавигацииПоОшибкамВыгрузки(ТаблицаСообщений);
...
Процедура СохранитьТексты(НЕЗаписыватьДокумент,
	...
	СочетанияРеглОтчетовИИндексовИзИменФайловВыгрузки = Новый Массив;
	...
	ВыполняетсяСохранениеБолееОдногоФайлаВыгрузкиВБраузереCHROME = (ВебБраузер = "CHROME" И СписокВыбора.Количество() > 1);
	...
	Для Каждого Стр Из СписокВыбора Цикл
		...
		Если НЕ ПустаяСтрока(ТекСтр.ИмяФайла) Тогда
			...
			Если ТекСтр.Тип = ВерсииФорматовВыгрузки.ПеречислениеВерсииФорматовВыгрузкиВерсия400
				...
				ИЛИ ТекСтр.Тип = ВерсииФорматовВыгрузки.ПеречислениеВерсииФорматовВыгрузкиВерсияЕФСПФР
				...
				ИЛИ ТекСтр.Тип = ВерсииФорматовВыгрузки.ПеречислениеВерсииФорматовВыгрузкиВерсияБанкРоссии
				ИЛИ ТекСтр.Тип = ВерсииФорматовВыгрузки.ПеречислениеВерсииФорматовВыгрузкиВерсияМинобороны Тогда
				...
				Если ((ТекСтр.Тип = ВерсииФорматовВыгрузки.ПеречислениеВерсииФорматовВыгрузкиВерсияФСГС)
					...
					ИЛИ (ТекСтр.Тип = ВерсииФорматовВыгрузки.ПеречислениеВерсииФорматовВыгрузкиВерсияФСС
					И ВРег(Лев(ИмяКонечногоФайла, 4)) = "POVD")
					ИЛИ ТекСтр.Тип = ВерсииФорматовВыгрузки.ПеречислениеВерсииФорматовВыгрузкиВерсияПФР
					ИЛИ ТекСтр.Тип = ВерсииФорматовВыгрузки.ПеречислениеВерсииФорматовВыгрузкиВерсияЕФСПФР
					ИЛИ ТекСтр.Тип = ВерсииФорматовВыгрузки.ПеречислениеВерсииФорматовВыгрузкиВерсияРПН
					ИЛИ ТекСтр.Тип = ВерсииФорматовВыгрузки.ПеречислениеВерсииФорматовВыгрузкиВерсияБанкРоссии
					...
					Если НЕ НЕЗаписыватьДокумент Тогда // НЕ вызов для выгрузки без сохранения в файл
						...
						Если ФайлВыгрузкиВоВременномХранилище = Неопределено Тогда
							...
							#Если ВебКлиент Тогда
								...
								Если ВВебКлиентеСохранятьФайлВыгрузкиИнтерактивно Тогда
									...
									Если АдресФайлаВыгрузки <> Неопределено Тогда
										...
										ИначеЕсли ВыполняетсяСохранениеБолееОдногоФайлаВыгрузкиВБраузереCHROME Тогда
										
											ДополнительныеПараметры = Новый Структура;
											ДополнительныеПараметры.Вставить("АдресФайлаВыгрузки", АдресФайлаВыгрузки);
											ДополнительныеПараметры.Вставить("ИмяКонечногоФайла", ИмяКонечногоФайла);
											
											ПоказатьВопрос(Новый ОписаниеОповещения("ПослеЗакрытияВопроса", ЭтотОбъект, ДополнительныеПараметры),
															СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
																НСтр("ru='Сохранить файл ""%1""?'"), ИмяКонечногоФайла),
									...
									Иначе
										...
										ИначеЕсли ВыполняетсяСохранениеБолееОдногоФайлаВыгрузкиВБраузереCHROME Тогда
											...
											ДополнительныеПараметры = Новый Структура;
											ДополнительныеПараметры.Вставить("АдресФайлаВыгрузки", ПоместитьФайлВыгрузкиВоВременноеХранилищеНаСервере(
														 Текст, ТекСтр.ИмяФайла, КодировкаТекстаВыгрузки));
											ДополнительныеПараметры.Вставить("ИмяКонечногоФайла", ИмяКонечногоФайла);
							...
							#Иначе
								...
								Если АдресФайлаВыгрузки <> Неопределено Тогда
									ДвоичныйФайл = ПолучитьИзВременногоХранилища(АдресФайлаВыгрузки);
									ДвоичныйФайл.Записать(ПолноеИмяКонечногоФайла);
								Иначе
									ТекстВыгрузки = ТекСтр.Текст;
							...
							Если СтрНайти(ТекСтр.ИмяФайла, "NO_BUHOTCH_") = 1
							 ИЛИ СтрНайти(ТекСтр.ИмяФайла, "NO_BUHOTCHRE_") = 1
							 ИЛИ СтрНайти(ТекСтр.ИмяФайла, "NO_BOUPR_") = 1
							 ИЛИ СтрНайти(ТекСтр.ИмяФайла, "NO_BOUPRRE_") = 1
							 ИЛИ СтрНайти(ТекСтр.ИмяФайла, "NO_BUHMP_") = 1 Тогда
								...
								#Если ВебКлиент Тогда
									...
									Если ВебБраузер = "CHROME" Тогда
										ВыполняетсяСохранениеБолееОдногоФайлаВыгрузкиВБраузереCHROME = Истина;
									КонецЕсли;
										...
										Если ВВебКлиентеСохранятьФайлВыгрузкиИнтерактивно Тогда
											...
											Для ИндМассива = 0 По МассивДополнительныхФайлов.Количество() - 1 Цикл
												...
												ИначеЕсли ВыполняетсяСохранениеБолееОдногоФайлаВыгрузкиВБраузереCHROME Тогда
													...
													ДополнительныеПараметры = Новый Структура;
													ДополнительныеПараметры.Вставить("АдресФайлаВыгрузки", АдресДанныхТекДопФайла);
													ДополнительныеПараметры.Вставить("ИмяКонечногоФайла", ИмяТекДопФайла);
											
													ПоказатьВопрос(Новый ОписаниеОповещения("ПослеЗакрытияВопроса", ЭтотОбъект, ДополнительныеПараметры),
																	СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
																		НСтр("ru='Сохранить файл ""%1""?'"), ИмяТекДопФайла),
    																РежимДиалогаВопрос.ДаНет);
				...
				Исключение
					...
					Если ФайлВыгрузкиВоВременномХранилище = Неопределено Тогда
						...
						Сообщение.Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Не удалось записать файл ""%1"". Возможно, нет прав для записи в выбранный каталог, нет доступа к файлу, т. к. файл открыт в другой программе, недостаточно места на диске.'"), ИмяКонечногоФайла);
					...
					Если НЕ НЕЗаписыватьДокумент Тогда // НЕ вызов для выгрузки без сохранения в файл
						...
						Если ФайлВыгрузкиВоВременномХранилище = Неопределено Тогда
							...
							#Иначе
								...
								Текст.Записать(ИмяКонечногоФайла, "cp866");
			...
			Если СтрНайти(ТекСтр.Текст, "КНД:1151074") > 0 Тогда
				Если СтрНайти(ТекСтр.Текст, "НАЛОГОВАЯ ДЕКЛАРАЦИЯ ПО АКЦИЗАМ НА ТАБАЧНЫЕ ИЗДЕЛИЯ") > 0 Тогда
					Для каждого ОснованиеВыгрузки Из Основания Цикл
						Если ОснованиеВыгрузки.ИсточникОтчета = "РегламентированныйОтчетАкцизыТабак" Тогда
							
							ОснованиеВыгрузкиПериод = Формат(Месяц(ОснованиеВыгрузки.ДатаОкончания), "ЧЦ=2; ЧВН=");
							ОснованиеВыгрузкиГод = Формат(Год(ОснованиеВыгрузки.ДатаОкончания), "ЧГ=");
							
							ТекстВыгрузкиПериодПозицияПервогоСимвола = СтрНайти(ТекСтр.Текст, "НомерПериодОтч:") + 15;
							ТекстВыгрузкиГодПозицияПервогоСимвола = СтрНайти(ТекСтр.Текст, "ГодПериодОтч:") + 13;
							
							Если ТекстВыгрузкиПериодПозицияПервогоСимвола > 15
								И ТекстВыгрузкиГодПозицияПервогоСимвола > 13 Тогда
								
								ТекстВыгрузкиПериод = Сред(ТекСтр.Текст, ТекстВыгрузкиПериодПозицияПервогоСимвола, 2);
								ТекстВыгрузкиГод = Сред(ТекСтр.Текст, ТекстВыгрузкиГодПозицияПервогоСимвола, 4);
								
								Если ОснованиеВыгрузкиПериод = ТекстВыгрузкиПериод
									И ОснованиеВыгрузкиГод = ТекстВыгрузкиГод Тогда
									
									СочетанияРеглОтчетовИИндексовИзИменФайловВыгрузки.Добавить(
									Новый Структура("СсылкаНаРеглОтчет, ИндексИзИмениФайла", ОснованиеВыгрузки.Ссылка, СчФ));
	...
	Если ВыгруженХотяБыОдинФайл Тогда
		...
		Иначе
			...
			Если НЕ НЕЗаписыватьДокумент Тогда
				...
				СоздатьНовыйДокументВыгрузкаРегламентированныхОтчетов(Объект.ПериодПо, Объект.Организация, Объект.КодНалоговогоОргана, Объект.Основная, Объект.Выгрузки, СочетанияРеглОтчетовИИндексовИзИменФайловВыгрузки);
			КонецЕсли;
...
&НаСервере
Процедура СоздатьНовыйДокументВыгрузкаРегламентированныхОтчетов(ПериодПо, Организация, КодНалоговогоОргана, Знач Основная, Знач Выгрузки, СочетанияРеглОтчетовИИндексовИзИменФайловВыгрузки = Неопределено)
	...
	КолВыгрузок = Выгрузки.Количество();
	
	Для Инд = 0 По КолВыгрузок - 1 Цикл
		ОбрИнд = КолВыгрузок - 1 - Инд;
		ТекСтрока = Выгрузки[ОбрИнд];
		Если ЭтоПояснительнаяЗапискаКДекларацииПоЕНВД(ТекСтрока.ИмяФайла) Тогда
			Выгрузки.Удалить(ОбрИнд);
		КонецЕсли;
	КонецЦикла;
			...
			Если ТипЗнч(СтруктураПараметров) = Тип("Структура") И СтруктураПараметров.Свойство("АдресФайлаВыгрузки") Тогда
				Если Лев(СтруктураПараметров.АдресФайлаВыгрузки, 2) = "e1" Тогда
					...
					Попытка
						...
						Если СтруктураПараметров.Свойство("ТипФайлаВыгрузки", ТипФайлаВыгрузки) И ТипФайлаВыгрузки = "ОтчетФСНБР" Тогда
							ТекСтрока.Текст = НСтр("ru='Отчет ФСН был сохранен в каталог в бинарном формате'");
						Иначе
							ТекСтрока.Текст = ПолучитьСтрокуИзВременногоХранилища(СтруктураПараметров.АдресФайлаВыгрузки, Кодировка);
						КонецЕсли;
					Исключение
						Сообщение = Новый СообщениеПользователю;
						ТекстСообщения = "Не удалось записать текст выгрузки, соответствующий файлу """ + ТекСтрока.ИмяФайла + """. Возможно, нет прав для записи в выбранный каталог, нет доступа к файлу, т. к. файл открыт в другой программе, недостаточно места на диске.";
						Сообщение.Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='%1'"), ТекстСообщения);
						Сообщение.Сообщить();
					КонецПопытки;
				КонецЕсли;
	...
	Если ТипЗнч(СочетанияРеглОтчетовИИндексовИзИменФайловВыгрузки) = Тип("Массив") Тогда
		Для каждого СочетаниеРеглОтчетИИндекс Из СочетанияРеглОтчетовИИндексовИзИменФайловВыгрузки Цикл
			СохранитьИндексИзИмениФайлаВыгрузкиРегламентированногоОтчета(НовыйДок.Ссылка,
			СочетаниеРеглОтчетИИндекс.СсылкаНаРеглОтчет, СочетаниеРеглОтчетИИндекс.ИндексИзИмениФайла);
		КонецЦикла;
	КонецЕсли;
...
&НаСервере
Процедура СохранитьИндексИзИмениФайлаВыгрузкиРегламентированногоОтчета(ВыгрузкаРеглОтчетов, РеглОтчет, ИндексИзИмениФайла)
	
	Рег = РегистрыСведений.ДополнительныеФайлыРегламентированныхОтчетов;
	МенЗап = Рег.СоздатьМенеджерЗаписи();
	МенЗап.РегламентированныйОтчет = РеглОтчет;
	МенЗап.ВидДополнительногоФайла = "ИндексВИмениФайлаВыгрузкиДекларации_" + ВыгрузкаРеглОтчетов.УникальныйИдентификатор();
	МенЗап.ДатаДобавления = ТекущаяДатаСеанса();
	МенЗап.СодержимоеФайла = Новый ХранилищеЗначения(ИндексИзИмениФайла);
	МенЗап.Записать();
...
Функция ОпределитьИмяФайлаВыгрузкиФССПоТекстуВыгрузки(ТекстВыгрузки)
	...
	Элементы_insurerInfo = ДеревоXML.Строки.НайтиСтроки(Новый Структура("Имя, Тип", "insurerInfo", "Э"), Истина);
	Если ЭлементыTITLE.Количество() = 0 И Элементы_insurerInfo.Количество() = 0 Тогда
	...
	ИначеЕсли Элементы_insurerInfo.Количество() > 0 Тогда
		
		Возврат "POVD_" + Формат(ТекущаяДатаСеанса(), "ДФ=""ггггММдд""") + "_" + Новый УникальныйИдентификатор + ".xml";
...
Процедура ПроверитьОтчеты(СообщенияОбОшибках = Неопределено)
	...
	//выгрузка в формате ЕФС ПФР
	Если ОтборЕФСПФР.Количество() <> 0 Тогда
		...
		Для Каждого Стр Из ОтборЕФСПФР Цикл
			...
			Иначе
				Если НЕ РезультатПроверки Тогда
					...
					Для Каждого СтрСообщения Из ТекФорма.ТаблицаСообщений Цикл
						НовСтр = ТаблицаСообщений.Добавить();
						ЗаполняемыеСвойства = "Раздел, Страница, Строка, СтрокаПП, ИмяЯчейки, Графа, Описание";
						ЗаполнитьЗначенияСвойств(НовСтр, СтрСообщения, ЗаполняемыеСвойства);
						Если СтрСообщения.Свойство("ОтчетДок") И ЗначениеЗаполнено(СтрСообщения.ОтчетДок) Тогда
							НовСтр.ОтчетДок = СтрСообщения.ОтчетДок;
							НовСтр.Отчет = СтрСообщения.Отчет;
						ИНаче
							НовСтр.ОтчетДок = Стр.Док;
							НовСтр.Отчет = Строка(РегламентированнаяОтчетностьКлиентСервер.ПредставлениеДокументаРеглОтч(Стр));
						КонецЕсли;
	...
	//*****************************************
	//выгрузка в формате Минобороны
	Если ОтборМинобороны.Количество() <> 0 Тогда
...
&НаКлиенте
Процедура СообщитьОбОбработкеОтчета(Отчет, НомерОтчета, КоличествоОтчетов)

	Текст 				= НСтр("ru='Обработка отчета %1 (%2 из %3) ...'");
	ПредставлениеОтчета = РегламентированнаяОтчетностьКлиентСервер.ПредставлениеДокументаРеглОтч(Отчет);
	
	Текст    = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(Текст, ПредставлениеОтчета, НомерОтчета, КоличествоОтчетов);
	Картинка = БиблиотекаКартинок.СохранитьСписок;
	
	РегламентированнаяОтчетностьКлиент.ВывестиСостояние(Текст, , , Картинка);
...
Процедура ПроверитьОтчетыСервер(ОшибкаВыгрузки)
	...
	Если РежимВызоваЭкспортируемогоМетодаФормы <> Истина Тогда
		НалоговыеОрганы.Очистить();
		Для Каждого Стр Из Основания Цикл
			Если Стр.Пометка И СокрЛП(Стр.КодНалоговогоОргана) <> "" Тогда // исключаем из проверки отчеты в ПФР с пустым кодом ИФНС
				НовСтр = НалоговыеОрганы.Добавить();
				НовСтр.КодНалоговогоОргана = Стр.КодНалоговогоОргана;
			КонецЕсли;
		КонецЦикла;
	...
	ОтчетыРасшифровкиБухОтчетности = Новый СписокЗначений;
	ОтчетыРасшифровкиБухОтчетности.Добавить("РасшифровкиБухгалтерскойОтчетности");
	...
	ОтчетыФСНБР = РегламентированнаяОтчетностьПовтИсп.ПолучитьОтчетыФСНБанкРоссии();
	ОтчетыМинобороны = РегламентированнаяОтчетностьПовтИсп.ПолучитьОтчетыМинобороны();
	ОтчетыБанкРоссии = РегламентированнаяОтчетностьПовтИсп.ПолучитьОтчетыБанкРоссии();
	...
	Для Каждого Стр Из ВерсииИОтчеты Цикл
		...
		Если ОтчетыСПолнойВнутреннейПроверкой.Найти(ИсточникОтчета) <> Неопределено
		 ИЛИ ОтчетыАлко.НайтиПоЗначению(ИсточникОтчета) <> Неопределено
		 ИЛИ ОтчетыФСГС.НайтиПоЗначению(ИсточникОтчета) <> Неопределено
		 ИЛИ ОтчетыФСНБР.НайтиПоЗначению(ИсточникОтчета) <> Неопределено
		 ИЛИ ОтчетыРПН.НайтиПоЗначению(ИсточникОтчета) <> Неопределено
		 ИЛИ ОтчетыЗаявления.НайтиПоЗначению(ИсточникОтчета) <> Неопределено
		 ИЛИ ОтчетыРасшифровкиБухОтчетности.НайтиПоЗначению(ИсточникОтчета) <> Неопределено Тогда
			Продолжить;
		КонецЕсли;
...
Процедура ПоказатьТекстВыгрузкиДляВыбранногоЭлемента(Выгрузка)
	...
	Если Выгрузка.Тип = ВерсииФорматовВыгрузки.ПеречислениеВерсииФорматовВыгрузкиВерсия400
	 ...
	 ИЛИ Выгрузка.Тип = ВерсииФорматовВыгрузки.ПеречислениеВерсииФорматовВыгрузкиВерсия700ПФР Тогда
		
		ТипФайла = "windows-1251";
		
	ИначеЕсли Выгрузка.Тип = ВерсииФорматовВыгрузки.ПеречислениеВерсииФорматовВыгрузкиВерсияФСС Тогда
		
		ТипФайла = ?(ВРег(Лев(Выгрузка.ИмяФайла, 4)) = "POVD", "utf-8", "windows-1251");
		
	ИначеЕсли Выгрузка.Тип = ВерсииФорматовВыгрузки.ПеречислениеВерсииФорматовВыгрузкиВерсияПФР
		ИЛИ Выгрузка.Тип = ВерсииФорматовВыгрузки.ПеречислениеВерсииФорматовВыгрузкиВерсияЕФСПФР Тогда
	...
	Если ВРег(СокрЛП(СтрПолучитьСтроку(Выгрузка.Текст, 1))) = ВРег(СокрЛП("Структура")) Тогда
		...
		КоличествоСтрок = СтрЧислоСтрок(Выгрузка.Текст);
	
		Для НомерСтроки = 1 По КоличествоСтрок Цикл
			
			Если СтрЧислоВхождений(СтрПолучитьСтроку(Выгрузка.Текст, НомерСтроки), "АдресФайлаВыгрузки:") > 0 Тогда
		
				АдресФайлаВыгрузки = СтрЗаменить(СтрПолучитьСтроку(Выгрузка.Текст, НомерСтроки), "АдресФайлаВыгрузки:", "");
...
Процедура ГодПриИзмененииЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	Если Результат Тогда
		Объект.ПериодПо = КонецМесяца(Дата(?(ЗначениеЗаполнено(Год), Год, 0001),
									  	   Месяц(Объект.ПериодПо), 1));
	Иначе
		Год = Год(Объект.ПериодПо);
	КонецЕсли;
...
Процедура ПриИзмененииМесяца()
	...
	Объект.ПериодПо = КонецМесяца(Дата(?(ЗначениеЗаполнено(Год), Год, 0001),
									   ?(ЗначениеЗаполнено(Месяц), Месяц, 1),
									   1));
...
Процедура ПослеЗакрытияВопроса(Результат, Параметры) Экспорт
	...
	ПолучитьФайл(Параметры.АдресФайлаВыгрузки, Параметры.ИмяКонечногоФайла, Истина);
	
КонецПроцедуры

&НаКлиентеНаСервереБезКонтекста
Функция ОтчетыСПолнойВнутреннейПроверкой()
	
	ОтчетыСВнутреннейПроверкой = Новый Массив;
	ОтчетыСВнутреннейПроверкой.Добавить("РегламентированныйОтчетБухОтчетность");
	ОтчетыСВнутреннейПроверкой.Добавить("РегламентированныйОтчетБухОтчетностьМП");
	
	Возврат ОтчетыСВнутреннейПроверкой;
