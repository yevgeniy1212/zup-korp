...
Функция ПечатнаяФормаХодатайстваОБронированииВСоответствииСПеречнем(МассивОбъектов, ОбъектыПечати)
	...
	Пока ВыборкаПоДокументам.Следующий() Цикл
		...
		Макет = УправлениеПечатью.МакетПечатнойФормы("Документ.ХодатайствоОБронированииГражданПребывающихВЗапасе.ПФ_MXL_ХодатайствоОБронированииВСоответствииСПеречнем2017");
		
		ИспользоватьФорму2017 = ВыборкаПоДокументам.ДатаДок >= '20170801';
		Если Не ИспользоватьФорму2017 Тогда
		...
		Пока ВыборкаПоСтрокам.НайтиСледующий(СтруктураПоиска) Цикл 
			...
			Если ИспользоватьФорму2017 Тогда 
				ДанныеДолжности = ВыборкаПоСтрокам.КодВЭД + Символы.ПС + ВыборкаПоСтрокам.КодДолжности;
				Если Не ЗначениеЗаполнено(ВыборкаПоСтрокам.КодВЭД) И Не ЗначениеЗаполнено(ВыборкаПоСтрокам.КодДолжности) Тогда 
					ДанныеДолжности = ВыборкаПоСтрокам.ОписаниеПунктаПеречня;
				КонецЕсли;
				СтрокаТаблицы.Параметры.ДанныеДолжности = ДанныеДолжности;
			КонецЕсли;
...
Функция ПечатнаяФормаХодатайстваОбИндивидуальномБронировании(МассивОбъектов, ОбъектыПечати)
	...
	Пока ВыборкаПоДокументам.Следующий() Цикл
		...
		Макет = УправлениеПечатью.МакетПечатнойФормы("Документ.ХодатайствоОБронированииГражданПребывающихВЗапасе.ПФ_MXL_ХодатайствоОбИндивидуальномБронировании2017");
		...
		Пока ВыборкаПоСтрокам.НайтиСледующий(СтруктураПоиска) Цикл 
			...
			Если ИспользоватьФорму2017 Тогда 
				...
				Если Не ЗначениеЗаполнено(ВыборкаПоСтрокам.КодВЭД) И Не ЗначениеЗаполнено(ВыборкаПоСтрокам.КодДолжности) Тогда 
					ДанныеДолжности = ВыборкаПоСтрокам.ОписаниеПунктаПеречня;
				КонецЕсли;
				Если Не ЗначениеЗаполнено(ВыборкаПоСтрокам.ПунктПеречня) Тогда 
					ДанныеДолжности = НСтр("ru = 'в Перечне нет'");
				КонецЕсли;
				СтрокаТаблицы.Параметры.ДанныеДолжности = ДанныеДолжности;
				Если Не ЗначениеЗаполнено(ВыборкаПоСтрокам.НомерКомандыПартии) Тогда 
					СтрокаТаблицы.Параметры.НомерКомандыПартии = "-";
				КонецЕсли;
			КонецЕсли;
...
Функция ДанныеДляПечатиДокументаХодатайствоОБронировании(МассивСсылок)
	...
	|	ЕСТЬNULL(ХодатайствоОБронированииСотрудники.ПунктПеречня.КодВЭД, """") КАК КодВЭД,
	|	ЕСТЬNULL(ХодатайствоОБронированииСотрудники.ПунктПеречня.КодДолжности, """") КАК КодДолжности,
	|	ЕСТЬNULL(ХодатайствоОБронированииСотрудники.ПунктПеречня.ОписаниеПункта, """") КАК ОписаниеПунктаПеречня,
