...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Параметры.Ключ.Пустая() Тогда
		...
		ЗаполнитьНаОснованииДоверенности(Параметры.ДокументОснование);
	КонецЕсли;
	
	Если Параметры.Свойство("ЗагрузитьДокументИзФайлаОбмена") И Параметры.ЗагрузитьДокументИзФайлаОбмена Тогда
		Записать();
		РезультатЗагрузки = ДокументооборотСКОВызовСервера.ЗагрузитьУПРУП(
...
Процедура ПослеЗаписи(ПараметрыЗаписи)
	...
	Оповестить("Запись_УведомлениеОПрекращенииПолномочийПредставителя", , Объект.Ссылка);
...
Процедура КомандаОтправить(Команда)
	...
	РезультатыПроверки = ДокументооборотСКОВызовСервера.ПроверитьУПРУП(Объект);
...
Процедура КомандаОтправить_Завершение(Результат, ВходящийКонтекст) Экспорт
	...
	КонтекстЭДОКлиент.ОповеститьОбОтправкеУведомленияОПрекращенииПолномочийПредставителя(Объект.Ссылка, Объект.Организация);
...
Процедура ИнициализироватьДанные(Параметры)
	...
	ЧерныйЦвет  = ЦветаСтиля.ЦветТекстаФормы;
	СерыйЦвет   = ЦветаСтиля.ЦветТекстаКнопки;
	СинийЦвет   = ЦветаСтиля.ЦветГиперссылкиБРО;
...
Процедура УправлениеФормой(Форма)
	...
	Форма.Элементы.ДокументОснование.ТолькоПросмотр 				= ТолькоПросмотрФормы;
...
Процедура ВвестиРеквизитыСтрахователя()
	...
	ОткрытьФорму(
		"Документ.УведомлениеОПрекращенииПолномочийПредставителя.Форма.ФормаВводаРеквизитов",
...
Процедура ВвестиРеквизитыСотрудникаСтрахователя()
	...
	ОткрытьФорму(
		"Документ.УведомлениеОПрекращенииПолномочийПредставителя.Форма.ФормаВводаРеквизитовФЛ",
		Новый Структура("СтруктураДанных", СтруктураДанных),
		ЭтаФорма,,,,
		ОписаниеОповещения,
		РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
...
Процедура ВвестиРеквизитыПредставителя()
	...
	Если РеквизитыПредставителяФЛ.Количество() > 0 И РеквизитыПредставителя.Количество() = 0 Тогда
	...
	НаименованиеФормы = "Документ.УведомлениеОПрекращенииПолномочийПредставителя.Форма.ФормаВводаРеквизитов";
	...
	ИначеЕсли Представитель_ФизическоеЛицо Тогда
		...
		НаименованиеФормы = "Документ.УведомлениеОПрекращенииПолномочийПредставителя.Форма.ФормаВводаРеквизитовФЛ";
...
Процедура ДокументОснованиеПриИзмененииНаСервере()
	...
	Реквизиты = Метаданные.Документы.УведомлениеОПрекращенииПолномочийПредставителя.Реквизиты;
	Для Каждого Реквизит Из Реквизиты Цикл
		Если Реквизит.Имя = "Организация" 
			ИЛИ Реквизит.Имя = "ДокументОснование"
	...
	ЗаполнитьНаОснованииДоверенности(Объект.ДокументОснование);
...
&НаСервере
Процедура ЗаполнитьНаОснованииДоверенности(ДокументОснование)
	
	Если ЗначениеЗаполнено(ДокументОснование) Тогда
		ДокОбъект = РеквизитФормыВЗначение("Объект");
		
		// заполнить общие данные
		ЗаполнитьЗначенияСвойств(ДокОбъект, ДокументОснование, , "Номер");
		
		// заполнить данные ФИО
		ДокОбъект.ФИО.Загрузить(ДокументОснование.ФИО.Выгрузить());
		
		// заполнить контактная информация
		ДокОбъект.КонтактнаяИнформация.Загрузить(ДокументОснование.КонтактнаяИнформация.Выгрузить());
		
		// заполнить реквизиты субъектов
		ДокОбъект.РеквизитыСубъектов.Загрузить(ДокументОснование.РеквизитыСубъектов.Выгрузить());
		
		Если НЕ ЗначениеЗаполнено(ДокОбъект.ДатаОтзыва) Тогда
			ДокОбъект.ДокументОснование = ДокументОснование;
		КонецЕсли;
		ДокОбъект.ДатаОтзыва = ТекущаяДата();
