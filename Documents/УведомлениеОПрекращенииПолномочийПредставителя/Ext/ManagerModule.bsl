...
Процедура ДобавитьКомандыПечати(КомандыПечати) Экспорт
	...
	КомандаПечати.Идентификатор = "УведомлениеОПрекращенииПолномочийПредставителя";
...
Функция ДанныеИзФайлаОбмена(
	...
	Если СведенияСтрахователя.Количество() = 1 Тогда
		...
		ИначеЕсли ДанныеУведомления.ТипОрганизации = "ФЛ" Тогда
			...
			СведенияСтрахователяФизЛицоФИО = ПолучитьВыборкуЭлементовDOM(
				ДокументОбработки,
				"/ЭДПФР/УПРУП/Страхователь/ИП/УТ2:ФИО");
	...
	СведенияПредставителяФизЛицо = ПолучитьВыборкуЭлементовDOM(
		ДокументОбработки, "/ЭДПФР/УПРУП/УполномоченныйПредставитель/ПредставительФизЛицо");
	СведенияПредставителяЮЛ = ПолучитьВыборкуЭлементовDOM(
		ДокументОбработки, "/ЭДПФР/УПРУП/УполномоченныйПредставитель/ПредставительЮЛ");
	СведенияПредставителяИП = ПолучитьВыборкуЭлементовDOM(
		ДокументОбработки, "/ЭДПФР/УПРУП/УполномоченныйПредставитель/ПредставительИП");
	...
	Если СведенияПредставителяФизЛицо.Количество() = 1 Тогда
		...
		СведенияПредставителяФизЛицоФИО = ПолучитьВыборкуЭлементовDOM(
			ДокументОбработки,
			"/ЭДПФР/УПРУП/УполномоченныйПредставитель/ПредставительФизЛицо/УТ2:ФИО");
	...
	ИначеЕсли СведенияПредставителяИП.Количество() = 1 Тогда
		...
		СведенияПредставителяФизЛицоФИО = ПолучитьВыборкуЭлементовDOM(
			ДокументОбработки,
			"/ЭДПФР/УПРУП/УполномоченныйПредставитель/ПредставительИП/УТ2:ФИО");
		ЭлементВыборкиФИО = СведенияПредставителяФизЛицоФИО[0];
	...
	СведенияПредставителяСотрудника = ПолучитьВыборкуЭлементовDOM(
		ДокументОбработки, "/ЭДПФР/УПРУП/ПредставительСотрудник");
	...
	Если СведенияПредставителяСотрудника.Количество() = 1 Тогда
		...
		СведенияПредставителяСотрудникаФИО = ПолучитьВыборкуЭлементовDOM(
			ДокументОбработки, "/ЭДПФР/УПРУП/ПредставительСотрудник/УТ2:ФИО");
...
Функция ВыгрузитьУведомление(Уведомление) Экспорт
	...
	Возврат КонтекстЭДОСервер.ВыгрузитьУведомлениеОПрекращенииПолномочийПредставителя(Уведомление);
...
Функция ПечатнаяФорма(Уведомление) Экспорт
	...
	Если ДействуетФорматАФ245д Тогда
		Бланк = Документы.УведомлениеОПрекращенииПолномочийПредставителя.ПолучитьМакет("УПРУПАФ_2_45д");
	КонецЕсли;
...
Процедура Печать(МассивОбъектов, ПараметрыПечати, КоллекцияПечатныхФорм, ОбъектыПечати, ПараметрыВывода) Экспорт
	...
	Для каждого СтрокаМассиваОбъектов Из МассивОбъектов Цикл
		УправлениеПечатью.ВывестиТабличныйДокументВКоллекцию(
			...
			НСтр("ru='Уведомление о прекращении полномочий представителя'"),
...
Функция ПроверитьВозможностьВыгрузки(ОбъектИлиСсылка) Экспорт
	...
	Если ТипЗнч(ОбъектИлиСсылка) = Тип("ДокументСсылка.УведомлениеОПрекращенииПолномочийПредставителя") Тогда
...
Процедура ОбработкаПолученияФормы(ВидФормы, Параметры, ВыбраннаяФорма, ДополнительнаяИнформация, СтандартнаяОбработка)
	...
	КонтекстЭДО.ОбработкаПолученияФормы("Документ", "УведомлениеОПрекращенииПолномочийПредставителя", ВидФормы, Параметры, ВыбраннаяФорма, ДополнительнаяИнформация, СтандартнаяОбработка);
...
Процедура ЗаполнитьШапку(Уведомление, ТабДокумент, Бланк)
	...
	ОбластьШапка.Параметры.Заголовок = НСтр("ru = 'Уведомление о прекращении полномочий представителя'");
...
Функция НайтиДокументИБ(ЗначениеПоУмолчанию, ФильтрыПоиска)
	...
	|ИЗ
	|	Документ.УведомлениеОПрекращенииПолномочийПредставителя КАК ОбъектПоиска
	|ГДЕ
	...
	ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "Документ.УведомлениеОПрекращенииПолномочийПредставителя", МетаданныеОбъекта.ПолноеИмя());
...
Функция ЗаписатьДокумент(ДанныеУведомления, ОбновлятьСуществующийИлиСсылкаИлиРежим)
	...
	Если ТипЗнч(ОбновлятьСуществующийИлиСсылкаИлиРежим) = Тип("ДокументСсылка.УведомлениеОПрекращенииПолномочийПредставителя") Тогда
	...
	Иначе
		Если ОбновлятьСуществующийИлиСсылкаИлиРежим = Истина
			...
			НашлиДокумент = НайтиДокументИБ(Документы.УведомлениеОПрекращенииПолномочийПредставителя.ПустаяСсылка(), СтрокаПоиска);
	...
	Если НЕ ЗначениеЗаполнено(НашлиДокумент) Тогда
		ОбъектДокумента = Документы.УведомлениеОПрекращенииПолномочийПредставителя.СоздатьДокумент();
	Иначе
	...
	Если ОбъектДокумента.ПроверитьЗаполнение() Тогда
		...
		ЭлементБлокировки = Блокировка.Добавить("Документ.УведомлениеОПрекращенииПолномочийПредставителя");
