...
Процедура ОтсутствующийСотрудникПриИзмененииНаСервере()
	...
	КадровыйУчетРасширенный.ДополнитьИменаКадровыхДанных(ИменаКадровыхДанных);
	...
	Если КадровыеДанныеОтсутствующегоСотрудника.Количество()>0 Тогда
		...
		Если ОбщегоНазначения.ПодсистемаСуществует("ЗарплатаКадрыКорпоративнаяПодсистемы.ОхранаТруда") Тогда
			МодульОхранаТрудаФормы = ОбщегоНазначения.ОбщийМодуль("ОхранаТрудаФормы");
			МодульОхранаТрудаФормы.УстановитьТекущееРабочееМесто(Объект, КадровыеДанные, "РабочееМесто");
		КонецЕсли;
