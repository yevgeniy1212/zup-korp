...
// 				 
// Параметры:
//  ДокументОбъект - ДокументОбъект.ПриказНаДоплатуДоСреднегоЗаработка, ДанныеФормыСтруктура - объект или данные формы, 
...
Процедура ЗаполнитьДатыЗапрета(ПараметрыОбновления) Экспорт
	...
	Запрос.Текст =
		"ВЫБРАТЬ ПЕРВЫЕ 100
		|	ПриказНаДоплатуДоСреднегоЗаработка.Ссылка КАК Ссылка,
		|	ПриказНаДоплатуДоСреднегоЗаработка.Дата КАК Дата
		|ИЗ
		|	Документ.ПриказНаДоплатуДоСреднегоЗаработка КАК ПриказНаДоплатуДоСреднегоЗаработка
		|ГДЕ
		|	ПриказНаДоплатуДоСреднегоЗаработка.ДатаЗапрета = ДАТАВРЕМЯ(1, 1, 1)
		|
		|УПОРЯДОЧИТЬ ПО
		|	ПриказНаДоплатуДоСреднегоЗаработка.Дата УБЫВ";
