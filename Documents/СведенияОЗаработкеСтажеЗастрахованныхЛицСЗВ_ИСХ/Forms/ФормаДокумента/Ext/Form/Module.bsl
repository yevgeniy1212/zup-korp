...
Процедура ОтчетныйПериодРегулирование(Элемент, Направление, СтандартнаяОбработка)
	...
	ПерсонифицированныйУчетКлиент.ПериодОтчетаРегулирование(Объект.ОтчетныйПериод, Объект.ОкончаниеОтчетногоПериода, ПериодСтрока, Направление, , , '20161001', '20161231');
...
Процедура ОрганизацияПриИзмененииНаСервере()
	...
	Запрос.УстановитьПараметр("Организация", УчетСтраховыхВзносов.ОтчитывающаясяПоВзносамОрганизация(Объект.Организация, Объект.Дата));
...
Процедура ЗаполнитьДанныеОСтажеСотрудников(ИдентификаторыСотрудников)
	...
	Пока Выборка.СледующийПоЗначениюПоля("Сотрудник") Цикл 
		...
		Пока Выборка.Следующий() Цикл
			...
			ПерсонифицированныйУчет.ОбработатьПериодПовышенияКвалификации(Объект.ЗаписиОСтаже, НоваяСтрока, ДатаПостановления322);
		КонецЦикла;
		ПерсонифицированныйУчетКлиентСервер.ВыполнитьНумерациюЗаписейОСтаже(СтрокиСтажаПоСотруднику);
	КонецЦикла;
