...
Процедура ПроверитьДанныеДокумента(Отказ = Ложь) Экспорт 
	...
	Если ОтчетныйПериод >= '20140101' Тогда 
		ПроверяемыйПериод = ОтчетныйПериод;
		ПерсонифицированныйУчет.ПроверитьЗаписиОСтаже(МенеджерВременныхТаблиц, Ссылка, ПроверяемыйПериод, Отказ, Истина, Истина, Истина, КонецДня(ОкончаниеОтчетногоПериода));
	КонецЕсли;
