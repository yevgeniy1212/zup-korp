...
Процедура ДобавитьКомандыПечати(КомандыПечати) Экспорт
	...
	КомандаПечати.Обработчик = "УправлениеПечатьюБЗККлиент.ВыполнитьКомандуПечати";
	КомандаПечати.МенеджерПечати = "Документ.РегистрацияПереработок";
	КомандаПечати.Идентификатор = "ПФ_MXL_ПриказНаОплатуПереработок_ПоПравиламВеденияДО";
	КомандаПечати.Представление = НСтр("ru = 'Приказ на оплату (По утвержденным правилам ДО)'");
...
Процедура Печать(МассивОбъектов, ПараметрыПечати, КоллекцияПечатныхФорм, ОбъектыПечати, ПараметрыВывода) Экспорт
	...
	// ЗУПК
	Если УправлениеПечатью.НужноПечататьМакет(КоллекцияПечатныхФорм, "ПФ_MXL_ПриказНаОплатуПереработок_ПоПравиламВеденияДО") Тогда
		УправлениеПечатью.ВывестиТабличныйДокументВКоллекцию(
						КоллекцияПечатныхФорм,
						"ПФ_MXL_ПриказНаОплатуПереработок_ПоПравиламВеденияДО",
						("ru = 'Приказ на оплату переработок'"),
						ПечатнаяФормаПриказаНаОплатуПереработокПоПравиламВеденияДО(МассивОбъектов, ОбъектыПечати), ,
						"Документ.РегистрацияПереработок.ПФ_MXL_ПриказНаОплатуПереработок_ПоПравиламВеденияДО");
	КонецЕсли;
	// ЗУПК
...
// ЗУПК
Функция ПечатнаяФормаПриказаНаОплатуПереработокПоПравиламВеденияДО(МассивОбъектов, ОбъектыПечати)
	...
	Макет = УправлениеПечатью.МакетПечатнойФормы("Документ.РегистрацияПереработок.ПФ_MXL_ПриказНаОплатуПереработок_ПоПравиламВеденияДО");
	
	ОбластьШапка 	  = Макет.ПолучитьОбласть("Шапка");
	ОбластьРаботник   = Макет.ПолучитьОбласть("Работник");
	ОбластьПодвал 	  = Макет.ПолучитьОбласть("Подвал");
	ОбластьПодвалПодписи 		  = Макет.ПолучитьОбласть("ПодвалПодписи");
	ОбластьПодвалПодписиСотрудник = Макет.ПолучитьОбласть("ПодвалПодписиСотрудник");
	...
	Пока Выборка.СледующийПоЗначениюПоля("ДокументСсылка") Цикл  
		...
		Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(Выборка.НазваниеОрганизации, "НаименованиеПолное") Тогда
			ФормированиеПечатныхФормПоПравиламВеденияДО.ЗаполнениеРеквизитовШапкиЧерезМассивОбъектов(ОбластьШапка, МассивОбъектов, "РегистрацияПереработок", Выборка.НазваниеОрганизации.НаименованиеПолное);
		КонецЕсли;
		
		ОбластьШапка.Параметры.ДатаДокумента = Выборка.ДатаДок;
		ОбластьШапка.Параметры.РегистрационныйНомерДокумента = Выборка.НомерДок;
		...
		// +++ УПК
		Организация = Выборка.НазваниеОрганизации.Ссылка;
		...
		ТабДокумент.Вывести(ОбластьПодвалПодписи);
		
		ВыборкаСотрудники = РезультатЗапроса.Выбрать();
		Пока ВыборкаСотрудники.Следующий() Цикл 
			
			ОбластьПодвалПодписиСотрудник.Параметры.Сотрудник = ВыборкаСотрудники.ФамилияИО;
			ТабДокумент.Вывести(ОбластьПодвалПодписиСотрудник);
...
Функция ДобавитьКомандыСозданияДокументов(КомандыСозданияДокументов, ДополнительныеПараметры) Экспорт
	...
	ЗарплатаКадрыРасширенный.ДобавитьВКоллекциюКомандуСозданияДокументаПоМетаданнымДокумента(
		КомандыСозданияДокументов, Метаданные.Документы.РегистрацияПереработок);
