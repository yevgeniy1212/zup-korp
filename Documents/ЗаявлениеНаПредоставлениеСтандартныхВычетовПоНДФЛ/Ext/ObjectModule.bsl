	...
	Если ВычетыНаДетейДействуетДоПР <> Неопределено Тогда
		Для Каждого ВычетНаДетей Из ВычетыНаДетей Цикл
			Если Не ЗначениеЗаполнено(ВычетНаДетей.ДействуетДо) Тогда
				...
				ТекстСообщения = СтрШаблон(ТекстСообщения, ВычетНаДетей.НомерСтроки);
...
Функция ДанныеДляПроведения()
	...
	ДанныеОВычетах.ДокументПодтверждающийПравоНаЛичныйВычет = СтрШаблон(
...
Функция ЗаявлениеИДокументыПодтверждающиеПраваНаВычеты(КодВычета, НачалоПериода, ОкончаниеПериода)
	...
	ВозвращаемоеЗначение = СтрШаблон(
			НСтр("ru = 'Заявление от %1 №%2. %3.'"),
			Формат(Дата, "ДЛФ=D"), 
			Символы.НПП + Номер, 
...
Процедура ЗаполнитьПоПараметрамЗаполнения(ДанныеЗаполнения)
	...
	Если ДанныеЗаполнения.Свойство("Вычеты") Тогда
		ИзменитьВычетыНаДетей = Истина;
		Для каждого Вычет Из ДанныеЗаполнения.Вычеты Цикл
			НовыйВычет = ВычетыНаДетей.Добавить();
			ЗаполнитьЗначенияСвойств(НовыйВычет, Вычет);
		КонецЦикла;
	КонецЕсли;
КонецПроцедуры
