...
&НаКлиенте
Процедура ОтобразитьДанныеВнешнихСистем()
	
	Если Не ОтображатьСобытияВнешнихСистем Тогда
		Возврат;
	КонецЕсли;
	
	ДлительнаяОперация = ДлительнаяОперацияПолученияДоступности();
	Если ДлительнаяОперация = Неопределено Тогда
		Возврат;
	КонецЕсли;
	
	Оповещение = Новый ОписаниеОповещения("ПолучитьЗанятостьЗавершение", ЭтотОбъект);
...
&НаСервере
Функция ДлительнаяОперацияПолученияДоступности()
	
	ОтображаемыеПомещения = ОтображаемыеПомещенияИзПланировщика();
	
	Если Не ЗначениеЗаполнено(ОтображаемыеПомещения) Тогда
		Возврат Неопределено;
	КонецЕсли;
	
	Возврат СинхронизацияКалендарей.ДлительнаяОперацияПолученияДоступностиПомещений(
		УникальныйИдентификатор,
		ОтображаемыеПомещения,
		НачалоДня(ОтображаемаяДата),
		КонецДня(ОтображаемаяДата));
	
КонецФункции

&НаКлиенте
Процедура ПолучитьЗанятостьЗавершение(Результат, ДополнительныеПараметры = Неопределено) Экспорт
	
	Если Не ЗначениеЗаполнено(Результат) Тогда
		Возврат;
	КонецЕсли;
	
	ОтобразитьДанныеВнешнихСистемСервер(Результат.АдресРезультата);
	
КонецПроцедуры

&НаСервере
Процедура ОтобразитьДанныеВнешнихСистемСервер(АдресДанныхОДоступностиПомещений)
	
	ДоступностьПомещений = ПолучитьИзВременногоХранилища(АдресДанныхОДоступностиПомещений);
	
	Помещения = ОтображаемыеПомещенияИзПланировщика();
	
	БронированиеПомещений.ОтобразитьДанныеВнешнихСистем(Планировщик, ДоступностьПомещений, Помещения,
		НачалоДня(ОтображаемаяДата), КонецДня(ОтображаемаяДата));
	
КонецПроцедуры

&НаСервере
Функция ОтображаемыеПомещенияИзПланировщика()
	
	Результат = Новый Массив;
	
	Измерение = Планировщик.Измерения.Найти("Помещение");
	Если Измерение = Неопределено Тогда
		Возврат Результат;
	КонецЕсли;
	
	Для Каждого ЭлементИзмерения Из Измерение.Элементы Цикл
		Результат.Добавить(ЭлементИзмерения.Значение);
	КонецЦикла;
