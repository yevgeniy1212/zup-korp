...
Функция ПолучитьДанныеДляПроведения(Ссылка)Экспорт
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("ЗарплатаКадрыКорпоративнаяПодсистемы.ФинансовыйУчет") Тогда
		...
		МодульФинансовыйУчет.ДополнитьДанныеДляПроведенияОбученияРазвитияСотрудников(Ссылка, ДанныеДляПроведения);
	КонецЕсли;
...
Функция СотрудникиОбучения(ДокументСсылка) Экспорт
	...
	Запрос.Текст = 
		...
		|	КомментарийКОценке
		|ИЗ
		|	Документ.ОбучениеРазвитиеСотрудников.Сотрудники
		|ГДЕ
		|	Ссылка = &Ссылка";
