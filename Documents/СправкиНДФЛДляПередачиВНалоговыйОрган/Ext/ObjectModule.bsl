...
Функция ТекстВыходногоФайла2011(ИмяФайла)
	...
	Для каждого СтрокаДанных Из Сотрудники Цикл 
		...
		Если ЗначениеЗаполнено(СтрокаДанных.Адрес) Тогда
			...
			СтруктураАдреса = КонтактнаяИнформацияБЗК.СвернутьСведенияОбАдресе(
				КонтактнаяИнформацияБЗК.СведенияОбАдресеСТипами(СтрокаДанных.Адрес), ИменаПолей);
		...
		Если ЗначениеЗаполнено(СтрокаДанных.АдресЗарубежом) Тогда
			СтруктураАдреса = РаботаСАдресами.СведенияОбАдресе(СтрокаДанных.АдресЗарубежом);
