...
Процедура СотрудникиПриОкончанииРедактированияНаСервере(ИдентификаторСтроки)
	...
	СтрокиСотрудников = ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(СтрокаСотрудника);
	ЗаполнитьТекущиеЗначенияСовокупныхТарифныхСтавокСотрудников(СтрокиСотрудников);
	РассчитатьКоэффициентИндексацииЗаработка(СтрокиСотрудников);
...
Процедура СотрудникиОбработкаВыбораНаСервере(ВыбранноеЗначение)
	...
	ЗаполнитьЗначенияСовокупныхТарифныхСтавокСотрудников(ФОТПлановыхНачисленийСотрудников.ТарифныеСтавки);
	ЗаполнитьТекущиеЗначенияСовокупныхТарифныхСтавокСотрудников(ДобавленныеСтроки);
	РассчитатьКоэффициентИндексацииЗаработка(ДобавленныеСтроки);
...
Процедура ДанныеВРеквизит()
	...
	ДанныеСотрудниковВРеквизитФормы();
	ФОТСотрудниковВРеквизитФормы();
	ЗаполнитьТекущиеЗначенияСовокупныхТарифныхСтавокСотрудников();
...
Функция НовыйЭлементФормыИзменяемыйПоказатель(ПутьКДанным, Группа, Заголовок)
	...
	Если ЭлементПоказатель = Неопределено Тогда
		...
		ЭлементПоказатель = Элементы.Добавить("ПоказателиСотрудников" + ПутьКДанным, Тип("ПолеФормы"), Группа);
...
Процедура ПоказателиСотрудникаВСтроку(СтрокаСотрудника, ПоказателиСотрудника)
	...
	Для каждого ПоказательСотрудника Из ПоказателиСотрудника Цикл
		ОписаниеИзменяемогоПоказателя = ОписаниеПоказателей.Получить(ПоказательСотрудника.Показатель);
...
Процедура ЗаполнитьДанныеСотрудникаИзВременногоХранилища(АдресВХранилище)
	...
	Если ДанныеИзХранилища <> Неопределено Тогда
		...
		РассчитатьКоэффициентИндексацииЗаработка(ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(СтрокаСотрудника));
...
&НаСервере
Процедура ЗаполнитьТекущиеЗначенияСовокупныхТарифныхСтавокСотрудников(СтрокиСотрудников = Неопределено)
	...
	Для Каждого ДанныеОплаты Из ТекущиеДанныеОплатыТруда Цикл
		Отбор = Новый Структура("Сотрудник,ВремяРегистрации", ДанныеОплаты.Сотрудник, ДанныеОплаты.Период);
		СтрокиСотрудника = Объект.Сотрудники.НайтиСтроки(Отбор);
		Для Каждого СтрокаСотрудника Из СтрокиСотрудника Цикл 
			СтрокаСотрудника.СовокупнаяТарифнаяСтавкаТекущееЗначение = ДанныеОплаты.СовокупнаяТарифнаяСтавка;
		КонецЦикла;
	КонецЦикла;
...
Процедура УчитыватьКакИндексациюЗаработкаПриИзмененииНаСервере()
	...
	Для Каждого СтрокаСотрудника Из Объект.Сотрудники Цикл 
		СтрокаСотрудника.КоэффициентИндексации = 0;
	КонецЦикла;
...
Процедура УстановитьВидимостьКоэффициентаИндексации()
	...
	ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "СотрудникиКоэффициентИндексации", "Видимость", Объект.УчитыватьКакИндексациюЗаработка);
	
КонецПроцедуры

&НаСервере
Процедура РассчитатьКоэффициентИндексацииЗаработка(СтрокиСотрудников = Неопределено)
