	...
	Для каждого СтрокаСотрудника Из Объект.Сотрудники Цикл
		...
		Если ДанныеСотрудника <> Неопределено Тогда
			...
			Если Не ЗначениеЗаполнено(ДанныеСотрудника.ПриказОПриеме)
				...
				НаборЗаписей.ДополнительныеСвойства.Вставить("ОбновитьПубликациюВСервисе");
...
Процедура СотрудникиВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	Если ЗначениеЗаполнено(ТекущиеДанные.Сотрудник) Тогда
		...
		Если Поле.Имя = "СотрудникиКоличествоСтавок" Тогда
			...
			КадровыйУчетКлиент.ИзменитьКоличествоСтавок(ЭтаФорма, "КоличествоСтавок", СтандартнаяОбработка, "КоличествоСтавокПредставление", Оповещение, ТекущиеДанные);
...
Процедура ОрганизацияПриИзмененииНаСервере()
	...
	Если ЗначениеЗаполнено(ОрганизацияПрежняя)
		И ОрганизацияПрежняя <> Объект.Организация Тогда
		
		Объект.Начисления.Очистить();
		Объект.Показатели.Очистить();
		Объект.ЕжегодныеОтпуска.Очистить();
		Объект.ОстаткиОтпусковПоРабочимГодам.Очистить();
		Объект.ОтсутствияСдвигающиеРабочийГод.Очистить();
	КонецЕсли;
...
Процедура ЗаполнитьПоДаннымДолжностиПоШтатномуРасписанию(ТекущиеДанныеСтрокиСотрудники, ДанныеДолжностиПоШтатномуРасписанию, ОчищатьСтарыеЗначений, ЗаполнитьТолькоНачисления = Ложь)
	...
	Если ПолучитьФункциональнуюОпциюФормы("ИспользоватьТарифныеСеткиПриРасчетеЗарплатыХозрасчет")
		...
		ТекущиеДанныеСтрокиСотрудники.РазрядКатегория = ДанныеДолжностиПоШтатномуРасписанию.РазрядКатегория;
...
Процедура ЗаполнитьДанныеТарифнойСетки(ТекущиеДанные, НачальноеЗаполнение = Ложь)
	...
	Если НачальноеЗаполнение Или ЗначениеЗаполнено(ТекущиеДанные.РазрядКатегория) Или ЗначениеЗаполнено(ТекущиеДанные.ПКУ) Тогда
		...
		РазрядКатегорияНадбавки = ТекущиеДанные.РазрядКатегория;
		Если ПолучитьФункциональнуюОпциюФормы("ИспользоватьШтатноеРасписание") Тогда
			...
			Если ЗначениеЗаполнено(ТекущиеДанные.ДолжностьПоШтатномуРасписанию) Тогда
				...
				РазрядКатегорияНадбавки = ДанныеПозиции.РазрядКатегорияНадбавки;
			...
			Иначе
				...
				РазрядКатегорияНадбавки = Справочники.РазрядыКатегорииДолжностей.ПустаяСсылка();
			КонецЕсли;
		...
		ЗначениеПоказателейТарифнойСеткиНадбавки = РазрядыКатегорииДолжностей.ЗначениеПоказателейТарифнойСетки(ТарифнаяСеткаНадбавки, РазрядКатегорияНадбавки, Объект.Месяц);
