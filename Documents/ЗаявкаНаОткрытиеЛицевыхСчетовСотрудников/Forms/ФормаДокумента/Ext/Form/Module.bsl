...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	ИначеЕсли ИмяСобытия = "ИзмененЗарплатныйПроект" И Параметр = Объект.ЗарплатныйПроект Тогда
		НастроитьФормуПоЗарплатномуПроекту();
	КонецЕсли;
...
Процедура ЛицевыеСчетаОбработкаВыбора(Элемент, ВыбранноеЗначение, СтандартнаяОбработка)
	...
	Для Каждого Сотрудник Из МассивСотрудников Цикл
		...
		НоваяСтрокаСотрудников.ФизическоеЛицо = Сотрудник;
		
	КонецЦикла;
	
	ЗаполнитьСтрокиНаСервере(МассивСотрудников);
...
Процедура ЛицевыеСчетаФизическоеЛицоПриИзменении(Элемент)
	...
	ЗаполнитьСтрокиНаСервере(ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(ТекущиеДанные.ФизическоеЛицо));
...
&НаКлиенте
Процедура ЛицевыеСчетаЭмбоссированныйТекст3ПриИзменении(Элемент)
	ЭмбоссированныйТекстПриИзменении(3, Элемент.ТекстРедактирования)
КонецПроцедуры

&НаКлиенте
Процедура ЛицевыеСчетаЭмбоссированныйТекст3ИзменениеТекстаРедактирования(Элемент, Текст, СтандартнаяОбработка)
	ЭмбоссированныйТекстПриИзменении(3, Текст)
КонецПроцедуры

&НаКлиенте
Процедура ЛицевыеСчетаЭмбоссированныйТекст2ПриИзменении(Элемент)
	ЭмбоссированныйТекстПриИзменении(2, Элемент.ТекстРедактирования)
КонецПроцедуры

&НаКлиенте
Процедура ЛицевыеСчетаЭмбоссированныйТекст2ИзменениеТекстаРедактирования(Элемент, Текст, СтандартнаяОбработка)
	ЭмбоссированныйТекстПриИзменении(2, Текст)
КонецПроцедуры

&НаКлиенте
Процедура ЛицевыеСчетаЭмбоссированныйТекст1ПриИзменении(Элемент)
	ЭмбоссированныйТекстПриИзменении(1, Элемент.ТекстРедактирования)
КонецПроцедуры

&НаКлиенте
Процедура ЛицевыеСчетаЭмбоссированныйТекст1ИзменениеТекстаРедактирования(Элемент, Текст, СтандартнаяОбработка)
	ЭмбоссированныйТекстПриИзменении(1, Текст)
КонецПроцедуры
...
Процедура Подбор(Команда)
	...
	Если ЗначениеЗаполнено(Объект.Подразделение) Тогда
		Отбор = Новый Структура("Подразделение", Объект.Подразделение);
...
&НаСервере
Процедура НастроитьФормуПоЗарплатномуПроекту()
	
	МаксимальнаяДлинаЭмбоссированногоТекста = Справочники.ЗарплатныеПроекты.МаксимальнаяДлинаИмениДержателяКарты(Объект.ЗарплатныйПроект);
		
	ОбменСБанкамиПоЗарплатнымПроектам.ЗаполнитьСписокСистемРасчетов(
		Объект.ЗарплатныйПроект, Элементы, "ЛицевыеСчетаСистемаРасчетовПоБанковскимКартам");
	
	ОбменСБанкамиПоЗарплатнымПроектам.УстановитьВидимостьЭлементов(
			ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Объект.ЗарплатныйПроект, "ФорматФайла"),
...
&НаКлиенте
Процедура ЭмбоссированныйТекстПриИзменении(НомерПоля, Текст)
	...
	ЭмбоссированныйТекст = ОбщегоНазначенияБЗККлиентСервер.ЗначенияВМассиве(
			ТекущиеДанные.ЭмбоссированныйТекст1,
			ТекущиеДанные.ЭмбоссированныйТекст2,
			ТекущиеДанные.ЭмбоссированныйТекст3);
    ЭмбоссированныйТекст[НомерПоля - 1] = Текст;
			
	ОбновитьОтображениеЭмбоссированногоТекста(ЭтотОбъект, ЭмбоссированныйТекст); 
	
КонецПроцедуры	

&НаКлиентеНаСервереБезКонтекста
Процедура ОбновитьОтображениеЭмбоссированногоТекста(Форма, ЭмбоссированныйТекст)
	ЦветТекста = ?(СтрДлина(СтрСоединить(ЭмбоссированныйТекст, " ")) > Форма.МаксимальнаяДлинаЭмбоссированногоТекста,
		Форма.ЦветСтиляПоясняющийОшибкуТекст, 
		Форма.ЦветСтиляЦветТекстаПоля);
	Для Поле = 1 По 3 Цикл
		ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(
			Форма.Элементы, "ЛицевыеСчетаЭмбоссированныйТекст" + Поле, 
			"ЦветТекста", ЦветТекста);
	КонецЦикла	
КонецПроцедуры
...
&НаСервере
Процедура ЗаполнитьСтрокиНаСервере(ФизическиеЛица)
	
	ДокументОбъект = РеквизитФормыВЗначение("Объект");
	ДокументОбъект.ЗаполнитьСтрокиТабличнойЧастиДаннымиДляОткрытияЛицевыхСчетов(ДокументОбъект.Дата, ФизическиеЛица);
	ЗначениеВРеквизитФормы(ДокументОбъект, "Объект")
...
Процедура ЛицевыеСчетаПриАктивизации()
	...
	// При переходе на сервер форма может измениться так, что возникнет новое событие активизации строки
	// (например, к повторному срабатыванию приводит заполнение списка выбора поля с идентификатором дизайна).
	// Для пресечения циклического обновления формы активизацию выполняем только при смене строки.
...
&НаСервере
Процедура ЛицевыеСчетаПриАктивизацииНаСервере(
		ИдентификаторСтроки, 
	...
	ОбновитьОтображениеЭмбоссированногоТекста(
		ЭтотОбъект,
...
&НаСервереБезКонтекста
Функция ИдентификаторыДизайна(СистемаРасчетов, КодВидаВклада, КодПодвидаВклада, ВалютаВклада, Знач Сотрудники, Ссылка)
...
Функция АдресСпискаПодобранныхСотрудников() Экспорт
	Возврат ПоместитьВоВременноеХранилище(Объект.Сотрудники.Выгрузить(,"ФизическоеЛицо").ВыгрузитьКолонку("ФизическоеЛицо"), ЭтотОбъект.УникальныйИдентификатор);
КонецФункции
