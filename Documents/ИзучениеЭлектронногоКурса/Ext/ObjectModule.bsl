...
Процедура СформироватьДвижения() Экспорт
	...
	ДанныеИзученияСтруктура = ДанныеИзученияСтруктура();
	...
	ЭтоКурсSCORMВерсии321 = Ложь; // Старый вариант активности SCORM в версии 3.2.1
	// В этой версии необходимо на уровне обработки cmi получать данные об обучении.
	...
	Пока ВыборкаЭлементовСодержания.Следующий() Цикл
		...
		Если НЕ ДанныеАктивности.Свойство("progress")
			И ДанныеАктивности.type = "SCO" Тогда
			
			ЭтоКурсSCORMВерсии321 = Истина;
	...
	Для каждого ДанныеАктивности Из ДанныеИзученияСтруктура.activities Цикл
		...
		Если ДанныеАктивности.type = "QUIZ" Тогда 
			...
			Для каждого ПопыткаТестирования Из ДанныеАктивности.quizAttempts Цикл
				...
				Движение.ИдентификаторАктивности = Новый УникальныйИдентификатор(ДанныеАктивности.uuid);
				Движение.ИдентификаторПопыткиТестирования = Новый УникальныйИдентификатор(ПопыткаТестирования.uuid);
	...
	Пока ВыборкаВопросов.Следующий() Цикл
		...
		Для каждого СтрокаТО Из ОтветыНаВопрос Цикл
			...
			Если ВыборкаВопросов.ТипВопроса = Перечисления.ТипыТестовыхВопросов.ОдинИзМногих
				ИЛИ ВыборкаВопросов.ТипВопроса = Перечисления.ТипыТестовыхВопросов.МногиеИзМногих
				ИЛИ ВыборкаВопросов.ТипВопроса = Перечисления.ТипыТестовыхВопросов.ОдинИзМногихНаКартинке Тогда
	...
	Если ЭтоКурсSCORMВерсии321 Тогда
...
Функция ДанныеИзученияСтруктура() Экспорт
	
	ДанныеИзученияСтрока = ДанныеИзучения.Получить();
	
	Если ТипЗнч(ДанныеИзученияСтрока) = Тип("Строка") И ЗначениеЗаполнено(ДанныеИзученияСтрока) Тогда
		Возврат ИзучениеЭлектронныхКурсовСлужебный.ДанныеИзученияСтруктураИзСтроки(ДанныеИзученияСтрока); 		
	ИначеЕсли ТипЗнч(ДанныеИзученияСтрока) = Тип("Структура") Тогда
		Возврат ДанныеИзученияСтрока;
	КонецЕсли;
