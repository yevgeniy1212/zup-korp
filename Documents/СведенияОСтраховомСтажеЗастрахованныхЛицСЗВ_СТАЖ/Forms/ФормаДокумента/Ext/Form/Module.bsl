...
Процедура УстановитьСвойстваЭлементовФормы()
	...
	ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(
		...
		"Доступность", 
		(ЭтоЕФС1 Или Объект.ТипСведений = Перечисления.ТипыСведенийСЗВ_СТАЖ.Исходная) 
...
Процедура ЗаполнитьДанныеДосрочногоНазначенияПенсии()
	...
	ЗаполнятьДанные = Ложь;
	
	Если Объект.ТипСведений = Перечисления.ТипыСведенийСЗВ_СТАЖ.Исходная Тогда 
		ЗаполнятьДанные = Истина;
	ИначеЕсли ПерсонифицированныйУчетКлиентСервер.ЭтоФормаЕФС1(Объект.Год) Тогда 
		Для Каждого ДанныеОСтаже Из ЗаписиОСтаже Цикл 
			Если ЗначениеЗаполнено(ДанныеОСтаже.ОсобыеУсловияТруда) Или ЗначениеЗаполнено(ДанныеОСтаже.ОснованиеВыслугиЛет) Тогда 
				ЗаполнятьДанные = Истина;
				Прервать;
			КонецЕсли;
		КонецЦикла;
	...
	Запрос.Текст = 
		...
		|ГДЕ
		|	ЕСТЬNULL(ДосрочноеНазначениеПенсии.ОсобыеУсловияТруда, ЗНАЧЕНИЕ(Справочник.ОсобыеУсловияТрудаПФР.ПустаяСсылка)) <> ЗНАЧЕНИЕ(Справочник.ОсобыеУсловияТрудаПФР.ПустаяСсылка)
		|
		|УПОРЯДОЧИТЬ ПО
	...
	Если Не ПерсонифицированныйУчетКлиентСервер.ЭтоФормаЕФС1(Объект.Год) Тогда
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "ЕСТЬNULL(ДосрочноеНазначениеПенсии.ОсобыеУсловияТруда, ЗНАЧЕНИЕ(Справочник.ОсобыеУсловияТрудаПФР.ПустаяСсылка)) <> ЗНАЧЕНИЕ(Справочник.ОсобыеУсловияТрудаПФР.ПустаяСсылка)", "Истина");
	КонецЕсли;
