...
Процедура СформироватьДеревьяЭД()
	...
	ОбменСБанкамиПереопределяемый.ДобавитьСвязанныеДокументы(ОбъектСсылка, МассивДокументов); 
	Если МассивДокументов.Найти(ОбъектСсылка) = Неопределено Тогда  
		МассивДокументов.Добавить(ОбъектСсылка);
	КонецЕсли;
	Для каждого ТекДокумент Из МассивДокументов Цикл
		ВывестиПодчиненныеДокументы(ТекДокумент, ДеревоПодчиненныеЭД);	
	КонецЦикла; 
