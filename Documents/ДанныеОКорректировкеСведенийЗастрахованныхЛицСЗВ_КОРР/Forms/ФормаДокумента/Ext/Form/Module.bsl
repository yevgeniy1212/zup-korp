...
Процедура УстановитьСвойстваЭлементовФормы()
	...
	ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "КорректируемыйПериод", "ТолькоПросмотр", ОтправленВПФР);
	
	ЭтоЕФС1 = ПерсонифицированныйУчетКлиентСервер.ЭтоФормаЕФС1(Год(Объект.КорректируемыйПериод));
...
Процедура ЗаполнитьДанныеОСтажеСотрудников(Сотрудники)
	...
	ДатаПостановления322 = РегистрыСведений.ДатыВступленияВСилуНА.ДатаВступленияВСилуНА("ПостановлениеПравительстваРФот4марта2021Номер322");
	...
	ПроверяемыеРеквизиты = СтроковыеФункцииКлиентСервер.РазложитьСтрокуВМассивПодстрок(
		...
		|ОснованиеВыслугиЛет,ПервыйПараметрВыслугиЛет,ВторойПараметрВыслугиЛет,ТретийПараметрВыслугиЛет,ОсобыеУсловияТруда, РайонныйКоэффициент,
		...
		|КлассПодклассУсловийТруда,НомерРабочегоМеста", , Истина, Истина);
		...
		Пока Выборка.Следующий() Цикл
			Если ЗначениеЗаполнено(Выборка.ТретийПараметрИсчисляемогоСтажа) И ЗначениеЗаполнено(Выборка.ЗамещениеГосударственныхМуниципальныхДолжностей) Тогда 
				...
				Иначе 
					...
					КонецЕсли;
				КонецЕсли;
				ДобавитьСтрокуЗаписейОСтаже(ТаблицаОтклонений, Выборка, ДатаПостановления322);
				Продолжить;
			КонецЕсли;
			...
			КонецЕсли;
			ВывестиСтрокиОтклонений(ТаблицаОтклонений, ПредыдущаяСтрока);
			ДобавитьСтрокуЗаписейОСтаже(Объект.ЗаписиОСтаже, Выборка, ДатаПостановления322);
		КонецЦикла;
		ВывестиСтрокиОтклонений(ТаблицаОтклонений, ПредыдущаяСтрока);
...
&НаСервере
Процедура ДобавитьСтрокуЗаписейОСтаже(ДанныеОСтаже, Выборка, ДатаПостановления322)
	
	НоваяСтрока = ДанныеОСтаже.Добавить();
	ЗаполнитьЗначенияСвойств(НоваяСтрока, Выборка);
	
	Если Объект.КорректируемыйПериод < '20170101' Тогда
		ПерсонифицированныйУчет.ОбработатьПериодПовышенияКвалификации(ДанныеОСтаже, НоваяСтрока, ДатаПостановления322);
		Возврат;
	КонецЕсли;
