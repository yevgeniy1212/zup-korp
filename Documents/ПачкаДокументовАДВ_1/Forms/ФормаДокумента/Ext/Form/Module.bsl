...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	Если ИмяСобытия = "ИзменениеДанныхФизическогоЛица" Тогда
		...
		Если КоличествоСтрок > 0 Тогда
			СтрокаСотрудника = СтрокиПоСотруднику[КоличествоСтрок - 1];
			...
			КонецЕсли;
			Если СокрЛП(СтрокаСотрудника.Фамилия) = "-" Тогда
				СтрокаСотрудника.Фамилия = "";
			КонецЕсли;
			Элементы.Сотрудники.ТекущаяСтрока = СтрокаСотрудника.ПолучитьИдентификатор();
		КонецЕсли;
...
Процедура УстановитьСвойстваЭлементовФормы()
	...
	ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "НомерАктовойЗаписиПриРождении", "ТолькоПросмотр", ВидДокумента <> Справочники.ВидыДокументовФизическихЛиц.СвидетельствоОРождении);
	ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "НомерАктовойЗаписиПриРождении", "АвтоОтметкаНезаполненного", ВидДокумента = Справочники.ВидыДокументовФизическихЛиц.СвидетельствоОРождении);
	
	Если ВидДокумента <> Справочники.ВидыДокументовФизическихЛиц.СвидетельствоОРождении Тогда
...
Процедура ОтобразитьДанныеФизическогоЛица()
	...
	Если ДанныеТекущейСтроки <> Неопределено Тогда
		...
		КонецЕсли;	
		ВидДокумента = ДанныеТекущейСтроки.ВидДокумента;
	Иначе
		Для Каждого КлючЗначение Из СтруктураСоответствияПолей Цикл
			ЭлементФормы = Элементы[КлючЗначение.Значение];	
