...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ПараметрыРазмещения.КоманднаяПанель = Элементы.ГруппаКоманд;
...
&НаКлиенте
Процедура БыстрыеОтборыЗначениеАвтоПодбор(
	Элемент,
	Текст,
	ДанныеВыбора,
	...
	Если Элементы.БыстрыеОтборы.ТекущиеДанные.Параметр = "СчетОрганизации" Тогда
		ДанныеВыбора = СписокБанковскихСчетов;
		СтандартнаяОбработка = Ложь;
	КонецЕсли;
...
Процедура СформироватьТаблицуБыстрогоОтбора()
	...
	Если ЗначениеЗаполнено(ПлатежноеПоручение) Тогда
		МассивВыбора.Добавить(Тип("ДокументСсылка." + ПлатежноеПоручение));
	КонецЕсли;
	МассивВыбора.Добавить(Тип("ДокументСсылка.ПисьмоОбменСБанками"));
	ДобавитьСтрокуОтбора(
		"Основание", НСтр("ru = 'Основание:'"), "ОпределяемыеТипы.ВладелецОбменСБанками", , МассивВыбора);
	
	ОрганизацииИБанки = ПолучитьИзВременногоХранилища(ТаблицаНастроек);
		
	МассивБанковскихСчетов = СписокБанковскихСчетов(ОрганизацииИБанки);
	Если СписокБанковскихСчетов.Количество() = 1 Тогда
		СчетОрганизации = МассивБанковскихСчетов.Получить(0);
	Иначе
		ДобавитьСтрокуОтбора("СчетОрганизации", НСтр("ru = 'Счет:'"), "ОпределяемыеТипы.СчетОрганизацииОбменСБанками", ,
			МассивБанковскихСчетов);
	КонецЕсли;
	СписокБанковскихСчетов.ЗагрузитьЗначения(МассивБанковскихСчетов);
	
	НазваниеСправочникаОрганизации = ОбщегоНазначенияБЭД.ИмяПрикладногоСправочника("Организации");
	...
	Иначе
		ДобавитьСтрокуОтбора("Организация", НСтр("ru = 'Организация:'"),
			"СправочникСсылка." + НазваниеСправочникаОрганизации, , Организации);
	КонецЕсли;
	
	НазваниеСправочникаБанки = ОбщегоНазначенияБЭД.ИмяПрикладногоСправочника("Банки");
	...
	Иначе
		ДобавитьСтрокуОтбора("Банк", НСтр("ru = 'Банк:'"), "СправочникСсылка."+ НазваниеСправочникаБанки, , Банки);
	КонецЕсли;
...
&НаСервере
Функция СписокБанковскихСчетов(ТаблицаОтбора)
	
	Запрос = Новый Запрос;
	Запрос.Текст =
	"ВЫБРАТЬ РАЗЛИЧНЫЕ
	|	ВременнаяТаблица.Организация,
	|	ВременнаяТаблица.Банк
	...
	|		ПО БанковскиеСчета.Владелец = ТаблицаОтбора.Организация
	|		И БанковскиеСчета.Банк = ТаблицаОтбора.Банк";
	
	Запрос.УстановитьПараметр("ВременнаяТаблица", ТаблицаОтбора);
	
	НазваниеСчетаВМетаданных = ОбменСБанкамиСлужебныйПовтИсп.НазваниеСчетаВМетаданных();
	Запрос.Текст = СтрЗаменить(Запрос.Текст, "БанковскиеСчета", НазваниеСчетаВМетаданных);
	
	ОрганизацияВСчете = ОбменСБанкамиСлужебный.ИмяПрикладногоРеквизита("БанковскийСчетОрганизации.Организация");
	Запрос.Текст = СтрЗаменить(Запрос.Текст, "Владелец", ОрганизацияВСчете);
	
	БанкВСчете = ОбменСБанкамиСлужебный.ИмяПрикладногоРеквизита("БанковскийСчетОрганизации.Банк");
	Запрос.Текст = СтрЗаменить(Запрос.Текст, "БанковскиеСчета.Банк", "БанковскиеСчета." + БанкВСчете);
...
Процедура СоздатьОтборПоРазделу(Знач Раздел, ГруппаОтбора)
	...
	Если Раздел = "Входящие" Тогда
		...
		ОбщегоНазначенияКлиентСервер.УстановитьЭлементОтбора(
			ГруппаОтбора, "Направление", Перечисления.НаправленияЭДО.Входящий, ВидСравненияКомпоновкиДанных.Равно, , Истина);
	...
	ИначеЕсли Раздел = "Исходящие" Тогда
		...
		ОбщегоНазначенияКлиентСервер.УстановитьЭлементОтбора(
			ГруппаОтбора, "Направление", Перечисления.НаправленияЭДО.Исходящий, ВидСравненияКомпоновкиДанных.Равно, , Истина);
	...
	ИначеЕсли Раздел = "ИсходящиеОтклоненные" Тогда
		...
		МассивОтбора = Новый Массив;
		МассивОтбора.Добавить(Перечисления.СтатусыОбменСБанками.ОтклоненБанком);
		МассивОтбора.Добавить(Перечисления.СтатусыОбменСБанками.ОшибкаПередачи);
...
Процедура УстановитьНедоступностьПоРолям()
	...
	Если Не ОбменСБанкамиСлужебный.ПравоОбработкиЭД() Тогда
		Элементы.НовоеПисьмо.Доступность = Ложь;
		Элементы.ФормаОтветить.Доступность = Ложь;
		Элементы.ФормаОтправитьПолучить.Доступность = Ложь;
	КонецЕсли;
