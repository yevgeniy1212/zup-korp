...
Процедура ОбновитьУдержаноИтог(Форма)
	...
	Для Каждого СтрокаЕП Из СтрокиЕП Цикл
		Форма.ИсчисленоЕП = Форма.ИсчисленоЕП + СтрокаЕП.НалогВзнос + СтрокаЕП.НалогВзносСПревышения;
	КонецЦикла;
...
Процедура УстановитьВидимостьПерерасчетов()
	...
	УстановитьПривилегированныйРежим(Истина);
	ПерерасчетыДоступны = ИсправлениеДокументовРасчетЗарплатыКлиентСервер.ПерерасчетыДоступны(ЭтаФорма,,"ДатаУвольнения") ИЛИ ПолучитьФункциональнуюОпцию("ИспользоватьРасчетыОтОбратного");
...
Процедура УстановитьВидимостьРасчетныхПолей()
    ...
    // ЗУПК
	ПлательщикОППВ = ПолучитьФункциональнуюОпцию("ПлательщикОППВЗарплатаКадры", Новый Структура("Организация", Объект.Организация));
