...
Процедура ОбработкаПроведения(Отказ, РежимПроведения)
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("ЗарплатаКадрыПриложения.ГосударственнаяСлужба.РасчетДенежногоСодержания") Тогда		
		...
		Модуль.ЗарегистрироватьКоэффициентыИндексацииДенежногоСодержания(Движения, ДанныеПроведения.КоэффициентыИндексацииДенежногоСодержания);
	КонецЕсли;
...
Процедура ОбработкаЗаполнения(ДанныеЗаполнения, СтандартнаяОбработка)
	Если ТипЗнч(ДанныеЗаполнения) = Тип("ДокументСсылка.ИндексацияШтатногоРасписания") Тогда
		...
		МесяцИндексации = ДанныеЗаполнения.МесяцИндексации;
		Организация = ДанныеЗаполнения.Организация;
		Подразделение = ДанныеЗаполнения.Подразделение;
		КоэффициентИндексации = ДанныеЗаполнения.КоэффициентИндексации;
		
		Показатели.Загрузить(ДанныеЗаполнения.ИндексируемыеПоказатели.Выгрузить());
		
		ПозицииШтатногоРасписанияДляОтбора = ОбщегоНазначения.ВыгрузитьКолонку(ДанныеЗаполнения.Позиции.Выгрузить(),"ПозицияШтатногоРасписания", Истина);
		
		ЗаполнитьДокумент(ЗарплатаКадрыРасширенный.ВремяРегистрацииДокумента(Ссылка, МесяцИндексации), , ПозицииШтатногоРасписанияДляОтбора);
...
Процедура ЗаполнитьДокумент(ДатаИндексации, СохранятьИсправления = Ложь, ПозицииШтатногоРасписанияДляОтбора = Неопределено) Экспорт
	...
	МассивСотрудников = Менеджер.СотрудникиДляИндексации(Организация, Подразделение, ДатаИндексации, ЗначениеЗаполнено(ВидИндексацииГосударственныхСлужащих), 
														 ИндексацияВоеннослужащих, ПозицииШтатногоРасписанияДляОтбора);
...
Функция ПолучитьДанныеДляПроведения()
	...
	Запрос.Текст = 
		...
		|				ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ИндексацияЗаработка.Сотрудники КАК ИндексацияЗаработкаСотрудники
		|				ПО (ИндексацияЗаработкаСотрудники.Сотрудник = КоэффициентыИндексацииЗаработка.Сотрудник)
		|		ГДЕ
		|			ИндексацияЗаработкаСотрудники.Ссылка = &Ссылка
		|			И КоэффициентыИндексацииЗаработка.Регистратор <> &Ссылка) КАК КоэффициентыИндексацииЗаработка
		|		ПО ИндексацияЗаработкаСотрудники.Сотрудник = КоэффициентыИндексацииЗаработка.Сотрудник
		|			И (ВЫБОР
		|				КОГДА ИндексацияЗаработкаСотрудники.КоэффициентИндексации > 1
		|					ТОГДА &Период >= КоэффициентыИндексацииЗаработка.ДатаИндексации
		|				ИНАЧЕ &Период = КоэффициентыИндексацииЗаработка.ДатаИндексации
		|			КОНЕЦ)
		|			И (ГОД(&Период) + МЕСЯЦ(&Период) - ГОД(&Период) - МЕСЯЦ(&Период) + 1 <= 12)
		|ГДЕ
		|	ИндексацияЗаработкаСотрудники.Ссылка = &Ссылка
		|
		|УПОРЯДОЧИТЬ ПО
	...
	// ЗУПК
	УчетСреднегоЗаработкаРК.ОбработатьТаблицуКоэффициентовИндексации(ДанныеДляПроведения.КоэффициентыИндексации);
	// ЗУПК
