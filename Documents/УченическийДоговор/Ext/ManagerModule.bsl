...
Процедура ПриЗаполненииНастроекОбработчиковПравилРегистрации(Настройки) Экспорт
	...
	ЗарплатаКадрыРасширенныйСинхронизацияДанных.ДляОграниченияПоРеквизиту(Настройки,
		Метаданные.Документы.УченическийДоговор.Реквизиты.ФизическоеЛицо, "УправленческиеДанные");
КонецПроцедуры
...
Функция ПечатьУченическогоДоговора(МассивОбъектов, ОбъектыПечати)
	...
	Для Каждого ДокументСсылка Из МассивОбъектов Цикл
		...
		РеквизитыДокумента = ДанныеДляПечатиДокумента(ДокументСсылка);
		...
		Пока ВыборкаОбязательств.Следующий() Цикл
			...
			ИначеЕсли ПерваяСтрока Тогда
				КомпенсацияРасходов = КомпенсацияРасходов + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					// ЗУПК
					НСтр("ru = 'при увольнении в период с %1 по %2 г. Сотрудник возмещает расходы в сумме %3 тг.'"),
					// ЗУПК
					Формат(ВыборкаОбязательств.ДатаНачалаОбязательств, "ДЛФ=Д"),
...
Функция ДанныеДляПечатиДокумента(ДокументСсылка)
	...
	ДанныеДляПечати = ПараметрыУченическогоДоговора();
	ЗаполнитьЗначенияСвойств(ДанныеДляПечати, ВыборкаЗапроса);
	
	ДанныеДляПечати.ДатаНачалаДоговора = Формат(ДанныеДляПечати.ДатаНачалаДоговора, "ДЛФ=Д");
	ДанныеДляПечати.ДатаОкончанияДоговора = Формат(ДанныеДляПечати.ДатаОкончанияДоговора, "ДЛФ=Д");
	
	ТаблицаКИ = КонтактнаяИнформацияБЗК.КонтактнаяИнформацияОбъектов(
		ДанныеДляПечати.Организация,
		,
		Справочники.ВидыКонтактнойИнформации.ЮрАдресОрганизации);
	ДанныеДляПечати.АдресОрганизации = ?(ТаблицаКИ.Количество() > 0, ТаблицаКИ[0].Представление, "");
		
	ДанныеДляПечати.РуководительДолжностьРодПадеж = СклонениеПредставленийОбъектов.ПросклонятьПредставление(
		Строка(ДанныеДляПечати.РуководительДолжность), 2);
	ФИОРуководителяВПадеже = "";
	ФизическиеЛицаЗарплатаКадры.Просклонять(
		Строка(ДанныеДляПечати.РуководительФИОПолные), 2, ФИОРуководителяВПадеже, ВыборкаЗапроса.РуководительПол, ВыборкаЗапроса.Руководитель);
	ДанныеДляПечати.РуководительФИОПолные = ФИОРуководителяВПадеже; 
