...
Функция ОписаниеСоставаОбъекта() Экспорт
	...
	МетаданныеДокумента = Метаданные.Документы.ПачкаРазделов6РасчетаРСВ_1;
...
Процедура СформироватьФайлыДокументов(СписокДокументов, ДатаПодписи) Экспорт 
	ФайлыДокументов = ФайлыДокументов(СписокДокументов, ДатаПодписи);	
	
	Для Каждого ОписаниеФайлаДокумента Из ФайлыДокументов Цикл
		ЗарплатаКадры.ЗаписатьФайлВАрхив(ОписаниеФайлаДокумента.Ключ, ОписаниеФайлаДокумента.Значение.ИмяФайла, ОписаниеФайлаДокумента.Значение.ТекстФайла);
	КонецЦикла;
КонецПроцедуры	
...
Функция ФайлыДокументов(СписокДокументов, ДатаПодписи) Экспорт
	...
	ФорматВходящейОписи = ЗарплатаКадры.ЗагрузитьФорматНабораЗаписей(ДеревоФормата, "Опись_Разделов_6");
	ФорматВходящейОписи.НомерВПачке.Значение = 1;
	ФорматРаздела6 = ЗарплатаКадры.ЗагрузитьФорматНабораЗаписей(ДеревоФормата, "Раздел_6");
	ФорматЗаписейСуммаВыплат = ОбщегоНазначения.СкопироватьРекурсивно(ФорматРаздела6.СведенияОсуммеВыплатИВознагражденийВпользуЗЛ.Значение);
	ФорматРаздела6.Удалить("СведенияОсуммеВыплатИвознагражденийВпользуЗЛ");
	ФорматСуммаВзносовНаОПС = Новый Структура("СуммаВзносовНаОПС", ОбщегоНазначения.СкопироватьРекурсивно(ФорматРаздела6.СуммаВзносовНаОПС));
	ФорматРаздела6.Удалить("СуммаВзносовНаОПС");
	ФорматЗаписейОКорректировках = ОбщегоНазначения.СкопироватьРекурсивно(ФорматРаздела6.СведенияОкорректировках.Значение);
	ФорматРаздела6.Удалить("СведенияОкорректировках");
	ФорматЗаписейВыплатПоДопТарифу = ОбщегоНазначения.СкопироватьРекурсивно(ФорматРаздела6.СведенияОсуммеВыплатИВознагражденийПоДопТарифу.Значение);
	ФорматРаздела6.Удалить("СведенияОсуммеВыплатИвознагражденийПоДопТарифу");
	ФорматСтажевыйПериод = ОбщегоНазначения.СкопироватьРекурсивно(ФорматРаздела6.СтажевыйПериод.Значение);
	ФорматРаздела6.Удалить("СтажевыйПериод");
	ФорматДатаЗаполнения = Новый Структура("ДатаЗаполнения", ОбщегоНазначения.СкопироватьРекурсивно(ФорматРаздела6.ДатаЗаполнения));
	ФорматРаздела6.Удалить("ДатаЗаполнения");
	
	ФорматЗаписейСуммаВыплатДляОтмФорм = ОбщегоНазначения.СкопироватьРекурсивно(ФорматЗаписейСуммаВыплат);
	...
	Пока ВыборкаПоШапкеДокумента.СледующийПоЗначениюПоля("Ссылка") Цикл 
		...
		ПерсонифицированныйУчет.ЗаполнитьИмяИЗаголовокФайла(УзелПФР, ДеревоФормата, ИмяФайла);
		УзелПачкаВходящихДокументов = ЗарплатаКадры.ДобавитьУзелВДеревоXML(УзелПФР, "ПачкаВходящихДокументов", "", );
		
		НаборЗаписейВходящаяОпись = ОбщегоНазначения.СкопироватьРекурсивно(ФорматВходящейОписи);
		...
		ПерсонифицированныйУчет.ЗаполнитьСоставительПачки(НаборЗаписейВходящаяОпись.СоставительПачки.Значение, ВыборкаПоШапкеДокумента); // ОрганизацияЮрФизЛицо, ОрганизацияИНН, ОрганизацияКПП, ОрганизацияОГРН, ОрганизацияНаименованиеОКОПФ, ОрганизацияНаименованиеПолное, ОрганизацияНаименованиеСокращенное
		...
		ЗарплатаКадры.ДобавитьИнформациюВДерево(ЗарплатаКадры.ДобавитьУзелВДеревоXML(УзелПачкаВходящихДокументов, "СВЕДЕНИЯ_ПО_ПАЧКЕ_ДОКУМЕНТОВ_РАЗДЕЛА_6",""), НаборЗаписейВходящаяОпись);
		
		НаборЗаписейПачки = ОбщегоНазначения.СкопироватьРекурсивно(ФорматРаздела6);
		Если ВыборкаПоШапкеДокумента.ТипСведенийСЗВ = Перечисления.ТипыСведенийСЗВ.ОТМЕНЯЮЩАЯ Тогда
		...
		Если ВыборкаПоШапкеДокумента.ОтчетныйПериод < '20150401' Тогда
			НаборЗаписейПачки.Удалить("СведенияОбУвольнении");
		КонецЕсли;
		...
		Если ВыборкаПоРаботникам.НайтиСледующий(Новый Структура("Ссылка", ВыборкаПоШапкеДокумента.Ссылка)) Тогда
			...
			Пока ВыборкаПоРаботникам.СледующийПоЗначениюПоля("НомерСтроки") Цикл
				...
				Если ВыборкаПоШапкеДокумента.ОтчетныйПериод >= '20150401' Тогда
					Если ЗначениеЗаполнено(ВыборкаПоРаботникам.СведенияОбУвольнении) Тогда
						НаборЗаписейРаздела6.СведенияОбУвольнении.Значение = ВыборкаПоРаботникам.СведенияОбУвольнении;
					Иначе
						НаборЗаписейРаздела6.СведенияОбУвольнении.НеВыводитьВФайл = Истина;
					КонецЕсли;
				КонецЕсли;
				
				ФорматДатаЗаполнения.ДатаЗаполнения.Значение = ДатаПодписи;
				
				УзелСотрудника = ЗарплатаКадры.ДобавитьУзелВДеревоXML(УзелПачкаВходящихДокументов, ТипДокументовПачки,"");
				ЗарплатаКадры.ДобавитьИнформациюВДерево(УзелСотрудника, НаборЗаписейРаздела6);
				...
				// СведенияОЗаработке
				ВывестиДоходыВФайл(УзелСотрудника, ФорматЗаписейСуммаВыплатТекущейПачки,  "СведенияОсуммеВыплатИвознагражденийВпользуЗЛ", Период, ВыборкаПоДоходам, "КатегорияЗастрахованныхЛиц", "4", ИменаПолейДоходов);
				
				// Заполнение отменяющей формы завершено.
				Если ВыборкаПоШапкеДокумента.ТипСведенийСЗВ = Перечисления.ТипыСведенийСЗВ.ОТМЕНЯЮЩАЯ Тогда
										
					// Выводим ДатаЗаполнения
					ЗарплатаКадры.ДобавитьИнформациюВДерево(УзелСотрудника, ФорматДатаЗаполнения);
					Продолжить;
					
				КонецЕсли;
				
				// Выводим взносы только в том случае если они заполнены.
				Если ВыборкаПоРаботникам.НачисленоСтраховая <> 0 Тогда
					ФорматСуммаВзносовНаОПС.СуммаВзносовНаОПС.Значение = ВыборкаПоРаботникам.НачисленоСтраховая;
					ЗарплатаКадры.ДобавитьИнформациюВДерево(УзелСотрудника, ФорматСуммаВзносовНаОПС);
				КонецЕсли;	
				...
				Если ВыборкаПоКорректирующимСведениям.НайтиСледующий(СтруктураПоиска) Тогда
					...
					Пока ВыборкаПоКорректирующимСведениям.Следующий() Цикл
						...
						ДанныеДляФайла.Квартал.Значение = ПерсонифицированныйУчет.КодПериодаДляРасчетаРСВ1(ВыборкаПоКорректирующимСведениям.КорректируемыйПериод);
						ДанныеДляФайла.ТипСтроки.Значение = "МЕСЦ";
						ДанныеДляФайла.НомерСтроки.Значение = НомерСтроки;
						
						ЗарплатаКадры.ДобавитьИнформациюВДерево(ЗарплатаКадры.ДобавитьУзелВДеревоXML(УзелСотрудника, "СведенияОкорректировках",""), ДанныеДляФайла);
				...
				ВывестиДоходыВФайл(УзелСотрудника, ФорматЗаписейВыплатПоДопТарифу, "СведенияОсуммеВыплатИвознагражденийПоДопТарифу", Период, ВыборкаПоДоходамЗанятыхНаВредныхИТяжелыхРаботах, "КлассУсловийТруда", "7", ИменаПолейДоходовПоДопТарифу);
				
				// Выводим стаж
				ПерсонифицированныйУчет.ВписатьЗаписиОСтажеВНаборДанных2014(УзелСотрудника, ФорматСтажевыйПериод, ВыборкаПоРаботникам);
	...
	Если Данные.ЗаработокЗаполнен Тогда
		...
		Пока Данные.СледующийПоЗначениюПоля(ИмяПоляГруппировки) Цикл
			...
			Пока Данные.Следующий() Цикл
				...
				ДанныеДляФайла.НомерСтроки.Значение = НомерСтроки;
				ЗарплатаКадры.ДобавитьИнформациюВДерево(ЗарплатаКадры.ДобавитьУзелВДеревоXML(УзелСотрудника, Тэг,""), ДанныеДляФайла);
				НомерСтроки = НомерСтроки + 1;
			КонецЦикла;
...
Функция СформироватьЗапросПоШапкеДляПечати(МассивСсылок)
	...
	|	ПачкаРазделов6РасчетаРСВ_1.ДолжностьРуководителя.Наименование КАК ДолжностьРуководителя,
	...
	|	ПачкаРазделов6РасчетаРСВ_1.СпециальныйДокумент2015года
	...
	"ВЫБРАТЬ
	|	ПачкаРазделов6РасчетаРСВ_1.Ссылка КАК Ссылка,
	|	ПачкаРазделов6РасчетаРСВ_1.РегистрационныйНомерПФР КАК РегистрационныйНомерПФР,
	|	ПачкаРазделов6РасчетаРСВ_1.НаименованиеОрганизации КАК НаименованиеОрганизации,
	|	ПачкаРазделов6РасчетаРСВ_1.ЮридическоеФизическоеЛицо,
	|	ПачкаРазделов6РасчетаРСВ_1.ИНН КАК ИНН,
	|	ПачкаРазделов6РасчетаРСВ_1.КПП КАК КПП,
	|	ПачкаРазделов6РасчетаРСВ_1.ОГРН КАК ОГРН,
	|	ПачкаРазделов6РасчетаРСВ_1.КодПоОКПО КАК ОКПО,
	|	ПачкаРазделов6РасчетаРСВ_1.НаименованиеПолное КАК НаименованиеПолное,
	|	ПачкаРазделов6РасчетаРСВ_1.НаименованиеСокращенное КАК НаименованиеСокращенное,
	|	ПачкаРазделов6РасчетаРСВ_1.ОтчетныйПериод,
	|	ПачкаРазделов6РасчетаРСВ_1.ТипСведенийСЗВ КАК ТипСведенийСЗВ,
	|	ПачкаРазделов6РасчетаРСВ_1.КорректируемыйПериод КАК КорректируемыйПериод,
...
Функция СформироватьЗапросПоСотрудникамДляПечати(МассивСсылок) Экспорт
	...
	|	ВЫБОР
	|		КОГДА ПачкаРазделов6РасчетаРСВ_1Сотрудники.Ссылка.ОтчетныйПериод < ДАТАВРЕМЯ(2015, 4, 1)
	|			ТОГДА ЛОЖЬ
	|		ИНАЧЕ ПачкаРазделов6РасчетаРСВ_1Сотрудники.СотрудникУволен
	|	КОНЕЦ КАК СотрудникУволен,
	|	ВЫБОР
	|		КОГДА ПачкаРазделов6РасчетаРСВ_1Сотрудники.Ссылка.ОтчетныйПериод < ДАТАВРЕМЯ(2015, 4, 1)
	|			ТОГДА """"
	|		КОГДА ПачкаРазделов6РасчетаРСВ_1Сотрудники.СотрудникУволен
	|			ТОГДА ""УВОЛЕН""
	|		ИНАЧЕ """"
	|	КОНЕЦ КАК СведенияОбУвольнении
	...
	|	ВЫБОР
	|		КОГДА ЗаписиОСтаже.Ссылка.ОтчетныйПериод < ДАТАВРЕМЯ(2015, 4, 1)
	|				ИЛИ &НеИспользоватьКоды2015Года
	|			ТОГДА ЗаписиОСтаже.ОсобыеУсловияТруда.КодДляОтчетности2010
	|		ИНАЧЕ ЗаписиОСтаже.ОсобыеУсловияТруда.КодДляОтчетности2015
	...
	|		КОГДА ЗаписиОСтаже.Ссылка.ОтчетныйПериод < ДАТАВРЕМЯ(2015, 4, 1)
	|				ИЛИ &НеИспользоватьКоды2015Года
	|			ТОГДА ЗаписиОСтаже.ОснованиеВыслугиЛет.КодДляОтчетности2010
	|		ИНАЧЕ ЗаписиОСтаже.ОснованиеВыслугиЛет.КодДляОтчетности2015
	...
	|	ПачкаРазделов6РасчетаРСВ_1Сотрудники.Имя,
	|	ПачкаРазделов6РасчетаРСВ_1Сотрудники.Отчество,
	|	ПачкаРазделов6РасчетаРСВ_1Сотрудники.СотрудникУволен,
	|	ПачкаРазделов6РасчетаРСВ_1Сотрудники.СведенияОбУвольнении,
	|	ПачкаРазделов6РасчетаРСВ_1Сотрудники.НачисленоСтраховая,
	|	ПачкаРазделов6РасчетаРСВ_1Сотрудники.ДоначисленоСтраховая,
	|	ПачкаРазделов6РасчетаРСВ_1Сотрудники.Ссылка КАК Ссылка,
	|	ВЫБОР
	...
	|УПОРЯДОЧИТЬ ПО
	|	ПорядокСортировки,
	|	ПачкаРазделов6РасчетаРСВ_1Сотрудники.Ссылка.КорректируемыйПериод УБЫВ,
	|	ПачкаРазделов6РасчетаРСВ_1Сотрудники.Ссылка.ТипСведенийСЗВ,
	|	ПачкаРазделов6РасчетаРСВ_1Сотрудники.Ссылка.НомерПачки,
	|	Ссылка,
	|	НомерСтроки,
	|	НомерОсновнойЗаписи,
...
Функция СформироватьЗапросПоДоходамЗанятыхНаВредныхИТяжелыхРаботахДляПечати(МассивОбъектов) Экспорт
	...
	|	ВЫБОР
	|		КОГДА ПачкаРазделов6РасчетаРСВ_1СведенияОЗаработке.КлассУсловийТруда = ЗНАЧЕНИЕ(Перечисление.КлассыУсловийТрудаПоРезультатамСпециальнойОценки.Опасный)
	|			ТОГДА ""О4""
	|		КОГДА ПачкаРазделов6РасчетаРСВ_1СведенияОЗаработке.КлассУсловийТруда = ЗНАЧЕНИЕ(Перечисление.КлассыУсловийТрудаПоРезультатамСпециальнойОценки.Вредный4)
	|			ТОГДА ""В3.4""
	|		КОГДА ПачкаРазделов6РасчетаРСВ_1СведенияОЗаработке.КлассУсловийТруда = ЗНАЧЕНИЕ(Перечисление.КлассыУсловийТрудаПоРезультатамСпециальнойОценки.Вредный3)
	|			ТОГДА ""В3.3""
	|		КОГДА ПачкаРазделов6РасчетаРСВ_1СведенияОЗаработке.КлассУсловийТруда = ЗНАЧЕНИЕ(Перечисление.КлассыУсловийТрудаПоРезультатамСпециальнойОценки.Вредный2)
	|			ТОГДА ""В3.2""
	|		КОГДА ПачкаРазделов6РасчетаРСВ_1СведенияОЗаработке.КлассУсловийТруда = ЗНАЧЕНИЕ(Перечисление.КлассыУсловийТрудаПоРезультатамСпециальнойОценки.Вредный1)
	|			ТОГДА ""В3.1""
	|		ИНАЧЕ """"
...
Функция СформироватьЗапросПоКорректирующимСведениям(Ссылки, ИсходныйДокумент = Неопределено)  Экспорт
	...
	|	0
	|ИЗ
	|	Документ.ПачкаДокументовСЗВ_6_1.Сотрудники КАК СведенияОТрудовомСтажеИЗаработкеСЗВ4РаботникиОрганизации
	|ГДЕ
	...
	|ГДЕ
	|	(ДанныеКорректирующихФорм.ДоначисленоСтраховая <> 0
	|			ИЛИ ДанныеКорректирующихФорм.ДоначисленоНакопительная <> 0
	|			ИЛИ ДанныеКорректирующихФорм.ДоначисленоНаОПС <> 0)
	|
	|СГРУППИРОВАТЬ ПО
	...
	|	ПачкаРазделов6РасчетаРСВ_1КорректирующиеСведения.Сотрудник,
	...
	Если ИсходныйДокумент = Неопределено Тогда
		ТекстУсловияИсходныйДокумент1 = "ИСТИНА";
		ТекстУсловияИсходныйДокумент2 = "ИСТИНА";
	Иначе
		ТекстУсловияИсходныйДокумент1 = "ПачкаРазделов6РасчетаРСВ_1Сотрудники.Ссылка = &ИсходныйДокумент";
		ТекстУсловияИсходныйДокумент2 = "ПачкаРазделов6РасчетаРСВ_1КорректирующиеСведения.Ссылка = &ИсходныйДокумент";
	КонецЕсли;
	
	Запрос.Текст = СтрЗаменить(Запрос.Текст, "&УсловиеИсходныйДокумент1", ТекстУсловияИсходныйДокумент1);
	Запрос.Текст = СтрЗаменить(Запрос.Текст, "&УсловиеИсходныйДокумент2",  ТекстУсловияИсходныйДокумент2);
	
	Возврат Запрос.Выполнить();
	
КонецФункции

Функция ТабличныеДокументыРаздела6РСВ_1(МассивОбъектов, ДокументыКомплекта, ОбъектыПечати = Неопределено, НумероватьСтраницы = Ложь, НомерПервойСтраницы = 0, НомерПоследнейСтраницы = 0, ДатаПодписи = Неопределено) Экспорт
	...
	Пока ВыборкаПоШапкеДокумента.СледующийПоЗначениюПоля("Ссылка") Цикл 
		...
		Если ВыборкаПоШапкеДокумента.ОтчетныйПериод < '20150401' Тогда
			Макет = УправлениеПечатью.МакетПечатнойФормы("Документ.ПачкаРазделов6РасчетаРСВ_1.ФормаРаздел6");
		Иначе
			Макет = УправлениеПечатью.МакетПечатнойФормы("Документ.ПачкаРазделов6РасчетаРСВ_1.ФормаРаздел6_2015");
		КонецЕсли;
		...
		Справочники.КомплектыОтчетностиПерсУчета.ДобавитьТабличныйДокументВКоллекциюПечатаемыхОбъектов(Результаты, ДокументРезультат, ВыборкаПоШапкеДокумента);
		
		ДокументРезультат.КлючПараметровПечати = "ПАРАМЕТРЫ_ПЕЧАТИ_ПачкаРазделов6РасчетаРСВ_1";
		...
		Иначе 
			ИнформацияОПачке.Вставить("КодКорректируемогоПериода", "");	
		КонецЕсли;
		...
		Иначе
			...
			ПерсонифицированныйУчет.ВывестиДанныеПоБуквенно("", ОбластьШапка, "РегистрационныйНомерПФРвКорректируемыйПериод", 12);
			ПерсонифицированныйУчет.ВывестиДанныеПоБуквенно("", ОбластьШапка, "КорректируемыйГод", 4);
		КонецЕсли;
			...
			Пока ВыборкаПоРаботникам.СледующийПоЗначениюПоля("НомерСтроки") Цикл
				...
				Если ВыборкаПоКорректирующимСведениям.НайтиСледующий(СтруктураПоиска) Тогда
					...
					Пока ВыборкаПоКорректирующимСведениям.Следующий() Цикл
						...
						ОбластьКорректирующиеСведенияСтрока.Параметры.КодКорректируемогоПериода = ПерсонифицированныйУчет.КодПериодаДляРасчетаРСВ1(ВыборкаПоКорректирующимСведениям.КорректируемыйПериод);
				...
				Если НумероватьСтраницы Тогда 
					Пока ТекущийЛист.ПроверитьВывод(ВыводимыеОбласти) Цикл
...
Процедура ВывестиОбласть(ДокументРезультат, ТекущийЛист = Неопределено, ВыводимаяОбласть, ЗавершатьТекущийЛист = Ложь, ПроверятьВывод = Ложь, ПроверяемыеОбласти = Неопределено)
	Если ТекущийЛист = Неопределено Тогда
		Если ПроверятьВывод Тогда
		    Если Не ДокументРезультат.ПроверитьВывод(ПроверяемыеОбласти) Тогда
	...
	Иначе	
		...
		Если ПроверятьВывод Тогда
		    Если Не ТекущийЛист.ПроверитьВывод(ПроверяемыеОбласти) Тогда
...
Функция СформироватьПечатнуюФормуРаздела6РСВ_1(МассивОбъектов, ОбъектыПечати, НумероватьСтраницы = Ложь, НомерПервойСтраницы = 0, НомерПоследнейСтраницы = 0) Экспорт
...
Процедура ВывестиОбластьСведенияОЗаработке(ТабличныйДокумент, ТекущийЛист, ОбластьЗаработок, ВыборкаПоСтрокамЗаработка, Ссылка, Сотрудник, ОтчетныйПериод, ИмяПоляГруппировки, ВременнаяТаблица, ОбластьЗаработокПустая, ОбластьЗаработокПустаяЧасть2)
	Если ТекущийЛист = Неопределено Тогда
		...
		Пока ВыборкаПоСтрокамЗаработка.СледующийПоЗначениюПоля(ИмяПоляГруппировки) Цикл			
			...
			Для каждого СтрокаТЗ Из ВременнаяТаблица Цикл
				...
				Если МесяцПериода > ПредыдущийМесяц + 1 Тогда
					Для СчПараметр = 0 По ОбластьЗаработок.Параметры.Количество() - 1 Цикл
						ОбластьЗаработок.Параметры[СчПараметр] = ""
					КонецЦикла;
					Для СчМесяцев = ПредыдущийМесяц + 1 По МесяцПериода - 1 Цикл
						ОбластьЗаработок.Параметры.НазваниеСтроки = "" + СчМесяцев + " месяц";
						ОбластьЗаработок.Параметры.КодСтроки = "" + ВтораяЦифраКода + "" + СчМесяцев;
						ВывестиОбласть(ТабличныйДокумент, ТекущийЛист, ОбластьЗаработок);
					КонецЦикла;
				КонецЕсли;
				ОбластьЗаработок.Параметры.Заполнить(СтрокаТЗ);
				Если МесяцПериода = 0 Тогда
					ОбластьЗаработок.Параметры.НазваниеСтроки = "Всего с начала расчетного периода, в том числе за последние три месяца отчетного периода:";
				Иначе
					ОбластьЗаработок.Параметры.НазваниеСтроки = "" + МесяцПериода + " месяц";
				КонецЕсли;
				ОбластьЗаработок.Параметры.КодСтроки = "" + ВтораяЦифраКода + "" + МесяцПериода;
				ВывестиОбласть(ТабличныйДокумент, ТекущийЛист, ОбластьЗаработок);
