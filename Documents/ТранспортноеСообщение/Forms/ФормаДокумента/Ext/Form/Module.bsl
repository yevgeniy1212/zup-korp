...
Процедура СодержимоеВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	// отчетность в ЦБ
	ИначеЕсли ТипДокумента = "ФайлОтчетностиЦБ" ИЛИ ТипДокумента = "ПротоколПриложениеЦБ" Тогда
		ПоказатьПриложениеПФР(Элемент.ТекущиеДанные);
	ИначеЕсли ТипДокумента = "ПодтверждениеЗагрузкиЦБ"
		ИЛИ ТипДокумента = "УведомлениеОбОшибкеЗагрузкиЦБ"
		ИЛИ ТипДокумента = "КвитанцияОПриемеВОбработкуОтчетаЦБ"
		ИЛИ ТипДокумента = "УведомлениеОбОтказеОбработкиОтчетаЦБ"
		ИЛИ ТипДокумента = "КвитанцияОРезультатахОбработкиОтчетаЦБ"
		ИЛИ ТипДокумента = "ПротоколЦБ"
		ИЛИ ТипДокумента = "КвитанцияПриложениеЦБ"
		ИЛИ ТипДокумента = "ДоверенностьЦБ" Тогда
		ПоказатьКакXML(ВыбраннаяСтрока);
...
&НаКлиенте
Процедура ОтветыНаЗапросыИОН(Команда)
	
	Запрос = КонтекстЭДОКлиент.ПолучитьПредметыЦиклаОбмена(Объект.ЦиклОбмена);
	СтруктураОтбора = Новый Структура("Запрос", Запрос);
	ОткрытьФорму("РегистрСведений.ОтветыНаЗапросыИОН.Форма.ФормаСписка", Новый Структура("Отбор", СтруктураОтбора));
...
Процедура ОбновитьДанные()
	...
	ИначеЕсли Объект.Тип = Перечисления.ТипыТранспортныхСообщений.УведомлениеОбОшибкеПФР
		...
		ИЛИ Объект.Тип = Перечисления.ТипыТранспортныхСообщений.УведомлениеОбОшибкеФСГС
		ИЛИ Объект.Тип = Перечисления.ТипыТранспортныхСообщений.УведомлениеОбОшибкеЦБ Тогда
		...
		ИначеЕсли Объект.Тип = Перечисления.ТипыТранспортныхСообщений.УведомлениеОбОшибкеЦБ Тогда
			ФайлыОшибкиОбработкиПакета = Вложения.НайтиСтроки(Новый Структура("Тип", Перечисления.ТипыСодержимогоТранспортногоКонтейнера.ОписаниеОшибкиЦБ));
		КонецЕсли;
	...
	// ЦБ
	ИначеЕсли Объект.Тип = Перечисления.ТипыТранспортныхСообщений.ПервичноеСообщениеСодержащееОтчетностьЦБ Тогда
		Элементы.ГруппаСодержимоеСтраницы.ТекущаяСтраница = Элементы.СодержимоеДерево;
		ЗаполнитьДеревоСодержимого(Вложения, СодержимоеТранспортныхСообщений, Элементы.СодержимоеТранспортныхСообщений);
		Элементы.ГруппаСостояниеСообщения.Видимость = Ложь;
		
	ИначеЕсли Объект.Тип = Перечисления.ТипыТранспортныхСообщений.РезультатЗагрузкиЦБ Тогда
		Элементы.ГруппаСодержимоеСтраницы.ТекущаяСтраница = Элементы.СодержимоеДерево;
		Элементы.ГруппаСостояниеСообщения.Видимость = Истина;
		ЗаполнитьДеревоСодержимого(Вложения, СодержимоеТранспортныхСообщений, Элементы.СодержимоеТранспортныхСообщений);
		
	ИначеЕсли Объект.Тип = Перечисления.ТипыТранспортныхСообщений.РезультатПриемаДокументЦБ Тогда
		Элементы.ГруппаСодержимоеСтраницы.ТекущаяСтраница = Элементы.СодержимоеДерево;
		Элементы.ГруппаСостояниеСообщения.Видимость = Истина;
		ЗаполнитьДеревоСодержимого(Вложения, СодержимоеТранспортныхСообщений, Элементы.СодержимоеТранспортныхСообщений);
		
	ИначеЕсли Объект.Тип = Перечисления.ТипыТранспортныхСообщений.РезультатОбработкиДокументЦБ Тогда
		Элементы.ГруппаСодержимоеСтраницы.ТекущаяСтраница = Элементы.СодержимоеДерево;
		Элементы.ГруппаСостояниеСообщения.Видимость = Истина;
		ЗаполнитьДеревоСодержимого(Вложения, СодержимоеТранспортныхСообщений, Элементы.СодержимоеТранспортныхСообщений);
	КонецЕсли;
	...
	ЭтоОтправкаВСФР = ДокументооборотСКОКлиентСервер.ЭтоОтправкаВСФР(
		?(ЗначениеЗаполнено(Объект.ЦиклОбмена), Объект.ЦиклОбмена.Предмет, Неопределено),,
		?(ЗначениеЗаполнено(Объект.ЦиклОбмена), Объект.ЦиклОбмена.ДатаСоздания, Объект.ДатаТранспорта),
		?(ЗначениеЗаполнено(Объект.ЦиклОбмена), Объект.ЦиклОбмена.Вид, Неопределено),
		?(ЗначениеЗаполнено(Объект.ЦиклОбмена), Объект.ЦиклОбмена.Тип, Неопределено));
	ЗаголовокТипаСообщения = КонтекстЭДОСервер.СформироватьСтрокуСостоянияТранспортногоСообщения(Объект);
	Элементы.НадписьПодробнаяИнформация.Заголовок = ДокументооборотСКОКлиентСервер.ЗаменитьПФРиФССнаСФР(
		ЗаголовокТипаСообщения, Истина,, НЕ ЭтоОтправкаВСФР);
	ТипТекстом = ДокументооборотСКОКлиентСервер.ЗаменитьПФРиФССнаСФР(
		Строка(Объект.Тип), Истина,, НЕ ЭтоОтправкаВСФР);
...
Процедура ЗаполнитьДеревоСодержимого(ВложенияТекущегоСообщения, Содержимое, ТПСодержимого)
	...
	// отчетность ЦБ
	ТипыДИВ.Добавить(Перечисления.ТипыСодержимогоТранспортногоКонтейнера.ФайлОтчетностиЦБ);
	ТипыДИВ.Добавить(Перечисления.ТипыСодержимогоТранспортногоКонтейнера.ПодтверждениеЗагрузкиЦБ);
	ТипыДИВ.Добавить(Перечисления.ТипыСодержимогоТранспортногоКонтейнера.УведомлениеОбОшибкеЗагрузкиЦБ);
	ТипыДИВ.Добавить(Перечисления.ТипыСодержимогоТранспортногоКонтейнера.КвитанцияОПриемеВОбработкуОтчетаЦБ);
	ТипыДИВ.Добавить(Перечисления.ТипыСодержимогоТранспортногоКонтейнера.УведомлениеОбОтказеОбработкиОтчетаЦБ);
	ТипыДИВ.Добавить(Перечисления.ТипыСодержимогоТранспортногоКонтейнера.КвитанцияПриложениеЦБ);
	ТипыДИВ.Добавить(Перечисления.ТипыСодержимогоТранспортногоКонтейнера.КвитанцияОРезультатахОбработкиОтчетаЦБ);
	ТипыДИВ.Добавить(Перечисления.ТипыСодержимогоТранспортногоКонтейнера.ПротоколЦБ);
	ТипыДИВ.Добавить(Перечисления.ТипыСодержимогоТранспортногоКонтейнера.ПротоколПриложениеЦБ);
	ТипыДИВ.Добавить(Перечисления.ТипыСодержимогоТранспортногоКонтейнера.ДоверенностьЦБ);
	...
	// отчетность ЦБ
	ПриоритетыПрорисовкиДИВ.Вставить(Перечисления.ТипыСодержимогоТранспортногоКонтейнера.ФайлОтчетностиЦБ, 3);
	ПриоритетыПрорисовкиДИВ.Вставить(Перечисления.ТипыСодержимогоТранспортногоКонтейнера.ПодтверждениеЗагрузкиЦБ, 1);
	ПриоритетыПрорисовкиДИВ.Вставить(Перечисления.ТипыСодержимогоТранспортногоКонтейнера.УведомлениеОбОшибкеЗагрузкиЦБ, 1);
	ПриоритетыПрорисовкиДИВ.Вставить(Перечисления.ТипыСодержимогоТранспортногоКонтейнера.КвитанцияОПриемеВОбработкуОтчетаЦБ, 1);
	ПриоритетыПрорисовкиДИВ.Вставить(Перечисления.ТипыСодержимогоТранспортногоКонтейнера.УведомлениеОбОтказеОбработкиОтчетаЦБ, 1);
	ПриоритетыПрорисовкиДИВ.Вставить(Перечисления.ТипыСодержимогоТранспортногоКонтейнера.КвитанцияПриложениеЦБ, 2);
	ПриоритетыПрорисовкиДИВ.Вставить(Перечисления.ТипыСодержимогоТранспортногоКонтейнера.КвитанцияОРезультатахОбработкиОтчетаЦБ, 1);
	ПриоритетыПрорисовкиДИВ.Вставить(Перечисления.ТипыСодержимогоТранспортногоКонтейнера.ПротоколЦБ, 1);
	ПриоритетыПрорисовкиДИВ.Вставить(Перечисления.ТипыСодержимогоТранспортногоКонтейнера.ПротоколПриложениеЦБ, 2);
	ПриоритетыПрорисовкиДИВ.Вставить(Перечисления.ТипыСодержимогоТранспортногоКонтейнера.ДоверенностьЦБ, 4);
	...
	Если ИменаПодписанныхФайловИзДругихСообщений.Количество() > 0 Тогда
		...
		Если НЕ ЗначениеЗаполнено(СообщениеВладелец) Тогда
			...
			ИначеЕсли Объект.Тип = Перечисления.ТипыСодержимогоТранспортногоКонтейнера.ПодтверждениеЗагрузкиЦБ
				ИЛИ Объект.Тип = Перечисления.ТипыСодержимогоТранспортногоКонтейнера.УведомлениеОбОшибкеЗагрузкиЦБ
				ИЛИ Объект.Тип = Перечисления.ТипыСодержимогоТранспортногоКонтейнера.КвитанцияОПриемеВОбработкуОтчетаЦБ
				ИЛИ Объект.Тип = Перечисления.ТипыСодержимогоТранспортногоКонтейнера.УведомлениеОбОтказеОбработкиОтчетаЦБ
				ИЛИ Объект.Тип = Перечисления.ТипыСодержимогоТранспортногоКонтейнера.КвитанцияОРезультатахОбработкиОтчетаЦБ
				ИЛИ Объект.Тип = Перечисления.ТипыСодержимогоТранспортногоКонтейнера.ПротоколЦБ Тогда
				
				СообщениеВладелец = КонтекстЭДОСервер.ПолучитьСообщениеЦиклаОбмена(Объект.ЦиклОбмена,
					Перечисления.ТипыТранспортныхСообщений.ПервичноеСообщениеСодержащееОтчетностьЦБ);
			КонецЕсли;
...
Процедура ОформитьСтрокуСодержимого(ДанныеСтроки)
	...
	// типы ЦБ
	ИначеЕсли ТипДанных = Перечисления.ТипыСодержимогоТранспортногоКонтейнера.ФайлОтчетностиЦБ Тогда
		
		ПредставлениеЯчейки = НСтр("ru = 'Файл отчетности'");
		Если ЗначениеЗаполнено(ДанныеСтроки.ТипФайлаОтчетности) Тогда
			ПредставлениеЯчейки = ПредставлениеЯчейки + " (" + ДанныеСтроки.ТипФайлаОтчетности + ")";
		КонецЕсли;
		
	ИначеЕсли ТипДанных = Перечисления.ТипыСодержимогоТранспортногоКонтейнера.ПодтверждениеЗагрузкиЦБ Тогда
		ПредставлениеЯчейки = НСтр("ru = 'Подтверждение загрузки документа'");
	ИначеЕсли ТипДанных = Перечисления.ТипыСодержимогоТранспортногоКонтейнера.УведомлениеОбОшибкеЗагрузкиЦБ Тогда
		ПредставлениеЯчейки = НСтр("ru = 'Уведомление об ошибке загрузки документа'");
	ИначеЕсли ТипДанных = Перечисления.ТипыСодержимогоТранспортногоКонтейнера.КвитанцияОПриемеВОбработкуОтчетаЦБ Тогда
		ПредставлениеЯчейки = НСтр("ru = 'Квитанция о приеме в обработку'");
	ИначеЕсли ТипДанных = Перечисления.ТипыСодержимогоТранспортногоКонтейнера.УведомлениеОбОтказеОбработкиОтчетаЦБ Тогда
		ПредставлениеЯчейки = НСтр("ru = 'Уведомление об отказе от приема в обработку'");
	ИначеЕсли ТипДанных = Перечисления.ТипыСодержимогоТранспортногоКонтейнера.КвитанцияПриложениеЦБ Тогда
		ПредставлениеЯчейки = НСтр("ru = 'Приложение к квитанции'");
	ИначеЕсли ТипДанных = Перечисления.ТипыСодержимогоТранспортногоКонтейнера.КвитанцияОРезультатахОбработкиОтчетаЦБ Тогда
		ПредставлениеЯчейки = НСтр("ru = 'Квитанция о результатах обработки'");
	ИначеЕсли ТипДанных = Перечисления.ТипыСодержимогоТранспортногоКонтейнера.ПротоколЦБ Тогда
		ПредставлениеЯчейки = НСтр("ru = 'Протокол ошибок'");
	ИначеЕсли ТипДанных = Перечисления.ТипыСодержимогоТранспортногоКонтейнера.ПротоколПриложениеЦБ Тогда
		ПредставлениеЯчейки = НСтр("ru = 'Приложение к протоколу ошибок'");
	ИначеЕсли ТипДанных = Перечисления.ТипыСодержимогоТранспортногоКонтейнера.ДоверенностьЦБ Тогда
		ПредставлениеЯчейки = НСтр("ru = 'Доверенность'");
	...
	ИначеЕсли ТипДанных = Перечисления.ТипыСодержимогоТранспортногоКонтейнера.ЭлектронноЦифроваяПодпись Тогда
		...
		Если ДанныеСтроки.ЭЦПЭтоПодписьАбонента Тогда
			...
			КонецЕсли;
		ИначеЕсли Объект.ЦиклОбмена.Вид = Перечисления.ВидыЦикловОбмена.ЦиклОбменаСЦБ Тогда
			ПредставлениеЯчейки = НСтр("ru = 'Электронная подпись представителя ЦБ'");
		...
		Иначе
			...
			Иначе
				...
				Иначе
					...
					// Реестр по НДС (приложение 1)
					Если Объект.Тип = Перечисления.ТипыТранспортныхСообщений.ПредставлениеНП 
						И Объект.ЦиклОбмена.ВидОтчета = Справочники.ВидыОтправляемыхДокументов.РеестрНДСПриложение1 Тогда
							ПредставлениеЯчейки = "Электронная подпись РЭЦ";
						КонецЕсли;
					// Реестр по НДС (приложение 5)
					Если Объект.Тип = Перечисления.ТипыТранспортныхСообщений.ПредставлениеНП 
						И Объект.ЦиклОбмена.ВидОтчета = Справочники.ВидыОтправляемыхДокументов.РеестрНДСПриложение5 Тогда
							ПредставлениеЯчейки = "Электронная подпись РЭЦ";
						КонецЕсли;
					// Декларация по НДС
					Если Объект.Тип = Перечисления.ТипыТранспортныхСообщений.ДекларацияНП 
						И Объект.ЦиклОбмена.ВидОтчета = Справочники.ВидыОтправляемыхДокументов.НДС Тогда
							ПредставлениеЯчейки = "Электронная подпись РЭЦ";
					КонецЕсли;
				КонецЕсли;
			КонецЕсли;
...
Процедура НадписьСостояниеРезультатПриемаНажатиеВопросЗавершение(Ответ, ДополнительныеПараметры) Экспорт
	...
	КонтекстЭДОКлиент.СоздатьРезультатПриемаНаКлиентеСУчетомВопроса(Оповещение, Объект.Ссылка, ПоложительныйРезультат);		
...
Процедура ПоказатьЭлектронноЦифровуюПодпись(ВыбраннаяСтрокаИД)
	...
	Если Строка(ВыбраннаяСтрока.ЭЦПЭтоПодписьАбонента) = Строка(Истина) Тогда
...
Процедура ПоказатьКакXML(СвойстваДокумента, ПолеНастроекПечати = Неопределено)
	...
	Если ЭтоВеб ИЛИ ОбщегоНазначенияКлиент.ЭтоLinuxКлиент() ИЛИ ОбщегоНазначенияКлиент.ЭтоMacOSКлиент() Тогда
...
&НаСервере
Процедура ДобавитьКомандуИОН()
	
	ЭтоИОН = Объект.ЦиклОбмена.Тип = Перечисления.ТипыЦикловОбмена.ИОН;
	ЭтоТестирование = ДокументооборотСКОВызовСервера.ИспользуетсяРежимТестирования();
	
	Элементы.ФормаОтветыНаЗапросыИОН.Видимость = ЭтоИОН И ЭтоТестирование;
