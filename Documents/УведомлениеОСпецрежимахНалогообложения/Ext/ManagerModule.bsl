		...
		Если Метаданные.Отчеты.Найти(ОтчетМенеджер) <> Неопределено Тогда 
			...
			Иначе
				...
				Для Каждого Стр Из ТФ Цикл 
					Если КонецДня(Стр.ДатаКонца) > ТДС И НачалоДня(Стр.ДатаНачала) < ТДС Тогда 
...
Функция ПолучитьБлокируемыеРеквизитыОбъекта() Экспорт
	...
	БлокируемыеРеквизиты.Добавить("ГруппаКнопокСохранитьИЗаписать;ГруппаКнопокСохранитьИЗаписать");
	БлокируемыеРеквизиты.Добавить("ГруппаКнопокЗаписатьЗакрыть;ГруппаКнопокЗаписатьЗакрыть");
	БлокируемыеРеквизиты.Добавить("ГруппаКнопок1;ГруппаКнопок1");
	
	БлокируемыеРеквизиты.Добавить("ФормаЗаписатьИЗакрыть;ФормаЗаписатьИЗакрыть");
	...
	БлокируемыеРеквизиты.Добавить("Комментарий;Комментарий");
...
Функция РегистрацияВФНСОрганизации(Организация, ДатаПодписи = Неопределено, КодНО = Неопределено) Экспорт
	Реквизиты = РегламентированнаяОтчетностьВызовСервера.ПолучитьСведенияОбОрганизации(
		Организация, ДатаПодписи, "КодНО,ПризнакНПЛКрупнейший, КодНОКрупнейший");
	Если Не Реквизиты.ПризнакНПЛКрупнейший = Истина 
		Или Не ЗначениеЗаполнено(Реквизиты.КодНОКрупнейший)
		Или ЗначениеЗаполнено(КодНО) Тогда 
		
		Реквизиты.КодНОКрупнейший = ?(ЗначениеЗаполнено(КодНО), КодНО, Реквизиты.КодНО);
	КонецЕсли;
...
Процедура ЗаполнитьДанныеНПФЛ(Объект, ОсновныеСведения) 
	...
	Попытка
		ОсновныеСведения.Вставить("КодИФНСПосредника",
			?(Объект.Ссылка.Организация.КрупнейшийНалогоплательщик,
			СокрЛП(Объект.Ссылка.Организация.КодНалоговогоОрганаПолучателя), ""));
	Исключение
	КонецПопытки;
...
Процедура ЗаполнитьДанныеНПЮЛ(Объект, ОсновныеСведения) 
	...
	Если Реквизиты.ПризнакНПЛКрупнейший = Истина И ЗначениеЗаполнено(Реквизиты.КодНОКрупнейший) Тогда 
...
Процедура ОбработкаОбновленияПриПереходеС82(Параметры = Неопределено) Экспорт
	Попытка
		...
		СоответствиеОбъектов = УведомлениеОСпецрежимахНалогообложенияПовтИсп.СоответствиеОбъектовПриКонвертацииС82();
		МассивДляПреобразования = УведомлениеОСпецрежимахНалогообложенияПовтИсп.МассивДляПреобразованияПриКонвертацииС82();
		...
		|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.СоответствиеРегОтчетовУведомлениям КАК СоответствиеРегОтчетовУведомлениям
		|		ПО СоответствиеРегОтчетовУведомлениям.РегОтчет = РегламентированныйОтчет.Ссылка
		|ГДЕ
		|	НЕ РегламентированныйОтчет.ПометкаУдаления
		|	И ЕСТЬNULL(СоответствиеРегОтчетовУведомлениям.РегОтчет, 0) = 0
...
Процедура ИзменениеИменМакетовПФ(Параметры = Неопределено) Экспорт
	Попытка
		...
		МассивДляПреобразования.Добавить("РегламентированноеУведомлениеФормаР13014");
...
Процедура ЗаполнениеТребованийОснований(Параметры = Неопределено) Экспорт
	Попытка
		МассивТребований = Новый Массив;
		МассивТребований.Добавить(Перечисления.ВидыУведомленийОСпецрежимахНалогообложения.НевозможностьПредоставленияДокументов);
		МассивТребований.Добавить(Перечисления.ВидыУведомленийОСпецрежимахНалогообложения.РеестрДокументовПодтверждающихЛьготы);
		...
		|	УведомлениеОСпецрежимахНалогообложения.Ссылка КАК Ссылка,
		|	УведомлениеОСпецрежимахНалогообложения.Основание КАК Основание,
		|	УведомлениеОСпецрежимахНалогообложения.ДанныеУведомления КАК ДанныеУведомления
		|ИЗ
		|	Документ.УведомлениеОСпецрежимахНалогообложения КАК УведомлениеОСпецрежимахНалогообложения
		|ГДЕ
		|	УведомлениеОСпецрежимахНалогообложения.ВидУведомления В(&МассивТребований)";
		...
		Пока Выборка.Следующий() Цикл 
			...
			Если ДанныеУведомления.СтруктураРеквизитов.Свойство("_Требование")
				И ЗначениеЗаполнено(ДанныеУведомления.СтруктураРеквизитов._Требование) Тогда 
				УведомлениеОбъект = Выборка.Ссылка.ПолучитьОбъект();
				УведомлениеОбъект.Основание = ДанныеУведомления.СтруктураРеквизитов._Требование;
				УведомлениеОбъект.Записать();
			КонецЕсли;
		КонецЦикла;
	Исключение
		Ошибка = ИнформацияОбОшибке();
		СтрОш = НСтр("ru = 'Ошибка при заполнении документов-оснований требования'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
		ЗаписьЖурналаРегистрации(СтрОш, УровеньЖурналаРегистрации.Ошибка,,, ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
	КонецПопытки;
КонецПроцедуры
