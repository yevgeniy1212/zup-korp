...
Функция ПечатнаяФорма(Сверка) Экспорт
	...
	Иначе
		Макет = "Сверка5_04";
	КонецЕсли;
	Бланк = Документы.ЗапросНаИнформационноеОбслуживаниеНалогоплательщика.ПолучитьМакет(Макет);
	...
	ВывестиТаблицуКБК(Сверка, ТабДокумент, Бланк, ПараметрыЗапроса);
...
Функция ВывестиТаблицуКБК(Сверка, ТабДокумент, Бланк, ПараметрыЗапроса)
	...
	ВыводитьКБК = 
		ЭтоФормат5_05 
		И Сверка.ЗапрашиваемыеНалоги.Количество() > 0 
		И ЭтоАкт
		ИЛИ НЕ ЭтоФормат5_05;
		
	Если ВыводитьКБК Тогда
		
		Если ЭтоФормат5_05 Тогда
			ШапкаПереченьКБК = Бланк.ПолучитьОбласть("ШапкаПереченьКБК");
			ТабДокумент.Вывести(ШапкаПереченьКБК);
		КонецЕсли;
...
Процедура ЗаполнитьВерхнююЧастьСверки(Сверка, ТабДокумент, Бланк, ПараметрыЗапроса)
	...
	Если ЭтоФормат5_05 Тогда
		...
		ЗаполнитьПериод5_05(Сверка, Бланк, ВерхняяЧастьБланка, ПараметрыЗапроса);
	Иначе
		...
		ЗаполнитьПериод5_04(Сверка, Бланк, ВерхняяЧастьБланка, ПараметрыЗапроса);
	КонецЕсли;
...
Процедура ЗаполнитьПризнакКБК(Сверка, ВерхняяЧастьБланка)

	Если Сверка.ВидУслуги = Перечисления.ВидыУслугПриИОН.ПредставлениеАктовСверкиРасчетов Тогда
		Если Сверка.ЗапрашиваемыеНалоги.Количество() = 0 Тогда
			ВерхняяЧастьБланка.Параметры["ПризнакКБК"] = "1";
		Иначе
			ВерхняяЧастьБланка.Параметры["ПризнакКБК"] = "2";
		КонецЕсли;
	КонецЕсли;
...
Процедура ЗаполнитьПериод5_05(Сверка, Бланк, ВерхняяЧастьБланка, ПараметрыЗапроса)
...
Процедура ЗаполнитьПериод5_04(Сверка, Бланк, ВерхняяЧастьБланка, ПараметрыЗапроса)
...
Процедура ЗаполнитьТабличнуюЧастьСверки(Сверка, ТабДокумент, Бланк, ПараметрыЗапроса)
	...
	Для каждого Налог Из Сверка.ЗапрашиваемыеНалоги Цикл
		...
		Если ЭтоФормат5_05 И ЭтоАкт Тогда
