...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	ФизическиеЛица = Получатели.ВыгрузитьКолонку("ФизическоеЛицо");
	Выборка = ВыборкаКонфликтующихРегистраторов(Ссылка, ДатаНачала, Организация, ВидПрочегоДохода, ФизическиеЛица);
	...
	Если Действие = Перечисления.ДействияСПрочимиДоходами.Прекратить Тогда
		Выборка = ВыборкаОтменяемыхБезОснованийДоходов(Ссылка, ДатаНачала, Организация, ВидПрочегоДохода, ФизическиеЛица);
		Пока Выборка.Следующий() Цикл
		
			ТекстОшибки = НСтр("ru = 'У сотрудника %1 отсутствует плановое начисление дохода %2.'");
			ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ТекстОшибки, Выборка.ФизическоеЛицо, ВидПрочегоДохода);
...
Функция ВыборкаОтменяемыхБезОснованийДоходов(Регистратор, Месяц, Организация, ВидПрочегоДохода, ФизическиеЛица)
	Запрос = Новый Запрос;
	
	Запрос.Текст =
	"ВЫБРАТЬ РАЗРЕШЕННЫЕ РАЗЛИЧНЫЕ
	|	ПРЕДСТАВЛЕНИЕССЫЛКИ(ФизическиеЛица.Ссылка) КАК ФизическоеЛицо
	|ИЗ
	|	Справочник.ФизическиеЛица КАК ФизическиеЛица
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ПлановыеПрочиеДоходы.СрезПоследних(
	|				&Период,
	|				Организация = &Организация
	|					И (Используется
	|						ИЛИ ИспользуетсяПоОкончании)
	|					И Регистратор <> &Регистратор
	|					И ВидПрочегоДохода = &ВидПрочегоДохода
	|					И ФизическоеЛицо В (&ФизическиеЛица)) КАК ПлановыеПрочиеДоходы
	|		ПО (ПлановыеПрочиеДоходы.ФизическоеЛицо = ФизическиеЛица.Ссылка)
	|ГДЕ
	|	ФизическиеЛица.Ссылка В(&ФизическиеЛица)
	|	И ПлановыеПрочиеДоходы.ФизическоеЛицо ЕСТЬ NULL";
...
Процедура СформироватьДвиженияПлановыхПрочихДоходов(Движения, ДанныеПлановыхПрочихДоходов)
	...
	|	ПлановыеПрочиеДоходы.ПериодВозвратногоСобытия как ДействуетДо
	|ИЗ
	...
	Для Каждого СтрокаДанных Из ДанныеПлановыхПрочихДоходов Цикл
		...
		Если ЗначениеЗаполнено(СтрокаДанных.ДействуетДо) Тогда
			...
			ВидДоходаИспользовался = Ложь;
			РазмерПоОкончании = 0;
			Для Каждого ДействующаяСтрока Из НайденныеСтроки Цикл
				Если ДействующаяСтрока.ДействуетДо <= СтрокаДанных.ДействуетДо Тогда
					Продолжить;
				КонецЕсли;
				ВидДоходаИспользовался = Истина;
				РазмерПоОкончании = ДействующаяСтрока.Размер;
			КонецЦикла;
			
			СтрокаДвижений.ИспользуетсяПоОкончании = ВидДоходаИспользовался;
			
			Если ВидДоходаИспользовался Тогда 
				СтрокаДвижений.РазмерПоОкончании = РазмерПоОкончании;
			КонецЕсли;
