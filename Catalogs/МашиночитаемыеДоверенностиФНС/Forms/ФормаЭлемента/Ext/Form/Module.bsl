...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	КоличествоВидовПолномочийПредставителя =
		Справочники.МашиночитаемыеДоверенностиФНС.КоличествоВидовПолномочийПредставителя();
...
Процедура ПослеЗаписи(ПараметрыЗаписи)
	...
	Оповестить("Запись_МашиночитаемыеДоверенностиФНС",, Объект.Ссылка);
...
Процедура ДекорацияРасширеннаяНастройкаСрокаДействияНажатие(Элемент)
	...
	ОписаниеОповещения = Новый ОписаниеОповещения("ДекорацияРасширеннаяНастройкаСрокаДействияНажатиеПослеВвода",
		ЭтотОбъект);
	ОткрытьФорму(
		"Справочник.МашиночитаемыеДоверенностиФНС.Форма.ФормаОкончанияСрокаДействия",
...
Процедура ПредставительПриИзменении(Элемент)
	...
	ПодписьНеЗагруженаИПредставительЗаданСтрокой = (НЕ ПодписьЗагружена И ТипЗнч(Объект.Представитель) = Тип("Строка"));
	
	ПодставитьРеквизитыПредставителя();
	
	Если ПодписьНеЗагруженаИПредставительЗаданСтрокой Тогда
...
Процедура ПредставительЯвляетсяСотрудникомПриИзменении(Элемент)
	...
	Иначе
		...
		Если НЕ Доверитель_ЮридическоеЛицо И НЕ Представитель_ЮридическоеЛицо Тогда
			Если ПредставительЯвляетсяСотрудником Тогда
				Элементы.Представитель.ОграничениеТипа = Новый ОписаниеТипов;
				Объект.Представитель = ПредставительФЛ_ФИО; // строковое заполнение указывает на сотрудника
			Иначе
...
Процедура ДекорацияПолномочияПредставителяНажатие(Элемент)
	...
	ОткрытьФорму(
		"Справочник.МашиночитаемыеДоверенностиФНС.Форма.ФормаВводаПолномочий",
		Новый Структура("СтруктураДанных", СтруктураДанных),
		ЭтаФорма,,,,
		ОписаниеОповещения,
		РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
...
Процедура Подписать(Команда)
	...
	ОписаниеОповещения = Новый ОписаниеОповещения("ПодписатьПослеПодтвержденияПриПредупреждении", ЭтотОбъект);
	Если ЗначениеЗаполнено(Объект.ДатаОкончания)
		И Объект.ДатаОкончания < НачалоДня(ОбщегоНазначенияКлиент.ДатаСеанса()) Тогда
		
		ПоказатьВопрос(
			ОписаниеОповещения,
			НСтр("ru = 'Дата окончания действия доверенности меньше текущей, продолжить подписание?'"),
			РежимДиалогаВопрос.ДаНет,,
			КодВозвратаДиалога.Нет);
...
Процедура Выгрузить(Команда)
	...
	Если НЕ ПодписьЗагружена Тогда
		РезультатыПроверки = ДокументооборотСКОВызовСервера.ПроверитьМЧД(Объект);
	...
	Если ПодписьЗагружена ИЛИ УведомлениеОтправлено Тогда
		ПоказатьВопрос(
			ОписаниеОповещения,
...
Процедура ВыгрузитьПослеПодтвержденияВыгрузкиПодписи(Результат, ДополнительныеПараметры) Экспорт
	...
	ПараметрыВозврата.Вставить("ПроверитьЧтоУведомлениеОтправлено", 	УведомлениеОтправлено);
...
Процедура Отозвать(Команда)
	...
	ПараметрыФормы = Новый Структура;
	ПараметрыФормы.Вставить("Доверенность", Объект.Ссылка);
	ОткрытьФорму("Справочник.ЗаявленияОбОтзывеМЧДФНС.ФормаОбъекта", ПараметрыФормы);	
...
Процедура Инициализация()
	...
	Представитель_ЮридическоеЛицо = ДокументооборотСКОВызовСервера.ЭтоЮридическоеЛицо(Объект.Представитель,
		ЗначениеЗаполнено(Объект.ПредставительЮЛ_ИНН));
	...
	ПредставительЯвляетсяСотрудником = НЕ ЗначениеЗаполнено(Объект.Представитель)
...
Процедура ВвестиРеквизитыДоверителя()
	...
	ОткрытьФорму(
		"Справочник.МашиночитаемыеДоверенностиФНС.Форма.ФормаДоверителя",
		Новый Структура("СтруктураДанных", СтруктураДанных),
		ЭтаФорма,,,,
		ОписаниеОповещения,
		РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
...
Процедура ВвестиРеквизитыЛицаБезДоверенностиЮЛ()
	...
	ОписаниеОповещения = Новый ОписаниеОповещения("ВвестиРеквизитыЛицаБезДоверенностиЮЛПослеВвода",
		ЭтотОбъект);
	ОткрытьФорму(
		"Справочник.МашиночитаемыеДоверенностиФНС.Форма.ФормаЛицаБезДоверенностиЮЛ",
		Новый Структура("СтруктураДанных", СтруктураДанных),
		ЭтаФорма,,,,
		ОписаниеОповещения,
		РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
...
Процедура ВвестиРеквизитыЛицаБезДоверенностиФЛ()
	...
	ОписаниеОповещения = Новый ОписаниеОповещения("ВвестиРеквизитыЛицаБезДоверенностиФЛПослеВвода",
		ЭтотОбъект);
	ОткрытьФорму(
		"Справочник.МашиночитаемыеДоверенностиФНС.Форма.ФормаЛицаБезДоверенностиФЛ",
		Новый Структура("СтруктураДанных", СтруктураДанных),
		ЭтаФорма,,,,
		ОписаниеОповещения,
		РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
...
Процедура ПодставитьРеквизитыПредставителя()
	...
	Если НЕ ПодписьЗагружена И ТипЗнч(Объект.Представитель) = Тип("СправочникСсылка.Организации")
		...
		Если ФизическоеЛицоПредставителяЗаполнено Тогда
			...
			Если НЕ Доверитель_ЮридическоеЛицо И НЕ Представитель_ЮридическоеЛицо И ПредставительЯвляетсяСотрудником Тогда
				Элементы.Представитель.ОграничениеТипа = Новый ОписаниеТипов;
				Объект.Представитель = ПредставительФЛ_ФИО; // строковое заполнение указывает на сотрудника
			КонецЕсли;
...
Процедура ВвестиРеквизитыПредставителя()
	...
	ОписаниеОповещения = Новый ОписаниеОповещения("ВвестиРеквизитыПредставителяПослеВвода",
		ЭтотОбъект);
	ОткрытьФорму(
		"Справочник.МашиночитаемыеДоверенностиФНС.Форма.ФормаПредставителя",
		Новый Структура("СтруктураДанных", СтруктураДанных),
		ЭтаФорма,,,,
		ОписаниеОповещения,
		РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
...
&НаКлиенте
Процедура ПодписатьПослеПодтвержденияПриПредупреждении(Результат, ДополнительныеПараметры) Экспорт
...
Процедура ПодписатьПослеПодтвержденияПодписи(Результат, ДополнительныеПараметры) Экспорт
	...
	ДокументооборотСКОКлиент.ПодписатьМЧДФНС(ОписаниеОповещения,, ЭтаФорма);
...
Процедура ПодписатьПослеПодписи(Результат, ДополнительныеПараметры) Экспорт
	...
	Если Результат <> Неопределено И ЗначениеЗаполнено(Результат.ИмяФайлаВыгрузки) Тогда
		Прочитать();
		Инициализация();
		УправлениеФормой(ЭтаФорма);
	КонецЕсли;
