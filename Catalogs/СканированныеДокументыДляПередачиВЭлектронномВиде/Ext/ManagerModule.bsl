...
Функция ПроверитьPDFПередЗаписью(ТаблицаФайлов, АдресФайлаПодписи, Ссылка = Неопределено) Экспорт
	...
	Для каждого Файл Из ТаблицаФайлов Цикл
		...
		Если ЗначениеЗаполнено(АдресФайлаПодписи) 
		...
		Если НЕ ЗначениеЗаполнено(АдресФайлаПодписи) 
			И НЕ Файл.ЕстьВстроеннаяПодпись
			И Файл.ВидPDF = ПредопределенноеЗначение("Перечисление.ВидыPDF.ДокPDF") Тогда
			
			Ошибка = 
				НСтр("ru = 'Формат приказа ФНС от 19 апреля 2023 г. N ЕД-7-26/263@ требует обязательного указания эл. подписи (встроенной или отсоединенной) для эл. документов, созданных в формате PDF.
                      |Исправьте для файла ""%1""'");
			СообщитьОшибкуПроверкиPDF(Ошибка, Файл.ИмяФайла, Ссылка);
			Возврат Ложь;
...
Функция ПроверитьPDFВЗаписанномСканированномДокументе(Ссылка) Экспорт
	...
	Для каждого ПрисоединенныйФайл Из МассивФайлов Цикл
		...
		Файл = Новый Структура("ИмяФайла, ВидPDF, ЕстьВстроеннаяПодпись");
		...
		Если МассивФайлов.Количество() = 1 Тогда
			...
			АдресПодписи = ЭП.АдресДанных;
		КонецЕсли;

	КонецЦикла;
	
	Результат = ПроверитьPDFПередЗаписью(ТаблицаФайлов, АдресПодписи, Ссылка);
