...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	Если ЗначениеЗаполнено(Владелец) И ЗначениеЗаполнено(Подразделение)
		И Владелец <> ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Подразделение, "Владелец") Тогда
		
		ОбщегоНазначения.СообщитьПользователю(НСтр("ru='Подразделение не соответствует организации позиции'"), Ссылка, "Подразделение", "Объект", Отказ);
...
Процедура ПередЗаписью(Отказ)
	...
	Если ПометкаУдаления И Утверждена И (Не Закрыта) Тогда 
		...
		ОбщегоНазначения.СообщитьПользователю(ТекстСообщения, Ссылка, "Объект.Закрыта", "Закрыта", Отказ);
		Возврат;
	КонецЕсли;
...
Процедура ПриЗаписи(Отказ)
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("ЗарплатаКадрыПриложения.ОрганизационнаяСтруктура") Тогда
		...
		МодульОрганизационнаяСтруктура.ШтатноеРасписаниеПриЗаписи(ЭтотОбъект, Отказ);
	КонецЕсли;
...
Процедура ПроверитьПодразделениеИДолжность(Ссылка, ДатаУтверждения, Подразделение, Должность, Отказ)
	...
	Если Выборка.Следующий() Тогда
		...
		Если Выборка.ОшибкаДолжность Тогда
			...
			ОбщегоНазначения.СообщитьПользователю(ТекстСообщения, Ссылка, "Объект.Должность", , Отказ);
		КонецЕсли;
...
Процедура ПроверитьЗаполнениеНачислений(Ссылка, Начисления, Показатели, Отказ)
	...
	Пока ВыборкаПоНачислениям.СледующийПоЗначениюПоля("НомерСтроки") Цикл
		...
		Если ВыборкаПоНачислениям.ДубльНачисление Тогда
			...
			ОбщегоНазначения.СообщитьПользователю(ТекстСообщения, Ссылка, "Объект.Начисления[" + Строка(ВыборкаПоНачислениям.НомерСтроки - 1) + "].Начисление", , Отказ);
		...
		Пока ВыборкаПоНачислениям.Следующий() Цикл
			...
			Если ВыборкаПоНачислениям.ОшибкаПоказателя Тогда
				...
				ОбщегоНазначения.СообщитьПользователю(ТекстСообщения, Ссылка, "Объект.Начисления[" + Строка(ВыборкаПоНачислениям.НомерСтроки - 1) + "].МинимальноеЗначение" + СчПоказателей,, Отказ);	
			КонецЕсли;
