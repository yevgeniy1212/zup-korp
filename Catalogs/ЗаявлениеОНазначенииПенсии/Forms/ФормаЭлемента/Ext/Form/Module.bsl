...
Процедура ПолучательПриИзменении(Элемент)
	...
	Если НЕ ПринимаетЗаявления Тогда
		...
		ТекстПредупреждения = НСтр("ru = 'Орган СФР (бывш. ПФР) %1 не принимает заявления о назначении и доставке пенсии по каналам связи.
...
Процедура ОткрытьПравила(Команда)
	...
	Отбор.Вставить("ВидОтчетности", ПредопределенноеЗначение("Перечисление.ВидыОтчетностиПоФизЛицам.ЗаявлениеОНазначенииИДоставкеПенсии"));
...
Процедура ЗаполнитьСпособПолучения(Команда)
	...
	Если НЕ ЗначениеЗаполнено(Объект.Сотрудник) Тогда
		ОбщегоНазначенияКлиент.СообщитьПользователю(НСтр("ru = 'Сначала выберите получателя пенсии'"),, "Объект.Сотрудник");
		Возврат;
	КонецЕсли;
...
Процедура ЗаполнитьСведенияОПредставителе(Команда)
	...
	ПараметрыФормы = 
		...
		|ПредставительПолномочияСерия,
		|ПредставительПолномочияНомер,
...
Процедура ЗаполнитьДопСведения(Команда)
	...
	Если НЕ ЗначениеЗаполнено(Объект.ВидПенсииОсновной) И НЕ ЗначениеЗаполнено(Объект.ВидПенсииВторой) И НЕ ЗначениеЗаполнено(Объект.НовыйВидПенсии) Тогда
		ОбщегоНазначенияКлиент.СообщитьПользователю(НСтр("ru = 'Сначала выберите вид пенсии'"),, "УказательВидПенсии");
		ЕстьОшибка = Истина;
	КонецЕсли;
...
Процедура ДобавитьОтсканированныеФайлы(ПрисоединенныйФайл, ЭлементИмя)
	...
	Требования = ЭлектронныйДокументооборотСКонтролирующимиОрганамиКлиент.ТребованияКИзображениямВЗаявленииНаПенсию(ДопустимыеТипыФайлов, МаксимальныйРазмерФайла);
...
&НаКлиенте
Процедура ДобавитьОтсканированныеФайлы_ПослеОбработки(Результат, ЭлементИмя) Экспорт
	...
	Для каждого ОписаниеФайла Из Результат.ОписанияФайлов Цикл
		...
		Файл.Вставить("Адрес", 		 ОписаниеФайла.Адрес);
		Файл.Вставить("ИсходноеИмя", ОписаниеФайла.Имя);
		Файл.Вставить("Имя", 		 ОписаниеФайла.Имя);
...
&НаКлиенте
Функция ФайлыДляПредпросмотра(ИмяТаблицы)
...
Процедура ДобавитьФайлыСПредпросмотром(Элемент)
	...
	ПараметрыФункции = ПараметрыПроцедурыВыбораФайлов(Элемент);
...
Процедура ИзменитьОформлениеПодсказкиЭтап3(Форма)
	...
	Если НЕ Форма.ПринимаетЗаявления Тогда
		Элементы.ВложениеПояснение2.Заголовок = Элементы.ВложениеПояснение2.Заголовок + Символы.ПС +
			НСтр("ru = 'Отнесите заявление в СФР (бывш. ПФР).'");
	КонецЕсли;
...
&НаКлиенте
Функция ПараметрыПроцедурыВыбораФайлов(Элемент)
	...
	Требования = ЭлектронныйДокументооборотСКонтролирующимиОрганамиКлиент.ТребованияКИзображениямВЗаявленииНаПенсию(ДопустимыеТипыФайлов, МаксимальныйРазмер);
	...
	ПараметрыФункции.Вставить("Файлы",                   ФайлыДляПредпросмотра(ИмяТаблицы));
...
Процедура ПереконвертироватьКартинки()
	...
	Требования = ЭлектронныйДокументооборотСКонтролирующимиОрганамиКлиент.ТребованияКИзображениямВЗаявленииНаПенсию(ДопустимыеТипыФайлов, МаксимальныйРазмерФайла, Истина);
...
Функция ФайлыДляКонвертации()
	...
	ДобавитьФайлыДляКонвертации(ОписаниеФайлов, ФайлыВложения, "СсылкаВложения");
	ДобавитьФайлыДляКонвертации(ОписаниеФайлов, ФайлыЗаявлениеОДоставке, "СсылкаЗаявлениеОНазначении");
	ДобавитьФайлыДляКонвертации(ОписаниеФайлов, ФайлыЗаявлениеОНазначении, "СсылкаЗаявлениеОДоставке");
...
Процедура ДобавитьФайлыДляКонвертации(ОписаниеФайлов, СписокФайлов, ИдентификаторСтроки)
	...
	Для Каждого ОписаниеФайла Из СписокФайлов Цикл
		...
		ПараметрыФайла.Вставить("Идентификатор", ИдентификаторСтроки);
...
Процедура ПереконвертироватьКартинки_Завершение(Результат, ВходящийКонтекст) Экспорт
	...
	ИменаЭлементов.Добавить("СсылкаВложения");
	ИменаЭлементов.Добавить("СсылкаЗаявлениеОНазначении");
	ИменаЭлементов.Добавить("СсылкаЗаявлениеОДоставке");
	
	Для каждого ЭлементИмя Из ИменаЭлементов Цикл
		
		ДокументыДанногоВида = Новый Массив;
		
		Для каждого ОписанияФайла Из Результат.ОписанияФайлов Цикл
			Если ОписанияФайла.Идентификатор = ЭлементИмя Тогда
				
				Файл = Новый Структура();
				Файл.Вставить("ИсходноеИмя", ОписанияФайла.Имя);
				Файл.Вставить("Размер",      ОписанияФайла.Размер);
				Файл.Вставить("Адрес",       ОписанияФайла.Адрес);
				
				ДокументыДанногоВида.Добавить(Файл);
				
			КонецЕсли;
		КонецЦикла;
		
		ТекущийРезультат = Новый Структура();
		ТекущийРезультат.Вставить("Выполнено", Результат.Выполнено);
		ТекущийРезультат.Вставить("Файлы",     ДокументыДанногоВида);
		
		ДополнительныеПараметры = Новый Структура();
		ДополнительныеПараметры.Вставить("ЭлементИмя", ЭлементИмя);
		
		ДобавитьФайлыСПредпросмотромПослеВыполнения(ТекущийРезультат, ДополнительныеПараметры);
