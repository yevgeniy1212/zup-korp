...
Функция СвойстваНабора(НаборыСвойств, Набор, Родитель = Неопределено, Наименования = Неопределено) Экспорт
	...
	Иначе
		...
		// ЗУПК
		// обработка разных идентификаторов элементов справочника РК и РФ
		Если Найти(Строка(НаборСсылка), "<Объект не найден>" ) <> 0 Тогда 
			НаборСсылкаПоИмениНабораПредопределенного = Справочники.НаборыДополнительныхРеквизитовИСведений.НайтиПоРеквизиту("ИмяПредопределенногоНабора", Набор.Имя);
			// Для новых объектов ситуация с <Объект не найден> возможна
			Если ЗначениеЗаполнено(НаборСсылкаПоИмениНабораПредопределенного) Тогда
				НаборСсылка = НаборСсылкаПоИмениНабораПредопределенного;	
			КонецЕсли;
		КонецЕсли;
		// ЗУПК
	КонецЕсли;
