...
Процедура ПослеИнициализацииРабочегоКаталога(ПолученныеФайлы, ДополнительныеПараметры) Экспорт
	...
	Если ОбщегоНазначенияКлиент.ЭтоOSXКлиент() Тогда
		...
		// Программа для MAC поставляется в архиве, чтобы сохранить права на исполнение.
		// Переименовываем его и потом распаковываем.
		
		ПрограммаКонвертации = ОписаниеПрограммыКонвертации(ТипПлатформыНаКлиенте, РежимОтладки);
		
		ПереместитьФайл(Задание.РабочийКаталог + ПрограммаКонвертации.ИмяФайла, Задание.РабочийКаталог + ПрограммаКонвертации.ИмяФайла + ".zip");
		
		ФайлАрхива = Новый ЧтениеZIPФайла(Задание.РабочийКаталог + ПрограммаКонвертации.ИмяФайла + ".zip");
		ФайлАрхива.ИзвлечьВсе(Задание.РабочийКаталог, РежимВосстановленияПутейФайловZIP.Восстанавливать); 
		ФайлАрхива.Закрыть(); 
