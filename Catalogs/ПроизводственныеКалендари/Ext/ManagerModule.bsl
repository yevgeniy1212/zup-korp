...
Процедура Печать(МассивОбъектов, ПараметрыПечати, КоллекцияПечатныхФорм, ОбъектыПечати, ПараметрыВывода) Экспорт
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.Печать") Тогда
		...
		МодульУправлениеПечатью.ВывестиТабличныйДокументВКоллекцию(
				...
				Заголовок,
				// ЗУПК
				?(ПараметрыПечати.НомерГода > 2014, ПечатнаяФормаПроизводственногоКалендаряРК(ПараметрыПечати), ПечатнаяФормаПроизводственногоКалендаря(ПараметрыПечати))
				,
				?(ПараметрыПечати.НомерГода > 2014, "Справочник.ПроизводственныеКалендари.ПФ_MXL_ПроизводственныйКалендарьРК", "Справочник.ПроизводственныеКалендари.ПФ_MXL_ПроизводственныйКалендарьРК"));
				// ЗУПК
	КонецЕсли;
...
Функция ЦветаОформленияВидовДнейПроизводственногоКалендаря() Экспорт
    ...
    // ЗУПК
	ЦветаОформления.Вставить(Перечисления.ВидыДнейПроизводственногоКалендаря.ДополнительныйВыходной,	ЦветаСтиля.ВидДняПроизводственногоКалендаряПредпраздничныйЦвет);
    // ЗУПК
...
Функция СписокВидовДня() Экспорт
	...
	Для Каждого МетаданныеВидаДней Из Метаданные.Перечисления.ВидыДнейПроизводственногоКалендаря.ЗначенияПеречисления Цикл
		// ЗУПК
		Если МетаданныеВидаДней = Метаданные.Перечисления.ВидыДнейПроизводственногоКалендаря.ЗначенияПеречисления.Предпраздничный Тогда
			Продолжить;
		КонецЕсли;
		// ЗУПК
...
Процедура ЗаполнитьПостоянныеПраздничныеДни(ВидыДней, ПереносыДней, НомерГода, КодКалендаря, КодБазовогоКалендаря = Неопределено)
	...
	Для Каждого СтрокаТаблицы Из ПраздничныеДни Цикл
		...
		// ЗУПК
		ИначеЕсли НЕ СтрокаТаблицы.ПереноситьВыходной Тогда
			ВидыДней.Вставить(ПраздничныйДень, Перечисления.ВидыДнейПроизводственногоКалендаря.ДополнительныйВыходной);
		// ЗУПК
		Иначе
...
Функция ПечатнаяФормаПроизводственногоКалендаря(ПараметрыПодготовкиПечатнойФормы)
	...
	// ЗУПК
	ВидыНерабочихДней.Добавить(Перечисления.ВидыДнейПроизводственногоКалендаря.ДополнительныйВыходной);
	// ЗУПК
...
// ЗУПК
Функция ПечатнаяФормаПроизводственногоКалендаряРК(ПараметрыПодготовкиПечатнойФормы)
	
	ТекстЗапроса = 
	"ВЫБРАТЬ
	|	КВАРТАЛ(ДанныеКалендаря.Дата) КАК КварталКалендаря,
	|	НАЧАЛОПЕРИОДА(ДанныеКалендаря.Дата, МЕСЯЦ) КАК МесяцКалендаря,
	|	СУММА(1) КАК КалендарныеДни,
	|	СУММА(ВЫБОР
	|			КОГДА НЕ ДанныеКалендаря.ВидДня = ЗНАЧЕНИЕ(Перечисление.ВидыДнейПроизводственногоКалендаря.Праздник)
	|				ТОГДА 1
	|			ИНАЧЕ 0
	|		КОНЕЦ) КАК КалендарныеДниБезПраздничных,
	|	СУММА(ЕСТЬNULL(ВЫБОР
	|				КОГДА ДанныеКалендаря.ВидДня = ЗНАЧЕНИЕ(Перечисление.ВидыДнейПроизводственногоКалендаря.Рабочий)
	|					ТОГДА 1
	|				ИНАЧЕ 0
	|			КОНЕЦ, 0)) КАК Пятидневка40Дни,
	|	СУММА(ЕСТЬNULL(ВЫБОР
	|				КОГДА ДанныеКалендаря.ВидДня = ЗНАЧЕНИЕ(Перечисление.ВидыДнейПроизводственногоКалендаря.Рабочий)
	|					ТОГДА 8
	|				ИНАЧЕ 0
	|			КОНЕЦ, 0)) КАК Пятидневка40Часы,
	|	СУММА(ЕСТЬNULL(ВЫБОР
	|				КОГДА ДанныеКалендаря.ВидДня = ЗНАЧЕНИЕ(Перечисление.ВидыДнейПроизводственногоКалендаря.Рабочий)
	|					ТОГДА 1
	|				ИНАЧЕ 0
	|			КОНЕЦ, 0)) КАК Пятидневка36Дни,
	|	СУММА(ЕСТЬNULL(ВЫБОР
	|				КОГДА ДанныеКалендаря.ВидДня = ЗНАЧЕНИЕ(Перечисление.ВидыДнейПроизводственногоКалендаря.Рабочий)
	|					ТОГДА 7.2
	|				ИНАЧЕ 0
	|			КОНЕЦ, 0)) КАК Пятидневка36Часы,
	|	СУММА(ЕСТЬNULL(ВЫБОР
	|				КОГДА ДанныеКалендаря.ВидДня = ЗНАЧЕНИЕ(Перечисление.ВидыДнейПроизводственногоКалендаря.Рабочий)
	|						ИЛИ ДанныеКалендаря.ВидДня = ЗНАЧЕНИЕ(Перечисление.ВидыДнейПроизводственногоКалендаря.Суббота)
	|					ТОГДА 1
	|				ИНАЧЕ 0
	|			КОНЕЦ, 0)) КАК Шестидневка40Дни,
	|	СУММА(ЕСТЬNULL(ВЫБОР
	|				КОГДА ДанныеКалендаря.ВидДня = ЗНАЧЕНИЕ(Перечисление.ВидыДнейПроизводственногоКалендаря.Рабочий)
	|					ТОГДА ВЫБОР
	|							КОГДА ДанныеКалендаря.ДатаПереноса = ДАТАВРЕМЯ(1, 1, 1)
	|								ТОГДА 7
	|							КОГДА ДанныеПереносовКалендаря.ВидДня = ЗНАЧЕНИЕ(Перечисление.ВидыДнейПроизводственногоКалендаря.Суббота)
	|								ТОГДА 5
	|							ИНАЧЕ 7
	|						КОНЕЦ
	...
	|							ИНАЧЕ 7
	|						КОНЕЦ
	|			КОНЕЦ, 0)) КАК Шестидневка40Часы,
	|	СУММА(ЕСТЬNULL(ВЫБОР
	|				КОГДА ДанныеКалендаря.ВидДня = ЗНАЧЕНИЕ(Перечисление.ВидыДнейПроизводственногоКалендаря.Рабочий)
	|						ИЛИ ДанныеКалендаря.ВидДня = ЗНАЧЕНИЕ(Перечисление.ВидыДнейПроизводственногоКалендаря.Суббота)
	|					ТОГДА 1
	|				ИНАЧЕ 0
	|			КОНЕЦ, 0)) КАК Шестидневка36Дни,
	|	СУММА(ЕСТЬNULL(ВЫБОР
	|				КОГДА ДанныеКалендаря.ВидДня = ЗНАЧЕНИЕ(Перечисление.ВидыДнейПроизводственногоКалендаря.Рабочий)
	|						ИЛИ ДанныеКалендаря.ВидДня = ЗНАЧЕНИЕ(Перечисление.ВидыДнейПроизводственногоКалендаря.Суббота)
	|					ТОГДА 6
	|				ИНАЧЕ 0
	|			КОНЕЦ, 0)) КАК Шестидневка36Часы
	|ИЗ
	|	РегистрСведений.ДанныеПроизводственногоКалендаря КАК ДанныеКалендаря
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ДанныеПроизводственногоКалендаря КАК ДанныеПереносовКалендаря
	|		ПО ДанныеКалендаря.Год = ДанныеПереносовКалендаря.Год
	|			И ДанныеКалендаря.ПроизводственныйКалендарь = ДанныеПереносовКалендаря.ПроизводственныйКалендарь
	|			И ДанныеКалендаря.ДатаПереноса = ДанныеПереносовКалендаря.Дата
	|ГДЕ
	|	ДанныеКалендаря.Год = &Год
	|	И ДанныеКалендаря.ПроизводственныйКалендарь = &ПроизводственныйКалендарь
	|
	...
	Макет = ПолучитьМакет("ПФ_MXL_ПроизводственныйКалендарьРК");
	...
	ИтоговаяСтрока = Макет.ПолучитьОбласть("СтрокаИтогов");
	СтрокаМесяца = Макет.ПолучитьОбласть("СтрокаМесяца");
	...
	КалендарныеДниНарастающийКвИтог = 0;
	КалендарныеДниБезПраздничныхНарастающийКвИтог = 0;
	Пятидневка40ДниНарастающийКвИтог = 0;
	Пятидневка40ЧасыНарастающийКвИтог = 0;
	Шестидневка40ДниНарастающийКвИтог = 0;
	Шестидневка40ЧасыНарастающийКвИтог = 0;
	Пятидневка36ДниНарастающийКвИтог = 0;
	Пятидневка36ЧасыНарастающийКвИтог = 0;
	Шестидневка36ДниНарастающийКвИтог = 0;
	Шестидневка36ЧасыНарастающийКвИтог = 0;
	
	ВыборкаПоКварталу = Результат.Выбрать(ОбходРезультатаЗапроса.ПоГруппировкам);
	Пока ВыборкаПоКварталу.Следующий() Цикл
		
		ВыборкаПоМесяцу = ВыборкаПоКварталу.Выбрать(ОбходРезультатаЗапроса.ПоГруппировкам);
		Пока ВыборкаПоМесяцу.Следующий() Цикл
			
			ЗаполнитьЗначенияСвойств(СтрокаМесяца.Параметры, ВыборкаПоМесяцу);
			СтрокаМесяца.Параметры.МесяцКалендаря = Формат(ВыборкаПоМесяцу.МесяцКалендаря, "ДФ=MMMM");
			ТабличныйДокумент.Вывести(СтрокаМесяца);
			
		КонецЦикла;
		
		ЗаполнитьЗначенияСвойств(ИтоговаяСтрока.Параметры, ВыборкаПоКварталу);
		ТекстКвартала = НСтр("ru = '%1 квартал'");
		
		НомерКвартала = "I";
		Если ВыборкаПоКварталу.КварталКалендаря = 2 Тогда
			НомерКвартала = "II";
		ИначеЕсли ВыборкаПоКварталу.КварталКалендаря = 3 Тогда
			НомерКвартала = "III";
		ИначеЕсли ВыборкаПоКварталу.КварталКалендаря = 4 Тогда
			НомерКвартала = "IV";
		КонецЕсли;
			
		ТекстКвартала = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ТекстКвартала, НомерКвартала);
		ИтоговаяСтрока.Параметры.ИтоговыйПериод = ТекстКвартала;
		ТабличныйДокумент.Вывести(ИтоговаяСтрока);
		
		КалендарныеДниНарастающийКвИтог = КалендарныеДниНарастающийКвИтог + ВыборкаПоКварталу.КалендарныеДни;
		КалендарныеДниБезПраздничныхНарастающийКвИтог = КалендарныеДниБезПраздничныхНарастающийКвИтог + ВыборкаПоКварталу.КалендарныеДниБезПраздничных;
		Пятидневка40ДниНарастающийКвИтог = Пятидневка40ДниНарастающийКвИтог + ВыборкаПоКварталу.Пятидневка40Дни;
		Пятидневка40ЧасыНарастающийКвИтог = Пятидневка40ЧасыНарастающийКвИтог + ВыборкаПоКварталу.Пятидневка40Часы;
		Шестидневка40ДниНарастающийКвИтог = Шестидневка40ДниНарастающийКвИтог + ВыборкаПоКварталу.Шестидневка40Дни;
		Шестидневка40ЧасыНарастающийКвИтог = Шестидневка40ЧасыНарастающийКвИтог + ВыборкаПоКварталу.Шестидневка40Часы;
		Пятидневка36ДниНарастающийКвИтог = Пятидневка36ДниНарастающийКвИтог + ВыборкаПоКварталу.Пятидневка36Дни;
		Пятидневка36ЧасыНарастающийКвИтог = Пятидневка36ЧасыНарастающийКвИтог + ВыборкаПоКварталу.Пятидневка36Часы;
		Шестидневка36ДниНарастающийКвИтог = Шестидневка36ДниНарастающийКвИтог + ВыборкаПоКварталу.Шестидневка36Дни;
		Шестидневка36ЧасыНарастающийКвИтог = Шестидневка36ЧасыНарастающийКвИтог + ВыборкаПоКварталу.Шестидневка36Часы;
		
		Если ВыборкаПоКварталу.КварталКалендаря = 2
			ИЛИ ВыборкаПоКварталу.КварталКалендаря = 3
			ИЛИ ВыборкаПоКварталу.КварталКалендаря = 4 Тогда
			
			Если ВыборкаПоКварталу.КварталКалендаря = 2 Тогда 
				ИтоговаяСтрока.Параметры.ИтоговыйПериод = НСтр("ru = 'I полугодие'");
			ИначеЕсли ВыборкаПоКварталу.КварталКалендаря = 3 Тогда
				ИтоговаяСтрока.Параметры.ИтоговыйПериод = НСтр("ru = '9 месяцев'");
			Иначе
				ИтоговаяСтрока.Параметры.ИтоговыйПериод = НСтр("ru = 'Год'");
			КонецЕсли;
			ИтоговаяСтрока.Параметры.КалендарныеДни = КалендарныеДниНарастающийКвИтог;
			ИтоговаяСтрока.Параметры.КалендарныеДниБезПраздничных = КалендарныеДниБезПраздничныхНарастающийКвИтог;
			ИтоговаяСтрока.Параметры.Пятидневка40Дни = Пятидневка40ДниНарастающийКвИтог;
			ИтоговаяСтрока.Параметры.Пятидневка40Часы = Пятидневка40ЧасыНарастающийКвИтог;
			ИтоговаяСтрока.Параметры.Шестидневка40Дни = Шестидневка40ДниНарастающийКвИтог;
			ИтоговаяСтрока.Параметры.Шестидневка40Часы = Шестидневка40ЧасыНарастающийКвИтог;
			ИтоговаяСтрока.Параметры.Пятидневка36Дни = Пятидневка36ДниНарастающийКвИтог;
			ИтоговаяСтрока.Параметры.Пятидневка36Часы = Пятидневка36ЧасыНарастающийКвИтог;
			ИтоговаяСтрока.Параметры.Шестидневка36Дни = Шестидневка36ДниНарастающийКвИтог;
			ИтоговаяСтрока.Параметры.Шестидневка36Часы = Шестидневка36ЧасыНарастающийКвИтог;
			ТабличныйДокумент.Вывести(ИтоговаяСтрока);
	...
	ИтоговаяСтрока.Параметры.ИтоговыйПериод = НСтр("ru = 'Среднемесячный баланс'");
	ИтоговаяСтрока.Параметры.КалендарныеДни = "";
	ИтоговаяСтрока.Параметры.КалендарныеДниБезПраздничных = Окр(КалендарныеДниБезПраздничныхНарастающийКвИтог / 12, 2);
	ИтоговаяСтрока.Параметры.Пятидневка40Дни = Окр(Пятидневка40ДниНарастающийКвИтог / 12, 2);
	ИтоговаяСтрока.Параметры.Пятидневка40Часы = Окр(Пятидневка40ЧасыНарастающийКвИтог / 12, 2);
	ИтоговаяСтрока.Параметры.Шестидневка40Дни = Окр(Шестидневка40ДниНарастающийКвИтог / 12, 2);
	ИтоговаяСтрока.Параметры.Шестидневка40Часы = Окр(Шестидневка40ЧасыНарастающийКвИтог / 12, 2);
	ИтоговаяСтрока.Параметры.Пятидневка36Дни = Окр(Пятидневка36ДниНарастающийКвИтог / 12, 2);
	ИтоговаяСтрока.Параметры.Пятидневка36Часы = Окр(Пятидневка36ЧасыНарастающийКвИтог / 12, 2);
	ИтоговаяСтрока.Параметры.Шестидневка36Дни = Окр(Шестидневка36ДниНарастающийКвИтог / 12, 2);
	ИтоговаяСтрока.Параметры.Шестидневка36Часы = Окр(Шестидневка36ЧасыНарастающийКвИтог / 12, 2);
	ТабличныйДокумент.Вывести(ИтоговаяСтрока);
