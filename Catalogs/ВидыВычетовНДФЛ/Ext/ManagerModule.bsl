...
Процедура ЗаполнитьКодыВычетовНДФЛ() Экспорт
	...
	, , 
	"-","213","213","213");
	ОписатьКодВычетаНДФЛ("Код214", "214", "Убытки по операциям, связанным с закрытием короткой позиции, и затраты, связанные с приобретением и реализацией ценных бумаг, являющимся объектом операций РЕПО",
	, , 
	"-", "214", "-","-");
	...
	Ложь, ГруппыВычетов.СоциальныеПоУведомлениюНО, 
	"-", "-", "325", "325", , 28);	
	ОписатьКодВычетаНДФЛ("Код329", "332/327", НСтр("ru = 'Сумма страховых взносов по договору (договорам) добровольного страхования жизни, если такие договоры заключаются на срок не менее пяти лет, заключенному (заключенным) со страховой организацией в свою пользу и (или) в пользу супруга (в том числе вдовы, вдовца), родителей (в том числе усыновителей), детей (в том числе усыновленных, находящихся под опекой (попечительством))'"), 
	Ложь, ГруппыВычетов.СоциальныеПоУведомлениюНО,
	"-", "-", "-", "327", , 29, "332");
	...
	,,
	"509","509","509","509");
	ОписатьКодВычетаНДФЛ("Код511", "512", "Вычет из суммы материальной помощи, оказываемой образовательной организацией студентам (курсантам), аспирантам, адъюнктам, ординаторам и ассистентам-стажерам",
	,,
	"-","-","-","512");
...
Процедура ОписатьВычетДляМатериальнойПомощиСтудентам() Экспорт 
...
Процедура ОписатьВычеты2021() Экспорт 

	// АПК:1297-выкл Не локализуется, т.к. является частью регламентированной формы, применяемой в РФ.
	
	ОписатьКодВычетаНДФЛ("Код260", "260", "Суммы фактически осуществленных и документально подтвержденных расходов, связанных с приобретением и хранением ценных бумаг",
	,,
	"-","-","-","260");

	ОписатьКодВычетаНДФЛ("Код280", "280", "Сумма ставки или интерактивной ставки, уменьшающая сумму выигрышей от участия в азартных играх, проводимых в букмекерской конторе и тотализаторе",
	,,
	"-","-","-","280");

	ОписатьКодВычетаНДФЛ("Код511ПоВыигрышам", "511", "Вычеты из суммы доходов в виде выигрышей, полученных участниками азартных игр и участниками лотерей",
	,,
	"-","-","-","511");
...
Процедура ОписатьВычеты2022() Экспорт 

	ГруппыВычетов = Перечисления.ГруппыВычетовПоНДФЛ;
	
	// АПК:1297-выкл Не локализуется, т.к. является частью регламентированной формы, применяемой в РФ.
	
	ОписатьКодВычетаНДФЛ("Код324", "324", НСтр("ru = 'Сумма, уплаченная налогоплательщиком в налоговом периоде за медицинские услуги, оказанные медицинскими организациями, индивидуальными предпринимателями, осуществляющими медицинскую деятельность, ему, его супругу (супруге), родителям, детям (в том числе усыновленным) в возрасте до 18 лет, подопечным в возрасте до 18 лет (в соответствии с перечнем медицинских услуг, утвержденным Правительством Российской Федерации) с учетом ограничения, установленного пунктом 2 статьи 219 Налогового кодекса Российской Федерации'"), 
	Ложь, ГруппыВычетов.СоциальныеПоУведомлениюНО, 
	"-", "-", "324", "324", , 30);	
	
	ОписатьКодВычетаНДФЛ("Код323", "323", НСтр("ru = 'Стоимость приобретенных налогоплательщиком, за счет собственных средств, лекарственных препаратов для медицинского применения, назначенных лечащим врачом ему, его супругу (супруге), родителям, детям (в том числе усыновленным) в возрасте до 18 лет, подопечным в возрасте до 18 лет - в размере фактически произведенных расходов, но с учетом ограничения, установленного пунктом 2 статьи 219 Налогового кодекса Российской Федерации'"), 
	Ложь, ГруппыВычетов.СоциальныеПоУведомлениюНО, 
	"-", "-", "-", "323", , 36);	

	ОписатьКодВычетаНДФЛ("Код329Спорт", "329", НСтр("ru = 'Сумма уплаченных налогоплательщиком в налоговом периоде за счет собственных средств за физкультурно-оздоровительные услуги, оказанные ему, его детям (в том числе усыновленным) в возрасте до 18 лет, подопечным в возрасте до 18 лет физкультурно-спортивными организациями, индивидуальными предпринимателями, осуществляющими деятельность в области физической культуры и спорта в качестве основного вида деятельности, - в размере фактически произведенных расходов с учетом ограничения, установленного пунктом 2 статьи 219 Налогового кодекса Российской Федерации'"), 
	Ложь, ГруппыВычетов.СоциальныеПоУведомлениюНО, 
	"-", "-", "-", "329", , 37);	
...
Процедура ОписатьКодВычетаНДФЛ(ИмяПредопределенныхДанных, Код, Наименование, ВычетКДоходу = Истина, ГруппаВычета = Неопределено, Код2010 = "", Код2011 = "", Код2015 = "", Код2016 = "", НеПредоставляетсяНерезидентам = Ложь, ПорядковыйНомерВРасчетеНалога = 0, Код2023 = "") 
	...
	КонецЕсли;
	Если ЗначениеЗаполнено(Код2023) Тогда
		Если Элемент.КодПрименяемыйВНалоговойОтчетностиС2023Года <> Код2023 Тогда
			Элемент.КодПрименяемыйВНалоговойОтчетностиС2023Года = Код2023
		КонецЕсли;
	ИначеЕсли Элемент.КодПрименяемыйВНалоговойОтчетностиС2023Года <> Элемент.КодПрименяемыйВНалоговойОтчетностиС2016Года Тогда
		Элемент.КодПрименяемыйВНалоговойОтчетностиС2023Года = Элемент.КодПрименяемыйВНалоговойОтчетностиС2016Года
	КонецЕсли;
