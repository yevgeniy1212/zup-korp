...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Объект.Тип = Перечисления.ТипыПерепискиСКонтролирующимиОрганами.ПерепискаСРПН Тогда
		Элементы.Отправитель.Видимость = Ложь;
		ОтправительТекстом = НСтр("ru = 'Росприроднадзор'");
		Элементы.ОтправительТекстом.Видимость = Истина;
		
		СсылкаНаУведомлениеДляЗакрытия = Объект.Ссылка;
		КонтекстЭДОСервер = ДокументооборотСКО.ПолучитьОбработкуЭДО();
		КонтекстЭДОСервер.ОтправитьПодтверждениеОткрытияУведомленияРПН(
			Объект.Организация,
			Объект.Идентификатор,
			СсылкаНаУведомлениеДляЗакрытия);
	КонецЕсли;
...
Процедура ОбработатьВложениеНавигационнойСсылки(ИмяФайла)
	...
	Иначе
		...
		ИначеЕсли Результат = Неопределено Тогда
			...
			|не обнаружено.'"), ИмяФайла));
		Иначе
			Если Объект.Тип = ПредопределенноеЗначение("Перечисление.ТипыПерепискиСКонтролирующимиОрганами.ПерепискаСПФР") Тогда
				// для некорректно распакованных писем ПФР
				ИмяФайла = ПолучитьИмяФайлаИзОписанияПисьма(Объект.Ссылка, ИмяФайла);
			КонецЕсли;
			...
			Для ИндексСтроки = 1 По СтрЧислоСтрок(ТекстПисьма) Цикл
				...
				Если ЭтоГиперссылка(ТекущаяСтрока) Тогда
					...
					Иначе
						...
						РазмерСтрокой = ОписаниеТипа.ПривестиЗначение(РазмерСтрокой);
...
Процедура УправлениеФормой()
	...
	Элементы.Содержание.Видимость = ЗначениеЗаполнено(Объект.Содержание)
									И Объект.Тип <> Перечисления.ТипыПерепискиСКонтролирующимиОрганами.ПерепискаСРПН;
...
&НаСервере
Функция ПолучитьИмяФайлаИзОписанияПисьма(ПерепискаСсылка, ИмяФайлаВКонтейнере)
	
	МассивИмениФайла = СтрРазделить(ИмяФайлаВКонтейнере, ".", Ложь);
	Если МассивИмениФайла.Количество() > 1 И МассивИмениФайла[1] <> "bin" Тогда
		Возврат ИмяФайлаВКонтейнере;
	Иначе
		ЗапросЦиклаОбмена = Новый Запрос;
		ЗапросЦиклаОбмена.Текст = "ВЫБРАТЬ РАЗРЕШЕННЫЕ
		                 |	ЦиклыОбмена.Ссылка КАК Ссылка
		                 |ИЗ
		                 |	Справочник.ЦиклыОбмена КАК ЦиклыОбмена
		                 |ГДЕ
		                 |	ЦиклыОбмена.Предмет = &Предмет";
		ЗапросЦиклаОбмена.УстановитьПараметр("Предмет", ПерепискаСсылка);
		ВыборкаЦиклаОбмена = ЗапросЦиклаОбмена.Выполнить().Выбрать();
		Если НЕ ВыборкаЦиклаОбмена.Следующий() Тогда
			Возврат ИмяФайлаВКонтейнере;
		КонецЕсли;
		
		ЦиклОбмена = ВыборкаЦиклаОбмена.Ссылка;
		
		КонтекстЭДОСервер = ДокументооборотСКО.ПолучитьОбработкуЭДО();
		ТипыСообщений = Новый Массив;
		ТипыСообщений.Добавить(ПредопределенноеЗначение("Перечисление.ТипыТранспортныхСообщений.ПисьмоВходящееПФР"));
		ТранспортныеСообщенияТаблица = КонтекстЭДОСервер.ПолучитьСообщенияЦиклаОбмена(ЦиклОбмена, ТипыСообщений);
		Если ТранспортныеСообщенияТаблица.Количество() = 0 Тогда 
			Возврат ИмяФайлаВКонтейнере;
		КонецЕсли;
		ТранспортноеСообщение = ТранспортныеСообщенияТаблица[0].Ссылка;
		
		ЗапросИД = Новый Запрос;
		ЗапросИД.Текст = "ВЫБРАТЬ РАЗРЕШЕННЫЕ
		                 |	ТранспортноеСообщениеПрисоединенныеФайлы.Идентификатор КАК Идентификатор
		                 |ИЗ
		                 |	Справочник.ТранспортноеСообщениеПрисоединенныеФайлы КАК ТранспортноеСообщениеПрисоединенныеФайлы
		                 |ГДЕ
		                 |	ТранспортноеСообщениеПрисоединенныеФайлы.ВладелецФайла = &ВладелецФайла
		                 |	И ТранспортноеСообщениеПрисоединенныеФайлы.ИсходноеИмяФайла = &ИсходноеИмяФайла";
		ЗапросИД.УстановитьПараметр("ВладелецФайла", ТранспортноеСообщение);
		ЗапросИД.УстановитьПараметр("ИсходноеИмяФайла", ИмяФайлаВКонтейнере);
		
		ВыборкаИД = ЗапросИД.Выполнить().Выбрать();
		Если НЕ ВыборкаИД.Следующий() Тогда
			Возврат ИмяФайлаВКонтейнере;
		КонецЕсли;
		
		ИдентификаторФайла = ВыборкаИД.Идентификатор;
		
		ТипОписаниеПисьма = ПредопределенноеЗначение("Перечисление.ТипыСодержимогоТранспортногоКонтейнера.ОписаниеПисьма");
		ОписанияПисьмаТаблица = КонтекстЭДОСервер.ПолучитьВложенияТранспортногоСообщения(
			ТранспортноеСообщение,
			Истина,
			ТипОписаниеПисьма,,
			Истина);
		
		Если ОписанияПисьмаТаблица.Количество() = 0 Тогда 
			Возврат ИмяФайлаВКонтейнере;
		КонецЕсли;
		ОписаниеПисьма = ОписанияПисьмаТаблица[0];
		
		ОписаниеПисьмаДД = ПолучитьИзВременногоХранилища(ОписаниеПисьма.Адрес);
		ИмяВременногоФайла = ПолучитьИмяВременногоФайла();
		ОписаниеПисьмаДД.Записать(ИмяВременногоФайла);
		
		ЧтениеXML = Новый ЧтениеXML;
		ЧтениеXML.ОткрытьФайл(ИмяВременногоФайла,,,"windows-1251");
		...
		ЗначениеВыражения = ДокументDOM.ВычислитьВыражениеXPath(
			"//списокПриложений/приложение[идентификаторДокумента = '" + ИдентификаторФайла + "']/имяФайла",
			ДокументDOM,
			Разыменователь);
		
		Узел = ЗначениеВыражения.ПолучитьСледующий();
