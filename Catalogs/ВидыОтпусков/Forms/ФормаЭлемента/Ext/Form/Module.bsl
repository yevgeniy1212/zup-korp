...
Процедура ПередЗаписью(Отказ, ПараметрыЗаписи)
	...
	Если Объект.ОтпускЯвляетсяЕжегодным И Объект.ОтпускБезОплаты Тогда
		ОтпускЧАЭС = ОбщегоНазначенияКлиент.ПредопределенныйЭлемент("Справочник.ВидыОтпусков.ОтпускПострадавшимВАварииЧАЭС");
...
Процедура ПередЗаписьюНаСервере(Отказ, ТекущийОбъект, ПараметрыЗаписи)
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("ЗарплатаКадрыКорпоративнаяПодсистемы.РеестрГосслужащих") Тогда
		...
		Модуль.ЗаписатьДополнительныеСвойстваВидаОтпуска(ЭтаФорма, ТекущийОбъект.Ссылка);
	КонецЕсли;	
...
Процедура ВыполнитьЧтениеНаСервере(ТекущийОбъект = Неопределено)
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("ЗарплатаКадрыКорпоративнаяПодсистемы.РеестрГосслужащих") Тогда
		Модуль = ОбщегоНазначения.ОбщийМодуль("РеестрГосслужащих");
		Модуль.ДополнитьФормуВидыОтпусков(ЭтаФорма);
		Модуль.ПрочитатьДополнительныеДанныеВидаОтпуска(ЭтаФорма);		
	КонецЕсли;
...
Процедура УстановитьДоступностьЭлементов(Форма)
	...
	ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "КоличествоДнейВГодГруппа", 	"Доступность", Объект.ОтпускЯвляетсяЕжегодным);
...
Процедура ПересчитатьНакопительныеОтпускаНаСервере()
	...
	Запрос.Текст = 
		...
		|ИЗ
		|	ВТСотрудникиОтпуска КАК ВТСотрудникиОтпуска";
	Запрос.УстановитьПараметр("Вредный", ОбщегоНазначения.ПредопределенныйЭлемент("Справочник.ВидыОтпусков.ОтпускЗаВредность"));
