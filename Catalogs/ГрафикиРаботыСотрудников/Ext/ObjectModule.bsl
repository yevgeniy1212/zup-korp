	...
	Пока ОбрабатываемаяДата <= ДатаОкончанияИнтервалаЗаполнения  Цикл
		...
		Если СпособЗаполнения = Перечисления.СпособыЗаполненияГрафиковРаботыСотрудников.ПоНеделям Тогда
	        Если ВидДня = Перечисления.ВидыДнейПроизводственногоКалендаря.Суббота Тогда
				// ЗУПК проблемы с шестидневкой 40 часов, когда субботы по 5, рабочие по 7, а производственный календарь по пятидневке
				Если РабочихДнейВНеделе = 6
					И ЗначениеЗаполнено(ДатаПереноса) Тогда
					ЧасовЗаДень = ЧасыПоДнямЦикла[ДеньНедели(ОбрабатываемаяДата)]
				Иначе
			...
			ИначеЕсли ДатаПереноса <> '00010101' Тогда
				// ЗУПК проблемы с шестидневкой 40 часов, когда субботы по 5, рабочие по 7
				Если РабочихДнейВНеделе = 6
					И ДеньНедели(ОбрабатываемаяДата) = 6 Тогда
					ЧасовЗаДень = ЧасыПоДнямЦикла[ДеньНедели(ОбрабатываемаяДата)];	
				Иначе	
...
Функция ЗаполнятьВремяЗаДень(ОбрабатываемаяДата, ВидДня, ПеренесенныеДни)
	...
	Если УчитыватьПраздники 
		...
		// ЗУПК
		Или ВидДня = Перечисления.ВидыДнейПроизводственногоКалендаря.ДополнительныйВыходной
		// ЗУПК
