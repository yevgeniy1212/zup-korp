...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)	
	...
	Элементы.РасписаниеРаботыСмена.ПараметрыВыбора = Новый ФиксированныйМассив(ПараметрыВыбора);
	...
	УстановитьВидимостьПоясненияЗаполненияРасписания();
...
#Область ОбработчикиСобытийЭлементовТаблицыФормыСписокВидовВремени
...
Процедура СохранитьИЗакрыть(Результат = Неопределено, ДополнительныеПараметры = Неопределено) Экспорт 
	Если (СпособЗаполнения = ПредопределенноеЗначение("Перечисление.СпособыЗаполненияГрафиковРаботыСотрудников.ПоЦикламПроизвольнойДлины")
		...
		ОбщегоНазначенияКлиент.СообщитьПользователю(НСтр("ru = 'Не заполнена дата отсчета.'"),
													,
													"ДатаОтсчета");
		Возврат;													
...
Процедура ПараметрыВДанныеФормы()
	...
	Параметры.Свойство("УчитыватьНерабочиеДни", УчитыватьНерабочиеДни);
...
Процедура ЗаполнитьСписокВидовВремени(ИспользуемыеВидыВремени = Неопределено)
	...
	Если ИспользуемыеВидыВремени.Количество() = 0 Тогда 
		ИспользуемыеВидыВремени.Добавить(ОбщегоНазначения.ПредопределенныйЭлемент("Справочник.ВидыИспользованияРабочегоВремени.Явка"));
	КонецЕсли;	
	...
	Запрос.УстановитьПараметр("РаботаНочныеЧасы", ОбщегоНазначения.ПредопределенныйЭлемент("Справочник.ВидыИспользованияРабочегоВремени.РаботаНочныеЧасы"));
	Запрос.УстановитьПараметр("РаботаВечерниеЧасы", ОбщегоНазначения.ПредопределенныйЭлемент("Справочник.ВидыИспользованияРабочегоВремени.РаботаВечерниеЧасы"));
	Запрос.УстановитьПараметр("КормлениеРебенка", ОбщегоНазначения.ПредопределенныйЭлемент("Справочник.ВидыИспользованияРабочегоВремени.КормлениеРебенка"));
	...
	Запрос.УстановитьПараметр("Вахта", ОбщегоНазначения.ПредопределенныйЭлемент("Справочник.ВидыИспользованияРабочегоВремени.Вахта"));
	...
	Если НеполноеРабочееВремя Тогда
		...
		ВидыСокращенногоВремени.Добавить(ОбщегоНазначения.ПредопределенныйЭлемент("Справочник.ВидыИспользованияРабочегоВремени.СокращенноеВремяОбучающихся"));
		ВидыСокращенногоВремени.Добавить(ОбщегоНазначения.ПредопределенныйЭлемент("Справочник.ВидыИспользованияРабочегоВремени.СокращенноеРабочееВремя"));
		ВидыСокращенногоВремени.Добавить(ОбщегоНазначения.ПредопределенныйЭлемент("Справочник.ВидыИспользованияРабочегоВремени.РаботаВРежимеНеполногоВремени"));
...
Процедура ДобавитьКолонкиВидовВремени(ОписаниеВидовВремени)	
	...
	Иначе	
		ИдентификаторыВидовВремени = Новый Соответствие(СоответствиеИдентификаторовВидамВремени);
	КонецЕсли;	
...
&НаСервере
Процедура УстановитьВидимостьПоясненияЗаполненияРасписания()
	
	ИспользуемыеВидыВремени = СписокВидовВремени.НайтиСтроки(Новый Структура("Использовать", Истина));
	ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(
		Элементы,
		"ВидыВремениПояснение",
		"Видимость",
		ИспользуемыеВидыВремени.Количество() > 1);
...
Функция ПараметрыЗаполненияГрафика()
	...
	Если ИспользуемыеВидыВремени.Количество() = 0 Тогда
		ИспользуемыеВидыВремени.Добавить(ОбщегоНазначенияКлиент.ПредопределенныйЭлемент("Справочник.ВидыИспользованияРабочегоВремени.Явка"));
	КонецЕсли;	
	...
	ПараметрыЗаполненияГрафика.Вставить("УчитыватьНерабочиеДни", УчитыватьНерабочиеДни);
...
&НаКлиенте
Процедура РасписаниеРаботыСменаНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
