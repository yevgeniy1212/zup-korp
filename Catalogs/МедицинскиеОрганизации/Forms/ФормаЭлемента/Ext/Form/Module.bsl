...
Процедура ОГРНПриИзменении(Элемент)
	Объект.ОГРН = СокрЛП(Объект.ОГРН);
КонецПроцедуры
...
Процедура АдресОбработкаВыбора(Элемент, ВыбранноеЗначение, СтандартнаяОбработка)
	...
	Объект.АдресВнутреннееПредставление = ВыбранноеЗначение.Значение;
...
Процедура ЗаполнитьКодКЛАДРНаСервере()
	КодыАдреса = ПрямыеВыплатыПособийСоциальногоСтрахования.КодыАдреса(Объект.АдресВнутреннееПредставление);
	Объект.АдресКодПоКЛАДР = КодыАдреса.КодКЛАДР;
КонецПроцедуры
