...
// По переданному адресу временного хранилища или из хранилища значения получает текст содержимого
//
// Параметры:
//  ДанныеФайла - адрес временного хранилища двоичных данных файла или хранилища значения с данными файла
//                или непосредственно двоичные данные или хранилище значения с двоичными данными
Функция ТекстСообщения(ДанныеФайла) Экспорт 

	ИмяФайла = ПолучитьИмяВременногоФайла("xml");
	Если ЭтоАдресВременногоХранилища(ДанныеФайла) Тогда
		ДанныеВХранилище = ПолучитьИзВременногоХранилища(ДанныеФайла);
	Иначе
		ДанныеВХранилище = ДанныеФайла;
	КонецЕсли;	
	
	Если ТипЗнч(ДанныеВХранилище) = Тип("ХранилищеЗначения") Тогда
		ДвДанные = ДанныеВХранилище.Получить();
	Иначе
		ДвДанные = ДанныеВХранилище;
	КонецЕсли;	
	ДвДанные.Записать(ИмяФайла);
	
	КонтекстЭДОСервер = ДокументооборотСКО.ПолучитьОбработкуЭДО();
	
	ЧтениеТекста = Новый ЧтениеТекста(ИмяФайла, "windows-1251");
	КонтекстЭДОСервер.ЧтениеТекстаОткрытьНаСервере(ЧтениеТекста, ИмяФайла); 
