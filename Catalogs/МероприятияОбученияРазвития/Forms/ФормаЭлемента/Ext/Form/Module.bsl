...
Процедура ОбязательстваПроцентПриИзменении(Элемент)
	...
	ТекущиеДанные.Сумма = ТекущиеДанные.Процент * Объект.Стоимость / 100;
...
Процедура ОбязательстваСуммаПриИзменении(Элемент)
	...
	Если Объект.Стоимость > 0 Тогда
		ТекущиеДанные.Процент = Окр(ТекущиеДанные.Сумма / Объект.Стоимость * 100, 2);
	КонецЕсли;
...
Процедура ЗаполнитьДополнительныеРеквизитыОбязательств()
	...
	Для Каждого Обязательство Из Объект.Обязательства Цикл
		Обязательство.Процент = ?(Объект.Стоимость = 0, 0, Окр(Обязательство.Сумма / Объект.Стоимость * 100, 2));
	КонецЦикла; 
