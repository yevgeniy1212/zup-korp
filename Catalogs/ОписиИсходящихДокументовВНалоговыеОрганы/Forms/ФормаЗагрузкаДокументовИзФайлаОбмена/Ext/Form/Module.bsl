...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	КонтекстЭДОСервер.ЗаполнитьСписокВыбораВидДокументаФНС(Элементы.ВидДокумента);
...
Функция ЗагрузитьФайлНаСервере(АдресФайлаОбменаВоВременномХранилище)
	...
	// читаем XML
	Если НайтиФайлы(КаталогРаспаковки, ИмяФайлаОписания).Количество() > 0 Тогда
		ТекстXML = КонтекстЭДОСервер.ПрочитатьТекстИзФайла(КаталогРаспаковки + ИмяФайлаОписания, , Истина);
	Иначе
		ТекстXML = КонтекстЭДОСервер.ПрочитатьТекстИзФайла(КаталогРаспаковки + НРег(ИмяФайлаОписания), , Истина);
	КонецЕсли;
...
Функция ЗаполнитьТаблицуЗагруженныеДокументы(ДеревоXML)
	...
	Для каждого Вид Из Виды Цикл
		Если ЗначениеЗаполнено(Вид.КНД) Тогда
			СоответствиеВидовДокументов.Вставить(Вид.КНД, Вид.Значение);
		КонецЕсли;
	КонецЦикла; 
	...
	Для каждого УзелДокумент Из УзлыДокумент Цикл
		...
		Если НЕ ЗначениеЗаполнено(УзелКНД)
		...
		НоваяСтрока.ВидДокумента = СоответствиеВидовДокументов[УзелКНД.Значение];
