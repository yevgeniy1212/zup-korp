...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Для каждого ДокументИсточник Из Параметры.ДокументыИсточники Цикл
		НоваяСтрока = ДокументыИсточники.Добавить();
		ЗаполнитьЗначенияСвойств(НоваяСтрока, ДокументИсточник);
	КонецЦикла; 
...
Функция ПроверкаПройдена()
	...
	Если ВыбраноДокументов = 0 Тогда
		ОбщегоНазначения.СообщитьПользователю(НСтр("ru = 'Выберите хотя бы один документ'"));
		ЕстьОшибка = Истина;
	КонецЕсли;
	...
	Для каждого ДокументИсточник Из ДокументыИсточники Цикл
		...
		Если ЗначениеЗаполнено(ДокументИсточник.Сканы) Тогда
			Для каждого Скан Из ДокументИсточник.Сканы Цикл
				...
				Если ОбработкаФайловPDFКлиентСервер.ЭтоPDFФайл(Скан.Значение.Имя) Тогда
					
					СвойстваФайла = Новый Структура("Имя, Адрес");
					СвойстваФайла.Имя = Скан.Значение.Имя; 
					СвойстваФайла.Адрес = Скан.Значение.АдресДанных;
