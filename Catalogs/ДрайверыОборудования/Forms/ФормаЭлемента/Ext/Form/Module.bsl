...
Процедура ПолучитьИнформациюДрайвераПоФайлу_Завершение(Результат, ПараметрыВыполнения) Экспорт 
	...
	Если ИмяФайлаРасширение = ".ZIP" Тогда
...
Процедура ЗагрузитьФайлДрайвераВБазу(ПолноеИмяФайла, ИмяФайла) Экспорт 
	...
	НачатьПомещениеФайловНаСервер(Оповещение, , , ПолноеИмяФайла); 
...
Процедура ЗагрузитьФайлДрайвераВБазу_Завершение(ПомещенныеФайлы, ИмяФайла) Экспорт 
	...
	Если ПомещенныеФайлы <> Неопределено И ПомещенныеФайлы.Количество() > 0 Тогда
		...
		СсылкаНаДрайвер = ПомещенныеФайлы[0].Адрес;
		ОбновитьСостояниеЭлементов();
	КонецЕсли;
...
Процедура ВыгрузитьМакетДрайвера()
	...
	НачатьПолучениеФайлаССервера(СсылкаНаФайл, Объект.ИмяМакетаДрайвера + ".zip"); 
...
Процедура ВыгрузитьФайлДрайвера()
	...
	НачатьПолучениеФайлаССервера(СсылкаНаФайлВИБ, Объект.ИмяФайлаДрайвера); 
