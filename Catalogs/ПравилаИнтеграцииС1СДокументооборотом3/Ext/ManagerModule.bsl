...
//     * Таблица - Строка
//
Функция ПолучитьРеквизитыОбъектаДО(ТипОбъектаДО, ВидДокументаДОID = Неопределено, НастройкиВида = Неопределено,
		ПолучитьВсе = Ложь, ДопЗапрос = Неопределено, ОтветНаДопЗапрос = Неопределено) Экспорт
...
Процедура ПроверитьПравило(ПравилоИнтеграции) Экспорт
	...
	РеквизитыТекущегоВидаДокументаДО = Справочники.ПравилаИнтеграцииС1СДокументооборотом3.ПолучитьРеквизитыОбъектаДО(
		ПравилоИнтеграции.ТипОбъектаДО,
		ПравилоИнтеграции.ВидДокументаДОID);
	
	ВсеРеквизитыОбъектаДО = Справочники.ПравилаИнтеграцииС1СДокументооборотом3.ПолучитьРеквизитыОбъектаДО(
		ПравилоИнтеграции.ТипОбъектаДО,
		ПравилоИнтеграции.ВидДокументаДОID,,
		Истина);
...
Функция СоздатьСвязанноеПравилоИнтеграции(Сообщение) Экспорт
	...
	Попытка
		...
		ПравилоОбъект = Справочники.ПравилаИнтеграцииС1СДокументооборотом3.СоздатьЭлемент();
...
Процедура ЗаполнитьРеквизитПоПравилу(ПравилоРеквизита, Реквизит, ОбъектИС, РеквизитИСМетаданные, ОбъектXDTO,
		ЯвляетсяЗаявкойНаОплату, НомерСтроки = Неопределено, ТекущаяСтрока = Неопределено)
...
Процедура ЗаполнитьСтрокиТаблицыПоПравилу(ОбъектИС, РеквизитыОбъектаИС, Таблица, ПравилаЗаполненияСтрок, КолВоСтрок,
		ОбъектXDTO, ЯвляетсяЗаявкойНаОплату, ТаблицаИсточника = Неопределено)
	...
	Для НомерСтроки = 0 По КолВоСтрок - 1 Цикл
		...
		Для Каждого ПравилоРеквизита Из ПравилаЗаполненияСтрок Цикл
			ЗаполнитьРеквизитПоПравилу(
				...
				ОбъектИС,
				РеквизитМетаданные(ОбъектИС, РеквизитыОбъектаИС, ПравилоРеквизита.ИмяРеквизитаОбъектаИС, ПравилоРеквизита.Таблица),
				ОбъектXDTO,
				ЯвляетсяЗаявкойНаОплату,
				НомерСтроки,
...
Процедура ЗаполнитьТаблицуПоПравилу(ПравилоРеквизита, Таблица, ОбъектИС, РеквизитыОбъектаИС, ОбъектXDTO,
		ЯвляетсяЗаявкойНаОплату, ПравилоИнтеграции)
	...
	Иначе
		...
		Для Каждого Реквизит Из РеквизитыОбъектаИС.НайтиСтроки(ОтборРеквизитыТаблицы) Цикл
			...
			НайденныеПравилаРеквизитов = ПравилоИнтеграции.ПравилаЗаполненияРеквизитовИС.НайтиСтроки(ПараметрыОтбора);
			Если НайденныеПравилаРеквизитов.Количество() = 0 Тогда
				Продолжить;
			КонецЕсли;
