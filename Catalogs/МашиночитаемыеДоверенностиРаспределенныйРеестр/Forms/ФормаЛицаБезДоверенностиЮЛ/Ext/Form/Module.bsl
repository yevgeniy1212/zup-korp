...
Функция СохранениеВозможно()
	...
	ИначеЕсли НЕ РегламентированнаяОтчетностьВызовСервера.ИННСоответствуетТребованиямНаСервере(ЛицоБезДовЮЛ_ИНН,
		Ложь) Тогда
	...
	ИначеЕсли СтрДлина(СокрЛП(ЛицоБезДовЮЛ_КПП)) <> 9 Тогда
		ОбщегоНазначенияКлиент.СообщитьПользователю(НСтр("ru = 'Указан некорректный КПП организации.'"),,
			"ЛицоБезДовЮЛ_КПП",, Отказ);
	КонецЕсли;
	
	Если ЗначениеЗаполнено(ЛицоБезДовЮЛ_ОГРН) И СтрДлина(СокрЛП(ЛицоБезДовЮЛ_ОГРН)) <> 13 Тогда
