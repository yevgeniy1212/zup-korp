...
Функция ДанныеИзФайлаОбмена(
	...
	Если СведенияФайла.Количество() = 1 Тогда
		...
		Если ЗначениеЗаполнено(ИмяФайлаВыгрузки) Тогда
			...
			ИначеЕсли ВРег(Лев(ИмяФайлаВыгрузки, 9)) <> "ON_DOVEL_" Тогда
				Возврат Новый Структура("ТекстОшибки", НСтр("ru = 'Неверный тип доверенности'"));
			КонецЕсли;
	...
	Если ЗначениеЗаполнено(ДанныеДоверенности.Организация)
		...
		НашлиДоверенность = НайтиЭлементИБ(Справочники.МашиночитаемыеДоверенностиРаспределенныйРеестр.ПустаяСсылка(), СтрокаПоиска);
...
Функция ВыгрузитьЭлементВФайлОбмена(
	...
	ВыгрузитьФИОУзла(
		ЗаписьXML,
		ДанныеСправочника.ПредставительФЛ_Фамилия,
		ДанныеСправочника.ПредставительФЛ_Имя,
...
Функция ПроверитьВозможностьВыгрузки(ОбъектИлиСсылка) Экспорт
	...
	Если Представитель_ЮридическоеЛицо Тогда
		...
		ИначеЕсли СтрДлина(СокрЛП(Доверенность.ПредставительЮЛ_КПП)) <> 9 Тогда
			Результат.Добавить(Новый Структура("ТекстОшибки, Поле",
				НСтр("ru = 'Указан некорректный КПП организации представителя'"), "Представитель"));
		КонецЕсли;
		
		Если ЗначениеЗаполнено(Доверенность.ПредставительЮЛ_ОГРН) И СтрДлина(СокрЛП(Доверенность.ПредставительЮЛ_ОГРН)) <> 13 Тогда
			Результат.Добавить(Новый Структура("ТекстОшибки, Поле",
	...
	ИначеЕсли ЗначениеЗаполнено(Доверенность.ПредставительФЛ_ОГРН)
		И СтрДлина(СокрЛП(Доверенность.ПредставительФЛ_ОГРН)) <> 15 Тогда
	...
	ИначеЕсли СтрДлина(СокрЛП(Доверенность.ПредставительФЛ_СНИЛС)) <> 14 Тогда
		Результат.Добавить(Новый Структура("ТекстОшибки, Поле",
