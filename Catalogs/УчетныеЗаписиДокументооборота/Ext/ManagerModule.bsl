...
Процедура ЗаполнитьМестоХраненияКлюча(ПараметрыОбновления = Неопределено) Экспорт
	...
	ТекстЗапроса =
		"ВЫБРАТЬ РАЗРЕШЕННЫЕ
		|	УчетныеЗаписиДокументооборота.Ссылка КАК УчетнаяЗаписьСсылка,
		|	УчетныеЗаписиДокументооборота.ЭлектроннаяПодписьВМоделиСервиса КАК ПодписьСервиса
		|ИЗ
		|	Справочник.УчетныеЗаписиДокументооборота КАК УчетныеЗаписиДокументооборота
		|ГДЕ
		|	УчетныеЗаписиДокументооборота.МодельХраненияЗакрытогоКлюча = ЗНАЧЕНИЕ(Перечисление.МодельХраненияЗакрытогоКлюча.ПустаяСсылка)";
	...
	Пока Выборка.Следующий() Цикл
		...
		Если Выборка.ПодписьСервиса Тогда
			УчетнаяЗаписьОбъект.МодельХраненияЗакрытогоКлюча = Перечисления.МодельХраненияЗакрытогоКлюча.ВМоделиСервиса;
		Иначе
			УчетнаяЗаписьОбъект.МодельХраненияЗакрытогоКлюча = Перечисления.МодельХраненияЗакрытогоКлюча.ЛокальныйКлюч;
		КонецЕсли;
