...
Процедура ПроверитьЗначенияИспользуемыеВДокументах(Отказ)
	...
	Запрос.УстановитьПараметр("ПоказательСдельныйЗаработок", ОбщегоНазначения.ПредопределенныйЭлемент("Справочник.ПоказателиРасчетаЗарплаты.СдельныйЗаработок"));
	...
	Пока Выборка.СледующийПоЗначениюПоля("ВидОшибки") Цикл
		...
		Если Выборка.ВидОшибки = "1_Показатели" Или Выборка.ВидОшибки = "1_ВидыВремени" Или Выборка.ВидОшибки = "1_ВидыРабот" Тогда
			...
			КонецЦикла;
		КонецЕсли;
		ОбщегоНазначения.СообщитьПользователю(ТекстОшибки, Ссылка, ПолеОшибки, , Отказ);
	КонецЦикла;
