...
Процедура Отправить(Команда)
	...
	Если (Модифицированность ИЛИ Параметры.Ключ.Пустая()) И НЕ Записать() ИЛИ НЕ ПроверитьЗаполнение()
		ИЛИ НЕ ЗначениеЗаполнено(Объект.ТипПолучателя) Тогда
		Возврат;
	КонецЕсли;
...
Процедура ПривестиПолучателяКТипуНаСервере()
	...
	ТипЦБ = Перечисления.ТипыКонтролирующихОрганов.ЦБ;
	...
	Если ЗначениеЗаполнено(ТипПолучателя) Тогда
		...
		ИначеЕсли ТипПолучателя = ТипФТС Тогда
			МассивТипов.Добавить(Тип("Неопределено"));
			ДопустимыйТип = Новый ОписаниеТипов(МассивТипов);
		ИначеЕсли ТипПолучателя = ТипЦБ Тогда
			МассивТипов.Добавить(Тип("Неопределено"));
			ДопустимыйТип = Новый ОписаниеТипов(МассивТипов);
		Иначе
			Элементы.Получатель.ОграничениеТипа = Новый ОписаниеТипов();
			Объект.Получатель = Неопределено;
			Объект.ТипПолучателя = Перечисления.ТипыКонтролирующихОрганов.ПустаяСсылка();
			Возврат;
		КонецЕсли;
		...
		Элементы.Получатель.КнопкаВыбора = ТипПолучателя <> ТипФСС И ТипПолучателя <> ТипФСРАР И ТипПолучателя <> ТипРПН
			И ТипПолучателя <> ТипФТС И ТипПолучателя <> ТипЦБ;
	Иначе
		Элементы.Получатель.ОграничениеТипа = Новый ОписаниеТипов();
		Объект.Получатель = Неопределено;
	КонецЕсли;
...
Функция ИмяТекущегоТипаПолучателя(ТипПолучателя)
	...
	Если ЗначениеЗаполнено(ТипПолучателя) Тогда
		...
		ИначеЕсли ТипПолучателя = ПредопределенноеЗначение("Перечисление.ТипыКонтролирующихОрганов.ФТС") Тогда
			Возврат "ФТС";
		ИначеЕсли ТипПолучателя = ПредопределенноеЗначение("Перечисление.ТипыКонтролирующихОрганов.ЦБ") Тогда
			Возврат "ЦБ";
		КонецЕсли;
	Иначе
		Возврат "ФНС";
...
Функция ИмпортироватьФайлыНаСервере(ВыбранныеФайлы, ВыполнитьРазделениеФайла, ВыполнятьПроверкуКомплектаУведомлений)
	...
	Если ВыбранныеФайлы.Количество() = 0 Тогда
		ОбщегоНазначения.СообщитьПользователю(
			НСтр("ru = 'Среди выбранных файлов не удалось обнаружить ни одного файла с расширением ""xml"", ""txt"", ""xlsm"", ""xls"", ""pdf"", ""docx"" или ""doc""!'"));
		Возврат Ложь;
	КонецЕсли;
...
Процедура ПривестиПолучателяКТипу(Форма)
	...
	ТипЦБ 		= ПредопределенноеЗначение("Перечисление.ТипыКонтролирующихОрганов.ЦБ");
	...
	Если ЗначениеЗаполнено(ТипПолучателя) Тогда
		...
		ЭтоФССФСРАРРПНФТСИлиЦБ = (ТипПолучателя = ТипФСС ИЛИ ТипПолучателя = ТипФСРАР ИЛИ ТипПолучателя = ТипРПН
			ИЛИ ТипПолучателя = ТипФТС ИЛИ ТипПолучателя = ТипЦБ);
		Элементы.Получатель.КнопкаВыбора 	= НЕ ЭтоФССФСРАРРПНФТСИлиЦБ;
		Элементы.Получатель.КнопкаОчистки 	= НЕ ЭтоФССФСРАРРПНФТСИлиЦБ;
...
Процедура ЗагрузитьЭлектронныеПредставления(МассивФайлов)
	...
	Если НЕ ЗагружаетсяНесколькоФайлов Тогда
		Для Каждого ВыбранныйФайл Из МассивФайлов Цикл
			РасширениеФайла = ОбщегоНазначенияКлиентСервер.РазложитьПолноеИмяФайла(ВыбранныйФайл.Имя).Расширение;
			РасширениеФайла = нрег(РасширениеФайла);
			Если РасширениеФайла <> ".txt" И РасширениеФайла <> ".xml" Тогда
				ЗагружаетсяНесколькоФайлов = Истина;
			КонецЕсли;
		КонецЦикла;
	КонецЕсли;
...
Функция ОпределитьТипФайлаПФРилиБО(ПолноеИмяФайла, ЗагружаетсяНесколькоФайлов)
	...
	// пытаемся определить, является ли файл отчетом Росприроднадзора и если Да, то каким
	Если Объект.ТипПолучателя = Перечисления.ТипыКонтролирующихОрганов.РПН Тогда
		...
		Если СтрНайти(ИмяФайлаПриведенное, "2TP_") > 0 ИЛИ СтрНайти(ИмяФайлаПриведенное, "DPNVOS") > 0
			ИЛИ СтрНайти(ИмяФайлаПриведенное, "DKT") > 0 ИЛИ СтрНайти(ИмяФайлаПриведенное, "ECOCHARGE") > 0 Тогда
	...
	// пытаемся определить, является ли файл отчетом ЦБ и если Да, то каким
	Если Объект.ТипПолучателя = Перечисления.ТипыКонтролирующихОрганов.ЦБ Тогда
		Возврат ПредопределенноеЗначение("Перечисление.ТипыФайловБухОтчетности.ОсновнойФайлОтчетности");
	КонецЕсли;
...
Функция ОпределитьТипФайлаПФР(ПолноеИмяФайла)
	...
	ИначеЕсли СвойстваУзла.Получить("УПРУП") <> Неопределено Тогда
		Возврат Перечисления.ТипыФайловОтчетностиПФР.ПачкаИС;
	ИначеЕсли СвойстваУзла.Получить("ЕФС-1") <> Неопределено ИЛИ СвойстваУзла.Получить("ЕФС_1") <> Неопределено Тогда
		Возврат Перечисления.ТипыФайловОтчетностиПФР.ПачкаИС;
	ИначеЕсли СвойстваУзла.Получить("СЗВ-ДСО") <> Неопределено ИЛИ СвойстваУзла.Получить("СЗВ_ДСО") <> Неопределено Тогда
		Возврат Перечисления.ТипыФайловОтчетностиПФР.ПачкаИС;
	ИначеЕсли СвойстваУзла.Получить("АДВ-1") <> Неопределено ИЛИ СвойстваУзла.Получить("АДВ_1") <> Неопределено Тогда
		Возврат Перечисления.ТипыФайловОтчетностиПФР.ПачкаИС;
	ИначеЕсли СвойстваУзла.Получить("АДВ-2") <> Неопределено ИЛИ СвойстваУзла.Получить("АДВ_2") <> Неопределено Тогда
		Возврат Перечисления.ТипыФайловОтчетностиПФР.ПачкаИС;
	ИначеЕсли СвойстваУзла.Получить("АДВ-3") <> Неопределено ИЛИ СвойстваУзла.Получить("АДВ_3") <> Неопределено Тогда
		Возврат Перечисления.ТипыФайловОтчетностиПФР.ПачкаИС;
	Иначе
...
Функция ПроанализироватьФайлВыгрузки(Адрес, КороткоеИмяФайла)
	...
	ИначеЕсли ЭтоФайлВыгрузкиЦБ(ПолноеИмяФайла) Тогда
		ТипПолучателя = Перечисления.ТипыКонтролирующихОрганов.ЦБ;
		РезультатАнализа = ПроанализироватьФайлВыгрузкиЦБ(ПолноеИмяФайла);
	КонецЕсли;
...
Функция ПроанализироватьФайлВыгрузкиФСРАР(ОсновнойФайл)
	...
	СоответствиеПрефиксовВидамОтправляемыхДокументов.Вставить("F4_", Справочники.ВидыОтправляемыхДокументов.ОборотИИспользованиеФармацевтическойСубстанцииСпиртаЭтилового2021);
	СоответствиеПрефиксовВидамОтправляемыхДокументов.Вставить("F5_", Справочники.ВидыОтправляемыхДокументов.ПроизводствоИзготовлениеИОборотСпиртосодержащихЛекарственныхПрепаратов2021);
	СоответствиеПрефиксовВидамОтправляемыхДокументов.Вставить("01_", Справочники.ВидыОтправляемыхДокументов.ПроизводствоИОборотАлкогольнойПродукции2021);
	СоответствиеПрефиксовВидамОтправляемыхДокументов.Вставить("02_", Справочники.ВидыОтправляемыхДокументов.ОборотЭтиловогоСпиртаАлкогольнойИСпиртосодержащейПродукции2021);
	СоответствиеПрефиксовВидамОтправляемыхДокументов.Вставить("03_", Справочники.ВидыОтправляемыхДокументов.ПоставкиЭтиловогоСпиртаАлкогольнойИСпиртосодержащейПродукции2021);
	СоответствиеПрефиксовВидамОтправляемыхДокументов.Вставить("04_", Справочники.ВидыОтправляемыхДокументов.ЗакупкиЭтиловогоСпиртаАлкогольнойИСпиртосодержащейПродукции2021);
	СоответствиеПрефиксовВидамОтправляемыхДокументов.Вставить("05_", Справочники.ВидыОтправляемыхДокументов.ПеревозкиЭтиловогоСпиртаАлкогольнойИСпиртосодержащейПродукции2021);
	СоответствиеПрефиксовВидамОтправляемыхДокументов.Вставить("06_", Справочники.ВидыОтправляемыхДокументов.ИспользованиеПроизводственныхМощностей2021);
	СоответствиеПрефиксовВидамОтправляемыхДокументов.Вставить("07_", Справочники.ВидыОтправляемыхДокументов.РозничнаяПродажаАлкогольнойИСпиртосодержащейПродукции2021);
	СоответствиеПрефиксовВидамОтправляемыхДокументов.Вставить("08_", Справочники.ВидыОтправляемыхДокументов.РозничнаяПродажаПиваИПивныхНапитков2021);
	...
	СоответствиеПрефиксовВидамОтправляемыхДокументов.Вставить("10_", Справочники.ВидыОтправляемыхДокументов.Виноградная1а2022);
	СоответствиеПрефиксовВидамОтправляемыхДокументов.Вставить("11_", Справочники.ВидыОтправляемыхДокументов.Виноградная1б2022);
...
Функция ЭтоФайлВыгрузкиФСРАР(ОсновнойФайл)
	...
	ВозможныеПрефиксы.Добавить("F4_");
	ВозможныеПрефиксы.Добавить("F5_");
	ВозможныеПрефиксы.Добавить("01_");
	ВозможныеПрефиксы.Добавить("02_");
	ВозможныеПрефиксы.Добавить("03_");
	ВозможныеПрефиксы.Добавить("04_");
	ВозможныеПрефиксы.Добавить("05_");
	ВозможныеПрефиксы.Добавить("06_");
	ВозможныеПрефиксы.Добавить("07_");
	ВозможныеПрефиксы.Добавить("08_");
...
Функция ПроанализироватьФайлВыгрузкиРПН(ОсновнойФайл)
	...
	// загружаем XML в дерево
	ФайлВыгрузки = Новый ЧтениеТекста(ОсновнойФайл, "utf-8");
	ТекстВыгрузки = ФайлВыгрузки.Прочитать();
	ФайлВыгрузки.Закрыть();
	ТекстВыгрузки = СтрЗаменить(ТекстВыгрузки, " type=""integer"">", ">"); // обход проблемы ОбъектXDTO
	ОсновнойФайлВременный = ПолучитьИмяВременногоФайла("xml");
	ФайлВыгрузки = Новый ЗаписьТекста(ОсновнойФайлВременный, "utf-8");
	ФайлВыгрузки.Записать(ТекстВыгрузки);
	ФайлВыгрузки.Закрыть();
	ДеревоXML = ДеревоXMLИзФайла(ОсновнойФайлВременный);
	ОперацииСФайламиЭДКО.УдалитьВременныйФайл(ОсновнойФайлВременный);
	...
	Если СтрНайти(Префикс, "рсч_") > 0 Тогда
		...
		КонецЕсли;
	ИначеЕсли СтрНайти(Префикс, "dpnvos") > 0 Тогда
		ВидОтправляемогоДокумента = Справочники.ВидыОтправляемыхДокументов.ДекларацияПлатаНВОС;
	...
	ИначеЕсли СтрНайти(Префикс, "2тпо_") > 0 ИЛИ СтрНайти(Префикс, "2tp_waste") > 0 Тогда
		ВидОтправляемогоДокумента = Справочники.ВидыОтправляемыхДокументов.Форма2ТПОтходы;
	ИначеЕсли СтрНайти(Префикс, "2тпа_") > 0 ИЛИ СтрНайти(Префикс, "2tp_air") > 0 Тогда
		ВидОтправляемогоДокумента = Справочники.ВидыОтправляемыхДокументов.Форма2ТПВоздух;
	ИначеЕсли СтрНайти(Префикс, "2тпр_") > 0 ИЛИ СтрНайти(Префикс, "2tp_recl") > 0 Тогда
		ВидОтправляемогоДокумента = Справочники.ВидыОтправляемыхДокументов.Форма2ТПРекультивация;
	ИначеЕсли СтрНайти(Префикс, "dkt") > 0 Тогда
		ВидОтправляемогоДокумента = Справочники.ВидыОтправляемыхДокументов.КоличествоТоваровПодлежащихУтилизации;
	ИначеЕсли СтрНайти(Префикс, "ecocharge") > 0 Тогда
		ВидОтправляемогоДокумента = Справочники.ВидыОтправляемыхДокументов.РасчетЭкологическийСбор;
	...
	Если ДеревоXML.Свойства().Получить("ORG_INFO") <> Неопределено
		И ТипЗнч(ДеревоXML.ORG_INFO) = Тип("СписокXDTO") Тогда
...
Функция ОпределитьПериодРПН(ДеревоXML)
	...
	ИначеЕсли ДеревоXML.Свойства().Получить("data") <> Неопределено
		И ДеревоXML.data.Свойства().Получить("service_request") <> Неопределено 
		И ДеревоXML.data.service_request.Свойства().Получить("report_year") <> Неопределено Тогда
		
		ОтчетныйГод = ДеревоXML.data.service_request.report_year;
		Если ТипЗнч(ОтчетныйГод) <> Тип("Число") Тогда
			ОтчетныйГод = СокрЛП(ОтчетныйГод);
		КонецЕсли;
...
Функция ОпределитьОрганизациюРПН(ДеревоXML)
	...
	ИначеЕсли ДеревоXML.Свойства().Получить("data") <> Неопределено
		И ДеревоXML.data.Свойства().Получить("service_request") <> Неопределено
		И ДеревоXML.data.service_request.Свойства().Получить("applicant_information") <> Неопределено Тогда
		
		УзелОрганизация = ДеревоXML.data.service_request.applicant_information;
		
		Если УзелОрганизация.Свойства().Получить("inn") <> Неопределено Тогда
			_ИНН = СокрЛП(УзелОрганизация.inn);
		КонецЕсли;
		Если УзелОрганизация.Свойства().Получить("kpp") <> Неопределено Тогда
			_КПП = СокрЛП(УзелОрганизация.kpp);
		КонецЕсли;
...
Функция ЭтоФайлВыгрузкиРПН(ОсновнойФайл)
	...
	Возврат (СтрНайти(Префикс, "рсч_") > 0
		...
		ИЛИ СтрНайти(Префикс, "2tp_") > 0
		ИЛИ СтрНайти(Префикс, "dpnvos") > 0
		ИЛИ СтрНайти(Префикс, "dkt") > 0
		ИЛИ СтрНайти(Префикс, "ecocharge") > 0);
...
&НаСервере
Функция ЭтоФайлВыгрузкиЦБ(ОсновнойФайл)
	
	КороткоеИмяФайла = КороткоеИмяФайлаПоПолному(ОсновнойФайл);
	КороткоеИмяФайла = ВРег(КороткоеИмяФайла);
	Расширение = нрег(РасширениеФайла(ОсновнойФайл));
	Расширение = нрег(Расширение);
	Если Расширение <> "xml" И Расширение <> "xlsm" И Расширение <> "xls" И Расширение <> "pdf" И Расширение <> "docx"
		И Расширение <> "doc" Тогда
	...
	Если Расширение = "xml" Тогда
		ФайлВыгрузки = Новый ЧтениеТекста(ОсновнойФайл, "windows-1251");
		ТекстВыгрузки = ФайлВыгрузки.Прочитать();
		Возврат СтрНайти(ТекстВыгрузки, "<reportTitle") > 0
			И (СтрНайти(ТекстВыгрузки, "<accountingInfoSection") > 0
			ИЛИ СтрНайти(ТекстВыгрузки, "<fellowSection") > 0
			ИЛИ СтрНайти(ТекстВыгрузки, "<outwardSection") > 0
			ИЛИ СтрНайти(ТекстВыгрузки, "<inwardSection") > 0);
		
	Иначе // Расширение = "xlsm" ИЛИ Расширение = "xls" ИЛИ Расширение = "pdf" ИЛИ Расширение = "docx" ИЛИ Расширение <> "doc"
		Возврат Истина;
	КонецЕсли;
	...
	Если Расширение = "xml" Тогда
		...
		Иначе 
			...
			Если НЕ ЗначениеЗаполнено(ГодПериодОтч) ИЛИ НЕ СтроковыеФункцииКлиентСервер.ТолькоЦифрыВСтроке(ГодПериодОтч) ИЛИ (ПериодОтч <> Неопределено И НЕ ЗначениеЗаполнено(ПериодОтч)) Тогда
				...
				Иначе
					ДатаУвед = ПоказателиВыгрузки["ДатаУвед"];
					Если ЗначениеЗаполнено(ДатаУвед) И СтроковыеФункцииКлиентСервер.ТолькоЦифрыВСтроке(Сред(ДатаУвед, 1, 2))
						И СтроковыеФункцииКлиентСервер.ТолькоЦифрыВСтроке(Сред(ДатаУвед, 4, 2))
						И СтроковыеФункцииКлиентСервер.ТолькоЦифрыВСтроке(Сред(ДатаУвед, 7)) Тогда
						
						ПериодичностьВыгрузки = Перечисления.Периодичность.День;
						ДатаНачалаПериодаОтчетаВыгрузки = Дата(Число(Сред(ДатаУвед, 7)), Число(Сред(ДатаУвед, 4, 2)), Число(Сред(ДатаУвед, 1, 2)));
						ДатаНачалаПериодаОтчетаВыгрузки = НачалоДня(ДатаНачалаПериодаОтчетаВыгрузки);
						ДатаОкончанияПериодаОтчетаВыгрузки = КонецДня(ДатаНачалаПериодаОтчетаВыгрузки);
...
Функция ПроанализироватьФайлВыгрузкиФСС(ПолноеИмяФайла, ПреобразуемыйАдрес)
	...
	ЭтоРеестрСтимулирующихВыплатМедицинскимИСоциальнымРаботникам = (Лев(КороткоеИмяФайла, 2) = "C_");
	...
	ИначеЕсли ЭтоРеестрСтимулирующихВыплатМедицинскимИСоциальнымРаботникам Тогда
		РезультатАнализа = ПроанализироватьФайлВыгрузкиРеестраСтимулирующихВыплатМедицинскимИСоциальнымРаботникам(
			ПолноеИмяФайла);
	Иначе
...
&НаСервере
Функция ПроанализироватьФайлВыгрузкиРеестраСтимулирующихВыплатМедицинскимИСоциальнымРаботникам(ПолноеИмяФайла)
	...
	// определение вида отчета
	ПараметрыИмпорта.Вставить("ВидОтчета",
		Справочники.ВидыОтправляемыхДокументов.РеестрСтимулирующихВыплатМедицинскимИСоциальнымРаботникам);
	
	// определение организации
	ОрганизацияВыгрузки = Неопределено;
	РегНомФСС = ?(ПоказателиВыгрузки["emplRegnum"] = Неопределено, "", ПоказателиВыгрузки["emplRegnum"]);
	Если ЗначениеЗаполнено(РегНомФСС) Тогда
		ОрганизацияВыгрузки = ОпределитьОрганизациюПоКодуФСС(РегНомФСС);
	КонецЕсли;
	
	Если НЕ ЗначениеЗаполнено(ОрганизацияВыгрузки) Тогда
		ИНН = ?(ПоказателиВыгрузки["emplInn"] = Неопределено, "", ПоказателиВыгрузки["emplInn"]);
		КПП = ?(ПоказателиВыгрузки["emplKpp"] = Неопределено, "", ПоказателиВыгрузки["emplKpp"]);
		Если ЗначениеЗаполнено(ИНН) ИЛИ ЗначениеЗаполнено(КПП) Тогда
			Если ЗначениеЗаполнено(ИНН) И СтрДлина(ИНН) = 12 И Лев(ИНН, 2) = "00" Тогда
				ИНН = Сред(ИНН, 3);
			КонецЕсли;
			ОрганизацияВыгрузки = ОпределитьОрганизациюПоИННиКПП(ИНН, КПП);
		КонецЕсли;
	...
	// определение органа ФСС
	КодПодчиненности = ПоказателиВыгрузки["emplParentNum"];
	Если ЗначениеЗаполнено(КодПодчиненности) Тогда
		ПараметрыИмпорта.Вставить("Получатель", Лев(КодПодчиненности, 4));
	КонецЕсли;
	
	// определение периода
	ДатаНачалаПериодаОтчетаВыгрузки = ПоказателиВыгрузки["dt1"];
	ДатаОкончанияПериодаОтчетаВыгрузки = ПоказателиВыгрузки["dt2"];
	Если ЗначениеЗаполнено(ДатаНачалаПериодаОтчетаВыгрузки) И ЗначениеЗаполнено(ДатаОкончанияПериодаОтчетаВыгрузки) Тогда
		ДатаНачалаПериодаОтчетаВыгрузки = КонтекстЭДОСервер().ДатаВремяИзСтрокиXML(ДатаНачалаПериодаОтчетаВыгрузки);
		ДатаОкончанияПериодаОтчетаВыгрузки = КонтекстЭДОСервер().ДатаВремяИзСтрокиXML(ДатаОкончанияПериодаОтчетаВыгрузки);
		
		ПараметрыИмпорта.Вставить("Периодичность", Перечисления.Периодичность.День);
		ПараметрыИмпорта.Вставить("ДатаНачалаПериода", ДатаНачалаПериодаОтчетаВыгрузки);
		ПараметрыИмпорта.Вставить("ДатаОкончанияПериода", ДатаОкончанияПериодаОтчетаВыгрузки);
		
	ИначеЕсли ЗначениеЗаполнено(ДатаНачалаПериодаОтчетаВыгрузки) Тогда
		ДатаНачалаПериодаОтчетаВыгрузки = КонтекстЭДОСервер().ДатаВремяИзСтрокиXML(ДатаНачалаПериодаОтчетаВыгрузки);
		ДатаНачалаПериодаОтчетаВыгрузки = НачалоМесяца(ДатаНачалаПериодаОтчетаВыгрузки);
		ДатаОкончанияПериодаОтчетаВыгрузки = КонецМесяца(ДатаНачалаПериодаОтчетаВыгрузки);
		
		ПараметрыИмпорта.Вставить("Периодичность", Перечисления.Периодичность.Месяц);
		ПараметрыИмпорта.Вставить("ДатаНачалаПериода", ДатаНачалаПериодаОтчетаВыгрузки);
		ПараметрыИмпорта.Вставить("ДатаОкончанияПериода", ДатаОкончанияПериодаОтчетаВыгрузки);
	КонецЕсли;
...
&НаСервере
Функция ПроанализироватьФайлВыгрузкиЦБ(ПолноеИмяФайла)
	...
	Если Расширение <> "xml" Тогда
		Если Расширение = "pdf" ИЛИ Расширение = "doc" ИЛИ Расширение = "docx"
			ИЛИ СтрНайти(КороткоеИмяФайла, "1-ПИ") > 0 ИЛИ СтрНайти(КороткоеИмяФайла, "1ПИ") > 0
			ИЛИ СтрНайти(КороткоеИмяФайла, "1PI") > 0 ИЛИ СтрНайти(КороткоеИмяФайла, "1-PI") > 0
			ИЛИ СтрНайти(КороткоеИмяФайла, "ПИ-1") > 0 ИЛИ СтрНайти(КороткоеИмяФайла, "ПИ1") > 0
			ИЛИ СтрНайти(КороткоеИмяФайла, "PI-1") > 0 ИЛИ СтрНайти(КороткоеИмяФайла, "PI1") > 0 Тогда
			Объект.ВидОтчета = Справочники.ВидыОтправляемыхДокументов.Форма1ПИ;
		ИначеЕсли СтрНайти(КороткоеИмяФайла, "1-АРЕНДА") > 0 ИЛИ СтрНайти(КороткоеИмяФайла, "1АРЕНДА") > 0
			ИЛИ СтрНайти(КороткоеИмяФайла, "1-ARENDA") > 0 ИЛИ СтрНайти(КороткоеИмяФайла, "1ARENDA") > 0
			ИЛИ СтрНайти(КороткоеИмяФайла, "АРЕНДА-1") > 0 ИЛИ СтрНайти(КороткоеИмяФайла, "АРЕНДА1") > 0
			ИЛИ СтрНайти(КороткоеИмяФайла, "ARENDA-1") > 0 ИЛИ СтрНайти(КороткоеИмяФайла, "ARENDA1") > 0 Тогда
			Объект.ВидОтчета = Справочники.ВидыОтправляемыхДокументов.Форма1АРЕНДА;
		ИначеЕсли СтрНайти(КороткоеИмяФайла, "1-ПОЕЗДКИ") > 0 ИЛИ СтрНайти(КороткоеИмяФайла, "1ПОЕЗДКИ") > 0
			ИЛИ СтрНайти(КороткоеИмяФайла, "1-POEZDKI") > 0 ИЛИ СтрНайти(КороткоеИмяФайла, "1POEZDKI") > 0
			ИЛИ СтрНайти(КороткоеИмяФайла, "ПОЕЗДКИ-1") > 0 ИЛИ СтрНайти(КороткоеИмяФайла, "ПОЕЗДКИ1") > 0
			ИЛИ СтрНайти(КороткоеИмяФайла, "POEZDKI-1") > 0 ИЛИ СтрНайти(КороткоеИмяФайла, "POEZDKI1") > 0 Тогда
			Объект.ВидОтчета = Справочники.ВидыОтправляемыхДокументов.Форма1ПОЕЗДКИ;
		ИначеЕсли СтрНайти(КороткоеИмяФайла, "1-РОУМИНГ") > 0 ИЛИ СтрНайти(КороткоеИмяФайла, "1РОУМИНГ") > 0
			ИЛИ СтрНайти(КороткоеИмяФайла, "1-ROUMING") > 0 ИЛИ СтрНайти(КороткоеИмяФайла, "1ROUMING") > 0
			ИЛИ СтрНайти(КороткоеИмяФайла, "1-ROAMING") > 0 ИЛИ СтрНайти(КороткоеИмяФайла, "1ROAMING") > 0
			ИЛИ СтрНайти(КороткоеИмяФайла, "РОУМИНГ-1") > 0 ИЛИ СтрНайти(КороткоеИмяФайла, "РОУМИНГ1") > 0
			ИЛИ СтрНайти(КороткоеИмяФайла, "ROUMING-1") > 0 ИЛИ СтрНайти(КороткоеИмяФайла, "ROUMING1") > 0
			ИЛИ СтрНайти(КороткоеИмяФайла, "1-ROAMING") > 0 ИЛИ СтрНайти(КороткоеИмяФайла, "1ROAMING") > 0 
			ИЛИ СтрНайти(КороткоеИмяФайла, "ROAMING-1") > 0 ИЛИ СтрНайти(КороткоеИмяФайла, "ROAMING1") > 0 Тогда
			Объект.ВидОтчета = Справочники.ВидыОтправляемыхДокументов.Форма1РОУМИНГ;
		ИначеЕсли СтрНайти(КороткоеИмяФайла, "1-ТРАНСПОРТ") > 0 ИЛИ СтрНайти(КороткоеИмяФайла, "1ТРАНСПОРТ") > 0
			ИЛИ СтрНайти(КороткоеИмяФайла, "1-TRANSPORT") > 0 ИЛИ СтрНайти(КороткоеИмяФайла, "1TRANSPORT") > 0
			ИЛИ СтрНайти(КороткоеИмяФайла, "ТРАНСПОРТ-1") > 0 ИЛИ СтрНайти(КороткоеИмяФайла, "ТРАНСПОРТ1") > 0
			ИЛИ СтрНайти(КороткоеИмяФайла, "TRANSPORT-1") > 0 ИЛИ СтрНайти(КороткоеИмяФайла, "TRANSPORT1") > 0 Тогда
			Объект.ВидОтчета = Справочники.ВидыОтправляемыхДокументов.Форма1ТРАНСПОРТ;
		ИначеЕсли СтрНайти(КороткоеИмяФайла, "2-ТРАНСПОРТ") > 0 ИЛИ СтрНайти(КороткоеИмяФайла, "2ТРАНСПОРТ") > 0
			ИЛИ СтрНайти(КороткоеИмяФайла, "2-TRANSPORT") > 0 ИЛИ СтрНайти(КороткоеИмяФайла, "2TRANSPORT") > 0
			ИЛИ СтрНайти(КороткоеИмяФайла, "ТРАНСПОРТ-2") > 0 ИЛИ СтрНайти(КороткоеИмяФайла, "ТРАНСПОРТ2") > 0
			ИЛИ СтрНайти(КороткоеИмяФайла, "TRANSPORT-2") > 0 ИЛИ СтрНайти(КороткоеИмяФайла, "TRANSPORT2") > 0 Тогда
			Объект.ВидОтчета = Справочники.ВидыОтправляемыхДокументов.Форма2ТРАНСПОРТ;
		ИначеЕсли СтрНайти(КороткоеИмяФайла, "3-ТРАНСПОРТ") > 0 ИЛИ СтрНайти(КороткоеИмяФайла, "3ТРАНСПОРТ") > 0
			ИЛИ СтрНайти(КороткоеИмяФайла, "3-TRANSPORT") > 0 ИЛИ СтрНайти(КороткоеИмяФайла, "3TRANSPORT") > 0
			ИЛИ СтрНайти(КороткоеИмяФайла, "ТРАНСПОРТ-3") > 0 ИЛИ СтрНайти(КороткоеИмяФайла, "ТРАНСПОРТ3") > 0
			ИЛИ СтрНайти(КороткоеИмяФайла, "TRANSPORT-3") > 0 ИЛИ СтрНайти(КороткоеИмяФайла, "TRANSPORT3") > 0 Тогда
			Объект.ВидОтчета = Справочники.ВидыОтправляемыхДокументов.Форма3ТРАНСПОРТ;
		ИначеЕсли СтрНайти(КороткоеИмяФайла, "1-МЕД") > 0 ИЛИ СтрНайти(КороткоеИмяФайла, "1МЕД") > 0
			ИЛИ СтрНайти(КороткоеИмяФайла, "1-MED") > 0 ИЛИ СтрНайти(КороткоеИмяФайла, "1MED") > 0
			ИЛИ СтрНайти(КороткоеИмяФайла, "МЕД-1") > 0 ИЛИ СтрНайти(КороткоеИмяФайла, "МЕД1") > 0
			ИЛИ СтрНайти(КороткоеИмяФайла, "MED-1") > 0 ИЛИ СтрНайти(КороткоеИмяФайла, "MED1") > 0 Тогда
			Объект.ВидОтчета = Справочники.ВидыОтправляемыхДокументов.Форма1МЕД;
		Иначе // СтрНайти(КороткоеИмяФайла, "1-ИЦБ") > 0 ИЛИ СтрНайти(КороткоеИмяФайла, "1ИЦБ") > 0
			// ИЛИ СтрНайти(КороткоеИмяФайла, "1-ICB") > 0 ИЛИ СтрНайти(КороткоеИмяФайла, "1ICB") > 0
			// ИЛИ СтрНайти(КороткоеИмяФайла, "ИЦБ-1") > 0 ИЛИ СтрНайти(КороткоеИмяФайла, "ИЦБ1") > 0
			// ИЛИ СтрНайти(КороткоеИмяФайла, "ICB-1") > 0 ИЛИ СтрНайти(КороткоеИмяФайла, "ICB1") > 0
			Объект.ВидОтчета = Справочники.ВидыОтправляемыхДокументов.Форма1ИЦБ;
		КонецЕсли;
		Объект.Версия 			= 0;
		Объект.Периодичность 	= Перечисления.Периодичность.Квартал;
		Объект.ТипПолучателя 	= Перечисления.ТипыКонтролирующихОрганов.ЦБ;
		Возврат Истина;
	КонецЕсли;
		...
		Пока ОбъектЧтениеXML.Прочитать() Цикл
			Если ОбъектЧтениеXML.ТипУзла = ТипУзлаXML.НачалоЭлемента Тогда
				...
				Если ОбъектЧтениеXML.Прочитать() Тогда
					Если ОбъектЧтениеXML.ТипУзла = ТипУзлаXML.Текст Тогда
						Если ПоказателиВыгрузки[ИмяУзла] = Неопределено Тогда
							ПоказателиВыгрузки.Вставить(ИмяУзла, ОбъектЧтениеXML.Значение);
						КонецЕсли;
					КонецЕсли;
				КонецЕсли;
			КонецЕсли;
	...
	// определение вида отчета
	ПараметрыИмпорта.Вставить("ТипПолучателя", Перечисления.ТипыКонтролирующихОрганов.ЦБ);
	
	// определение вида отчета
	ПараметрыИмпорта.Вставить("ВидОтчета", Справочники.ВидыОтправляемыхДокументов.Форма1ПИ);
	
	// определение организации
	ОрганизацияВыгрузки = Неопределено;
	ИНН = ?(ПоказателиВыгрузки["innCode"] = Неопределено, "", ПоказателиВыгрузки["innCode"]);
	Если ИНН <> ""  Тогда
	...
	// определение вида документа
	НомКорр = 0;
	ПараметрыИмпорта.Вставить("ВидДокумента", НомКорр);
	
	// определение периода
	ПериодичностьВыгрузки 				= Перечисления.Периодичность.Квартал;
	ДатаНачалаПериодаОтчетаВыгрузки 	= 0;
	ДатаОкончанияПериодаОтчетаВыгрузки 	= 0;
	ОшибкаОпределенияПериода 			= Ложь;

	ГодПериодОтч 	= ПоказателиВыгрузки["Year"];
	ПериодОтч 		= ПоказателиВыгрузки["Quarter"];
	
	Если НЕ ЗначениеЗаполнено(ГодПериодОтч) ИЛИ НЕ СтроковыеФункцииКлиентСервер.ТолькоЦифрыВСтроке(ГодПериодОтч)
		ИЛИ СтрДлина(ГодПериодОтч) > 4 ИЛИ НЕ ЗначениеЗаполнено(ПериодОтч)
		ИЛИ НЕ СтроковыеФункцииКлиентСервер.ТолькоЦифрыВСтроке(ПериодОтч)
		ИЛИ Число(ПериодОтч) < 1 ИЛИ Число(ПериодОтч) > 4 Тогда
		
		ОшибкаОпределенияПериода = Истина;
		
	Иначе
		ДатаНачалаПериодаОтчетаВыгрузки 	= НачалоМесяца(Дата(Число(ГодПериодОтч), Число(ПериодОтч * 3 - 2), 1));
		ДатаОкончанияПериодаОтчетаВыгрузки 	= КонецМесяца(Дата(Число(ГодПериодОтч), Число(ПериодОтч * 3), 1));
	КонецЕсли;
...
Функция ОпределитьВидОтчетаПФР(ДеревоXML, ВидКомплектовОтправляемыхДокументов = Неопределено, КороткоеИмяФайла = "")
	...
	ИначеЕсли СвойстваУзла.Получить("ЕФС-1") <> Неопределено ИЛИ СвойстваУзла.Получить("ЕФС_1") <> Неопределено Тогда
		Если СвойстваУзла.Получить("ЕФС_1") <> Неопределено
			И УзелПачкаВходящихДокументов.ЕФС_1.Свойства().Получить("ОСС") <> Неопределено Тогда
			Возврат Справочники.ВидыОтправляемыхДокументов.ЕФС1ОСС;
		Иначе
			Возврат Справочники.ВидыОтправляемыхДокументов.ЕФС1СЗПГос;
		КонецЕсли;
	ИначеЕсли СвойстваУзла.Получить("СЗВ-ДСО") <> Неопределено ИЛИ СвойстваУзла.Получить("СЗВ_ДСО") <> Неопределено Тогда
		Возврат Справочники.ВидыОтправляемыхДокументов.СведенияДляИсчисленияДоплатыКПенсииСЗВ_ДСО;
	ИначеЕсли СвойстваУзла.Получить("АДВ-1") <> Неопределено ИЛИ СвойстваУзла.Получить("АДВ_1") <> Неопределено Тогда
		Возврат Справочники.ВидыОтправляемыхДокументов.АнкетаЗастрахованногоЛица;
	ИначеЕсли СвойстваУзла.Получить("АДВ-2") <> Неопределено ИЛИ СвойстваУзла.Получить("АДВ_2") <> Неопределено Тогда
		Возврат Справочники.ВидыОтправляемыхДокументов.ПачкаДокументовАДВ2;
	ИначеЕсли СвойстваУзла.Получить("АДВ-3") <> Неопределено ИЛИ СвойстваУзла.Получить("АДВ_3") <> Неопределено Тогда
		Возврат Справочники.ВидыОтправляемыхДокументов.ПачкаДокументовАДВ3;
	Иначе
		Возврат Неопределено;
	КонецЕсли;
...
Функция ОпределитьПериодПФР(КороткоеИмяФайла, ДеревоXML)
	...
	Для Каждого СвойствоУзла Из СвойстваУзла Цикл
		...
		Если СвойстваТекущегоУзла.Получить("СЗВ") <> Неопределено Тогда
			
			ДатаОП = '00010101';
			
			УзелБлокСведенийОЗП = ТекУзел.СЗВ;
			Если УзелБлокСведенийОЗП.Свойства().Получить("ЗЛ") <> Неопределено Тогда
				
				СписокУзловСведенийОЗЛ = Новый Массив;
				
				УзлыСведенияОЗП = УзелБлокСведенийОЗП.ЗЛ;
				Если ТипЗнч(УзлыСведенияОЗП) = Тип("СписокXDTO") Тогда
					СписокУзловСведенийОЗЛ = УзлыСведенияОЗП;
				ИначеЕсли ТипЗнч(УзлыСведенияОЗП) = Тип("ОбъектXDTO") Тогда
					СписокУзловСведенийОЗЛ.Добавить(УзлыСведенияОЗП);
				КонецЕсли;
				
				Для Каждого УзелСведенияОЗП Из СписокУзловСведенийОЗЛ Цикл
					Если УзелСведенияОЗП.Свойства().Получить("СЗПГос") <> Неопределено Тогда
						
						СписокУзловСведенийСЗПГос= Новый Массив;
						
						УзлыСведенияСЗПГос = УзелСведенияОЗП.СЗПГос;
						Если ТипЗнч(УзлыСведенияСЗПГос) = Тип("СписокXDTO") Тогда
							СписокУзловСведенийСЗПГос = УзлыСведенияСЗПГос;
						ИначеЕсли ТипЗнч(УзлыСведенияСЗПГос) = Тип("ОбъектXDTO") Тогда
							СписокУзловСведенийСЗПГос.Добавить(УзлыСведенияСЗПГос);
						КонецЕсли;
						
						Для Каждого УзелСведенийСЗПГос Из СписокУзловСведенийСЗПГос Цикл
							Если УзелСведенийСЗПГос.Свойства().Получить("ОтчетныйПериод") <> Неопределено Тогда
								ГодОП   = Число(" " + УзелСведенийСЗПГос.ОтчетныйПериод.Год);
								МесяцОП = Число(" " + УзелСведенийСЗПГос.ОтчетныйПериод.Месяц);
								ДатаОП = Макс(ДатаОП, Дата(ГодОП, МесяцОП, 1));
							КонецЕсли;
						КонецЦикла;
			...
			Если ДатаОП <> '00010101' Тогда
				ДатаВОтчетномПериоде = ДатаОП;
			Иначе
				ДатаВОтчетномПериоде = ДобавитьМесяц(ТекущаяДатаСеанса(), -1);
			КонецЕсли;
			
			ГодОтчетногоПериода   = Год(ДатаВОтчетномПериоде);
			МесяцОтчетногоПериода = Месяц(ДатаВОтчетномПериоде);
			
			Результат.Вставить("ДатаНачалаПериода",НачалоМесяца(Дата(ГодОтчетногоПериода, МесяцОтчетногоПериода, 1)));
			Результат.Вставить("ДатаОкончанияПериода", КонецМесяца(Дата(ГодОтчетногоПериода, МесяцОтчетногоПериода, 1)));
			Результат.Вставить("Периодичность", Перечисления.Периодичность.Месяц);
			
		КонецЕсли;
		
		Если СвойстваТекущегоУзла.Получить("ОСС") <> Неопределено Тогда
			УзелБлокОСС = ТекУзел.ОСС;
			Если УзелБлокОСС.Свойства().Получить("Период") <> Неопределено Тогда
				УзелПериод = УзелБлокОСС.Период;
				Если УзелПериод.Свойства().Получить("Код") <> Неопределено
					И УзелПериод.Свойства().Получить("Год") <> Неопределено Тогда
					
					СтрКалендарныйГод = СокрЛП(УзелПериод.Год);
					Если ЗначениеЗаполнено(СтрКалендарныйГод) И СтроковыеФункцииКлиентСервер.ТолькоЦифрыВСтроке(СтрКалендарныйГод) Тогда
						КалендарныйГод = Число(СтрКалендарныйГод);
					Иначе
						Продолжить;
					...
					КодПериода = 0;
					Если УзелПериод.Свойства().Получить("Код") <> Неопределено Тогда
						СтрКодПериода = СокрЛП(УзелПериод.Код);
						Если ЗначениеЗаполнено(СтрКодПериода) И СтроковыеФункцииКлиентСервер.ТолькоЦифрыВСтроке(СтрКодПериода) Тогда
							КодПериода = Число(СтрКодПериода);
						КонецЕсли;
					КонецЕсли;
					
					Если КодПериода = 0 ИЛИ КодПериода = 12 Тогда
						Результат.Вставить("ДатаНачалаПериода", Дата(КалендарныйГод, 1, 1));
						Результат.Вставить("ДатаОкончанияПериода", Дата(КалендарныйГод, 12, 31));
						Результат.Вставить("Периодичность", Перечисления.Периодичность.Год);
					...
					Иначе
						...
						Если НомерКвартала > 0 И НомерКвартала * 3 <= 12 Тогда
							...
							Если НомерКвартала = 2 Тогда
								Результат.Вставить("Периодичность", Перечисления.Периодичность.Полугодие);
							Иначе
								Результат.Вставить("Периодичность", Перечисления.Периодичность.Квартал);
							КонецЕсли;
						КонецЕсли;
		...
		Если (СвойствоУзла.Имя = "АДВ-1" ИЛИ СвойствоУзла.Имя = "АДВ_1"
			ИЛИ СвойствоУзла.Имя = "АДВ-2" ИЛИ СвойствоУзла.Имя = "АДВ_2"
			ИЛИ СвойствоУзла.Имя = "АДВ-3" ИЛИ СвойствоУзла.Имя = "АДВ_3")
			И СвойстваТекущегоУзла.Получить("ДатаЗаполнения") <> Неопределено Тогда
			
			СтрОтчетныйГод = Лев(СокрЛП(ТекУзел.ДатаЗаполнения), 4);
			Если ЗначениеЗаполнено(СтрОтчетныйГод) И СтроковыеФункцииКлиентСервер.ТолькоЦифрыВСтроке(СтрОтчетныйГод) Тогда
				ОтчетныйГод = Число(СтрОтчетныйГод);
			Иначе
				Продолжить;
...
Функция ЭтоФайлВыгрузкиФНС(ОсновнойФайл)
	...
	ИЛИ ВРег(Лев(КороткоеИмяФайла, 3)) = "KO_" И Расширение = "xml"
...
Функция ЭтоФайлВыгрузкиФСС(ОсновнойФайл)
	...
	Если нрег(КороткоеИмяФайла) = "9999999999_9999.xml" 			// 4аФСС
		...
		ИЛИ Лев(КороткоеИмяФайла, 4) = "POVD" 						// подтверждение вида деятельности
		ИЛИ Лев(КороткоеИмяФайла, 18) = "C_9999999999_9999_" Тогда 	// реестр стимулирующих выплат мед. и соц. работникам
...
Процедура ПрорисоватьФайлы(Знач ЕстьФайлыВАрхиве = Ложь)
	...
	Иначе
		...
		Если НЕ (КонтекстЭДОСервер.ЭтоФайлВыгрузкиУведомленияОКонтролируемыхСделках(ОсновнойФайл.ИмяФайла)
			И СвойстваФайлов.Количество() > 1)
			И НЕ (Объект.ТипПолучателя = ПредопределенноеЗначение("Перечисление.ТипыКонтролирующихОрганов.ЦБ")
			И (нрег(РасширениеФайла(ОсновнойФайл.ИмяФайла)) <> "xml")) Тогда
...
Процедура УправлениеФормой(Форма)
	...
	ЗагружаетсяНесколькоФайлов = 
		...
		ИЛИ Объект.ВидОтчета = ПредопределенноеЗначение("Справочник.ВидыОтправляемыхДокументов.ЖалобаФНС")
		ИЛИ Объект.ТипПолучателя = ПредопределенноеЗначение("Перечисление.ТипыКонтролирующихОрганов.ЦБ")
		И Форма.СвойстваФайлов.Количество() >= 1 И нрег(РасширениеФайла(Форма.СвойстваФайлов[0].ИмяФайла)) <> "xml";
	...
	Если Форма.ТипДокумента = ПредопределенноеЗначение("Перечисление.ТипыОтправляемыхДокументов.ЗаявлениеОВвозеТоваров")
		...
		ИЛИ Форма.ТипДокумента = ПредопределенноеЗначение("Перечисление.ТипыОтправляемыхДокументов.ОтчетФСНБР")
		...
		ИЛИ Объект.ВидОтчета = ПредопределенноеЗначение("Справочник.ВидыОтправляемыхДокументов.ПодтверждениеВидаДеятельности")
		ИЛИ Объект.ВидОтчета = ПредопределенноеЗначение("Справочник.ВидыОтправляемыхДокументов.РеестрСтимулирующихВыплатМедицинскимИСоциальнымРаботникам") Тогда
	...
	ИначеЕсли ЗагружаетсяНесколькоФайлов
		...
		ИЛИ ДокументооборотСКОКлиентСервер.ЭтоВидОтправляемогоДокументаРеестраАкцизов(Объект.ВидОтчета)
		ИЛИ Объект.ВидОтчета = ПредопределенноеЗначение("Справочник.ВидыОтправляемыхДокументов.НевозможностьПредоставленияДокументов")
		ИЛИ Объект.ВидОтчета = ПредопределенноеЗначение("Справочник.ВидыОтправляемыхДокументов.ЖалобаФНС")) Тогда
	...
	Элементы.Получатель.Видимость =
		...
		И Объект.ТипПолучателя <> ПредопределенноеЗначение("Перечисление.ТипыКонтролирующихОрганов.ЦБ");
...
Функция ВыбранныеФайлыСНужнымРасширением(ВыбранныеФайлы)
	...
	Для Каждого ВыбранныйФайл Из ВыбранныеФайлы Цикл
		...
		Если РасширениеВыбранногоФайла = ".txt"
			ИЛИ РасширениеВыбранногоФайла = ".xml"
			ИЛИ РасширениеВыбранногоФайла = ".xlsm"
			ИЛИ РасширениеВыбранногоФайла = ".xls"
			ИЛИ РасширениеВыбранногоФайла = ".pdf"
			ИЛИ РасширениеВыбранногоФайла = ".docx"
			ИЛИ РасширениеВыбранногоФайла = ".doc" Тогда
			
			ПриведенноеИмяФайла = ВРЕГ(ВыбранныйФайл.Имя);
			РазрешеныНетекстовыеВложения = (Лев(ПриведенноеИмяФайла, 11) = "ON_UVNPDUS_"
				ИЛИ СтрНайти(ПриведенноеИмяФайла, "2TP_") > 0
				ИЛИ СтрНайти(ПриведенноеИмяФайла, "DPNVOS") > 0
				ИЛИ СтрНайти(ПриведенноеИмяФайла, "DKT") > 0
				ИЛИ СтрНайти(ПриведенноеИмяФайла, "ECOCHARGE") > 0);
	...
	Если РазрешеныНетекстовыеВложения Тогда
		Для каждого ВыбранныйФайл Из ВыбранныеФайлы Цикл
			...
			Если РасширениеВыбранногоФайла <> ".txt"
				И РасширениеВыбранногоФайла <> ".xml"
				И РасширениеВыбранногоФайла <> ".xlsm"
				И РасширениеВыбранногоФайла <> ".xls"
				И РасширениеВыбранногоФайла <> ".pdf"
				И РасширениеВыбранногоФайла <> ".docx"
				И РасширениеВыбранногоФайла <> ".doc" Тогда
