...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Параметры.Свойство("ОтображатьСобытияВнешнихСистем") Тогда
		ОтображатьСобытияВнешнихСистем = Параметры.ОтображатьСобытияВнешнихСистем;
	КонецЕсли;
	
	Если ОтображатьСобытияВнешнихСистем И Параметры.Свойство("АдресТаблицыЗанятостиИзВнешнихСистем") Тогда
		АдресТаблицыЗанятостиИзВнешнихСистем = Параметры.АдресТаблицыЗанятостиИзВнешнихСистем;
	КонецЕсли;
...
Процедура ОбновитьРекомендации()
	...
	ТаблицаРекомендаций = РаботаСРабочимКалендарем.ПолучитьТаблицуРекомендаций(МассивУчастников,
		ОтображаемаяДата, ОтображаемаяДатаКонецПериода, Длительность, ИсключенияЗанятости, АдресТаблицыЗанятостиИзВнешнихСистем);
...
Процедура ПриИзмененииОтображаемойДаты()
	...
	Если ОтображатьСобытияВнешнихСистем Тогда
		ДлительнаяОперация = ДлительнаяОперацияПолученияТаблицыЗанятости();
		Если ДлительнаяОперация = Неопределено Тогда
			Возврат;
		КонецЕсли;
		
		Оповещение = Новый ОписаниеОповещения("ПриИзмененииОтображаемойДатыЗавершение", ЭтотОбъект);
	...
	Иначе
		ПриИзмененииОтображаемойДатыЗавершение();
	КонецЕсли;
...
Процедура ПриИзмененииДатыВыбора()
	...
	Если ОтображатьСобытияВнешнихСистем Тогда
		...
		Оповещение = Новый ОписаниеОповещения("ПриИзмененииДатыВыбораЗавершение", ЭтотОбъект);
		...
		ДлительныеОперацииКлиент.ОжидатьЗавершение(ДлительнаяОперация, Оповещение, ПараметрыОжидания);
	Иначе
		ПриИзмененииДатыВыбораЗавершение();
	КонецЕсли;
	
КонецПроцедуры

&НаКлиенте
Процедура ПриИзмененииОтображаемойДатыЗавершение(Результат = Неопределено, ДополнительныеПараметры = Неопределено) Экспорт
...
&НаКлиенте
Процедура ПриИзмененииДатыВыбораЗавершение(Результат = Неопределено, ДополнительныеПараметры = Неопределено) Экспорт
...
&НаСервере
Функция ДлительнаяОперацияПолученияТаблицыЗанятости()
	
	Возврат СинхронизацияКалендарей.ДлительнаяОперацияПолученияТаблицыЗанятости(УникальныйИдентификатор,
		УчастникиМероприятия(Ложь), НачалоДня(ОтображаемаяДата), КонецДня(ОтображаемаяДата));
