...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("ЗарплатаКадрыКорпоративнаяПодсистемы.ОхранаТруда") Тогда
		...
		МодульОхранаТрудаФормы.УточнитьПараметрыОтбораФормыВыбораСотрудников(ЭтотОбъект);
	КонецЕсли;
	...
	Для каждого ОписаниеПараметра Из КоллекцияИсключаемыхПараметров Цикл
		...
		Для каждого Отборы Из КоллекцииОтборов Цикл
			...
			Для каждого ЭлементОтбора Из Отборы Цикл
				...
				ЭлементНастройки = ИдентификаторыПользовательскихОтборов.Получить(ЭлементОтбора.ИдентификаторПользовательскойНастройки);
				Если ЭлементНастройки <> Неопределено Тогда
					
					Если ЭлементНастройки.ЛевоеЗначение = Новый ПолеКомпоновкиДанных(ОписаниеПараметра.Ключ) Тогда
