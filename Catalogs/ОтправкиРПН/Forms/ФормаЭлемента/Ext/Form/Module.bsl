...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ИначеЕсли ТипЗнч(Объект.ОтчетСсылка) = Тип("ДокументСсылка.УведомлениеОСпецрежимахНалогообложения") Тогда
		ОтчетПредставление = Строка(Объект.ОтчетСсылка.ВидУведомления);
		
	Иначе // электронное представление
		ОтчетПредставление = Объект.ОтчетСсылка.Наименование;
	КонецЕсли;
	
	Если Объект.ВидОтчета = Справочники.ВидыОтправляемыхДокументов.УведомлениеПриродопользователю Тогда
		Элементы.ГруппаПодтверждениеНаПодтверждениеОтправки.Видимость = Ложь;
		Элементы.ФормаВыгрузитьПротокол.Видимость = Ложь;
	КонецЕсли;
