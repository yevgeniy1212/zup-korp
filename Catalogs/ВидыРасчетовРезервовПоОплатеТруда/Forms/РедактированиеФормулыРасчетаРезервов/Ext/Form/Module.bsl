...
&НаКлиенте
Процедура СоздатьПоказатель(Команда)
	
	ОткрытьФорму("Справочник.ПоказателиРасчетаРезервовПоОплатеТруда.Форма.ФормаЭлемента",,ЭтотОбъект.ВладелецФормы);
...
Процедура ОткрытьПоказатель(Команда)
	...
	ОткрытьФорму(ИмяФормыОбъекта(ТекущиеДанные.Ссылка), ПараметрыФормы, ЭтотОбъект.ВладелецФормы);
...
Функция ИмяФормыОбъекта(Ссылка)
	Возврат ОбщегоНазначения.ИмяТаблицыПоСсылке(Ссылка) + ".ФормаОбъекта";
КонецФункции
...
Функция РезультатыРедактирования(Формула, ВидРасчета, НедоступныеПоказатели, ДополнятьРезультаты = Истина)
	...
	ПараметрыВыполненияФормулы = РезервыПоОплатеТрудаРасширенный.ПараметрыВыполненияФормулы(Формула, Истина, НедоступныеПоказатели);
	...
	Иначе
		...
		Если ДополнятьРезультаты Тогда
			Результаты.Вставить("СведенияОПоказателях", РезервыПоОплатеТрудаРасширенный.СведенияОПоказателяхРасчетаРезервов(ПоказателиФормулы));
