...
Процедура ОбновитьСоставСотрудниковГруппы(Команда)
	...
	Иначе
		ПоказатьСоставСотрудниковАвтогруппы(Ложь);
	КонецЕсли;
...
Процедура НастроитьКритерииОтбораЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	Если Результат <> Неопределено Тогда
		...
		ПоказатьСоставСотрудниковАвтогруппы(Истина);
...
&НаСервере
Процедура ПоказатьСоставСотрудниковАвтогруппы(ОчиститьСохраненныйМакет)
	
	Если ОчиститьСохраненныйМакет Тогда
		
		Если ЗначениеЗаполнено(Объект.Ссылка) Тогда
			ЗарплатаКадрыОтчеты.ОчиститьМакетОтчета(Метаданные.Отчеты.ОтчетыПоСотрудникам, Объект.Ссылка.УникальныйИдентификатор());
		КонецЕсли;
	...
	Если НастройкиЗаполнения <> Неопределено Тогда
		...
		СотрудникиГруппы = ГруппыСотрудников.СотрудникиГруппыПоискаПоНастройкам(НастройкиЗаполнения, Объект.Ссылка);
