...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("ЗарплатаКадрыКорпоративнаяПодсистемы.ПодборПерсонала") Тогда
		...
		МодульПодборПерсоналаФормы.ФизическиеЛицаСПохожимиДаннымиПриСозданииНаСервере(Параметры);
	КонецЕсли;
...
Процедура ЗаполнитьДанныеФизическихЛиц(Параметры)
	...
	ПодсистемаПодборПерсоналаСуществует =
		ОбщегоНазначения.ПодсистемаСуществует("ЗарплатаКадрыКорпоративнаяПодсистемы.ПодборПерсонала");
	Если ПодсистемаПодборПерсоналаСуществует И Параметры.Свойство("ДанныеКандидатов") Тогда
			
		МодульПоборПерсоналаФормы = ОбщегоНазначения.ОбщийМодуль("ПодборПерсоналаФормы");
		МодульПоборПерсоналаФормы.ДобавитьРеквизитыФормыПоискаОднофамильцев(ЭтотОбъект);
		МодульПоборПерсоналаФормы.ДополнитьОднофамильцев(Однофамильцы, Параметры.ДанныеКандидатов);
		
		ФизическиеЛицаСКандидатами = Новый Массив(ФизическиеЛица);
		ЛичыеДанныеКандидатов = ОбщегоНазначения.ВыгрузитьКолонку(Параметры.ДанныеКандидатов, "ЛичныеДанныеКандидата",
			Истина);
		ОбщегоНазначенияКлиентСервер.ДополнитьМассив(ФизическиеЛицаСКандидатами, ЛичыеДанныеКандидатов);
		ФизическиеЛица = Новый ФиксированныйМассив(ФизическиеЛицаСКандидатами);
	...
	Для Каждого СтрокаДанных Из Параметры.ДанныеФизическихЛиц Цикл
		...
		Если ПодсистемаПодборПерсоналаСуществует Тогда
			МодульПоборПерсонала = ОбщегоНазначения.ОбщийМодуль("ПодборПерсонала");
			НоваяСтрока.ЛичныеДанныеКандидата = МодульПоборПерсонала.ЛичныеДанныеКандидатаФизическогоЛица(
				СтрокаДанных.ФизическоеЛицо);
		КонецЕсли;
	КонецЦикла;
...
Процедура ВыбратьФизическоеЛицоИЗакрытьФорму()
	...
	РезультатВыбора = ФизическоеЛицо;
