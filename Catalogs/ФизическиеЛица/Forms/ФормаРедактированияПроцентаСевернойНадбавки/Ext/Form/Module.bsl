...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ЗначениеЗаполнено(Параметры.Сотрудник) Тогда
		ФизическоеЛицо = ЗарплатаКадрыРасширенныйПовтИсп.ФизическоеЛицоСотрудника(Параметры.Сотрудник);
	Иначе
...
Процедура ОбработкаПроверкиЗаполненияНаСервере(Отказ, ПроверяемыеРеквизиты)
	...
	Если КоличествоЗаписейОСтаже > 1 Тогда
		...
		Для Каждого СтрокаСтажа Из НаборЗаписейСеверногоСтажа Цикл
			...
			ИначеЕсли СтрокаСтажа.Лет = 0 И СтрокаСтажа.РазмерМесяцев = 0 И СтрокаСтажа.РазмерДней = 0 Тогда
			
				ТекстСообщения = НСтр("ru='Не заполнен стаж'");
	...
	Иначе
		...
		Если ПоследняяЗаписьОСтаже.Лет = 0 И ПоследняяЗаписьОСтаже.РазмерМесяцев = 0 И ПоследняяЗаписьОСтаже.РазмерДней = 0 Тогда
			
			ТекстСообщения = НСтр("ru='Не заполнен стаж'");
			
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
				ТекстСообщения, , "ДнейСтажа", , Отказ);
...
Процедура ЗаполнитьИсториюИзмененияПроцента(ТекущаяЗапись)
	...
	Если Не ЗначениеЗаполнено(Период) Тогда
		ПроцентыСевернойНадбавкиФизическихЛицНаборЗаписей.Удалить(ТекущаяЗапись);
		Возврат;
	КонецЕсли;
	
	ПроцентНадбавки = ТекущаяЗапись.ПроцентСевернойНадбавки; 
	
	ПроцентыСевернойНадбавкиФизическихЛицНаборЗаписей.Сортировать("Период");
	...
	Для каждого Запись Из ПроцентыСевернойНадбавкиФизическихЛицНаборЗаписей Цикл
		...
		Если УдалитьЗапись И Запись <> ТекущаяЗапись Тогда
				...
				Для каждого ЗаписьНовогоНабора Из ПроцентыСевернойНадбавкиФизическихЛицНаборЗаписей Цикл
					...
					Если СтрокиТекущихЗаписей.Количество() = 0
						...
						Если Набор.Количество() = 0
							Или Набор[0].ПроцентСевернойНадбавки <> ЗаписьНовогоНабора.ПроцентСевернойНадбавки
							Или Набор[0].ДействуетДо <> ДействуетДо Тогда
							...
							Иначе
								Запись = Набор[0];
								Если Набор[0].ПроцентСевернойНадбавки = ЗаписьНовогоНабора.ПроцентСевернойНадбавки Тогда
		...
		Если ПараметрыИсчисленияПроцентаСевернойНадбавкиФизическихЛиц.ПроцентСевернойНадбавкиИзменяется
			...
			Если ЗначениеЗаполнено(ДатаОтсчетаСтажа) Тогда
				...
				Иначе
					...
					ДатаОтсчетаТекущегоСтажа = ДатаОтсчетаСтажа - Продолжительность.Дней * 86400;
					Для Каждого УчастокШкалы Из ОписаниеШкалы Цикл
					...
					ДатаОтсчетаТекущегоСтажа = ДобавитьМесяц(ДатаОтсчетаТекущегоСтажа, - РазмерПериода);
